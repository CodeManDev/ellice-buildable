...
public record AccountTypeData(
   ...
   public AccountTypeData(
      ...
      if (type == AccountTypeData.Type.SESSION || sessionAccessToken.isEmpty() && sessionExpiresAt.equals(Instant.EPOCH)) {
         ...
         if (type == AccountTypeData.Type.OFFLINE) {
            if (!isValidOfflineUsername(username)) {
               throw new IllegalArgumentException("Offline username must be 1–16 characters using letters, numbers, or _.");
            }
         ...
         throw new IllegalArgumentException("Only cookie sessions can carry an in-memory Minecraft token");
      }
   }

   public AccountTypeData(UUID uUID, String text, AccountTypeData.Type type, String currentText, Instant instant, Instant currentInstant) {
      this(uUID, text, type, currentText, instant, currentInstant, "", Instant.EPOCH);
   }

   public AccountTypeData(UUID uUID, String text, String currentText, Instant instant, Instant currentInstant) {
      this(uUID, text, AccountTypeData.Type.MICROSOFT, currentText, instant, currentInstant);
   }

   public static AccountTypeData session(UUID uUID, String text, String currentText, Instant instant) {
      if (currentText != null && !currentText.isBlank() && instant != null && instant.isAfter(Instant.now())) {
         Instant currentInstant = Instant.now();
         return new AccountTypeData(uUID, text, AccountTypeData.Type.SESSION, "", currentInstant, currentInstant, currentText, instant);
      } else {
   ...
   public static AccountTypeData offline(String text) {
      ...
      if (!isValidOfflineUsername(currentText)) {
         throw new IllegalArgumentException("Username must be 1–16 characters using letters, numbers, or _.");
      }

      Instant instant = Instant.now();
      return new AccountTypeData(createUUID(currentText), currentText, AccountTypeData.Type.OFFLINE, "", instant, instant);
   }
   ...
   public boolean hasCurrentSession() {
      return this.isSession() && !this.sessionAccessToken.isBlank() && this.sessionExpiresAt.isAfter(Instant.now());
   }
   ...
   public AccountTypeData withUsername(String name) {
      if (this.isOffline()) {
         ...
         if (!isValidOfflineUsername(currentName)) {
            ...
            throw new IllegalStateException("An offline username defines the account identity; add a new account instead");
         } else {
            return this;
         }
      } else {
         return new AccountTypeData(
            this.uuid, name, this.type, this.msRefreshToken, this.addedAt, this.lastUsedAt, this.sessionAccessToken, this.sessionExpiresAt
         );
      }
   }
   ...
   public AccountTypeData withRefreshToken(String text) {
      if (!this.isOffline() && !this.isSession()) {
         return new AccountTypeData(this.uuid, this.username, this.type, text, this.addedAt, this.lastUsedAt);
   ...
   public String toString() {
      return "MinecraftAccount[uuid=" + this.uuid + ", username=" + this.username + ", type=" + this.type + ", credentials=redacted]";
   }
