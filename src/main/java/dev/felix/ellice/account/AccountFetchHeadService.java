...
public final class AccountFetchHeadService {
   ...
   public static CompletableFuture<BufferedImage> fetchHead(UUID uUID) {
      ...
      HttpRequest httpRequest = HttpRequest.newBuilder(URI.create(profileUrl))
         .timeout(Duration.ofSeconds(10L))
         .header("Accept", "application/json")
         .GET()
         .build();
      return client.sendAsync(httpRequest, BodyHandlers.ofString())
         .thenCompose(
            profileResponse -> {
               if (profileResponse.statusCode() != 200) {
                  return CompletableFuture.failedFuture(new IOException("profile lookup failed: HTTP " + profileResponse.statusCode()));
               }

               String skinUrl = createText(profileResponse.body());
               return skinUrl == null
                  ? CompletableFuture.failedFuture(new IOException("no SKIN texture in profile properties"))
                  : client.sendAsync(
                     HttpRequest.newBuilder(URI.create(skinUrl)).timeout(Duration.ofSeconds(10L)).GET().build(), BodyHandlers.ofByteArray()
                  );
            }
         )
         .thenApply(skinResponse -> {
            if (skinResponse.statusCode() != 200) {
               throw new RuntimeException("skin download failed: HTTP " + skinResponse.statusCode());
            }

            try {
               BufferedImage bufferedImage = ImageIO.read(new ByteArrayInputStream((byte[])skinResponse.body()));
   ...
   private static String createText(String text) {
      try {
         ...
         for (JsonElement jsonElement : jsonObject.getAsJsonArray("properties")) {
            ...
            if (currentJsonObject.has("name") && "textures".equals(currentJsonObject.get("name").getAsString())) {
               String currentText = currentJsonObject.get("value").getAsString();
               String nextText = new String(Base64.getDecoder().decode(currentText), StandardCharsets.UTF_8);
