...
public final class AccountNewThreadService {
      ...
      Executors.newCachedThreadPool(
          new ThreadFactory() {
            ...
            public Thread newThread(Runnable runnable) {
              Thread thread =
                  new Thread(runnable, "ellice-msauth-" + this.atomicInteger.incrementAndGet());
  ...
  private final HttpClient client =
      HttpClient.newBuilder()
          ...
          .build();
  private final net.lenni0451.commons.httpclient.HttpClient client2 =
      MinecraftAuth.createHttpClient("ElliceClient Microsoft Login");

  public CompletableFuture<AccountNewThreadService.DeviceCode> requestDeviceCode() {
    String text =
        "client_id=00000000402b5328&scope="
            + createText2("service::user.auth.xboxlive.com::MBI_SSL")
            + "&response_type=device_code";
    HttpRequest httpRequest =
        HttpRequest.newBuilder(URI.create("https://login.live.com/oauth20_connect.srf"))
  ...
  public CompletableFuture<AccountNewThreadService.LoginResult> completeLogin(
      AccountNewThreadService.DeviceCode deviceCode, BooleanSupplier booleanSupplier) {
    return this.createCompletableFuture(deviceCode, booleanSupplier)
        .thenApplyAsync(this::createLoginResult, executor);
  }

  public CompletableFuture<AccountTypeData> completeAuth(
      AccountNewThreadService.DeviceCode deviceCode, BooleanSupplier booleanSupplier) {
    return this.completeLogin(deviceCode, booleanSupplier)
        .thenApply(AccountNewThreadService.LoginResult::account);
  }

  public CompletableFuture<AccountNewThreadService.LoginResult> login(
    ...
    if (accountTypeData.isSession()) {
      return !accountTypeData.hasCurrentSession()
          ...
          : CompletableFuture.completedFuture(
              new AccountNewThreadService.LoginResult(
                  accountTypeData,
                  accountTypeData.sessionAccessToken(),
                  accountTypeData.sessionExpiresAt()));
    } else {
  ...
  public CompletableFuture<AccountNewThreadService.LoginResult> loginWithCookies(
      AccountCodec accountCodec, BooleanSupplier booleanSupplier) {
  ...
  public CompletableFuture<AccountNewThreadService.LoginResult> loginWithCookies(
    ...
    return CompletableFuture.supplyAsync(
        () -> {
          ...
          AccountNewThreadService.LoginResult loginResult =
              AccountToStringService.login(accountCodec, booleanSupplier, consumer);
          MicrosoftOAuthBrowserFlow.checkCancelled(booleanSupplier);
          return loginResult;
        },
        executor);
  ...
  private CompletableFuture<AccountNewThreadService.MsTokens> createCompletableFuture(
      AccountNewThreadService.DeviceCode deviceCode, BooleanSupplier booleanSupplier) {
    CompletableFuture completableFuture = new CompletableFuture();
    Instant instant = Instant.now().plus(deviceCode.expiresIn());
    long size = Math.max(deviceCode.interval().toMillis(), 1000L);
    executor.execute(
        () -> this.updateState(deviceCode, booleanSupplier, instant, size, completableFuture));
    return completableFuture;
  }

  private void updateState(
      AccountNewThreadService.DeviceCode currentDeviceCode,
      BooleanSupplier booleanSupplier,
      Instant instant,
      long offset,
      CompletableFuture<AccountNewThreadService.MsTokens> completableFuture) {
    ...
    try {
      while (Instant.now().isBefore(instant)) {
        if (booleanSupplier.getAsBoolean()) {
          completableFuture.completeExceptionally(
              new AccountCodeService("CANCELLED", "User cancelled"));
          return;
        }
        ...
        String text =
            "grant_type=device_code&client_id=00000000402b5328&device_code="
                + createText2(currentDeviceCode.deviceCode());
        ...
        switch (currentText) {
          ...
          case "expired_token":
            completableFuture.completeExceptionally(
                new AccountCodeService("EXPIRED", "Code expired before login completed"));
            return;
          case "access_denied":
            completableFuture.completeExceptionally(
                new AccountCodeService("DENIED", "User declined the Microsoft login"));
            return;
          default:
            completableFuture.completeExceptionally(
                new AccountCodeService(currentText, createText(jsonObject, currentText)));
            return;
        }
      }
      ...
      completableFuture.completeExceptionally(
          new AccountCodeService("INTERRUPTED", "Login was interrupted"));
    } catch (Exception exception) {
      completableFuture.completeExceptionally(exception);
    }
  }

  private CompletableFuture<AccountNewThreadService.MsTokens> createCompletableFuture2(
      String refreshToken) {
    String requestBody =
        "grant_type=refresh_token&client_id=00000000402b5328&refresh_token="
        ...
        .thenApply(
            response -> {
              if (response.statusCode() == 200) {
                AccountNewThreadService.MsTokens msTokens =
                    createMsTokens(JsonParser.parseString(response.body()).getAsJsonObject());
                return msTokens.refreshToken() == null
                    ? new AccountNewThreadService.MsTokens(
                        msTokens.accessToken(), refreshToken, msTokens.expiresAt())
                    : msTokens;
              } else {
                JsonObject jsonObject = createJsonObject(response.body());
                String errorCode =
                    jsonObject != null && jsonObject.has("error")
                        ? jsonObject.get("error").getAsString()
                        : "MS_REFRESH_HTTP_" + response.statusCode();
                String errorMessage =
                    "invalid_grant".equals(errorCode)
                        ? "This saved login uses the old Microsoft flow or has expired. Link the"
                              + " account again."
                        : (jsonObject != null
                            ? createText(
                                jsonObject, "Microsoft sign-in expired; link the account again.")
  ...
  private AccountNewThreadService.LoginResult createLoginResult(
      AccountNewThreadService.MsTokens msTokens) {
    if (msTokens.refreshToken() != null && !msTokens.refreshToken().isBlank()) {
      try {
        MsaToken msaToken =
            new MsaToken(
                msTokens.expiresAt().toEpochMilli(),
                msTokens.accessToken(),
                msTokens.refreshToken());
        JavaAuthManager javaAuthManager = JavaAuthManager.create(this.client2).login(msaToken);
        MinecraftToken minecraftToken =
            (MinecraftToken) javaAuthManager.getMinecraftToken().getUpToDate();
        MinecraftProfile minecraftProfile =
            (MinecraftProfile) javaAuthManager.getMinecraftProfile().getUpToDate();
        ...
        return new AccountNewThreadService.LoginResult(
            ...
            Instant.ofEpochMilli(minecraftToken.getExpireTimeMs()));
      } catch (MinecraftProfileNotFoundException minecraftProfileNotFoundException) {
        throw new AccountCodeService(
            "NO_MC_LICENSE", "This Microsoft account doesn't own Minecraft: Java Edition.");
      } catch (XblRequestException xblRequestException) {
        throw new AccountCodeService(
            "XBL_" + xblRequestException.getErrorCode(), xblRequestException.getErrorMessage());
        ...
        throw new AccountCodeService(
            "TITLE_AUTH_FAILED",
            text != null && !text.isBlank() ? text : "Microsoft/Xbox authentication failed.");
      }
    } else {
      throw new AccountCodeService(
          "NO_MS_REFRESH_TOKEN", "Microsoft did not return a refresh token. Try signing in again.");
    }
  }
  ...
  private static AccountNewThreadService.MsTokens createMsTokens(JsonObject jsonObject) {
    return new AccountNewThreadService.MsTokens(
        ...
        jsonObject.has("expires_in")
            ? Instant.now().plusSeconds(jsonObject.get("expires_in").getAsLong())
            : Instant.now().plusSeconds(3600L));
  }

  private static AccountCodeService createAccountCodeService(
      String text, HttpResponse<String> httpResponse) {
    return new AccountCodeService(
        ...
        text
            ...
            + " — "
            + createText3((String) httpResponse.body(), 256));
  }
