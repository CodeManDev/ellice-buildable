...
public record ParsedServerEndpoint(String host, int port) {
   ...
   public static ParsedServerEndpoint parse(String text) {
      ...
      if (!currentText.isEmpty() && currentText.length() <= 255 && !currentText.chars().anyMatch(Character::isWhitespace) && !currentText.matches(".*[/\\\\?#@].*")) {
         ...
         try {
            ...
            } else if (currentText.indexOf(58) == currentText.lastIndexOf(58) && currentText.contains(":")) {
            ...
            if (nextText.contains(":")) {
               if (!nextText.matches("[0-9a-fA-F:.]+") || !(InetAddress.getByName(nextText) instanceof Inet6Address)) {
                  throw new IllegalArgumentException();
               }
            } else {
         ...
         throw new IllegalArgumentException("Enter a Minecraft server address, for example play.example.com.");
      }
   }
