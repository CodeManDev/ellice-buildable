...
public final class ServerQueryService {
   ...
   public ServerQueryService.Status query(ParsedServerEndpoint parsedServerEndpoint, InetSocketAddress inetSocketAddress, int value) throws IOException {
      try (Socket socket = new Socket()) {
         Thread thread = Thread.ofVirtual().name("ellice-server-deadline").start(() -> {
            try {
               Thread.sleep(10000L);
               socket.close();
         ...
         try {
            ...
            String text = new String(currentDataInputStream.readNBytes(currentValue), StandardCharsets.UTF_8);
            ...
            try {
               ...
               DataOutputStream nextDataOutputStream = new DataOutputStream(currentByteArrayOutputStream);
         ...
         if (currentJsonObject.has("sample") && currentJsonObject.get("sample").isJsonArray()) {
            for (JsonElement jsonElement : currentJsonObject.getAsJsonArray("sample")) {
               ...
               if (jsonElement.isJsonObject()) {
                  String previousText = clean(createText2(jsonElement.getAsJsonObject(), "name"), 64).strip();
   ...
   static String clean(String text, int value) {
      String currentText = text.replaceAll("(?i)§[0-9a-fk-orx]", "").replaceAll("[\\p{Cntrl}&&[^\\n]]", "").strip();
   ...
   private static JsonObject createJsonObject(JsonObject jsonObject, String text) {
      return jsonObject.has(text) && jsonObject.get(text).isJsonObject() ? jsonObject.getAsJsonObject(text) : new JsonObject();
   }

   private static String createText2(JsonObject jsonObject, String text) {
      return jsonObject.has(text) && jsonObject.get(text).isJsonPrimitive() ? jsonObject.get(text).getAsString() : "";
   }
   ...
   private static DataInputStream createDataInputStream(DataInputStream dataInputStream) throws IOException {
   ...
   private static void updateState(DataOutputStream dataOutputStream, byte[] bytes) throws IOException {
   ...
   public record Status(
      int players, int capacity, long latencyMs, String version, String motd, byte[] icon, List<String> samplePlayers, int protocol
   ) {
      public Status(
         int players, int capacity, long latencyMs, String version, String motd, byte[] icon, List<String> samplePlayers, int protocol
      ) {
      ...
      public Status(int value, int currentValue, long longValue, String text, String currentText, byte[] bytes) {
