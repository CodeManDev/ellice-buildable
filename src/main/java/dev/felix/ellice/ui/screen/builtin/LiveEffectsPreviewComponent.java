...
final class LiveEffectsPreviewComponent extends ScenePctService<LiveEffectsPreviewComponent> {
   ...
   protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      ...
      compositorPushPresentationScale.roundedRect(
         this.cx, this.cy, this.cw, this.ch, 20.0F, mulAlpha(MaterialIsLightService.SURFACE_LOW, this.effectiveOpacity)
      );
      ...
      for (int index = 0; index < 8; index++) {
         compositorPushPresentationScale.roundedRect(
            ...
            10.0F,
            mulAlpha(
               index % 3 == 0 ? MaterialIsLightService.PRIMARY : (index % 3 == 1 ? MaterialIsLightService.TERTIARY : MaterialIsLightService.SECONDARY_CONTAINER),
