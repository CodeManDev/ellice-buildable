...
public final class BuiltinControllerService extends ScenePctService<BuiltinControllerService>
  ...
  public BuiltinControllerService() {
    ...
    this.onScrollEvent(
        item -> this.value3 = Math.clamp(this.value3 + item.floatValue() * 0.06, 0.7, 1.45));
  ...
  private void updateState(float currentValue, float nextValue) {
    ...
    this.value2 = Math.clamp(this.value2 + (nextValue - this.value5) * 0.35, -25.0, 25.0);
  ...
  protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
    if (this.capeRevisionService != null
      ...
      double width = Math.min(0.05, (longValue - this.timestamp) / 1.0E9);
      ...
      if (capeSize != this.capeSizeService
          || !currentStyle.equals(this.renderer2)
