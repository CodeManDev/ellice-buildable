...
final class FONTComponent {
  ...
  FONTComponent(
      ...
      TextFontDirectoryService textFontDirectory,
  ...
  void tick() {
    if (this.sceneComponent42 != null
        && this.count2 != this.textFontDirectoryService.entries().size()) {
      this.updateState3();
    }

    TextFontDirectoryService.FontEntry fontEntry = this.textFontDirectoryService.find(this.text3);
    this.updateState4(fontEntry);
    this.sceneTextService.fontFamily(
        !this.text3.isEmpty()
                && this.compositorPushPresentationScaleService.isFontFamilyReady(this.text3)
            ? this.text3
            : null);
  }
  ...
  private void updateState3() {
    ...
    SceneCornerRadiusService previousSize =
        new SceneCornerRadiusService()
            .size(74.0F, 22.0F)
            .cornerRadius(7.0F)
            ...
            .onClick(
                () -> CompatOpenUriService.openPath(this.textFontDirectoryService.fontDirectory()));
    ...
    LayoutContainerNode sourceSize =
        new LayoutContainerNode()
            .flex(1.0F)
            .size(ScenePctService.pct(100.0F), -1.0F)
            .direction(ScenePctService.Direction.COLUMN)
    ...
    for (int index = 0; index < targetSize; index++) {
      sourceSize.addChild(
          this.createSceneCornerRadiusService(
              (TextFontDirectoryService.FontEntry) items.get(index)));
    }

    currentSize.addChild(sourceSize);
    this.sceneComponent42 =
        ComponentUnmountService.mountCentered(
            this.scenePctService, currentSize, 12.0F, this::close);
  }

  private SceneCornerRadiusService createSceneCornerRadiusService(
      TextFontDirectoryService.FontEntry fontEntry) {
    ...
    SceneTextService sceneText =
        new SceneTextService(fontEntry.display(), 8.2F, enabled ? -1 : -788529153);
    if (fontEntry.kind() == TextFontDirectoryService.Kind.MINECRAFT) {
      sceneText.fontFamily("minecraft");
    } else if (!fontEntry.key().isEmpty()
        && this.compositorPushPresentationScaleService.isFontFamilyReady(fontEntry.key())) {
      sceneText.fontFamily(fontEntry.key());
    }

    currentSize.addChild(sceneText);
    currentSize.addChild(
        new SceneTextService(fontEntry.kind().name().toLowerCase(), 6.5F, 1442840575));
    return currentSize;
  }
  ...
  private void updateState4(TextFontDirectoryService.FontEntry fontEntry) {
    if (fontEntry != null && fontEntry.source() != null && !fontEntry.key().isEmpty()) {
      this.compositorPushPresentationScaleService.requestFontFamily(
          fontEntry.key(), fontEntry.source(), fontEntry.ref());
    }
  }
}
