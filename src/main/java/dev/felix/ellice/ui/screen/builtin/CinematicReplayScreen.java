...
public final class CinematicReplayScreen implements ScreenOperationHandler {
  ...
  public ScenePctService<?> build(ScreenScreenIdService screenScreenId) {
    ...
    this.materialTerrainTooltipsService.onLayout(
        () -> {
          ...
          if (Math.abs(this.value2 - value) > 0.5F || Math.abs(this.value3 - currentValue) > 0.5F) {
            this.value2 = value;
    ...
    this.componentMountService =
        new ComponentMountService().mount(this::createComponentKeyService, screenScreenId.theme());
    this.componentMountService
        .absolute()
  ...
  public void tick(ScreenScreenIdService screenScreenId) {
    ...
    if (cinematicExporter == null) {
      ...
      if (this.tIMELINEComponent != null) {
        this.tIMELINEComponent.state(
            (float) doubleValue, this.value4, this.value5, this.value6, this.value7, this.enabled2);
      }
  ...
  private boolean checkCondition4() {
    ...
    return currentState == CinematicExporter.State.EXPORTING
        || currentState == CinematicExporter.State.FINISHING;
  }

  private void updateState3() {
    if (this.enabled
        && this.implClipsService.clips() != null
        && this.implClipsService.clips().takeDuration() > 0.0) {
      this.implClipsService.trim(
          this.value4,
          Math.max(0.0F, (float) this.implClipsService.clips().takeDuration() - this.value5));
    }
  }
  ...
  private ComponentKeyService<?> createComponentKeyService(ComponentThemeService componentTheme) {
    this.materialTerrainTooltipsService.backgroundColor(
        ThemeCornerData.current().dimBackground() ? 1694498816 : 0);
  ...
  private ComponentKeyService<?> createComponentKeyService2() {
    return ComponentBoxService.row(
            item ->
                item.id("cinematic-replay.header")
                    .size(
                        LayoutOperationHandler.percent(100.0F),
  ...
  private ComponentKeyService<?> createComponentKeyService3() {
    ...
    return ComponentBoxService.panel(
            item ->
                item.id("cinematic-replay.preview-card")
                    .width(
                        this.checkCondition()
                            ? LayoutOperationHandler.percent(100.0F)
                            : LayoutOperationHandler.auto())
            ...
            ComponentBoxService.panel(
                item ->
                    item.id("cinematic-replay.preview")
                        .size(
                            LayoutOperationHandler.percent(100.0F),
                ...
                ComponentBoxService.<SceneTextureService>node(
                        ...
                        item ->
                            item.id("cinematic-replay.image")
                                .size(
                                    LayoutOperationHandler.percent(100.0F),
                                    LayoutOperationHandler.percent(100.0F))
                                .cornerRadius(18.0F)
                ...
                enabled
                    ...
                    : createComponentKeyService11(
                            this.implClipsService.clips().recording()
                                ? "Recording 3D scene…"
                                : "Record a take to preview it here",
                            14.0F,
                            MaterialIsLightService.ON_SURFACE_VARIANT,
                            false)
            ...
            ComponentBoxService.row(
                ...
                createComponentKeyService11(
                        enabled
                            ? createText(this.value6)
                                + "  /  "
                                + createText(this.implClipsService.clips().takeDuration())
                ...
                createComponentKeyService11(
                    enabled ? "24 FPS" : "3D TAKE", 11.0F, MaterialIsLightService.PRIMARY, true)))
        .key("preview-card");
  }
  ...
  private ComponentKeyService<?> createComponentKeyService4() {
    ...
    return ComponentBoxService.panel(
            item ->
                item.id("cinematic-replay.settings")
                    .width(
                        this.checkCondition()
                            ? LayoutOperationHandler.percent(100.0F)
                    ...
                    .gap(14.0F),
            createComponentKeyService11(
                "Capture & output", 18.0F, MaterialIsLightService.ON_SURFACE, true),
            ...
            MaterialTextService.divider(),
            createComponentKeyService11(
                "Resolution", 12.0F, MaterialIsLightService.ON_SURFACE_VARIANT, true),
            ComponentBoxService.row(
            ...
            this.createComponentKeyService9(
                ...
                (value != 0)),
            createComponentKeyService11(
                    "MP4 files are saved in ellice/clips.",
                    11.0F,
                    MaterialIsLightService.ON_SURFACE_VARIANT,
                    false)
  ...
  private ComponentKeyService<?> createComponentKeyService5() {
    ...
    return ComponentBoxService.panel(
            item ->
                item.id("cinematic-replay.timeline")
            ...
            ComponentBoxService.row(
                item ->
                    item.width(LayoutOperationHandler.percent(100.0F))
                        .align(ScenePctService.Align.CENTER),
                createComponentKeyService11(
                        "Edit timeline", 18.0F, MaterialIsLightService.ON_SURFACE, true)
            ...
            ComponentBoxService.row(
                ...
                createComponentKeyService10(
                    "to-in", "|◀", false, true, () -> this.updateState2(this.value4)),
                createComponentKeyService10(
                    "frame-back",
                    "−1f",
                    false,
                ...
                createComponentKeyService10(
                    "to-out", "▶|", false, true, () -> this.updateState2(this.value5)),
                createComponentKeyService10(
                    "speed",
                    this.value8 + "×",
                    false,
                    ...
                    () -> {
                      this.value8 =
                          this.value8 == 0.5F ? 1.0F : (this.value8 == 1.0F ? 2.0F : 0.5F);
                      this.updateState();
                    })),
            ComponentBoxService.<TIMELINEComponent>node(
                    ...
                    item ->
                        item.id("cinematic-replay.filmstrip")
                            .state(value, this.value4, this.value5, this.value6, this.value7)
                            .size(
                                LayoutOperationHandler.percent(100.0F),
                                LayoutOperationHandler.px(223.0F))
                            .onChange(
                                (start, end, progress) -> {
                                  this.value4 = start;
                                  this.value5 = end;
            ...
            ComponentBoxService.row(
                ...
                createComponentKeyService11(
                    this.value7 + "×", 11.0F, MaterialIsLightService.ON_SURFACE_VARIANT, true),
                createComponentKeyService10(
                    "zoom-in",
                    "+",
                    false,
  ...
  private ComponentKeyService<?> createComponentKeyService7() {
    ...
    return ComponentBoxService.row(
            item ->
                item.id("cinematic-replay.footer")
                    .size(
                        LayoutOperationHandler.percent(100.0F),
            ...
            createComponentKeyService10(
                ...
                false,
                currentValue != 0
                    || currentState == CinematicExporter.State.EMPTY
  ...
  private ComponentKeyService<?> createComponentKeyService8(String text, boolean enabled) {
    return createComponentKeyService10(
        "resolution-" + text,
        text,
        text.equals(this.implClipsService.resolution()),
        enabled,
        () -> {
  ...
  private ComponentKeyService<?> createComponentKeyService9(
    ...
    return ComponentBoxService.column(
            ...
            createComponentKeyService11(
                currentText
                    + (text.equals("duration") ? " · " + Math.round(currentValue) + " s" : ""),
                12.0F,
                MaterialIsLightService.ON_SURFACE_VARIANT,
                true),
  ...
  private static ComponentKeyService<MaterialJoinedService> createComponentKeyService10(
      String text, String currentText, boolean enabled, boolean currentEnabled, Runnable runnable) {
    int value =
  ...
  private static String createText(double doubleValue) {
    return String.format(Locale.ROOT, "%d:%04.1f", (int) (doubleValue / 60.0), doubleValue % 60.0);
  }
