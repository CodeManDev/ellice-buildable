...
public final class ScreenDelayQueue {
   ...
   ScreenDelayQueue(SceneDtService sceneDt, ThemeIsSetService themeIsSet, ScreenDelayQueue.HostBridge currentHostBridge) {
   ...
   private boolean checkCondition(ScreenOperationHandler screenOperation) {
      ...
      if (screenOperation == this.screenOperationHandler && this.lifecycle == ScreenDelayQueue.Lifecycle.OPEN) {
         return true;
      }
      ...
      scenePct.size(ScenePctService.pct(100.0F), ScenePctService.pct(100.0F))
         .layerBreak(true)
         .opacity(0.0F)
         .pointerEvents(true)
         .visible(false);
      if (this.screenOperationHandler != null) {
         if (screenOperation.opensAsWindow() && this.lifecycle == ScreenDelayQueue.Lifecycle.OPEN) {
            ...
            try {
               ...
               logger.error("Failed to suspend {}", createText(currentScreenOperation), currentException);
            }
            ...
            this.items
               .push(
                  new ScreenDelayQueue.Suspended(this.screenOperationHandler, this.screenScreenIdService, this.scenePctService, this.values3, this.options2)
               );
            this.updateState3();
      ...
      try {
         ...
         logger.error("Client screen {} failed during onOpen", createText(screenOperation), nextException);
   ...
   public void close() {
      if (this.screenOperationHandler != null && this.lifecycle != ScreenDelayQueue.Lifecycle.CLOSING) {
         long longValue = ++this.timestamp;
         ScreenOperationHandler screenOperation = this.screenOperationHandler;
         ...
         this.updateState();
         if (longValue == this.timestamp && this.screenOperationHandler == screenOperation && this.scenePctService == scenePct) {
      ...
      if (this.screenOperationHandler != null) {
         ...
         if (this.lifecycle != ScreenDelayQueue.Lifecycle.CLOSING) {
            ...
            try {
               ...
               logger.error("Client screen {} failed during tick", createText(screenOperation), exception);
            }
         } else {
            ScenePctService scenePct = this.scenePctService;
            if (scenePct == null || !this.sceneDtService.transitions().isTransitioning(scenePct)) {
   ...
   public boolean keyPressed(int value, int currentValue) {
      if (this.screenOperationHandler == null) {
         ...
         if (screenOperation.keyPressed(screenScreenId, value, currentValue)) {
            return true;
         } else if (this.screenOperationHandler != screenOperation || this.lifecycle == ScreenDelayQueue.Lifecycle.CLOSING) {
            return true;
   ...
   public boolean navigationKeyPressed(int value, int currentValue) {
      if (this.capturesKeybindInput()) {
         ...
         int previousValue = !"module-studio".equals(this.screenOperationHandler.id()) || value != 256 && (value != 83 || (currentValue & 10) == 0) ? 0 : 1;
         return value != 258 && nextValue == 0 && previousValue == 0 ? false : this.keyPressed(value, currentValue);
      } else {
         return false;
      }
   }
   ...
   public boolean capturesKeybindInput() {
      return this.screenOperationHandler instanceof ClickGuiScreen clickGuiScreen && clickGuiScreen.capturesKeybindInput();
   }

   public boolean isClosing() {
      return this.screenOperationHandler != null && this.lifecycle == ScreenDelayQueue.Lifecycle.CLOSING;
   }

   public float backgroundDesaturation() {
      return this.screenOperationHandler != null ? this.screenOperationHandler.backgroundDesaturation() : 0.0F;
   }
   ...
   private void updateState() {
      if (this.screenOperationHandler != null && !this.enabled) {
         ...
         try {
            ...
            logger.error("Client screen {} failed during onClose", createText(this.screenOperationHandler), exception);
         }
      }
   }
   ...
   private static final class MinecraftHostBridge implements ScreenDelayQueue.HostBridge {
      ...
      public void activate(ScreenOperationHandler screenOperation) {
         ...
         } else if (minecraft.screen instanceof CoreSetBehindScreenService currentCoreSetBehindScreen) {
            currentCoreSetBehindScreen.setBehindScreen(screenOperation.overlaysPreviousScreen() && !(this.screen2 instanceof TitleScreen) ? this.screen2 : null);
         }
      }
      ...
      public void release() {
         if (this.enabled2) {
            ...
            if (minecraft.screen instanceof CoreSetBehindScreenService) {
               ...
               if (minecraft.level == null && !(value instanceof JoinMultiplayerScreen) && !(value instanceof SelectWorldScreen)) {
   ...
   private record Suspended(
      ScreenOperationHandler screen, ScreenScreenIdService context, ScenePctService<?> root, SceneCodec.Preset preset, SceneCodec.Options options
   ) {
   }
}
