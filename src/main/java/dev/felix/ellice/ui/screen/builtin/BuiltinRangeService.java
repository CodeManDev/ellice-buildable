...
public final class BuiltinRangeService {
  ...
  public String error() {
    ...
    if (currentValue != null && nextValue != null) {
      if (currentValue < this.value2
          || currentValue > this.value3
          || nextValue < this.value2
          || nextValue > this.value3) {
  ...
  public static String format(float value, float currentValue, float nextValue) {
    int previousValue =
        currentValue > 0.0F ? Math.max(calculateValue(currentValue), calculateValue(nextValue)) : 6;
    return new BigDecimal(Float.toString(value))
        .setScale(Math.min(8, previousValue), RoundingMode.HALF_UP)
        .stripTrailingZeros()
        .toPlainString();
  }
  ...
  public static int maxCharacters(float value, float currentValue, float nextValue) {
    int previousValue =
        nextValue > 0.0F
            ? Math.min(8, Math.max(calculateValue(nextValue), calculateValue(value)))
            : 6;
    int currentLength =
        new BigDecimal(Float.toString(Math.max(Math.abs(value), Math.abs(currentValue))))
