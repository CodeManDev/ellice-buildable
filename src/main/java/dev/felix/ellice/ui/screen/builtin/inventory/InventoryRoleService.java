...
public final class InventoryRoleService extends ScenePctService<InventoryRoleService> {
   ...
   protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      ...
      if (textureUvRegion != null && textureUvRegion.textureId() != 0) {
         ...
         String currentText = this.role2 == InventoryRoleData.Role.LOCKED ? "–" : this.role2.title().substring(0, 2);
         float currentValue = width * 0.4F;
         compositorPushPresentationScale.text(
            height + (width - compositorPushPresentationScale.textWidth(currentText, currentValue, TextMode.REGULAR, "material-roboto-medium")) / 2.0F,
            value + (width - compositorPushPresentationScale.textLineHeight(currentValue, TextMode.REGULAR, "material-roboto-medium")) / 2.0F,
            currentText,
            currentValue,
