...
final class TIMELINEComponent extends ScenePctService<TIMELINEComponent> {
  ...
  private final RhiBlendStateService.TextureHandle[] textureHandle =
      new RhiBlendStateService.TextureHandle[20];
  ...
  private float calculateValue4(float value) {
    return this.calculateValue2()
        + (value - this.value6) / this.calculateValue() * this.calculateValue3();
  }

  private float calculateValue5(float currentValue) {
    return Math.clamp(
        this.value6
            + (currentValue - this.calculateValue2())
  ...
  protected void onPress(float value, float currentValue) {
    if (!(currentValue < this.cy + 28.0F) && !(currentValue > this.cy + this.ch - 24.0F)) {
      ...
      if (Math.abs(value - nextValue) <= 12.0F
          && Math.abs(value - nextValue) <= Math.abs(value - previousValue)) {
  ...
  protected void onRelease(boolean enabled) {
    if ((this.drag2 == TIMELINEComponent.Drag.IN || this.drag2 == TIMELINEComponent.Drag.OUT)
        && this.runnable != null) {
      this.runnable.run();
    }
  ...
  protected boolean handleScroll(float currentValue) {
    ...
    this.value6 =
        Math.clamp(
            this.value6 - currentValue * this.calculateValue() * 0.12F,
            0.0F,
  ...
  protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
    ...
    compositorPushPresentationScale.roundedRect(
        ...
        this.ch,
        20.0F,
        this.calculateValue7(MaterialIsLightService.SURFACE_HIGH));
    ...
    float nextValue =
        currentValue <= 3.0F
            ...
            : (currentValue <= 8.0F
                ? 1.0F
                : (currentValue <= 18.0F ? 2.0F : (currentValue <= 35.0F ? 5.0F : 10.0F)));
        ...
        if (bytes != null && this.byte2[index] != bytes) {
          ...
          this.textureHandle[index] =
              compositorPushPresentationScale.uploadRgbaTexture(
                  RhiBlendStateService.TextureHandle.NONE, 160, 90, currentLength);
    ...
    compositorPushPresentationScale.roundedRect(
        y,
        height,
        Math.max(0.0F, initialValue - y),
        102.0F,
        0.0F,
        this.calculateValue7(-1340861928));
    compositorPushPresentationScale.roundedRect(
        resolvedValue,
        height,
        Math.max(0.0F, y + width - resolvedValue),
        102.0F,
        0.0F,
        this.calculateValue7(-1340861928));
