...
public final class ServerPreviewScreen implements ScreenOperationHandler {
  ...
  public ServerPreviewScreen(
      ServerViewTracker serverViewTracker,
      UUID uUID,
      String string,
      String string2,
      BiConsumer<String, String> biConsumer) {
    this.serverViewTracker = serverViewTracker;
    this.text2 = string;
    this.text3 = string2;
    this.uUID = uUID;
    this.renderer = new ServerListRenderer(serverViewTracker, biConsumer);
    this.renderer.account(uUID, "", true);
  }
  ...
  public void viewport(float f, float f2) {
    if (this.fbg6iovryrvm == f && this.value2 == f2) {
      return;
    }
    this.fbg6iovryrvm = f;
    this.value2 = f2;
  ...
  public ScenePctService<?> build(ScreenScreenIdService screenScreenIdService) {
    ...
    ((SceneCornerRadiusService)
            ((SceneCornerRadiusService)
                    ((SceneCornerRadiusService)
                            this.materialTerrainTooltipsService.id("server-preview.canvas"))
                        .size(
    ...
    this.materialTerrainTooltipsService.addChild(
        (ScenePctService<?>)
            ((SceneFocusService)
                    ((SceneFocusService)
                            ((SceneFocusService) new SceneFocusService().backdrop(true).absolute())
  ...
  private ComponentKeyService<?> createComponentKeyService(
    ...
    ServerViewTracker.View view = this.serverViewTracker.view(this.text2);
        ...
        arrayList.add(
            ComponentBoxService.row(
                    layoutContainerNode ->
                        ((LayoutContainerNode)
                                ((LayoutContainerNode)
                                        layoutContainerNode.gap(Float.intBitsToFloat(1092616192)))
                                    .padding(Float.intBitsToFloat(0x41000000)))
                            .align(ScenePctService.Align.CENTER),
                    MaterialTextService.icon("person", MaterialIsLightService.PRIMARY),
      ...
      arrayList.add(
          MaterialTextService.text(
                  view.checkedAt() == 0L
                      ? "Checking the server\u2026"
                      : (!view.reachable()
                          ? "The server is not responding to status requests."
                          : "This server does not share a public player sample."),
                  Float.intBitsToFloat(1096810496),
    ...
    ComponentKeyService<?> componentKeyService =
        this.renderer.previewDetails(componentThemeService, this.text2, this.text3);
    ComponentKeyService<LayoutContainerNode> componentKeyService2 =
        ComponentBoxService.column(
            layoutContainerNode ->
                ((LayoutContainerNode) layoutContainerNode.id("preview.player-panel"))
            ...
            ComponentBoxService.panel(
                ...
                MaterialTextService.text(
                    ...
                    MaterialIsLightService.ON_SURFACE),
                MaterialTextService.text(
                        "A public sample shared by the server. It may not include every player"
                            + " online.",
                        Float.intBitsToFloat(1096810496),
                        MaterialIsLightService.ON_SURFACE_VARIANT)
                    .props(sceneTextService -> sceneTextService.wordWrap(true)),
                ComponentBoxService.column(
                    layoutContainerNode ->
                        ((LayoutContainerNode) layoutContainerNode.id("preview.players"))
            ...
            ComponentBoxService.panel(
                sceneCornerRadiusService ->
                    ((SceneCornerRadiusService)
                        ...
                        .gap(Float.intBitsToFloat(0x41400000)),
                MaterialTextService.text(
                        "Just looking? You're welcome.",
                        Float.intBitsToFloat(1101004800),
                        MaterialIsLightService.ON_SURFACE)
                    .props(sceneTextService -> sceneTextService.wordWrap(true)),
                MaterialTextService.text(
                        "Previewing checks public server information. Your account stays in the"
                            + " menu until you choose Join server.",
                        Float.intBitsToFloat(1096810496),
                        MaterialIsLightService.ON_SURFACE_VARIANT)
                    .props(sceneTextService -> sceneTextService.wordWrap(true)),
                MaterialTextService.button(
                    "preview.favorite",
                    this.serverViewTracker.library().usage(this.uUID, this.text2).favorite()
                        ? "Saved to favourites"
                        : "Save to favourites",
                    () -> {
                      this.serverViewTracker.toggleFavorite(this.uUID, this.text2);
                      this.updateState();
                    },
                    false)),
            ComponentBoxService.panel(
                ...
                MaterialTextService.text(
                    ...
                    MaterialIsLightService.ON_SURFACE),
                MaterialTextService.text(
                        "Minecraft server chat requires a signed-in play connection. A status"
                            + " preview cannot read or send in-game messages.",
                        Float.intBitsToFloat(1096810496),
                        MaterialIsLightService.ON_SURFACE_VARIANT)
                    .props(sceneTextService -> sceneTextService.wordWrap(true))));
                            ...
                            ((LayoutContainerNode)
                                    layoutContainerNode.size(
                                        ...
                                        LayoutOperationHandler.percent(
                                            Float.intBitsToFloat(1120403456))))
                                .padding(
                                    this.fbg6iovryrvm < Float.intBitsToFloat(1142292480)
                                        ? 0.0f
                                        : Float.intBitsToFloat(1103101952)))
                        .align(ScenePctService.Align.CENTER))
                                        ...
                                        ((SceneCornerRadiusService)
                                                ((SceneCornerRadiusService)
                                                    ...
                                                    .size(
                                                        ...
                                                        LayoutOperationHandler.percent(
                                                            Float.intBitsToFloat(1120403456))))
                                            .maxWidth(Float.intBitsToFloat(1149370368)))
                                    .maxHeight(Float.intBitsToFloat(0x444D0000)))
                            .backgroundColor(
                                ScenePctService.mulAlpha(
                                    MaterialIsLightService.SURFACE,
                                    Float.intBitsToFloat(1064849900)))
                                ...
                                scenePctService.width(
                                    bl
                                        ...
                                        : LayoutOperationHandler.percent(
                                            Float.intBitsToFloat(1120403456))))
                            .flexShrink(0.0f)),
                componentKeyService2.props(
                    scenePctService ->
                        ((ScenePctService)
                                ((ScenePctService) scenePctService.flex(bl ? 1.0f : 0.0f))
                                    .minWidth(0.0f))
                            .flexShrink(0.0f)))));
  }
  ...
  public void tick(ScreenScreenIdService screenScreenIdService) {
    ...
    if (screenScreenIdService != null && l >= this.timestamp2) {
      this.timestamp2 = l + 200L;
      this.serverViewTracker.seedIcon(this.text2, null);
      this.serverViewTracker.request(this.text2, false);
    }
