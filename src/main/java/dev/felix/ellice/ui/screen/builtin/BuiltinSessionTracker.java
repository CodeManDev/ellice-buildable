...
public final class BuiltinSessionTracker implements ScreenOperationHandler {
  ...
  public BuiltinSessionTracker(
      AccountAllService accountAll,
    ...
    this(
        ...
        () -> {
          ...
          return new BuiltinSessionTracker.Session(
  ...
  public BuiltinSessionTracker(
      AccountAllService accountAll,
      CompositorPushPresentationScaleService compositorPushPresentationScale,
      ServerViewTracker currentServerViewTracker,
      Supplier<BuiltinSessionTracker.Session> currentSupplier) {
  ...
  public ScenePctService<?> build(ScreenScreenIdService screenScreenId) {
    ...
    ServerViewTracker currentServerViewTracker =
        this.serverViewTracker != null
            ? this.serverViewTracker
            : (CoreIsInitializedHandler.isReady()
                ? CoreIsInitializedHandler.get().serverHub()
                : null);
    if (currentServerViewTracker != null) {
      ...
      this.renderer =
          new ServerListRenderer(
              currentServerViewTracker,
              (item, currentItem) -> {
    ...
    this.sceneCornerRadiusService
        ...
        .backgroundColor(
            ScenePctService.mulAlpha(
                ...
                MaterialIsLightService.isLight() ? 0.24F : 0.18F))
        .blur(8.0F)
        .interactive(true);
  ...
  private float calculateValue() {
    return !(this.value2 < 600.0F) && !(this.value3 < 360.0F) ? 24.0F : 8.0F;
  }
  ...
  private ComponentKeyService<?> createComponentKeyService(ComponentThemeService componentTheme) {
    this.sceneCornerRadiusService.backgroundColor(
        ScenePctService.mulAlpha(
            MaterialIsLightService.SURFACE_LOWEST,
            MaterialIsLightService.isLight() ? 0.24F : 0.18F));
    ...
    arrayList.add(
        this.createComponentKeyService6()
            .props(
                item -> item.visible(!this.enabled3 || !this.enabled6 || this.renderer == null)));
    if (this.renderer != null) {
      this.renderer.account(
          this.session2.uuid(),
          this.session2.name(),
          !this.session2.inWorld() && this.uUID == null);
    ...
    currentArrayList.add(
        ComponentBoxService.row(
                item ->
                    item.id("accounts.workspace")
                        .width(LayoutOperationHandler.percent(100.0F))
                        .flex(1.0F)
                        .minHeight(0.0F)
                        .padding(this.enabled5 ? 4.0F : (this.enabled4 ? 8.0F : 16.0F))
                        .gap(16.0F)
                    ...
                    item -> {
                      ...
                      item.id("accounts.sheet")
                          ...
                          .border(
                              1.0F,
                              ScenePctService.mulAlpha(
                                  MaterialIsLightService.OUTLINE_VARIANT, 0.7F))
    ...
    return this.enabled3
        ...
        ? ComponentBoxService.row(
                ...
                this.createComponentKeyService4()
                    .props(
                        item ->
                            item.width(LayoutOperationHandler.auto()).flex(1.0F).minWidth(0.0F)),
                MaterialTextService.iconButton(
                    "accounts.network", "swap_horiz", "FRITZ!Box · Reconnect", this::updateState4),
                MaterialTextService.iconButton(
                        "accounts.add", "person_add", "Add account", this::updateState6)
                    ...
                    .children(
                        MaterialTextService.icon(
                            ...
                            value != 0
                                ? MaterialIsLightService.ON_SURFACE_VARIANT
                                : ScenePctService.mulAlpha(
                                    MaterialIsLightService.ON_SURFACE, 0.38F))),
                MaterialTextService.iconButton(
                    "accounts.close", "close", "Close · Esc", this::updateState3))
            .key("toolbar")
        : ComponentBoxService.row(
                item ->
                    item.id("accounts.toolbar")
                        .height(LayoutOperationHandler.px(this.enabled4 ? 64.0F : 72.0F))
                        .padding(8.0F, this.enabled4 ? 4.0F : 16.0F)
                        .gap(8.0F)
                        .align(ScenePctService.Align.CENTER)
                        .flexShrink(0.0F),
                this.mode == BuiltinSessionTracker.Mode.LIST
                    ...
                    : MaterialTextService.iconButton(
                        "accounts.back", "arrow_back", "Back to accounts", this::updateState16),
                ...
                (this.enabled4
                        ...
                        : createComponentKeyService12(
                                ...
                                (value != 0))
                            .children(
                                MaterialTextService.icon("person_add", currentValue),
                                MaterialTextService.label("Add account", currentValue)))
                    .props(
                        item ->
                            item.available(value != 0)
                                .visible(this.mode == BuiltinSessionTracker.Mode.LIST)
                                .tooltip(
                                    this.session2.inWorld()
                                        ? "Return to the main menu to add an account"
                                        : "Add account")),
                MaterialTextService.iconButton(
                        "accounts.network",
                        "swap_horiz",
  ...
  private ComponentKeyService<AnimatedSelectionIndicator> createComponentKeyService4() {
    return ComponentBoxService.<AnimatedSelectionIndicator>node(
            ...
            item ->
                item.selection(this.enabled6 ? 1 : 0)
                    .id("accounts.tabs")
  ...
  private ComponentKeyService<?> createComponentKeyService6() {
    ArrayList arrayList = new ArrayList();
    List<AccountTypeData> items =
        this.accountAllService.all().stream()
            .sorted(
                Comparator.<AccountTypeData, Boolean>comparing(
                        item -> !item.uuid().equals(this.session2.uuid()))
    ...
    if (this.session2.uuid() != null
      ...
      arrayList.add(
          this.createComponentKeyService7(this.session2.uuid(), this.session2.name(), null));
    }
    ...
    for (AccountTypeData accountTypeData : (Iterable<AccountTypeData>) (Iterable<?>) (items)) {
      if (this.checkCondition(
          ...
          accountTypeData.isOffline()
              ? "offline"
              : (accountTypeData.isSession() ? "cookie session" : "microsoft"))) {
        arrayList.add(
            this.createComponentKeyService7(
                accountTypeData.uuid(), accountTypeData.username(), accountTypeData));
      }
    }
    ...
    if (arrayList.isEmpty()) {
      arrayList.add(
          ComponentBoxService.column(
              item -> item.id("accounts.empty").padding(24.0F, 12.0F).gap(8.0F),
              MaterialTextService.text(
    ...
    currentArrayList.add(
        ComponentBoxService.column(
            ...
            createComponentKeyService11(
                this.session2.inWorld()
                    ? "Return to the main menu to switch accounts."
                    : "Choose a profile to play with.",
                MaterialIsLightService.ON_SURFACE_VARIANT)));
    currentArrayList.add(
  ...
  private boolean checkCondition(UUID uUID, String text, String currentText) {
    ...
    return nextText.isEmpty()
        || (text + " " + PrivacyRevisionService.replace(text) + " " + uUID + " " + currentText)
            .toLowerCase(Locale.ROOT)
            .contains(nextText);
  }

  private ComponentKeyService<?> createComponentKeyService7(
      UUID currentUUID, String currentText, AccountTypeData accountTypeData) {
    ...
    String nextText =
        accountTypeData == null
            ...
            : (accountTypeData.isOffline()
                ? "Offline account"
                : (accountTypeData.isSession()
                    ? (accountTypeData.hasCurrentSession()
                        ? "Cookie session"
                        : "Cookie session · import again")
                    : "Microsoft account"));
    String previousText =
        currentEnabled ? "Signing in…" : (enabled ? "Active · " + nextText : nextText);
    int value =
        enabled
            ? MaterialIsLightService.SECONDARY_CONTAINER
            : MaterialIsLightService.SURFACE_CONTAINER;
    int currentValue =
        enabled ? MaterialIsLightService.ON_SECONDARY_CONTAINER : MaterialIsLightService.ON_SURFACE;
    ArrayList arrayList = new ArrayList();
    arrayList.add(
        ComponentBoxService.node(
            "account-select",
            ...
            item -> {
              item.colors(0, currentValue)
                  ...
                  .disabledSurface(0)
                  .available(
                      accountTypeData != null && !this.session2.inWorld() && this.uUID == null);
                      ...
                      accountTypeData == null
                          ...
                          : (currentEnabled
                              ...
                              : "Use "
                                  + currentText
                                  + " · last used "
                                  + createText(accountTypeData.lastUsedAt())))
                  .onClick(() -> this.updateState23(accountTypeData));
            },
            this.createComponentKeyService8(
                currentUUID, currentText, accountTypeData == null || !accountTypeData.isOffline()),
            ComponentBoxService.column(
    ...
    if (accountTypeData != null) {
      arrayList.add(
          MaterialTextService.iconButton(
                  "accounts.remove." + currentUUID,
                  "delete",
                  "Remove " + currentText + " from saved accounts",
                  () -> {
  ...
  private ComponentKeyService<?> createComponentKeyService8(
      UUID uUID, String currentText, boolean enabled) {
        ...
        ComponentBoxService.row(
            item ->
                item.size(40.0F, 40.0F)
                    ...
                    .visible(!value),
            MaterialTextService.text(
                    currentText, 20.0F, MaterialIsLightService.ON_PRIMARY_CONTAINER)
                .props(item -> item.initial(true))),
        ComponentBoxService.node(
            "account-avatar",
            SceneTextureService::new,
            item ->
                item.textureName(nextText).size(40.0F, 40.0F).cornerRadius(20.0F).visible(value)));
  }
  ...
  private ComponentKeyService<?> createComponentKeyService9() {
    ...
    switch (this.mode) {
      case CHOOSE:
        arrayList.add(
            MaterialTextService.text(
                    "Make yourself at home", 28.0F, MaterialIsLightService.ON_SURFACE)
                .props(item -> item.lineHeight(36.0F).wordWrap(true).visible(!this.enabled5)));
        arrayList.add(
            createComponentKeyService11(
                    "Add a profile, then choose it whenever you want to play.",
                    MaterialIsLightService.ON_SURFACE_VARIANT)
                .props(item -> item.visible(!this.enabled5)));
        arrayList.add(
            this.createComponentKeyService10(
                "accounts.choose.microsoft",
                "login",
                "Microsoft account",
        ...
        arrayList.add(
            this.createComponentKeyService10(
                "accounts.choose.cookie",
                "cookie",
                "Cookie login",
                "Use an existing Microsoft session",
                this::updateState8));
        arrayList.add(
            this.createComponentKeyService10(
                "accounts.choose.offline",
                "person",
                "Offline account",
                "For servers that allow offline players",
                this::updateState7));
        arrayList.add(
      ...
      case MICROSOFT:
        arrayList.add(
            MaterialTextService.text(
                    "Sign in with Microsoft", 28.0F, MaterialIsLightService.ON_SURFACE)
                .props(item -> item.lineHeight(36.0F).wordWrap(true)));
        arrayList.add(
            createComponentKeyService11(
                "Open microsoft.com/link and enter this code.",
                MaterialIsLightService.ON_SURFACE_VARIANT));
        arrayList.add(
            ComponentBoxService.node(
                "device-code",
                ...
                MaterialTextService.text(
                        this.text7, 28.0F, MaterialIsLightService.ON_SECONDARY_CONTAINER)
                    .props(item -> item.letterSpacing(2.0F).flex(1.0F).minWidth(0.0F)),
                MaterialTextService.icon(
                    "content_copy", MaterialIsLightService.ON_SECONDARY_CONTAINER)));
        arrayList.add(
            createComponentKeyService11(
                    this.text8,
                    this.enabled7
      ...
      case COOKIE:
        arrayList.add(
            MaterialTextService.text(
                    "Sign in with a cookie", 28.0F, MaterialIsLightService.ON_SURFACE)
                .props(item -> item.lineHeight(36.0F).wordWrap(true)));
        arrayList.add(
            createComponentKeyService11(
                "Paste a Microsoft session cookie or import your cookie export.",
                MaterialIsLightService.ON_SURFACE_VARIANT));
        arrayList.add(
        ...
        arrayList.add(
            createComponentKeyService12(
                ...
                .children(
                    MaterialTextService.icon(
                        "folder", MaterialIsLightService.ON_SECONDARY_CONTAINER),
                    MaterialTextService.label(
                        "Choose cookie file", MaterialIsLightService.ON_SECONDARY_CONTAINER)));
        arrayList.add(
            createComponentKeyService11(
                "A value without a name is treated as __Host-MSAAUTH. Minecraft.net opens in a"
                    + " separate sign-in window. The session lasts until it expires or you restart"
                    + " the client; import your cookies again to renew it.",
                MaterialIsLightService.ON_SURFACE_VARIANT));
        arrayList.add(
            ComponentBoxService.row(
                item -> item.gap(8.0F),
      ...
      case OFFLINE:
        arrayList.add(
            MaterialTextService.text(
                    "Add offline account", 28.0F, MaterialIsLightService.ON_SURFACE)
                .props(item -> item.lineHeight(36.0F).wordWrap(true)));
        arrayList.add(
            createComponentKeyService11(
                "Offline accounts only work on servers that allow offline players.",
                MaterialIsLightService.ON_SURFACE_VARIANT));
        arrayList.add(
            ComponentBoxService.<ControlLetterSpacingService>node(
                    "offline-name",
        ...
        arrayList.add(
            ComponentBoxService.row(
                ...
                createComponentKeyService12(
                    ...
                    this::updateState27,
                    true,
                    AccountTypeData.isValidOfflineUsername(this.text5.trim())
                        && !this.session2.inWorld()),
                MaterialTextService.button(
                    "accounts.cancel",
                    "Cancel",
    ...
    return ComponentBoxService.column(
            item ->
                item.id("accounts.form.scroll")
                    .flex(1.0F)
                    .minHeight(0.0F)
                    .padding(this.enabled5 ? 8.0F : (this.enabled4 ? 16.0F : 32.0F))
                    .scrollable(true)
                    .clip(true)
                    .scrollbarWidth(3.0F)
                    .scrollbarColor(MaterialIsLightService.OUTLINE_VARIANT)
  ...
  private ComponentKeyService<?> createComponentKeyService10(
    ...
    return ComponentBoxService.<MaterialJoinedService>node(
            ...
            ComponentBoxService.column(
                ...
                MaterialTextService.text(
                        previousText, 16.0F, MaterialIsLightService.ON_SECONDARY_CONTAINER)
                    .props(item -> item.wordWrap(true)),
                createComponentKeyService11(
                    sourceText, MaterialIsLightService.ON_SECONDARY_CONTAINER)),
            MaterialTextService.icon(
                "chevron_right", MaterialIsLightService.ON_SECONDARY_CONTAINER))
        .key(currentText);
  }

  private static ComponentKeyService<SceneTextService> createComponentKeyService11(
  ...
  private static ComponentKeyService<MaterialJoinedService> createComponentKeyService12(
      String text, String currentText, Runnable runnable, boolean enabled, boolean currentEnabled) {
    int value =
        currentEnabled
            ? (enabled
                ? MaterialIsLightService.PRIMARY
    ...
    int currentValue =
        currentEnabled
            ? (enabled
                ...
                : MaterialIsLightService.ON_SECONDARY_CONTAINER)
            : ScenePctService.mulAlpha(MaterialIsLightService.ON_SURFACE, 0.38F);
    return MaterialTextService.button(text, currentText, runnable, value, currentValue, 20.0F)
        .props(item -> item.available(currentEnabled));
  }
                  ...
                  updateState31(
                      () -> {
                        if (this.atomicLong.get() == offset
                            && this.mode == BuiltinSessionTracker.Mode.COOKIE) {
                          this.enabled2 = false;
                          if (currentItem != null) {
  ...
  private void updateState11() {
    if (this.mode == BuiltinSessionTracker.Mode.COOKIE
      ...
      this.accountNewThreadService
          ...
          .whenComplete(
              (item, currentItem) ->
                  updateState31(() -> this.updateState12(offset, item, currentItem)));
    }
  }

  private void updateState12(
      long offset, AccountNewThreadService.LoginResult loginResult, Throwable exception) {
    if (this.atomicLong.get() == offset && this.mode == BuiltinSessionTracker.Mode.COOKIE) {
      ...
      if (exception != null) {
        ...
        this.updateState25(
            "Signed in", accountTypeData.username(), SceneRenderer.Type.SUCCESS, 3.5F);
        this.updateState16();
      }
    }
  }
  ...
  private void updateState13(Throwable exception) {
    ...
    this.enabled = true;
    this.text3 =
        currentException instanceof AccountCodeService accountCode
  ...
  private void updateState16() {
    ...
    MaterialEnterService.enter(
        this.sceneCornerRadiusService == null
            ? null
            : this.sceneCornerRadiusService.findById("accounts.workspace"),
        -16.0F,
        0.0F);
  }
  ...
  public void onOpen(ScreenScreenIdService screenScreenId) {
    MaterialEnterService.enter(
        this.sceneCornerRadiusService == null
            ? null
            : this.sceneCornerRadiusService.findById("accounts.sheet"),
        0.0F,
        18.0F);
  }
  ...
  private void updateState18(String text) {
    ...
    this.text9 =
        currentText.isEmpty()
            ...
            : (validOfflineUsername
                ? "Ready to add " + currentText + "."
                : "Use 1–16 letters, numbers or underscores only.");
  ...
  private void updateState21(Throwable exception) {
    this.text8 =
        createThrowable(exception) instanceof AccountCodeService accountCode
            ? accountCode.getMessage()
  ...
  private void updateState23(AccountTypeData accountTypeData) {
    if (accountTypeData != null && this.uUID == null) {
      ...
      if (currentUUID != null && currentUUID.equals(accountTypeData.uuid())) {
        ...
        this.updateState25(
            "Switched account",
            accountTypeData.username() + " (offline)",
        ...
        long offset = this.atomicLong.incrementAndGet();
        this.updateState25(
            "Signing in", accountTypeData.username() + "…", SceneRenderer.Type.INFO, 2.5F);
        this.accountNewThreadService
            .login(accountTypeData)
            .whenComplete(
                (item, currentItem) ->
                    updateState31(
                        () -> {
                          if (this.atomicLong.get() == offset
                              && this.uUID != null
                              && this.uUID.equals(accountTypeData.uuid())) {
                            this.uUID = null;
                            if (currentItem != null) {
                              ...
                              AccountTypeData signedInAccount =
                                  this.accountAllService.upsert(item.account());
  ...
  private void updateState24(Throwable exception, String text) {
    ...
    this.updateState25(
        text != null ? "Sign-in failed: " + text : "Sign-in failed",
        currentText,
        SceneRenderer.Type.ERROR,
        5.0F);
  }

  private void updateState25(
      String text, String currentText, SceneRenderer.Type type, float value) {
    try {
                                            ...
                                            () -> {
                                              if (this.atomicLong.get() == offset) {
                                                if (loginFailure != null) {
                                                  ...
                                                  this.enabled7 = true;
                                                  this.text8 =
                                                      "Return to the main menu and try signing in"
                                                          + " again.";
                                                  this.updateState2();
                                                } else {
                                                  AccountApplyOfflineService.apply(completedLogin);
                                                  AccountTypeData accountTypeData =
                                                      this.accountAllService.upsert(
                                                          completedLogin.account());
                                                  this.accountAllService.setActive(
                                                      accountTypeData.uuid());
                                                  this.updateState5();
                                                  this.text7 = "Linked";
                                                  this.text8 =
  ...
  private void updateState27() {
    if (this.controlLetterSpacingService != null && !this.supplier.get().inWorld()) {
      String currentText =
          this.controlLetterSpacingService.text() == null
              ? ""
              : this.controlLetterSpacingService.text().trim();
      if (!AccountTypeData.isValidOfflineUsername(currentText)) {
        ...
        AccountTypeData currentAccountTypeData =
            this.accountAllService.all().stream()
                .filter(AccountTypeData::isOffline)
                .filter(item -> item.username().equalsIgnoreCase(currentText))
                .findFirst()
                .orElse(null);
        AccountTypeData nextAccountTypeData =
            this.accountAllService.get(accountTypeData.uuid()).orElse(null);
        if (currentAccountTypeData == null
            && nextAccountTypeData != null
            && !nextAccountTypeData.isOffline()) {
          this.text9 = "That UUID is already used by a Microsoft account.";
          this.enabled8 = true;
          this.updateState2();
          this.updateState20();
        } else {
          AccountTypeData previousAccountTypeData =
              currentAccountTypeData != null ? currentAccountTypeData : nextAccountTypeData;
          if (previousAccountTypeData != null) {
            accountTypeData =
                new AccountTypeData(
                    previousAccountTypeData.uuid(),
                    previousAccountTypeData.username(),
                    AccountTypeData.Type.OFFLINE,
                    "",
                    previousAccountTypeData.addedAt(),
                    Instant.now());
          }
          ...
          this.updateState33();
          this.updateState25(
              "Offline account added",
              sourceAccountTypeData.username(),
              SceneRenderer.Type.SUCCESS,
              3.0F);
          this.updateState16();
        }
  ...
  public void tick(ScreenScreenIdService screenScreenId) {
    ...
    if (this.renderer != null && CoreIsInitializedHandler.isReady()) {
      this.renderer.tick(
          this.mode == BuiltinSessionTracker.Mode.LIST && (!this.enabled3 || this.enabled6));
    }
  }
  ...
  public boolean keyPressed(ScreenScreenIdService screenScreenId, int value, int currentValue) {
    if (value == 256) {
      if (this.mode != BuiltinSessionTracker.Mode.LIST) {
        ...
        && this.mode == BuiltinSessionTracker.Mode.LIST) {
      String text =
          this.enabled3 && this.enabled6 && this.renderer != null
              ? "servers.search"
              : "accounts.search";
      if (this.sceneCornerRadiusService.findById(text)
      ...
      if (!arrayList.isEmpty()) {
        ...
        for (ScenePctService currentScenePct :
          ...
          if (currentScenePct instanceof ControlLetterSpacingService currentControlLetterSpacing
        ...
        this.scenePctService =
            (ScenePctService<?>)
                arrayList.get(
                    index < 0
                        ? (nextValue != 0 ? arrayList.size() - 1 : 0)
                        : Math.floorMod(index + (nextValue != 0 ? -1 : 1), arrayList.size()));
  ...
  private static boolean checkCondition2(ScenePctService<?> scenePct) {
    for (ScenePctService currentScenePct = scenePct;
      ...
      if (!(currentScenePct.scrollMin() >= 0.0F)) {
        float value =
            scenePct.computedY() < currentScenePct.computedY() + 6.0F
                ...
                : (scenePct.computedY() + scenePct.computedH()
                        > currentScenePct.computedY() + currentScenePct.computedH() - 6.0F
