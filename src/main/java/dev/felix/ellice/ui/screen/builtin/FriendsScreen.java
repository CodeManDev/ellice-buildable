...
public final class FriendsScreen implements ScreenOperationHandler {
   ...
   public FriendsScreen(FriendsStateController currentFriendsStateController, Supplier<List<FriendsData>> supplier) {
   ...
   private List<FriendProfile> collectValues() {
      ...
      return this.friendsStateController
         ...
         .filter(
            item -> (item.displayName() + " " + item.note() + " " + (item.uuid() == null ? "" : item.uuid()))
   ...
   private ComponentKeyService<?> createComponentKeyService(ComponentThemeService componentTheme) {
      FriendProfile friendProfile = this.uUID == null ? null : this.friendsStateController.get(this.uUID);
      ...
      if (!this.checkCondition() || this.enabled2 && friendProfile != null) {
         arrayList.add(friendProfile == null ? this.createComponentKeyService8() : this.createComponentKeyService9(friendProfile));
      }
      ...
      ComponentKeyService<?> currentSize = ComponentBoxService.column(
         ...
         ComponentBoxService.panel(
               item -> item.id("friends.workspace")
                  .size(LayoutOperationHandler.percent(100.0F), LayoutOperationHandler.percent(100.0F))
                  .maxWidth(1120.0F)
                  .maxHeight(860.0F)
      ...
      return ComponentBoxService.stack(
         item -> item.size(LayoutOperationHandler.percent(100.0F), LayoutOperationHandler.percent(100.0F)),
         currentSize,
         this.enabled5 ? this.createComponentKeyService14() : ComponentBoxService.box(item -> item.visible(false)).key("add-dialog")
      );
   }

   private ComponentKeyService<?> createComponentKeyService2() {
      return ComponentBoxService.row(
         item -> item.height(LayoutOperationHandler.px(this.checkCondition2() ? 52.0F : 80.0F))
            .padding(
               4.0F, this.checkCondition() ? 8.0F : 16.0F
            )
            .gap(8.0F)
            .align(ScenePctService.Align.CENTER)
            .flexShrink(0.0F),
         ...
         ComponentBoxService.column(
            ...
            MaterialTextService.text("Your people. Your rules.", 12.0F, MaterialIsLightService.ON_SURFACE_VARIANT)
               .props(item -> item.visible(!this.checkCondition2() && !this.checkCondition()))
         ),
         MaterialTextService.iconButton("friends.add", "person_add", "Add a friend by name or UUID", this::updateState3)
            .props(item -> item.colors(MaterialIsLightService.PRIMARY_CONTAINER, MaterialIsLightService.ON_PRIMARY_CONTAINER))
      );
   }
   ...
   private ComponentKeyService<?> createComponentKeyService3() {
      ...
      if (this.enabled3) {
         for (FriendsData friendsData : this.items2
            .stream()
            .filter(item -> item.name().toLowerCase(Locale.ROOT).contains(this.text3.strip().toLowerCase(Locale.ROOT)))
            .toList()) {
         ...
         if (arrayList.isEmpty()) {
            arrayList.add(
               this.createComponentKeyService11(
                  "No players here",
                  this.items2.isEmpty() ? "Join a server to add players from its player list." : "Try another player name.",
                  "person"
               )
            );
         }
         ...
         if (arrayList.isEmpty()) {
            arrayList.add(
               this.createComponentKeyService11(
                  this.friendsStateController.entries().isEmpty() ? "Keep your friends close" : "No matching friends",
      ...
      return ComponentBoxService.column(
            item -> item.id("friends.directory")
               .width(
                  this.checkCondition()
            ...
            ComponentBoxService.row(
               ...
               this.createComponentKeyService4(
                  "saved", "Saved · " + this.friendsStateController.entries().size(), !this.enabled3, () -> this.enabled3 = false
               ),
               this.createComponentKeyService4(
                  "server", "On server · " + this.items2.size(), this.enabled3, () -> this.enabled3 = true
               )
            ),
            ComponentBoxService.row(
               item -> item.gap(4.0F).align(ScenePctService.Align.CENTER).flexShrink(0.0F),
               MaterialTextService.search("friends.search", "Search people or notes", this.text3, item -> {
                  this.text3 = item;
                  this.updateState();
               }, true).props(item -> item.flex(1.0F).minWidth(0.0F)),
               MaterialTextService.iconButton("friends.favorites", this.enabled4 ? "star_filled" : "star", "Show favorite friends", () -> {
                  this.enabled4 = !this.enabled4;
                  this.updateState();
               }).props(item -> {
                  item.visible(!this.enabled3).size(40.0F, 40.0F);
                  item.colors(this.enabled4 ? MaterialIsLightService.SECONDARY_CONTAINER : 0, MaterialIsLightService.PRIMARY);
               })
            ),
            ComponentBoxService.column(
               item -> item.id("friends.list")
                  .width(LayoutOperationHandler.percent(100.0F))
                  .flex(1.0F)
                  .minHeight(0.0F)
                  .scrollable(true)
   ...
   private ComponentKeyService<?> createComponentKeyService4(String currentText, String nextText, boolean enabled, Runnable runnable) {
      return MaterialTextService.button("friends.tab." + currentText, nextText, () -> {
            this.materialKeyService.clear(this.materialTerrainTooltipsService);
            runnable.run();
            this.updateState();
         }, false)
   ...
   private ComponentKeyService<?> createComponentKeyService5(String currentText, boolean enabled, int value) {
   ...
   private ComponentKeyService<?> createComponentKeyService6(FriendProfile friendProfile) {
      ...
      return MaterialTextService.button("friends.row." + friendProfile.id(), friendProfile.displayName(), () -> this.updateState2(friendProfile), false)
         .props(
            item -> {
               ...
               item.tooltip(friendProfile.note().isEmpty() ? "Edit module exclusions for " + friendProfile.displayName() : friendProfile.note());
            }
         )
         .children(
            ...
            ComponentBoxService.column(
               ...
               MaterialTextService.label(friendProfile.displayName(), MaterialIsLightService.ON_SURFACE),
               MaterialTextService.text(
                  currentEnabled ? "On this server" : (friendProfile.uuid() == null ? "Name saved · UUID pending" : "Saved friend"),
                  11.0F,
                  currentEnabled ? MaterialIsLightService.PRIMARY : MaterialIsLightService.ON_SURFACE_VARIANT
               )
            ),
            MaterialTextService.icon(
                  friendProfile.favorite() ? "star_filled" : "chevron_right", friendProfile.favorite() ? MaterialIsLightService.PRIMARY : MaterialIsLightService.OUTLINE
               )
   ...
   private ComponentKeyService<?> createComponentKeyService7(FriendsData friendsData) {
      FriendProfile friendProfile = this.friendsStateController.find(friendsData.uuid(), friendsData.name());
      return MaterialTextService.button("friends.player." + friendsData.uuid(), friendsData.name(), () -> {
            if (friendProfile != null) {
               this.updateState2(friendProfile);
            } else {
               this.updateState6(() -> {
                   FriendProfile addedFriend = this.friendsStateController.add(friendsData.uuid(), friendsData.name());
         ...
         .props(
            item -> {
               item.colors(MaterialIsLightService.SURFACE_CONTAINER, MaterialIsLightService.ON_SURFACE).shape(18.0F, 0.0F);
               item.width(LayoutOperationHandler.percent(100.0F))
                  .height(LayoutOperationHandler.px(72.0F))
                  .padding(10.0F, 12.0F)
                  .gap(12.0F)
         ...
         .children(
            ...
            MaterialTextService.icon(friendProfile == null ? "person_add" : "check", MaterialIsLightService.PRIMARY)
   ...
   private ComponentKeyService<?> createComponentKeyService8() {
      return ComponentBoxService.panel(
         ...
         MaterialTextService.text("A place for your people", 24.0F, MaterialIsLightService.ON_SURFACE)
            .props(item -> item.wordWrap(true)),
         MaterialTextService.text(
               "Choose a friend to adjust their module exclusions, pin them as a favorite or leave a private note.",
               14.0F,
               MaterialIsLightService.ON_SURFACE_VARIANT
            )
            .props(item -> item.maxWidth(340.0F).wordWrap(true)),
         MaterialTextService.button("friends.welcome.add", "Add your first friend", this::updateState3, true)
      );
   }
   ...
   private ComponentKeyService<?> createComponentKeyService9(FriendProfile friendProfile) {
      ...
      arrayList.add(
         ComponentBoxService.row(
            ...
            ComponentBoxService.column(
               item -> item.flex(1.0F).minWidth(0.0F),
               MaterialTextService.text(friendProfile.displayName(), 22.0F, MaterialIsLightService.ON_SURFACE),
               MaterialTextService.text(
            ...
            MaterialTextService.iconButton(
                  ...
                  () -> this.updateState7(friendProfile.favorite(!friendProfile.favorite()), "Favorite updated")
               )
      ...
      arrayList.add(
         MaterialTextService.text(
               friendProfile.uuid() == null ? "UUID resolves when this player is on your server." : friendProfile.uuid().toString(),
               11.0F,
               MaterialIsLightService.OUTLINE
            )
            .props(
               item -> item.id("friends.identity")
                  .flexShrink(0.0F)
                  .wordWrap(true)
                  .tooltip(
      ...
      arrayList.add(
         ComponentBoxService.column(
            ...
            MaterialTextService.text("Checked modules skip this friend.", 13.0F, MaterialIsLightService.ON_SURFACE_VARIANT)
         )
      );
      arrayList.add(
         ComponentBoxService.row(
            ...
            MaterialTextService.button(
                  "friends.defaults",
                  "Combat defaults",
                  () -> this.updateState7(friendProfile.excluded(FriendsMode.defaults()), "Combat protection restored"),
                  false
               )
               .props(
                  item -> {
                     ...
                     item.height(LayoutOperationHandler.px(36.0F))
                        .padding(0.0F, 12.0F)
                        .tooltip("Exclude from all five combat modules; keep visual modules visible.");
                  }
               )
               .children(MaterialTextService.text("Combat defaults", 12.0F, MaterialIsLightService.ON_SECONDARY_CONTAINER)),
            MaterialTextService.button("friends.clear", "Clear", () -> this.updateState7(friendProfile.excluded(Set.of()), "All exclusions cleared"), false)
               .props(item -> {
               ...
               .children(MaterialTextService.text("Clear", 12.0F, MaterialIsLightService.ON_SURFACE_VARIANT))
         )
      );

      for (boolean currentEnabled : new boolean[]{true, false}) {
         arrayList.add(
            MaterialTextService.text(currentEnabled ? "COMBAT" : "VISUALS & INTEL", 11.0F, MaterialIsLightService.OUTLINE)
               .props(
                  item -> item.margin(
                        8.0F, 4.0F, 0.0F, 4.0F
                     )
      ...
      arrayList.add(MaterialTextService.divider());
      arrayList.add(MaterialTextService.label("Private note", MaterialIsLightService.ON_SURFACE).props(item -> item.flexShrink(0.0F)));
      String currentText = this.text2.getOrDefault(friendProfile.id(), friendProfile.note());
      arrayList.add(this.createComponentKeyService12("friends.note", "How do you know them?", currentText, 240, item -> {
      ...
      arrayList.add(
         ComponentBoxService.row(
            ...
            MaterialTextService.button("friends.note.save", "Save note", () -> this.updateState6(() -> {
               this.friendsStateController.update(this.friendsStateController.get(friendProfile.id()).note(this.text2.getOrDefault(friendProfile.id(), friendProfile.note())));
               this.text2.remove(friendProfile.id());
            }, "Note saved"), false).props(item -> item.available(!currentText.equals(friendProfile.note()))),
            ComponentBoxService.box(item -> item.flex(1.0F)),
            MaterialTextService.iconButton("friends.remove", "delete", "Remove friend · can be undone", () -> this.updateState6(() -> {
      ...
      return ComponentBoxService.panel(
            item -> item.direction(ScenePctService.Direction.COLUMN)
               ...
               .scrollbarColor(MaterialIsLightService.OUTLINE)
               .clip(true),
            arrayList.stream().map(componentKey -> componentKey.props(component -> component.flexShrink(0.0F))).toArray(ComponentKeyService[]::new)
         )
         .key("editor:" + friendProfile.id())
         .onMount(
            item -> MaterialEnterService.enter(
               item, this.checkCondition() ? 18.0F : 10.0F, 0.0F
            )
         );
   }

   private ComponentKeyService<?> createComponentKeyService10(FriendProfile friendProfile, FriendsMode friendsMode) {
      ...
      return MaterialTextService.button(
            ...
            () -> {
               ...
               this.updateState7(
                   currentProfile.excluded(enumSet), friendsMode.label + (enumSet.contains(friendsMode) ? " will skip " : " will include ") + currentProfile.displayName()
               );
            },
            false
         )
         ...
         .children(
            MaterialTextService.icon(enabled ? "check_box" : "check_box_outline_blank", enabled ? MaterialIsLightService.PRIMARY : MaterialIsLightService.OUTLINE)
            ...
            MaterialTextService.text(
               enabled ? "Excluded" : "Included", 11.0F, enabled ? MaterialIsLightService.PRIMARY : MaterialIsLightService.OUTLINE
            )
         );
   }

   private ComponentKeyService<?> createComponentKeyService11(String currentText, String nextText, String previousText) {
      return ComponentBoxService.column(
         ...
         MaterialTextService.text(currentText, 18.0F, MaterialIsLightService.ON_SURFACE).props(item -> item.wordWrap(true)),
         MaterialTextService.text(nextText, 13.0F, MaterialIsLightService.ON_SURFACE_VARIANT).props(item -> item.wordWrap(true))
      );
   }

   private ComponentKeyService<?> createComponentKeyService12(String currentText, String nextText, String previousText, int value, Consumer<String> consumer) {
   ...
   private ComponentKeyService<?> createComponentKeyService13() {
      ...
      return ComponentBoxService.row(
         ...
         MaterialTextService.text(
               currentText,
               11.0F,
               !this.enabled6 && this.friendsStateController.notice().isEmpty() ? MaterialIsLightService.ON_SURFACE_VARIANT : MaterialIsLightService.ERROR
            )
            .props(item -> item.id("friends.notice").flex(1.0F).minWidth(0.0F).tooltip(currentText)),
         MaterialTextService.button("friends.undo", "Undo", () -> this.updateState6(() -> {
            this.friendsStateController.restore(this.friendsData2);
            this.updateState2(this.friendsData2);
            this.friendsData2 = null;
         }, "Friend restored"), false).props(item -> {
   ...
   private void updateState5() {
      try {
         ...
         this.text6 = iOExceptionIllegalArgumentException instanceof IOException ? "Could not save friends. " + this.friendsStateController.notice() : iOExceptionIllegalArgumentException.getMessage();
      }
   ...
   private ComponentKeyService<?> createComponentKeyService14() {
      return ComponentBoxService.panel(
            item -> item.id("friends.add.scrim")
               .size(LayoutOperationHandler.percent(100.0F), LayoutOperationHandler.percent(100.0F))
               .layerBreak(true)
               .padding(16.0F)
               ...
               .stopPropagation(true),
            ComponentBoxService.panel(
               item -> item.id("friends.add.dialog")
                  .width(LayoutOperationHandler.percent(100.0F))
                  .maxWidth(460.0F)
               ...
               MaterialTextService.text(
                     "Enter their player name or UUID. Players on this server are identified immediately.",
                     14.0F,
                     MaterialIsLightService.ON_SURFACE_VARIANT
                  )
                  .props(item -> item.wordWrap(true).flexShrink(0.0F).visible(!this.checkCondition2())),
               this.createComponentKeyService12("friends.add.input", "Player name or UUID", this.text4, 36, item -> {
                  this.text4 = item;
                  this.text6 = "";
                  this.updateState();
               }),
               MaterialTextService.text(
                     this.text6.isEmpty()
                        ? (this.checkCondition2() ? "Combat protection starts enabled." : "New friends start with combat protection enabled.")
                        : this.text6,
                     12.0F,
                     this.text6.isEmpty() ? MaterialIsLightService.ON_SURFACE_VARIANT : MaterialIsLightService.ERROR
                  )
               ...
               ComponentBoxService.row(
                  ...
                  MaterialTextService.button("friends.add.cancel", "Cancel", this::updateState4, false),
                  MaterialTextService.button("friends.add.confirm", "Add friend", this::updateState5, true)
   ...
   private void updateState6(FriendsScreen.Edit edit, String text) {
      try {
         ...
         this.text5 = iOExceptionIllegalArgumentException instanceof IOException
   ...
   public boolean keyPressed(ScreenScreenIdService screenScreenId, int value, int currentValue) {
      ...
      if (value == 257
         && this.enabled5
         && this.materialTerrainTooltipsService.findById("friends.add.input") instanceof ControlLetterSpacingService controlLetterSpacing
      ...
      if (value == 70 && (currentValue & 10) != 0) {
         if (!this.enabled5) {
            this.enabled2 = false;
            this.updateState();
            if (this.materialTerrainTooltipsService.findById("friends.search") instanceof ControlLetterSpacingService currentControlLetterSpacing) {
         ...
         return this.materialKeyService.key(this.enabled5 ? this.materialTerrainTooltipsService.findById("friends.add.dialog") : this.materialTerrainTooltipsService, value, currentValue);
      }
   }
