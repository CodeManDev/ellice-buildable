...
public final class OverlayOnActivateHandler {
   ...
   public OverlayOnActivateHandler(SceneDtService sceneDt, CompositorPushPresentationScaleService compositorPushPresentationScale) {
      ...
      this.sceneCornerRadiusService
         .onHoverChange(
            item -> this.sceneCornerRadiusService.animate("scale", item ? 1.02F : 1.0F, SceneEaseHandler.Spring.SNAPPY)
         );
      ...
      this.sceneColorService = new SceneColorService()
         .position(0.0F, 0.0F)
         .size(30.0F, 30.0F)
         .cornerRadius(15.0F)
      ...
      this.sceneTextureService = new SceneTextureService()
         .position(0.0F, 0.0F)
         .size(30.0F, 30.0F)
         .cornerRadius(15.0F);
      ...
      LayoutContainerNode currentSize = new LayoutContainerNode()
         ...
         .gap(1.0F)
         .size(-1.0F, -1.0F);
      this.sceneTextService = new SceneTextService()
         .text("Player")
         .fontSize(10.0F)
         .color(-434628574)
         .fontVariant(TextMode.BOLD)
         .size(-1.0F, 12.0F);
      this.sceneTextService2 = new SceneTextService()
   ...
   private void updateState2(String text) {
      ...
      if (uUID != null) {
         ...
         if (this.compositorPushPresentationScaleService.getNamedTexture(currentText) != null && this.compositorPushPresentationScaleService.getNamedTexture(currentText).valid()) {
            this.updateState3(currentText);
         } else if (this.text2.putIfAbsent(currentText, Boolean.TRUE) == null) {
            AccountFetchHeadService.fetchHead(uUID).whenComplete((item, currentItem) -> minecraft.execute(() -> {
               if (currentItem == null && item != null) {
                  this.compositorPushPresentationScaleService.registerImageTexture(currentText, item);
   ...
   private static boolean checkCondition(Screen screen) {
      return screen instanceof TitleScreen || screen instanceof JoinMultiplayerScreen || screen instanceof SelectWorldScreen;
   }
}
