...
public final class WorldCreationScreen implements ScreenOperationHandler {
   private static final MotionFiniteService motionFiniteService = MotionFiniteService.finite("knobPosition", item -> item instanceof ControlCompactService);
   ...
   private ComponentKeyService<?> createComponentKeyService(ComponentThemeService componentTheme) {
      ...
      arrayList.add(
         ComponentBoxService.column(
               ...
               MaterialTextService.text("Something only you could build.", 28.0F, MaterialIsLightService.ON_SURFACE)
                  .props(item -> item.lineHeight(36.0F).wordWrap(true)),
               MaterialTextService.text(
                  "Choose how your next adventure begins.", 14.0F, MaterialIsLightService.ON_SURFACE_VARIANT
               )
            )
            .key("intro")
      );
      arrayList.add(
         this.createComponentKeyService3(
            "creation.name", "World name", "My world", currentState.name(), this.menuHardcoreHandler::name, 80, "The name shown in your worlds library."
         )
      );
      arrayList.add(
         ComponentBoxService.column(
               item -> item.gap(4.0F),
               MaterialTextService.text("Game mode", 14.0F, MaterialIsLightService.ON_SURFACE_VARIANT),
               ComponentBoxService.row(
                  item -> item.gap(4.0F),
                  this.createComponentKeyService4(MenuHardcoreHandler.Mode.SURVIVAL, currentState),
                  this.createComponentKeyService4(MenuHardcoreHandler.Mode.CREATIVE, currentState),
                  this.createComponentKeyService4(MenuHardcoreHandler.Mode.HARDCORE, currentState)
               )
            )
            .key("modes")
      );
      arrayList.add(
         ComponentBoxService.row(
            ...
            this.createComponentKeyService5(
                  ...
                  () -> this.updateState(
                     () -> this.menuHardcoreHandler
                        .difficulty(
                           MenuHardcoreHandler.Difficulty.values()[(currentState.difficulty().ordinal() + 1) % MenuHardcoreHandler.Difficulty.values().length]
                        )
                  ),
            ...
            this.createComponentKeyService5(
                  ...
                  "Click to change the landscape generator. Amplified worlds need more processing power.",
                  () -> this.updateState(() -> this.menuHardcoreHandler.worldType(Math.floorMod(currentState.worldType() + 1, currentState.worldTypes().size()))),
      ...
      arrayList.add(
         MaterialTextService.button(
               "creation.random",
               "Roll a new seed",
               () -> this.updateState(() -> this.menuHardcoreHandler.seed(Long.toString(ThreadLocalRandom.current().nextLong()))),
               0,
               MaterialIsLightService.PRIMARY,
               20.0F
            )
            .children(MaterialTextService.icon("refresh", MaterialIsLightService.PRIMARY), MaterialTextService.label("Roll a new seed", MaterialIsLightService.PRIMARY))
      ...
      arrayList.add(
         ComponentBoxService.column(
            ...
            this.createComponentKeyService6(
               "creation.commands",
               "Allow commands",
               currentState.hardcore() ? "Unavailable in Hardcore." : "Enable commands such as /gamemode and /tp.",
            ...
            this.createComponentKeyService6(
               "creation.bonus",
               "Bonus chest",
               currentState.hardcore() ? "Unavailable in Hardcore." : "Start with a chest of supplies near spawn.",
      ...
      ComponentKeyService<?> currentHeight = ComponentBoxService.column(
            item -> item.id("creation.scroll")
               .flex(1.0F)
               .minWidth(0.0F)
               .minHeight(0.0F)
         ...
         ComponentBoxService.panel(
            ...
            ComponentBoxService.row(
               item -> item.id("creation.toolbar")
                  ...
                  .align(ScenePctService.Align.CENTER)
                  .flexShrink(0.0F),
               MaterialTextService.iconButton("creation.back", "arrow_back", "Back to your worlds · Esc", this::updateState4),
               MaterialTextService.text("Create your world", 22.0F, MaterialIsLightService.ON_SURFACE)
            ...
            ComponentBoxService.row(
               ...
               currentHeight,
               this.createComponentKeyService2(currentState).props(item -> item.visible(this.value2 >= 900.0F))
            ),
            ComponentBoxService.row(
               item -> item.id("creation.actions")
                  ...
                  .flexShrink(0.0F),
               MaterialTextService.text(
                     currentState.hardcore() ? "One life. Make it count." : "Ready when you are.",
                     14.0F,
                     currentState.hardcore() ? MaterialIsLightService.TERTIARY : MaterialIsLightService.ON_SURFACE_VARIANT
                  )
                  .props(item -> item.flex(1.0F).minWidth(0.0F).visible(!this.checkCondition())),
               MaterialTextService.button(
                     "creation.cancel", "Cancel", this::updateState4, 0, MaterialIsLightService.PRIMARY, 20.0F
                  )
                  .props(item -> item.visible(this.checkCondition())),
               MaterialTextService.button("creation.create", "Create world", this::updateState3, true)
                  .props(item -> item.available(!currentState.name().isBlank() && !this.enabled).flex(this.checkCondition() ? 1.0F : 0.0F))
                  .children(
                     MaterialTextService.icon("play_arrow", MaterialIsLightService.ON_PRIMARY), MaterialTextService.label("Create world", MaterialIsLightService.ON_PRIMARY)
                  )
            )
         )
      );
   ...
   private ComponentKeyService<?> createComponentKeyService2(MenuHardcoreHandler.State state) {
      return ComponentBoxService.column(
         item -> item.width(LayoutOperationHandler.px(260.0F))
            .height(LayoutOperationHandler.percent(100.0F))
            .flexShrink(0.0F)
            .gap(16.0F)
            .scrollable(true)
            .clip(true),
         ComponentBoxService.panel(
            item -> item.backgroundColor(MaterialIsLightService.PRIMARY_CONTAINER)
               .cornerRadius(24.0F)
               .padding(24.0F)
               .direction(ScenePctService.Direction.COLUMN)
               .gap(16.0F),
            ComponentBoxService.node(
               ...
               item -> item.shape(state.hardcore() ? "cookie9" : "clover4")
                  .active(true)
                  .tint(state.hardcore() ? MaterialIsLightService.TERTIARY : MaterialIsLightService.PRIMARY)
                  .size(96.0F, 96.0F)
            ),
            MaterialTextService.text(createText(state), 24.0F, MaterialIsLightService.ON_PRIMARY_CONTAINER)
               .props(item -> item.wordWrap(true)),
            MaterialTextService.text("Your world, your rules.", 14.0F, MaterialIsLightService.ON_PRIMARY_CONTAINER)
         ),
         MaterialTextService.text(
               state.name().isBlank() ? "Give your world a name" : state.name(), 22.0F, MaterialIsLightService.ON_SURFACE
            )
            .props(item -> item.wordWrap(true)),
         MaterialTextService.text(
         ...
         MaterialTextService.text(state.folder(), 14.0F, MaterialIsLightService.ON_SURFACE).props(item -> item.wordWrap(true)),
         MaterialTextService.text(
               state.hardcore()
                  ? "Hardcore uses hard difficulty and gives you one life. Commands and the bonus chest are disabled."
                  : "Your world appears in the library after creation.",
               14.0F,
               state.hardcore() ? MaterialIsLightService.TERTIARY : MaterialIsLightService.ON_SURFACE_VARIANT
            )
            .props(item -> item.wordWrap(true))
      );
   }

   private ComponentKeyService<?> createComponentKeyService3(String currentText, String nextText, String previousText, String sourceText, Consumer<String> consumer, int value, String targetText) {
      return ComponentBoxService.column(
            item -> item.gap(6.0F),
   ...
   private ComponentKeyService<?> createComponentKeyService4(MenuHardcoreHandler.Mode currentMode, MenuHardcoreHandler.State state) {
      return MaterialTextService.button(
         ...
         .props(item -> {
            item.flex(1.0F).flexShrink(1.0F).minWidth(0.0F).padding(0.0F, 8.0F).tooltip(switch (currentMode) {
               case SURVIVAL -> "Gather resources, craft and survive.";
               case CREATIVE -> "Unlimited blocks and flight for building.";
               case HARDCORE -> "Hard difficulty. One life. Commands and bonus chest are disabled.";
            });
         });
   }

   private ComponentKeyService<?> createComponentKeyService5(String currentText, String nextText, String previousText, Runnable runnable, boolean enabled) {
      return ComponentBoxService.column(
         ...
         MaterialTextService.button("creation." + currentText, nextText, runnable, false)
            ...
            .children(
               MaterialTextService.label(nextText, enabled ? MaterialIsLightService.ON_SECONDARY_CONTAINER : MaterialIsLightService.ON_SURFACE_VARIANT)
                  .props(item -> item.flex(1.0F).minWidth(0.0F)),
   ...
   private ComponentKeyService<?> createComponentKeyService6(String currentText, String nextText, String previousText, boolean enabled, boolean currentEnabled, Consumer<Boolean> consumer) {
      return MaterialTextService.button(
            currentText, nextText, () -> this.updateState(() -> consumer.accept(!enabled)), 0, MaterialIsLightService.ON_SURFACE, 16.0F
         )
         .props(
            item -> item.available(currentEnabled)
               .height(LayoutOperationHandler.px(56.0F))
               .tooltip(previousText)
               .padding(0.0F, 8.0F)
         )
         .children(
            MaterialTextService.text(nextText, 14.0F, currentEnabled ? MaterialIsLightService.ON_SURFACE : MaterialIsLightService.ON_SURFACE_VARIANT)
               .props(item -> item.flex(1.0F).minWidth(0.0F)),
            ComponentBoxService.node(
               "creation-switch",
               ...
               item -> {
                  ...
                  if (item.value() != enabled) {
                     ...
                     MotionAnimateService.animate(item, motionFiniteService, enabled ? 1.0F : 0.0F, MaterialIsLightService.FAST_SPATIAL);
                  }
               }
            )
         );
   ...
   private static String createText(MenuHardcoreHandler.State state) {
      return state.worldType() >= 0 && state.worldType() < state.worldTypes().size() ? state.worldTypes().get(state.worldType()) : "Custom";
   }
   ...
   public boolean keyPressed(ScreenScreenIdService screenScreenId, int value, int currentValue) {
      ...
      if (value == 70 && (currentValue & 10) != 0) {
         if (this.materialTerrainTooltipsService.findById("creation.name") instanceof ControlLetterSpacingService controlLetterSpacing) {
            this.materialKeyService.clear(this.materialTerrainTooltipsService);
