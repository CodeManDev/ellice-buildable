...
public final class KeybindKeyService extends ScenePctService<KeybindKeyService> {
  private static final MotionFiniteService motionFiniteService =
      MotionFiniteService.finite("keyTravel", item -> item instanceof KeybindKeyService);
  private static final MotionFiniteService motionFiniteService2 =
      MotionFiniteService.finite("keySelection", item -> item instanceof KeybindKeyService);
  private static final MotionFiniteService motionFiniteService3 =
      MotionFiniteService.finite("keyPulse", item -> item instanceof KeybindKeyService);
  private float value;
  ...
  public KeybindKeyService key(
    ...
    if (this.enabled != currentEnabled) {
      ...
      MotionAnimateService.animate(
          this, motionFiniteService2, currentEnabled ? 1.0F : 0.0F, MaterialIsLightService.EFFECTS);
    }
  ...
  protected void onPress(float value, float currentValue) {
    MotionAnimateService.animate(
        this, motionFiniteService, 1.0F, MaterialIsLightService.FAST_SPATIAL);
  }
  ...
  protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
    ...
    float height = Math.max(-0.3F, Math.min(1.0F, this.value + this.value3 * 0.35F));
    ...
    int resultValue =
        MaterialIsLightService.layer(
            this.enabled3 ? MaterialIsLightService.OUTLINE : MaterialIsLightService.ON_SURFACE,
    ...
    compositorPushPresentationScale.roundedRect(
        ...
        mulAlpha(
            MaterialIsLightService.PRIMARY, this.effectiveOpacity * (width * 0.18F + y * 0.08F)),
        0.6F,
        mulAlpha(
            MaterialIsLightService.layer(
                MaterialIsLightService.OUTLINE_VARIANT, MaterialIsLightService.PRIMARY, width),
    ...
    if (this.value3 > 0.01F) {
      ...
      compositorPushPresentationScale.roundedRect(
          ...
          mulAlpha(
              ...
              this.effectiveOpacity * Math.max(0.0F, this.value3) * 0.6F),
          1.0F);
    }

    float currentLength =
        this.text2.length() > 4 ? 10.0F : (this.text2.length() > 1 ? 11.0F : 15.0F);
    float selectedValue =
        compositorPushPresentationScale.textWidth(
            this.text2, currentLength, TextMode.REGULAR, "material-roboto-medium");
            ...
            ? nextValue
                + (sourceValue
                        - compositorPushPresentationScale.textLineHeight(
                            currentLength, TextMode.REGULAR, "material-roboto-medium"))
                    / 2.0F
            : nextValue + 7.0F;
    ...
    if (sourceValue >= 36.0F && !this.text3.isBlank()) {
      ...
      if (compositorPushPresentationScale.textWidth(
              currentText, initialValue, TextMode.REGULAR, "material-roboto")
          > previousValue - 14.0F) {
