...
final class TimelineScrubber extends ScenePctService<TimelineScrubber> {
   ...
   TimelineScrubber enabled(boolean currentEnabled) {
      ...
      this.cursorStyle(currentEnabled ? ScenePctService.CursorStyle.POINTER : ScenePctService.CursorStyle.DEFAULT);
      return this;
   }

   @Override
   protected void onPress(float currentValue, float nextValue) {
      if (!this.notebotDecoderTracker.importing() && this.notebotDecoderTracker.playback().song() != null) {
   ...
   private void updateState(boolean enabled) {
      if (this.enabled2) {
         this.enabled2 = false;
         if (this.notebotTitleService2 == this.notebotDecoderTracker.playback().song() && !this.notebotDecoderTracker.importing()) {
            if (enabled) {
               this.notebotDecoderTracker.seek(this.value);
            }

            if (this.enabled3 && this.notebotDecoderTracker.playback().phase() == NotebotRepository.Phase.PAUSED) {
               this.notebotDecoderTracker.playPause();
            }
         }
   ...
   protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      ...
      float currentValue = notebotTitle == null ? 0.0F : (float)(this.notebotDecoderTracker.playback().displayPosition() / notebotTitle.durationTicks());
      ...
      if (this.enabled2 && notebotTitle != null) {
         ...
         float previousValue = compositorPushPresentationScale.textWidth(currentText, nextValue, TextMode.REGULAR, "material-roboto");
         float sourceValue = Math.max(this.cx, Math.min(this.cx + this.cw - previousValue, this.cx + this.value * this.cw - previousValue / 2.0F));
         compositorPushPresentationScale.text(sourceValue, this.cy + this.ch - nextValue, currentText, nextValue, mulAlpha(MaterialIsLightService.ON_SURFACE, this.effectiveOpacity), MaterialIsLightService.BODY);
      }
   }
}
