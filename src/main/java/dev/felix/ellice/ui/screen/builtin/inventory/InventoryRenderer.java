...
public final class InventoryRenderer implements ComponentOperationHandler {
  private InventoryRoleData inventoryRoleData;
  private int count;
  private boolean enabled;
  private float value = Float.intBitsToFloat(1146552320);
  private ComponentThemeService componentThemeService;

  public InventoryRenderer(InventoryRoleData inventoryRoleData) {
    this.inventoryRoleData = inventoryRoleData;
  }

  public InventoryRoleData draft() {
    return this.inventoryRoleData;
  }
  ...
  public void select(int n) {
    if (this.enabled) {
      this.inventoryRoleData = this.inventoryRoleData.swap(this.count, n);
      this.enabled = false;
    }
    this.count = n;
    this.m5pkfdqfrjqs();
  ...
  public void assign(InventoryRoleData.Role role) {
    this.inventoryRoleData = this.inventoryRoleData.with(this.count, role);
    this.enabled = false;
    this.m5pkfdqfrjqs();
  }

  private void updateState(InventoryRoleData inventoryRoleData) {
    this.inventoryRoleData = inventoryRoleData;
    this.enabled = false;
    this.m5pkfdqfrjqs();
  }
  ...
  public boolean keyPressed(int n, int n2) {
    ...
    if ((n2 & 1) != 0 && (n == 263 || n == 262)) {
      int n3 = Math.clamp((long) (this.count + (n == 263 ? -1 : 1)), 0, 8);
      this.inventoryRoleData = this.inventoryRoleData.swap(this.count, n3);
      this.count = n3;
      this.m5pkfdqfrjqs();
      return true;
    }
  ...
  public ComponentKeyService<?> render(ComponentThemeService componentThemeService) {
    ...
    int n3 = this.value < Float.intBitsToFloat(0x44200000) ? 1 : 0;
    ArrayList arrayList = new ArrayList();
    arrayList.add(
        MaterialTextService.text(
                "Select a slot, then choose what belongs there. The best matching item fills each"
                    + " role.",
    ...
    arrayList2.add(
        MaterialTextService.button(
            "hotbar.preset.balanced",
            "Balanced",
            () -> this.updateState(InventoryRoleData.BALANCED),
            this.inventoryRoleData.equals(InventoryRoleData.BALANCED)));
    arrayList2.add(
        MaterialTextService.button(
            "hotbar.preset.combat",
            "Combat",
            () -> this.updateState(InventoryRoleData.COMBAT),
            this.inventoryRoleData.equals(InventoryRoleData.COMBAT)));
    arrayList2.add(
        MaterialTextService.button(
            "hotbar.preset.building",
            "Building",
            () -> this.updateState(InventoryRoleData.BUILDING),
            this.inventoryRoleData.equals(InventoryRoleData.BUILDING)));
    arrayList.add(
                        ...
                        .map(
                            componentKeyService ->
                                componentKeyService.props(
                                    scenePctService ->
                                        ((ScenePctService)
                                                ((ScenePctService) scenePctService.flex(1.0f))
                                                    .minWidth(0.0f))
                                            .padding(0.0f, Float.intBitsToFloat(0x40800000))))
                        .toArray(ComponentKeyService[]::new))
            .key("presets"));
    int n4 = n3 != 0 ? 3 : 9;
    ArrayList<ComponentKeyService<LayoutContainerNode>> arrayList3 =
                  ...
                  layoutContainerNode ->
                      ((LayoutContainerNode)
                              ((LayoutContainerNode)
                                  ...
                                  .gap(Float.intBitsToFloat(0x40C00000)))
                          .flexShrink(0.0f),
                  (ComponentKeyService[]) roleArray.toArray(ComponentKeyService[]::new))
              .key("row-" + n2));
    }
    arrayList.add(
    ...
    arrayList.add(
        ComponentBoxService.row(
                    ...
                    (String)
                        (this.enabled
                            ...
                            : "Slot "
                                + (this.count + 1)
                                + " \u00b7 "
                                + this.inventoryRoleData.slots().get(this.count).title()),
            ...
            MaterialTextService.button(
                    "hotbar.swap",
                    this.enabled ? "Cancel swap" : "Swap slots",
                    () -> {
                ...
                .props(
                    materialJoinedService ->
                        materialJoinedService.tooltip(
                            "Swap two slot roles \u00b7 Or use Shift + Left / Right to move the"
                                + " selected role"))));
    n2 = n3 != 0 ? 2 : 4;
    ...
    for (n = 0; n < roles.length; n += n2) {
      ...
      for (int i = n; i < Math.min(roles.length, n + n2); ++i) {
        arrayList4.add(this.createComponentKeyService2(roles[i]));
      }
      arrayList.add(
          ComponentBoxService.row(
                  layoutContainerNode ->
                      ((LayoutContainerNode)
                              ((LayoutContainerNode)
                                  ...
                                  .gap(Float.intBitsToFloat(0x40C00000)))
                          .flexShrink(0.0f),
                  (ComponentKeyService[]) arrayList4.toArray(ComponentKeyService[]::new))
              .key("roles-" + n));
    }
    arrayList.add(
        MaterialTextService.text(
                "Locked slots stay untouched. Missing items leave their slots alone. Open your"
                    + " inventory to apply the arrangement in game.",
    ...
    return ComponentBoxService.node(
        "hotbar-layout-editor",
        MaterialTerrainTooltipsService::new,
  ...
  private ComponentKeyService<?> mv36lcf1ym5(int n) {
    InventoryRoleData.Role role = this.inventoryRoleData.slots().get(n);
    boolean bl = this.count == n;
    ...
    return ComponentBoxService.node(
            ...
            materialJoinedService -> {
              materialJoinedService
                  .colors(
                      bl ? MaterialIsLightService.PRIMARY : MaterialIsLightService.SURFACE_HIGH, n2)
                  .shape(Float.intBitsToFloat(0x41400000), Float.intBitsToFloat(1119879168));
              ((SceneCornerRadiusService)
                      ((SceneCornerRadiusService)
                              ((SceneCornerRadiusService)
                                      ((SceneCornerRadiusService)
            ...
            MaterialTextService.text(Integer.toString(n + 1), Float.intBitsToFloat(1092616192), n2),
            ComponentBoxService.node(
                "hotbar-item",
                InventoryRoleService::new,
            ...
            MaterialTextService.text(role.title(), Float.intBitsToFloat(1092616192), n2)
                .props(
                    sceneTextService ->
                        ((SceneTextService)
                                sceneTextService.maxWidth(
                                    LayoutOperationHandler.percent(
                                        Float.intBitsToFloat(1120403456))))
                            .minWidth(0.0f)))
        .key("slot-" + n)
        .onMount(
            materialJoinedService -> {
              ...
              MotionAnimateService.animate(
                  ...
                  (float) n * Float.intBitsToFloat(1016296636));
              MotionAnimateService.animate(
                  materialJoinedService,
                  MotionColorsContainer.Floats.TRANSLATE_Y,
                  0.0f,
                  (SceneEaseHandler) MaterialIsLightService.SPATIAL,
                  (float) n * Float.intBitsToFloat(1016296636));
            });
  }

  private ComponentKeyService<?> createComponentKeyService2(InventoryRoleData.Role role) {
    boolean bl = this.inventoryRoleData.slots().get(this.count) == role;
    int n =
        bl
            ...
            materialJoinedService -> {
              materialJoinedService
                  .colors(
                      ...
                      n)
                  .shape(Float.intBitsToFloat(0x41400000), Float.intBitsToFloat(0x42400000))
                  .outlined(bl);
              ((SceneCornerRadiusService)
                      ((SceneCornerRadiusService)
                              ((SceneCornerRadiusService)
            ...
            ComponentBoxService.node(
                ...
                inventoryRoleService ->
                    ((InventoryRoleService)
                        ...
                        .pointerEvents(false),
                new ComponentKeyService[0]),
            MaterialTextService.text(role.title(), Float.intBitsToFloat(1095761920), n)
                .props(
                    sceneTextService ->
                        ((SceneTextService) sceneTextService.flex(1.0f)).minWidth(0.0f)))
        .key(role.name());
  ...
  public static ComponentKeyService<?> preview(
      String string, ModuleLayoutService moduleLayoutService, Runnable runnable) {
            ...
            materialJoinedService -> {
              materialJoinedService
                  .colors(
                      MaterialIsLightService.SECONDARY_CONTAINER,
                      MaterialIsLightService.ON_SECONDARY_CONTAINER)
                  .shape(Float.intBitsToFloat(0x41400000), Float.intBitsToFloat(1112539136))
                  .available(moduleLayoutService.isActive());
              ((SceneCornerRadiusService)
                      ((SceneCornerRadiusService)
                              ((SceneCornerRadiusService)
                                      ((SceneCornerRadiusService)
                                  ...
                                  .align(ScenePctService.Align.CENTER))
                          .tooltip(
                              "Edit all nine hotbar slots \u00b7 Choose roles, lock slots, and save"
                                  + " presets"))
                  .onClick(runnable);
            },
            (ComponentKeyService[]) arrayList.toArray(ComponentKeyService[]::new))
