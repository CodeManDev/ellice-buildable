...
final class BezierConnectionRenderer {
   static void draw(
      CompositorPushPresentationScaleService compositorPushPresentationScale, float y, float width, float height, float value, int currentValue, float nextValue, float previousValue, float sourceValue, boolean enabled
   ) {
      if (compositorPushPresentationScale.vectorRenderer() != null) {
         int targetValue = Math.max(24, Math.min(96, (int)(Math.hypot(height - y, value - width) / 8.0)));
         ...
         for (int index = 0; index <= targetValue; index++) {
            ...
            float selectedValue = 3.0F * candidateValue * candidateValue * inputValue
         ...
         float[] previousFloats = new float[]{
         ...
         for (int nextIndex = 0; nextIndex < targetValue; nextIndex++) {
            for (int previousIndex = 0; previousIndex < 6; previousIndex++) {
               ...
               floats[targetIndex] = currentFloats[sourceIndex] + currentFloats[sourceIndex + 2] * previousFloats[previousIndex];
               floats[targetIndex + 1] = currentFloats[sourceIndex + 1] + currentFloats[sourceIndex + 3] * previousFloats[previousIndex];
         ...
         if (enabled) {
            float[] sourceFloats = createFloat(y, width, height, value, inputValue, sourceValue * 0.24F % 1.0F);
   ...
   private static float[] createFloat(float value, float currentValue, float nextValue, float previousValue, float sourceValue, float targetValue) {
