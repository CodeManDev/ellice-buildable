...
final class ServerListRenderer implements ComponentOperationHandler {
   ...
   ComponentKeyService<?> previewDetails(ComponentThemeService componentTheme, String text, String currentText) {
      this.componentThemeService = componentTheme;
      return this.createComponentKeyService(new ServerListRenderer.Server(text, currentText, this.serverViewTracker.library().usage(this.uUID, text)));
   }
   ...
   ServerListRenderer(ServerViewTracker currentServerViewTracker, BiConsumer<String, String> biConsumer) {
   ...
   void account(UUID currentUUID, String text, boolean enabled) {
      if (!Objects.equals(currentUUID, this.uUID) || !Objects.equals(text, this.text4) || this.enabled3 != enabled) {
   ...
   void tick(boolean enabled) {
      if (enabled) {
         if (this.enabled7 && this.scenePctService != null) {
            ...
            if (scenePct != null) {
               float value = Math.max(
                  ...
                  Math.min(0.0F, this.scenePctService.scrollY() + this.scenePctService.computedY() - scenePct.computedY())
               );
               this.scenePctService.scrollTarget(value);
               MotionAnimateService.animate(this.scenePctService, MotionColorsContainer.Floats.SCROLL_Y, value, MaterialIsLightService.SPATIAL);
               this.enabled7 = false;
            }
         }
         ...
         if (longValue >= this.timestamp4) {
            ...
            for (ServerListRenderer.Server currentServer : (Iterable<ServerListRenderer.Server>) (Iterable<?>) (items.stream().limit(64L).toList())) {
               this.serverViewTracker.seedIcon(currentServer.address(), null);
               this.serverViewTracker.request(currentServer.address(), false);
            }

            if (this.timestamp2 != this.serverViewTracker.revision() || longValue - this.timestamp3 > 5000L) {
   ...
   private void updateState2() {
      if (this.scenePctService != null) {
         ...
         MotionAnimateService.animate(this.scenePctService, MotionColorsContainer.Floats.SCROLL_Y, 0.0F, MaterialIsLightService.SPATIAL);
      }
   }
   ...
   public ComponentKeyService<?> render(ComponentThemeService componentTheme) {
      ...
      arrayList.add(
         ComponentBoxService.row(
               ...
               ComponentBoxService.column(
                  ...
                  MaterialTextService.text(
                        this.text4 + " · " + longValue + (longValue == 1L ? " favourite" : " favourites"),
                        14.0F,
                        MaterialIsLightService.ON_SURFACE_VARIANT
                     )
                     .props(item -> item.visible(!this.enabled6))
               ),
               MaterialTextService.iconButton("servers.add", "add", "Add server", () -> {
      ...
      arrayList.add(MaterialTextService.search("servers.search", "Search servers", this.text6, item -> {
         this.text6 = item;
         this.updateState();
      }, this.enabled6));
      arrayList.add(
      ...
      for (ServerListRenderer.Server nextServer : (Iterable<ServerListRenderer.Server>) (Iterable<?>) (items)) {
         currentArrayList.add(this.createComponentKeyService2(nextServer, currentServer));
      }

      if (items.isEmpty()) {
         String currentText = !this.text6.isBlank()
            ? "No matching servers"
            : (
               this.renderer == ServerListRenderer.Filter.FAVORITES
                  ? "Keep your favourites close"
                  : (this.renderer == ServerListRenderer.Filter.RECENT ? "Your next session starts here" : "Find your next place to play")
            );
         String nextText = !this.text6.isBlank()
         ...
         currentArrayList.add(
            ComponentBoxService.column(
                  item -> item.id("servers.empty")
                     .padding(24.0F, 12.0F)
                     .gap(8.0F),
                  MaterialTextService.text(currentText, 20.0F, MaterialIsLightService.ON_SURFACE).props(item -> item.wordWrap(true)),
      ...
      if (!this.serverViewTracker.saveError().isBlank()) {
         arrayList.add(createComponentKeyService8(this.serverViewTracker.saveError(), MaterialIsLightService.ERROR).key("save-error"));
      }

      return currentValue != 0
         ? ComponentBoxService.row(
            item -> item.id("servers.pane")
               .size(LayoutOperationHandler.percent(100.0F), LayoutOperationHandler.percent(100.0F))
               .gap(16.0F),
            ComponentBoxService.column(
                  item -> item.id("server-dashboard-scroll")
            ...
            ComponentBoxService.column(
               item -> item.id("servers.inspector")
               ...
               currentServer == null
                  ? ComponentBoxService.column(
                     ...
                     MaterialTextService.text("Make yourself at home", 22.0F, MaterialIsLightService.ON_SURFACE)
                        .props(item -> item.wordWrap(true)),
                     createComponentKeyService8("Choose a server to see its players, latency and your play history.", MaterialIsLightService.ON_SURFACE_VARIANT)
                  )
   ...
   private ComponentKeyService<?> createComponentKeyService(ServerListRenderer.Server currentServer) {
      ...
      String currentText = currentView.checkedAt() == 0L
         ? "Checking server…"
         : (
            currentView.loading()
               ? "Refreshing…"
      ...
      String nextText = value != 0 && currentStatus.players() >= 0 ? createText(currentStatus.players()) : "—";
      String previousText = value != 0 && currentStatus.capacity() >= 0 ? "of " + createText(currentStatus.capacity()) + " players" : "Players online";
      String sourceText = value != 0 && currentStatus.latencyMs() >= 0L ? currentStatus.latencyMs() + " ms" : "—";
      String targetText = value != 0 && !currentStatus.version().isBlank() ? currentStatus.version() : "Not available";
      CompositorEmptyService compositorEmpty = this.enabled2 ? currentView.ping() : currentView.players();
      String inputText = this.enabled2 ? "Latency" : "Players online";
      String outputText = compositorEmpty.validCount() == 0L
         ? "Waiting for a measurement"
         : (compositorEmpty.validCount() == 1L ? "First sample · collecting every 30s" : "Live samples · every 30s");
      ServerKeyService.Usage currentUsage = currentServer.usage();
      String resultText = currentUsage.joins() == 0
      ...
      return ComponentBoxService.panel(
         ...
         ComponentBoxService.row(
            ...
            ComponentBoxService.column(
               item -> item.flex(1.0F).minWidth(0.0F).gap(4.0F),
               MaterialTextService.text(currentServer.name(), 20.0F, MaterialIsLightService.ON_SURFACE),
               MaterialTextService.text(currentServer.address(), 12.0F, MaterialIsLightService.ON_SURFACE_VARIANT)
                  .props(item -> item.tooltip(currentServer.address()))
            ),
            MaterialTextService.iconButton("servers.copy", "content_copy", "Copy server address", () -> CompatSavedService.copyAddress(currentServer.address()))
         ),
         createComponentKeyService8(currentText, value != 0 ? MaterialIsLightService.TERTIARY : MaterialIsLightService.ON_SURFACE_VARIANT),
         MaterialTextService.button("servers.preview", "Server preview", () -> {
               if (this.text3 != null) {
                  this.text3.accept(currentServer.address(), currentServer.name());
               }
            }, false)
            .props(
               item -> item.visible(this.text3 != null)
                  .tooltip("Inspect live status, graphs and the server's public player sample without joining.")
            ),
         ComponentBoxService.row(
            item -> item.gap(8.0F),
            MaterialTextService.button(
                  "servers.join",
                  this.enabled3 ? (currentUsage.joins() > 0 ? "Play again" : "Join server") : "In game",
                  () -> {
                     if (this.enabled3) {
            ...
            MaterialTextService.iconButton("servers.refresh", "refresh", "Refresh server status", () -> {
               this.serverViewTracker.request(currentServer.address(), true);
               this.updateState();
            }).props(item -> item.available(!currentView.loading()))
         ),
         ComponentBoxService.row(
            item -> item.gap(8.0F), this.createComponentKeyService5(nextText, previousText), this.createComponentKeyService5(sourceText, "Round-trip ping")
         ),
         MaterialTextService.text("Minecraft · " + targetText, 12.0F, MaterialIsLightService.ON_SURFACE_VARIANT)
         ...
         createComponentKeyService8(
               currentStatus == null
                  ...
                  : (
                     value == 0
                        ? "The server may be offline or have status disabled. You can still try joining."
                        : (currentStatus.motd().isBlank() ? "This server has no description." : currentStatus.motd())
                  ),
               MaterialIsLightService.ON_SURFACE_VARIANT
            )
         ...
         ComponentBoxService.row(
            ...
            MaterialTextService.iconButton("servers.graph.toggle", "swap_horiz", this.enabled2 ? "Show players" : "Show latency", () -> {
         ...
         ComponentBoxService.row(
            item -> item.gap(8.0F),
            ComponentBoxService.column(
               item -> item.width(LayoutOperationHandler.px(36.0F)).justify(ScenePctService.Justify.SPACE_BETWEEN),
               MaterialTextService.text(createText((long)compositorEmpty.ceiling()), 12.0F, MaterialIsLightService.ON_SURFACE_VARIANT),
            ...
            ComponentBoxService.<SceneDataService>node(
                  ...
                  item -> item.data(compositorEmpty)
                     .color(this.enabled2 ? MaterialIsLightService.TERTIARY : MaterialIsLightService.PRIMARY)
         ...
         ComponentBoxService.row(
            item -> item.gap(8.0F).align(ScenePctService.Align.CENTER),
            MaterialTextService.text(
               compositorEmpty.size() > 0 ? dateTimeFormatter.format(Instant.ofEpochMilli(compositorEmpty.samples().getFirst().timestamp())) : "",
               12.0F,
               MaterialIsLightService.ON_SURFACE_VARIANT
            ),
            MaterialTextService.text(outputText, 12.0F, MaterialIsLightService.ON_SURFACE_VARIANT)
               .props(item -> item.flex(1.0F).minWidth(0.0F).wordWrap(true).textAlign(SceneTextService.TextAlign.CENTER)),
            MaterialTextService.text(
               compositorEmpty.size() > 1 ? dateTimeFormatter.format(Instant.ofEpochMilli(compositorEmpty.samples().getLast().timestamp())) : "",
               12.0F,
               MaterialIsLightService.ON_SURFACE_VARIANT
            )
         ),
         createComponentKeyService8(resultText, MaterialIsLightService.ON_SURFACE_VARIANT)
            .props(item -> item.tooltip("Playtime and visits recorded locally by ellice for this account."))
      );
   }

   private ComponentKeyService<?> createComponentKeyService2(ServerListRenderer.Server currentServer, ServerListRenderer.Server nextServer) {
      ServerViewTracker.View currentView = this.serverViewTracker.view(currentServer.address());
      int currentValue = nextServer != null && nextServer.address().equals(currentServer.address()) ? 1 : 0;
      String currentText = currentView.checkedAt() == 0L
         ? "Checking…"
         : (
            currentView.reachable() && currentView.status() != null
               ? (currentView.status().players() < 0 ? "Online" : createText(currentView.status().players()) + " online")
               : "Unavailable"
         );
      int nextValue = currentValue != 0 ? MaterialIsLightService.SECONDARY_CONTAINER : MaterialIsLightService.SURFACE_CONTAINER;
      int previousValue = currentValue != 0 ? MaterialIsLightService.ON_SECONDARY_CONTAINER : MaterialIsLightService.ON_SURFACE;
      return ComponentBoxService.<MaterialResponsiveService>node(
            "server-row",
            MaterialResponsiveService::new,
            item -> item.surface(nextValue)
            ...
            ComponentBoxService.node(
               ...
               ComponentBoxService.column(
                  ...
                  MaterialTextService.text(
                        this.enabled4 ? currentText : currentServer.address() + " · " + currentText,
                        12.0F,
                        currentValue != 0 ? MaterialIsLightService.ON_SECONDARY_CONTAINER : MaterialIsLightService.ON_SURFACE_VARIANT
                     )
                     .props(item -> item.tooltip(currentServer.address()))
               ),
               ComponentBoxService.node(
                  "server-sparkline",
                  SceneDataService::new,
                  item -> item.data(currentView.players())
                     .color(MaterialIsLightService.PRIMARY)
                     .size(72.0F, 28.0F)
                     .flexShrink(0.0F)
                     .pointerEvents(false)
                     .visible(this.enabled5 && this.value > 980.0F && currentView.players().validCount() > 1L)
               )
            ),
      ...
      return MaterialTextService.iconButton(
         ...
         .children(
            ComponentBoxService.<MaterialSelectedSelector>node(
                  ...
                  item -> item.selected(enabled)
                     .src(MaterialTextService.iconPath(enabled ? "star_filled" : "star"))
                     .tintColor(enabled ? MaterialIsLightService.PRIMARY : MaterialIsLightService.ON_SURFACE_VARIANT)
                     .size(24.0F, 24.0F)
                     .flexShrink(0.0F)
                     .pointerEvents(false)
   ...
   private ComponentKeyService<?> createComponentKeyService4(ServerListRenderer.Server currentServer, float value) {
      return ComponentBoxService.<SceneImageService>node(
            "server-icon",
            SceneImageService::new,
            item -> item.identity(currentServer.address(), currentServer.name())
               .material(true)
   ...
   private ComponentKeyService<?> createComponentKeyService6(String text, ServerListRenderer.Filter currentFilter) {
      int value = this.renderer == currentFilter ? 1 : 0;
      int currentValue = value != 0 ? MaterialIsLightService.ON_SECONDARY_CONTAINER : MaterialIsLightService.ON_SURFACE_VARIANT;
      return ComponentBoxService.<MaterialJoinedService>node(
            "server-filter",
            MaterialJoinedService::new,
            item -> {
               item.colors(value != 0 ? MaterialIsLightService.SECONDARY_CONTAINER : MaterialIsLightService.SURFACE_CONTAINER, currentValue)
                  .shape(20.0F, 40.0F)
                  .joined(currentFilter == ServerListRenderer.Filter.ALL, currentFilter == ServerListRenderer.Filter.RECENT, value != 0);
   ...
   private ComponentKeyService<?> createComponentKeyService7() {
      return ComponentBoxService.panel(
            item -> item.id("servers.add-form")
               ...
               .gap(16.0F)
               .flexShrink(0.0F),
            MaterialTextService.text("Add to your servers", 20.0F, MaterialIsLightService.ON_SURFACE),
            ComponentBoxService.<ControlLetterSpacingService>node(
               ...
               .key("address"),
            createComponentKeyService8(
               this.text8.isBlank() ? "Saved to Minecraft and favourited for " + this.text4 + "." : this.text8,
               this.text8.isBlank() ? MaterialIsLightService.ON_SURFACE_VARIANT : MaterialIsLightService.ERROR
            ),
            ComponentBoxService.row(
               item -> item.gap(8.0F),
               MaterialTextService.button("servers.save", "Save server", this::updateState3, true),
               MaterialTextService.button("servers.cancel", "Cancel", () -> {
                  this.enabled = false;
                  this.updateState();
               }, 0, MaterialIsLightService.PRIMARY, 20.0F)
            )
         )
         .onMount(item -> MaterialEnterService.enter(item, 0.0F, 12.0F));
   }

   private static ComponentKeyService<SceneTextService> createComponentKeyService8(String currentText, int value) {
   ...
   private List<ServerListRenderer.Server> collectValues() {
      ...
      for (CompatSavedService.Saved currentSaved : this.serverViewTracker.saved()) {
         linkedHashMap.put(
            ...
            new ServerListRenderer.Server(currentSaved.address(), currentSaved.name(), this.serverViewTracker.library().usage(this.uUID, currentSaved.address()))
         );
      }
      ...
      this.serverViewTracker
         .library()
         .history(this.uUID)
         .forEach((item, currentItem) -> linkedHashMap.putIfAbsent(item, new ServerListRenderer.Server(item, this.serverViewTracker.library().name(item), currentItem)));
      return linkedHashMap.values()
         .stream()
         .sorted(
            Comparator.<ServerListRenderer.Server, Boolean>comparing(item -> !item.usage().favorite())
               .thenComparing(Comparator.<ServerListRenderer.Server>comparingLong(item -> item.usage().lastJoined()).reversed())
   ...
   private List<ServerListRenderer.Server> collectValues2() {
      ...
      Stream currentStream = this.collectValues()
         .stream()
         .filter(item -> this.renderer != ServerListRenderer.Filter.FAVORITES || item.usage().favorite())
         .filter(item -> this.renderer != ServerListRenderer.Filter.RECENT || item.usage().joins() > 0)
         .filter(item -> item.name().toLowerCase(Locale.ROOT).contains(text) || item.address().toLowerCase(Locale.ROOT).contains(text));
      if (this.renderer == ServerListRenderer.Filter.RECENT) {
         currentStream = currentStream.sorted(Comparator.<ServerListRenderer.Server>comparingLong(item -> item.usage().lastJoined()).reversed());
      }
   ...
   private static String createText2(long size) {
      ...
      return currentSize < 1L ? "<1m" : (currentSize < 60L ? currentSize + "m" : currentSize / 60L + "h " + currentSize % 60L + "m");
   }
