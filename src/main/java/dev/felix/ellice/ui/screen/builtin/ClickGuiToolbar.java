...
final class ClickGuiToolbar implements ComponentOperationHandler {
   ...
   ClickGuiToolbar(Consumer<String> currentConsumer, Consumer<Boolean> nextConsumer, Runnable currentRunnable) {
   ...
   boolean dismissSearch() {
      if (!this.text3.isEmpty()) {
         this.updateState();
         return true;
      } else if (this.controlLetterSpacingService != null && this.controlLetterSpacingService.focused()) {
   ...
   public ComponentKeyService<?> render(ComponentThemeService componentTheme) {
      ...
      ComponentStyleService currentSize = ComponentStyleService.style()
         ...
         .build();
      ComponentKeyService<?> nextSize = ComponentBoxService.row(
            item -> item.size(LayoutOperationHandler.auto(), LayoutOperationHandler.percent(100.0F))
               .gap(8.0F)
               .align(ScenePctService.Align.CENTER)
               .flexShrink(0.0F)
               .pointerEvents(false),
            ComponentBoxService.panel(
               item -> item.size(26.0F, 26.0F)
                  .cornerRadius(8.0F)
            ...
            ComponentBoxService.column(
               item -> item.gap(1.0F).justify(ScenePctService.Justify.CENTER).visible(!this.enabled3).pointerEvents(false),
               ComponentBoxService.text(
                  "ellice",
                  item -> item.fontSize(11.0F)
               ...
               ComponentBoxService.text(
                  ...
                  item -> item.id("clickgui.toolbar.summary")
                     .fontSize(6.2F)
                     .color(-5456944)
                     .inheritEdgeSoftness(false)
               )
         ...
         .key("brand");
      ComponentKeyService<?> previousSize = ComponentBoxService.panel(
            item -> item.id("clickgui.toolbar.search-field")
            ...
            ComponentBoxService.<ControlLetterSpacingService>node(
                  ...
                  item -> item.id("clickgui.toolbar.search")
      ...
      ComponentKeyService<?> sourceSize = ComponentBoxService.panel(
            item -> item.id("clickgui.toolbar.enabled")
               .size(
               ...
               .stopPropagation(true)
               .tooltip(this.enabled2 ? "Show all modules" : "Show enabled modules (" + this.count3 + ")")
               .onClick(() -> {
   ...
   private static ComponentKeyService<SceneSrcService> createComponentKeyService(String text, float value, int currentValue) {
      return ComponentBoxService.svg(
         BuiltinComponent.iconPath(text),
         item -> item.size(value, value).flexShrink(0.0F).tintColor(currentValue).inheritEdgeSoftness(false).pointerEvents(false)
      );
   }

   private static ComponentKeyService<SceneCornerRadiusService> createComponentKeyService2(String text, String currentText, String nextText, Runnable runnable) {
      return ComponentBoxService.panel(
            item -> item.id("clickgui.toolbar." + text)
               .size(28.0F, 28.0F)
               .flexShrink(0.0F)
               .cornerRadius(7.0F)
               .backgroundColor(0)
