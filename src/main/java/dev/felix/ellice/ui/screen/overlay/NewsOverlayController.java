...
public final class NewsOverlayController {
  ...
  public NewsOverlayController(SceneDtService sceneDt, String text) {
    ...
    this.sceneCornerRadiusService.onHoverChange(
        item ->
            this.sceneCornerRadiusService.animate(
                "scale", item ? 1.035F : 1.0F, SceneEaseHandler.Spring.SNAPPY));
    ...
    this.sceneTextService =
        new SceneTextService("What's New", 7.0F, -1139271646)
            .fontVariant(TextMode.BOLD)
            .visible(false);
  ...
  public void setHasNews(boolean currentEnabled) {
    ...
    if (currentEnabled) {
      this.sceneCornerRadiusService2.loop(
          "opacity", 0.35F, 1.0F, SceneEaseHandler.Tween.ease(0.75F), true);
    } else {
