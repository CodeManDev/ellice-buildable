...
public final class KeybindTargetService {
  ...
  public KeybindTargetService(
      KeybindTargetService.Target target,
      List<KeybindTargetService.Target> currentItems,
      Map<Integer, List<String>> entries,
      Map<Integer, String> currentEntries) {
    this.target2 = target;
  ...
  public List<KeybindTargetService.Use> uses(int value) {
    ...
    for (KeybindTargetService.Target target : this.items) {
      if (!target.id.equals(this.target2.id) && target.read.getAsInt() == value) {
        arrayList.add(
            new KeybindTargetService.Use(
                target.owner + " · " + target.title, false, target.exclusive));
      }
    }
  ...
  public boolean replacesModule() {
    return this.target2.exclusive
        && this.uses(this.count).stream().anyMatch(item -> !item.minecraft && item.exclusive);
  }
  ...
  public String tooltip(int value) {
    ...
    if (items.isEmpty()) {
      stringBuilder
          .append("\nFree for ")
          .append(this.target2.owner)
          .append(" · ")
          .append(this.target2.title);
    } else {
      for (KeybindTargetService.Use use :
          (Iterable<KeybindTargetService.Use>) (Iterable<?>) (items)) {
        stringBuilder
            .append("\n")
            .append(use.minecraft ? "Minecraft · " : "ellice · ")
            .append(use.label);
      }
    }
