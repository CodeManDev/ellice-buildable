...
public class TextTextService {
  ...
  public TextTextService(
      RhiOperationHandler rhiOperation, Map<TextMode, TextTextureService> entries) {
    this(rhiOperation, entries, null);
  }

  public TextTextService(
      RhiOperationHandler rhiOperation,
      Map<TextMode, TextTextureService> currentEntries,
  ...
  public void text(
    ...
    if (currentText != null && !currentText.isEmpty()) {
      ...
      if (TextCodec.contains(currentText)) {
        this.updateState2(
            value, currentValue, currentText, nextValue, previousValue, currentTextData, floats);
      } else {
        this.updateState(
            value, currentValue, currentText, nextValue, previousValue, currentTextData, floats);
      }
  ...
  private void updateState(
    ...
    TextTextureService textTexture =
        this.createTextTextureService2(textData.fontFamily(), textMode);
    if (textData.letterSpacing() == 0.0F
        && this.checkCondition3(text, textData.fontFamily(), textMode)) {
      this.updateState3(
          textTexture,
          this.createTextTextureService22(textTexture, textMode),
          textMode,
          textData,
          floats,
          value,
  ...
  public float measureWidth(
    ...
    if (id == null) {
      ...
      TextData textData =
          TextData.NONE
              .withVariant(textMode)
              .withFontFamily(currentId)
              .withLetterSpacing(currentValue);
      return this.calculateValue3(id, value, textData);
    } else {
  ...
  private float calculateValue2(
      String text, float value, TextMode textMode, String currentText, float currentValue) {
    ...
    while (index < text.length()) {
      ...
      if (sourceValue != 13) {
        if (sourceValue == 10) {
          ...
          TextTextService.GlyphRun glyphRun =
              this.createGlyphRun(textTexture, textMode, sourceValue);
          if (glyphRun != null) {
  ...
  private float calculateValue3(String currentText, float value, TextData textData) {
    ...
    for (TextCodec.Segment segment : TextCodec.parse(currentText, -1, textData)) {
      ...
      while (true) {
        ...
        String previousText =
            sourceValue >= 0
                ? nextText.substring(previousValue, sourceValue)
                : nextText.substring(previousValue);
        nextValue +=
            this.calculateValue2(
                previousText,
                value,
  ...
  public float caretX(
      String text, int value, float currentValue, TextMode textMode, String currentText) {
    textMode = this.createTextMode(textMode);
    if (text != null && !text.isEmpty()) {
      ...
      if (TextCodec.contains(text)) {
        ...
        return this.checkCondition3(text, currentText, textMode)
            ? this.createTextTextureService22(
                        this.createTextTextureService2(currentText, textMode), textMode)
                    .caretX(text, value)
                * currentValue
  ...
  public float[] selectionSpans(
    ...
    if (text != null && !text.isEmpty() && value != currentValue) {
      ...
      if (TextCodec.contains(text)) {
        float sourceValue =
            this.measureWidth(text.substring(0, value), nextValue, textMode, currentText);
        float targetValue =
            this.measureWidth(text.substring(0, currentValue), nextValue, textMode, currentText);
        return new float[] {
          Math.min(sourceValue, targetValue), Math.abs(targetValue - sourceValue)
        };
      }
              ...
              if (batch.atlas != null) {
                ...
                if (textureHandle != null && textureHandle.valid()) {
                  rhiCommandBuffer.bindPipeline(
                      batch.color ? this.pipelineHandle2 : this.pipelineHandle);
                  rhiCommandBuffer.bindVertexBuffer(this.bufferHandle, 0);
                  rhiCommandBuffer.bindIndexBuffer(
                      this.fighjurpy8, RhiBlendStateService.IndexType.UINT32);
                  ...
                  CompositorIntersectService.apply(
                      rhiCommandBuffer, batch.clip, (float) doubleValue, currentValue);
                  ...
                  if (!batch.color) {
                    rhiCommandBuffer.pushFloat(
                        "uOutlineWidth", batch.style.outlineWidth() * previousValue);
                    if (batch.style.hasOutline()) {
                      updateState10(rhiCommandBuffer, "uOutlineColor", batch.style.outlineColor());
                    }

                    rhiCommandBuffer.pushFloat(
                        "uEdgeSoftness", batch.style.edgeSoftness() * (float) doubleValue);
                  }
  ...
  private TextTextureService createTextTextureService(TextMode textMode) {
    ...
    return textTexture != null && textTexture.isReady()
        ? textTexture
        : this.entries.get(TextMode.REGULAR);
  }

  private TextTextureService createTextTextureService2(String text, TextMode textMode) {
    if (text != null) {
      if ("minecraft".equals(text)
          && this.textTextureService != null
          && this.textTextureService.isReady()) {
      ...
      if (currentTextTexture != null
        ...
        try {
          ...
          CoreIsInitializedHandler.LOGGER.warn(
              "Failed to release replaced font atlas for {}", text, exception);
        }
      }
    }
  }
  ...
  private boolean checkCondition3(String text, String currentText, TextMode textMode) {
    ...
    if (textTexture == null) {
      ...
      return !this.checkCondition5(text, textTexture)
          ? false
          : !this.checkCondition2(currentText) || !checkCondition4(text);
    }
  }
    ...
    return this.entries2.computeIfAbsent(
        ...
        item ->
            new GlyphAtlas(
                ...
                textMode,
                textTexture != null ? textTexture : new TextTextureService(textMode)));
  }

  private TextTextService.GlyphRun createGlyphRun(
      TextTextureService textTexture, TextMode textMode, int value) {
    if (textTexture != null && textTexture.isReady() && textTexture.variant() == textMode) {
      ...
      if (glyphInfo != null) {
        return new TextTextService.GlyphRun(
            textTexture, glyphInfo, textTexture.colorGlyph(glyphInfo));
      }
    }
    ...
    return currentGlyphInfo != null
        ? new TextTextService.GlyphRun(
            glyphAtlas, currentGlyphInfo, glyphAtlas.colorGlyph(currentGlyphInfo))
        : null;
  }
  ...
  private boolean checkCondition5(String text, TextTextureService textTexture) {
    ...
    while (index < text.length()) {
      ...
      if (currentValue != 8205
          && currentValue != 8204
          && !Character.isEmojiPresentation(currentValue)) {
        if (currentValue != 13
            && currentValue != 10
            && currentValue != 9
            && textTexture.glyph(currentValue) == null) {
          return true;
        }
        ...
        if (nextValue != 6 && nextValue != 8 && nextValue != 7) {
          ...
          if (currentByteValue != 1 && currentByteValue != 2) {
            ...
            if ((value == 0 || currentValue != 105 && currentValue != 108 && currentValue != 102)
                && (byteValue == 0 || currentValue != 105 && currentValue != 108)) {
  ...
  private float calculateValue5(
      String text, float value, TextMode textMode, TextTextureService textTexture) {
  ...
  private float calculateValue6(
    ...
    while (index < text.length()) {
      ...
      if (inputValue == 13) {
        previousValue +=
            calculateValue7(
                glyphAtlas, text.substring(sourceValue, targetValue), value, currentValue);
        sourceValue = index;
  ...
  private static float calculateValue7(
      GlyphAtlas glyphAtlas, String text, float value, float currentValue) {
    if (text.isEmpty()) {
      return 0.0F;
    }
      ...
      if (resolvedValue == 13) {
        resultValue +=
            this.calculateValue8(
                ...
                candidateValue,
                resultValue,
                text.substring(defaultValue, initialValue),
                nextValue,
                sourceValue,
                targetValue,
                inputValue,
    ...
    for (GlyphAtlas.ShapedGlyph shapedGlyph : shapedLine.glyphs()) {
      ...
      if (!(glyphInfo.width() <= 0.0F)) {
        ...
        float currentX =
            value
                + nextValue
                + (shapedGlyph.x() + glyphInfo.bearingX()) * previousValue
                + resultValue;
    ...
    while (index < text.length()) {
      ...
      if (defaultValue != 13) {
        if (defaultValue == 10) {
          ...
          TextTextService.GlyphRun glyphRun =
              this.createGlyphRun(textTexture, textMode, defaultValue);
          if (glyphRun != null) {
            ...
            if (glyphInfo.width() <= 0.0F) {
              ...
              float initialValue =
                  value
                      + resultValue
                      + glyphInfo.bearingX() * nextValue
                      + textData.letterSpacing() / 2.0F;
  ...
  private void updateState6() {
    ...
    if (value > 0) {
      this.items.add(
          new TextTextService.Batch(
              this.count8,
              value,
              this.textData,
              this.float6,
              this.textOperationHandler,
              this.enabled3));
    }
  }
  ...
  private boolean checkCondition6(int value) {
    ...
    if (index > 32768) {
      if (!this.enabled2) {
        CoreIsInitializedHandler.LOGGER.warn(
            "TextRenderer at glyph cap ({}); subsequent glyphs in this frame dropped", 32768);
        this.enabled2 = true;
      }
    ...
    try {
      ...
      RhiBlendStateService.ShaderHandle shaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.VERTEX, RhiRepository.vertex("msdf_text"));
      RhiBlendStateService.ShaderHandle currentShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.FRAGMENT, RhiRepository.fragment("msdf_text"));
      RhiBlendStateService.ShaderHandle nextShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.FRAGMENT, RhiRepository.fragment("color_text"));
      this.pipelineHandle =
              ...
              RhiBuilderData.graphics()
                  ...
                  .vertexLayout(
                      RhiBlendStateService.VertexLayout.of(
                          ...
                          new RhiBlendStateService.VertexAttribute(
                              1, 2, RhiBlendStateService.VertexFormat.FLOAT, 8L),
                          new RhiBlendStateService.VertexAttribute(
                              2, 4, RhiBlendStateService.VertexFormat.FLOAT, 16L),
                          new RhiBlendStateService.VertexAttribute(
                              3, 4, RhiBlendStateService.VertexFormat.FLOAT, 32L)))
  ...
  private void updateState12() {
    try {
      ...
      GlyphAtlas glyphAtlas =
          this.createTextTextureService22(
              this.createTextTextureService(TextMode.REGULAR), TextMode.REGULAR);
  ...
  private record Batch(
      ...
      TextData style,
      float[] clip,
      TextOperationHandler atlas,
      boolean color) {}

  private record GlyphRun(
      TextOperationHandler atlas, TextTextureService.GlyphInfo glyph, boolean color) {}
