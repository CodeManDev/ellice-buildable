...
public class ColorColorService extends ScenePctService<ColorColorService> {
  ...
  private static final int[] int2 =
      new int[] {
        -48060, -29696, -10496, -11870592, -14494738, -10262799, -5745161, -1292135, -1, -7829368,
        -12303292, -16777216
      };
  private int count4;
  ...
  public void eyedropperPick(int value) {
    ...
    if (this.sceneCornerRadiusService != null) {
      ...
      if (this.sceneCornerRadiusService.findById("hex")
  ...
  private void updateState4() {
    ...
    this.sceneCornerRadiusService.addChild(
        this.createControlCompactService2(
            "R", this.count >> 16 & 0xFF, -1096636, controlLetterSpacing));
    this.sceneCornerRadiusService.addChild(
        this.createControlCompactService2(
            "G", this.count >> 8 & 0xFF, -11870592, controlLetterSpacing));
    this.sceneCornerRadiusService.addChild(
        this.createControlCompactService2("B", this.count & 0xFF, -10443270, controlLetterSpacing));
    ...
    for (int previousValue : int2) {
      sourceLayoutContainerNode.addChild(
          this.createSceneCornerRadiusService2(previousValue, 10.0F, controlLetterSpacing));
    }
    ...
    if (!items.isEmpty()) {
      ...
      for (int sourceValue : items) {
        targetLayoutContainerNode.addChild(
            this.createSceneCornerRadiusService2(sourceValue, 10.0F, controlLetterSpacing));
      }
  ...
  private SceneCornerRadiusService createSceneCornerRadiusService(String text, int value) {
    ...
    SceneTextService sceneText =
        new SceneTextService(text, 7.0F, currentValue != 0 ? -1 : -2130706433);
  ...
  private SceneCornerRadiusService createSceneCornerRadiusService2(
      int value, float currentValue, ControlLetterSpacingService controlLetterSpacing) {
    SceneCornerRadiusService sceneCornerRadius = new SceneCornerRadiusService();
    sceneCornerRadius
        .size(currentValue, currentValue)
        .cornerRadius(2.0F)
  ...
  private void updateState5(
      LayoutContainerNode layoutContainerNode, ControlLetterSpacingService controlLetterSpacing) {
  ...
  private SliderControl createControlCompactService2(
      String channelName,
      int channelValue,
      int fillColor,
      ControlLetterSpacingService controlLetterSpacing) {
    SliderControl sliderControl = new SliderControl();
    sliderControl
        .id("rgb-" + channelName)
        .value(channelValue)
  ...
  private int calculateValue(String text) {
    return this.sceneCornerRadiusService.findById("rgb-" + text)
            instanceof SliderControl sliderControl
  ...
  private void updateState9(ControlLetterSpacingService controlLetterSpacing) {
    ...
    if (this.sceneCornerRadiusService.findById("harmonies")
        instanceof LayoutContainerNode layoutContainerNode) {
    ...
    if (this.sceneCornerRadiusService.findById("fmt-hsl")
        instanceof SceneTextService currentSceneText) {
  ...
  private void updateState10(SceneTextService sceneText) {
    ...
    int previousCount = this.count & 0xFF;
    sceneText.text(
        String.format(Locale.ROOT, "rgb(%d, %d, %d)", currentCount, nextCount, previousCount));
  }
  ...
  private void updateState11(SceneTextService sceneText) {
    ...
    float currentValue =
        value != 0.0F && value != 1.0F
            ? (this.value4 - value) / Math.min(value, 1.0F - value)
            : 0.0F;
    sceneText.text(
        String.format(
            Locale.ROOT,
    ...
    if (text != null && !text.isEmpty()) {
      try {
        ...
        if (this.sceneCornerRadiusService.findById("harmonies")
            instanceof LayoutContainerNode layoutContainerNode) {
          ControlLetterSpacingService controlLetterSpacing =
              this.sceneCornerRadiusService.findById("hex")
                      instanceof ControlLetterSpacingService currentControlLetterSpacing
                  ? currentControlLetterSpacing
                  : null;
          this.updateState5(layoutContainerNode, controlLetterSpacing);
        }
