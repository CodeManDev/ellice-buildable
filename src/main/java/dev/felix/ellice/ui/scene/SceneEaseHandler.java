   ...
   enum Easing {
      ...
      public float apply(float value) {
         ...
         return switch (this) {
            ...
            case EASE_IN_OUT -> currentValue < 0.5F
               ? 8.0F * currentValue * currentValue * currentValue * currentValue
               : 1.0F
                  - (float)Math.pow(-2.0F * currentValue + 2.0F, 4.0) / 2.0F;
      ...
      private static float calculateValue(float value, float currentValue, float nextValue, float previousValue, float sourceValue) {
         if (!(value <= 0.0F) && !(value >= 1.0F)) {
            ...
            for (int index = 0; index < 10; index++) {
               ...
               float defaultValue = selectedValue > 1.0E-5F
                  ? outputValue - candidateValue / selectedValue
                  : (targetValue + inputValue) * 0.5F;
               outputValue = defaultValue > targetValue && defaultValue < inputValue ? defaultValue : (targetValue + inputValue) * 0.5F;
            }
      ...
      private static SceneEaseHandler.Easing[] $values() {
         return new SceneEaseHandler.Easing[]{LINEAR, EASE_IN, EASE_OUT, EASE_IN_OUT, EXPRESSIVE_OUT, EXPRESSIVE_IN};
      }
   }

   record Spring(float damping, float stiffness) implements SceneEaseHandler {
      public static final SceneEaseHandler.Spring DEFAULT = new SceneEaseHandler.Spring(
         14.0F, 170.0F
      );
      public static final SceneEaseHandler.Spring SNAPPY = new SceneEaseHandler.Spring(
         22.0F, 400.0F
      );
      public static final SceneEaseHandler.Spring GENTLE = new SceneEaseHandler.Spring(
         18.0F, 60.0F
      );
      public static final SceneEaseHandler.Spring BOUNCY = new SceneEaseHandler.Spring(
         15.0F, 320.0F
      );

      SceneEaseHandler.Spring.Sample advance(float value, float currentValue, float nextValue, float previousValue) {
         ...
         if (Math.abs(nextDoubleValue) < 1.0E-7 * Math.max(1.0F, this.stiffness)) {
            ...
            sourceDoubleValue = (currentValue - currentDoubleValue * targetDoubleValue * previousValue) * inputDoubleValue;
            ...
            previousDoubleValue = defaultDoubleValue * (doubleValue * candidateDoubleValue + (currentValue + currentDoubleValue * doubleValue) * selectedDoubleValue);
            sourceDoubleValue = defaultDoubleValue * (currentValue * candidateDoubleValue - (currentDoubleValue * currentValue + this.stiffness * doubleValue) * selectedDoubleValue);
         } else {
            ...
            double cachedDoubleValue = (currentValue - resolvedDoubleValue * doubleValue) / (computedDoubleValue - resolvedDoubleValue);
            double pendingDoubleValue = doubleValue - cachedDoubleValue;
            double activeDoubleValue = cachedDoubleValue * Math.exp(computedDoubleValue * previousValue);
            double fallbackDoubleValue = pendingDoubleValue * Math.exp(resolvedDoubleValue * previousValue);
            previousDoubleValue = activeDoubleValue + fallbackDoubleValue;
            sourceDoubleValue = computedDoubleValue * activeDoubleValue + resolvedDoubleValue * fallbackDoubleValue;
         }

         return new SceneEaseHandler.Spring.Sample((float)(nextValue + previousDoubleValue), (float)sourceDoubleValue);
      }
