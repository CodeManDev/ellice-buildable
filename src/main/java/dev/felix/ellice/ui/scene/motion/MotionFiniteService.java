...
public final class MotionFiniteService {
  ...
  private MotionFiniteService(
      String currentText,
      Predicate<ScenePctService<?>> currentPredicate,
      MotionFiniteService.ValueConstraint currentValueConstraint,
      String nextText) {
  ...
  public static MotionFiniteService of(
      String text,
      Predicate<ScenePctService<?>> predicate,
      MotionFiniteService.ValueConstraint valueConstraint,
      String currentText) {
  ...
  void validate(float value) {
    if (!this.valueConstraint.accepts(value)) {
      throw new IllegalArgumentException(
          "Property '" + this.text + "' requires " + this.text2 + ", got " + value);
    }
  }
