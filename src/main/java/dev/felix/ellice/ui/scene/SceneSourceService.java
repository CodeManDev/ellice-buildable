...
public class SceneSourceService extends ScenePctService<SceneSourceService> {
   ...
   public float getAnimProperty(String text) {
      return switch (text) {
         ...
         case "cornerRadiusTL", "cornerRadiusTopLeft" -> this.value2 >= 0.0F ? this.value2 : this.value;
         case "cornerRadiusTR", "cornerRadiusTopRight" -> this.value3 >= 0.0F ? this.value3 : this.value;
         case "cornerRadiusBR", "cornerRadiusBottomRight" -> this.value4 >= 0.0F ? this.value4 : this.value;
         case "cornerRadiusBL", "cornerRadiusBottomLeft" -> this.value5 >= 0.0F ? this.value5 : this.value;
   ...
   protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      ...
      if (this.cw > 0.0F && this.ch > 0.0F) {
         ...
         this.mediaSourceService.outputSize((int)Math.ceil(this.cw * y), (int)Math.ceil(this.ch * y), this.createScaleMode());
      }
   ...
   private void updateState(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      ...
      if (decodedFrame != null && decodedFrame.version() != this.timestamp) {
         ...
         if (currentWidth > 0 && currentHeight > 0) {
            ...
            if (byteBuffer != null) {
               if (this.textureHandle.valid() && (this.count != currentWidth || this.count2 != currentHeight)) {
                  compositorPushPresentationScale.destroyTexture(this.textureHandle);
                  this.textureHandle = RhiBlendStateService.TextureHandle.NONE;
                  this.count = this.count2 = 0;
               }

               RhiBlendStateService.TextureHandle currentTextureHandle = compositorPushPresentationScale.uploadRgbaTexture(this.textureHandle, currentWidth, currentHeight, byteBuffer);
