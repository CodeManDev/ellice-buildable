...
public final class ModeSettingControl extends ScenePctService<ModeSettingControl> {
   ...
   public ModeSettingControl(ModuleSetting.Mode currentMode) {
      ...
      this.sceneTextService = createSceneTextService((String)currentMode.get(), 7.2F, -637534209)
      ...
      for (int index = 0; index < strings.length; index++) {
         ...
         SceneTextService sceneText = createSceneTextService(text, 7.0F, -1459617793)
            .overflow(SceneTextService.Overflow.ELLIPSIS)
            .flex(1.0F);
      ...
      this.sceneContentService = new SceneContentService(this.sceneComponent4)
   ...
   public void refreshState() {
      if (!this.enabled6) {
         ...
         this.sceneCornerRadiusService
            ...
            .cursorStyle(enabled ? ScenePctService.CursorStyle.POINTER : ScenePctService.CursorStyle.DEFAULT);
   ...
   private static SceneTextService createSceneTextService(String text, float value, int currentValue) {
      return new SceneTextService(text, value, currentValue).fontVariant(TextMode.REGULAR).inheritEdgeSoftness(false);
   }
   ...
   private static String createText2(String text) {
      String currentText = text.toLowerCase(Locale.ROOT).replaceAll("[^a-z0-9]+", "-").replaceAll("(^-|-$)", "");
