...
public final class SceneBrandService extends ScenePctService<SceneBrandService> {
   ...
   private static final Map<String, String> text3 = Map.ofEntries(
   ...
   public static BufferedImage image(String text) {
      ...
      try (InputStream inputStream = SceneBrandService.class.getResourceAsStream("/assets/ellice/brands/" + text)) {
   ...
   protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      ...
      if (currentText != null && currentText.endsWith(".svg")) {
         if (compositorPushPresentationScale.svgRenderer() != null) {
            textureHandle = compositorPushPresentationScale.svgRenderer().getClasspathTexture("/assets/ellice/brands/" + currentText, 64, 64);
         }
      ...
      if (textureHandle != null && textureHandle.valid()) {
         ...
         String nextLength = switch (this.text5) {
            case "General" -> "S";
            case "BlocksMC" -> "B";
            case "Spectre" -> "SP";
            default -> this.text5.length() > 3 ? this.text5.substring(0, 2) : this.text5;
         };
         float previousLength = Math.min(this.cw, this.ch)
            * (nextLength.length() > 2 ? 0.28F : 0.38F);
         float nextValue = compositorPushPresentationScale.textWidth(nextLength, previousLength, TextMode.REGULAR, "material-roboto");
