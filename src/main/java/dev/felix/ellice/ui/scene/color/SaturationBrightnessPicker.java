...
public class SaturationBrightnessPicker extends ScenePctService<SaturationBrightnessPicker> {
  ...
  private static final MotionFiniteService motionFiniteService =
      MotionFiniteService.finite(
          "materialColorPress", item -> item instanceof SaturationBrightnessPicker);
  private static final MotionFiniteService motionFiniteService2 =
      MotionFiniteService.finite(
          "materialMarkerSat", item -> item instanceof SaturationBrightnessPicker);
  private static final MotionFiniteService motionFiniteService3 =
      MotionFiniteService.finite(
          "materialMarkerBri", item -> item instanceof SaturationBrightnessPicker);
  ...
  protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
    if (!(this.effectiveOpacity < 0.003F)) {
      compositorPushPresentationScale.colorField(
          this.cx, this.cy, this.cw, this.ch, this.value, this.value4, this.effectiveOpacity);
      ...
      if (this.enabled2) {
        if (!this.enabled5 || this.pressed) {
          ...
          MotionAnimateService.animate(
              this, motionFiniteService2, this.value2, MaterialEnterService.PAGE);
          MotionAnimateService.animate(
              this, motionFiniteService3, this.value3, MaterialEnterService.PAGE);
        }
      ...
      float y = this.enabled2 ? 6.0F + 4.0F * Math.max(0.0F, Math.min(1.06F, this.value5)) : 5.0F;
      ...
      float height =
          this.cx
              + Math.max(
                  width,
                  Math.min(this.cw - width, (this.enabled2 ? this.value6 : this.value2) * this.cw));
      float currentValue =
          this.cy
              + Math.max(
                  width,
                  Math.min(
                      this.ch - width,
                      (1.0F - (this.enabled2 ? this.value7 : this.value3)) * this.ch));
