...
public final class SceneResizeMarginService extends SceneCornerRadiusService {
   ...
   public SceneResizeMarginService constrainToParent() {
      ...
      float value = calculateValue3(this.width, calculateValue(this.value2, parentBox.width), parentBox.width);
      float currentValue = calculateValue3(this.height, calculateValue(this.value3, parentBox.height), parentBox.height);
      float nextValue = calculateValue3(this.x + this.marginLeft, 0.0F, Math.max(0.0F, parentBox.width - value));
      float previousValue = calculateValue3(this.y + this.marginTop, 0.0F, Math.max(0.0F, parentBox.height - currentValue));
   ...
   protected void onScenePress(float currentValue, float nextValue) {
      ...
      if (Float.isFinite(previousValue) && Float.isFinite(sourceValue) && this.checkCondition2() && this.checkCondition()) {
         ...
         if (parentBox == null) {
            ...
            ScenePctService.PresentationTransform nextPresentationTransform = this.parent == null
   ...
   protected void updateWhileScenePressed(float value, float currentValue) {
      if (this.count7 != -1) {
         ...
         if (parentBox != null) {
            ...
            if (this.count7 == 0) {
               float inputValue = calculateValue3(this.value10 + nextValue, 0.0F, Math.max(0.0F, parentBox.width - this.value12));
               float outputValue = calculateValue3(this.value11 + previousValue, 0.0F, Math.max(0.0F, parentBox.height - this.value13));
               ...
               if ((this.count7 & 1) != 0) {
                  resultValue = calculateValue3(this.value10 + nextValue, 0.0F, candidateValue - sourceValue);
               } else if ((this.count7 & 2) != 0) {
                  candidateValue = calculateValue3(candidateValue + nextValue, resultValue + sourceValue, parentBox.width);
               }

               if ((this.count7 & 4) != 0) {
                  selectedValue = calculateValue3(this.value11 + previousValue, 0.0F, defaultValue - targetValue);
               } else if ((this.count7 & 8) != 0) {
                  defaultValue = calculateValue3(defaultValue + previousValue, selectedValue + targetValue, parentBox.height);
               }
   ...
   private SceneResizeMarginService.ParentBox createParentBox() {
      if (this.parent != null
         ...
         float nextValue = Math.max(0.0F, this.parent.cw - this.parent.paddingLeft - this.parent.paddingRight);
         float previousValue = Math.max(0.0F, this.parent.ch - this.parent.paddingTop - this.parent.paddingBottom);
         return Float.isFinite(nextValue) && Float.isFinite(previousValue) ? new SceneResizeMarginService.ParentBox(value, currentValue, nextValue, previousValue) : null;
      } else {
         return null;
      }
   }
   ...
   private static float calculateValue3(float value, float currentValue, float nextValue) {
      ...
      return !Float.isFinite(value) ? previousValue : Math.max(previousValue, Math.min(sourceValue, value));
   }
