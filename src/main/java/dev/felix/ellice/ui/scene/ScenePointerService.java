...
public final class ScenePointerService extends ScenePctService<ScenePointerService> {
  ...
  private ScenePointerService.Pointer pointer2 =
      new ScenePointerService.Pointer(0.0F, 0.0F, false, false, false, 1.0F);
  ...
  public ScenePointerService(TerrainMapController terrainMapController, boolean currentEnabled) {
    ...
    this.interactive(currentEnabled)
        ...
        .clip(true)
        .cursorStyle(ScenePctService.CursorStyle.DEFAULT);
  ...
  protected void onPress(float currentValue, float nextValue) {
    ...
    this.terrainKindData =
        !this.enabled4 && !this.enabled5
            ? this.createTerrainKindData(this.value, this.value2)
            : null;
    this.enabled6 = false;
  ...
  protected void updateWhilePressed(float currentValue, float nextValue) {
    ...
    if (previousValue != this.value || sourceValue != this.value2) {
      if (this.terrainKindData != null) {
        ...
        if (this.enabled6) {
          this.renderer.moveWaypointPreview(
              this.renderer.pick(previousValue - this.value8, sourceValue - this.value9));
        }
        ...
        this.renderer
            .camera(true)
            .pan(this.value, this.value2, previousValue, sourceValue, SceneDtService.dt());
      }
    ...
    if (this.terrainKindData != null) {
      ...
      if (nextTerrainKindData != null) {
        ...
        if (longValue - this.timestamp2 < 350000000L
            && Math.hypot(this.value - this.value5, this.value2 - this.value6) < 5.0) {
  ...
  protected boolean handleScroll(float value) {
    ...
    this.renderer.zoom(
        value,
        currentPresentationTransform.inverseX(this.pointer2.x) - this.cx,
        currentPresentationTransform.inverseY(this.pointer2.y) - this.cy);
    return true;
  }
  ...
  protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
    ...
    Vector3d nextVector3d =
        this.renderer.world() == null ? currentFrame.focus() : this.renderer.world().player();
    TerrainKindData currentTerrainKindData = this.renderer.navigation().target();

    for (TerrainKindData nextTerrainKindData :
        this.renderer.visibleWaypoints().stream()
            .sorted(
                Comparator.<TerrainKindData>comparingInt(
                        item -> item.id().equals(this.renderer.selected()) ? 0 : 1)
                    .thenComparingDouble(
                        item -> currentFrame.focus().distanceSquared(item.x(), item.y(), item.z())))
            .limit(64L)
            .toList()) {
      Vector3d currentX =
          currentFrame.project(
              nextTerrainKindData.x(), nextTerrainKindData.y(), nextTerrainKindData.z());
      if (currentX != null) {
        arrayList.add(
            new TerrainReservedService.Anchor(
                ...
                (float) currentX.x,
                (float) currentX.y,
                nextVector3d.distance(
                    nextTerrainKindData.x(), nextTerrainKindData.y(), nextTerrainKindData.z()),
                nextTerrainKindData.id().equals(this.renderer.selected()),
                currentTerrainKindData != null
                    && currentTerrainKindData.id().equals(nextTerrainKindData.id())));
      }
    }

    this.terrainReservedService.present(
        arrayList,
    ...
    if (this.enabled && this.renderer.loadedSections() == 0) {
      String currentText =
          this.renderer.cache().pending() > 0 ? "Loading terrain…" : "No explored terrain here";
  ...
  private void updateState() {
    ...
    this.terrainKindData2 =
        this.isHovered() && !this.enabled2 ? this.createTerrainKindData(value, currentValue) : null;
    this.cursorStyle(
        !this.pointer2.shift && (!this.enabled6 || this.terrainKindData == null)
            ? (this.terrainKindData2 != null
  ...
  private void updateState2(
    ...
    for (CompatLoadedHandler.MapPlayer mapPlayer : this.renderer.players()) {
      ...
      if (currentX != null) {
        double nextX =
            vector3d == null
                ? frame.focus().distance(mapPlayer.x(), mapPlayer.y(), mapPlayer.z())
                : vector3d.distance(mapPlayer.x(), mapPlayer.y(), mapPlayer.z());
        String text = mapPlayer.name();
        String currentText =
            MaterialIsLightService.isLight() ? TextCodec.normalizeWhite(text) : text;
        String nextText = Math.round(nextX) + "m";
        float value =
            compositorPushPresentationScale.textWidth(
                nextText, 8.5F, TextMode.REGULAR, "material-roboto");
        String previousText =
            createText(compositorPushPresentationScale, currentText, 200.0F - value - 6.0F);
        float currentValue = Math.clamp(mapPlayer.health() / mapPlayer.maxHealth(), 0.0F, 1.0F);
        NametagsData nametagsData =
            new NametagsData(previousText, nextText, -5650945, currentValue, 0.0F, null, false);
        float currentWidth =
            NametagsRenderer.width(
                compositorPushPresentationScale,
                nametagsData,
                1.0F,
                Math.max(56.0F, this.cw - 8.0F));
  ...
  private void updateState3(
    ...
    float targetValue =
        compositorPushPresentationScale.textWidth(
            text, 10.5F, TextMode.REGULAR, "material-roboto-medium");
    ...
    compositorPushPresentationScale.roundedRect(
        ...
        candidateValue,
        candidateValue / 2.0F,
        mulAlpha(selectedValue, this.effectiveOpacity));
    ...
    if (defaultValue > 0.1F) {
      ...
      compositorPushPresentationScale.roundedRect(
          ...
          cachedValue,
          cachedValue / 2.0F,
          mulAlpha(initialValue, this.effectiveOpacity));
      ...
      compositorPushPresentationScale.roundedRect(
          ...
          pendingValue,
          pendingValue / 2.0F,
          mulAlpha(ThemeMixService.lighten(initialValue, 0.22F), this.effectiveOpacity * 0.8F));
    }
  }
  ...
  private static String createText(
    ...
    while (!currentText.isEmpty()
        && compositorPushPresentationScale.textWidth(
                currentText + "…", 10.5F, TextMode.REGULAR, "material-roboto-medium")
            > value) {
      currentText =
  ...
  public record Pointer(
      float x, float y, boolean orbit, boolean shift, boolean control, float density) {}
}
