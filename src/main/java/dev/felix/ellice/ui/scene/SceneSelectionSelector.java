...
public final class SceneSelectionSelector extends LayoutContainerNode {
  private static final MotionFiniteService motionFiniteService =
      MotionFiniteService.finite(
          "menuSelectionLeft", item -> item instanceof SceneSelectionSelector);
  private static final MotionFiniteService motionFiniteService2 =
      MotionFiniteService.finite(
          "menuSelectionRight", item -> item instanceof SceneSelectionSelector);
  private float value;
  ...
  protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
    if (this.count >= 0 && this.count < this.children().size()) {
      ...
      if (!this.enabled) {
        ...
        MotionAnimateService.animate(
            ...
            new SceneEaseHandler.Spring(
                currentValue != 0 ? 27.0F : 30.0F, currentValue != 0 ? 210.0F : 380.0F));
        MotionAnimateService.animate(
            this,
            motionFiniteService2,
            height,
            new SceneEaseHandler.Spring(
                currentValue != 0 ? 30.0F : 27.0F, currentValue != 0 ? 380.0F : 210.0F));
      }
      ...
      float previousValue =
          Math.max(24.0F, Math.min(scenePct.computedW() + 48.0F, this.value2 - this.value));
      float sourceValue =
          Math.max(
              previousValue * 0.5F,
              Math.min(
                  this.cw / nextValue - previousValue * 0.5F, (this.value + this.value2) * 0.5F));
      compositorPushPresentationScale.roundedRect(
