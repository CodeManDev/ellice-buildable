...
public class ControlLetterSpacingService extends ScenePctService<ControlLetterSpacingService> {
   ...
   private static final MotionFiniteService motionFiniteService = MotionFiniteService.finite("materialInputFocus", item -> item instanceof ControlLetterSpacingService);
   ...
   private void updateState() {
      ...
      if (snapshot == null || !snapshot.text.equals(this.text2)) {
         this.items.push(new ControlLetterSpacingService.Snapshot(this.text2, this.count, this.count2));
   ...
   private void updateState2() {
      if (!this.items.isEmpty()) {
         this.items2.push(new ControlLetterSpacingService.Snapshot(this.text2, this.count, this.count2));
   ...
   private int calculateValue(float currentValue) {
      ...
      float sourceValue = compositorPushPresentationScale.textWidth(text, nextValue, TextMode.REGULAR, this.text4, this.value6);
      if (previousValue >= sourceValue) {
         return this.text2.length();
      }

      int targetValue = 0;
      float inputValue = Math.abs(compositorPushPresentationScale.textCaretX(text, 0, nextValue, TextMode.REGULAR, this.text4, this.value6) - previousValue);
      int index = 0;

      while (index < text.length()) {
         index = TextEndsService.nextBoundary(text, index);
         float outputValue = compositorPushPresentationScale.textCaretX(text, index, nextValue, TextMode.REGULAR, this.text4, this.value6);
   ...
   private String createText() {
      return this.checkCondition() ? this.text2.substring(this.calculateValue2(), this.calculateValue3()) : this.text2;
   }
         ...
         if (Character.isHighSurrogate(character)) {
            ...
            if (!Character.isISOControl(value)) {
               ...
               if (this.text2.length() + text.length() <= this.ff6vta1kve) {
                  this.updateState();
                  this.text2 = this.text2.substring(0, this.count)
                     + text
                     + this.text2.substring(this.count);
   ...
   public void paste(String text) {
      if (this.enabled && text != null) {
         ...
         if (!stringBuilder.isEmpty()) {
            this.updateState();
            this.text2 = this.text2.substring(0, this.count) + stringBuilder + this.text2.substring(this.count);
   ...
   public float intrinsicHeight(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      return this.enabled3
         ? 56.0F
         : compositorPushPresentationScale.textLineHeight(this.calculateValue7(this.value, "textinput.font-size")) + 6.0F;
   }
   ...
   protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      ...
      int sourceValue = ThemeIsSetService.isSet(this.count7) ? this.count7 : mulAlpha(nextValue, 0.45F);
      ...
      if (this.enabled3 && !this.interactive) {
         height = MaterialIsLightService.layer(MaterialIsLightService.SURFACE_CONTAINER, MaterialIsLightService.ON_SURFACE, 0.04F);
         nextValue = MaterialIsLightService.layer(MaterialIsLightService.SURFACE_CONTAINER, MaterialIsLightService.ON_SURFACE, 0.38F);
      }

      if (this.enabled3 && this.enabled != this.enabled6) {
         this.enabled6 = this.enabled;
         MotionAnimateService.animate(this, motionFiniteService, this.enabled ? 1.0F : 0.0F, MaterialIsLightService.FAST_EFFECTS);
      }
      ...
      if (this.enabled3) {
         int candidateValue = this.enabled5 ? MaterialIsLightService.ERROR : MaterialIsLightService.ON_SURFACE_VARIANT;
         compositorPushPresentationScale.roundedRect(
            this.cx,
      ...
      float defaultValue = this.cy
         + (
               this.ch
                  - (!this.enabled3 && !this.enabled4 ? targetValue : compositorPushPresentationScale.textLineHeight(targetValue, TextMode.REGULAR, this.text4))
            )
            / 2.0F;
      ...
      float resolvedValue = compositorPushPresentationScale.textCaretX(currentText, currentLength, targetValue, TextMode.REGULAR, this.text4, this.value6);
      if (this.enabled) {
      ...
      float cachedValue = compositorPushPresentationScale.textWidth(currentText, targetValue, TextMode.REGULAR, this.text4, this.value6);
      ...
      if (this.checkCondition()) {
         float[] nextLength = compositorPushPresentationScale.textSelectionSpans(
            currentText,
         ...
         for (byte index = 0; index + 1 < nextLength.length; index += 2) {
            float activeValue = !this.enabled3 && !this.enabled4
               ? this.ch - 4.0F
               : Math.min(this.ch - 8.0F, targetValue + 8.0F);
            compositorPushPresentationScale.roundedRect(pendingValue + nextLength[index], this.cy + (this.ch - activeValue) / 2.0F, nextLength[index + 1], activeValue, 2.0F, mulAlpha(previousValue, y));
         }
      }
      ...
      if (this.enabled && (int)(this.value3 * 2.0F) % 2 == 0) {
         ...
         float primaryValue = !this.enabled3 && !this.enabled4
            ? this.ch - 6.0F
            : Math.min(this.ch - 8.0F, targetValue + 8.0F);
         compositorPushPresentationScale.roundedRect(
            fallbackValue,
            ...
            primaryValue,
            0.5F,
            mulAlpha(!this.enabled3 && !this.enabled4 ? nextValue : (this.enabled5 ? MaterialIsLightService.ERROR : MaterialIsLightService.PRIMARY), y)
         );
      }
