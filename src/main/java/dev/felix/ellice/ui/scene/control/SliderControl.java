...
public class SliderControl extends ScenePctService<SliderControl> {
   ...
   private static final MotionFiniteService motionFiniteService = MotionFiniteService.of(
      "materialPress", item -> item instanceof SliderControl, Float::isFinite, "a finite value"
   );
   ...
   private float calculateValue3() {
      return this.value6 > this.value5 ? (this.value3 - this.value5) / (this.value6 - this.value5) : 0.0F;
   }
   ...
   protected void updateWhilePressed(float value, float currentValue) {
      if (!(this.cw <= 0.0F) && this.value6 > this.value5) {
         float nextValue = this.materialStyle
            ? 4.0F
            : this.calculateValue2(this.value10, "slider.thumb-radius") * 2.0F * Math.max(0.0F, Math.min(2.0F, this.value13));
         float previousValue = Math.max(1.0F, this.cw - nextValue);
         float sourceValue = (value - this.cx - nextValue / 2.0F) / previousValue;
         this.updateState2(this.value5 + Math.max(0.0F, Math.min(1.0F, sourceValue)) * (this.value6 - this.value5));
      }
   }
   ...
   protected boolean handleScroll(float value) {
      this.updateState2(
         this.value3
            + value
               * (this.value7 > 0.0F ? this.value7 : (this.value6 - this.value5) / 50.0F)
      );
      this.updateState4();
      return true;
   }
   ...
   protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      if (this.materialStyle) {
         ...
         compositorPushPresentationScale.text(this.cx, this.cy, String.format(Locale.ROOT, this.text2, this.value3), sourceValue, mulAlpha(previousValue, y));
         compositorPushPresentationScale.roundedRect(
            this.cx, resultValue, this.cw, targetValue, candidateValue, candidateValue, candidateValue, candidateValue, mulAlpha(currentCount, y), 0.0F, 0.0F, 0, 0.0F, 0, 1.0F, width
         );
         ...
         compositorPushPresentationScale.roundedRect(
            this.cx, resultValue, cachedValue, targetValue, candidateValue, candidateValue, candidateValue, candidateValue, mulAlpha(value, y), 0.0F, 0.0F, 0, 0.0F, 0, 1.0F, width
         );
