...
public class HueTrianglePicker extends ScenePctService<HueTrianglePicker> {
   ...
   private static final MotionFiniteService motionFiniteService = MotionFiniteService.finite("trianglePress", item -> item instanceof HueTrianglePicker);
   ...
   protected void onPress(float currentValue, float nextValue) {
      ...
      if (!(previousValue <= 0.0F)) {
         ...
         if (!(Math.hypot(sourceValue, targetValue) > 0.48499998450279236)) {
            this.drag = ColorPickerMath.contains(sourceValue, targetValue, this.value) ? HueTrianglePicker.Drag.TRIANGLE : HueTrianglePicker.Drag.HUE;
   ...
   protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      if (!(this.effectiveOpacity < 0.003F)) {
         compositorPushPresentationScale.colorTriangle(this.cx, this.cy, this.cw, this.ch, this.value, this.effectiveOpacity);
   ...
   private void updateState(CompositorPushPresentationScaleService compositorPushPresentationScale, ColorPickerMath.Point point, float value, float currentValue, int nextValue) {
