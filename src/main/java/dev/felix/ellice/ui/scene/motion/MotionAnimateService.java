...
public final class MotionAnimateService {
   ...
   public static <N extends ScenePctService<?>> N animate(N n, MotionFiniteService motionFinite, float value, SceneEaseHandler sceneEase) {
      return animate((N)n, motionFinite, value, sceneEase, 0.0F);
   }

   public static <N extends ScenePctService<?>> N animate(N n, MotionFiniteService motionFinite, float value, SceneEaseHandler sceneEase, float currentValue) {
      return animate((N)n, motionFinite, value, sceneEase, currentValue, null);
   }

   public static <N extends ScenePctService<?>> N animate(N n, MotionFiniteService motionFinite, float value, SceneEaseHandler sceneEase, float currentValue, Runnable runnable) {
   ...
   public static <N extends ScenePctService<?>> N animate(N n, MotionCompatibilityNameService motionCompatibilityName, int value, SceneEaseHandler sceneEase) {
      return animate((N)n, motionCompatibilityName, value, sceneEase, 0.0F);
   }

   public static <N extends ScenePctService<?>> N animate(N n, MotionCompatibilityNameService motionCompatibilityName, int value, SceneEaseHandler sceneEase, float currentValue) {
      Objects.requireNonNull(motionCompatibilityName, "property").requireSupported(n);
      validate(sceneEase);
      updateState2(currentValue);
      n.animateColor(motionCompatibilityName.compatibilityName(), value, sceneEase, currentValue, null);
      return (N)n;
   }

   public static <N extends ScenePctService<?>> N loop(N n, MotionFiniteService motionFinite, float value, float currentValue, SceneEaseHandler sceneEase, boolean enabled) {
   ...
   public static <N extends ScenePctService<?>> N cancel(N n, MotionCompatibilityNameService motionCompatibilityName) {
      Objects.requireNonNull(motionCompatibilityName, "property").requireSupported(n);
      int value = motionCompatibilityName.get(n);
      n.animateColor(motionCompatibilityName.compatibilityName(), value, SceneEaseHandler.Tween.linear(1.0E-6F), 0.0F, null);
      return (N)n;
   }
   ...
   private static void updateState(ScenePctService<?> scenePct, MotionFiniteService motionFinite, float value, SceneEaseHandler sceneEase, float currentValue) {
   ...
   private static void updateState2(float value) {
      if (!Float.isFinite(value) || value < 0.0F) {
         throw new IllegalArgumentException("Animation delay must be finite and greater than or equal to zero");
      }
   }
}
