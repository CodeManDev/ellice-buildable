...
public final class MotionCompatibilityNameService {
   ...
   private MotionCompatibilityNameService(String currentText, Predicate<ScenePctService<?>> currentPredicate) {
   ...
   public static MotionCompatibilityNameService of(String text, Predicate<ScenePctService<?>> predicate) {
   ...
   void requireSupported(ScenePctService<?> scenePct) {
      ...
      if (!this.predicate.test(scenePct)) {
         throw new IllegalArgumentException(
            "Color property '" + this.text + "' is not supported by " + scenePct.getClass().getSimpleName()
         );
      }
   }
