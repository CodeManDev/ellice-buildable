...
public class ControlOptionsService extends ScenePctService<ControlOptionsService> {
   ...
   public ControlOptionsService build() {
      ...
      for (String currentText : this.text) {
         ...
         SceneTextService currentWidth = new SceneTextService(currentText, this.value, enabled ? -1 : this.count3)
   ...
   public float intrinsicWidth(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      ...
      for (ScenePctService scenePct : this.children()) {
         if (scenePct.visible) {
            value += scenePct.width == -1.0F ? scenePct.intrinsicWidth(compositorPushPresentationScale) : scenePct.width;
         }
      }
   ...
   public float intrinsicHeight(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      ...
      for (ScenePctService scenePct : this.children()) {
         if (scenePct.visible) {
            value = Math.max(value, scenePct.height == -1.0F ? scenePct.intrinsicHeight(compositorPushPresentationScale) : scenePct.height);
         }
      }
