...
public final class SceneImageService extends ScenePctService<SceneImageService> {
  ...
  protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
    ...
    if (this.bufferedImage != null
        && (this.bufferedImage2 != this.bufferedImage || !this.textureHandle.valid())) {
      RhiBlendStateService.TextureHandle currentTextureHandle =
          compositorPushPresentationScale.uploadImageTexture(
              RhiBlendStateService.TextureHandle.NONE, this.bufferedImage);
    ...
    if (this.textureHandle.valid()) {
      compositorPushPresentationScale.drawTexture(
          this.textureHandle, this.cx, this.cy, this.cw, this.ch, this.effectiveOpacity, width);
    } else {
      ...
      compositorPushPresentationScale.roundedRect(
          this.cx, this.cy, this.cw, this.ch, width, currentCount & 16777215 | value << 24);
      ...
      if (this.enabled) {
        ...
        float nextValue =
            compositorPushPresentationScale.textWidth(
                this.text2, currentValue, TextMode.REGULAR, textData.fontFamily());
        ...
        compositorPushPresentationScale.text(
            this.cx
                + (this.cw - compositorPushPresentationScale.textWidth(this.text2, currentValue))
                    / 2.0F,
            this.cy + (this.ch - currentValue) / 2.0F,
            this.text2,
            currentValue,
