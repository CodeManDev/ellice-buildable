...
public final class ColorPickerMath {
  ...
  public static float hue(float value, float currentValue) {
    return wrap((float) (Math.atan2(currentValue, value) / 6.283185307179586 + 0.25));
  }
  ...
  public static ColorPickerMath.Point polar(float value, float currentValue) {
    ...
    return new ColorPickerMath.Point(
        (float) Math.cos(doubleValue) * currentValue, (float) Math.sin(doubleValue) * currentValue);
  }
  ...
  public static ColorPickerMath.Point marker(float value, float currentValue, float nextValue) {
    ...
    return new ColorPickerMath.Point(
        point.x * currentValue * nextValue
            + currentPoint.x * (1.0F - currentValue) * nextValue
            + nextPoint.x * (1.0F - nextValue),
        point.y * currentValue * nextValue
            + currentPoint.y * (1.0F - currentValue) * nextValue
            + nextPoint.y * (1.0F - nextValue));
  }
  ...
  public static boolean contains(float value, float currentValue, float nextValue) {
    ...
    return floats[0] >= -1.0E-5 && floats[1] >= -1.0E-5 && floats[2] >= -1.0E-5;
  }
  ...
  public static ColorPickerMath.SB pick(float value, float currentValue, float nextValue) {
    if (!contains(value, currentValue, nextValue)) {
      ColorPickerMath.Point[] points =
          new ColorPickerMath.Point[] {
      ...
      for (int index = 0; index < 3; index++) {
        ...
        float inputValue =
            Math.clamp(
                ((value - currentPoint.x) * sourceValue
                        + (currentValue - currentPoint.y) * targetValue)
                    / (sourceValue * sourceValue + targetValue * targetValue),
                0.0F,
                1.0F);
        ColorPickerMath.Point previousPoint =
            new ColorPickerMath.Point(
                currentPoint.x + inputValue * sourceValue,
                currentPoint.y + inputValue * targetValue);
        float outputValue =
            (value - previousPoint.x) * (value - previousPoint.x)
                + (currentValue - previousPoint.y) * (currentValue - previousPoint.y);
  ...
  private static float[] createFloat(float value, float currentValue, float nextValue) {
    ...
    float previousValue =
        (currentPoint.y - nextPoint.y) * (point.x - nextPoint.x)
            + (nextPoint.x - currentPoint.x) * (point.y - nextPoint.y);
    float sourceValue =
        ((currentPoint.y - nextPoint.y) * (value - nextPoint.x)
                + (nextPoint.x - currentPoint.x) * (currentValue - nextPoint.y))
            / previousValue;
    float targetValue =
        ((nextPoint.y - point.y) * (value - nextPoint.x)
                + (point.x - nextPoint.x) * (currentValue - nextPoint.y))
            / previousValue;
