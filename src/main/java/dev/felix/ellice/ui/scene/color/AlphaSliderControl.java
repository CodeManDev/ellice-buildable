...
public class AlphaSliderControl extends ScenePctService<AlphaSliderControl> {
   ...
   private static final MotionFiniteService motionFiniteService = MotionFiniteService.finite("materialAlphaPress", item -> item instanceof AlphaSliderControl);
   ...
   protected void updateWhilePressed(float value, float currentValue) {
      if (!(this.cw <= 0.0F)) {
         float nextValue = calculateValue((value - this.cx - (this.enabled2 ? 2 : 0)) / Math.max(1.0F, this.cw - (this.enabled2 ? 4 : 0)));
   ...
   protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      if (!(this.effectiveOpacity < 0.003F)) {
         if (this.enabled2) {
            ...
            float y = 4.0F
               - 2.0F * Math.max(-0.2F, Math.min(1.0F, this.value6));
            ...
            if (width - currentValue > this.cx) {
               this.updateState(compositorPushPresentationScale, this.cx, height, width - currentValue - this.cx, 8.0F, 2.0F);
            }

            if (width + currentValue < this.cx + this.cw) {
               this.updateState(compositorPushPresentationScale, width + currentValue, height, this.cx + this.cw - width - currentValue, 2.0F, 8.0F);
            }
            ...
            float previousValue = this.cx + Math.max(nextValue, Math.min(this.cw - nextValue, this.value4 * this.cw));
   ...
   private void updateState(CompositorPushPresentationScaleService compositorPushPresentationScale, float currentValue, float nextValue, float previousValue, float sourceValue, float targetValue) {
      compositorPushPresentationScale.pushClip(currentValue, nextValue, previousValue, 16.0F, sourceValue, targetValue, targetValue, sourceValue);
      compositorPushPresentationScale.alphaBar(
         this.cx,
         nextValue,
         this.cw,
