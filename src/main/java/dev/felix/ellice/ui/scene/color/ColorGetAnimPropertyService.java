...
public class ColorGetAnimPropertyService extends ScenePctService<ColorGetAnimPropertyService> {
  ...
  private static final MotionFiniteService motionFiniteService =
      MotionFiniteService.finite(
          "materialHuePress", item -> item instanceof ColorGetAnimPropertyService);
  ...
  protected void updateWhilePressed(float currentValue, float nextValue) {
    if (!(this.cw <= 0.0F)) {
      float previousValue =
          calculateValue(
              (currentValue - this.cx - (this.enabled2 ? 2 : 0))
  ...
  protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
    if (!(this.effectiveOpacity < 0.003F)) {
      if (this.enabled2) {
        ...
        float y = 4.0F - 2.0F * Math.max(-0.2F, Math.min(1.0F, this.value3));
        ...
        compositorPushPresentationScale.hueBar(
            this.cx, this.cy, this.cw, this.ch, this.value2, this.effectiveOpacity);
        ...
        float candidateValue =
            this.cx + Math.max(resultValue, Math.min(this.cw - resultValue, this.value * this.cw));
  ...
  private void updateState(
    ...
    compositorPushPresentationScale.pushClip(
        ...
        previousValue);
    compositorPushPresentationScale.hueBar(
        this.cx, currentValue, this.cw, 16.0F, 8.0F, this.effectiveOpacity);
