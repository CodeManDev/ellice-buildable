...
public class ColorHueService extends ScenePctService<ColorHueService> {
   ...
   protected void updateWhilePressed(float currentValue, float nextValue) {
      ...
      this.value = (float)(Math.atan2(inputValue, targetValue) / 6.283185307179586)
         + 0.5F;
      if (this.value < 0.0F) {
         this.value++;
      }
      ...
      this.value2 = Math.min(1.0F, (float)Math.sqrt(targetValue * targetValue + inputValue * inputValue) / outputValue);
