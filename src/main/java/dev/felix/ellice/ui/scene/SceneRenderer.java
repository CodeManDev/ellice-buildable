...
public class SceneRenderer {
   ...
   public SceneRenderer() {
      this.componentMountService
         ...
         .pointerEvents(false)
         .clip(true);
      this.componentMountService.mount(item -> this.createComponentKeyService(), new ThemeIsSetService());
   }
   ...
   public void show(String text, String currentText, float value, SceneRenderer.Type type) {
      ...
      if (themeCornerData.acceptsNotification(type == SceneRenderer.Type.WARNING || type == SceneRenderer.Type.ERROR)) {
         float currentValue = Float.isFinite(value)
            ? Math.clamp(value, 0.5F, 30.0F)
            : 3.0F;
         this.items.add(new SceneRenderer.Toast(++this.timestamp, text, currentText, currentValue, type));
         this.enabled = true;
   ...
   public void tick(float value, float currentValue, float nextValue) {
      ...
      for (SceneRenderer.Toast toast : this.items) {
         toast.age += value;
         if (!toast.exiting && toast.age > toast.duration * ThemeCornerData.current().notificationDuration().scale()) {
            toast.exiting = true;
   ...
   public void render(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      ...
      if (!this.items.isEmpty()) {
         ...
         this.componentMountService.performLayout(compositorPushPresentationScale, 0.0F, 0.0F, this.value6, this.value7);
   ...
   private ComponentKeyService<?> createComponentKeyService() {
      ...
      ComponentKeyService[] componentKeies = arrayList.stream().map(this::createComponentKeyService2).toArray(ComponentKeyService[]::new);
      return ComponentBoxService.column(
         item -> item.id("toast.stack")
            .absolute()
            .size(
            ...
            .inset(
               this.corner.top() ? LayoutOperationHandler.px(16.0F) : LayoutOperationHandler.auto(),
               this.corner.left() ? LayoutOperationHandler.auto() : LayoutOperationHandler.px(16.0F),
               this.corner.top() ? LayoutOperationHandler.auto() : LayoutOperationHandler.px(16.0F),
               this.corner.left() ? LayoutOperationHandler.px(16.0F) : LayoutOperationHandler.auto()
            )
            .gap(10.0F)
            .pointerEvents(false),
         componentKeies
   ...
   private ComponentKeyService<?> createComponentKeyService2(SceneRenderer.Toast currentToast) {
      ...
      return ComponentBoxService.<SceneRenderer.ToastCard>node(
            ...
            item -> item.id("toast." + currentToast.id)
            ...
            ComponentBoxService.panel(
               item -> item.size(32.0F, 32.0F)
                  .flexShrink(0.0F)
                  .cornerRadius(12.0F)
                  .backgroundColor(MaterialIsLightService.layer(MaterialIsLightService.SURFACE_HIGH, value, 0.12F))
                  .direction(ScenePctService.Direction.ROW)
                  .align(ScenePctService.Align.CENTER)
            ...
            ComponentBoxService.column(
               ...
               MaterialTextService.label(currentToast.title, MaterialIsLightService.ON_SURFACE)
                  .props(
                     item -> item.id("toast." + currentToast.id + ".title")
                        .width(LayoutOperationHandler.percent(100.0F))
                        .wordWrap(true)
                        .maxLines(2)
                        .flexShrink(0.0F)
                  ),
               MaterialTextService.text(currentToast.subtitle, 13.0F, MaterialIsLightService.ON_SURFACE_VARIANT)
                  .props(
                     item -> item.id("toast." + currentToast.id + ".body")
                        .width(LayoutOperationHandler.percent(100.0F))
                        .wordWrap(true)
                        .maxLines(3)
                        .flexShrink(0.0F)
         ...
         .onMount(item -> {
            ...
            MotionAnimateService.animate(item, MotionColorsContainer.Floats.OPACITY, 1.0F, SceneEaseHandler.Tween.ease(0.18F));
            MotionAnimateService.animate(item, MotionColorsContainer.Floats.TRANSLATE_X, 0.0F, MaterialIsLightService.SPATIAL);
         });
   }

   private void updateState2(ThemeCornerData themeCornerData) {
      if (this.items
         .removeIf(item -> !themeCornerData.acceptsNotification(item.type == SceneRenderer.Type.WARNING || item.type == SceneRenderer.Type.ERROR))) {
         this.enabled = true;
      }
   ...
   public static final class Toast {
      ...
      Toast(long longValue, String text, String currentText, float value, SceneRenderer.Type currentType) {
         this.id = longValue;
   ...
   private static final class ToastCard extends SceneCornerRadiusService {
      ...
      public float intrinsicHeight(CompositorPushPresentationScaleService compositorPushPresentationScale, float value) {
         if (this.children().size() == 2 && Float.isFinite(value)) {
            float currentValue = Math.max(
               1.0F, value - 32.0F - 32.0F - 12.0F
            );
            return 28.0F
               + Math.max(32.0F, this.children().get(1).intrinsicHeight(compositorPushPresentationScale, currentValue));
         } else {
