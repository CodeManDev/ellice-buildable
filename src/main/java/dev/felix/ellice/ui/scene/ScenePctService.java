...
public abstract class ScenePctService<Self extends ScenePctService<Self>> {
   ...
   private void updateState2(ScenePctService<?> scenePct) {
      for (ScenePctService currentScenePct = this; currentScenePct != null; currentScenePct = currentScenePct.parent) {
   ...
   public Self size(LayoutOperationHandler layoutOperation, LayoutOperationHandler currentLayoutOperation) {
   ...
   public Self flex(float value) {
      ...
      LayoutOperationHandler layoutOperation = currentValue > 0.0F ? LayoutOperationHandler.px(0.0F) : LayoutOperationHandler.auto();
      if (checkCondition(this.flex, currentValue) && checkCondition(this.flexShrink, 1.0F) && this.layoutOperationHandler.equals(layoutOperation)) {
         return this.self();
      }
   ...
   public Self positionType(ScenePctService.PositionType currentPositionType) {
      ScenePctService.PositionType nextPositionType = Objects.requireNonNull(currentPositionType, "positionType");
   ...
   public Self inset(LayoutOperationHandler layoutOperation, LayoutOperationHandler currentLayoutOperation) {
      return this.inset(layoutOperation, currentLayoutOperation, layoutOperation, currentLayoutOperation);
   }

   public Self inset(LayoutOperationHandler layoutOperation, LayoutOperationHandler currentLayoutOperation, LayoutOperationHandler nextLayoutOperation, LayoutOperationHandler previousLayoutOperation) {
      LayoutOperationHandler sourceLayoutOperation = createLayoutOperationHandler(layoutOperation);
      LayoutOperationHandler targetLayoutOperation = createLayoutOperationHandler(currentLayoutOperation);
      LayoutOperationHandler inputLayoutOperation = createLayoutOperationHandler(nextLayoutOperation);
      LayoutOperationHandler outputLayoutOperation = createLayoutOperationHandler(previousLayoutOperation);
   ...
   public Self top(LayoutOperationHandler layoutOperation) {
      return this.inset(layoutOperation, this.layoutOperationHandler3, this.layoutOperationHandler4, this.layoutOperationHandler5);
   }

   public Self right(LayoutOperationHandler layoutOperation) {
      return this.inset(this.layoutOperationHandler2, layoutOperation, this.layoutOperationHandler4, this.layoutOperationHandler5);
   }

   public Self bottom(LayoutOperationHandler layoutOperation) {
      return this.inset(this.layoutOperationHandler2, this.layoutOperationHandler3, layoutOperation, this.layoutOperationHandler5);
   }

   public Self left(LayoutOperationHandler layoutOperation) {
      return this.inset(this.layoutOperationHandler2, this.layoutOperationHandler3, this.layoutOperationHandler4, layoutOperation);
   }
   ...
   public Self animate(String text, float value, SceneEaseHandler sceneEase, float currentValue, int nextValue, boolean enabled, Runnable runnable) {
      ...
      ScenePctService.RunningAnim currentRunningAnim = new ScenePctService.RunningAnim(text, this.getAnimProperty(text), value, sceneEase);
   ...
   public Self loop(String text, float value, float currentValue, SceneEaseHandler sceneEase, boolean enabled) {
   ...
   public Self animateColor(String text, int color, SceneEaseHandler sceneEase, float value, Runnable runnable) {
      ...
      if (currentColor == color && !(value > 0.0F)) {
         ...
         ScenePctService.RunningColorAnim currentRunningColorAnim = new ScenePctService.RunningColorAnim(text, currentColor, color, sceneEase);
      ...
      while (index < this.items3.size()) {
         ...
         if (currentValue <= 0.0F) {
            ...
            if (runningAnim.anim instanceof SceneEaseHandler.Spring spring) {
               SceneEaseHandler.Spring.Sample sample = spring.advance(nextValue, runningAnim.velocity, runningAnim.to, currentValue);
               ...
               this.setAnimProperty(runningAnim.property, runningAnim.from + (runningAnim.to - runningAnim.from) * tween.easing().apply(targetValue));
               this.invalidate();
               if (targetValue >= 1.0F) {
                   animationFinished = this.checkCondition3(runningAnim);
               }
      ...
      while (index < this.items4.size()) {
         ...
         if (inputValue <= 0.0F) {
            ...
            if (runningColorAnim.anim instanceof SceneEaseHandler.Spring currentSpring) {
               SceneEaseHandler.Spring.Sample currentSample = currentSpring.advance(runningColorAnim.progress, runningColorAnim.velocity, 1.0F, inputValue);
               ...
               this.setColorProperty(runningColorAnim.property, lerpColorOklab(runningColorAnim.from, runningColorAnim.to, Math.max(0.0F, Math.min(1.0F, outputValue))));
               this.invalidate();
               ...
               this.setColorProperty(runningColorAnim.property, lerpColorOklab(runningColorAnim.from, runningColorAnim.to, runningColorAnim.progress));
               this.invalidate();
   ...
   private void updateState4(float value) {
      if (!(value <= 0.0F) && Float.isFinite(value) && this.scrollable && (this.scrollBounce || this.scrollbarAutoHide)) {
         if (this.scrollBounce && this.enabled) {
            float currentValue = value;
            int nextValue = !(this.scrollTarget > 0.0F) && !(this.scrollTarget < this.scrollMin) ? 0 : 1;
         ...
         if (!this.scrollbarAutoHide) {
            ...
            float candidateValue = this.scrollbarOpacity + (inputValue - this.scrollbarOpacity) * resultValue;
   ...
   public void setAnimProperty(String text, float value) {
      ...
      switch (text) {
         ...
         case "scrollTarget":
            ...
            break;
         case "padding":
            currentValue = checkCondition(this.paddingTop, value)
                  && checkCondition(this.paddingRight, value)
                  && checkCondition(this.paddingBottom, value)
                  && checkCondition(this.paddingLeft, value)
               ? 0
         ...
         case "paddingLeft":
            ...
            break;
         case "margin":
            currentValue = checkCondition(this.marginTop, value)
                  && checkCondition(this.marginRight, value)
                  && checkCondition(this.marginBottom, value)
                  && checkCondition(this.marginLeft, value)
               ? 0
   ...
   void applyScrollDelta(float value, SceneEaseHandler sceneEase) {
      if (this.scrollable && !(this.scrollMin >= -1.0E-4F) && Float.isFinite(value)) {
         ...
         if (!this.scrollBounce) {
            ...
            int sourceValue = value < 0.0F && this.scrollY <= this.scrollMin + 0.75F && nextValue < this.scrollMin ? 1 : 0;
   ...
   void dragScrollTo(float value) {
      if (this.hasScrollableOverflow() && Float.isFinite(value)) {
         ...
         this.value5 = 0.22F;
         if (!checkCondition(this.scrollY, currentValue) || !checkCondition(this.scrollTarget, currentValue)) {
   ...
   float scrollbarGrabOffset(float value, float currentValue) {
      ...
      float nextValue = this.scrollsHorizontally()
   ...
   void dragScrollbarTo(float value, float currentValue, float nextValue) {
      ScenePctService.ScrollbarMetrics scrollbarMetrics = this.createScrollbarMetrics();
      if (scrollbarMetrics != null) {
         float previousValue = this.scrollsHorizontally()
            ? this.presentationTransform().inverseX(value) - this.cx
            : this.presentationTransform().inverseY(currentValue) - this.cy;
         float sourceValue = Math.max(0.0F, scrollbarMetrics.trackSize - scrollbarMetrics.thumbSize);
         float targetValue = Math.clamp(previousValue - nextValue, scrollbarMetrics.trackStart, scrollbarMetrics.trackStart + sourceValue);
         float inputValue = sourceValue > 0.0F ? (targetValue - scrollbarMetrics.trackStart) / sourceValue : 0.0F;
   ...
   private ScenePctService.ScrollbarMetrics createScrollbarMetrics() {
      if (this.hasScrollableOverflow()
         ...
         float targetValue = Math.min(previousValue, Math.max(this.scrollbarWidth * 4.0F, previousValue * sourceValue));
         float inputValue = Math.max(0.0F, this.scrollContentSize - this.scrollViewSize);
         float outputValue = inputValue > 0.0F ? -this.calculateValue8(this.scrollY) / inputValue : 0.0F;
         float resultValue = nextValue + outputValue * Math.max(0.0F, previousValue - targetValue);
         float candidateValue = this.scrollsHorizontally()
            ? this.ch - currentValue - 3.0F
            : this.cw - currentValue - 3.0F;
         float selectedValue = currentValue;
         return new ScenePctService.ScrollbarMetrics(nextValue, previousValue, resultValue, targetValue, candidateValue, selectedValue);
      } else {
         return null;
      }
   }
   ...
   private float calculateValue5(float value, float currentValue) {
      ...
      if (value > 0.0F) {
         return currentValue > 0.0F ? Math.min(previousValue, value + calculateValue6(currentValue, value, previousValue)) : Math.max(nextValue, value + currentValue);
      }

      if (value < nextValue) {
         if (currentValue < 0.0F) {
            float sourceValue = nextValue - value;
            return Math.max(nextValue - previousValue, value + calculateValue6(currentValue, sourceValue, previousValue));
         } else {
         ...
         if (targetValue > 0.0F) {
            ...
            return Math.max(nextValue - previousValue, nextValue + calculateValue6(outputValue, 0.0F, previousValue));
         } else {
            return targetValue;
         }
      }
   ...
   public float intrinsicWidth(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      ...
      for (ScenePctService scenePct : this.items) {
         if (scenePct.visible && scenePct.positionType != ScenePctService.PositionType.ABSOLUTE) {
            float nextValue = calculateValue11(scenePct.width, scenePct.intrinsicWidth(compositorPushPresentationScale));
            currentValue = Math.max(currentValue, nextValue + scenePct.marginLeft + scenePct.marginRight);
         }
      }
   ...
   public float intrinsicHeight(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      ...
      for (ScenePctService scenePct : this.items) {
         if (scenePct.visible && scenePct.positionType != ScenePctService.PositionType.ABSOLUTE) {
            float nextValue = calculateValue11(scenePct.height, scenePct.intrinsicHeight(compositorPushPresentationScale));
            currentValue = Math.max(currentValue, nextValue + scenePct.marginTop + scenePct.marginBottom);
         }
      }
   ...
   public float intrinsicHeight(CompositorPushPresentationScaleService compositorPushPresentationScale, float value) {
      if (this.direction == ScenePctService.Direction.COLUMN && !this.items.isEmpty() && Float.isFinite(value) && !(value <= 0.0F)) {
         ...
         for (ScenePctService scenePct : this.items) {
            if (scenePct.visible && scenePct.positionType != ScenePctService.PositionType.ABSOLUTE) {
               float previousValue = this.calculateValue12(compositorPushPresentationScale, scenePct, currentValue);
               float sourceValue = calculateValue11(scenePct.height, scenePct.intrinsicHeight(compositorPushPresentationScale, previousValue));
   ...
   protected abstract void draw(CompositorPushPresentationScaleService compositorPushPresentationScale);

   ScenePctService.PresentationTransform presentationTransform(ScenePctService.PresentationTransform currentPresentationTransform) {
   ...
   ScenePctService.PresentationTransform presentationTransform() {
      ...
      ScenePctService.PresentationTransform currentPresentationTransform = ScenePctService.PresentationTransform.IDENTITY;

      for (int currentSize = arrayList.size() - 1; currentSize >= 0; currentSize += -1) {
         currentPresentationTransform = currentPresentationTransform.append((ScenePctService<?>)arrayList.get(currentSize));
      }
   ...
   public void drawTree(CompositorPushPresentationScaleService compositorPushPresentationScale, float y) {
      this.drawTree(compositorPushPresentationScale, y, ScenePctService.PresentationTransform.IDENTITY);
   }

   void drawTree(CompositorPushPresentationScaleService compositorPushPresentationScale, float y, ScenePctService.PresentationTransform currentPresentationTransform) {
      if (this.visible) {
         if (!this.clip || !(this.cw <= 0.0F) && !(this.ch <= 0.0F)) {
            this.effectiveOpacity = Math.max(0.0F, this.opacity) * y;
            if (!(this.effectiveOpacity < 0.005F)) {
               ScenePctService.PresentationTransform nextPresentationTransform = this.presentationTransform(currentPresentationTransform);
               float value = this.cx;
               ...
               try {
                  float targetValue = this.inheritEdgeSoftness && this.parent != null ? this.parent.effectiveEdgeSoftness : 0.0F;
                  float inputValue = this.presentationScale > 1.0E-6F ? 1.0F / this.presentationScale : 0.0F;
                  float outputValue = Math.min(this.cw, this.ch) * inputValue * 0.4F;
                  this.effectiveEdgeSoftness = Math.min(Math.max(targetValue, this.ownEdgeSoftness()), outputValue);
                  ...
                  if (this.clip) {
                     if (this instanceof SceneCornerRadiusService sceneCornerRadius) {
                        float resultValue = sceneCornerRadius.cornerTL >= 0.0F ? sceneCornerRadius.cornerTL : sceneCornerRadius.cornerRadius;
                        float candidateValue = sceneCornerRadius.cornerTR >= 0.0F ? sceneCornerRadius.cornerTR : sceneCornerRadius.cornerRadius;
                        float selectedValue = sceneCornerRadius.cornerBR >= 0.0F ? sceneCornerRadius.cornerBR : sceneCornerRadius.cornerRadius;
                        float defaultValue = sceneCornerRadius.cornerBL >= 0.0F ? sceneCornerRadius.cornerBL : sceneCornerRadius.cornerRadius;
                        compositorPushPresentationScale.pushClip(this.cx, this.cy, this.cw, this.ch, resultValue, candidateValue, selectedValue, defaultValue);
                     } else {
                  ...
                  for (ScenePctService scenePct : this.items) {
                     scenePct.drawTree(compositorPushPresentationScale, this.effectiveOpacity, nextPresentationTransform);
                  }
   ...
   private void updateState7(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      ...
      if (!(value < 0.003F)) {
         if (this.scrollsHorizontally()) {
            ...
            float currentValue = this.scrollbarWidth * this.presentationScale * (this.enabled4 ? 1.65F : 1.0F);
            ...
            if (!(previousValue <= 0.0F)) {
               ...
               float targetValue = Math.min(
                  previousValue, Math.max(this.scrollbarWidth * this.presentationScale * 4.0F, previousValue * sourceValue)
               );
               ...
               float resolvedValue = this.scrollbarRadius >= 0.0F ? this.scrollbarRadius : this.scrollbarWidth / 2.0F;
               float computedValue = this.effectiveOpacity * value;
               if (this.scrollbarTrackColor != 0) {
                  compositorPushPresentationScale.roundedRect(initialValue, this.cy + nextValue, currentValue, previousValue, resolvedValue, mulAlpha(this.scrollbarTrackColor, computedValue));
               }
               ...
               compositorPushPresentationScale.roundedRect(
                  ...
                  targetValue,
                  resolvedValue,
                  mulAlpha(this.enabled4 ? calculateValue7(this.scrollbarColor) : this.scrollbarColor, computedValue)
               );
            }
         }
      }
   ...
   private void updateState8(CompositorPushPresentationScaleService compositorPushPresentationScale, float value) {
      float currentValue = this.scrollbarWidth * this.presentationScale * (this.enabled4 ? 1.65F : 1.0F);
      float nextValue = this.scrollInset * this.presentationScale;
      float previousValue = this.cw - nextValue * 2.0F;
      if (!(previousValue <= 0.0F)) {
         float sourceValue = this.scrollViewSize / this.scrollContentSize;
         float targetValue = Math.min(previousValue, Math.max(this.scrollbarWidth * this.presentationScale * 4.0F, previousValue * sourceValue));
         float inputValue = this.scrollContentSize - this.scrollViewSize;
         float outputValue = inputValue > 0.0F ? -this.calculateValue8(this.scrollY) / inputValue : 0.0F;
         ...
         float selectedValue = this.scrollbarRadius >= 0.0F ? this.scrollbarRadius : this.scrollbarWidth / 2.0F;
         float defaultValue = this.effectiveOpacity * value;
         if (this.scrollbarTrackColor != 0) {
            compositorPushPresentationScale.roundedRect(this.cx + nextValue, candidateValue, previousValue, currentValue, selectedValue, mulAlpha(this.scrollbarTrackColor, defaultValue));
         }

         compositorPushPresentationScale.roundedRect(
            resultValue, candidateValue, targetValue, currentValue, selectedValue, mulAlpha(this.enabled4 ? calculateValue7(this.scrollbarColor) : this.scrollbarColor, defaultValue)
         );
      }
   }
   ...
   public void performLayout(CompositorPushPresentationScaleService compositorPushPresentationScale, float value, float currentValue, float nextValue, float previousValue) {
      this.updateState10(compositorPushPresentationScale, value, currentValue, nextValue, previousValue, false);
   }

   private void updateState9(CompositorPushPresentationScaleService compositorPushPresentationScale, float value, float currentValue, float nextValue, float previousValue) {
      this.updateState10(compositorPushPresentationScale, value, currentValue, nextValue, previousValue, true);
   }

   private void updateState10(CompositorPushPresentationScaleService compositorPushPresentationScale, float value, float currentValue, float nextValue, float previousValue, boolean enabled) {
      ...
      try {
         if (enabled) {
            ...
            this.ch = calculateValue18(clampSize(this.ch, this.minHeight, this.maxHeight, previousValue));
         }
         ...
         if (!this.items.isEmpty()) {
            if (this.direction == ScenePctService.Direction.NONE) {
               this.updateState11(compositorPushPresentationScale, resultValue, candidateValue, inputValue, outputValue);
            } else {
               this.updateState12(compositorPushPresentationScale, resultValue, candidateValue, inputValue, outputValue);
            }
         }
   ...
   private void updateState11(CompositorPushPresentationScaleService compositorPushPresentationScale, float value, float currentValue, float nextValue, float previousValue) {
      for (ScenePctService scenePct : this.items) {
         if (scenePct.visible) {
            if (scenePct.positionType == ScenePctService.PositionType.ABSOLUTE) {
               this.updateState14(compositorPushPresentationScale, scenePct, value, currentValue, nextValue, previousValue);
            } else {
               float sourceValue = calculateValue14(scenePct.width, nextValue, scenePct.intrinsicWidth(compositorPushPresentationScale));
               float targetValue = calculateValue14(scenePct.height, previousValue, scenePct.intrinsicHeight(compositorPushPresentationScale, sourceValue));
               ...
               scenePct.updateState9(compositorPushPresentationScale, value + scenePct.marginLeft, currentValue + scenePct.marginTop, sourceValue, targetValue);
            }
         }
      }
   }

   private void updateState12(CompositorPushPresentationScaleService compositorPushPresentationScale, float value, float currentValue, float nextValue, float previousValue) {
      ...
      for (int currentIndex = 0; currentIndex < index; currentIndex++) {
         ...
         float resultValue = sourceValue != 0 ? currentScenePct.intrinsicWidth(compositorPushPresentationScale) : currentScenePct.intrinsicHeight(compositorPushPresentationScale, this.calculateValue12(compositorPushPresentationScale, currentScenePct, inputValue));
         float candidateValue = sourceValue != 0 ? currentScenePct.width : currentScenePct.height;
         float selectedValue = currentScenePct.layoutOperationHandler instanceof LayoutOperationHandler.Auto
         ...
         currentFloats[currentIndex] = calculateValue17(currentScenePct, floats[currentIndex], targetValue, (sourceValue != 0));
         outputValue += sourceValue != 0 ? currentScenePct.marginLeft + currentScenePct.marginRight : currentScenePct.marginTop + currentScenePct.marginBottom;
      }

      float defaultValue = index > 1 ? this.gap * (index - 1) : 0.0F;
      float initialValue = targetValue - outputValue - defaultValue;
      updateState13(currentSize, floats, currentFloats, initialValue, targetValue, (sourceValue != 0));
      ...
      for (int nextIndex = 0; nextIndex < index; nextIndex++) {
         ...
         float temporaryValue = sourceValue != 0 ? nextScenePct.marginRight : nextScenePct.marginBottom;
         ...
         float maximumValue = sourceValue != 0 ? nextScenePct.intrinsicHeight(compositorPushPresentationScale, secondaryValue) : nextScenePct.intrinsicWidth(compositorPushPresentationScale);
         float startValue = this.align == ScenePctService.Align.STRETCH && minimumValue == -1.0F
            ? expectedValue
         ...
         float endValue = switch (this.align) {
         ...
         if (sourceValue != 0) {
            nextScenePct.updateState9(compositorPushPresentationScale, fallbackValue + pendingValue, primaryValue + endValue, secondaryValue, startValue);
         } else {
            nextScenePct.updateState9(compositorPushPresentationScale, fallbackValue + endValue, primaryValue + pendingValue, startValue, secondaryValue);
         }
      ...
      for (ScenePctService previousScenePct : this.items) {
         if (previousScenePct.visible && previousScenePct.positionType == ScenePctService.PositionType.ABSOLUTE) {
            this.updateState14(compositorPushPresentationScale, previousScenePct, fallbackValue, primaryValue, nextValue, previousValue);
         }
      }
   }

   private static void updateState13(List<ScenePctService<?>> items, float[] floats, float[] currentFloats, float value, float currentValue, boolean enabled) {
      ...
      for (int currentIndex = 0; currentIndex <= index; currentIndex++) {
         ...
         for (int nextIndex = 0; nextIndex < index; nextIndex++) {
            if (!booleans[nextIndex]) {
               ...
               float outputValue = targetValue != 0 ? scenePct.flex : scenePct.flexShrink * Math.max(0.0F, floats[nextIndex]);
               if (outputValue > 0.0F) {
         ...
         for (int previousIndex = 0; previousIndex < index; previousIndex++) {
            if (!booleans[previousIndex]) {
               ...
               float resultValue = targetValue != 0 ? currentScenePct.flex : currentScenePct.flexShrink * Math.max(0.0F, floats[previousIndex]);
               float candidateValue = currentFloats[previousIndex] + sourceValue * resultValue / inputValue;
               float selectedValue = calculateValue17(currentScenePct, candidateValue, currentValue, enabled);
   ...
   private void updateState14(CompositorPushPresentationScaleService compositorPushPresentationScale, ScenePctService<?> scenePct, float value, float currentValue, float nextValue, float previousValue) {
      ...
      float selectedValue = resultValue != 0 && checkCondition5(sourceValue) && checkCondition5(targetValue)
         ? nextValue - sourceValue - targetValue - scenePct.marginLeft - scenePct.marginRight
         : calculateValue14(scenePct.width, nextValue, scenePct.intrinsicWidth(compositorPushPresentationScale));
      float defaultValue = candidateValue != 0 && checkCondition5(inputValue) && checkCondition5(outputValue)
         ? previousValue - inputValue - outputValue - scenePct.marginTop - scenePct.marginBottom
         : calculateValue14(scenePct.height, previousValue, scenePct.intrinsicHeight(compositorPushPresentationScale, selectedValue));
      ...
      if (checkCondition5(inputValue)) {
         ...
         resolvedValue = currentValue + previousValue - outputValue - defaultValue - scenePct.marginBottom;
      } else {
         resolvedValue = currentValue + scenePct.marginTop;
      }

      scenePct.updateState9(compositorPushPresentationScale, initialValue, resolvedValue, selectedValue, defaultValue);
   }
   ...
   private float calculateValue9() {
      return Math.min(
         36.0F,
         Math.max(16.0F, this.scrollViewSize * 0.12F)
      );
   }
   ...
   private float calculateValue10(CompositorPushPresentationScaleService compositorPushPresentationScale, boolean enabled) {
      float value = 0.0F;
      int index = 0;
      ...
      for (ScenePctService scenePct : this.items) {
         if (scenePct.visible && scenePct.positionType != ScenePctService.PositionType.ABSOLUTE) {
            ...
            float nextValue = enabled ? scenePct.intrinsicWidth(compositorPushPresentationScale) : scenePct.intrinsicHeight(compositorPushPresentationScale);
            value += calculateValue11(currentValue, nextValue);
            value += enabled ? scenePct.marginLeft + scenePct.marginRight : scenePct.marginTop + scenePct.marginBottom;
            index++;
         }
      }
   ...
   private static float calculateValue11(float value, float currentValue) {
      return value != -1.0F && !isPct(value) ? calculateValue18(value) : calculateValue18(currentValue);
   }

   private float calculateValue12(CompositorPushPresentationScaleService compositorPushPresentationScale, ScenePctService<?> scenePct, float value) {
      float currentValue = Math.max(0.0F, value - scenePct.marginLeft - scenePct.marginRight);
      float nextValue = this.align == ScenePctService.Align.STRETCH && scenePct.width == -1.0F
         ? currentValue
         : calculateValue14(scenePct.width, value, scenePct.intrinsicWidth(compositorPushPresentationScale));
   ...
   private static float calculateValue14(float value, float currentValue, float nextValue) {
      return value == -1.0F ? calculateValue18(nextValue) : calculateValue18(resolve(value, currentValue));
   }

   private static float calculateValue15(LayoutOperationHandler layoutOperation, float currentValue, float nextValue) {
      return switch (layoutOperation) {
         case LayoutOperationHandler.Auto auto -> calculateValue18(nextValue);
         case LayoutOperationHandler.Px px -> px.value();
         case LayoutOperationHandler.Percent percent -> calculateValue18(currentValue) * percent.value() / 100.0F;
         default -> throw new MatchException(null, null);
      };
   }

   private static float calculateValue16(LayoutOperationHandler layoutOperation, float currentValue) {
      return switch (layoutOperation) {
   ...
   private static float calculateValue17(ScenePctService<?> scenePct, float value, float currentValue, boolean enabled) {
      float nextValue = enabled ? clampSize(value, scenePct.minWidth, scenePct.maxWidth, currentValue) : clampSize(value, scenePct.minHeight, scenePct.maxHeight, currentValue);
      return calculateValue18(nextValue);
   }
   ...
   record PresentationTransform(float scale, float tx, float ty) {
      static final ScenePctService.PresentationTransform IDENTITY = new ScenePctService.PresentationTransform(1.0F, 0.0F, 0.0F);
      ...
      ScenePctService.PresentationTransform append(ScenePctService<?> scenePct) {
         ...
         return new ScenePctService.PresentationTransform(this.scale * value, this.scale * targetValue + this.tx, this.scale * inputValue + this.ty);
      }
   ...
   private record ScrollbarMetrics(float trackStart, float trackSize, float thumbStart, float thumbSize, float crossStart, float crossSize) {
   }
}
