...
public final class SceneCodec {
  ...
  private static final float value = 1.0E-5f;
  private final Map<ScenePctService<?>, State> entries = new IdentityHashMap();

  public void transition(
      ScenePctService<?> scenePctService, Action action, Preset preset, Options options) {
    Action action2;
    ...
    if (action2 == Action.TOGGLE) {
      State state = this.entries.get(scenePctService);
      if (state != null) {
  ...
  public void show(ScenePctService<?> scenePctService, Preset preset, Options options) {
    ...
    if (state == null) {
      Baseline baseline =
          new Baseline(
              motionFiniteService.get(scenePctService),
              motionFiniteService2.get(scenePctService),
              normalizedOptions.scale(),
              normalizedOptions.opacity(),
              SceneCodec.m8vgq2z7bq8j(scenePctService));
      state = new State(baseline, Phase.ENTERING, spec);
      this.entries.put(scenePctService, state);
    ...
    if (state == null) {
      Baseline baseline =
          new Baseline(
              motionFiniteService.get(scenePctService),
              motionFiniteService2.get(scenePctService),
              motionFiniteService3.get(scenePctService),
              motionFiniteService4.get(scenePctService),
              SceneCodec.m8vgq2z7bq8j(scenePctService));
  ...
  public void tick(float f) {
    if (!Float.isFinite(f) || f <= 0.0f || this.entries.isEmpty()) {
      return;
    }
    Iterator<Map.Entry<ScenePctService<?>, State>> iterator = this.entries.entrySet().iterator();
  ...
  public void clear() {
    for (Map.Entry<ScenePctService<?>, State> entry : this.entries.entrySet()) {
  ...
  public boolean isTransitioning(ScenePctService<?> scenePctService) {
    return (scenePctService != null && this.entries.containsKey(scenePctService) ? 1 : 0) != 0;
  }
  ...
  private void mcgxhlos0vxs(ScenePctService<?> scenePctService, State state, boolean bl) {
    ...
    if (bl) {
      motionFiniteService4.set(scenePctService, 0.0f);
      motionFiniteService3.set(scenePctService, baseline.scale() * spec.startScaleFactor());
    ...
    f =
        Math.max(
            f,
            SceneCodec.mf03b46fduno(
                scenePctService,
                motionFiniteService4,
                baseline.opacity(),
                spec.enterOpacityAnim()));
    f =
        Math.max(
            f,
            SceneCodec.mf03b46fduno(
                ...
                baseline.scale(),
                spec.enterTransformAnim()));
    if (ftxbgif29y4.supports(scenePctService)) {
      f =
          Math.max(
              f,
              SceneCodec.mf03b46fduno(
                  scenePctService, ftxbgif29y4, baseline.edgeSoftness(), spec.enterEdgeAnim()));
    }
  ...
  private void mcvpayld2c8h(ScenePctService<?> scenePctService, State state) {
    ...
    f =
        Math.max(
            f,
            SceneCodec.mf03b46fduno(
                scenePctService, motionFiniteService4, 0.0f, spec.exitOpacityAnim()));
    f =
        Math.max(
            f,
            SceneCodec.mf03b46fduno(
                scenePctService,
                motionFiniteService3,
                baseline.scale() * spec.exitScaleFactor(),
                spec.exitTransformAnim()));
      ...
      f =
          Math.max(
              ...
              SceneCodec.mf03b46fduno(
                  scenePctService,
                  ftxbgif29y4,
                  baseline.edgeSoftness() + spec.exitBlur(),
                  spec.exitEdgeAnim()));
    }
    state.remaining = f = Math.max(f, SceneCodec.m79kho1ym98(scenePctService, baseline, spec));
  }

  private void mry0h8m3y4n(
      ScenePctService<?> scenePctService, State state, NormalizedOptions normalizedOptions) {
    Baseline baseline;
    if (state != null) {
      SceneCodec.m8eni77lz33n(scenePctService, state.spec);
    ...
    scenePctService.visible(true);
    this.entries.remove(scenePctService);
  }
  ...
  private static void mj8kwqlys6de(ScenePctService<?> scenePctService, Baseline baseline) {
    motionFiniteService.set(scenePctService, baseline.translateX());
    motionFiniteService2.set(scenePctService, baseline.translateY());
    motionFiniteService3.set(scenePctService, baseline.scale());
    motionFiniteService4.set(scenePctService, baseline.opacity());
    if (ftxbgif29y4.supports(scenePctService)) {
      ftxbgif29y4.set(scenePctService, baseline.edgeSoftness());
    }
  }

  private static float mf03b46fduno(
      ScenePctService<?> scenePctService,
      MotionFiniteService motionFiniteService,
      float f,
      SceneEaseHandler sceneEaseHandler) {
  ...
  private static float m1h92thw646c(
      ScenePctService<?> scenePctService, Baseline baseline, Spec spec) {
  ...
  private static float m79kho1ym98(
      ScenePctService<?> scenePctService, Baseline baseline, Spec spec) {
    return switch (spec.axis().ordinal()) {
      default -> throw new MatchException(null, null);
      case 1 ->
          SceneCodec.mf03b46fduno(
              scenePctService,
              motionFiniteService,
              baseline.translateX() + (float) spec.sign() * spec.offset(),
              spec.exitTransformAnim());
      case 2 ->
          SceneCodec.mf03b46fduno(
              scenePctService,
              motionFiniteService2,
              baseline.translateY() + (float) spec.sign() * spec.offset(),
              spec.exitTransformAnim());
      case 0 -> 0.0f;
    };
  }

  private static void m4qxr21qmusk(
      ScenePctService<?> scenePctService, Baseline baseline, Spec spec, float f) {
    switch (spec.axis().ordinal()) {
      case 1:
        {
          motionFiniteService.set(scenePctService, baseline.translateX() + (float) spec.sign() * f);
          break;
        }
      case 2:
        {
          motionFiniteService2.set(
              scenePctService, baseline.translateY() + (float) spec.sign() * f);
          break;
        }
    }
  }
  ...
  private static void m8eni77lz33n(ScenePctService<?> scenePctService, Spec spec) {
    MotionAnimateService.cancel(scenePctService, motionFiniteService4);
    MotionAnimateService.cancel(scenePctService, motionFiniteService3);
    if (spec != null) {
      if (spec.axis() == Axis.X) {
        MotionAnimateService.cancel(scenePctService, motionFiniteService);
      }
      if (spec.axis() == Axis.Y) {
        MotionAnimateService.cancel(scenePctService, motionFiniteService2);
      }
    }
    if (ftxbgif29y4.supports(scenePctService)) {
    ...
    double d5 =
        d =
            d4 <= 0.0
                ...
                : (d2 - Math.sqrt(d4)) * Double.longBitsToDouble(4602678819172646912L);
    ...
    return (float)
        Math.max(
            ...
            Math.min(
                ...
                Math.log(Double.longBitsToDouble(4652007308841189376L)) / d));
  }
  ...
  private static NormalizedOptions m8wfaa39atxa(Preset preset, Options options) {
    ...
    float f =
        Float.isFinite(options2.duration) && options2.duration > 0.0f
            ? options2.duration
            : SceneCodec.mj7pmkqqrk0w(preset);
    float f2 =
        Float.isFinite(options2.offset) && options2.offset >= 0.0f
            ? options2.offset
            : SceneCodec.m9blk30g6ezv(preset);
    float f3 =
        Float.isFinite(options2.opacity) ? Math.max(0.0f, Math.min(1.0f, options2.opacity)) : 1.0f;
  ...
  private static Spec mhychxh4x3ah(Preset preset, NormalizedOptions normalizedOptions) {
    ...
    Record record =
        preset == Preset.FADE ? SceneEaseHandler.Tween.ease(f) : SceneEaseHandler.Spring.SNAPPY;
    SceneEaseHandler.Tween tween =
        SceneEaseHandler.Tween.ease(Math.min(f, Float.intBitsToFloat(1046562734)));
    SceneEaseHandler.Tween tween2 =
        SceneEaseHandler.Tween.ease(Math.min(f, Float.intBitsToFloat(1049582633)));
    ...
    return switch (preset.ordinal()) {
      ...
      case 0 ->
          new Spec(
              ...
              Axis.NONE,
              1,
              (SceneEaseHandler) ((Object) record),
              tween,
              tween2,
              tween3,
              tween4,
      ...
      case 1 ->
          new Spec(
              Float.intBitsToFloat(1063339950),
      ...
      case 7 ->
          new Spec(
              Float.intBitsToFloat(1062333317),
              Float.intBitsToFloat(1063004406),
              0.0f,
              Axis.NONE,
              1,
              (SceneEaseHandler) ((Object) record),
      ...
      case 9 ->
          new Spec(
              Float.intBitsToFloat(1064715682),
              Float.intBitsToFloat(1065084781),
              f2,
              Axis.Y,
              1,
              new SceneEaseHandler.Spring(
                  Float.intBitsToFloat(1109131264), Float.intBitsToFloat(1140457472)),
              SceneEaseHandler.Tween.exit(Float.intBitsToFloat(1042536202)),
              SceneEaseHandler.Tween.enter(Float.intBitsToFloat(1043878380)),
              SceneEaseHandler.Tween.exit(Float.intBitsToFloat(1041194025)),
              SceneEaseHandler.Tween.enter(Float.intBitsToFloat(1043878380)),
              SceneEaseHandler.Tween.exit(Float.intBitsToFloat(1041194025)),
              0.0f,
              0.0f);
      case 8 -> {
        ...
        SceneEaseHandler.Tween var12_12 =
            SceneEaseHandler.Tween.enter(f * Float.intBitsToFloat(1060655596));
        SceneEaseHandler.Tween var13_13 =
            SceneEaseHandler.Tween.exit(f * Float.intBitsToFloat(1061662228));
        yield new Spec(
            Float.intBitsToFloat(1064883454),
            Float.intBitsToFloat(1065051226),
            0.0f,
            Axis.NONE,
            1,
            var10_10,
  ...
  private record Spec(
      ...
      Axis axis,
      int sign,
      SceneEaseHandler enterTransformAnim,
      SceneEaseHandler exitTransformAnim,
      SceneEaseHandler enterOpacityAnim,
      SceneEaseHandler exitOpacityAnim,
      SceneEaseHandler enterEdgeAnim,
      SceneEaseHandler exitEdgeAnim,
      float startBlur,
      float exitBlur) {}

  private record Baseline(
      float translateX, float translateY, float scale, float opacity, float edgeSoftness) {}
