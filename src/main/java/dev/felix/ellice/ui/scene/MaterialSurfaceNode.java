...
public class MaterialSurfaceNode extends LayoutContainerNode
{
    ...
    public float getAnimProperty(final String s) {
        return switch (s) {
            ...
            case "cornerRadiusTL",  "cornerRadiusTopLeft" -> (this.fcfizzf4a801 >= 0.0f) ? this.fcfizzf4a801 : this.fh41zlzpxng4;
            case "cornerRadiusTR",  "cornerRadiusTopRight" -> (this.f19hso0oq35h >= 0.0f) ? this.f19hso0oq35h : this.fh41zlzpxng4;
            case "cornerRadiusBR",  "cornerRadiusBottomRight" -> (this.f57xhv4nfrbw >= 0.0f) ? this.f57xhv4nfrbw : this.fh41zlzpxng4;
            case "cornerRadiusBL",  "cornerRadiusBottomLeft" -> (this.fctdx880k132 >= 0.0f) ? this.fctdx880k132 : this.fh41zlzpxng4;
    ...
    public int getColorProperty(final String anObject) {
        ...
        return ("color".equals(anObject) || "tintColor".equals(anObject)) ? this.f3xjzqa5j64i : super.getColorProperty(anObject);
    }
    ...
    @Override
    public void drawTree(final CompositorPushPresentationScaleService compositorPushPresentationScaleService, final float n) {
        this.drawTree(compositorPushPresentationScaleService, n, PresentationTransform.IDENTITY);
    }
    
    @Override
    void drawTree(final CompositorPushPresentationScaleService compositorPushPresentationScaleService, final float n, final PresentationTransform presentationTransform) {
        if (!this.visible) {
            return;
        }
        final float max = Math.max(0.0f, this.opacity);
        this.effectiveOpacity = max * n;
        if (this.effectiveOpacity < Float.intBitsToFloat(1000593162) || this.cw <= 0.0f || this.ch <= 0.0f) {
            return;
        }
        final PresentationTransform presentationTransform2 = this.presentationTransform(presentationTransform);
        ...
        try {
            this.effectiveEdgeSoftness = Math.min(Math.max((this.parent != null) ? this.parent.effectiveEdgeSoftness : 0.0f, this.fhhzyl9d2dwc), Math.min(cw, ch) * Float.intBitsToFloat(1053609165));
            final CompositorPushPresentationScaleService.LayerEffect layerEffect = new CompositorPushPresentationScaleService.LayerEffect(this.cx, this.cy, this.cw, this.ch, ((this.fcfizzf4a801 >= 0.0f) ? this.fcfizzf4a801 : this.fh41zlzpxng4) * presentationTransform2.scale(), ((this.f19hso0oq35h >= 0.0f) ? this.f19hso0oq35h : this.fh41zlzpxng4) * presentationTransform2.scale(), ((this.f57xhv4nfrbw >= 0.0f) ? this.f57xhv4nfrbw : this.fh41zlzpxng4) * presentationTransform2.scale(), ((this.fctdx880k132 >= 0.0f) ? this.fctdx880k132 : this.fh41zlzpxng4) * presentationTransform2.scale(), max, this.f8yncn0wzoi2 * presentationTransform2.scale(), this.fihicp8ssnan, this.felp8gm02hoa, this.f25nw8f70jw3, this.f20tgdlhmu0k, this.f3xjzqa5j64i, this.effectiveEdgeSoftness * presentationTransform2.scale(), this.f9kc950746tn.shaderId, this.f68qpeq7slwd, this.fr1pv49hkei ? this.f6b8waj8zs0s : 0, this.subtreeVersion(), this.f4cdt7ij1hs7.shaderId, this.fiidemq6d4oo, this.fwnvh6ql5fq, this.faz5i39520ck, this.fesr2ocbeo22, this.fewdqj8hx8gp, this.fa6igwldey2, this.hoverProgress, this.pressProgress);
            if (this.fr1pv49hkei && compositorPushPresentationScaleService.isLayerCacheValid(this.f6b8waj8zs0s, this.subtreeVersion(), this.cx, this.cy, this.cw, this.ch)) {
                compositorPushPresentationScaleService.nextLayer(layerEffect);
                compositorPushPresentationScaleService.nextLayer();
                return;
            }
            compositorPushPresentationScaleService.nextLayer(layerEffect);
            compositorPushPresentationScaleService.pushClip(this.cx, this.cy, this.cw, this.ch, this.fh41zlzpxng4);
            try {
                final Iterator<ScenePctService<?>> iterator = this.children().iterator();
                while (iterator.hasNext()) {
                    iterator.next().drawTree(compositorPushPresentationScaleService, n, presentationTransform2);
                }
            }
            finally {
    ...
    public enum Material
    {
        ...
        static Material parse(final String s) {
            ...
            final String replace = s.trim().toLowerCase().replace("_", "").replace("-", "").replace(" ", "");
            return switch (replace) {
                case "holo",  "hologlass",  "holographic",  
                     "hologram" -> Material.HOLO_GLASS;
                ...
                case "chroma",  "chromatic",  "iridescent",  
                     "ellice" -> Material.CHROMA;
