...
public final class SvgFillTrianglesService {
  ...
  private static void updateState(float[] floats, int index, List<Float> items) {
    if (index >= 3) {
      ...
      while (value > 2 && previousIndex < value) {
        ...
        if (calculateValue2(
                    sourceValue, targetValue, inputValue, outputValue, resultValue, candidateValue)
                > 0.0F
            && checkCondition(
                floats,
                ints,
  ...
  private static float calculateValue(float[] floats, int value) {
    ...
    for (int index = 0; index < value; index++) {
      ...
      currentValue +=
          floats[index * 2] * floats[nextValue * 2 + 1]
              - floats[nextValue * 2] * floats[index * 2 + 1];
    }
  ...
  private static float calculateValue2(
      ...
      float sourceValue,
      float targetValue) {
    return (nextValue - value) * (targetValue - currentValue)
  ...
  private static boolean checkCondition2(
      ...
      float outputValue) {
    float resultValue =
        calculateValue2(value, currentValue, nextValue, previousValue, sourceValue, targetValue);
    float candidateValue =
        calculateValue2(value, currentValue, sourceValue, targetValue, inputValue, outputValue);
    float selectedValue =
        calculateValue2(value, currentValue, inputValue, outputValue, nextValue, previousValue);
    return !(resultValue < 0.0F) && !(candidateValue < 0.0F) && !(selectedValue < 0.0F)
        || !(resultValue > 0.0F) && !(candidateValue > 0.0F) && !(selectedValue > 0.0F);
  }

  public static float[] strokeTriangles(
      SvgCodec svgCodec,
      float value,
    ...
    if (index >= 2) {
      ...
      if (index >= 2) {
        ...
        for (int previousIndex = 1; previousIndex < index; previousIndex++) {
          previousFloats[previousIndex] =
              previousFloats[previousIndex - 1] + nextFloats[previousIndex - 1];
        }
          ...
          if (!(nextFloats[sourceIndex] < 0.001F)) {
            ...
            float targetValue =
                inputIndex > targetIndex
                    ? previousFloats[inputIndex]
                    : previousFloats[index - 1] + nextFloats[currentIndex - 1];
            float[] sourceFloats =
                createFloat(
                    targetIndex,
                    sourceIndex,
            ...
            updateState5(
                items,
                subPath.x(targetIndex) + sourceFloats[0],
                subPath.y(targetIndex) + sourceFloats[1],
                sourceValue);
            updateState5(
                items,
                subPath.x(targetIndex) + sourceFloats[2],
                subPath.y(targetIndex) + sourceFloats[3],
                sourceValue);
            updateState5(
                items,
                subPath.x(inputIndex) + targetFloats[0],
                subPath.y(inputIndex) + targetFloats[1],
                targetValue);
            updateState5(
                items,
                subPath.x(targetIndex) + sourceFloats[2],
            ...
            updateState5(
                items,
                subPath.x(inputIndex) + targetFloats[2],
                subPath.y(inputIndex) + targetFloats[3],
                targetValue);
            updateState5(
                items,
                subPath.x(inputIndex) + targetFloats[0],
        ...
        for (int outputIndex = inputValue; outputIndex < outputValue; outputIndex++) {
          int resultIndex =
              enabled ? (outputIndex - 1 + currentIndex) % currentIndex : outputIndex - 1;
        ...
        if (!enabled) {
          float previousX =
              nextFloats[0] > 0.001F ? (subPath.x(1) - subPath.x(0)) / nextFloats[0] : 1.0F;
          float previousY =
              nextFloats[0] > 0.001F ? (subPath.y(1) - subPath.y(0)) / nextFloats[0] : 0.0F;
          updateState4(
              items,
              subPath.x(0),
              subPath.y(0),
          ...
          float sourceX =
              nextFloats[selectedIndex] > 0.001F
                  ? (subPath.x(index - 1) - subPath.x(index - 2)) / nextFloats[selectedIndex]
                  : 1.0F;
          float sourceY =
              nextFloats[selectedIndex] > 0.001F
                  ? (subPath.y(index - 1) - subPath.y(index - 2)) / nextFloats[selectedIndex]
                  : 0.0F;
          updateState4(
              items,
              subPath.x(index - 1),
              ...
              sourceX,
              sourceY,
              floats[selectedIndex],
              currentFloats[selectedIndex],
              value,
              previousFloats[index - 1],
              lineCap,
              false);
        }
      }
  ...
  private static float[] createFloat(
    ...
    if (enabled
        || (outputValue == 0 || value != 0) && (outputValue != 0 || value != currentValue - 1)) {
      int currentIndex =
          outputValue != 0 ? (index - 1 + nextValue) % nextValue : (index + 1) % nextValue;
      ...
      float pendingValue =
          (float) Math.sqrt(computedValue * computedValue + cachedValue * cachedValue);
      ...
      return secondaryValue > 0.0F
          ? new float[] {
            computedValue * primaryValue,
            cachedValue * primaryValue,
            -targetValue * previousValue,
            -inputValue * previousValue
          }
          : new float[] {
  ...
  private static void updateState3(
    ...
    float defaultValue =
        (float) Math.sqrt(candidateValue * candidateValue + selectedValue * selectedValue);
    if (!(defaultValue < 0.001F)) {
      ...
      if (!(resolvedValue <= resultValue * outputValue)) {
        ...
        if (lineJoin == SvgFillTrianglesService.LineJoin.ROUND) {
          float primaryValue =
              (float) Math.atan2(cachedValue * sourceValue, cachedValue * previousValue);
          float secondaryValue =
              (float) Math.atan2(cachedValue * inputValue, cachedValue * targetValue);
          ...
          int temporaryValue =
              Math.max(2, (int) Math.ceil(Math.abs(tertiaryValue) / 0.7853981633974483));
          ...
          for (int index = 0; index < temporaryValue; index++) {
            ...
            updateState5(
                items,
                value + outputValue * (float) Math.cos(requestedValue),
                currentValue + outputValue * (float) Math.sin(requestedValue),
                nextValue);
            updateState5(
                items,
                value + outputValue * (float) Math.cos(actualValue),
                currentValue + outputValue * (float) Math.sin(actualValue),
                nextValue);
          }
        } else {
          updateState5(items, activeValue, fallbackValue, nextValue);
          updateState5(
              items,
              value + cachedValue * previousValue * outputValue,
              currentValue + cachedValue * sourceValue * outputValue,
              nextValue);
          updateState5(
              items,
              value + cachedValue * targetValue * outputValue,
              currentValue + cachedValue * inputValue * outputValue,
              nextValue);
        }
      }
    }
    ...
    if (lineCap != SvgFillTrianglesService.LineCap.BUTT) {
      if (lineCap == SvgFillTrianglesService.LineCap.ROUND) {
        ...
        for (int index = 0; index < 8; index++) {
          ...
          updateState5(
              items,
              value + inputValue * (float) Math.cos(resolvedValue),
              currentValue + inputValue * (float) Math.sin(resolvedValue),
              outputValue);
          updateState5(
              items,
              value + inputValue * (float) Math.cos(computedValue),
              currentValue + inputValue * (float) Math.sin(computedValue),
              outputValue);
        }
        ...
        updateState5(
            items,
            value + activeValue + cachedValue,
            currentValue + fallbackValue + pendingValue,
            outputValue);
        updateState5(items, value - activeValue, currentValue - fallbackValue, outputValue);
        updateState5(
            items,
  ...
  private static void updateState5(
      List<Float> items, float value, float currentValue, float nextValue) {
