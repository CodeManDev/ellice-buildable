...
public final class SvgGetTextureService {
   ...
   public RhiBlendStateService.TextureHandle getTexture(Path path, int textureId, int currentTextureId) {
      ...
      try (InputStream inputStream = Files.newInputStream(path)) {
         return this.createTextureHandle(inputStream, path.toUri(), currentText, textureId, currentTextureId);
   ...
   public RhiBlendStateService.TextureHandle getSdfTexture(Path path, float value) {
      if (Float.isFinite(value) && !(value <= 0.0F)) {
         ...
         try (InputStream inputStream = Files.newInputStream(path)) {
            RhiBlendStateService.TextureHandle currentTextureHandle = this.createTextureHandle2(inputStream, path.toUri(), currentText, value);
            this.text.put(currentText, currentTextureHandle);
            return currentTextureHandle;
         } catch (Exception exception) {
            CoreIsInitializedHandler.LOGGER.warn("SVG SDF load failed: {}", path.getFileName(), exception);
            this.text.put(currentText, RhiBlendStateService.TextureHandle.NONE);
            return RhiBlendStateService.TextureHandle.NONE;
         }
      } else {
   ...
   public RhiBlendStateService.TextureHandle getClasspathTexture(String path, int textureId, int currentTextureId) {
      ...
      try (InputStream inputStream = this.getClass().getResourceAsStream(path)) {
         if (inputStream == null) {
            ...
            return this.createTextureHandle(inputStream, null, currentPath, textureId, currentTextureId);
         }
      } catch (Exception exception) {
   ...
   private RhiBlendStateService.TextureHandle createTextureHandle(InputStream inputStream, URI uRI, String currentText, int textureId, int currentTextureId) {
      try {
         ...
         if (sVGDocument == null) {
            ...
            sVGDocument.renderWithPlatform(NullPlatformSupport.INSTANCE, graphics2D, new ViewBox(0.0F, 0.0F, nextTextureId, previousTextureId));
            graphics2D.dispose();
   ...
   private RhiBlendStateService.TextureHandle createTextureHandle2(InputStream inputStream, URI uRI, String text, float value) {
      try {
         SVGDocument sVGDocument = this.sVGLoader.load(inputStream, uRI, this.loaderContext);
         if (sVGDocument == null) {
            return RhiBlendStateService.TextureHandle.NONE;
         ...
         graphics2D.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY);
         graphics2D.setRenderingHint(RenderingHints.KEY_STROKE_CONTROL, RenderingHints.VALUE_STROKE_PURE);
         sVGDocument.renderWithPlatform(NullPlatformSupport.INSTANCE, graphics2D, new ViewBox(0.0F, 0.0F, shortValue, currentShortValue));
         graphics2D.dispose();
   ...
   private RhiBlendStateService.TextureHandle createTextureHandle4(BufferedImage bufferedImage, int textureId, int currentTextureId, float value) {
      if (this.rhiOperationHandler == null) {
         return RhiBlendStateService.TextureHandle.NONE;
      }
   ...
   private static float[] createFloat(BufferedImage bufferedImage, int value, int currentValue, float nextValue) {
      ...
      for (int previousIndex = 0; previousIndex < nextFloats.length; previousIndex++) {
         float resultValue = (float)(Math.sqrt(floats[previousIndex]) - Math.sqrt(currentFloats[previousIndex]));
   ...
   private static void updateState2(float[] floats, float[] currentFloats, int[] ints, float[] nextFloats, int value) {
      int index = -1;
      ...
      for (int currentIndex = 0; currentIndex < value; currentIndex++) {
         if (!(floats[currentIndex] >= 1.0E9F)) {
            ...
            while (true) {
               ...
               currentValue = (floats[currentIndex] + currentIndex * currentIndex - (floats[nextIndex] + nextIndex * nextIndex)) / (2.0F * currentIndex - 2.0F * nextIndex);
