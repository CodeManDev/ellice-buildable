...
public final class MaterialIsLightService {
  ...
  public static int layer(int value, int currentValue, float nextValue) {
    int previousValue =
        Math.round(
            (value >>> 16 & 0xFF) * (1.0F - nextValue) + (currentValue >>> 16 & 0xFF) * nextValue);
    int sourceValue =
        Math.round(
            (value >>> 8 & 0xFF) * (1.0F - nextValue) + (currentValue >>> 8 & 0xFF) * nextValue);
    int targetValue =
        Math.round((value & 0xFF) * (1.0F - nextValue) + (currentValue & 0xFF) * nextValue);
