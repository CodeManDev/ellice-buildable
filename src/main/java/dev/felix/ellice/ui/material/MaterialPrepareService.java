...
public final class MaterialPrepareService extends ScenePctService<MaterialPrepareService> {
   ...
   public void prepare(CompositorPushPresentationScaleService compositorPushPresentationScale, String text, float currentValue) {
      text = PrivacyRevisionService.replace(text);
      if (!text.equals(this.text2) || this.value4 != currentValue || this.timestamp != compositorPushPresentationScale.fontMetricsRevision()) {
         if (!text.equals(this.text2)) {
            this.text2 = text;
            this.opacity(0.0F);
            this.translateY(5.0F);
            MotionAnimateService.animate(this, MotionColorsContainer.Floats.OPACITY, 1.0F, SceneEaseHandler.Tween.ease(0.16F));
            MotionAnimateService.animate(this, MotionColorsContainer.Floats.TRANSLATE_Y, 0.0F, MaterialIsLightService.SPATIAL);
         }

         this.value4 = currentValue;
         this.timestamp = compositorPushPresentationScale.fontMetricsRevision();
         float nextValue = Math.max(
            24.0F, Math.min(286.0F, currentValue - 28.0F)
         );
         String[] strings = text.split("\\n", 2);
         this.text3 = createText(compositorPushPresentationScale, strings[0], 13.0F, "material-roboto-medium", nextValue);
         ...
         for (String currentText : currentLength.split("\\n")) {
            ...
            for (String nextText : currentText.split("\\s+")) {
               ...
               if (!stringBuilder.isEmpty()
                  && compositorPushPresentationScale.textWidthLiteral(previousText, 11.0F, TextMode.REGULAR, "material-roboto", 0.0F) > nextValue) {
                  arrayList.add(stringBuilder.toString());
                  stringBuilder.setLength(0);
               }

               if (compositorPushPresentationScale.textWidthLiteral(nextText, 11.0F, TextMode.REGULAR, "material-roboto", 0.0F) > nextValue) {
                  for (int previousValue : nextText.codePoints().toArray()) {
                     ...
                     if (!stringBuilder.isEmpty()
                        && compositorPushPresentationScale.textWidthLiteral(
                              stringBuilder.toString() + sourceText, 11.0F, TextMode.REGULAR, "material-roboto", 0.0F
                           )
                           > nextValue) {
                        arrayList.add(stringBuilder.toString());
                        stringBuilder.setLength(0);
         ...
         if (arrayList.size() > 4) {
            ...
            arrayList.set(3, createText(compositorPushPresentationScale, (String)arrayList.get(3) + "…", 11.0F, "material-roboto", nextValue));
         }

         this.text4 = List.copyOf(arrayList);
         float sourceValue = compositorPushPresentationScale.textWidthLiteral(
            this.text3, 13.0F, TextMode.REGULAR, "material-roboto-medium", 0.0F
         );

         for (String targetText : this.text4) {
            sourceValue = Math.max(
               sourceValue, compositorPushPresentationScale.textWidthLiteral(targetText, 11.0F, TextMode.REGULAR, "material-roboto", 0.0F)
            );
         }
   ...
   private static String createText(CompositorPushPresentationScaleService compositorPushPresentationScale, String text, float value, String currentText, float currentValue) {
      if (compositorPushPresentationScale.textWidthLiteral(text, value, TextMode.REGULAR, currentText, 0.0F) <= currentValue) {
         return text;
      }

      String nextText = text;

      while (!nextText.isEmpty() && compositorPushPresentationScale.textWidthLiteral(nextText + "…", value, TextMode.REGULAR, currentText, 0.0F) > currentValue) {
