...
public final class MaterialTextService {
  ...
  public static ComponentKeyService<SceneTextService> text(
      String currentText, float value, int currentValue) {
    return ComponentBoxService.text(
        currentText,
        item ->
            item.fontFamily("material-roboto")
                .fontSize(value)
                .lineHeight(
                    value <= 12.0F
                        ? 16.0F
                        : (value <= 14.0F
                            ? 20.0F
                            : (value <= 16.0F ? 24.0F : (value <= 22.0F ? 28.0F : 32.0F))))
                .letterSpacing(
                    value <= 12.0F
                        ? 0.4F
                        : (value <= 14.0F ? 0.2F : (value <= 16.0F ? 0.5F : 0.0F)))
                .color(currentValue)
  ...
  public static ComponentKeyService<MaterialJoinedService> button(
      String text, String currentText, Runnable runnable, boolean enabled) {
    return button(
        text,
        currentText,
        runnable,
  ...
  public static ComponentKeyService<MaterialJoinedService> iconButton(
  ...
  public static ComponentKeyService<SceneCornerRadiusService> divider() {
    return ComponentBoxService.panel(
        item ->
            item.size(LayoutOperationHandler.percent(100.0F), LayoutOperationHandler.px(1.0F))
  ...
  public static ComponentKeyService<InteractiveSurfacePanel> search(
      String text, String currentText, String nextText, Consumer<String> consumer) {
  ...
  public static ComponentKeyService<InteractiveSurfacePanel> search(
    ...
    return ComponentBoxService.<InteractiveSurfacePanel>node(
            ...
            item ->
                item.height(LayoutOperationHandler.px(enabled ? 40.0F : 56.0F))
                    .minWidth(0.0F)
                    .cornerRadius(28.0F)
                    .flexShrink(0.0F)
                    .direction(ScenePctService.Direction.ROW)
                    .align(ScenePctService.Align.CENTER)
                    .padding(0.0F, 8.0F),
            icon("search", MaterialIsLightService.ON_SURFACE)
                .props(item -> item.margin(0.0F, 8.0F)),
            ...
            ComponentBoxService.<MaterialJoinedService>node(
                    ...
                    item -> {
                      ...
                      item.id(currentText + ".clear")
                          .size(enabled ? 32.0F : 48.0F, enabled ? 32.0F : 48.0F)
                          .flexShrink(0.0F)
                          .direction(ScenePctService.Direction.ROW)
                          .align(ScenePctService.Align.CENTER)
                          .justify(ScenePctService.Justify.CENTER)
                          ...
                          .onClick(
                              () -> {
                                if (item.parent() != null
                                    && item.parent().findById(currentText)
                                        instanceof
