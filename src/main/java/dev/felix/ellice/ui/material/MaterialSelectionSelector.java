...
public final class MaterialSelectionSelector extends LayoutContainerNode {
   private static final MotionFiniteService motionFiniteService = MotionFiniteService.finite("materialChoiceLeft", item -> item instanceof MaterialSelectionSelector);
   private static final MotionFiniteService motionFiniteService2 = MotionFiniteService.finite("materialChoiceRight", item -> item instanceof MaterialSelectionSelector);
   private String text = "";
   private float value;
   ...
   private void updateState() {
      ScenePctService scenePct = this.children()
         .stream()
      ...
      if (scenePct == null) {
         ...
         if (!this.enabled || previousValue != 0) {
            ...
            MotionAnimateService.animate(this, motionFiniteService, currentValue, sourceValue != 0 ? MaterialEnterService.TRAILING_EDGE : MaterialEnterService.LEADING_EDGE);
            MotionAnimateService.animate(this, motionFiniteService2, nextValue, sourceValue != 0 ? MaterialEnterService.LEADING_EDGE : MaterialEnterService.TRAILING_EDGE);
         }
         ...
         if (this.enabled2 || previousValue != 0) {
            ...
            if (targetValue != this.scrollTarget) {
               ...
               MotionAnimateService.animate(this, MotionColorsContainer.Floats.SCROLL_Y, targetValue, MaterialEnterService.PAGE);
            }
         }
      }
   }
   ...
   protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      if (this.enabled) {
         ...
         float width = Math.max(
            24.0F,
            Math.min(this.value4 - this.value3 + 32.0F, this.value2 - this.value)
         );
         float height = Math.max(
            this.paddingLeft + width / 2.0F, Math.min(this.value6 - width / 2.0F, (this.value + this.value2) / 2.0F)
         );
         compositorPushPresentationScale.roundedRect(
