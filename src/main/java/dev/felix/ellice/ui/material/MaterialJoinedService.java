...
public final class MaterialJoinedService extends SceneCornerRadiusService {
   ...
   private static final MotionFiniteService motionFiniteService = MotionFiniteService.finite("materialDepth", item -> item instanceof MaterialJoinedService);
   private static final MotionFiniteService motionFiniteService2 = MotionFiniteService.finite("materialInnerRadius", item -> item instanceof MaterialJoinedService);
   private static final MotionFiniteService motionFiniteService3 = MotionFiniteService.finite("materialRipple", item -> item instanceof MaterialJoinedService);
   ...
   public MaterialJoinedService joined(boolean enabled, boolean currentEnabled, boolean nextEnabled) {
   ...
   public MaterialJoinedService colors(int color, int currentColor) {
      ...
      if (!this.enabled4) {
         ...
         MotionAnimateService.animate(this, MotionColorsContainer.Colors.BACKGROUND, color, MaterialIsLightService.FAST_EFFECTS);
      }
   ...
   public MaterialJoinedService shape(float currentValue, float nextValue) {
      if (!this.enabled13) {
         ...
         this.value = currentValue;
         if (!this.pressed) {
            MotionAnimateService.animate(this, MotionColorsContainer.Floats.CORNER_RADIUS, currentValue, MaterialIsLightService.FAST_SPATIAL);
         }
      }
   ...
   public MaterialJoinedService available(boolean currentEnabled) {
      ...
      this.interactive(currentEnabled).pointerEvents(currentEnabled).cursorStyle(currentEnabled ? ScenePctService.CursorStyle.POINTER : ScenePctService.CursorStyle.DEFAULT);
      return this;
   }
   ...
   protected void onPress(float currentValue, float nextValue) {
      if (this.enabled) {
         this.value4 = this.cw > 0.0F ? Math.max(0.0F, Math.min(1.0F, (currentValue - this.cx) / this.cw)) : 0.5F;
         this.value5 = this.ch > 0.0F ? Math.max(0.0F, Math.min(1.0F, (nextValue - this.cy) / this.ch)) : 0.5F;
         this.enabled7 = true;
         motionFiniteService3.set(this, 0.0F);
         MotionAnimateService.animate(this, motionFiniteService3, 1.0F, MaterialIsLightService.EFFECTS);
         MotionAnimateService.animate(this, motionFiniteService, 1.0F, MaterialEnterService.PRESS);
         MotionAnimateService.animate(
            this, MotionColorsContainer.Floats.CORNER_RADIUS, Math.min(10.0F, this.value), MaterialEnterService.PRESS
         );
      }
   }
   ...
   protected void onRelease(boolean currentEnabled) {
      if (this.enabled) {
         MotionAnimateService.animate(this, motionFiniteService, 0.0F, MaterialEnterService.RELEASE);
         MotionAnimateService.animate(this, MotionColorsContainer.Floats.CORNER_RADIUS, this.value, MaterialEnterService.RELEASE);
      }
   }
   ...
   protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      ...
      float targetValue = Math.max(0.0F, Math.min(currentValue / 2.0F, this.getAnimProperty("cornerRadius")));
      int inputValue = this.enabled
      ...
      if (this.enabled5 != this.hovered || this.enabled6 != this.pressed) {
         ...
         MotionAnimateService.animate(this, MotionColorsContainer.Floats.PRESS_PROGRESS, this.pressed ? 1.0F : 0.0F, MaterialIsLightService.FAST_EFFECTS);
      }

      float outputValue = this.enabled8 && !this.enabled9 ? Math.max(0.0F, Math.min(this.value7, targetValue)) : targetValue;
      float resultValue = this.enabled8 && !this.enabled10 ? Math.max(0.0F, Math.min(this.value7, targetValue)) : targetValue;
      ...
      if (this.enabled) {
         float defaultValue = this.enabled2
            ? 0.1F
      ...
      if (this.enabled7 && this.value6 < 0.995F && this.enabled) {
         compositorPushPresentationScale.pushClip(previousValue, sourceValue, nextValue, currentValue, outputValue, resultValue, resultValue, outputValue);
