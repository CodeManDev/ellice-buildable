...
public final class MaterialEnterService {
  ...
  private static SceneEaseHandler.Spring createSpring(float value, float currentValue) {
    return new SceneEaseHandler.Spring(
        2.0F * value * (float) Math.sqrt(currentValue), currentValue);
  }
  ...
  public static void enter(
      ScenePctService<?> scenePct, float value, float currentValue, float nextValue) {
    if (scenePct != null) {
      ...
      if (value != 0.0F) {
        MotionAnimateService.animate(
            scenePct, MotionColorsContainer.Floats.TRANSLATE_X, 0.0F, PAGE, nextValue);
      }

      if (currentValue != 0.0F) {
        MotionAnimateService.animate(
            scenePct, MotionColorsContainer.Floats.TRANSLATE_Y, 0.0F, PAGE, nextValue);
      }
    }
  }
  ...
  public static void reveal(ScenePctService<?> scenePct, float value, float currentValue) {
    if (scenePct != null) {
      ...
      MotionAnimateService.animate(
          scenePct, MotionColorsContainer.Floats.OPACITY, 1.0F, SceneEaseHandler.Tween.ease(0.26F));
    }
  }
}
