...
public final class MaterialResponsiveService extends SceneCornerRadiusService {
  ...
  private static final MotionFiniteService motionFiniteService =
      MotionFiniteService.finite(
          "materialSurfaceActivity", item -> item instanceof MaterialResponsiveService);
  private static final MotionFiniteService motionFiniteService2 =
      MotionFiniteService.finite(
          "materialSurfaceEngagement", item -> item instanceof MaterialResponsiveService);
  private static final MotionFiniteService motionFiniteService3 =
      MotionFiniteService.finite(
          "materialSurfaceSelection", item -> item instanceof MaterialResponsiveService);
  ...
  public MaterialResponsiveService selected(boolean enabled) {
    if (!this.enabled4) {
      ...
      MotionAnimateService.animate(
          this, motionFiniteService3, enabled ? 1.0F : 0.0F, MaterialEnterService.RELEASE);
    }
  ...
  private static int calculateValue(ScenePctService<?> scenePct) {
    if (scenePct.visible && scenePct.pointerEvents()) {
      int value =
          !scenePct.isPressed()
                  && !(scenePct instanceof ControlLetterSpacingService controlLetterSpacing
                      && controlLetterSpacing.focused())
              ? (scenePct.isHovered() ? 1 : 0)
              : 3;
  ...
  protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
    ...
    if (y != this.count2) {
      ...
      MotionAnimateService.animate(
          ...
          (y & 2) != 0 ? 1.0F : 0.0F,
          (y & 2) != 0 ? MaterialEnterService.PRESS : MaterialEnterService.RELEASE);
      this.count2 = y;
    }
    ...
    if (targetValue > 0.001F) {
      float inputValue =
          Math.min(20.0F * this.presentationScale, this.ch - 12.0F * this.presentationScale)
              * targetValue;
      compositorPushPresentationScale.roundedRect(
          ...
          1.0F,
          mulAlpha(
              MaterialIsLightService.PRIMARY, this.effectiveOpacity * Math.min(1.0F, targetValue)));
    }
  }
}
