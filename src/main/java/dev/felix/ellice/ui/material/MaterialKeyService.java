   ...
   public boolean key(ScenePctService<?> scenePct, int value, int currentValue) {
      if (value == 258) {
         ...
         for (ScenePctService nextScenePct : (Iterable<ScenePctService>) (Iterable<?>) (arrayList)) {
            if (nextScenePct instanceof ControlLetterSpacingService controlLetterSpacing && controlLetterSpacing.focused()) {
         ...
         this.scenePctService = (ScenePctService<?>)arrayList.get(
            index < 0 ? (currentIndex < 0 ? arrayList.size() - 1 : 0) : Math.floorMod(index + currentIndex, arrayList.size())
         );
         ...
         if ((value == 257 || value == 32) && this.scenePctService instanceof MaterialJoinedService currentMaterialJoined && currentMaterialJoined.available()) {
            ArrayList currentArrayList = new ArrayList();
            updateState(scenePct, currentArrayList);
            if (currentArrayList.stream().anyMatch(item -> item instanceof ControlLetterSpacingService controlLetterSpacing && controlLetterSpacing.focused())) {
               return false;
            }
      ...
      for (ScenePctService currentScenePct = scenePct.parent(); currentScenePct != null; currentScenePct = currentScenePct.parent()) {
         if (!(currentScenePct.scrollMin() >= 0.0F)) {
            float value = scenePct.computedY() < currentScenePct.computedY() + 6.0F
               ...
               : Math.min(
                  0.0F, currentScenePct.computedY() + currentScenePct.computedH() - 6.0F - scenePct.computedY() - scenePct.computedH()
               );
            if (value != 0.0F) {
