...
public final class MaterialCheckboxValidator extends ScenePctService<MaterialCheckboxValidator> {
  private static final MotionFiniteService motionFiniteService =
      MotionFiniteService.finite(
          "materialSelection", item -> item instanceof MaterialCheckboxValidator);
  ...
  public MaterialCheckboxValidator selected(boolean enabled) {
    if (!this.enabled3) {
      ...
      MotionAnimateService.animate(
          this, motionFiniteService, enabled ? 1.0F : 0.0F, MaterialIsLightService.FAST_SPATIAL);
    }
  ...
  protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
    ...
    int sourceValue =
        MaterialIsLightService.layer(
            MaterialIsLightService.ON_SURFACE_VARIANT, MaterialIsLightService.PRIMARY, y);
    ...
    if (this.enabled && y > 0.001F && compositorPushPresentationScale.svgRenderer() != null) {
      RhiBlendStateService.TextureHandle textureHandle =
          compositorPushPresentationScale
              .svgRenderer()
              .getSdfTexture(MaterialTextService.iconPath("checkbox_check"));
      ...
      float targetValue = 10.0F * width * Math.max(0.0F, Math.min(1.06F, this.value));
      compositorPushPresentationScale.roundedRect(
