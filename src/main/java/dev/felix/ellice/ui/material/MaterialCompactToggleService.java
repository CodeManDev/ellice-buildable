...
public final class MaterialCompactToggleService {
   ...
   private static void updateState(
      ...
      if (!(nextValue <= 0.0F) && !(previousValue <= 0.0F)) {
         compositorPushPresentationScale.roundedRect(value, currentValue, nextValue, previousValue, sourceValue, targetValue, inputValue, outputValue, calculateValue(resultValue, candidateValue), 0.0F, 0.0F, 0, 0.0F, 0, 1.0F, 0.0F);
      }
   }
   ...
   public static void compactToggle(
      ...
      int selectedValue = currentEnabled
         ? MaterialIsLightService.layer(MaterialIsLightService.SURFACE_HIGHEST, MaterialIsLightService.PRIMARY, outputValue)
         : MaterialIsLightService.layer(MaterialIsLightService.SURFACE_CONTAINER, MaterialIsLightService.ON_SURFACE, 0.12F);
      ...
      float defaultValue = 12.0F
         + 4.0F * outputValue
         + (6.0F - 4.0F * outputValue)
            * Math.max(-0.2F, Math.min(1.0F, targetValue));
      float initialValue = resultValue + 11.0F + 14.0F * outputValue;
      int resolvedValue = currentEnabled
         ? MaterialIsLightService.layer(MaterialIsLightService.OUTLINE, MaterialIsLightService.ON_PRIMARY, outputValue)
         : MaterialIsLightService.layer(MaterialIsLightService.SURFACE_CONTAINER, MaterialIsLightService.ON_SURFACE, 0.38F);
      updateState(
   ...
   public static void compactSlider(
      ...
      float selectedValue = resultValue + (candidateValue - resultValue) * Math.max(0.0F, Math.min(1.0F, sourceValue));
      float defaultValue = resultValue + (candidateValue - resultValue) * Math.max(0.0F, Math.min(1.0F, targetValue));
      float initialValue = currentValue + (previousValue - 6.0F) / 2.0F;
      int resolvedValue = nextEnabled
         ? MaterialIsLightService.PRIMARY
         : MaterialIsLightService.layer(MaterialIsLightService.SURFACE_CONTAINER, MaterialIsLightService.ON_SURFACE, 0.38F);
      int computedValue = nextEnabled
   ...
   public static void toggle(
      ...
      int selectedValue = MaterialIsLightService.layer(MaterialIsLightService.SURFACE_HIGHEST, MaterialIsLightService.PRIMARY, outputValue);
      if (!currentEnabled) {
         selectedValue = MaterialIsLightService.layer(MaterialIsLightService.SURFACE_CONTAINER, MaterialIsLightService.ON_SURFACE, 0.12F);
      }
      ...
      if (currentEnabled && (enabled || nextEnabled || targetValue > 0.01F)) {
         updateState(
            ...
            20.0F,
            calculateValue(
               outputValue > 0.5 ? MaterialIsLightService.PRIMARY : MaterialIsLightService.ON_SURFACE,
      ...
      float cachedValue = (16.0F + 8.0F * outputValue) * (1.0F - computedValue)
         + 28.0F * computedValue;
      int pendingValue = MaterialIsLightService.layer(MaterialIsLightService.OUTLINE, MaterialIsLightService.ON_PRIMARY, outputValue);
      if (currentEnabled && (enabled || targetValue > 0.01F)) {
         pendingValue = MaterialIsLightService.layer(MaterialIsLightService.ON_SURFACE_VARIANT, MaterialIsLightService.PRIMARY_CONTAINER, outputValue);
      }

      if (!currentEnabled) {
         pendingValue = outputValue > 0.5
   ...
   public static void slider(
      ...
      int resolvedValue = nextEnabled ? MaterialIsLightService.PRIMARY : calculateValue(MaterialIsLightService.ON_SURFACE, 0.38F);
      int computedValue = nextEnabled ? MaterialIsLightService.SECONDARY_CONTAINER : calculateValue(MaterialIsLightService.ON_SURFACE, 0.12F);
      ...
      updateState(compositorPushPresentationScale, defaultValue - pendingValue / 2.0F, secondaryValue, pendingValue, 44.0F, 2.0F, 2.0F, 2.0F, 2.0F, resolvedValue, outputValue);
      if (enabled) {
         updateState(compositorPushPresentationScale, selectedValue - activeValue / 2.0F, secondaryValue, activeValue, 44.0F, 2.0F, 2.0F, 2.0F, 2.0F, resolvedValue, outputValue);
      }
