   ...
   private void updateState() {
      ...
      for (ScenePctService scenePct : (Iterable<ScenePctService>) (Iterable<?>) (currentItems)) {
         ...
         if (currentValue != 0 || !enabled) {
            ...
            float sourceValue = MotionColorsContainer.Floats.TRANSLATE_Y.get(scenePct) + previousValue - nextValue;
            MotionColorsContainer.Floats.TRANSLATE_Y
               .set(scenePct, Math.max(-160.0F, Math.min(160.0F, sourceValue)));
            MotionAnimateService.animate(scenePct, MotionColorsContainer.Floats.TRANSLATE_Y, 0.0F, MaterialEnterService.PAGE);
         }
      }

      this.entries.clear();

      for (ScenePctService currentScenePct : (Iterable<ScenePctService>) (Iterable<?>) (currentItems)) {
         this.entries.put(currentScenePct, currentScenePct.computedY() - this.computedY() - this.scrollY());
      }
