...
public final class AnimatedSelectionIndicator extends LayoutContainerNode {
   ...
   private static final MotionFiniteService motionFiniteService = MotionFiniteService.finite("materialIndicatorLeft", item -> item instanceof AnimatedSelectionIndicator);
   private static final MotionFiniteService motionFiniteService2 = MotionFiniteService.finite("materialIndicatorRight", item -> item instanceof AnimatedSelectionIndicator);
   private float value;
   ...
   private void updateState(float currentValue, float nextValue) {
      if (!this.enabled) {
         ...
         MotionAnimateService.animate(this, motionFiniteService, currentValue, previousValue != 0 ? MaterialEnterService.TRAILING_EDGE : MaterialEnterService.LEADING_EDGE);
         MotionAnimateService.animate(this, motionFiniteService2, nextValue, previousValue != 0 ? MaterialEnterService.LEADING_EDGE : MaterialEnterService.TRAILING_EDGE);
      }
   }
   ...
   protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      ...
      if (this.count >= 0 && this.count < this.children().size()) {
         ...
         if (this.enabled2 || width != this.value5) {
            ...
            MotionAnimateService.animate(this, MotionColorsContainer.Floats.SCROLL_Y, previousValue, MaterialEnterService.PAGE);
         }

         float sourceValue = 24.0F;
         if (!scenePct.children().isEmpty() && scenePct.children().getFirst() instanceof SceneTextService sceneText) {
            compositorPushPresentationScale.pushPresentationScale(1.0F);

            try {
               sourceValue = Math.max(
                  24.0F, Math.min(nextValue - 24.0F, sceneText.intrinsicWidth(compositorPushPresentationScale))
               );
            } finally {
         ...
         float inputValue = Math.max(
            12.0F, Math.min(sourceValue + 40.0F, this.value2 - this.value)
         );
