...
public final class ComponentMountService extends LayoutContainerNode {
   ...
   public ComponentMountService mount(ComponentOperationHandler componentOperation, ThemeIsSetService themeIsSet) {
      this.runnable.run();
      ...
      if (this.componentOperationHandler != null && this.componentThemeService != null) {
         if (this.enabled) {
            ...
            do {
               ...
               try {
                  ComponentKeyService<?> componentKey = Objects.requireNonNull(this.componentOperationHandler.render(this.componentThemeService), "Component returned null");
                  ComponentMountService.Mounted currentMounted = this.createMounted(this.mounted, componentKey);
   ...
   private ComponentMountService.Mounted createMounted(ComponentMountService.Mounted mounted, ComponentKeyService<?> componentKey) {
   ...
   private List<ComponentMountService.Mounted> collectValues(List<ComponentMountService.Mounted> items, List<ComponentKeyService<?>> currentItems) {
      ...
      for (ComponentKeyService<?> componentKey : currentItems) {
         ComponentMountService.Mounted currentMounted = componentKey.key() != null ? (ComponentMountService.Mounted)hashMap.get(componentKey.key()) : null;
         if (currentMounted == null && componentKey.key() == null) {
            while (index < arrayList.size()) {
               ComponentMountService.Mounted nextMounted = (ComponentMountService.Mounted)arrayList.get(index++);
   ...
   private static List<ScenePctService<?>> collectValues2(List<ComponentMountService.Mounted> items) {
   ...
   private record Mounted(ComponentKeyService<?> element, ScenePctService<?> node, List<ComponentMountService.Mounted> children) {
   }
}
