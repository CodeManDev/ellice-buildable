...
public final class DiscordStateController implements AutoCloseable {
  ...
  public DiscordStateController() {
    if (this.text == null) {
      CoreIsInitializedHandler.LOGGER.warn(
          "Discord Rich Presence needs discord_application_id or ELLICE_DISCORD_APPLICATION_ID");
    }
  ...
  public void tick(Minecraft minecraft, int value) {
    if (this.enabled3) {
      ...
      if (minecraft.level == null) {
        ...
        if (sourceText == null
            || sourceText.isBlank()
            || serverData.ip != null && sourceText.equalsIgnoreCase(serverData.ip)) {
        ...
        currentText = createText(value);
        nextText =
            serverData != null && serverData.ip != null && !serverData.ip.isBlank()
                ? serverIcon(serverData.ip)
                : "multiplayer";
        previousText = createText3(sourceText, 128);
      }

      DiscordStateController.Activity currentActivity =
          new DiscordStateController.Activity(
              text, currentText, nextText, previousText, this.timestamp3);
    ...
    while (this.enabled2) {
      try {
        ...
        CoreIsInitializedHandler.LOGGER.debug(
            "Discord Rich Presence is waiting for Discord desktop", iOException);
        this.updateState2(10000L);
      }
    }
  ...
  private static String createText2() {
    ...
    if (text == null || text.isBlank()) {
      try (InputStream inputStream =
          DiscordStateController.class.getResourceAsStream(
              "/assets/ellice/discord/application-id.txt")) {
        if (inputStream != null) {
  ...
  record Activity(
      String details, String state, String smallImage, String smallText, long startedAt) {
    JsonObject json() {
      ...
      nextJsonObject.addProperty(
          "large_text",
          "ellice "
              + CoreIsInitializedHandler.VERSION
              + " • "
              + CompatAdapterService.metadata().displayName());
