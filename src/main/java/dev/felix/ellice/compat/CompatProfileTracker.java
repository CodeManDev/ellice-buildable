...
public final class CompatProfileTracker {
  ...
  private static final CompatProfileTracker.Profile profile =
      new CompatProfileTracker.Profile(
          new ScaffoldChooseService.Policy(true, "Held First", 0, List.of(), false, Set.of(), 0),
          false,
          true,
          4);
  ...
  public static boolean depletedVisibleHand(Minecraft minecraft) {
    return scaffoldActiveService.active()
        && scaffoldActiveService.owned()
    ...
    for (int index = 0; index < 9; index++) {
      ...
      arrayList.add(
          new ScaffoldChooseService.Stack(
              ...
              itemStack.getCount(),
              checkCondition2(itemStack)
                  && (minecraft.level == null
                      || itemStack.isItemEnabled(minecraft.level.enabledFeatures()))));
    }

    int value =
        scaffoldActiveService.active()
            ? scaffoldActiveService.visibleSlot()
            : InventoryInputActions.selected(minecraft);
    return ScaffoldChooseService.choose(
        arrayList,
        value,
        scaffoldActiveService.active() ? scaffoldActiveService.blockSlot() : count,
        profile2.blocks());
  }
  ...
  public static boolean place(
    ...
    if (!paused() && value >= 0 && value == find(minecraft)) {
      scaffoldActiveService.acquire(
          new CompatProfileTracker.Hand(minecraft), value, profile2.silent(), profile2.restore());
      return scaffoldActiveService.use(
          () -> scaffoldCompatibility.place(minecraft, placementCandidate));
    } else {
      return false;
    }
  }
  ...
  public record Profile(
      ScaffoldChooseService.Policy blocks, boolean silent, boolean restore, int manualPause) {}
}
