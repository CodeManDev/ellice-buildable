...
public final class FriendSyncController {
  ...
  public static boolean excluded(FriendsMode friendsMode, Entity entity) {
    return entity instanceof Player
            ...
            && CoreIsInitializedHandler.get().friends() != null
        ? CoreIsInitializedHandler.get()
            .friends()
            .excludes(friendsMode, entity.getUUID(), entity.getName().getString())
        : false;
  }
          ...
          .filter(
              item ->
                  item.uuid() != null
                      ...
                      && !item.name().isBlank())
          .sorted(
              Comparator.comparing(FriendsData::name, String.CASE_INSENSITIVE_ORDER)
                  .thenComparing(FriendsData::uuid))
          .toList();
    } else {
      return List.of();
    }
