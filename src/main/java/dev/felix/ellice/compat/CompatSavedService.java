...
public final class CompatSavedService {
   ...
   public static List<CompatSavedService.Saved> saved() {
      ...
      for (int index = 0; index < serverList.size(); index++) {
         ...
         try {
            arrayList.add(new CompatSavedService.Saved(ParsedServerEndpoint.parse(serverData.ip).address(), serverData.name, serverData.getIconBytes()));
   ...
   public static CompatSavedService.Session session() {
      ...
      if (minecraft.level != null && minecraft.getConnection() != null && serverData != null && !serverData.isRealm()) {
         try {
            return new CompatSavedService.Session(
               minecraft.getUser().getProfileId(), new CompatSavedService.Saved(ParsedServerEndpoint.parse(serverData.ip).address(), serverData.name, serverData.getIconBytes())
            );
         } catch (IllegalArgumentException illegalArgumentException) {
            return null;
         }
   ...
   public static InetSocketAddress resolve(ParsedServerEndpoint parsedServerEndpoint) throws IOException {
      return ((ResolvedServerAddress)ServerNameResolver.DEFAULT
            .resolveAddress(new ServerAddress(parsedServerEndpoint.host(), parsedServerEndpoint.port()))
   ...
   public static void connect(String text, String currentText) {
      ...
      if (minecraft.level == null) {
         ...
         ConnectScreen.startConnecting(
            (Screen)(minecraft.screen != null && !(minecraft.screen instanceof CoreSetBehindScreenService) ? minecraft.screen : new TitleScreen()),
            minecraft,
