...
public final class CompatTrianglesService {
  ...
  public static float[] triangles(
    ...
    return entries.computeIfAbsent(
        value, item -> createFloat(floats, currentFloats, textureAtlasSprite, currentValue));
  }

  private static float[] createFloat(
      float[] floats, float[] currentFloats, TextureAtlasSprite textureAtlasSprite, int value) {
    ...
    for (int index = 0; index < 4; index++) {
      nextFloats[index * 2] =
          (currentFloats[index * 2] - textureAtlasSprite.getU0())
              / (textureAtlasSprite.getU1() - textureAtlasSprite.getU0());
      nextFloats[index * 2 + 1] =
          (currentFloats[index * 2 + 1] - textureAtlasSprite.getV0())
              / (textureAtlasSprite.getV1() - textureAtlasSprite.getV0());
    }
    ...
    for (int nextIndex = Math.max(0, (int) (resultValue * currentHeight));
        nextIndex < Math.min(currentHeight, (int) Math.ceil(candidateValue * currentHeight))
            && byteValue != 0;
        nextIndex++) {
      for (int previousIndex = Math.max(0, (int) (inputValue * currentWidth));
          previousIndex < Math.min(currentWidth, (int) Math.ceil(outputValue * currentWidth));
          previousIndex++) {
    ...
    for (int sourceIndex = Math.max(0, (int) (resultValue * currentHeight));
        sourceIndex < Math.min(currentHeight, (int) Math.ceil(candidateValue * currentHeight));
        sourceIndex++) {
      ...
      while (targetIndex < selectedValue) {
        while (targetIndex < selectedValue
            && spriteContents.isTransparent(value, targetIndex, sourceIndex)) {
          targetIndex++;
        }
        ...
        if (defaultValue != targetIndex) {
          ...
          for (int outputIndex = 0; outputIndex < items.size(); outputIndex++) {
            ...
            float computedValue =
                (initialValue * sourceValue - resolvedValue * previousValue) / targetValue;
            float cachedValue =
                (currentValue * resolvedValue - nextValue * initialValue) / targetValue;

            for (int resultIndex = 0; resultIndex < 3; resultIndex++) {
              currentSize[outputIndex * 3 + resultIndex] =
                  floats[resultIndex]
  ...
  private static List<float[]> collectValues(
      List<float[]> items, int index, float value, boolean enabled) {
    ArrayList arrayList = new ArrayList();
    if (items.isEmpty()) {
      return arrayList;
    }
    ...
    int currentValue =
        enabled ? (floats[index] >= value ? 1 : 0) : (floats[index] <= value ? 1 : 0);

    for (float[] currentFloats : items) {
      int nextValue =
          enabled
              ? (currentFloats[index] >= value ? 1 : 0)
              : (currentFloats[index] <= value ? 1 : 0);
      if (nextValue != currentValue) {
        float previousValue = (value - floats[index]) / (currentFloats[index] - floats[index]);
        arrayList.add(
            new float[] {
              floats[0] + previousValue * (currentFloats[0] - floats[0]),
              floats[1] + previousValue * (currentFloats[1] - floats[1])
            });
      }
