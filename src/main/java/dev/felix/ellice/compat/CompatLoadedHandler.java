...
public interface CompatLoadedHandler {
  ...
  default TravelLoadedHandler traversal() {
    return this instanceof TravelLoadedHandler travelLoaded
        ? travelLoaded
        : new TravelLoadedHandler() {
          @Override
  ...
  record SkyState(Vector3f sunDirection, float daylight) {
    public SkyState(Vector3f sunDirection, float daylight) {
      sunDirection =
          sunDirection == null ? new Vector3f(0.0F, 1.0F, 0.0F) : new Vector3f(sunDirection);
  ...
  record Textures(
      int atlas, int lightmap, int width, int height, int mipLevels, boolean modernSampling) {}

  record World(
      String identity, String dimension, Vector3d player, int minSection, int maxSection) {}
}
