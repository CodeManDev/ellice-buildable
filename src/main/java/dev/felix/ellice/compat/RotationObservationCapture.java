...
public final class RotationObservationCapture {
   ...
   public static Optional<RotationObservationCapture.Observation> capture(Minecraft minecraft, long longValue, RotationTarget rotationTarget, RotationData rotationData, double doubleValue) {
      if (rotationTarget != null
         ...
         if (localPlayer.isAlive()
            ...
            if (minecraft.level.getEntity(rotationTarget.entityId()) instanceof LivingEntity livingEntity
               ...
               double currentDoubleValue = scaffoldCompatibility.groundInputAcceleration(minecraft, false);
               ...
               if (Double.isFinite(currentDoubleValue)
                  ...
                  if (Double.isFinite(sourceDoubleValue) && !(sourceDoubleValue <= 0.0)) {
                     ...
                     CombatDecisionTracker.Fighter currentX = new CombatDecisionTracker.Fighter(
                        ...
                        (localPlayer.getHealth() + localPlayer.getAbsorptionAmount()) / Math.max(1.0F, localPlayer.getMaxHealth()),
                        localPlayer.getAttackStrengthScale(0.0F),
                        localPlayer.getCurrentItemAttackStrengthDelay(),
                        Math.max(0, localPlayer.hurtTime)
                     );
                     boolean enabled = CompatBlocksAttackFromService.blocksAttackFrom(livingEntity, localPlayer.position());
                     CombatDecisionTracker.Opponent opponent = new CombatDecisionTracker.Opponent(
                        livingEntity.getUUID(),
                        new RotationBoundingBox(aABB.minX, aABB.minY, aABB.minZ, aABB.maxX, aABB.maxY, aABB.maxZ),
                        enabled ? livingEntity.getYHeadRot() : livingEntity.getYRot(),
                        (livingEntity.getHealth() + livingEntity.getAbsorptionAmount()) / Math.max(1.0F, livingEntity.getMaxHealth()),
                        Math.max(0, livingEntity.hurtTime),
                        enabled
                     );
                     CompatEnableHandler.Status currentStatus = CompatEnableHandler.status();
                     int value = CompatEnableService.replaying() || livingEntity.getUUID().equals(currentStatus.target()) && currentStatus.packets() > 0 ? 0 : 1;
                     CombatDecisionTracker.Frame frame = new CombatDecisionTracker.Frame(
                        longValue,
                        currentX,
                        opponent,
                        localPlayer.getYRot(),
                        ...
                        sourceDoubleValue,
                        new CombatDecisionTracker.Physics(currentDoubleValue, nextDoubleValue, previousDoubleValue),
                        (value != 0)
                     );
                     return Optional.of(new RotationObservationCapture.Observation(frame, new RotationObservationCapture.Terrain(minecraft)));
                  } else {
                     return Optional.empty();
                  }
               } else {
   ...
   public record Observation(CombatDecisionTracker.Frame frame, CombatAllowsService.Terrain terrain) {
   }
   ...
   private static final class Terrain implements CombatAllowsService.Terrain {
      ...
      private Terrain(Minecraft minecraft) {
         ...
         this.aABB = minecraft.player
            .getBoundingBox()
            .deflate(1.0E-5, 0.0, 1.0E-5);
         this.value = Math.min(0.6, minecraft.player.maxUpStep());
      }

      @Override
      public boolean safeAt(double doubleValue, double currentDoubleValue) {
         return this.entries.computeIfAbsent(new RotationObservationCapture.Terrain.Point(doubleValue, currentDoubleValue), item -> this.checkCondition(doubleValue, currentDoubleValue));
      }
         ...
         if (Math.hypot(doubleValue, currentDoubleValue) > 3.0) {
            ...
            if (this.checkCondition2(currentAABB)) {
               ...
               return this.value >= 0.5
                     && this.checkCondition2(currentAABB.move(0.0, 0.5, 0.0))
                  ? true
                  : this.value > 0.5
                     && this.checkCondition2(currentAABB.move(0.0, this.value, 0.0));
            }
         }
      }
      ...
      private boolean checkCondition2(AABB aABB) {
         ...
         if (!this.minecraft2
            .level
            .noCollision(this.minecraft2.player, aABB.deflate(0.0, 1.0E-5, 0.0))) {
            return false;
         }
         ...
         for (VoxelShape voxelShape : this.minecraft2.level.getBlockCollisions(this.minecraft2.player, currentAABB)) {
