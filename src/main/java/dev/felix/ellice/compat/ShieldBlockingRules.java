...
final class ShieldBlockingRules {
  ...
  static boolean blocksAttackFrom(LivingEntity livingEntity, DamageSource damageSource) {
    ...
    BlocksAttacks blocksAttacks =
        itemStack == null ? null : (BlocksAttacks) itemStack.get(DataComponents.BLOCKS_ATTACKS);
    if (blocksAttacks != null
        && !blocksAttacks
            .bypassedBy()
            .map(item -> item.contains(damageSource.typeHolder()))
            .orElse(false)) {
      ...
      double doubleValue =
          Math.acos(new Vec3(currentVec3.x, 0.0, currentVec3.z).normalize().dot(vec3));
  ...
  static boolean canDisableWithHeldWeapon(LocalPlayer localPlayer, LivingEntity livingEntity) {
    ...
    return blocksAttacks != null
        && Math.round(
                localPlayer.getSecondsToDisableBlocking()
