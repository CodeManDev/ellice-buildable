...
public final class CompatOptionsTracker implements RodStateService.Environment {
   ...
   private final RotationSynchronizeService rotationSynchronizeService = new RotationSynchronizeService();
   private final CombatCommitService combatCommitService = new CombatCommitService();
   private CompatOptionsTracker.Options options2 = new CompatOptionsTracker.Options(
      true,
      true,
      false,
      false,
   ...
   public void update(long offset, CompatOptionsTracker.Options options) {
      ...
      if (this.context()) {
         ...
         if (this.livingEntity != null && this.checkCondition2(this.livingEntity)) {
            ...
            this.motion = this.bowObserveService2
               .observe(offset, this.uUID, CompatContextReadyService.observedPosition(this.livingEntity), this.livingEntity.onGround());
         }
      }
   }
   ...
   public static boolean handBusy() {
      return CompatBeginTickService.blocking()
         || compatOptionsTracker != null && compatOptionsTracker.checkCondition() && !compatOptionsTracker.enabled2 && !compatOptionsTracker.enabled4;
   }
      ...
      return localPlayer != null
         ...
         && (
            !localPlayer.isOnFire()
               ...
               || !CoreIsInitializedHandler.get().modules().get("AutoExtinguish").map(item -> item.isEnabled()).orElse(false)
         );
   }

   private ImplDecisionTracker createImplDecisionTracker() {
      return CoreIsInitializedHandler.isReady()
         ? CoreIsInitializedHandler.get().modules().get(ImplDecisionTracker.class).filter(Module::isEnabled).orElse(null)
         : null;
   }
   ...
   public boolean meleeReady() {
      ...
      if (minecraft.player != null && this.livingEntity != null) {
         double doubleValue = implDecisionTracker == null
            ? minecraft.player.entityInteractionRange()
            : Math.min(minecraft.player.entityInteractionRange(), implDecisionTracker.supportAttackRange());
         return this.livingEntity.getBoundingBox().distanceToSqr(minecraft.player.getEyePosition()) <= doubleValue * doubleValue;
      } else {
         return false;
      }
   }
   ...
   private LivingEntity createLivingEntity(Minecraft minecraft) {
      ImplDecisionTracker implDecisionTracker = this.createImplDecisionTracker();
      UUID currentUUID = implDecisionTracker == null ? null : implDecisionTracker.targetUuid().orElse(null);
      if (currentUUID != null) {
      ...
      RotationData rotationData = new RotationData(minecraft.player.getYRot(), minecraft.player.getXRot());
      return minecraft.level
         .players()
         .stream()
         .filter(this::checkCondition2)
         .filter(
            item -> item.distanceToSqr(minecraft.player)
         ...
         .filter(
            item -> Math.abs(
                  RotationData.yawDelta(
                     rotationData.yaw(),
                     RotationData.lookAt(createRotationData7(minecraft.player.getEyePosition()), createRotationData7(item.getBoundingBox().getCenter())).yaw()
                  )
               )
               <= 90.0
         )
         .filter(
            item -> checkCondition4(minecraft, createRotationData7(minecraft.player.getEyePosition()), createRotationData7(item.getBoundingBox().getCenter()), item)
         )
         .min(Comparator.comparingDouble(item -> item.distanceToSqr(minecraft.player) + (item.getUUID().equals(this.uUID) ? -3 : 0)))
         .orElse(null);
   }
         ...
         for (RodTrajectorySolver.Kind currentKind : RodTrajectorySolver.Kind.values()) {
            if ((
                  currentKind == RodTrajectorySolver.Kind.ROD
                     ? this.options2.rod() && currentEnabled && !(this.livingEntity.distanceTo(minecraft.player) > this.options2.rodRange())
                     : nextEnabled
                        && !(this.livingEntity.distanceTo(minecraft.player) > this.options2.range())
                        && (currentKind == RodTrajectorySolver.Kind.SNOWBALL ? this.options2.snow() : this.options2.eggs())
               )
               && (currentKind != RodTrajectorySolver.Kind.ROD || !this.options2.saveRod() || this.calculateValue(minecraft) >= 0)) {
               ...
               if (value >= 0) {
                  ...
                  if (currentShot != null) {
                     ...
                     this.rotationData = CombatOwnsService.claim(CombatOwnsService.Owner.ROD)
                        .orElseGet(
                           () -> RotationObserveService.current().orElse(new RotationData(this.localPlayer.getYRot(), this.localPlayer.getXRot()))
                        );
                     this.bowNextService.reset();
                     return this.kind;
                  }
   ...
   private int calculateValue(Minecraft minecraft) {
      ...
      int value = compatOptionsTracker == this ? this.count2 : InventoryInputActions.selected(minecraft);
   ...
   private int calculateValue2(Minecraft minecraft, RodTrajectorySolver.Kind kind) {
      for (int value : new int[]{40, 0, 1, 2, 3, 4, 5, 6, 7, 8}) {
         ItemStack itemStack = value == 40 ? minecraft.player.getOffhandItem() : minecraft.player.getInventory().getItem(value);
         if (checkCondition3(itemStack, kind)
            ...
            && (!itemStack.isDamageableItem() || itemStack.getMaxDamage() - itemStack.getDamageValue() > 5)) {
            return value;
         }
      }
   ...
   private ItemStack createItemStack() {
      return this.count == 40 ? this.localPlayer.getOffhandItem() : this.localPlayer.getInventory().getItem(this.count);
   }
   ...
   public boolean targetLost() {
      return this.livingEntity == null
         ...
         || this.livingEntity.distanceTo(this.localPlayer) > Math.max(this.options2.range(), this.options2.rodRange()) + 1.0;
   }
   ...
   public RodStateService.Hook hook() {
      if (this.checkCondition() && this.localPlayer.fishing != null) {
         ...
         return !this.fishingHook.onGround() && !this.fishingHook.horizontalCollision && !this.fishingHook.isInWater()
   ...
   public void aim(boolean currentEnabled) {
      if (this.valid() && !this.enabled4) {
         ...
         RotationData currentRotationData = this.shot != null ? this.shot.solution().rotation() : this.rotationData;
         if (this.enabled && this.livingEntity != null && !this.targetLost()) {
            currentRotationData = RotationData.lookAt(
               createRotationData7(this.localPlayer.getEyePosition()), createRotationData7(this.livingEntity.getBoundingBox().getCenter())
            );
         }

         RotationData nextRotationData = RotationObserveService.current().orElse(this.rotationData);
         this.rotationData = this.bowNextService
            .next(
               nextRotationData,
               currentRotationData,
         ...
         if (this.createImplDecisionTracker() != null) {
            this.rotationSynchronizeService.synchronize(RotationObserveService.previousTickRotation().orElse(nextRotationData), nextRotationData);
            this.rotationData = this.rotationSynchronizeService
               .next(nextRotationData, this.rotationData, (Double)currentMinecraft.options.sensitivity().get(), 0.0, this.options2.turn())
               .rotation();
         }
         ...
         if (this.combatCommitService.commit(this.timestamp, this.localPlayer.getYRot(), this.rotationData, input)) {
   ...
   public void reconcile(long longValue) {
      if (this.valid() && !this.enabled4) {
         if (LocalPhysicsFrameBus.wasAborted(longValue)) {
            ...
            PlacementOverrideBus.applied().ifPresent(item -> this.combatCommitService.reconcile(longValue, item));
         }
      }
   }
      ...
      if (this.valid() && !this.enabled4) {
         if (this.timestamp3 != 0L && System.nanoTime() - this.timestamp3 < 200000000L) {
            ...
            if (currentSnapshot != null
               && !(
                  RotationData.distance(currentSnapshot.rotation(), new RotationData((float)this.rotationData.yaw(), (float)this.rotationData.pitch()))
                     > 1.0E-5
               )
               && !(
               ...
               return enabled
                  || rotationVector != null
                     && this.shot != null
                     && !this.targetLost()
                     && this.calculateValue3(minecraft, this.shot, this.kind, currentSnapshot.rotation(), currentSnapshot.wireEyePosition(), rotationVector)
   ...
   public boolean use(boolean currentEnabled) {
      if (CompatBeginTickService.canAct() && this.aligned(currentEnabled) && !this.localPlayer.getCooldowns().isOnCooldown(this.createItemStack())) {
         if (currentEnabled
            ...
            try {
               ...
               InteractionResult interactionResult = ScaffoldForActorService.withRotation(
                  this.localPlayer, this.rotationData, () -> minecraft.gameMode.useItem(this.localPlayer, this.createInteractionHand())
               );
               if (this.enabled3) {
   ...
   public void accepted(Object value) {
      if (this.enabled2 && value instanceof ServerboundUseItemPacket serverboundUseItemPacket && serverboundUseItemPacket.getHand() == this.createInteractionHand()) {
         this.enabled3 = true;
      }
   }
   ...
   private CompatOptionsTracker.Shot createShot(Minecraft minecraft, RodTrajectorySolver.Kind kind) {
      if (this.livingEntity != null && this.motion != null) {
         ...
         RotationVector currentRotationVector = rotationVector.subtract(createRotationData7(this.livingEntity.position()));
         AABB currentX = this.livingEntity.getBoundingBox().move(currentRotationVector.x(), currentRotationVector.y(), currentRotationVector.z());
         ArrayList<CompatOptionsTracker.Body> arrayList = new ArrayList<>();

         for (BowObserveService.Weighted weighted : this.motion
            .ensemble()
            .at(
            ...
            List<RotationVector> currentSize = CompatForecastService.forecast(
               currentX,
               ...
               item -> {
                   ...
                   minecraft.level.getBlockCollisions(this.livingEntity, item).forEach(collisions::add);
                   return collisions;
               },
               item -> minecraft.level.hasChunkAt(BlockPos.containing(item.minX, item.minY, item.minZ))
            );
            arrayList.add(new CompatOptionsTracker.Body(createRotationData2(currentX), currentSize, weighted.weight()));
         }
         ...
         if (!minecraft.hasSingleplayerServer() && playerInfo != null) {
            doubleValue += Math.clamp(
               playerInfo.getLatency() / 50.0, 0.0, 3.0
            );
         }
         ...
         for (CompatOptionsTracker.Body body : arrayList.stream()
            .sorted(Comparator.comparingDouble(CompatOptionsTracker.Body::weight).reversed())
            .limit(3L)
            .toList()) {
            RodTrajectorySolver.Solution solution = RodTrajectorySolver.solve(
                  kind,
                  nextRotationVector,
               ...
               .orElse(null);
            if (solution != null) {
               CompatOptionsTracker.Shot currentShot = new CompatOptionsTracker.Shot(solution, arrayList, doubleValue);
               double previousDoubleValue = this.calculateValue3(minecraft, currentShot, kind, solution.rotation(), nextRotationVector, previousRotationVector);
   ...
   private RodTrajectorySolver.Launch createLaunch(Minecraft currentMinecraft) {
      ...
      ScaffoldRemapService.Input input = new ScaffoldRemapService.Input(
         ...
         currentOptions.keyJump.isDown(),
         currentOptions.keyShift.isDown(),
         currentOptions.keySprint.isDown() || ImplRequestsSprintClient.requestsSprint(currentMinecraft) || localPlayer.isSprinting()
      );
      double doubleValue = RotationObserveService.current().map(RotationData::yaw).orElse((double)localPlayer.getYRot());
      ScaffoldRemapService.Result result = new ScaffoldRemapService().remap(localPlayer.getYRot(), doubleValue, input);
      ...
      double currentDoubleValue = localPlayer.onGround()
         ? scaffoldCompatibility.groundInputAccelerationEstimate(currentMinecraft, currentInput.forwardAxis(), currentInput.leftAxis(), currentInput.sprint())
         : scaffoldCompatibility.airInputAccelerationEstimate(currentMinecraft, currentInput.forwardAxis(), currentInput.leftAxis())
      ...
      Vec3 currentX = new Vec3(vec3.x + currentWorldVector.x() * currentDoubleValue, nextDoubleValue, vec3.z + currentWorldVector.z() * currentDoubleValue);
      if (localPlayer.onGround() && currentInput.jump() && currentInput.sprint()) {
         ScaffoldRemapService.WorldVector nextWorldVector = ScaffoldRemapService.worldVector(doubleValue, 1, 0);
         currentX = currentX.add(
            nextWorldVector.x() * 0.2, 0.0, nextWorldVector.z() * 0.2
         );
      }

      Vec3 currentVec3 = Entity.collideBoundingBox(localPlayer, currentX, localPlayer.getBoundingBox(), currentMinecraft.level, List.of());
      int value = currentX.y < 0.0 && Math.abs(currentVec3.y - currentX.y) > 1.0E-7 ? 1 : 0;
      return new RodTrajectorySolver.Launch(
         createRotationData7(localPlayer.getEyePosition().add(currentVec3)), new RotationVector(currentVec3.x, value != 0 ? 0.0 : currentVec3.y, currentVec3.z)
      );
   }

   private double calculateValue3(
      Minecraft minecraft, CompatOptionsTracker.Shot shot, RodTrajectorySolver.Kind kind, RotationData rotationData, RotationVector rotationVector, RotationVector currentRotationVector
   ) {
      double doubleValue = 0.0;
      RodTrajectorySolver.Launch currentLaunch = RodTrajectorySolver.launch(kind, rotationVector, currentRotationVector, rotationData);
      ...
      for (CompatOptionsTracker.Body body : shot.bodies()) {
         if (!(body.weight() < 0.01)) {
            ...
            for (int index = 1; index <= Math.min(24, (int)Math.ceil(shot.solution().ticks()) + 3); index++) {
               RotationVector previousRotationVector = RodTrajectorySolver.position(kind, currentLaunch, index);
               OptionalDouble optionalDouble = BowTrajectorySolver.hitFraction(
                  nextRotationVector, previousRotationVector, currentRotationBoundingBox, body.offset(index - 1 + shot.lead()), body.offset(index + shot.lead())
               );
               RotationVector sourceRotationVector = optionalDouble.isPresent() ? nextRotationVector.add(previousRotationVector.subtract(nextRotationVector).multiply(optionalDouble.getAsDouble())) : previousRotationVector;
               if (!checkCondition4(minecraft, nextRotationVector, sourceRotationVector, this.livingEntity)) {
                  break;
               }
   ...
   private static boolean checkCondition4(Minecraft minecraft, RotationVector rotationVector, RotationVector currentRotationVector, Entity entity) {
      if (minecraft.level.hasChunkAt(BlockPos.containing(rotationVector.x(), rotationVector.y(), rotationVector.z()))
         && minecraft.level.hasChunkAt(BlockPos.containing(currentRotationVector.x(), currentRotationVector.y(), currentRotationVector.z()))) {
         Vec3 vec3 = createVec3(rotationVector);
         Vec3 currentVec3 = createVec3(currentRotationVector);
         if (minecraft.level.clip(new ClipContext(vec3, currentVec3, Block.COLLIDER, Fluid.ANY, minecraft.player)).getType() != Type.MISS) {
            return false;
         }
   ...
   private record Body(RotationBoundingBox box, List<RotationVector> offsets, double weight) {
      RotationVector offset(double doubleValue) {
         ...
         return this.offsets
            .get(value)
            .add(this.offsets.get(Math.min(value + 1, this.offsets.size() - 1)).subtract(this.offsets.get(value)).multiply(currentSize - value));
      }
   }
   ...
   private record Shot(RodTrajectorySolver.Solution solution, List<CompatOptionsTracker.Body> bodies, double lead) {
   }
}
