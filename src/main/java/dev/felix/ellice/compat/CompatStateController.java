...
public final class CompatStateController {
  ...
  public String tick(
      TaskAction.Craft craft, TaskResolveService.Station currentStation, TaskData taskData) {
    ...
    if (minecraft.player != null
      ...
      if (this.screen2 == null
          && this.enabled
          && minecraft.screen instanceof CraftingScreen craftingScreen) {
  ...
  private boolean checkCondition(Screen screen) {
    return this.station == TaskResolveService.Station.INVENTORY
  ...
  private boolean checkCondition2(Minecraft minecraft) {
    return this.screen2 != null
        && minecraft.screen == this.screen2
        && this.checkCondition(this.screen2);
  }
  ...
  private String createText2(Minecraft minecraft) {
    ...
    if (this.station == TaskResolveService.Station.INVENTORY) {
      ...
      try {
        if (minecraft
            .gameMode
            .useItemOn(minecraft.player, InteractionHand.MAIN_HAND, (BlockHitResult) result.get())
            .consumesAction()) {
  ...
  private String createText5(Minecraft minecraft, TaskData taskData, long currentSize) {
    ...
    if (this.abstractContainerMenu != null
      ...
      if (slot.isActive() && (!slot.hasItem() || slot.mayPickup(minecraft.player))) {
        ...
        InventoryInputActions.clickMenu(
            minecraft,
            this.abstractContainerMenu.containerId,
            0,
            0,
            SoupClickTracker.Click.QUICK_MOVE);
          ...
          if (items.size() == 1 && createText7((ItemStack) items.getFirst()).equals(this.text)) {
            ...
            try {
              minecraft.gameMode.handlePlaceRecipe(
                  this.abstractContainerMenu.containerId, recipeDisplayEntry.id(), false);
            } finally {
              this.enabled2 = false;
            }
  ...
  private static BlockPos createBlockPos(Minecraft minecraft) {
    ...
    for (int index = blockPos.getX() - 5; index <= blockPos.getX() + 5; index++) {
      for (int currentIndex = blockPos.getY() - 3;
          currentIndex <= blockPos.getY() + 3;
          currentIndex++) {
        for (int nextIndex = blockPos.getZ() - 5; nextIndex <= blockPos.getZ() + 5; nextIndex++) {
          ...
          if (minecraft.level.hasChunkAt(nextBlockPos)
              && minecraft.level.getBlockState(nextBlockPos).is(Blocks.CRAFTING_TABLE)) {
            double currentDoubleValue =
                minecraft.player.distanceToSqr(index + 0.5, currentIndex + 0.5, nextIndex + 0.5);
