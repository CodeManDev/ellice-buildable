    ...
    public static Snapshot read() {
        ...
        for (int length = keyMappings.length, i = 0; i < length; ++i) {
            ...
            if (key.getType() == InputConstants.Type.KEYSYM) {
                if (key.getValue() >= 0) {
                    ((List<String>)map.computeIfAbsent(key.getValue(), p0 -> new ArrayList())).add(Component.translatable(keyMapping.getName()).getString());
                }
            }
        }
        ...
        return new Snapshot(Map.copyOf((Map<? extends Integer, ? extends List<String>>)map), (Map<Integer, String>)Map.copyOf((Map<?, ?>)map2));
    }
