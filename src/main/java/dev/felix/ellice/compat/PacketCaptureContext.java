...
public final class PacketCaptureContext {
   ...
   public static PacketCaptureContext.Context capture() {
      ...
      return new PacketCaptureContext.Context(
         ...
         RotationPublishService.current().orElseGet(() -> new RotationData(localPlayer.getYRot(), localPlayer.getXRot()))
      );
   }
   ...
   public static void accepted(Packet<?> packet, PacketCaptureContext.Context context) {
      ...
      if (packet instanceof ServerboundMovePlayerPacket serverboundMovePlayerPacket) {
         if (context == null) {
            ...
            RotationObserveService.observe(serverboundMovePlayerPacket, context.feet(), context.eye(), context.rotation());
         }
      ...
      CoreIsInitializedHandler.get().bus().post(EventAttackInputService.OUTGOING_PACKET_ACCEPTED, new EventAttackInputService.OutgoingPacketAccepted(packet));
   }
