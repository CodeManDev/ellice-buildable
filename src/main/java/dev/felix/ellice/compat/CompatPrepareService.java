...
public final class CompatPrepareService {
   ...
   public static RotationData prepare(
      Minecraft minecraft, ScaffoldCompatibility scaffoldCompatibility, ScaffoldPlayerSnapshot scaffoldPlayerSnapshot, ScaffoldRemapService.Input input, RotationData rotationData, double doubleValue, double currentDoubleValue
   ) {
      clear();
      if (scaffoldPlayerSnapshot.onGround()
         ...
         AABB currentX = minecraft.player
            .getBoundingBox()
            .expandTowards(
               currentWorldVector.x() * 0.9, 0.0, currentWorldVector.z() * 0.9
            );
         BlockPos blockPos = null;
         RotationVector rotationVector = null;
         double nextDoubleValue = Double.POSITIVE_INFINITY;

         for (BlockPos currentBlockPos : BlockPos.betweenClosed(
            ...
            if (blockState.getBlock() instanceof SnowLayerBlock
               ...
               if (!voxelShape.isEmpty()) {
                  Vec3 nextX = voxelShape.bounds().getCenter().add(currentBlockPos.getX(), currentBlockPos.getY(), currentBlockPos.getZ());
         ...
         if (blockPos != null && !(nextDoubleValue > doubleValue * doubleValue)) {
            RotationData currentRotationData = RotationObserveService.current().orElse(scaffoldPlayerSnapshot.playerRotation());
            ...
            ScaffoldBlockHit scaffoldBlockHit = scaffoldCompatibility.raycastBlockFromEye(minecraft, currentRotationData, doubleValue, scaffoldPlayerSnapshot.eyePosition()).orElse(null);
            ScaffoldBlockHit currentScaffoldBlockHit = currentSnapshot == null ? null : scaffoldCompatibility.raycastBlockFromEye(minecraft, currentRotationData, doubleValue, currentSnapshot.wireEyePosition()).orElse(null);
            BlockCoordinates previousX = new BlockCoordinates(blockPos.getX(), blockPos.getY(), blockPos.getZ());
            if (scaffoldBlockHit != null
               ...
               blockHitResult = new BlockHitResult(
                  new Vec3(scaffoldBlockHit.hitPoint().x(), scaffoldBlockHit.hitPoint().y(), scaffoldBlockHit.hitPoint().z()),
               ...
               return ScaffoldPlacementHeadingService.turnToward(nextRotationData, RotationData.lookAt(scaffoldPlayerSnapshot.eyePosition(), rotationVector), scaffoldPlayerSnapshot.mouseSensitivity(), currentDoubleValue);
            }
         } else {
            return null;
         }
