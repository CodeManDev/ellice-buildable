...
public final class CombatRotationOverride {
   ...
   public static void start(Minecraft minecraft, CombatOwnsService.Owner owner, double doubleValue, double currentDoubleValue) {
      if (CoreIsInitializedHandler.isReady()
         ...
         CombatRotationOverride.Session currentSession = new CombatRotationOverride.Session(minecraft, owner, doubleValue, currentDoubleValue);
         session = currentSession;
         EventSubscribeService eventSubscribe = CoreIsInitializedHandler.get().bus();
         currentSession.items2.add(eventSubscribe.subscribe(EventAttackInputService.PRE_PLAYER_MOVEMENT, EventIsAfterHandler.Priority.LATE, item -> {
            if (session == currentSession) {
               currentSession.updateState(minecraft);
            }
         }));
         currentSession.items2.add(eventSubscribe.subscribe(EventAttackInputService.MOVEMENT_PACKET_PREPARE, EventIsAfterHandler.Priority.LATE, item -> {
            if (session == currentSession) {
               currentSession.updateState2();
            }
         }));
         currentSession.items2.add(eventSubscribe.subscribe(EventAttackInputService.WORLD, item -> cancel()));
         currentSession.items2.add(eventSubscribe.subscribe(EventAttackInputService.PLAYER_POSITION_CORRECTED, item -> cancel()));
      }
   }
   ...
   public static boolean cancelForManualInteraction(KeyMapping keyMapping) {
      if (session != null && CombatOwnsService.owns(CombatOwnsService.Owner.RETURN)) {
         ...
         if (minecraft.player == session.localPlayer
            ...
            && (keyMapping == minecraft.options.keyAttack || keyMapping == minecraft.options.keyUse)) {
            boolean down = manualInteractionPressed(keyMapping.isDown(), ((ScaffoldKeyMappingAccess)keyMapping).ellice$pendingClicks());
            if (!down) {
               return false;
            }
   ...
   private static final class Session {
      ...
      private Session(Minecraft minecraft, CombatOwnsService.Owner currentOwner, double doubleValue, double currentDoubleValue) {
      ...
      private void updateState(Minecraft minecraft) {
         if (CombatOwnsService.owns(CombatOwnsService.Owner.RETURN)
            ...
            if (currentRotationData == null) {
            ...
            } else if (this.owner == CombatOwnsService.Owner.KILL_AURA && this.rotationNextService3.completed(currentRotationData)) {
               CombatRotationOverride.cancel();
            } else {
               RotationData nextRotationData = new RotationData(this.localPlayer.getYRot(), this.localPlayer.getXRot());
               double doubleValue = (Double)minecraft.options.sensitivity().get();
               double currentDoubleValue = new RotationVanillaGcdService().vanillaGcd(doubleValue) * 1.1
                  + 1.0E-4;
               RotationPublishService.Snapshot currentSnapshot = RotationPublishService.snapshot().orElse(null);
               if (this.owner != CombatOwnsService.Owner.KILL_AURA
                  ...
                  && Math.abs(RotationData.yawDelta(currentSnapshot.bodyYaw(), this.localPlayer.yBodyRot))
                     < 0.8) {
                  CombatRotationOverride.cancel();
               } else {
                  RotationNextService.Step step = this.owner == CombatOwnsService.Owner.KILL_AURA
                     ? this.rotationNextService3.next(currentRotationData, nextRotationData, doubleValue, this.value, this.value2)
                     : this.rotationNextService.next(currentRotationData, nextRotationData, doubleValue, this.value, this.value2);
                  this.rotationData = step.settled() ? step.rotation() : null;
                  RotationPublishService.publishReturning(nextRotationData, step.rotation(), this.localPlayer.yBodyRot);
                  ...
                  if (this.combatCommitService.commit(++this.timestamp, nextRotationData.yaw(), step.rotation(), input)) {
