...
public final class CompatActivateService implements SurvivalEnvironmentTracker.Environment {
   ...
   private boolean checkCondition() {
      ...
      return this.inventoryMenu2 != null && minecraft.player != null && minecraft.player.inventoryMenu == this.inventoryMenu2;
   }
   ...
   public SurvivalItemData capture() {
      ...
      if (minecraft.player != null
         ...
         int currentCount = (this.count2 < 36 || this.count2 > 44 || value == this.count2 - 36)
            ...
            ? 0
            : 1;
         int currentValue = currentCount == 0 && (minecraft.screen != null || !minecraft.options.keyUse.isDown()) ? 0 : 1;
   ...
   public boolean use(SurvivalItemData survivalItemData, int value) {
      if (this.checkCondition2(survivalItemData) && !survivalItemData.usingItem() && value >= 36 && value <= 45) {
         if (SurvivalAllService.isKnownFood(survivalItemData.readySlot(value).id()) && survivalItemData.food() < 20) {
            Minecraft minecraft = Minecraft.getInstance();
   ...
   public boolean swap(SurvivalItemData survivalItemData, int value, int currentValue) {
      ...
      if (!itemStack.isEmpty() && SurvivalAllService.isKnownFood(createText(itemStack))) {
         ...
         if (slot.mayPickup(minecraft.player) && currentSlot.mayPlace(itemStack) && slot.mayPlace(currentItemStack)) {
            this.enabled = true;

            try {
               InventoryInputActions.click(minecraft, value, currentValue, SoupClickTracker.Click.SWAP);
               return ItemStack.matches(minecraft.player.inventoryMenu.getSlot(value).getItem(), currentItemStack)
                  && ItemStack.isSameItemSameComponents(minecraft.player.inventoryMenu.getSlot(36 + currentValue).getItem(), itemStack);
            } finally {
               this.enabled = false;
            }
         } else {
   ...
   public static boolean holdUse() {
      if (compatActivateService != null && compatActivateService.count2 >= 0) {
         ...
         return minecraft.player != null
            ...
            && minecraft.screen == null
            && minecraft.player.isUsingItem()
            && ItemStack.isSameItemSameComponents(minecraft.player.getUseItem(), compatActivateService.itemStack)
            && minecraft.player.getUsedItemHand() == (compatActivateService.count2 == 45 ? InteractionHand.OFF_HAND : InteractionHand.MAIN_HAND)
            && (compatActivateService.count2 == 45 || InventoryInputActions.selected(minecraft) == compatActivateService.count2 - 36);
      } else {
         return false;
      }
   }
   ...
   public static boolean handBusy() {
      if (compatActivateService != null && compatActivateService.count2 >= 0 && !compatActivateService.enabled) {
         Minecraft minecraft = Minecraft.getInstance();
         return minecraft.player != null
            && minecraft.player.inventoryMenu == compatActivateService.inventoryMenu2
            && (compatActivateService.count2 == 45 || InventoryInputActions.selected(minecraft) == compatActivateService.count2 - 36);
   ...
   static SurvivalItemData.Item read(ItemStack itemStack) {
      return itemStack.isEmpty() ? SurvivalItemData.Item.EMPTY : new SurvivalItemData.Item(createText(itemStack), itemStack.getCount());
   }
