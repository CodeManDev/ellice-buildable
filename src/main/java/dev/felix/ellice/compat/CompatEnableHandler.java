...
public final class CompatEnableHandler {
  private static final BacktrackDelayQueue<CompatEnableHandler.Pending> backtrackDelayQueue =
      new BacktrackDelayQueue<>(256);
  ...
  public static void attackPacket(Packet<?> packet) {
    ...
    if (implProfileService4 != null && minecraft.isSameThread() && minecraft.level != null) {
      ...
      if (minecraft.level.getEntity(value) instanceof LivingEntity livingEntity
          && checkCondition2(minecraft, livingEntity, implProfileService4.profile())) {
  ...
  public static boolean intercept(Packet<?> packet, PacketListener packetListener) {
    ...
    if (minecraft != null
      ...
      if (!(currentDoubleValue > currentProfile.maximumRange() + 1.5)
          && !(previousDoubleValue > 1.0)) {
        if (enabled) {
        ...
        if (backtrackDelayQueue.isEmpty()) {
          ...
          count2 =
              currentProfile.minimumDelayMillis()
                  + random.nextInt(
                      currentProfile.maximumDelayMillis()
                          - currentProfile.minimumDelayMillis()
                          + 1);
          if (count2 == 0) {
            return false;
        ...
        if (!backtrackDelayQueue.offer(
            new CompatEnableHandler.Pending(packet, packetListener, minecraft.level),
            longValue,
            count2,
            value)) {
          updateState();
    ...
    if (minecraft.player != null
      ...
      UUID currentUUID =
          switch (currentProfile.mode()) {
            case "Attack" ->
                longValue - timestamp <= currentProfile.attackWindowMillis() * 1000000L
                    ? uUID
                    : null;
            case "KillAura" ->
                (UUID)
      ...
      int value =
          count4 == minecraft.player.tickCount
                  ...
                  && (livingEntity == null
                      || minecraft.level.getEntity(livingEntity.getId()) == livingEntity)
              ? 0
              : 1;
      Vec3 vec3 = createVec3(minecraft);
      if (value != 0 && currentProfile.mode().equals("Nearest")) {
        currentLivingEntity =
            StreamSupport.<Entity>stream(
                ...
                .filter(
                    item ->
                        calculateValue3(vec3, item.getBoundingBox())
                            <= currentProfile.maximumRange())
                .min(
                    Comparator.comparingDouble(
                        item -> calculateValue3(vec3, item.getBoundingBox())))
                .orElse(null);
      } else if (value != 0) {
        currentLivingEntity =
            currentUUID == null
                ...
                : StreamSupport.<Entity>stream(
                        minecraft.level.entitiesForRendering().spliterator(), false)
                    .filter(
                        item -> item instanceof LivingEntity && item.getUUID().equals(currentUUID))
                    .map(item -> (LivingEntity) item)
                    .findFirst()
                    .orElse(null);
      }
      ...
      if (livingEntity != null
          && !CompatEnableService.coordinatesBacktrack(livingEntity.getUUID())) {
        double doubleValue = calculateValue3(vec3, livingEntity.getBoundingBox());
        int currentValue =
            !(doubleValue <= currentProfile.maximumRange())
                    || !(doubleValue >= currentProfile.minimumRange())
                        && backtrackDelayQueue.isEmpty()
                ? 0
                : 1;
        return currentValue != 0
            && currentProfile.maximumDelayMillis() > 0
            && (!currentProfile.pauseHurt() || minecraft.player.hurtTime == 0);
      } else {
        return false;
      }
    } else {
  ...
  private static boolean checkCondition2(
      Minecraft minecraft, LivingEntity livingEntity, CompatEnableHandler.Profile profile) {
      ...
      return CompatDecisionTracker.excluded(AntibotFeatureType.BACKTRACK, livingEntity)
          ...
          : livingEntity != minecraft.player
              ...
              && (livingEntity instanceof Player
                  ? profile.players() && minecraft.player.canHarmPlayer((Player) livingEntity)
                  : livingEntity instanceof Mob && profile.mobs())
  ...
  private static int calculateValue(Packet<?> packet, Minecraft minecraft) {
    if (!(packet instanceof BundlePacket bundlePacket)) {
      if (packet instanceof ClientboundMoveEntityPacket clientboundMoveEntityPacket) {
        return clientboundMoveEntityPacket.getEntity(minecraft.level) == livingEntity
            ? 1
            : (clientboundMoveEntityPacket.getEntity(minecraft.level) == minecraft.player ? 0 : -2);
      } else if (packet instanceof ClientboundRotateHeadPacket clientboundRotateHeadPacket) {
        return clientboundRotateHeadPacket.getEntity(minecraft.level) == livingEntity
            ? 1
            : (clientboundRotateHeadPacket.getEntity(minecraft.level) == minecraft.player ? 0 : -2);
      } else if (packet instanceof ClientboundAnimatePacket clientboundAnimatePacket) {
        return clientboundAnimatePacket.getId() == livingEntity.getId()
            ? -1
            : (clientboundAnimatePacket.getId() == minecraft.player.getId() ? 0 : -2);
        ...
        return calculateValue2(clientboundSetEquipmentPacket.getEntity(), minecraft);
          ...
          instanceof ClientboundUpdateAttributesPacket clientboundUpdateAttributesPacket) {
  ...
  private static boolean checkCondition3(Packet<?> packet, Minecraft minecraft) {
    if (!(packet instanceof BundlePacket bundlePacket)) {
      if (packet instanceof ClientboundMoveEntityPacket clientboundMoveEntityPacket
          && clientboundMoveEntityPacket.hasPosition()
          && clientboundMoveEntityPacket.getEntity(minecraft.level) == livingEntity) {
        vecDeltaCodec.setBase(
            vecDeltaCodec.decode(
                clientboundMoveEntityPacket.getXa(),
                clientboundMoveEntityPacket.getYa(),
                clientboundMoveEntityPacket.getZa()));
        return true;
      } else {
        return false;
      }
  ...
  private static void updateState2(CompatEnableHandler.Pending pending) {
    Minecraft minecraft = CoreIsInitializedHandler.mc();
    if (pending.listener() == minecraft.getConnection()
        && pending.world() == minecraft.level
        && pending.listener().shouldHandleMessage(pending.packet())) {
      try {
  ...
  public static Optional<AABB> receivedBox(UUID uUID) {
    return implProfileService4 != null
            && livingEntity != null
            && livingEntity.getUUID().equals(uUID)
        ? receivedBox()
        : Optional.empty();
  }
  ...
  private static AABB createAABB() {
    return livingEntity
        .getBoundingBox()
        .move(vecDeltaCodec.getBase().subtract(livingEntity.position()));
  }
