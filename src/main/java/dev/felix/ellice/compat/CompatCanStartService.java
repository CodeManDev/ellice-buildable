...
public final class CompatCanStartService {
  ...
  public static boolean start(Minecraft minecraft) {
    ...
    RotationObserveService.Snapshot currentSnapshot =
        RotationObserveService.snapshot().orElseThrow();
    ...
    EventCancelHandler eventCancel =
        CoreIsInitializedHandler.get()
            ...
            .subscribe(
                ...
                item -> {
                  if (item.packet() instanceof ServerboundUseItemPacket serverboundUseItemPacket
                      && serverboundUseItemPacket.getHand() == InteractionHand.OFF_HAND) {
                    booleans[0] = true;
                  }
                });
    ...
    try {
      ScaffoldForActorService.withRotation(
          ...
          () -> minecraft.gameMode.useItem(currentLease.player, InteractionHand.OFF_HAND));
    } finally {
      eventCancel.cancel();
  ...
  public static boolean holding(Minecraft minecraft) {
    return lease != null
        && lease.matches(minecraft)
        && !lease.pendingRelease
        && checkCondition3(lease);
  }
  ...
  public static boolean release(Minecraft minecraft) {
    ...
    if (checkCondition3(currentLease)
        || currentLease.pendingRelease && !currentLease.player.isUsingItem()) {
      if (checkCondition3(currentLease)
          && checkCondition2(minecraft)
        ...
        EventCancelHandler eventCancel =
            CoreIsInitializedHandler.get()
                ...
                .subscribe(
                    ...
                    item -> {
                      if (item.packet()
                              instanceof ServerboundPlayerActionPacket serverboundPlayerActionPacket
  ...
  private static final class Lease {
    ...
    boolean matches(Minecraft minecraft) {
      return minecraft != null
          && minecraft.player == this.player
          && minecraft.level == this.level
          && minecraft.getConnection() == this.connection;
    }
  }
}
