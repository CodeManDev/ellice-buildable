...
public final class ItemStackDecoder {
  private static final Set<String> text =
      Set.of(
          ...
          "ominous_trial_key",
          "totem_of_undying");
  private static final Set<String> text2 =
      Set.of(
          "water_bucket",
          "lava_bucket",
          "bucket",
  ...
  public static InventoryKindData read(ItemStack itemStack) {
      ...
      ((Holder) entry.getKey())
          ...
          .ifPresent(
              item -> {
                ...
                hashMap.put(
                    text.startsWith("minecraft:") ? text.substring(10) : text, entry.getIntValue());
              });
    }
    ...
    InventoryKindData.Stats stats =
        new InventoryKindData.Stats(
            doubleValue,
  ...
  private static InventoryKindData.Kind createKind(
      ItemStack itemStack, String currentText, EquipmentSlot equipmentSlot, double doubleValue) {
    if (currentText.equals("elytra")) {
      return InventoryKindData.Kind.ELYTRA;
    }

    if (doubleValue > 0.0) {
      InventoryKindData.Kind kind =
        ...
        default:
          ...
          if (potionContents != null) {
            ...
            for (MobEffectInstance mobEffectInstance : potionContents.getAllEffects()) {
              if (((MobEffect) mobEffectInstance.getEffect().value()).getCategory()
                  == MobEffectCategory.HARMFUL) {
                return InventoryKindData.Kind.OTHER;
              }

              byteValue |=
                  ((MobEffect) mobEffectInstance.getEffect().value()).getCategory()
            ...
            return consumable != null
                    && consumable.onConsumeEffects().stream()
                        ...
                        .anyMatch(
                            statusEffect ->
                                statusEffect.probability() > 0.0F
                                    && statusEffect.effects().stream()
                                        .anyMatch(
                                            effect ->
                                                ((MobEffect) effect.getEffect().value())
                                                        .getCategory()
                                                    == MobEffectCategory.HARMFUL))
  ...
  private static double calculateValue(
    ...
    for (net.minecraft.world.item.component.ItemAttributeModifiers.Entry entry :
    ...
    return Math.max(
        0.0, (currentDoubleValue + currentDoubleValue * nextDoubleValue) * previousDoubleValue);
  }
}
