    ...
    if (minecraft.player != null
      ...
      for (int index = 5; index <= 45; index++) {
        ...
        arrayList.add(
            new InventoryCellData.Cell(
                index,
                slot.hasItem() ? ItemStackDecoder.read(slot.getItem()) : null,
                slot.isActive() && slot.mayPickup(minecraft.player)));
      }
      ...
      return Optional.of(
          new InventoryCellData(
              ...
              arrayList,
              CompatRememberInteractionService.paused(
                      minecraft, CompatAdapterService.windowHandle(minecraft))
  ...
  public boolean execute(InventoryCellData inventoryCellData, InventoryPlanService.Action action) {
    ...
    if (currentInventoryCellData != null
      ...
      if (action.input() != InventoryPlanService.Input.THROW) {
        ...
        if (!slot.hasItem()
            || slot.mayPickup(minecraft.player) && currentSlot.mayPlace(slot.getItem())) {
          if (!currentSlot.hasItem()
              || currentSlot.mayPickup(minecraft.player) && slot.mayPlace(currentSlot.getItem())) {
            if (action.input() == InventoryPlanService.Input.QUICK_MOVE) {
              ...
              ContainerInputActions.quickMove(
                  minecraft, currentInventoryMenu.containerId, action.source());
            } else {
              if (action.destination() < 36 || action.destination() > 44) {
                return false;
              }

              ContainerInputActions.swap(
                  minecraft,
                  currentInventoryMenu.containerId,
                  action.source(),
                  action.destination() - 36);
            }
      ...
      } else if (action.source() >= 9
          && action.source() <= 44
          && slot.hasItem()
          && slot.isActive()
          && slot.mayPickup(minecraft.player)) {
        ContainerInputActions.throwStack(
            minecraft, currentInventoryMenu.containerId, action.source());
        return true;
      } else {
        return false;
