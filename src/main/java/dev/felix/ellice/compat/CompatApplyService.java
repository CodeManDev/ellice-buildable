...
public final class CompatApplyService {
  private static final Identifier identifier =
      Identifier.fromNamespaceAndPath("ellice", "dynamic/local_cape");
    ...
    if (CapeEnabledService.active() && !enabled) {
      try {
        ...
        if (dynamicTexture == null || dynamicTexture.getPixels() == null) {
          NativeImage width =
              new NativeImage(
                  currentBufferedImage.getWidth(), currentBufferedImage.getHeight(), false);
        ...
        return new PlayerSkin(
            playerSkin.body(),
            new ResourceTexture(identifier, identifier),
            playerSkin.elytra(),
            playerSkin.model(),
            playerSkin.secure());
