...
public final class CompatStatusTracker {
  private static final PulseDelayQueue<CompatStatusTracker.Pending> pulseDelayQueue =
      new PulseDelayQueue<>(6);
  ...
  public static boolean intercept(
      Connection connection, Packet<?> currentPacket, PacketCaptureContext.Context context) {
    Minecraft minecraft = Minecraft.getInstance();
    if (implProfileService2 == null
        || !minecraft.isSameThread()
    ...
    if (!enabled
        && (currentPacket instanceof ServerboundMovePlayerPacket
            || currentPacket instanceof ServerboundClientTickEndPacket)) {
      ...
      if (!(currentPacket instanceof ServerboundClientTickEndPacket)) {
        ServerboundMovePlayerPacket serverboundMovePlayerPacket =
            (ServerboundMovePlayerPacket) currentPacket;
        if (pulseDelayQueue.count(item -> item.packet() instanceof ServerboundMovePlayerPacket)
            >= 3) {
          text = "Movement limit";
          updateState();
          return false;
        ...
        if (currentSnapshot != null && context != null) {
          ...
          double currentX =
              serverboundMovePlayerPacket.getX(rotationVector.x()) - rotationVector.x();
          double currentY =
              serverboundMovePlayerPacket.getY(rotationVector.y()) - rotationVector.y();
          double doubleValue =
              serverboundMovePlayerPacket.getZ(rotationVector.z()) - rotationVector.z();
          if (Double.isFinite(currentX + currentY + doubleValue)
              && !(currentX * currentX + currentY * currentY + doubleValue * doubleValue
                  > profile2.distance() * profile2.distance())) {
            if (!pulseDelayQueue.isEmpty()
                || serverboundMovePlayerPacket.hasPosition()
                    && !(currentX * currentX + currentY * currentY + doubleValue * doubleValue
                        < 1.0E-4)) {
              boolean empty = pulseDelayQueue.isEmpty();
              if (!pulseDelayQueue.offer(
                  new CompatStatusTracker.Pending(serverboundMovePlayerPacket, context),
                  longValue,
                  profile2.holdMillis())) {
                text = "Packet limit";
                updateState();
        ...
        if (pulseDelayQueue.offer(
            new CompatStatusTracker.Pending(currentPacket, null),
            longValue,
            profile2.holdMillis())) {
          return true;
        }
  ...
  private static boolean checkCondition(Minecraft minecraft, long longValue) {
    Connection currentConnection =
        minecraft.getConnection() == null ? null : minecraft.getConnection().getConnection();
    if (minecraft.level != client2
  ...
  private static void updateState() {
    if (!pulseDelayQueue.isEmpty()) {
      if (!checkCondition2()) {
        ...
        timestamp =
            System.nanoTime() + (profile2 == null ? 150 : profile2.intervalMillis()) * 1000000L;
        boolean currentEnabled = enabled;
        enabled = true;

        try {
          pulseDelayQueue.releaseAll(
              item -> {
                if (!checkCondition2()) {
                  timestamp5++;
                } else {
  ...
  public static CompatStatusTracker.Status status() {
    return new CompatStatusTracker.Status(
        pulseDelayQueue.size(),
        pulseDelayQueue.ageMillis(System.nanoTime()),
        timestamp3,
        timestamp4,
        timestamp5,
        text);
            ...
            .map(
                item -> {
                  ...
                  return localPlayer
                      .getBoundingBox()
                      .move(
                          rotationVector.x() - localPlayer.getX(),
                          rotationVector.y() - localPlayer.getY(),
                          rotationVector.z() - localPlayer.getZ());
                })
        : Optional.empty();
  }
  ...
  public record Status(
      int packets, long ageMillis, long windows, long released, long discarded, String reason) {}
}
