...
public final class SoupInventoryBridge implements SoupClickTracker.Environment {
   ...
   public static void serverSlot(int value, int currentValue, ItemStack itemStack) {
      if (compatActivateService2 != null) {
         ...
         if (minecraft.player != null) {
            ...
            compatActivateService2.updateState2(minecraft.player.inventoryMenu, currentValue, itemStack);
         }
      }
   }
   ...
   public SoupConfirmedData capture() {
      ...
      if (minecraft.player != null
         ...
         int currentCount = (this.count2 < 36 || this.count2 > 44 || value == this.count2 - 36)
               && (this.count2 != 45 || value == this.count)
            ? 0
            : 1;
         int currentValue = calculateValue() >= this.timestamp3
   ...
   public boolean openInventory(SoupConfirmedData soupConfirmedData) {
      if (!soupConfirmedData.inventoryOpen() && !soupConfirmedData.usingItem() && soupConfirmedData.cursor().empty() && this.checkCondition(soupConfirmedData)) {
         Minecraft minecraft = Minecraft.getInstance();
   ...
   public boolean closeInventory(SoupConfirmedData soupConfirmedData) {
      if (soupConfirmedData.ownedInventory() && soupConfirmedData.canReturnCraftingItems() && this.checkCondition(soupConfirmedData)) {
   ...
   @Override
   public boolean click(SoupConfirmedData soupConfirmedData, int value, int currentValue, SoupClickTracker.Click currentClick) {
      if (soupConfirmedData.inventoryOpen() && this.checkCondition(soupConfirmedData) && value >= 0 && value < 46) {
         Minecraft minecraft = Minecraft.getInstance();
   ...
   public boolean placeRecipe(SoupConfirmedData soupConfirmedData) {
      if (soupConfirmedData.inventoryOpen() && soupConfirmedData.emptyGrid() && soupConfirmedData.cursor().empty() && this.checkCondition(soupConfirmedData)) {
         Minecraft minecraft = Minecraft.getInstance();

         for (int index = 9; index <= 44; index++) {
            SoupKindData soupKindData = soupConfirmedData.slot(index);
            if (Set.of(SoupKindData.Kind.BOWL, SoupKindData.Kind.RED_MUSHROOM, SoupKindData.Kind.BROWN_MUSHROOM).contains(soupKindData.kind())
         ...
         for (RecipeCollection recipeCollection : minecraft.player.getRecipeBook().getCollections()) {
            for (RecipeDisplayEntry recipeDisplayEntry : recipeCollection.getRecipes()) {
               if (recipeDisplayEntry.display() instanceof ShapelessCraftingRecipeDisplay shapelessCraftingRecipeDisplay && shapelessCraftingRecipeDisplay.ingredients().size() == 3 && recipeDisplayEntry.canCraft(stackedItemContents)) {
                  ...
                  if (items.size() == 1
                     ...
                     if (byteValue != 0
                        && hashSet.equals(Set.of(SoupKindData.Kind.BOWL, SoupKindData.Kind.RED_MUSHROOM, SoupKindData.Kind.BROWN_MUSHROOM))) {
                        this.enabled = true;
   ...
   public boolean use(SoupConfirmedData soupConfirmedData, int value) {
      if (!soupConfirmedData.inventoryOpen()
         ...
         if (!minecraft.player.getCooldowns().isOnCooldown(itemStack) && !minecraft.gameMode.isDestroying()) {
            ...
            try {
               ...
               minecraft.gameMode.useItem(minecraft.player, value == 45 ? InteractionHand.OFF_HAND : InteractionHand.MAIN_HAND);
               return true;
            } finally {
               this.enabled = false;
            }
   ...
   public void finishUse(boolean enabled) {
      if (this.count2 >= 0) {
         Minecraft minecraft = Minecraft.getInstance();
         if (minecraft.player != null && minecraft.player.inventoryMenu == this.inventoryMenu2 && minecraft.gameMode != null) {
            int value = this.count2 != 45 && InventoryInputActions.selected(minecraft) != this.count2 - 36 ? 0 : 1;
            if (value != 0
               && minecraft.player.isUsingItem()
               && minecraft.player.getUsedItemHand() == (this.count2 == 45 ? InteractionHand.OFF_HAND : InteractionHand.MAIN_HAND)
   ...
   public static boolean holdUse() {
      if (compatActivateService2 != null && compatActivateService2.count2 >= 0) {
         ...
         return minecraft.player != null
            ...
            && minecraft.player.isUsingItem()
            && minecraft.player.getUseItem().is(Items.MUSHROOM_STEW)
            && minecraft.player.getUsedItemHand() == (compatActivateService2.count2 == 45 ? InteractionHand.OFF_HAND : InteractionHand.MAIN_HAND)
            && (compatActivateService2.count2 == 45 || InventoryInputActions.selected(minecraft) == compatActivateService2.count2 - 36);
      } else {
         return false;
      }
   }
   ...
   public static boolean handBusy() {
      if (compatActivateService2 != null && compatActivateService2.count2 >= 0 && !compatActivateService2.enabled) {
         Minecraft minecraft = Minecraft.getInstance();
         return minecraft.player != null
            && minecraft.player.inventoryMenu == compatActivateService2.inventoryMenu2
            && (compatActivateService2.count2 == 45 || InventoryInputActions.selected(minecraft) == compatActivateService2.count2 - 36);
   ...
   static SoupKindData read(ItemStack itemStack) {
      ...
      SoupKindData.Kind kind = itemStack.is(Items.MUSHROOM_STEW)
            ...
            itemStack.is(Items.BOWL)
               ...
               : (
                  itemStack.is(Items.RED_MUSHROOM)
                     ...
                     : (itemStack.is(Items.BROWN_MUSHROOM) ? SoupKindData.Kind.BROWN_MUSHROOM : SoupKindData.Kind.OTHER)
               )
         );
      int value = Set.of(SoupKindData.Kind.BOWL, SoupKindData.Kind.RED_MUSHROOM, SoupKindData.Kind.BROWN_MUSHROOM).contains(kind)
      ...
      return new SoupKindData(
         ...
         new SoupInventoryBridge.Identity(BuiltInRegistries.ITEM.getKey(itemStack.getItem()).toString(), itemStack.getComponentsPatch()),
         (value != 0)
      );
   }
