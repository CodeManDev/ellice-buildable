...
public final class CompatSinkService {
    ...
    this.entries.forEach(
        (item, currentItem) -> {
          ...
          for (int index = 0; index < currentItem.size(); index++) {
            ...
            value =
                calculateValue(
                    currentSize, nextSize, value, currentValue, currentValue + 1, currentValue + 2);
            value =
                calculateValue(
                    currentSize, nextSize, value, currentValue, currentValue + 2, currentValue + 3);
          }
          ...
          if (value > 0) {
            ...
            for (int previousIndex = 0; previousIndex < currentLength.length; previousIndex++) {
              if (currentLength[previousIndex] == 0) {
                ...
                System.arraycopy(
                    currentSize, previousIndex * 12, currentSize, nextIndex++ * 12, 12);
              }
            }
            ...
            arrayList.add(
                new CompatLoadedHandler.PlayerPart(
                    new GeometryVertexCountService(
                        Arrays.copyOf(currentSize, nextIndex * 12), Arrays.copyOf(nextSize, value)),
                    item.texture,
                    item.color));
          }
        });
  ...
  private static int calculateValue(
      float[] floats, int[] ints, int index, int value, int currentValue, int nextValue) {
    int currentIndex = value * 12;
    int nextIndex = currentValue * 12;
    int previousIndex = nextValue * 12;
    Vector3f vector3f =
        new Vector3f(
            floats[nextIndex] - floats[currentIndex],
            floats[nextIndex + 1] - floats[currentIndex + 1],
            floats[nextIndex + 2] - floats[currentIndex + 2]);
    Vector3f currentVector3f =
        new Vector3f(
            floats[previousIndex] - floats[currentIndex],
            floats[previousIndex + 1] - floats[currentIndex + 1],
            floats[previousIndex + 2] - floats[currentIndex + 2]);
  ...
  private final class Sink implements VertexConsumer {
    ...
    private void updateState() {
      if (this.enabled) {
        ...
        Vector3f vector3f =
            new Vector3f(this.float2[value + 3], this.float2[value + 4], this.float2[value + 5]);
        ...
        if (++this.count4 == 4) {
          ...
          if (this.count3 > 0 && CompatSinkService.this.count2 <= 65536) {
            CompatSinkService.this
                .entries
                .computeIfAbsent(
                    new CompatSinkService.Key(this.count3, this.count6), item -> new ArrayList<>())
                .add(this.float2);
          }
