...
public final class CompatEnableService implements TickbaseCreditService.Environment {
   ...
   private static void updateState2() {
      ...
      if (localPlayer != minecraft.player || object != minecraft.level || object2 != minecraft.getConnection()) {
         reset();
   ...
   public static void attackInput() {
      if (!tickbaseCreditService.replaying() && manualFollowup != null) {
         ...
         if (minecraft.player == currentManualFollowup.player()
            ...
            && minecraft.level.getEntity(currentManualFollowup.target().getId()) == currentManualFollowup.target()
   ...
   public static void accepted(Packet<?> packet) {
      ...
      if (minecraft.isSameThread() && minecraft.getConnection() != null) {
         if (packet instanceof ServerboundClientTickEndPacket) {
            timestamp2++;
         } else if (!(packet instanceof ServerboundKeepAlivePacket) && !(packet instanceof ServerboundPongPacket)) {
            TickbaseBeginService.Skip skip = tickbaseBeginService.activity();
            if (skip != TickbaseBeginService.Skip.NONE) {
               tickbaseCreditService.rejectSkippedTick(skip == TickbaseBeginService.Skip.REPAY, compatEnableService);
            }
         }
      }
   }
   ...
   public static void beforeIncoming(Packet<?> packet, PacketListener packetListener) {
      Minecraft minecraft = Minecraft.getInstance();
      if (implProfileService3 != null && minecraft.isSameThread() && packetListener == minecraft.getConnection()) {
         ...
         } else if (packet instanceof ClientboundPlayerPositionPacket || packet instanceof ClientboundPlayerRotationPacket) {
            corrected();
   ...
   public static String lastResult() {
      return calculateValue() - compatEnableService.timestamp4 < 3000L ? compatEnableService.text3 : "";
   }
   ...
   public static String compactStatus(TickBaseModule tickBaseModule) {
      ...
      if (text.startsWith("Cooldown")) {
         ...
         return text.startsWith("Paused while ") ? text.replace("Paused while ", "Paused: ").replace(" is active", "") : text;
      }
   }
   ...
   public static boolean coordinatesBacktrack(UUID currentUUID) {
      return implProfileService3 != null
         ...
         && (tickbaseCreditService.replaying() || tickbaseCreditService.debt() > 0 || tickbaseCreditService.credit() > 0);
   }
   ...
   public static boolean beforeTick() {
      if (implProfileService3 == null || calculateValue() < timestamp || tickbaseCreditService.replaying()) {
         return false;
      }
      ...
      if (!tickbaseBeginService.canSkip()) {
         ...
         if (!tickbaseCreditService.beforeTick(calculateValue(), implProfileService3.profile().ticks(), implProfileService3.profile().timing(), compatEnableService)) {
            return false;
         }
   ...
   public static void afterTick() {
      if (implProfileService3 != null && calculateValue() >= timestamp && !tickbaseCreditService.replaying()) {
         ...
         if (tickbaseCreditService.executed() > longValue) {
            compatEnableService.text3 = "Shifted " + (tickbaseCreditService.executed() - longValue) + " ticks";
   ...
   public boolean valid() {
      ...
      if (implProfileService3 != null
         ...
         return this.localPlayer2 == null
            || localPlayer == this.localPlayer2 && minecraft.level == this.object3 && minecraft.getConnection() == this.object4;
      }
   ...
   private static boolean checkCondition(String text) {
      return CoreIsInitializedHandler.isReady() && CoreIsInitializedHandler.get().modules().get(text).map(item -> item.isEnabled()).orElse(false);
   }
   ...
   private Vec3 createVec3() {
      ...
      if (implProfileService3.profile().syncAura() && frame != null) {
         ...
         ScaffoldRemapService.WorldVector currentWorldVector = ScaffoldRemapService.worldVector(frame.yaw(), currentInput.forwardAxis(), currentInput.leftAxis());
         return new Vec3(currentWorldVector.x(), 0.0, currentWorldVector.z());
      } else {
         int value = (currentOptions.keyUp.isDown() ? 1 : 0) - (currentOptions.keyDown.isDown() ? 1 : 0);
         int currentValue = (currentOptions.keyLeft.isDown() ? 1 : 0) - (currentOptions.keyRight.isDown() ? 1 : 0);
   ...
   public int plan(int value) {
      ...
      if (implProfileService3.profile().syncAura()) {
         this.implDecisionTracker = CoreIsInitializedHandler.get().modules().get(ImplDecisionTracker.class).filter(Module::isEnabled).orElse(null);
         this.tickBaseWindow2 = this.implDecisionTracker == null ? null : this.implDecisionTracker.tickBaseWindow().orElse(null);
      ...
      for (Entity entity : minecraft.level.entitiesForRendering()) {
         if (entity instanceof LivingEntity currentLivingEntity
            ...
            && (this.tickBaseWindow2 == null || currentLivingEntity.getUUID().equals(this.tickBaseWindow2.target()))) {
      ...
      double doubleValue = this.tickBaseWindow2 == null ? this.localPlayer2.entityInteractionRange() : this.tickBaseWindow2.reach();
      ...
      for (LivingEntity nextLivingEntity : (Iterable<LivingEntity>) (Iterable<?>) (arrayList)) {
         ...
         if (!(currentDoubleValue > implProfileService3.profile().range())) {
            ...
            for (int currentIndex = 0; currentIndex < items.size(); currentIndex++) {
               ...
               CompatEnableService.Prediction prediction = (CompatEnableService.Prediction)items.get(currentIndex);
               ...
               if (!(nextDoubleValue >= currentDoubleValue - 0.04)
                  ...
                  index++;
                  int nextValue = implProfileService3.profile().timing() == TickbaseCreditService.Mode.FUTURE ? 0 : currentValue;
                  if ((
                        this.tickBaseWindow2 == null
                           || index >= this.tickBaseWindow2.stableTicks() && this.implDecisionTracker.tickBaseWeaponReady(nextValue)
                     )
                     && (this.tickBaseWindow2 != null || !(this.localPlayer2.getAttackStrengthScale(nextValue) < 1.0F))) {
                     RotationData rotationData = RotationObserveService.current()
                        .orElse(new RotationData(this.localPlayer2.getYRot(), this.localPlayer2.getXRot()));
                     Vec3 previousVec3 = nextLivingEntity.getBoundingBox().getCenter();
                     RotationData currentRotationData = RotationData.lookAt(
                        new RotationVector(prediction.eye().x, prediction.eye().y, prediction.eye().z), new RotationVector(previousVec3.x, previousVec3.y, previousVec3.z)
                     );
                     if (this.tickBaseWindow2 == null
                        || !(
                           RotationData.distance(rotationData, currentRotationData)
                              > Math.max(
                                    5.0,
                                    this.tickBaseWindow2.turnSpeed() * 0.5
                                 )
                                 * currentValue
                        )) {
                        this.livingEntity = nextLivingEntity;
                        uUID = nextLivingEntity.getUUID();
                        this.timestamp3 = this.implDecisionTracker == null ? 0L : this.implDecisionTracker.issuedAttacks();
                        this.count = currentValue;
                        this.text2 = "Planned " + currentValue + " ticks for " + nextLivingEntity.getName().getString();
                        return currentValue;
                     }
                  }
               } else {
   ...
   private boolean checkCondition3(LivingEntity livingEntity) {
      if (FriendSyncController.excluded(FriendsMode.TICK_BASE, livingEntity)) {
         ...
         if (livingEntity != localPlayer
            ...
            && !(livingEntity instanceof Player currentPlayer && (currentPlayer.getAbilities().instabuild || !localPlayer.canHarmPlayer(currentPlayer)))
            && (currentProfile.syncAura() || currentProfile.players() && livingEntity instanceof Player || currentProfile.mobs() && livingEntity instanceof Mob)
      ...
      if (this.valid()
         ...
         if (this.implDecisionTracker != null) {
            if (this.implDecisionTracker.issuedAttacks() != this.timestamp3
               ...
               || this.implDecisionTracker.targetUuid().filter(this.livingEntity.getUUID()::equals).isEmpty()) {
               return false;
            }
         ...
         double doubleValue = this.calculateValue2(this.localPlayer2.getEyePosition(), this.livingEntity);
         if (doubleValue > implProfileService3.profile().range() || doubleValue < 0.65) {
            return false;
         }

         if (this.implDecisionTracker == null && doubleValue <= this.localPlayer2.entityInteractionRange()) {
            return false;
         }

         List items = this.collectValues(1);
         return !items.isEmpty()
            && this.calculateValue2(((CompatEnableService.Prediction)items.getFirst()).eye(), this.livingEntity)
               <= doubleValue + 0.03;
      } else {
         return false;
      }
   ...
   private List<CompatEnableService.Prediction> collectValues(int value) {
      ...
      for (int index = 0; index < value; index++) {
         ...
         float nextValue = clientLevel.getBlockState(BlockPos.containing(nextVec3.x, aABB.minY - 0.5, nextVec3.z))
            .getBlock()
            .getFriction();
          double currentDoubleValue = onGround
            ? this.localPlayer2.getSpeed() * 0.21600002 / (nextValue * nextValue * nextValue)
            : (
               this.localPlayer2.isSprinting()
                  ? 0.026
                  : 0.02
            );
         ...
         Vec3 sourceVec3 = Entity.collideBoundingBox(this.localPlayer2, previousVec3, aABB, clientLevel, List.of());
         if (Math.abs(sourceVec3.x - previousVec3.x) > 1.0E-5
            || Math.abs(sourceVec3.z - previousVec3.z) > 1.0E-5) {
            break;
         }

         int previousValue = previousVec3.y < 0.0 && Math.abs(sourceVec3.y - previousVec3.y) > 1.0E-5 ? 1 : 0;
         ...
         if (!clientLevel.getWorldBorder().isWithinBounds(aABB) || !clientLevel.hasChunkAt(BlockPos.containing(aABB.getCenter()))) {
            break;
         }
         ...
         arrayList.add(new CompatEnableService.Prediction(new Vec3(aABB.getCenter().x, aABB.minY + doubleValue, aABB.getCenter().z)));
         double previousDoubleValue = Math.abs(sourceVec3.y - previousVec3.y) > 1.0E-5 ? 0.0 : previousVec3.y;
         vec3 = new Vec3(
            sourceVec3.x * nextDoubleValue, (previousDoubleValue - this.localPlayer2.getGravity()) * 0.98, sourceVec3.z * nextDoubleValue
         );
      }
   ...
   private boolean checkCondition4(Vec3 vec3, LivingEntity livingEntity) {
      AABB aABB = CompatEnableHandler.receivedBox(livingEntity.getUUID()).orElse(livingEntity.getBoundingBox());
      return this.checkCondition5(vec3, livingEntity.getBoundingBox().getCenter()) && this.checkCondition5(vec3, aABB.getCenter());
   }

   private boolean checkCondition5(Vec3 vec3, Vec3 currentVec3) {
      return Minecraft.getInstance().level.clip(new ClipContext(vec3, currentVec3, Block.COLLIDER, Fluid.NONE, this.localPlayer2)).getType()
         == Type.MISS;
   }

   private double calculateValue2(Vec3 vec3, LivingEntity livingEntity) {
      AABB aABB = CompatEnableHandler.receivedBox(livingEntity.getUUID()).orElse(livingEntity.getBoundingBox());
      return Math.max(calculateValue3(vec3, livingEntity.getBoundingBox()), calculateValue3(vec3, aABB));
   }
   ...
   private static double calculateValue3(Vec3 vec3, AABB aABB) {
      ...
      return Math.sqrt(doubleValue * doubleValue + currentDoubleValue * currentDoubleValue + nextDoubleValue * nextDoubleValue);
   }
   ...
   public boolean extraTick() {
      ...
      if (minecraft.player == localPlayer && minecraft.level == clientLevel) {
         ...
         if (this.localPlayer2 == localPlayer
            && this.implDecisionTracker == null
            && this.livingEntity != null
            && this.calculateValue2(localPlayer.getEyePosition(), this.livingEntity) <= localPlayer.entityInteractionRange()) {
            manualFollowup = new CompatEnableService.ManualFollowup(localPlayer, clientLevel, this.livingEntity);
         }
   ...
   private void updateState3(Minecraft minecraft) {
      if (this.localPlayer2 != null && this.livingEntity != null && this.valid()) {
         if (this.implDecisionTracker != null) {
            if (this.calculateValue2(this.localPlayer2.getEyePosition(), this.livingEntity) < this.tickBaseWindow2.reach()
   ...
   private static void updateState4(Minecraft minecraft, LocalPlayer localPlayer, LivingEntity livingEntity) {
      if (minecraft.options.keyAttack.isDown() && !(localPlayer.getAttackStrengthScale(0.0F) < 1.0F)) {
         RotationObserveService.Snapshot currentSnapshot = RotationObserveService.snapshot().orElse(null);
         if (currentSnapshot != null) {
            EntityHitResult entityHitResult = CompatAvailableService.pick(minecraft, currentSnapshot.wireEyePosition(), currentSnapshot.rotation(), localPlayer.entityInteractionRange())
               .orElse(null);
            if (entityHitResult != null && entityHitResult.getEntity() == livingEntity) {
               ...
               try {
                  ScaffoldForActorService.withRotation(localPlayer, currentSnapshot.rotation(), () -> ((MinecraftAttackAccess)minecraft).ellice$startAttack());
               } finally {
