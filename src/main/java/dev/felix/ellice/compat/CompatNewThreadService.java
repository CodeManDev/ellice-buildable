...
public final class CompatNewThreadService {
   private static final Executor executor = Executors.newCachedThreadPool(new ThreadFactory() {
      ...
      public Thread newThread(Runnable runnable) {
         Thread thread = new Thread(runnable, "ellice-cookie-import-" + this.atomicInteger.incrementAndGet());
         ...
         () -> {
            try {
               ...
               throw new CompletionException(
                  new AccountCodeService("COOKIE_FILE", "Couldn't open the cookie file. Try again or paste a cookie instead.")
               );
            }
         },
         executor
   ...
   private static Optional<AccountCodec> findResult() throws Exception {
      if (Desktop.isDesktopSupported()) {
         try {
            ...
            fileDialog.setFilenameFilter(
               (item, currentItem) -> currentItem != null && (currentItem.toLowerCase().endsWith(".txt") || currentItem.toLowerCase().endsWith(".json"))
            );
      ...
      try {
         ...
         String nextName = TinyFileDialogs.tinyfd_openFileDialog("Import Microsoft cookies", "", pointerBuffer, "Cookie exports (*.txt, *.json)", false);
         result = nextName == null ? Optional.empty() : Optional.of(AccountCodec.read(Path.of(nextName)));
