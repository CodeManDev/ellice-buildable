...
public final class CompatExecuteService {
   ...
   public String execute(TaskAction taskAction, TaskData taskData, TravelLoadedHandler travelLoaded) {
   ...
   public void release() {
      ...
      try {
         ...
         if (this.count >= 0 && minecraft.player != null && minecraft.gameMode != null && CombatOwnsService.owns(CombatOwnsService.Owner.TRAVEL)) {
   ...
   private void updateState2(RotationData rotationData, ScaffoldRemapService.Input currentInput, boolean enabled) {
      if ((!currentInput.forward() || !currentInput.backward()) && (!currentInput.left() || !currentInput.right())) {
         if (enabled && currentInput.sprint()) {
            currentInput = new ScaffoldRemapService.Input(currentInput.forward(), currentInput.backward(), currentInput.left(), currentInput.right(), currentInput.jump(), currentInput.sneak(), false);
         }

         long offset = this.timestamp++;
         PlacementOverrideBus.publish(new PlacementOverrideBus.Override(true, currentInput, false, enabled, true, true));
         ScaffoldPublishService.publish(currentInput, enabled);
         LocalPhysicsFrameBus.publish(new LocalPhysicsFrameBus.Frame(offset, (float)rotationData.yaw(), currentInput, enabled));
      } else {
         throw new IllegalArgumentException("Travel input must use canonical Vanilla directions");
      }
   }

   private String createText(Minecraft minecraft, TaskData taskData, TravelLoadedHandler travelLoaded, TaskAction.Goto gotoAction) {
      this.updateState6(minecraft);
      ...
      TravelSteerService.Steer currentSteer = this.travelSteerService
         .steer(gotoAction.route(), vector3d, taskData.onGround(), taskData.inWater(), gotoAction.canSprint(), travelLoaded::jumpHeadroom);
      if (currentSteer.done()) {
         this.updateState2(new RotationData(currentSteer.yawDeg(), currentSteer.pitchDeg()), createMap(), true);
         return "";
      }

       boolean shouldJump = currentSteer.jump();
       if (!shouldJump && currentSteer.sprint() && TravelSteerService.gapAhead(travelLoaded::floor, vector3d, currentSteer.yawDeg())) {
          shouldJump = true;
      }

      RotationData rotationData = new RotationData((float)currentSteer.yawDeg(), (float)currentSteer.pitchDeg());
      CompatActiveService.follow(minecraft, rotationData, 0.35F, 100.0F);
      RotationPublishService.publish(new RotationData(minecraft.player.getYRot(), minecraft.player.getXRot()), rotationData);
      this.updateState2(
          rotationData, new ScaffoldRemapService.Input(currentSteer.forward(), false, false, false, shouldJump, false, currentSteer.sprint()), !currentSteer.sprint()
      );
      return "";
   }
   ...
   private String createText2(Minecraft minecraft, TaskData taskData, TaskAction.Mine mine) {
      ...
      if (aim(minecraft, currentX).isEmpty()) {
         BlockHitResult blockHitResult = minecraft.level.clip(new ClipContext(vec3, Vec3.atCenterOf(currentX), Block.OUTLINE, Fluid.NONE, minecraft.player));
      ...
      TaskBlockPosition nextX = new TaskBlockPosition(currentX.getX(), currentX.getY(), currentX.getZ());
      Vec3 currentVec3 = Vec3.atCenterOf(currentX);
      RotationData rotationData = RotationData.lookAt(new RotationVector(vec3.x, vec3.y, vec3.z), new RotationVector(currentVec3.x, currentVec3.y, currentVec3.z));
      ...
      if (this.taskData2 != null && this.taskData2.equals(nextX)) {
         minecraft.gameMode.continueDestroyBlock(currentX, ((BlockHitResult)result.get()).getDirection());
      } else {
         ...
         boolean enabled = minecraft.gameMode.startDestroyBlock(currentX, ((BlockHitResult)result.get()).getDirection());
         this.updateState(
            "start "
               + mine.pos()
               + " ok="
   ...
   private void updateState3(Minecraft minecraft, BlockPos blockPos) {
      ...
      for (int index = 0; index < 2 && value < 0; index++) {
         for (int currentIndex = 0; currentIndex < 9; currentIndex++) {
            ...
            if (!itemStack.isEmpty() && (index != 0 || !itemStack.isDamageableItem() || itemStack.getMaxDamage() - itemStack.getDamageValue() > 5)) {
               ...
               double currentDoubleValue = (nextValue != 0 ? currentValue * 30.0F : currentValue)
   ...
   private String createText3(Minecraft minecraft, TaskData taskData, TaskAction.Place place) {
      ...
      Direction direction = switch (place.face()) {
      ...
      BlockPos currentX = new BlockPos(place.pos().x() - direction.getStepX(), place.pos().y() - direction.getStepY(), place.pos().z() - direction.getStepZ());
      Vec3 nextX = new Vec3(
      ...
      RotationData rotationData = RotationData.lookAt(new RotationVector(vec3.x, vec3.y, vec3.z), new RotationVector(nextX.x, nextX.y, nextX.z));
      CompatActiveService.follow(minecraft, rotationData, 0.25F, 180.0F);
      RotationPublishService.publish(new RotationData(minecraft.player.getYRot(), minecraft.player.getXRot()), rotationData);
      this.updateState2(rotationData, createMap(), true);
      InteractionResult interactionResult = minecraft.gameMode
         .useItemOn(minecraft.player, InteractionHand.MAIN_HAND, new BlockHitResult(nextX, direction, currentX, false));
   ...
   private int calculateValue(Minecraft minecraft, String text) {
      ...
      if (value < 0) {
         ...
         while (true) {
            ...
            InventoryInputActions.click(minecraft, 9 + currentValue, index, SoupClickTracker.Click.SWAP);
            if (!ItemStack.isSameItemSameComponents(minecraft.player.inventoryMenu.getSlot(36 + index).getItem(), itemStack)) {
               return -1;
            }
   ...
   private String createText4(Minecraft minecraft, TaskAction.Strike strike) {
      ...
      if (entity != null && entity.isAlive()) {
         ...
         if (minecraft.player.distanceToSqr(entity.getX(), entity.getY(), entity.getZ()) > 10.240000000000002) {
            return "Out of reach";
         }

         Vec3 vec3 = minecraft.player.getEyePosition();
         RotationData currentX = RotationData.lookAt(
            new RotationVector(vec3.x, vec3.y, vec3.z),
            new RotationVector(entity.getX(), entity.getY() + entity.getBbHeight() * 0.5, entity.getZ())
         );
         CompatActiveService.follow(minecraft, currentX, 0.25F, 180.0F);
         RotationPublishService.publish(new RotationData(minecraft.player.getYRot(), minecraft.player.getXRot()), currentX);
   ...
   private void updateState5(Minecraft currentMinecraft) {
      ...
      for (String text : strings) {
         ...
         if (value >= 0) {
            ...
            if (!itemStack.isDamageableItem() || itemStack.getMaxDamage() - itemStack.getDamageValue() > 5) {
               if (this.count < 0) {
   ...
   private String createText5(Minecraft currentMinecraft, TaskAction.Cast cast) {
      ...
      if (value < 0) {
         ...
         RotationPublishService.publish(new RotationData(currentMinecraft.player.getYRot(), currentMinecraft.player.getXRot()), rotationData);
         this.updateState2(rotationData, createMap(), true);
   ...
   private String createText6(Minecraft currentMinecraft) {
      ...
      this.updateState2(new RotationData(currentMinecraft.player.getYRot(), currentMinecraft.player.getXRot()), createMap(), true);
      currentMinecraft.gameMode.useItem(currentMinecraft.player, InteractionHand.MAIN_HAND);
      return "";
   }
   ...
   private static int calculateValue2(Minecraft minecraft, String text) {
      for (int index = 0; index < 9; index++) {
         ...
         if (!itemStack.isEmpty() && BuiltInRegistries.ITEM.getKey(itemStack.getItem()).toString().equals(text)) {
            return index;
         }
      }
