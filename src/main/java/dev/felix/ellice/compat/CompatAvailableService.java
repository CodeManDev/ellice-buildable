...
public final class CompatAvailableService {
   ...
   public static boolean available(Minecraft minecraft) {
      return availableForPlanning(minecraft) && !minecraft.player.isUsingItem() && !minecraft.player.isHandsBusy();
   }
   ...
   public static Optional<EntityHitResult> pick(Minecraft minecraft, RotationVector rotationVector, RotationData rotationData, double doubleValue) {
      if (minecraft.player != null && minecraft.level != null && rotationVector != null && rotationData != null) {
         LocalPlayer localPlayer = minecraft.player;
         ...
         if (Double.isFinite(currentDoubleValue) && !(currentDoubleValue <= 0.0)) {
            ...
            Vec3 vec3 = Vec3.directionFromRotation((float)rotationData.pitch(), (float)rotationData.yaw());
            Vec3 currentVec3 = currentX.add(vec3.scale(currentDoubleValue));
            BlockHitResult blockHitResult = minecraft.level.clip(new ClipContext(currentX, currentVec3, Block.OUTLINE, Fluid.NONE, localPlayer));
            ...
            AABB aABB = localPlayer.getBoundingBox().move(currentX.subtract(localPlayer.getEyePosition())).expandTowards(currentVec3.subtract(currentX)).inflate(1.0);
            EntityHitResult entityHitResult = ProjectileUtil.getEntityHitResult(
               localPlayer, currentX, currentVec3, aABB, item -> !item.isSpectator() && item.isPickable(), nextDoubleValue
            );
            return entityHitResult != null && currentX.distanceToSqr(entityHitResult.getLocation()) < nextDoubleValue ? Optional.of(entityHitResult) : Optional.empty();
         } else {
   ...
   public static boolean attack(Minecraft minecraft, EntityHitResult entityHitResult, RotationData rotationData) {
      return attack(minecraft, entityHitResult, rotationData, 1.0F);
   }

   public static boolean attack(Minecraft minecraft, EntityHitResult entityHitResult, RotationData rotationData, float value) {
      if (entityHitResult != null && rotationData != null && available(minecraft)) {
         if (CompatDecisionTracker.excluded(AntibotFeatureType.KILL_AURA, entityHitResult.getEntity())) {
            return false;
         }
         ...
         if (Float.isFinite(currentValue) && !(currentValue < nextValue)) {
            ...
            try {
               return ScaffoldForActorService.withRotation(localPlayer, rotationData, () -> {
