...
public final class CompatShowService implements MenuHardcoreHandler {
   ...
   public static boolean show(CreateWorldScreen createWorldScreen) {
      if (!CoreIsInitializedHandler.isReady() || CoreIsInitializedHandler.get().screens() == null || Minecraft.getInstance().level != null) {
         return false;
      }
      ...
      CoreIsInitializedHandler.get().screens().open(new WorldCreationScreen(new CompatShowService(createWorldScreen)));
   ...
   public void difficulty(MenuHardcoreHandler.Difficulty currentDifficulty) {
      if (!this.worldCreationUiState.isHardcore()) {
         this.worldCreationUiState.setDifficulty(net.minecraft.world.Difficulty.valueOf(currentDifficulty.name()));
      }
   }
   ...
   public void create() {
      if (!this.enabled) {
         ...
         try {
            ...
            CompatReaderService.failure("Could not create this world. Return to your worlds and try again.");
         }
      }
   }
