...
public final class CompatCaptureService {
   ...
   public static List<CompatLoadedHandler.PlayerPart> capture(Player currentPlayer, float value) {
      try {
         ...
         EntityRenderer entityRenderer = minecraft.getEntityRenderDispatcher().getRenderer(currentPlayer);
         ...
         entityRenderer.submit(entityRenderState, poseStack, new CompatCaptureService.Collector(compatSink), cameraRenderState);
   ...
   private static int calculateValue(RenderType renderType) {
      ...
      TextureAndSampler textureAndSampler = (TextureAndSampler)((MapRenderTypeAccessor)renderType).ellice$mapState().getTextures().get("Sampler0");
      return textureAndSampler != null && textureAndSampler.textureView().texture() instanceof GlTexture glTexture ? glTexture.glId() : 0;
   }
   ...
   private static final class Collector implements SubmitNodeCollector {
      ...
      private VertexConsumer createVertexConsumer(RenderType renderType, TextureAtlasSprite textureAtlasSprite) {
         VertexConsumer vertexConsumer = this.compatSinkService.sink(CompatCaptureService.calculateValue(renderType));
      ...
      private void updateState(Pose currentPose, BakedQuad bakedQuad, int[] ints) {
         ...
         VertexConsumer vertexConsumer = this.compatSinkService.sink(CompatCaptureService.calculateValue(bakedQuad.materialInfo().itemRenderType()));
         Vector3f vector3f = new Vector3f(bakedQuad.direction().getStepX(), bakedQuad.direction().getStepY(), bakedQuad.direction().getStepZ());
         currentPose.normal().transform(vector3f).normalize();

         for (int currentIndex = 0; currentIndex < 4; currentIndex++) {
            Vector3f currentVector3f = currentPose.pose().transformPosition(new Vector3f(bakedQuad.position(currentIndex)));
      ...
      public <S> void submitModel(
         ...
         model.setupAnim(s);
         model.renderToBuffer(poseStack, this.createVertexConsumer(renderType, textureAtlasSprite), value, currentValue, nextValue);
      }
      ...
      public void submitModelPart(
         ...
         modelPart.render(poseStack, this.createVertexConsumer(renderType, textureAtlasSprite), value, currentValue, nextValue);
      }
      ...
      public void submitCustomGeometry(PoseStack poseStack, RenderType renderType, CustomGeometryRenderer customGeometryRenderer) {
         customGeometryRenderer.render(poseStack.last(), this.compatSinkService.sink(CompatCaptureService.calculateValue(renderType)));
      }
