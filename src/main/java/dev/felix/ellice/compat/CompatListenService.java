...
public final class CompatListenService {
  private static final AtomicReference<CompatListenService.Listener> items =
      new AtomicReference<>();

  private CompatListenService() {}

  public static CompatListenService.Registration listen(
      CompatListenService.Listener currentListener) {
  ...
  public static CompatListenService.Ticket request(UUID uUID, int value) {
    ...
    try {
      return listener.wantsCapture(uUID, value)
          ? new CompatListenService.Ticket(uUID, value, listener)
          : null;
  ...
  private static void updateState(
      CompatListenService.Listener currentListener, String text, RuntimeException exception) {
    items.compareAndSet(currentListener, null);
    CoreIsInitializedHandler.LOGGER.error(
        "Disabled player-model capture listener after failure while {}", text, exception);
  }
  ...
  public static final class Ticket {
    ...
    public void publish(float[] floats, float[] currentFloats) {
      ...
      if (CompatListenService.items.get() == this.items2) {
        try {
          CompatPlayerIdService compatPlayerId =
              new CompatPlayerIdService(this.uUID, this.count, floats, currentFloats);
          if (compatPlayerId.hasGeometry()) {
            this.items2.onCapture(compatPlayerId);
          }
        } catch (RuntimeException exception) {
          CompatListenService.updateState(
              this.items2, "publishing player-model capture", exception);
        }
      }
    }
  }
