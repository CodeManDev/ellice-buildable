...
public final class Mc261SnapshotService extends CompatInstanceIdentityService {
  ...
  public CompatLoadedHandler.BuildJob snapshot(TerrainData terrainData) {
    ...
    RenderSectionRegion renderSectionRegion =
        new RenderRegionCache().createRegion(minecraft.level, currentX.asLong());
    ...
    return () -> {
      ...
      try {
        ...
        for (Entry entry : results.renderedLayers.entrySet()) {
          ...
          String text =
              ((ChunkSectionLayer) entry.getKey())
                  .pipeline()
                  .getShaderDefines()
                  .values()
                  .getOrDefault("ALPHA_CUTOUT", "0");
          arrayList.add(
  ...
  public CompatLoadedHandler.Textures textures() {
    ...
    GpuTexture gpuTexture =
        minecraft.getTextureManager().getTexture(TextureAtlas.LOCATION_BLOCKS).getTexture();
    GpuTexture currentGpuTexture = minecraft.gameRenderer.levelLightmap().texture();
    return gpuTexture instanceof GlTexture glTexture
            && currentGpuTexture instanceof GlTexture currentGlTexture
        ? new CompatLoadedHandler.Textures(
            glTexture.glId(),
            currentGlTexture.glId(),
            gpuTexture.getWidth(0),
            gpuTexture.getHeight(0),
    ...
    try {
      if (player instanceof AbstractClientPlayer abstractClientPlayer) {
        ...
        AbstractTexture abstractTexture =
            Minecraft.getInstance().getTextureManager().getTexture(playerSkin.body().texturePath());
        return abstractTexture != null
                && abstractTexture.getTexture() instanceof GlTexture glTexture
            ? new CompatInstanceIdentityService.SkinInfo(
                glTexture.glId(),
                playerSkin.model() == PlayerModelType.SLIM,
                glTexture.getHeight(0) < 64)
