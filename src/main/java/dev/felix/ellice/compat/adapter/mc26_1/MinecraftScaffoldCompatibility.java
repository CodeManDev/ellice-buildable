...
final class MinecraftScaffoldCompatibility implements ScaffoldCompatibility {
   ...
   public boolean isReplaceable(Minecraft minecraft, BlockCoordinates blockCoordinates) {
      return CompatCanReplaceService.canReplace(minecraft, blockCoordinates, this.findPlaceableHotbarSlot(minecraft));
   }
   ...
   public boolean isPlayerSupporting(Minecraft minecraft, BlockCoordinates blockCoordinates) {
      ...
      return !blockState.canBeReplaced() && blockState.isCollisionShapeFullBlock(minecraft.level, blockPos);
   }

   @Override
   public double groundFriction(Minecraft minecraft) {
      return minecraft.player != null && minecraft.level != null
         ? minecraft.level.getBlockState(minecraft.player.getBlockPosBelowThatAffectsMyMovement()).getBlock().getFriction()
         : 1.0;
   }
   ...
   @Override
   public double groundInputAccelerationEstimate(Minecraft minecraft, int value, int currentValue, boolean enabled) {
      if (value < -1 || value > 1 || currentValue < -1 || currentValue > 1) {
         ...
         return value == 0 && currentValue == 0 ? 0.0 : this.groundInputAcceleration(minecraft, enabled) * calculateValue(minecraft, value, currentValue);
      }
   }
   ...
   @Override
   public boolean isFaceSturdy(Minecraft minecraft, BlockCoordinates blockCoordinates, ScaffoldMode scaffoldMode) {
      if (minecraft.level == null) {
         return false;
      }

      BlockPos blockPos = createBlockPos(blockCoordinates);
      return minecraft.level.getBlockState(blockPos).isFaceSturdy(minecraft.level, blockPos, createDirection(scaffoldMode));
   }
   ...
   public boolean canStartUseItem(Minecraft minecraft) {
      return minecraft.player != null && minecraft.gameMode != null && !minecraft.gameMode.isDestroying() && !minecraft.player.isHandsBusy();
   }

   @Override
   public Optional<PlacementCandidate> raycastPlacement(Minecraft minecraft, PlacementCandidate placementCandidate, RotationData rotationData, double doubleValue) {
      return minecraft.player != null && minecraft.level != null
         ? this.raycastPlacementFromEye(minecraft, placementCandidate, rotationData, doubleValue, createRotationData7(minecraft.player.getEyePosition()))
         : Optional.empty();
   }

   @Override
   public Optional<PlacementCandidate> raycastPlacementFromEye(
      Minecraft minecraft, PlacementCandidate placementCandidate, RotationData rotationData, double doubleValue, RotationVector rotationVector
   ) {
      return this.raycastBlockFromEye(minecraft, rotationData, doubleValue, rotationVector)
         ...
         .map(
            item -> new PlacementCandidate(placementCandidate.targetPosition(), placementCandidate.supportPosition(), placementCandidate.clickedFace(), item.hitPoint(), item.inside())
         );
   }

   @Override
   public Optional<ScaffoldBlockHit> raycastBlockFromEye(Minecraft minecraft, RotationData rotationData, double doubleValue, RotationVector rotationVector) {
      if (minecraft.player != null && minecraft.level != null) {
         Vec3 vec3 = createVec3(rotationVector);
         Vec3 currentVec3 = minecraft.player.calculateViewVector((float)rotationData.pitch(), (float)rotationData.yaw());
         ...
         if (blockHitResult instanceof BlockHitResult currentBlockHitResult && blockHitResult.getType() == Type.BLOCK) {
            ...
            AABB aABB = minecraft.player.getBoundingBox().move(previousVec3).expandTowards(currentVec3.scale(currentDoubleValue)).inflate(1.0);
            EntityHitResult entityHitResult = ProjectileUtil.getEntityHitResult(minecraft.player, vec3, nextVec3, aABB, EntitySelector.CAN_BE_PICKED, nextDoubleValue);
            return entityHitResult != null && vec3.distanceToSqr(entityHitResult.getLocation()) < nextDoubleValue
               ? Optional.empty()
               : Optional.of(
   ...
   public boolean place(Minecraft minecraft, PlacementCandidate placementCandidate) {
      if (minecraft.player != null && minecraft.gameMode != null) {
         BlockHitResult blockHitResult = new BlockHitResult(
            createVec3(placementCandidate.hitPoint()), createDirection(placementCandidate.clickedFace()), createBlockPos(placementCandidate.supportPosition()), placementCandidate.inside()
         );
         ItemStack itemStack = minecraft.player.getMainHandItem();
         int value = itemStack.getCount();
         RotationData rotationData = RotationPublishService.current().orElseGet(() -> new RotationData(minecraft.player.getYRot(), minecraft.player.getXRot()));
         InteractionResult interactionResult = ScaffoldForActorService.withRotation(
            minecraft.player, rotationData, () -> minecraft.gameMode.useItemOn(minecraft.player, InteractionHand.MAIN_HAND, blockHitResult)
         );
         if (interactionResult instanceof Success success && success.swingSource() == SwingSource.CLIENT) {
            minecraft.player.swing(InteractionHand.MAIN_HAND);
            if (!itemStack.isEmpty() && (itemStack.getCount() != value || minecraft.player.hasInfiniteMaterials())) {
   ...
   @Override
   public ScaffoldCompatibility.PlacementFeedback placementFeedback(Object value, BlockCoordinates blockCoordinates) {
      if (value instanceof ClientboundBlockUpdatePacket clientboundBlockUpdatePacket && clientboundBlockUpdatePacket.getPos().equals(createBlockPos(blockCoordinates))) {
         return checkCondition2(clientboundBlockUpdatePacket.getBlockState(), createBlockPos(blockCoordinates))
            ? ScaffoldCompatibility.PlacementFeedback.ACCEPTED
            : ScaffoldCompatibility.PlacementFeedback.REJECTED;
      } else if (value instanceof ClientboundSectionBlocksUpdatePacket clientboundSectionBlocksUpdatePacket) {
         AtomicReference atomicReference = new AtomicReference<>(ScaffoldCompatibility.PlacementFeedback.NONE);
         clientboundSectionBlocksUpdatePacket.runUpdates((item, currentItem) -> {
            if (item.equals(createBlockPos(blockCoordinates))) {
               atomicReference.set(checkCondition2(currentItem, item) ? ScaffoldCompatibility.PlacementFeedback.ACCEPTED : ScaffoldCompatibility.PlacementFeedback.REJECTED);
            }
         });
   ...
   public int placementSequence(Object value, PlacementCandidate placementCandidate) {
      if (!(value instanceof ServerboundUseItemOnPacket serverboundUseItemOnPacket)) {
         ...
         return blockHitResult.getBlockPos().equals(createBlockPos(placementCandidate.supportPosition())) && blockHitResult.getDirection() == createDirection(placementCandidate.clickedFace())
   ...
   public int acknowledgedPlacementSequence(Object value) {
      return value instanceof ClientboundBlockChangedAckPacket clientboundBlockChangedAckPacket ? clientboundBlockChangedAckPacket.sequence() : -1;
   }
   ...
   private static boolean checkCondition(ItemStack itemStack) {
      if (!(!itemStack.isEmpty() && itemStack.getItem() instanceof BlockItem blockItem)) {
         ...
         return blockItem.getBlock() instanceof FallingBlock ? false : checkCondition2(blockItem.getBlock().defaultBlockState(), BlockPos.ZERO);
      }
   }

   private static boolean checkCondition2(BlockState blockState, BlockPos blockPos) {
      return !blockState.canBeReplaced() && blockState.isCollisionShapeFullBlock(EmptyBlockGetter.INSTANCE, blockPos);
   }
