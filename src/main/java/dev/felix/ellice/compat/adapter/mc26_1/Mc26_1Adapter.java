...
public final class Mc26_1Adapter implements CompatibilityProvider {
  private static final CompatibilityCapabilities CAPABILITIES =
      new CompatibilityCapabilities("26.1.x", true, true, true);
  private static final CompatibilityDescriptor METADATA =
      new CompatibilityDescriptor(
          "mc26_1",
          "Minecraft 26.1.x",
          "26.1.x",
          List.of("26.1.x", "26.1.2"),
          CompatMode.SUPPORTED,
          CAPABILITIES);
    ...
    try {
      if (stack != null && !stack.isEmpty()) {
        ...
        return gpu instanceof GlTexture gl
            ? new TextureUvRegion(
                gl.glId(), sprite.getU0(), sprite.getV0(), sprite.getU1(), sprite.getV1())
  ...
  public CompatData worldMatrices(GameRenderer renderer) {
    ...
    return new CompatData(
        new Matrix4f(camera.viewRotationMatrix), new Matrix4f(camera.projectionMatrix), camera.pos);
  }
        ...
        (AfterInit)
            (mc, screen, w, h) ->
                ScreenMouseEvents.allowMouseClick(screen)
                    .register(
                        (AllowMouseClick)
                            (s, event) -> guard.allow((float) event.x(), (float) event.y())));
  }
}
