...
final class MinecraftCombatCompatibility implements CombatCompatibility {
   ...
   @Override
   public Optional<RotationFrameContext> capture(Minecraft minecraft, RotationTargetFilter rotationTargetFilter) {
      if (minecraft.player != null && minecraft.level != null && minecraft.player.isAlive() && !minecraft.player.isSpectator()) {
   ...
   @Override
   public boolean hasBlockLineOfSight(Minecraft minecraft, RotationVector rotationVector, RotationVector currentRotationVector) {
      if (minecraft.level != null && minecraft.player != null) {
         BlockHitResult currentX = minecraft.level
            .clip(
               new ClipContext(
                  new Vec3(rotationVector.x(), rotationVector.y(), rotationVector.z()), new Vec3(currentRotationVector.x(), currentRotationVector.y(), currentRotationVector.z()), Block.COLLIDER, Fluid.NONE, minecraft.player
               )
            );
   ...
   @Override
   public Optional<CombatData> captureAttack(Minecraft minecraft, RotationTarget rotationTarget, double doubleValue) {
      return this.findResult(minecraft, rotationTarget, doubleValue).map(MinecraftCombatCompatibility.AttackCandidate::state);
   }
   ...
   @Override
   public boolean attack(Minecraft minecraft, RotationTarget rotationTarget, double doubleValue, float value) {
      CompatStatusTracker.beforeInteraction();
      MinecraftCombatCompatibility.AttackCandidate attackCandidate = this.findResult(minecraft, rotationTarget, doubleValue).orElse(null);
      ...
      return attackCandidate != null && !(attackCandidate.state().attackStrength() < currentValue)
         ? CompatAvailableService.attack(minecraft, attackCandidate.hit(), attackCandidate.sentRotation(), currentValue)
         : false;
   }

   private Optional<MinecraftCombatCompatibility.AttackCandidate> findResult(Minecraft minecraft, RotationTarget rotationTarget, double doubleValue) {
      if (rotationTarget != null && CompatAvailableService.availableForPlanning(minecraft)) {
         ...
         if (entity != null && entity.getUUID().equals(rotationTarget.uuid()) && checkCondition(localPlayer, entity)) {
            ...
            EntityHitResult entityHitResult = CompatAvailableService.pick(minecraft, rotationVector, currentSnapshot.rotation(), doubleValue).orElse(null);
            if (entityHitResult != null && entityHitResult.getEntity() == entity) {
               if (!localPlayer.isPassenger()) {
                  EntityHitResult currentEntityHitResult = CompatAvailableService.pick(minecraft, currentSnapshot.wireEyePosition(), currentSnapshot.rotation(), doubleValue).orElse(null);
                  if (currentEntityHitResult == null || currentEntityHitResult.getEntity() != entity) {
                     return Optional.empty();
                  }
               }

               if (CompatLocalBoxService.reachable(entity, rotationVector, currentSnapshot.rotation(), doubleValue, localPlayer.entityInteractionRange())
                  && (
                     localPlayer.isPassenger()
                        || CompatLocalBoxService.reachable(entity, currentSnapshot.wireEyePosition(), currentSnapshot.rotation(), doubleValue, localPlayer.entityInteractionRange())
                  )) {
                  ...
                  return Float.isFinite(value) && Float.isFinite(currentValue) && !(currentValue <= 0.0F)
                     ? Optional.of(
                        new MinecraftCombatCompatibility.AttackCandidate(
                           entityHitResult, currentSnapshot.rotation(), new CombatData(value, currentValue, Math.max(0, ((LivingEntity)entity).hurtTime))
                        )
                     )
                     : Optional.empty();
               } else {
   ...
   private static ArrayList<RotationVector> collectValues(Minecraft minecraft, Vec3 vec3, RotationBoundingBox rotationBoundingBox) {
      ArrayList arrayList = new ArrayList();

      for (RotationVector rotationVector : rotationBoundingBox.aimSamples()) {
         if (INSTANCE.hasBlockLineOfSight(minecraft, new RotationVector(vec3.x, vec3.y, vec3.z), rotationVector)) {
   ...
   private record AttackCandidate(EntityHitResult hit, RotationData sentRotation, CombatData state) {
   }
}
