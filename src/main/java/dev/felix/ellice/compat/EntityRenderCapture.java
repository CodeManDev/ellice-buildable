...
public final class EntityRenderCapture {
  ...
  public static void capture(
      EspTargetCapture.Target target, EventAttackInputService.WorldRender worldRender) {
    CompatListenService.Ticket ticket =
        CompatListenService.request(
            target.id(), target.entity() == null ? -1 : target.entity().getId());
    if (ticket != null) {
      ...
      if (target.entity() != null) {
        ...
        EntityRenderer entityRenderer =
            minecraft.getEntityRenderDispatcher().getRenderer(currentEntity);
        EntityRenderState entityRenderState =
            entityRenderer.createRenderState(currentEntity, worldRender.tickDelta());
        entityRenderState.nameTag = null;
        Vec3 vec3 = entityRenderer.getRenderOffset(entityRenderState);
        PoseStack poseStack = new PoseStack();
        poseStack.translate(
            entityRenderState.x - worldRender.cameraPos().x + vec3.x,
            entityRenderState.y - worldRender.cameraPos().y + vec3.y,
            entityRenderState.z - worldRender.cameraPos().z + vec3.z);
        ...
        for (BlockEntity blockEntity : target.blocks()) {
          ...
          currentPoseStack.translate(
              blockPos.getX() - worldRender.cameraPos().x,
              blockPos.getY() - worldRender.cameraPos().y,
              blockPos.getZ() - worldRender.cameraPos().z);
          BlockEntityRenderer blockEntityRenderer =
              minecraft.getBlockEntityRenderDispatcher().getRenderer(blockEntity);
          if (blockEntityRenderer != null) {
            ...
            blockEntityRenderer.submit(
                blockEntityRenderState, currentPoseStack, collector, cameraRenderState);
          } else {
            ...
            blockStateModel.collectParts(
                RandomSource.create(blockEntity.getBlockState().getSeed(blockPos)), arrayList);

            for (BlockStateModelPart blockStateModelPart :
                (Iterable<BlockStateModelPart>) (Iterable<?>) (arrayList)) {
    ...
    void quad(Pose currentPose, BakedQuad bakedQuad) {
      ...
      this.transformed(
          CompatTrianglesService.triangles(
              ...
              bakedQuad.materialInfo().sprite().contents().getUniqueFrames().getInt(0)),
          currentPose.pose());
    }
  }
}
