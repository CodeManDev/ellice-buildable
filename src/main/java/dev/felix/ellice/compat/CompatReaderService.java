...
public final class CompatReaderService {
   ...
   public static MenuLoaderTracker.Loader reader() {
      return () -> {
         ...
         List items = (List)levelStorageSource.loadLevelSummaries(levelStorageSource.findLevelCandidates()).get();
         ArrayList arrayList = new ArrayList();

         for (LevelSummary levelSummary : (Iterable<LevelSummary>) (Iterable<?>) (items)) {
            int value = levelSummary.getSettings() != null ? 1 : 0;
            String text = value != 0 ? levelSummary.getGameMode().getShortDisplayName().getString() : "Unavailable";
            arrayList.add(
   ...
   private static BufferedImage createBufferedImage(Path path) {
      try {
         if (path != null && Files.isRegularFile(path, LinkOption.NOFOLLOW_LINKS) && Files.size(path) <= 262144L) {
   ...
   public static void play(String text) {
      ...
      if (minecraft.level == null) {
         ...
         try {
            minecraft.createWorldOpenFlows().openWorld(text, () -> minecraft.setScreen(new TitleScreen()));
         } catch (Exception exception) {
   ...
   public static void edit(String text) {
      ...
      if (minecraft.level == null) {
         ...
         try {
            ...
            EditWorldScreen editWorldScreen = EditWorldScreen.create(minecraft, levelStorageAccess, item -> {
   ...
   static void failure(String message) {
      Minecraft minecraft = Minecraft.getInstance();
      minecraft.setScreen(
         new AlertScreen(() -> minecraft.setScreen(new TitleScreen()), Component.literal("World unavailable"), Component.literal(message))
      );
   }
}
