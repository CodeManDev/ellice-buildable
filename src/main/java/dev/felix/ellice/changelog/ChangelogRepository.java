...
public final class ChangelogRepository {
  ...
  public static ChangelogRepository load() {
    try (InputStream inputStream =
        ChangelogRepository.class.getResourceAsStream("/assets/ellice/changelog.json")) {
      if (inputStream == null) {
        CoreIsInitializedHandler.LOGGER.warn(
            "Bundled changelog is missing: {}", "/assets/ellice/changelog.json");
      ...
      if (jsonElement.isJsonArray()) {
        ...
        jsonArray =
            jsonObject.has("releases") ? jsonObject.getAsJsonArray("releases") : new JsonArray();
      }
      ...
      for (JsonElement currentJsonElement : jsonObject.getAsJsonArray("entries")) {
        if (currentJsonElement.isJsonPrimitive()) {
          currentArrayList.add(
              new ChangelogData(ChangelogFeatureType.CHANGED, currentJsonElement.getAsString()));
        } else if (currentJsonElement.isJsonObject()) {
          JsonObject currentJsonObject = currentJsonElement.getAsJsonObject();
          currentArrayList.add(
              new ChangelogData(
                  ChangelogFeatureType.from(createText(currentJsonObject, "type")),
                  createText(currentJsonObject, "text")));
        }
      }
    }

    return new ChangelogRelease(
        createText(jsonObject, "version"),
        createText(jsonObject, "date"),
        createText(jsonObject, "title"),
        arrayList,
        currentArrayList);
  }
  ...
  private static String createText(JsonObject jsonObject, String text) {
    try {
      return jsonObject.has(text) && !jsonObject.get(text).isJsonNull()
