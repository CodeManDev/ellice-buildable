...
public record DiagnosticsData(
   int samples, double averageMs, double p95Ms, double peakMs, double seconds, Map<String, Double> sections, CompositorEmptyService graph
) {
   ...
   public static DiagnosticsData capture(DiagnosticsAddFrameListenerService diagnosticsAddFrameListener) {
      return capture(diagnosticsAddFrameListener.history(), diagnosticsAddFrameListener.head(), diagnosticsAddFrameListener.filled());
   }

   static DiagnosticsData capture(DiagnosticsAddFrameListenerService.Frame[] frames, int value, int currentValue) {
      ...
      for (int nextValue = index; nextValue > 0; nextValue += -1) {
         DiagnosticsAddFrameListenerService.Frame currentLength = frames[Math.floorMod(value - nextValue, frames.length)];
         if (currentLength != null && currentLength.totalNanos > 0L) {
            ...
            currentLength.sections
               .forEach(
                  (item, currentItem) -> linkedHashMap.merge(item, Math.max(0L, currentItem) / 1000000.0, Double::sum)
               );
         }
      }

      int previousValue = currentIndex;
      linkedHashMap.replaceAll((item, currentItem) -> previousValue == 0 ? 0.0 : currentItem / previousValue);
