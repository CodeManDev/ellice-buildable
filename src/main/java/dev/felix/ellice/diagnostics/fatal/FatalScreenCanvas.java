...
final class FatalScreenCanvas {
  ...
  int text(int value, int currentValue, int nextValue, int previousValue, String currentText) {
    ...
    for (int index = 0; index < currentText.length(); index++) {
      this.updateState2(
          sourceValue, currentValue, nextValue, previousValue, currentText.charAt(index));
    ...
    if (currentText != null && !currentText.isEmpty()) {
      ...
      while (!nextText.isEmpty()) {
        ...
        this.text(
            ...
            previousValue,
            targetValue,
            nextText.substring(0, currentLength).trim());
  ...
  String clip(String text, int y, int width) {
    ...
    while (currentLength > 0
        && this.textWidth(text.substring(0, currentLength) + currentText, width) > y) {
      currentLength += -1;
    }
      ...
      for (int index = 0; index < booleans.length; index++) {
        for (int currentIndex = 0; currentIndex < booleans[index].length; currentIndex++) {
          if (booleans[index][currentIndex]) {
            this.fill(
                targetValue + (currentIndex + byteValue) * previousValue,
                inputValue + (index + byteValue) * previousValue,
                previousValue,
                previousValue,
                -15658735);
          }
  ...
  private void updateState2(
      int value, int currentValue, int nextValue, int previousValue, char character) {
      ...
      for (int nextIndex = 0; nextIndex < 5; nextIndex++) {
        if ((sourceValue & 1 << 4 - nextIndex) != 0) {
          this.fill(
              ...
              currentValue + currentIndex * nextValue,
              nextValue,
              nextValue,
              previousValue);
        }
