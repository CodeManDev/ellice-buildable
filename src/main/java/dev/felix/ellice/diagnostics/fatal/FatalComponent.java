...
final class FatalComponent {
   ...
   static void draw(FatalScreenCanvas fatalScreenCanvas, int y, int currentWidth, String text, int value, int currentValue) {
      ...
      for (int index = 0; index < text.length(); index++) {
         ...
         for (int currentIndex = 0; currentIndex < glyph.height; currentIndex++) {
            for (int nextIndex = 0; nextIndex < glyph.width; nextIndex++) {
               ...
               if (sourceValue != 0 && nextValue >= 0 && previousValue >= 0 && nextValue < fatalScreenCanvas.w && previousValue < fatalScreenCanvas.h) {
                  int previousIndex = previousValue * fatalScreenCanvas.w + nextValue;
                  fatalScreenCanvas.px[previousIndex] = blend(fatalScreenCanvas.px[previousIndex], currentValue, sourceValue);
               }
            }
         }
   ...
   static int blend(int value, int currentValue, int nextValue) {
      ...
      int sourceValue = ((value >> 16 & 0xFF) * previousValue + (currentValue >> 16 & 0xFF) * nextValue) / 255;
      int targetValue = ((value >> 8 & 0xFF) * previousValue + (currentValue >> 8 & 0xFF) * nextValue) / 255;
   ...
   private static Map<Integer, FatalComponent.Glyph[]> createMap() {
      try (InputStream inputStream = FatalComponent.class.getResourceAsStream("/assets/ellice/fonts/crash-ui.bin.gz")) {
         if (inputStream == null) {
            return Map.of();
         }
         ...
         try (DataInputStream dataInputStream = new DataInputStream(new GZIPInputStream(inputStream))) {
            ...
            for (int index = 0; index < value; index++) {
               ...
               for (int nextIndex = 0; nextIndex < currentIndex; nextIndex++) {
                  ...
                  glyphs[nextIndex] = new FatalComponent.Glyph(nextValue, previousValue, sourceValue, targetValue, bytes);
               }
