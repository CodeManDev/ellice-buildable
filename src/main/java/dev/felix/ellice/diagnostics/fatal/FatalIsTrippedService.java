...
public final class FatalIsTrippedService {
   ...
   private static final AtomicReference<FatalCaptureService> atomicReference = new AtomicReference<>();
   ...
   static boolean trip(Throwable exception, FatalCaptureService.Kind currentKind, Path path) {
      ...
      try {
         ...
         FatalCaptureService fatalCapture = FatalCaptureService.capture(exception, text, currentText, currentKind, null);
         if (path != null) {
            try {
         ...
         logger.error("ellice fatal {} crash (id {})", new Object[]{fatalCapture.kind().name().toLowerCase(Locale.ROOT), fatalCapture.id(), exception});
         return true;
      } catch (Throwable nextException) {
         logger.error("Failed to record ellice crash", nextException);
         FatalCaptureService currentFatalCapture = FatalCaptureService.capture(exception, "?", "?", currentKind, null);
