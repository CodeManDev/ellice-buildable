...
final class FatalReportLayout {
   ...
   static FatalReportLayout.Frame paint(FatalCaptureService fatalCapture, int value, int currentValue, int nextValue, boolean enabled) {
      return new FatalReportLayout(value, currentValue).createFrame(fatalCapture, nextValue, enabled);
   }

   private FatalReportLayout.Frame createFrame(FatalCaptureService fatalCapture, int value, boolean enabled) {
      ...
      for (int index = 0; index < this.canvas.h; index++) {
         for (int currentIndex = 0; currentIndex < this.canvas.w; currentIndex++) {
            double doubleValue = Math.hypot(
               ((double)currentIndex / this.uiScale - currentValue * 0.75) / Math.max(1, currentValue),
               (double)index / this.uiScale / Math.max(1, nextValue)
            );
            int previousValue = (int)(Math.max(0.0, 1.0 - doubleValue) * 36.0);
            this.canvas.px[index * this.canvas.w + currentIndex] = FatalComponent.blend(BACKGROUND_COLOR, -9084780, previousValue);
         }
      }
      ...
      this.drawText(inputValue + targetValue - this.measureTextWidth(text, 12), resultValue + 2, 12, SECONDARY_TEXT_COLOR, text);
      ...
      this.drawText(inputValue, selectedValue, defaultValue, PRIMARY_TEXT_COLOR, fatalCapture.title());
      this.drawWrappedText(inputValue, selectedValue + (outputValue != 0 ? 38 : 43), targetValue, 14, 21, SECONDARY_TEXT_COLOR, fatalCapture.guidance(), outputValue != 0 ? 1 : 2);
      ...
      if (computedValue >= 28) {
         ...
         if (computedValue >= 76) {
            this.drawText(fallbackValue, resolvedValue + 15, 12, SECONDARY_TEXT_COLOR, fatalCapture.kind() == FatalCaptureService.Kind.LICENSE ? "WHAT TO DO" : "WHAT HAPPENED");
         }

         int secondaryValue = resolvedValue + (computedValue >= 76 ? 38 : 6);
         int tertiaryValue = Math.max(1, Math.min(4, (computedValue - 65) / 21));
         int temporaryValue = this.drawWrappedText(fallbackValue, secondaryValue, primaryValue, 14, 21, PRIMARY_TEXT_COLOR, fatalCapture.message(), tertiaryValue);
         if (temporaryValue + 34 < resolvedValue + computedValue) {
            this.drawText(fallbackValue, temporaryValue + 8, 12, ACCENT_TEXT_COLOR, this.truncateText(fatalCapture.exceptionType(), primaryValue, 12));
         }
         ...
         if (requestedValue + 37 < resolvedValue + computedValue) {
            ...
            for (int nextIndex = 3; nextIndex < strings.length && requestedValue + 50 < resolvedValue + computedValue; nextIndex++) {
               this.drawText(fallbackValue, requestedValue, 12, SECONDARY_TEXT_COLOR, this.truncateText(strings[nextIndex], primaryValue, 12));
               requestedValue += 18;
            }

            this.drawText(
               fallbackValue, resolvedValue + computedValue - 27, 12, SECONDARY_TEXT_COLOR, this.truncateText("Full details are included in the report.", primaryValue, 12)
            );
         }
      }
      ...
      if (cachedValue != 0) {
         ...
         this.drawText(actualValue + 20, resolvedValue + 15, 12, SECONDARY_TEXT_COLOR, "TAKE THE REPORT");
         int expectedValue = Math.min(128, computedValue - 64);

         try {
            boolean[][] booleans = FatalCodec.encode(fatalCapture.qrPayload());
            this.canvas
               .qr(booleans, (actualValue + (pendingValue - expectedValue) / 2) * this.uiScale, (resolvedValue + 33) * this.uiScale, expectedValue * this.uiScale);
         } catch (Throwable exception) {
            this.drawText(actualValue + 20, resolvedValue + 60, 14, SECONDARY_TEXT_COLOR, "Use Copy report");
         }

         String currentText = "Scan to save details";
         this.drawText(actualValue + (pendingValue - this.measureTextWidth(currentText, 12)) / 2, resolvedValue + 39 + expectedValue, 12, SECONDARY_TEXT_COLOR, currentText);
      }
      ...
      for (int previousIndex = 0; previousIndex < arrayList.size(); previousIndex++) {
         ...
         int storedValue = localValue != 0 ? (value == maximumValue ? -1 : -1578253) : (value == maximumValue ? -13617595 : -14802386);
         this.drawRoundedRect(startValue, endValue, minimumValue, 42, 8, storedValue);

         String nextText = switch (maximumValue) {
         ...
         currentArrayList.add(
            new FatalReportLayout.Action(
               maximumValue, startValue * this.uiScale, endValue * this.uiScale, minimumValue * this.uiScale, 42 * this.uiScale
            )
         );
      }

      String previousText = (fatalCapture.allowsContinue() ? "Esc  Continue     " : "")
         + "C  Copy report     "
         + (fatalCapture.dumpFile() != null ? "O  Open folder     " : "")
         + "Q  Quit";
      this.drawText(inputValue, candidateValue - 9, 12, SECONDARY_TEXT_COLOR, this.truncateText(previousText, targetValue, 12));
   ...
   private void drawPanel(int value, int currentValue, int nextValue, int previousValue) {
      ...
      this.drawRoundedRect(value + 1, currentValue + 1, nextValue - 2, previousValue - 2, 11, PANEL_BORDER_COLOR);
   }

   private void drawDivider(int value, int currentValue, int nextValue, int previousValue) {
      this.canvas.fill(value * this.uiScale, currentValue * this.uiScale, nextValue * this.uiScale, this.uiScale, previousValue);
   }

   private void drawRoundedRect(int value, int currentValue, int nextValue, int previousValue, int sourceValue, int targetValue) {
      int inputValue = Math.max(0, Math.min(sourceValue, Math.min(nextValue, previousValue) / 2)) * this.uiScale;
      int outputValue = nextValue * this.uiScale;
      int resultValue = previousValue * this.uiScale;

      for (int index = 0; index < resultValue; index++) {
         int candidateValue = index < inputValue ? inputValue - index - 1 : (index >= resultValue - inputValue ? index - (resultValue - inputValue) : 0);
         int selectedValue = candidateValue == 0 ? 0 : inputValue - (int)Math.sqrt((double)inputValue * inputValue - (double)candidateValue * candidateValue);
         this.canvas.fill(value * this.uiScale + selectedValue, currentValue * this.uiScale + index, outputValue - selectedValue * 2, 1, targetValue);
      }
   }
   ...
   private void drawText(int value, int currentValue, int nextValue, int previousValue, String currentText) {
      if (FatalComponent.available(nextValue * 10 + this.uiScale)) {
         FatalComponent.draw(this.canvas, value * this.uiScale, currentValue * this.uiScale, currentText, nextValue * 10 + this.uiScale, previousValue);
      } else {
         this.canvas
            .text(value * this.uiScale, (currentValue + 3) * this.uiScale, Math.max(1, nextValue / 9) * this.uiScale, previousValue, currentText);
      }
   }
   ...
   private String truncateText(String text, int value, int currentValue) {
      ...
      while (currentLength > 0 && this.measureTextWidth(text.substring(0, currentLength) + "...", currentValue) > value) {
         currentLength += -1;
      }
   ...
   private int drawWrappedText(int value, int currentValue, int nextValue, int previousValue, int sourceValue, int targetValue, String text, int inputValue) {
      String currentText = text.replaceAll("\\s+", " ").trim();

      for (int index = 0; index < inputValue && !currentText.isEmpty(); index++) {
         if (index == inputValue - 1 || this.measureTextWidth(currentText, previousValue) <= nextValue) {
            this.drawText(value, currentValue, previousValue, targetValue, this.truncateText(currentText, nextValue, previousValue));
            return currentValue + sourceValue;
         }

         int currentLength = currentText.length();

         while (currentLength > 1 && this.measureTextWidth(currentText.substring(0, currentLength), previousValue) > nextValue) {
            currentLength += -1;
         }
         ...
         this.drawText(value, currentValue, previousValue, targetValue, currentText.substring(0, currentLength));
