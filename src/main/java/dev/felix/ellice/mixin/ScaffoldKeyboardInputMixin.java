...
public abstract class ScaffoldKeyboardInputMixin {
  ...
  private boolean ellice$scaffoldInput(KeyMapping mapping) {
    ...
    boolean requested =
        MovementAvailableService.requested(mapping)
            || CoreAutoSprintRequestedClient.autoSprintRequested(mapping);
    ...
    if (mapping == this.options.keyShift) {
      ...
      return mapping == this.options.keySprint && override.overrideSprint()
          ? movement.sprint()
          : requested;
    }
  }
