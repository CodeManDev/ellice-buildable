...
public interface HudElementRenderer {
  ...
  default HudElementRenderer.Size measure(
      LayoutIsContainerService layoutIsContainer,
      LayoutOperationHandler layoutOperation,
      float value,
      float currentValue) {
    return new HudElementRenderer.Size(
        layoutIsContainer.width >= 0.0F ? layoutIsContainer.width : Math.max(0.0F, value),
        layoutIsContainer.height >= 0.0F ? layoutIsContainer.height : Math.max(0.0F, currentValue));
  }
