...
public final class PanelElementRenderer implements HudElementRenderer {
   ...
   public void draw(
      ...
      int targetValue = enabled ? LayoutStringService.color(jsonObject, "gradientEnd", sourceValue) : sourceValue;
      ...
      float outputValue = (currentEnabled ? LayoutStringService.number(jsonObject, "cornerTL", inputValue) : inputValue) * previousValue;
      float resultValue = (currentEnabled ? LayoutStringService.number(jsonObject, "cornerTR", inputValue) : inputValue) * previousValue;
      float candidateValue = (currentEnabled ? LayoutStringService.number(jsonObject, "cornerBR", inputValue) : inputValue) * previousValue;
      float selectedValue = (currentEnabled ? LayoutStringService.number(jsonObject, "cornerBL", inputValue) : inputValue) * previousValue;
      float defaultValue = LayoutStringService.number(jsonObject, "blur", 0.0F) * previousValue;
      float initialValue = LayoutStringService.number(jsonObject, "edgeSoftness", 0.0F) * previousValue;
      float resolvedValue = LayoutStringService.number(jsonObject, "borderWidth", 0.0F) * previousValue;
      int computedValue = LayoutStringService.withOpacity(LayoutStringService.color(jsonObject, "borderColor", 0), nextValue);
      boolean nextEnabled = LayoutStringService.bool(jsonObject, "border2", false);
      int cachedValue = nextEnabled
         ? LayoutStringService.withOpacity(LayoutStringService.color(jsonObject, "borderColor2", LayoutStringService.color(jsonObject, "borderColor", 0)), nextValue)
         : 0;
      if (LayoutStringService.bool(jsonObject, "glass", false)) {
         ...
         float primaryValue = LayoutStringService.number(jsonObject, "glassRefraction", 8.0F) * previousValue;
         ...
         int actualValue = LayoutStringService.withOpacity(LayoutStringService.color(jsonObject, "shadowColor", 1610612736), nextValue);
         ...
         float minimumValue = LayoutStringService.number(jsonObject, "innerHighlightSize", 0.0F) * previousValue;
