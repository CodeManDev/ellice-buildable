...
public final class KeystrokesElementRenderer implements HudElementRenderer {
  ...
  public void draw(
      ...
      float previousValue) {
    float sourceValue =
        Math.max(6.0F, LayoutStringService.number(layoutIsContainer.props, "size", 16.0F))
            * previousValue;
    float targetValue =
        Math.max(0.0F, LayoutStringService.number(layoutIsContainer.props, "gap", 3.0F))
            * previousValue;
    ...
    String text =
        layoutIsContainer.id == null
            ? Integer.toHexString(System.identityHashCode(layoutIsContainer))
            : layoutIsContainer.id;
    KeystrokesElementRenderer.ElementAnim elementAnim =
        this.text2.computeIfAbsent(text, item -> new KeystrokesElementRenderer.ElementAnim());
    long longValue = System.nanoTime();
    float inputValue =
        Math.max(0.001F, Math.min(0.05F, (float) (longValue - elementAnim.lastNanos) / 1.0E9F));
    ...
    this.updateState(
        ...
        elementAnim,
        "left",
        createText(currentKeyMapping, "A"),
        currentKeyMapping.isDown(),
        width,
        height + outputValue,
        sourceValue,
        sourceValue,
    ...
    this.updateState(
        ...
        elementAnim,
        "down",
        createText(nextKeyMapping, "S"),
        nextKeyMapping.isDown(),
        width + outputValue,
        height + outputValue,
        sourceValue,
        sourceValue,
    ...
    if (enabled) {
      ...
      this.updateState(
          ...
          width + selectedValue + targetValue,
          resultValue,
          selectedValue,
          sourceValue,
          nextValue,
  ...
  private void updateState(
      ...
      float inputValue,
      float outputValue) {
    KeystrokesElementRenderer.KeyAnim keyAnim =
        elementAnim.keys.computeIfAbsent(
            currentText, item -> new KeystrokesElementRenderer.KeyAnim());
    ...
    int tertiaryValue =
        LayoutStringService.withOpacity(
            ThemeMixService.mix(defaultValue, initialValue, activeValue), targetValue);
    ...
    if (keyAnim.ripple > 0.0F) {
      ...
      int expectedValue =
          LayoutStringService.withOpacity(
              resolvedValue, targetValue * (1.0F - keyAnim.ripple) * 0.75F);
      compositorPushPresentationScale.roundedRect(
    ...
    if (!nextText.isEmpty()) {
      float minimumValue =
          Math.min(
              sourceValue * 0.42F,
              LayoutStringService.number(jsonObject, "fontSize", 7.5F) * inputValue);
      ...
      float maximumValue =
          compositorPushPresentationScale.textWidth(
              nextText, minimumValue, TextMode.BOLD, previousText);
  ...
  private static String createText2(LayoutIsContainerService layoutIsContainer) {
    ...
    return text != null && !text.isBlank() && !"client".equalsIgnoreCase(text)
