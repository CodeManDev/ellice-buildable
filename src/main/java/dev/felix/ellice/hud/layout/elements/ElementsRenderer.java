...
public final class ElementsRenderer extends TextElementRenderer {
   @Override
   public void draw(
      ...
      super.draw(compositorPushPresentationScale, this.createLayoutIsContainerService(layoutIsContainer), width, height, value, currentValue, layoutOperation, nextValue, previousValue);
   }
   ...
   public HudElementRenderer.Size measure(LayoutIsContainerService layoutIsContainer, LayoutOperationHandler layoutOperation, float value, float currentValue) {
      return super.measure(this.createLayoutIsContainerService(layoutIsContainer), layoutOperation, value, currentValue);
   }
   ...
   public HudElementRenderer.Size measure(LayoutIsContainerService layoutIsContainer, LayoutOperationHandler layoutOperation, float value, float currentValue, CompositorPushPresentationScaleService compositorPushPresentationScale) {
      return super.measure(this.createLayoutIsContainerService(layoutIsContainer), layoutOperation, value, currentValue, compositorPushPresentationScale);
   }

   private LayoutIsContainerService createLayoutIsContainerService(LayoutIsContainerService layoutIsContainer) {
      if (layoutIsContainer.props != null && layoutIsContainer.props.has("text")) {
         return layoutIsContainer;
      }

      LayoutIsContainerService currentLayoutIsContainer = new LayoutIsContainerService(layoutIsContainer.id, layoutIsContainer.type);
      ...
      currentLayoutIsContainer.props = layoutIsContainer.props != null ? layoutIsContainer.props.deepCopy() : new JsonObject();
      currentLayoutIsContainer.props.addProperty("text", "{player.coords}");
