...
public final class ProgressBarElementRenderer implements HudElementRenderer {
  ...
  public void draw(
    ...
    float candidateValue =
        layoutIsContainer.props.has("cornerRadius")
            ? LayoutStringService.number(layoutIsContainer.props, "cornerRadius", 0.0F)
                * previousValue
            : Math.min(value, currentValue) * 0.5F;
    int selectedValue =
        LayoutStringService.withOpacity(
            LayoutStringService.color(layoutIsContainer.props, "trackColor", 1090519039),
            nextValue);
    int defaultValue =
        LayoutStringService.withOpacity(
            LayoutStringService.color(layoutIsContainer.props, "fillColor", -14494823), nextValue);
    float initialValue =
        LayoutStringService.number(layoutIsContainer.props, "borderWidth", 0.0F) * previousValue;
    int resolvedValue =
        LayoutStringService.withOpacity(
            LayoutStringService.color(layoutIsContainer.props, "borderColor", 0), nextValue);
