...
public final class ArmorElementRenderer implements HudElementRenderer {
  private static final EquipmentSlot[] equipmentSlot =
      new EquipmentSlot[] {
  ...
  public void draw(
      ...
      float previousValue) {
    float sourceValue =
        Math.max(8.0F, LayoutStringService.number(layoutIsContainer.props, "size", 26.0F))
            * previousValue;
    float targetValue =
        Math.max(0.0F, LayoutStringService.number(layoutIsContainer.props, "gap", 5.0F))
            * previousValue;
    boolean enabled =
        "vertical"
            .equalsIgnoreCase(
                LayoutStringService.string(layoutIsContainer.props, "orientation", "horizontal"));
    ...
    int inputValue =
        LayoutStringService.withOpacity(
            LayoutStringService.color(layoutIsContainer.props, "accent", -10262799), nextValue);
    ...
    for (int index = 0; index < items.size(); index++) {
      ...
      int selectedValue =
          LayoutStringService.withOpacity(itemStack.isEmpty() ? 807411752 : 1881153584, nextValue);
      ...
      if (!itemStack.isEmpty()) {
        ...
        if (nextEnabled && itemStack.isDamageableItem() && itemStack.getMaxDamage() > 0) {
          float resolvedValue =
              1.0F - (float) itemStack.getDamageValue() / itemStack.getMaxDamage();
          ...
          compositorPushPresentationScale.roundedRect(
              ...
              activeValue,
              activeValue * 0.5F,
              LayoutStringService.withOpacity(2013265920, nextValue));
          compositorPushPresentationScale.roundedRect(
              computedValue,
              cachedValue,
  ...
  public HudElementRenderer.Size measure(
    ...
    int currentSize =
        Math.max(
            1,
            collectValues(LayoutStringService.bool(layoutIsContainer.props, "showEmpty", false))
                .size());
    boolean enabled =
        "vertical"
            .equalsIgnoreCase(
    ...
    return enabled
        ? new HudElementRenderer.Size(
            nextValue, currentSize * nextValue + Math.max(0, currentSize - 1) * previousValue)
        : new HudElementRenderer.Size(
            currentSize * nextValue + Math.max(0, currentSize - 1) * previousValue, nextValue);
  }
