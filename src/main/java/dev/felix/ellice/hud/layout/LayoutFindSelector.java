...
public final class LayoutFindSelector {
  ...
  private static LayoutIsContainerService createLayoutIsContainerService(
      List<LayoutIsContainerService> items, String text) {
    if (items == null) {
      return null;
    }
    ...
    for (LayoutIsContainerService layoutIsContainer : items) {
      ...
      LayoutIsContainerService currentLayoutIsContainer =
          createLayoutIsContainerService(layoutIsContainer.children, text);
