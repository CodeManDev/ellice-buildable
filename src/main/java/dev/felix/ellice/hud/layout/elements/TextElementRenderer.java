...
public class TextElementRenderer implements HudElementRenderer {
   ...
   public void draw(
      ...
      if (!currentText.isEmpty()) {
         ...
         if (!nextText.isEmpty()) {
            float sourceValue = LayoutStringService.number(layoutIsContainer.props, "fontSize", 9.0F) * previousValue;
            int targetValue = LayoutStringService.withOpacity(LayoutStringService.color(layoutIsContainer.props, "color", -1), nextValue);
            TextMode textMode = this.createTextMode(LayoutStringService.string(layoutIsContainer.props, "variant", "regular"));
            TextData textData = TextData.NONE.withVariant(textMode).withFontFamily(createText(layoutIsContainer));
            float inputValue = LayoutStringService.number(layoutIsContainer.props, "shadowX", 0.0F) * previousValue;
            float outputValue = LayoutStringService.number(layoutIsContainer.props, "shadowY", 0.0F) * previousValue;
            int resultValue = LayoutStringService.color(layoutIsContainer.props, "shadowColor", Integer.MIN_VALUE);
            if (inputValue != 0.0F || outputValue != 0.0F) {
               textData = textData.withShadow(inputValue, outputValue, LayoutStringService.withOpacity(resultValue, nextValue));
            }

            float candidateValue = LayoutStringService.number(layoutIsContainer.props, "outlineWidth", 0.0F) * previousValue;
            if (candidateValue > 0.0F) {
               int selectedValue = LayoutStringService.color(layoutIsContainer.props, "outlineColor", -16777216);
               textData = textData.withOutline(candidateValue, LayoutStringService.withOpacity(selectedValue, nextValue));
            }

            compositorPushPresentationScale.text(width, height, nextText, sourceValue, targetValue, textData);
            if (LayoutStringService.bool(layoutIsContainer.props, "underline", false)) {
               float defaultValue = compositorPushPresentationScale.textWidth(nextText, sourceValue, textMode, createText(layoutIsContainer));
               float initialValue = Math.max(1.0F, sourceValue * 0.07F);
               compositorPushPresentationScale.roundedRect(width, height + sourceValue * 0.92F, defaultValue, initialValue, 0.0F, targetValue);
            }
         }
      }
   }
   ...
   @Override
   public HudElementRenderer.Size measure(LayoutIsContainerService layoutIsContainer, LayoutOperationHandler layoutOperation, float value, float currentValue, CompositorPushPresentationScaleService compositorPushPresentationScale) {
      ...
      if (nextValue < 0.0F || previousValue < 0.0F) {
         ...
         if (nextValue < 0.0F) {
            String currentText = layoutOperation.interpolate(LayoutStringService.string(layoutIsContainer.props, "text", ""));
            TextMode textMode = this.createTextMode(LayoutStringService.string(layoutIsContainer.props, "variant", "regular"));
            nextValue = compositorPushPresentationScale != null
               ? Math.max(1.0F, compositorPushPresentationScale.textWidth(currentText, sourceValue, textMode, createText(layoutIsContainer)))
   ...
   private static String createText(LayoutIsContainerService layoutIsContainer) {
      ...
      return text != null && !text.isEmpty() && !"client".equalsIgnoreCase(text) ? text.toLowerCase(Locale.ROOT) : null;
   }
