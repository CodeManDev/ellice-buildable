...
public final class HudMeshService {
   ...
   public static HudMeshService.Mesh build(List<HudMeshService.Row> items, float value, float currentValue) {
      List currentItems = items.stream()
         .filter(
            item -> Float.isFinite(item.width)
      ...
      arrayList.add(new HudMeshService.Point(((HudMeshService.Row)currentItems.getFirst()).width, 0.0F));
      ...
      for (int index = 0; index < currentItems.size(); index++) {
         ...
         if (index + 1 >= currentItems.size()
            || !(Math.abs(row.width - ((HudMeshService.Row)currentItems.get(index + 1)).width) < 0.001F)) {
            arrayList.add(new HudMeshService.Point(row.width, nextValue));
            if (index + 1 < currentItems.size()) {
               arrayList.add(new HudMeshService.Point(((HudMeshService.Row)currentItems.get(index + 1)).width, nextValue));
            }
         }
      }
      ...
      for (int currentIndex = 0; currentIndex < arrayList.size(); currentIndex++) {
         HudMeshService.Point currentSize = (HudMeshService.Point)arrayList.get((currentIndex + arrayList.size() - 1) % arrayList.size());
         HudMeshService.Point point = (HudMeshService.Point)arrayList.get(currentIndex);
         HudMeshService.Point nextSize = (HudMeshService.Point)arrayList.get((currentIndex + 1) % arrayList.size());
         ...
         for (int nextIndex = 1; nextIndex <= 6; nextIndex++) {
            ...
            updateState(
               ...
               new HudMeshService.Point(
                  resultValue * resultValue * currentPoint.x + 2.0F * resultValue * outputValue * point.x + outputValue * outputValue * nextPoint.x,
                  resultValue * resultValue * currentPoint.y + 2.0F * resultValue * outputValue * point.y + outputValue * outputValue * nextPoint.y
               )
            );
         }
      }
      ...
      if (currentArrayList.size() > 1
         && calculateValue2((HudMeshService.Point)currentArrayList.getFirst(), (HudMeshService.Point)currentArrayList.getLast()) <= 1.0E-5F) {
         currentArrayList.removeLast();
      }

      return new HudMeshService.Mesh(createFloat(currentArrayList), createFloat2(currentArrayList, currentValue), previousValue, nextValue);
   }
   ...
   private static float[] createFloat(List<HudMeshService.Point> items) {
      ...
      for (int index = 0; index + 1 < currentItems.size(); index++) {
         ...
         for (int currentIndex = 0; currentIndex < items.size(); currentIndex++) {
            ...
            HudMeshService.Point nextSize = (HudMeshService.Point)items.get((currentIndex + 1) % items.size());
            if (!(doubleValue <= Math.min(point.y, nextSize.y)) && !(doubleValue >= Math.max(point.y, nextSize.y))) {
               double previousDoubleValue = (double)nextSize.x - point.x;
               double sourceDoubleValue = (double)nextSize.y - point.y;
               double targetDoubleValue = point.x + (doubleValue - point.y) / sourceDoubleValue * previousDoubleValue;
               float outputValue = (float)(point.x + (currentValue - point.y) / sourceDoubleValue * previousDoubleValue);
               float resultValue = (float)(point.x + (nextValue - point.y) / sourceDoubleValue * previousDoubleValue);
         ...
         if (Float.isFinite(previousValue) && Float.isFinite(targetValue)) {
            value = calculateValue(currentSize, value, previousValue, currentValue, targetValue, currentValue, inputValue, nextValue);
            value = calculateValue(currentSize, value, previousValue, currentValue, inputValue, nextValue, sourceValue, nextValue);
         }
      }
   ...
   private static int calculateValue(float[] floats, int index, float value, float currentValue, float nextValue, float previousValue, float sourceValue, float targetValue) {
      if (Math.abs((nextValue - value) * (targetValue - currentValue) - (previousValue - currentValue) * (sourceValue - value)) < 1.0E-7F) {
         return index;
      }
   ...
   private static float[] createFloat2(List<HudMeshService.Point> items, float value) {
      ...
      for (int index = 0; index < currentSize; index++) {
         HudMeshService.Point point = (HudMeshService.Point)items.get((index + currentSize - 1) % currentSize);
         ...
         float previousValue = (currentPoint.y - point.y) / currentValue + (nextPoint.y - currentPoint.y) / nextValue;
         float sourceValue = (point.x - currentPoint.x) / currentValue + (currentPoint.x - nextPoint.x) / nextValue;
         float targetValue = Math.max(0.001F, (float)Math.hypot(previousValue, sourceValue));
         previousValue /= targetValue;
         sourceValue /= targetValue;
         float inputValue = previousValue * (nextPoint.y - currentPoint.y) / nextValue + sourceValue * (currentPoint.x - nextPoint.x) / nextValue;
      ...
      for (int currentIndex = 0; currentIndex < currentSize; currentIndex++) {
         for (int nextIndex = 0; nextIndex < 6; nextIndex++) {
            ...
            currentFloats[previousIndex] = ((HudMeshService.Point)items.get(resultValue)).x + floats[resultValue * 2] * nextFloats[nextIndex];
            currentFloats[previousIndex + 1] = ((HudMeshService.Point)items.get(resultValue)).y + floats[resultValue * 2 + 1] * nextFloats[nextIndex];
   ...
   private static void updateState(List<HudMeshService.Point> items, HudMeshService.Point point) {
      if (items.isEmpty() || calculateValue2((HudMeshService.Point)items.getLast(), point) > 1.0E-5F) {
         items.add(point);
      }
   }

   private static float calculateValue2(HudMeshService.Point point, HudMeshService.Point currentPoint) {
      return (float)Math.hypot(currentPoint.x - point.x, currentPoint.y - point.y);
   }

   private static HudMeshService.Point createPoint(HudMeshService.Point point, HudMeshService.Point currentPoint, float width) {
      return new HudMeshService.Point(point.x + (currentPoint.x - point.x) * width, point.y + (currentPoint.y - point.y) * width);
   }
