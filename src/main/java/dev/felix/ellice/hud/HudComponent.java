  ...
  protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
    List currentItems =
        this.children().stream()
            .map(item -> new HudMeshService.Row(item.computedW(), item.computedH()))
            .toList();
    if (this.mesh == null || !currentItems.equals(this.items) || this.value2 != this.value) {
      this.mesh = HudMeshService.build(currentItems, 5.0F * this.value, 0.8F * this.value);
    ...
    if (!(this.mesh.height() <= 0.001F)) {
      this.effectiveOpacity =
          this.effectiveOpacity
              * ActiveModuleSlot.smooth(this.mesh.height() / Math.max(0.001F, 12.0F * this.value));
