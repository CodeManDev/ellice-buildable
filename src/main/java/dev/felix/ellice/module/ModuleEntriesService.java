...
public final class ModuleEntriesService extends ModuleSetting.Mode {
   ...
   public ModuleEntriesService(String text, List<ModuleEntriesService.Entry> currentItems, String currentText) {
      super(text, currentItems.stream().map(ModuleEntriesService.Entry::id).toArray(String[]::new), currentText);
   ...
   public ModuleEntriesService.Entry selected() {
      return this.items.stream().filter(item -> item.id().equals(this.get())).findFirst().orElseThrow();
   }
   ...
   public List<ModuleEntriesService.Entry> search(String query, String edition, String group) {
      ...
      return this.items
         ...
         .filter(item -> {
            String searchableText = (item.label() + " " + item.edition() + " " + item.group() + " " + item.description())
   ...
   public record Entry(String id, String label, String edition, String group, String description, boolean deprecated) {
   }
}
