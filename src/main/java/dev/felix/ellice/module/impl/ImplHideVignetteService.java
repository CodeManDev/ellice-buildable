...
public final class ImplHideVignetteService extends ModuleSettingsService {
   private final ModuleSetting.Bool moduleBool = this.setting(
      new ModuleSetting.Bool("Vignette", true)
         .description("Hides vanilla edge darkening, including the low-light vignette around the screen.")
   );
   private final ModuleSetting.Bool moduleBool2 = this.setting(
      new ModuleSetting.Bool("Helmet Overlays", true)
         .description("Hides first-person texture overlays from worn head items, such as the carved-pumpkin mask.")
   );
   private final ModuleSetting.Bool moduleBool3 = this.setting(
      new ModuleSetting.Bool("Powder Snow", false)
         .description("Hides the frozen-border overlay shown while the player is inside powder snow.")
   );
   private final ModuleSetting.Bool moduleBool4 = this.setting(
      new ModuleSetting.Bool("Portal", true).description("Hides the purple distortion overlay while standing in a Nether portal.")
   );
   private final ModuleSetting.Bool moduleBool5 = this.setting(
      new ModuleSetting.Bool("Nausea", true).description("Hides the screen distortion produced by the Nausea status effect.")
   );
   private final ModuleSetting.Bool moduleBool6 = this.setting(
      new ModuleSetting.Bool("Spyglass Scope", false)
         .description("Hides the dark scope frame while using a spyglass; the zoom itself remains active.")
   );
   ...
   public ImplHideVignetteService() {
      super(
         ModuleBuilderData.builder("Clear GUI")
            .description("Hides selected first-person overlays such as the vignette, portals, and helmets.")
