...
public final class TriggerComponent extends ModuleSettingsService {
   private final ModuleSetting.Mode moduleMode = this.setting(
      new ModuleSetting.Mode("Trigger", new String[]{"Middle", "Left", "Right"}, "Middle")
         .description("Middle = pick-block key, no vanilla effect in survival. Left/Right also fire the vanilla action.")
   );
   ...
   private final ModuleSetting.Number moduleNumber2 = this.setting(
      new ModuleSetting.Number(
            "Burst range", 40.0F, 10.0F, 48.0F, 1.0F
         )
         .description("Up to this distance the fast chokeless burst fires (Burst8/Dual8). Beyond it the trigger snap chain runs.")
   );
   private final ModuleSetting.Number moduleNumber3 = this.setting(
      new ModuleSetting.Number("Burst per tick", 2.0F, 1.0F, 4.0F, 1.0F)
         .description("Burst claims per tick. 2/tick = Dual8 level (16/tick, 0 flags STILL).")
   );
   private final ModuleSetting.Number moduleNumber4 = this.setting(
      new ModuleSetting.Number("Max snaps", 3.0F, 1.0F, 6.0F, 1.0F)
         .description("Max trigger snaps per shot (~48 blocks each, 60-tick pause between, DualSnapDelay).")
   );
   private final ModuleSetting.Number moduleNumber5 = this.setting(
      new ModuleSetting.Number(
         ...
         .description("Pause between shots. Covers the ~1.8s Vulcan reference frame with margin.")
   );
   private final ModuleSetting.Bool moduleBool = this.setting(
      new ModuleSetting.Bool("Require standstill", true)
         .description(
            "Fires only with no move input, near-zero velocity and on ground (lab STILL context; firing while moving flags, BurstLegit 16F). Preview turns amber while moving."
         )
   );
   ...
   private final ModuleSetting.Bool moduleBool2 = this.setting(
      ...
      new ModuleSetting.Bool("Show target", true).description("Shows the landing box and, while travelling, the ghost box.")
   );
   private final ModuleSetting.Color moduleColor = this.setting(this.moduleNameService, new ModuleSetting.Color("Target color", -866194732));
   private final ModuleSetting.Color moduleColor2 = this.setting(this.moduleNameService, new ModuleSetting.Color("Ghost color", -855916764));
   private final ModuleSetting.Bool moduleBool3 = this.setting(
      this.moduleNameService, new ModuleSetting.Bool("See through", true).description("Keeps the boxes visible through terrain.")
   );
   ...
   public TriggerComponent() {
      super(
         ModuleBuilderData.builder("ClickTeleport")
            ...
            .description("Click to teleport: fast burst nearby, trigger snap chain far away. Server carries every landing.")
            .build()
      );
   ...
   protected void onEnable() {
      ...
      this.on(EventAttackInputService.TICK).priority(EventIsAfterHandler.Priority.FIRST).run(item -> this.updateState2());
   ...
   private void updateState2() {
      ...
      if (localPlayer != null && clientPacketListener != null && minecraft.level != null) {
         if (this.count2 < this.items.size()) {
            ...
            while (this.count2 < this.items.size() && this.items.get(this.count2).tick() <= this.count) {
            ...
            boolean down = switch ((String)this.moduleMode.get()) {
            ...
            if (value != 0 && minecraft.screen == null && localPlayer.isAlive() && this.vec33 != null) {
               if (!(Boolean)this.moduleBool.get() || this.checkCondition(minecraft, localPlayer)) {
                  if (System.currentTimeMillis() - this.timestamp >= Math.round((Float)this.moduleNumber5.get())) {
   ...
   private Vec3 createVec3(LocalPlayer localPlayer) {
      HitResult hitResult = localPlayer.pick(((Float)this.moduleNumber.get()).floatValue(), 1.0F, false);
      if (hitResult instanceof BlockHitResult blockHitResult) {
         return this.createVec32(blockHitResult.getBlockPos().relative(blockHitResult.getDirection()));
      } else {
         return hitResult instanceof EntityHitResult entityHitResult ? this.createVec32(entityHitResult.getEntity().blockPosition()) : hitResult.getLocation();
      }
   }
   ...
   private Vec3 createVec32(BlockPos blockPos) {
      ...
      for (int index = 0; index < 4; index++) {
         if (clientLevel.getBlockState(blockPos).isAir() && clientLevel.getBlockState(blockPos.above()).isAir()) {
            return new Vec3(
               blockPos.getX() + 0.5,
               blockPos.getY(),
               blockPos.getZ() + 0.5
   ...
   private void updateState3(LocalPlayer localPlayer, Vec3 currentVec3) {
      ...
      double doubleValue = Math.sqrt(previousVec3.x * previousVec3.x + previousVec3.z * previousVec3.z);
      double[] doubles = doubleValue < 1.0E-4
         ? new double[]{1.0, 0.0}
         : new double[]{previousVec3.x / doubleValue, previousVec3.z / doubleValue};
      double currentDoubleValue = Math.sqrt(
         Math.pow(currentVec3.x - nextVec3.x, 2.0)
            + Math.pow(currentVec3.z - nextVec3.z, 2.0)
      );
      if (currentDoubleValue <= ((Float)this.moduleNumber2.get()).floatValue()) {
         this.items = MovementBurstScheduleService.burstSchedule(nextVec3, currentVec3, doubles, Math.max(1, Math.round((Float)this.moduleNumber3.get())));
         this.enabled = false;
      } else {
         this.items = MovementBurstScheduleService.triggerSchedule(nextVec3, currentVec3, Math.max(1, Math.round((Float)this.moduleNumber4.get())));
         this.enabled = true;
      }
   ...
   private void updateState4(EventAttackInputService.WorldRender worldRender) {
      if ((Boolean)this.moduleBool2.get()) {
         ...
         if (localPlayer != null) {
            ...
            int currentValue = value != 0 ? (Integer)this.moduleColor.get() : (Integer)this.moduleColor2.get();
            if (this.vec32 != null) {
            ...
            if (!arrayList.isEmpty()) {
               this.renderer.render(arrayList, worldRender, window.getWidth(), window.getHeight(), this.createStyle(currentValue));
            }

            if (!currentArrayList.isEmpty()) {
               this.renderer
                  .render(currentArrayList, worldRender, window.getWidth(), window.getHeight(), this.createStyle((Integer)this.moduleColor2.get()));
            }
         }
      }
   }
   ...
   private BoxOverlayRenderer.Style createStyle(int value) {
      ...
      return new BoxOverlayRenderer.Style(currentValue, value, 0.012F, true, true, (Boolean)this.moduleBool3.get());
   }
}
