...
public final class NoteStudioModule extends ModuleSettingsService implements ModuleOperationHandler {
   private NotebotDecoderTracker notebotDecoderTracker;

   public NoteStudioModule() {
      super(
         ModuleBuilderData.builder("Note Studio")
            .description("Choose an MP3, auto-tune nearby note blocks, and perform its melody and chords.")
   ...
   public NotebotDecoderTracker controller() {
      if (this.notebotDecoderTracker == null) {
         this.notebotDecoderTracker = new NotebotDecoderTracker(new CompatContextService(), item -> Minecraft.getInstance().execute(item));
      }
   ...
   protected void onDisable() {
      ...
      if (CoreIsInitializedHandler.isReady() && CoreIsInitializedHandler.get().screens() != null && "note-studio".equals(CoreIsInitializedHandler.get().screens().currentId())) {
