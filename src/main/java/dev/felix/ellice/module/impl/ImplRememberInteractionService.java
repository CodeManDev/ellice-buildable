...
public final class ImplRememberInteractionService extends ModuleSettingsService {
   private final ModuleNameService moduleNameService = this.settingCategory("Selection")
      .description("Compares chest contents with equipped armor, the hotbar, inventory, and offhand.");
   private final ModuleNameService moduleNameService2 = this.settingCategory("Stock targets")
      .description("Refills using whole stacks. The final stack may exceed a target; nothing is discarded.");
   private final ModuleNameService moduleNameService3 = this.settingCategory("Pacing")
      .description("One normal inventory action at a time, with reading and inspection pauses.");
   private final ModuleNameService moduleNameService4 = this.settingCategory("Controls")
      .description("Controls activation and containers. Your own slot input always pauses collection.");
   private final ModuleSetting.Mode moduleMode = this.setting(
      ...
      new ModuleSetting.Mode("Loot mode", new String[]{"Smart", "Everything"}, "Smart")
         .description("Smart takes equipment upgrades and useful supplies. Everything collects all categories; exclusions still apply.")
   );
   private final ModuleSetting.MultiSelect moduleMultiSelect = this.setting(
      this.moduleNameService,
      new ModuleSetting.MultiSelect(
            "Take items",
            new String[]{"Armor", "Weapons", "Tools", "Food", "Blocks", "Supplies", "Valuables", "Other"},
            "Armor",
            "Weapons",
            "Tools",
            "Food",
         ...
         .description("Selects item groups for Smart mode. Silk Touch tools are compared separately from ordinary and Fortune tools.")
   );
   private final ModuleSetting.Bool moduleBool = this.setting(
      this.moduleNameService,
      new ModuleSetting.Bool("Skip cursed gear", true)
         .description("Leaves items with Curse of Binding or Curse of Vanishing in the container.")
   );
   private final ModuleSetting.Number moduleNumber = this.setting(
      ...
      new ModuleSetting.Number("Minimum durability (%)", 10.0F, 0.0F, 100.0F, 1.0F)
         .description(
            "Leaves equipment below this remaining durability. Armor protection, toughness, enchantments and wear all affect upgrade ranking."
         )
   );
   private final ModuleSetting.Text moduleText = this.setting(
      ...
      new ModuleSetting.Text("Ignore items", "", 1024)
         .description(
            "Comma-separated item IDs to leave behind, such as minecraft:cobblestone, minecraft:rotten_flesh. Applies in both modes."
         )
   );
   ...
   private final ModuleSetting.Number moduleNumber8 = this.setting(
      ...
      new ModuleSetting.Number("Keep free slots", 0.0F, 0.0F, 9.0F, 1.0F)
         .description("Keeps this many inventory slots empty. Matching stacks can still merge without using another slot.")
   );
   private final ModuleSetting.Bool moduleBool2 = this.setting(
      ...
      new ModuleSetting.Bool("Thoughtful pacing", true)
         .description(
            "Adds time to inspect equipment and reach distant slots, with a varied tempo and occasional short pauses. Does not guarantee human behavior."
         )
   );
   private final ModuleSetting.Range moduleRange = this.setting(
      ...
      new ModuleSetting.Range(
         ...
         .description("Base delay between transfers. Inspection pauses can add time; with Thoughtful pacing off, the lower value is used.")
   );
   private final ModuleSetting.Range moduleRange2 = this.setting(
      ...
      new ModuleSetting.Range(
         ...
         .description("Waits after opening a container before selecting its contents. No backlog of clicks is sent after a slow frame.")
   );
   private final ModuleSetting.Bool moduleBool3 = this.setting(
      this.moduleNameService3,
      new ModuleSetting.Bool("Close when done", true)
         .description(
            "Closes after no useful items remain and the closing delay has elapsed. A full inventory or failed transfer keeps the container open."
         )
   );
   private final ModuleSetting.Number moduleNumber9 = this.setting(
      ...
      new ModuleSetting.Number(
         ...
         .description("Waits after the final inventory change before the normal close action; newly arriving loot cancels closing.")
   );
   private final ModuleSetting.Bool moduleBool4 = this.setting(
      ...
      new ModuleSetting.Bool("Only storage blocks", true)
         .description(
            "Requires opening a chest, barrel, ender chest or shulker block. Turning away while the menu loads is fine. Turn off for virtual chest menus or chest minecarts."
         )
   );
   private final ModuleSetting.Bool moduleBool5 = this.setting(
      this.moduleNameService4,
      new ModuleSetting.Bool("Include shulker boxes", true)
         .description("Also collects from open shulker boxes. Crafting tables, furnaces, merchants and player inventories are excluded.")
   );
   private final ModuleSetting.Bool moduleBool6 = this.setting(
      this.moduleNameService4,
      new ModuleSetting.Bool("Hold to collect", false)
         .description("Collects only while the activation key is held; releasing it pauses pending selections.")
   );
   private final ModuleSetting.Keybind moduleKeybind = this.setting(
      this.moduleNameService4,
      new ModuleSetting.Keybind("Collection key", 342)
         .description("Key held for Hold to collect. Choose it using the rendered keyboard in ClickGUI.")
   );
   private final InventoryEnvironmentTracker inventoryEnvironmentTracker = new InventoryEnvironmentTracker(new Random());
   private final CompatRememberInteractionService compatRememberInteractionService = new CompatRememberInteractionService(
      this.moduleBool4::get,
      this.moduleBool5::get,
      this.moduleBool6::get,
      () -> ModuleBindService.isKeyboardKey((Integer)this.moduleKeybind.get()) ? (Integer)this.moduleKeybind.get() : -1
   );

   public ImplRememberInteractionService() {
      super(
         ModuleBuilderData.builder("ChestStealer")
            .description("Selects useful chest loot, compares equipment, and refills supplies with paced inventory clicks.")
            .category(ModuleFeatureType.PLAYER)
            .build()
      );
      this.moduleMultiSelect.visibleWhen(this.moduleMode, "Smart"::equals);

      for (ModuleSetting.Number number : List.of(
         this.moduleNumber2, this.moduleNumber3, this.moduleNumber4, this.moduleNumber5, this.moduleNumber6, this.moduleNumber7
      )) {
   ...
   private ModuleSetting.Number createNumber(String text, int value, int currentValue) {
      return this.setting(
         ...
         new ModuleSetting.Number(text, value, 0.0F, currentValue, 1.0F)
            .description(
               "Collects whole stacks until this item group's carried count reaches the target. Zero skips the group; the last stack can exceed the target."
            )
      );
   }
   ...
   protected void onEnable() {
      ...
      this.on(EventAttackInputService.TICK)
         .run(item -> this.inventoryEnvironmentTracker.tick(System.nanoTime() / 1000000L, this.compatRememberInteractionService, this.policy(), this.createTiming()));
   ...
   public InventoryData policy() {
      ...
      Set currentValues = Arrays.stream(((String)this.moduleText.get()).toLowerCase(Locale.ROOT).split("[,\\s]+"))
         .map(String::trim)
