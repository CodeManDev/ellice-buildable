...
public final class TargetsComponent extends ModuleSettingsService {
  ...
  private final ModuleSetting.Bool moduleBool2 =
      this.setting(this.moduleNameService, new ModuleSetting.Bool("Dropped Items", true));
  private final ModuleSetting.Bool moduleBool3 =
      this.setting(
          this.moduleNameService,
          new ModuleSetting.Bool("Important Items Only", true)
              .description(
                  "Rare, enchanted, renamed, diamond/netherite, combat and endgame items."));
  private final ModuleSetting.Bool moduleBool4 =
      this.setting(
          this.moduleNameService,
          new ModuleSetting.Bool("Named Entities", true)
              .description("Includes named pets and other entities with a custom name."));
  ...
  private final ModuleSetting.Bool moduleBool5 =
      this.setting(this.moduleNameService, new ModuleSetting.Bool("Self in Third Person", false));
  private final ModuleSetting.Bool moduleBool6 =
      this.setting(this.moduleNameService2, new ModuleSetting.Bool("Health Bar", false));
  private final ModuleSetting.Bool moduleBool7 =
      this.setting(this.moduleNameService2, new ModuleSetting.Bool("Distance", false));
  private final ModuleSetting.Bool moduleBool8 =
      this.setting(
          this.moduleNameService2,
          new ModuleSetting.Bool("Animations", true)
              .description("A short fade-in and smooth health changes; no looping or bouncing."));
  private final ModuleSetting.Number moduleNumber =
      this.setting(
          this.moduleNameService2, new ModuleSetting.Number("Scale", 1.0F, 0.65F, 1.6F, 0.05F));
  private final ModuleSetting.Number moduleNumber2 =
      this.setting(
          this.moduleNameService3, new ModuleSetting.Number("Range", 64.0F, 8.0F, 160.0F, 4.0F));
  private final ModuleSetting.Number moduleNumber3 =
      this.setting(
          this.moduleNameService3, new ModuleSetting.Number("Max Tags", 24.0F, 4.0F, 64.0F, 1.0F));
  private final ModuleSetting.Bool moduleBool9 =
      this.setting(
          this.moduleNameService3,
          new ModuleSetting.Bool("Avoid Overlap", true)
              .description(
                  "Prioritizes players, then items and named entities when labels overlap."));
  private final ModuleSetting.Bool moduleBool10 =
      this.setting(this.moduleNameService3, new ModuleSetting.Bool("Through Walls", false));
  ...
  private void updateState2(EventAttackInputService.WorldRender worldRender) {
    if (!this.checkCondition()) {
      ...
      for (NametagTargetCapture.Target target : this.items2) {
        ...
        NametagsProjectService.Point point =
            NametagsProjectService.project(
                vec3.x,
                vec3.y,
                vec3.z,
        ...
        if (point != null
            && ((Boolean) this.moduleBool10.get()
                || NametagTargetCapture.visible(target, worldRender.cameraPos()))) {
          NametagsData nametagsData =
              NametagTargetCapture.describe(
                  target, (Boolean) this.moduleBool6.get(), (Boolean) this.moduleBool7.get());
          float nextWidth =
              NametagsRenderer.width(
                  compositorPushPresentationScale, nametagsData, value, currentWidth - 8.0F);
          NametagsProjectService.Bounds nextHeight =
              NametagsProjectService.bounds(
                  point,
                  nextWidth,
                  NametagsRenderer.height(nametagsData, value),
                  currentWidth,
                  currentHeight);
          if (nextHeight != null
              && (!(Boolean) this.moduleBool9.get()
                  || !arrayList.stream()
                      .anyMatch(item -> item.overlaps(nextHeight, 3.0F * value)))) {
            UUID uUID = target.entity().getUUID();
            TargetsComponent.Tag tag = this.entries.get(uUID);
            if (tag == null) {
              NametagsRenderer nametagsRenderer =
                  new NametagsRenderer(nametagsData, value, (Boolean) this.moduleBool8.get());
              ComponentMountService componentMount =
                  new ComponentMountService()
                      .mount(nametagsRenderer, CoreIsInitializedHandler.get().theme());
              ...
              this.sceneCornerRadiusService.addChild(componentMount);
                ...
                nametagsData, value, (Boolean) this.moduleBool8.get())) {
              tag.host.invalidateComponent();
            }

            tag.host
                .position(nextHeight.x(), nextHeight.y())
                .size(nextHeight.width(), nextHeight.height());
