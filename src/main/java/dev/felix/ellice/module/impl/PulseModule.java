...
public final class PulseModule extends ModuleSettingsService {
  private final ModuleSetting.Number moduleNumber =
      this.setting(
          new ModuleSetting.Number("Hold (ms)", 100.0F, 50.0F, 150.0F, 10.0F)
              .description(
                  "Groups at most three native movement packets. The first packet fixes the"
                      + " deadline."));
  private final ModuleSetting.Number moduleNumber2 =
      this.setting(
          new ModuleSetting.Number("Interval (ms)", 150.0F, 100.0F, 500.0F, 25.0F)
              .description(
                  "Normal transmission between pulses. Attacks, item use and changed input release"
                      + " movement immediately."));
  private final ModuleSetting.Number moduleNumber3 =
      this.setting(
          new ModuleSetting.Number("Max separation", 0.8F, 0.3F, 1.0F, 0.05F)
              .description(
                  "Maximum distance in blocks from the last transmitted position before releasing"
                      + " the window."));
  private final ModuleSetting.Bool moduleBool =
      this.setting(
          new ModuleSetting.Bool("Combat only", true)
              .description(
                  "Activates with a KillAura target or held attack input. Pulse takes priority over"
                      + " TickBase."));
  private final ModuleSetting.Bool moduleBool2 =
      this.setting(
          new ModuleSetting.Bool("Sent position", true)
              .description(
                  "Outlines your last transmitted position while movement is held. This is not a"
                      + " server acknowledgement."));
  private final ModuleSetting.Color moduleColor =
      this.setting(new ModuleSetting.Color("Position color", -996161537));
  ...
  public PulseModule() {
    super(
        ModuleBuilderData.builder("Pulse")
            ...
            .description(
                "Releases short groups of real movement packets between interactions, without"
                    + " changing local tick speed.")
            .build());
  ...
  protected void onEnable() {
    ...
    this.on(EventAttackInputService.TICK)
        ...
        .run(item -> CompatStatusTracker.tick());
    ...
    this.on(EventAttackInputService.PLAYER_POSITION_CORRECTED)
        .run(item -> CompatStatusTracker.corrected());
                        ...
                        sentBox -> {
                          ...
                          this.renderer.render(
                              ...
                              new BoxOverlayRenderer.Style(
                                  currentValue, value, 0.008F, true, true, false));
                        });
              }
            });
  }
