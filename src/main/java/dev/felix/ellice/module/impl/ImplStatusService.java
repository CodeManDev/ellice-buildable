...
public final class ImplStatusService extends ModuleSettingsService {
  private final ModuleSetting.Bool moduleBool =
      this.setting(
          new ModuleSetting.Bool("Lava only", false)
              .description(
                  "When off, also extinguishes fire after leaving lava or taking fire damage."));
  private final ModuleSetting.Bool moduleBool2 =
      this.setting(
          new ModuleSetting.Bool("Use powder snow", true)
              .description(
                  "Uses a powder-snow bucket if water cannot be placed, including in the Nether."
                      + " Fire can consume the snow."));
  private final ModuleSetting.Bool moduleBool3 =
      this.setting(
          new ModuleSetting.Bool("Recover bucket", true)
              .description(
                  "Picks up only the placed source after you stop burning and nearby lava no longer"
                      + " needs the water."));
  private final ModuleSetting.Number moduleNumber =
      this.setting(new ModuleSetting.Number("Retry interval (ms)", 500.0F, 250.0F, 3000.0F, 50.0F));
  ...
  private final CompatReleaseTracker values =
      new CompatReleaseTracker(CompatReleaseTracker.Purpose.EXTINGUISH);
  ...
  public ImplStatusService() {
    super(
        ModuleBuilderData.builder("AutoExtinguish")
            ...
            .description(
                "Automatically puts out fire with a hotbar/offhand bucket, then restores your slot"
                    + " and view.")
            .build());
  }
  ...
  private BucketStageService.Policy createPolicy() {
    return new BucketStageService.Policy(
        ...
        ((Float) this.moduleNumber.get()).longValue(),
