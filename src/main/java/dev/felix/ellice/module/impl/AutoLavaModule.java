...
public final class AutoLavaModule extends ModuleSettingsService {
  ...
  private final ModuleSetting.Text moduleText =
      this.setting(
          new ModuleSetting.Text("Excluded players", "", 256)
              .description(
                  "Friend names or UUIDs separated by commas. Scoreboard teammates are always"
                      + " excluded."));
  private final ModuleSetting.Number moduleNumber =
      this.setting(
          new ModuleSetting.Number("Range", 4.0F, 2.0F, 4.5F, 0.1F)
              .description(
                  "Also limited by Vanilla block reach. Tracks moving opponents and places below"
                      + " short jumps when ground is reachable."));
  private final ModuleSetting.Bool moduleBool3 =
      this.setting(
          new ModuleSetting.Bool("Recover lava", true)
              .description(
                  "Tries to recover only its own lava source; stops if the source or bucket has"
                      + " changed."));
  private final ModuleSetting.Number moduleNumber2 =
      this.setting(new ModuleSetting.Number("Lava duration (ms)", 500.0F, 250.0F, 2000.0F, 50.0F));
  private final ModuleSetting.Number moduleNumber3 =
      this.setting(
          new ModuleSetting.Number("Placement interval (ms)", 1500.0F, 500.0F, 6000.0F, 100.0F));
  private final ModuleSetting.Number moduleNumber4 =
      this.setting(new ModuleSetting.Number("Server wait (ms)", 2000.0F, 500.0F, 6000.0F, 100.0F));
  private final BucketStageService bucketStageService = new BucketStageService();
  private final CompatReleaseTracker values =
      new CompatReleaseTracker(CompatReleaseTracker.Purpose.LAVA);
  ...
  public AutoLavaModule() {
    super(
        ModuleBuilderData.builder("AutoLava")
            ...
            .description(
                "Places a hotbar/offhand lava bucket at an opponent's feet when reach, terrain and"
                    + " self-distance allow it.")
            .build());
  ...
  protected void onEnable() {
    ...
    this.on(EventAttackInputService.TICK)
        .run(
            item ->
                this.bucketStageService.watchdog(
                    calculateValue(), this.values, this.createPolicy()));
    ...
    this.on(EventAttackInputService.POST_MOVEMENT_PACKET)
        .run(
            item ->
                this.bucketStageService.afterMovement(
                    calculateValue(), this.values, this.createPolicy()));
    ...
    this.on(EventAttackInputService.PLAYER_POSITION_CORRECTED)
        .run(item -> this.bucketStageService.stop(this.values));
  }
