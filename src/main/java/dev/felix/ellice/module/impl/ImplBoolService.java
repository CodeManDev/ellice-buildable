...
public final class ImplBoolService extends ModuleSettingsService {
   public final ModuleEntriesService mode = this.setting(
      new ModuleEntriesService("Mode", SpeedDefinitionService.ALL.stream().map(SpeedDefinitionService.Definition::entry).toList(), "nextgen/LegitHop")
   );
   public final ModuleSetting.Bool autoSprint = this.setting(
      new ModuleSetting.Bool("Auto sprint", true).description("Sprint while moving. Legacy Legit controls forward sprint itself.")
   );
   public final ModuleSetting.Bool pauseScaffold = this.setting(
      new ModuleSetting.Bool("Pause with scaffold", true).description("Pause Speed while ScaffoldWalk controls movement.")
   );
   ...
   public ImplBoolService() {
      super(
         ModuleBuilderData.builder("Speed")
            ...
            .description(
               SpeedDefinitionService.ALL.size() + " movement modes from LiquidBounce and six historical clients, organized by client and family."
            )
            .build()
      );
      this.mode.description("Browse modes by client, edition, group and name. Each mode keeps its own settings.");
      ...
      for (SpeedDefinitionService.Definition definition : SpeedDefinitionService.ALL) {
         ...
         for (SpeedDefinitionService.Option option : definition.options()) {
            ...
            if (option.value() instanceof Boolean enabled) {
               ...
               moduleSetting = new ModuleSetting.Number(currentText, doubleValue.floatValue(), (float)option.min(), (float)option.max(), (float)option.step());
            } else {
               moduleSetting = new ModuleSetting.Mode(currentText, option.choices().toArray(String[]::new), (String)option.value());
            }

            moduleSetting.label(option.key());
            moduleSetting.visibleWhen(this.mode, definition.id()::equals);
            moduleSetting.description(option.key() + " for " + definition.edition() + " / " + definition.name() + ".");
      ...
      this.updateState("nextgen/Custom", "Horizontal modification", "Horizontal acceleration", "Jump off multiplier", "Boost delay");
      this.updateState("nextgen/Custom", "Vertical modification", "Jump height", "Pull down", "Fall pull down");
      this.updateState("nextgen/Custom", "Strafe", "Strafe strength", "Custom speed", "Speed", "Velocity timeout", "Strafe knockback");
   ...
   private void updateState(String currentText, String nextText, String... strings) {
      ...
      for (String previousText : strings) {
         ((ModuleSetting)entries.get(previousText)).visibleWhen((ModuleSetting.Bool)entries.get(nextText));
      }
   }
   ...
   protected void onEnable() {
      ...
      this.on(EventAttackInputService.PRE_MOVEMENT_PACKET).run(item -> SpeedActivateService.motion(false));
      this.on(EventAttackInputService.POST_MOVEMENT_PACKET).run(item -> SpeedActivateService.motion(true));
      this.on(EventAttackInputService.PACKET).priority(EventIsAfterHandler.Priority.LAST).run(SpeedActivateService::packet);
   }
