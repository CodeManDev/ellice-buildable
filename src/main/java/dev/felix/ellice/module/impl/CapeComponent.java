...
public final class CapeComponent extends ModuleSettingsService {
   private final ModuleNameService moduleNameService = this.settingCategory("Cape").description("Controls ellice's procedural cape renderer.");
   private final ModuleNameService moduleNameService2 = this.settingCategory(this.moduleNameService, "Visibility")
      .description("Chooses which players and camera views render capes.");
   private final ModuleNameService moduleNameService3 = this.settingCategory(this.moduleNameService, "Appearance")
      .description("Controls cape material, colors, transparency, and depth behavior.");
   private final ModuleNameService moduleNameService4 = this.settingCategory(this.moduleNameService, "Geometry")
   ...
   private final ModuleSetting.Bool moduleBool = this.setting(
      this.moduleNameService, new ModuleSetting.Bool("Capes", true).description("Master switch for ellice's procedural cape renderer.")
   );
   private final ModuleSetting.Bool moduleBool2 = this.setting(
      this.moduleNameService2, new ModuleSetting.Bool("Self Cape", true).description("Renders the procedural cape on your own player model.")
   );
   private final ModuleSetting.Bool moduleBool3 = this.setting(
      ...
      new ModuleSetting.Bool("Other Players", false)
         .description("Also renders procedural capes on visible, non-spectating players within Render Range.")
   );
   private final ModuleSetting.Bool moduleBool4 = this.setting(
      this.moduleNameService2,
      new ModuleSetting.Bool("First Person", false)
         .description("Keeps your own cape visible in first person; it may enter the camera view while moving.")
   );
   private final ModuleSetting.Bool moduleBool5 = this.setting(
      this.moduleNameService3,
      new ModuleSetting.Bool("See Through", false)
         .description("Disables depth testing so rendered capes remain visible through players and terrain.")
   );
   private final ModuleSetting.Mode moduleMode = this.setting(
      this.moduleNameService3,
      new ModuleSetting.Mode("Material", new String[]{"ellice", "Aurora", "Hologram"}, "ellice")
         .description("Selects the cape shader pattern: crystalline ellice, flowing Aurora, or scanlined Hologram.")
   );
   ...
   private final ModuleSetting.Number moduleNumber3 = this.setting(
      ...
      new ModuleSetting.Number("Motion Response", 0.7F, 0.0F, 2.0F, 0.05F)
         .description("Controls how strongly the cloth trails player movement; 0 stays rigid and high values swing farther.")
   );
   private final ModuleSetting.Number moduleNumber4 = this.setting(
      ...
      new ModuleSetting.Number("Shader Energy", 0.45F, 0.0F, 2.0F, 0.05F)
         .description("Controls procedural cloth ripples and material animation; higher values move faster and farther.")
   );
   private final ModuleSetting.Number moduleNumber5 = this.setting(
      this.moduleNameService3,
      new ModuleSetting.Number(
            "Opacity", 0.82F, 0.1F, 1.0F, 0.01F
         )
         .description("Sets overall cape transparency before the individual color alpha values are applied.")
   );
   private final ModuleSetting.Number moduleNumber6 = this.setting(
      this.moduleNameService4,
      new ModuleSetting.Number(
            "Render Range", 64.0F, 8.0F, 160.0F, 1.0F
         )
         .description("Limits cape rendering to players within this many blocks; shorter ranges reduce geometry work.")
   );
   private final ModuleSetting.Color moduleColor = this.setting(
      this.moduleNameService3, new ModuleSetting.Color("Top Color", -429332225).description("Sets the tint and alpha at the cape's shoulder end.")
   );
   private final ModuleSetting.Color moduleColor2 = this.setting(
      this.moduleNameService3,
      new ModuleSetting.Color("Bottom Color", -661955338)
         .description("Sets the tint and alpha at the cape's lower end, forming a vertical gradient.")
   );
   private final ModuleSetting.Color moduleColor3 = this.setting(
      this.moduleNameService3,
      new ModuleSetting.Color("Edge Color", -117440513)
         .description("Sets the accent tint and alpha along the cape sides and material highlights.")
   );
   ...
   private List<Player> collectValues(Minecraft minecraft) {
      ...
      for (Player currentPlayer : minecraft.level.players()) {
         ...
         if ((
               value != 0
                  ? !CapeEnabledService.active() && (Boolean)this.moduleBool2.get() && (!firstPerson || (Boolean)this.moduleBool4.get())
