...
public final class FlowPathModule extends ModuleSettingsService {
  private final ModuleNameService moduleNameService =
      this.settingCategory("Autonomy")
          .description(
              "One task at a time. Stone age gathers the missing materials and builds a stone"
                  + " pickaxe.");
  private final ModuleSetting.Mode moduleMode =
      this.setting(
          ...
          new ModuleSetting.Mode(
                  "Task",
                  new String[] {"Off", "Mine diamonds", "Chop trees", "Stone age", "Survive"},
                  "Off")
              .description(
                  "Mine diamonds needs an iron pickaxe or better. Chop trees replants saplings on"
                      + " soil. Survive runs the stone age under the survival director."));
  private final ModuleSetting.Number moduleNumber =
      this.setting(
          this.moduleNameService,
          new ModuleSetting.Number("Target count", 2.0F, 1.0F, 64.0F, 1.0F)
              .description(
                  "Diamonds or logs to gather before the task reports done. Stone age derives"
                      + " material needs from recipes and current inventory."));
  private final ModuleSetting.Bool moduleBool =
      this.setting(
          this.moduleNameService,
          new ModuleSetting.Bool("Status display", true)
              .description("Shows the current task, stage and abort reason below the crosshair."));
  ...
  public FlowPathModule() {
    super(
        ModuleBuilderData.builder("FlowPath")
            ...
            .description(
                "Autonomous mining, woodcutting and tool crafting: scans loaded ground, walks"
                    + " planned routes, mines and crafts vanilla blocks.")
            .build());
    this.moduleNumber.visibleWhen(
        this.moduleMode,
        item -> !item.equals("Off") && !item.equals("Stone age") && !item.equals("Survive"));
  }
  ...
  public String status() {
    if (this.taskOperationHandler3 == null) {
      ...
      return this.text2.isEmpty()
          ? this.taskOperationHandler3.status()
          : this.taskOperationHandler3.status() + " · " + this.text2;
    }
  }
  ...
  private void updateState2() {
    String currentText =
        (String) this.moduleMode.get() + "/" + ((Float) this.moduleNumber.get()).intValue();
    ...
    if (((String) this.moduleMode.get()).equals("Off")) {
      ...
      if (result.isEmpty()) {
        ...
        if (this.taskOperationHandler3 == null || !this.text.equals(currentText)) {
          ...
          this.taskOperationHandler3 =
              switch ((String) this.moduleMode.get()) {
                case "Mine diamonds" ->
                    new TaskConfigTracker(
                        TaskConfigTracker.Config.diamonds(
                            ((Float) this.moduleNumber.get()).intValue()));
                case "Chop trees" ->
                    new TreeHarvestTask(
                        TreeHarvestTask.Config.overworld(
                            ((Float) this.moduleNumber.get()).intValue()));
                case "Stone age" -> MissionTask.stoneAge();
                case "Survive" ->
                    new SurvivalTask(SurvivalTask.Config.standard(), MissionTask.stoneAge());
                default -> null;
              };
          this.text = currentText;
          if (this.taskOperationHandler3 == null) {
          ...
          CoreIsInitializedHandler.LOGGER.info(
              "[FlowPath] starting {}", this.taskOperationHandler3.name());
        }

        TaskData taskData = this.compatCurrentService.sense();
        TaskAction taskAction =
            this.compatStateController
                .pendingCommand()
                .map(item -> (TaskAction) item)
                .orElseGet(
                    () ->
                        this.taskOperationHandler3.tick(
                            System.currentTimeMillis(), this.compatCurrentService));
        if (!this.enabled2) {
          this.enabled2 = true;
          CoreIsInitializedHandler.LOGGER.info(
              "[FlowPath] first command: {}", createText2(taskAction));
        }
        ...
        if (taskAction instanceof TaskAction.Craft craft) {
          ...
          this.compatStateController2.closeIfOwned();
          this.text2 =
              this.compatExecuteService.execute(
                  taskAction, taskData, this.compatCurrentService.traversal());
        }
  ...
  private void updateState3(String text) {
    if (!text.equals(this.text5)) {
      ...
      CoreIsInitializedHandler.LOGGER.warn(
          "[FlowPath] waiting: {}",
          text.equals("sense-blocked")
              ? "no gameplay sense (world or player state)"
              : "no terrain adapter for this version");
    }
  }
  ...
  private static String createText2(TaskAction taskAction) {
    if (taskAction instanceof TaskAction.Goto gotoAction) {
      ...
      return taskAction instanceof TaskAction.Abort abort
          ? "Abort " + abort.reason()
          : String.valueOf(taskAction);
    }
  }
