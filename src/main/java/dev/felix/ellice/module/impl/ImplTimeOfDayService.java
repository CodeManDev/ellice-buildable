...
public final class ImplTimeOfDayService extends ModuleSettingsService {
  public final ModuleSetting.Mode mode =
      this.setting(
          new ModuleSetting.Mode(
              "Time", new String[] {"Day", "Sunset", "Night", "Midnight", "Custom"}, "Day"));
  public final ModuleSetting.Number custom =
      this.setting(new ModuleSetting.Number("Custom time", 6000.0F, 0.0F, 23999.0F, 1.0F));
  ...
  public static long displayedTime(long longValue) {
    return implTimeOfDayService == null
        ? longValue
        : longValue - Math.floorMod(longValue, 24000L) + implTimeOfDayService.timeOfDay();
  }
}
