...
public final class PlayerlookupApiService extends ModuleSettingsService
    implements ModuleOperationHandler {
  private static final PlayerlookupLooksValidValidator playerlookupLooksValidValidator =
      new PlayerlookupLooksValidValidator();
  private final ModuleSetting.Bool moduleBool =
      this.setting(
          new ModuleSetting.Bool("Auto-copy UUID", false)
              .description(
                  "Copies the dashed UUID to your system clipboard after each successful player"
                      + " lookup."));
  private final ModuleSetting.Bool moduleBool2 =
      this.setting(
          new ModuleSetting.Bool("Remember last query", true)
              .description(
                  "Keeps the latest valid query for this session and runs it again when the lookup"
                      + " screen reopens."));
  private String text = "";
  ...
  public PlayerlookupApiService() {
    super(
        ModuleBuilderData.builder("Player Lookup")
            .description(
                "Looks up a Minecraft profile by name or UUID, including skin and cape data.")
