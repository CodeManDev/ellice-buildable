...
public final class ImplHotbarLayoutService extends ModuleSettingsService {
   private final ModuleSetting.Bool moduleBool = this.setting(
      new ModuleSetting.Bool("AutoArmor", true)
         .description(
            "Equips better armor while your inventory is open. Compares protection, toughness, enchantments and durability; replaced armor is kept unless duplicate cleanup is enabled."
         )
   );
   private final ModuleSetting.Bool moduleBool2 = this.setting(
      new ModuleSetting.Bool("Organize hotbar", true)
         .description("Places the best matching items into your chosen hotbar slots while your inventory is open.")
   );
   private final ModuleSetting.Bool moduleBool3 = this.setting(
      new ModuleSetting.Bool("Drop junk", true)
         .description(
            "Throws out stacks of rotten flesh, poisonous potatoes and dead bushes while your inventory is open. Other materials and supplies are kept."
         )
   );
   private final ModuleSetting.Bool moduleBool4 = this.setting(
      new ModuleSetting.Bool("Drop inferior equipment", true)
         .description(
            "Throws out worse or equal duplicate equipment after equipping and sorting. Keeps the best usable item per type, Silk Touch and Riptide variants, equipped items and matching hotbar slots. Locked slots are never dropped."
         )
   );
   private final ModuleLayoutService moduleLayoutService = this.setting(
      new ModuleLayoutService("Hotbar layout", InventoryRoleData.BALANCED)
         .description(
            "Design all nine hotbar slots in the visual editor. Locked slots are never changed or used as a source. Missing roles leave their slots alone."
         )
   );
   private final ModuleSetting.Bool moduleBool5 = this.setting(
      new ModuleSetting.Bool("Keep equipped elytra", true)
         .description("Preserves an equipped elytra instead of replacing it with a chestplate.")
   );
   private final ModuleSetting.Bool moduleBool6 = this.setting(
      new ModuleSetting.Bool("Skip cursed equipment", true)
         .description("Avoids gear with Binding or Vanishing. Equipped Curse of Binding armor is always left in place.")
   );
   private final ModuleSetting.Number moduleNumber = this.setting(
      new ModuleSetting.Number("Minimum durability (%)", 10.0F, 0.0F, 100.0F, 1.0F)
         .description("Rejects almost-broken replacements and hotbar tools below this remaining durability.")
   );
   private final ModuleSetting.Range moduleRange = this.setting(
      new ModuleSetting.Range(
         ...
         .description(
            "Delay between individual native inventory actions. Mouse input, cursor items, closing the inventory and container changes cancel queued work."
         )
   );
   private final InventoryActionTracker inventoryEnvironmentTracker2 = new InventoryActionTracker(new Random());
   ...
   public ImplHotbarLayoutService() {
      super(
         ModuleBuilderData.builder("InventoryManager")
            ...
            .description("Equips better armor, organizes your hotbar and drops junk and inferior equipment while your inventory is open.")
            .build()
      );
   ...
   protected void onEnable() {
      ...
      this.on(EventAttackInputService.TICK)
         .run(
            item -> this.inventoryEnvironmentTracker2
               .tick(
