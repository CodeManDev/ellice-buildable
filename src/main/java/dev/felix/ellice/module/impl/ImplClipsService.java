...
public final class ImplClipsService extends ModuleSettingsService
  ...
  private final ModuleSetting.Keybind moduleKeybind =
      this.setting(
          ...
          new ModuleSetting.Keybind("Record Clip", 297)
              .description("Start a 3D recording; press again to finish early."));
  private final ModuleSetting.Keybind moduleKeybind2 =
      this.setting(
          this.moduleNameService,
          new ModuleSetting.Keybind("Open Replay Editor", 296)
              .description("Open the cinematic timeline and export workspace."));
  private final ModuleSetting.Number moduleNumber =
      this.setting(
          this.moduleNameService,
          new ModuleSetting.Number("Duration (seconds)", 20.0F, 5.0F, 60.0F, 1.0F)
  ...
  private final ModuleSetting.Mode moduleMode =
      this.setting(
          ...
          new ModuleSetting.Mode(
                  "Clip Resolution", new String[] {"1080p", "1440p", "2160p"}, "1440p")
              .description("Native video resolution; higher values take longer to export."));
  private final ModuleSetting.Number moduleNumber4 =
      this.setting(
          this.moduleNameService2,
          new ModuleSetting.Number("Clip Bitrate (Mbps)", 200.0F, 25.0F, 400.0F, 5.0F)
              .description("Video bitrate target. Higher values produce larger MP4 files."));
  private final ModuleSetting.Keybind moduleKeybind3 =
      this.setting(
          this.moduleNameService2,
          new ModuleSetting.Keybind("Render Clip", 298)
              .description("Export the edited take to an MP4 in ellice/clips."));
  private final ModuleSetting.Keybind moduleKeybind4 =
      this.setting(
          this.moduleNameService2,
          new ModuleSetting.Keybind("Discard Clip", -1)
              .description("Discard the take or cancel its export."));
        ...
        new CinematicExporter(
            ...
            item ->
                CoreIsInitializedHandler.get()
                    .scene()
                    .toasts()
                    .show("Cinematic Replay", item, 6.0F));
  ...
  private static boolean checkCondition(ModuleSetting.Keybind keybind, Minecraft minecraft) {
    return keybind.isBound()
        && GLFW.glfwGetKey(CompatAdapterService.windowHandle(minecraft), (Integer) keybind.get())
            == 1;
  }
  ...
  private void updateState2() {
    ...
    if (this.renderer.recording() && currentWorld != null && currentWorld.player() != null) {
      ...
      if (offset - this.timestamp2 > 250000000L) {
        this.items =
            this.compatLoadedHandler.sections(currentWorld.player().x, currentWorld.player().z, 6);
        this.timestamp2 = offset;
      }

      this.terrainStateController.update(
          this.items, currentWorld.player().x, currentWorld.player().z);
