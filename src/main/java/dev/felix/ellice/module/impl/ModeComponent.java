...
public final class ModeComponent extends ModuleSettingsService {
  private final ModuleSetting.Mode moduleMode =
      this.setting(
          new ModuleSetting.Mode("Mode", new String[] {"Normal", "Silent"}, "Normal")
              .description(
                  "Silent keeps your visible hotbar slot and held item while the server uses the"
                      + " mining tool."));
  private final ModuleSetting.Bool moduleBool =
      this.setting(
          new ModuleSetting.Bool("Restore slot", true)
              .description(
                  "Returns to your previous slot after mining. Manual slot changes always take"
                      + " priority."));
  private final ModuleSetting.Bool moduleBool2 =
      this.setting(
          new ModuleSetting.Bool("Protect tools", true)
              .description("Avoids selecting tools at or below the durability reserve."));
  ...
  public ModeComponent() {
    super(
        ModuleBuilderData.builder("AutoTool")
            ...
            .description(
                "Uses the best hotbar tool for each block, with optional silent switching and"
  ...
  protected void onEnable() {
    ...
    this.on(EventAttackInputService.TICK)
        ...
        .run(item -> CompatConfigureService.watchdog());
    ...
    this.on(EventAttackInputService.PLAYER_POSITION_CORRECTED)
        .run(item -> CompatConfigureService.cancel());
  }
