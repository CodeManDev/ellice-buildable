...
public final class FullbrightModule extends ModuleSettingsService {
   private final ModuleSetting.Number moduleNumber = this.setting(
      new ModuleSetting.Number("Strength", 1.0F, 0.0F, 1.0F, 0.01F)
         .description("Sets the minimum client lightmap brightness; 0 preserves vanilla lighting and 1 is fully bright.")
   );
   private final ModuleSetting.Bool moduleBool = this.setting(
      new ModuleSetting.Bool("Reduce Darkness", true)
         .description("Also suppresses Darkness-effect and boss-overlay dimming in proportion to Strength.")
   );
