...
public final class BacktrackModule extends ModuleSettingsService {
   private final ModuleNameService moduleNameService = this.settingCategory("Tracking")
      .description("Chooses the opponent and the distance at which a delay can help.");
   private final ModuleNameService moduleNameService2 = this.settingCategory("Timing")
      .description("Bounds the delay and releases updates when the target closes the distance.");
   private final ModuleNameService moduleNameService3 = this.settingCategory("Visual")
   ...
   private final ModuleSetting.Mode moduleMode = this.setting(
      ...
      new ModuleSetting.Mode("Target Mode", new String[]{"Attack", "KillAura", "Nearest"}, "Attack")
         .description("Attack follows your last actual attack; KillAura follows its selected target; Nearest works independently.")
   );
   private final ModuleSetting.Bool moduleBool = this.setting(this.moduleNameService, new ModuleSetting.Bool("Players", true));
   private final ModuleSetting.Bool moduleBool2 = this.setting(this.moduleNameService, new ModuleSetting.Bool("Mobs", false));
   private final ModuleSetting.Range moduleRange = this.setting(
      this.moduleNameService,
      new ModuleSetting.Range(
            "Range", 1.0F, 3.0F, 0.0F, 6.0F, 0.1F
         )
         .description("Distance to the displayed hitbox where a backtrack window may remain active.")
   );
   private final ModuleSetting.Number moduleNumber = this.setting(
      ...
      new ModuleSetting.Number(
         ...
         .description("How long an actually attacked target remains selected in Attack mode.")
   );
   private final ModuleSetting.Range moduleRange2 = this.setting(
      ...
      new ModuleSetting.Range(
         ...
         .description("Samples one delay for a packet window. Oldest packets always release first.")
   );
   ...
   private final ModuleSetting.Bool moduleBool3 = this.setting(
      ...
      new ModuleSetting.Bool("Only Retreating", true)
         .description("Starts only when received movement takes the opponent farther away; approaching movement releases the window.")
   );
   private final ModuleSetting.Bool moduleBool4 = this.setting(
      this.moduleNameService2,
      new ModuleSetting.Bool("Pause When Hurt", true)
         .description("Releases queued updates immediately when you take damage and pauses while your hurt animation is active.")
   );
   private final ModuleSetting.Bool moduleBool5 = this.setting(
      ...
      new ModuleSetting.Bool("Received Position", true)
         .description("Outlines the latest received position. A bright filled box means movement is currently delayed.")
   );
   private final ModuleSetting.Bool moduleBool6 = this.setting(
      this.moduleNameService3,
      new ModuleSetting.Bool("Show Tracked Target", true)
         .description("Keeps a faint unfilled outline on the tracked opponent between active delay windows.")
   );
   private final ModuleSetting.Color moduleColor = this.setting(this.moduleNameService3, new ModuleSetting.Color("Position Color", -863187981));
   private final ModuleSetting.Bool moduleBool7 = this.setting(this.moduleNameService3, new ModuleSetting.Bool("Fill", true));
   private final ModuleSetting.Bool moduleBool8 = this.setting(
      this.moduleNameService3,
      new ModuleSetting.Bool("See Through", false)
         .description("Keeps the received-position box visible through entities and terrain if the delayed opponent covers it.")
   );
   ...
   public BacktrackModule() {
      super(
         ModuleBuilderData.builder("Backtrack")
            ...
            .description("Briefly delays the selected opponent's movement while tracking its received position.")
            .build()
      );
   ...
   protected void onEnable() {
      ...
      this.on(EventAttackInputService.TICK).priority(EventIsAfterHandler.Priority.FIRST).run(item -> CompatEnableHandler.tick());
      this.on(EventAttackInputService.OUTGOING_PACKET_ACCEPTED).run(item -> CompatEnableHandler.attackPacket(item.packet()));
      this.on(EventAttackInputService.WORLD).run(item -> CompatEnableHandler.worldChanged());
      this.on(EventAttackInputService.PLAYER_POSITION_CORRECTED).run(item -> CompatEnableHandler.reset());
