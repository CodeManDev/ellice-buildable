...
public final class ColorComponent extends ModuleSettingsService {
   private final ModuleSetting.Color moduleColor = this.setting(
      new ModuleSetting.Color("Color", -50312).description("Sets the heart's base, emissive, and glow hue; the selected alpha is ignored.")
   );
   private final ModuleSetting.Number moduleNumber = this.setting(
      new ModuleSetting.Number(
         ...
         .description("Sets the heart's base world-space scale before its subtle pulse is applied.")
   );
   ...
   private final ModuleSetting.Number moduleNumber3 = this.setting(
      new ModuleSetting.Number("Metallic", 0.58F, 0.0F, 1.0F, 0.01F)
         .description("Controls the PBR metal response; 0 behaves like a dielectric and 1 like polished metal.")
   );
   private final ModuleSetting.Number moduleNumber4 = this.setting(
      new ModuleSetting.Number(
            "Roughness", 0.2F, 0.045F, 1.0F, 0.01F
         )
         .description("Controls reflection spread; low values look glossy and sharp, while high values look matte.")
   );
   private final ModuleSetting.Number moduleNumber5 = this.setting(
      new ModuleSetting.Number(
            "Emission", 1.15F, 0.0F, 4.0F, 0.05F
         )
         .description("Sets the heart surface's self-lit intensity independently of the surrounding glow.")
   );
   private final ModuleSetting.Number moduleNumber6 = this.setting(
      new ModuleSetting.Number(
            "Glow Strength", 2.4F, 0.0F, 12.0F, 0.1F
         )
         .description("Controls the intensity of the depth-aware bloom around the heart; 0 disables visible glow.")
   );
   private final ModuleSetting.Number moduleNumber7 = this.setting(
      new ModuleSetting.Number("Blur Radius", 30.0F, 0.0F, 96.0F, 1.0F)
         .description("Sets how widely the heart's glow is blurred in screen pixels; larger radii cost more GPU time.")
   );
   private final ModuleSetting.Number moduleNumber8 = this.setting(
      new ModuleSetting.Number(
            "Pulse Speed", 1.25F, 0.0F, 4.0F, 0.05F
         )
         .description("Sets size-pulse cycles per second; 0 holds the heart at its base size.")
   );
      ...
      if (minecraft.player != null && minecraft.level != null) {
         ...
         if (render3dScene != null) {
            ...
            if (worldRender.camera() != null) {
               ...
               vector3f = new Vector3f((float)(-Math.sin(doubleValue) * nextDoubleValue), (float)(-Math.sin(currentDoubleValue)), (float)(Math.cos(doubleValue) * nextDoubleValue));
            ...
            double targetDoubleValue = this.timestamp == 0L
               ? 0.0
               : Math.min(
                  (offset - this.timestamp) / 1.0E9, 0.25
               );
            ...
            Vector3f previousVector3f = new Vector3f(currentVector3f).sub(new Vector3f(nextVector3f).mul(currentVector3f.dot(nextVector3f))).normalize();
            ...
            Matrix3f matrix3f = new Matrix3f().setColumn(0, sourceVector3f).setColumn(1, previousVector3f).setColumn(2, nextVector3f);
            float currentValue = (float)Math.sin(sourceDoubleValue * 0.82) * 0.045F;
            Quaternionf quaternionf = new Quaternionf().setFromNormalized(matrix3f).rotateZ(currentValue);
            float nextValue = this.moduleNumber8.get() <= 0.0F
               ? 1.0F
               : 1.0F + 0.065F * (float)Math.sin(this.value);
            float previousValue = (Float)this.moduleNumber.get() * nextValue;
            this.renderer2
               .transform(
                  new GeometryXService(vec3.x + vector3f.x * previousDoubleValue, vec3.y + vector3f.y * previousDoubleValue, vec3.z + vector3f.z * previousDoubleValue, quaternionf, new Vector3f(previousValue))
               );
            ...
            this.materialErFactory
               ...
               .uniform("uEmissive", calculateValue(sourceValue), calculateValue2(sourceValue), calculateValue3(sourceValue))
