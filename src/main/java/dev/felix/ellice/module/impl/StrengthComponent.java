...
public final class StrengthComponent extends ModuleSettingsService {
  private final ModuleSetting.Number moduleNumber =
      this.setting(
          new ModuleSetting.Number("Strength", 0.55F, 0.0F, 1.0F, 0.01F)
              .description(
                  "Controls how much frame history is blended into the current image; 0 disables"
                      + " visible blur."));
  private final ModuleSetting.Number moduleNumber2 =
      this.setting(
          new ModuleSetting.Number("Persistence", 0.75F, 0.0F, 1.0F, 0.01F)
              .description(
                  "Controls how long previous frames remain visible; high values produce longer"
                      + " ghost trails."));
  private final ModuleSetting.Mode moduleMode =
      this.setting(
          new ModuleSetting.Mode(
                  "Mode", new String[] {"Smooth", "Smear", "Trail", "Gamma"}, "Smooth")
              .description(
                  "Selects the history blend: even Smooth, heavier Smear, long Trail, or"
                      + " highlight-preserving Gamma."));
  ...
  private void updateState() {
    ...
    if (compositorRenderer != null) {
      compositorRenderer.setMotionBlur(
          this.isEnabled(),
