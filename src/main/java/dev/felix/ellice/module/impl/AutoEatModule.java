...
public final class AutoEatModule extends ModuleSettingsService {
   private final ModuleNameService moduleNameService = this.settingCategory("Consumption")
      .description("Eats the best stored food by saturation. Healing bites enable health regen when critically hurt.");
   private final ModuleNameService moduleNameService2 = this.settingCategory("Inventory & timing")
      .description("Moves stored food into the hotbar with a predicted swap, without opening any screen. Manual input takes priority.");
   private final ModuleSetting.Number moduleNumber = this.setting(
      ...
      new ModuleSetting.Number("Eat below (food)", 14.0F, 0.0F, 19.0F, 1.0F)
         .description("Vanilla food points, from 0 to 20. Eats the highest-saturation food available.")
   );
   private final ModuleSetting.Bool moduleBool = this.setting(
      this.moduleNameService,
      new ModuleSetting.Bool("Eat to heal", true)
         .description("Eats below the critical health threshold even when not hungry, so health can regenerate.")
   );
   private final ModuleSetting.Number moduleNumber2 = this.setting(
      this.moduleNameService,
      new ModuleSetting.Number(
            "Critical below (hearts)",
            5.0F,
            0.5F,
            10.0F,
            0.5F
         )
         .description("Healing bites trigger at or below this health, with room left to regenerate.")
   );
   private final ModuleSetting.Bool moduleBool2 = this.setting(
      ...
      new ModuleSetting.Bool("Refill hotbar", true)
         .description("Swaps the best stored food into the first empty hotbar slot, otherwise the selected one.")
   );
   private final ModuleSetting.Bool moduleBool3 = this.setting(
      this.moduleNameService2,
      new ModuleSetting.Bool("Restore selected slot", true)
         .description("Returns to your previous hotbar selection after eating, unless you changed it yourself.")
   );
   private final ModuleSetting.Number moduleNumber3 = this.setting(
      ...
      new ModuleSetting.Number(
         ...
         .description("Minimum spacing for eating actions. At most one is sent per client tick, including after lag.")
   );
   private final ModuleSetting.Number moduleNumber4 = this.setting(
      ...
      new ModuleSetting.Number(
         ...
         .description("Waits for the server to confirm the bite before stopping. Eating itself gets at least 3 seconds.")
   );
   private final SurvivalEnvironmentTracker survivalEnvironmentTracker = new SurvivalEnvironmentTracker();
   ...
   public AutoEatModule() {
      super(
         ModuleBuilderData.builder("AutoEat")
            ...
            .description("Eats the best food by saturation and refills the hotbar through Vanilla inventory interactions.")
            .build()
      );
   ...
   protected void onEnable() {
      ...
      this.on(EventAttackInputService.TICK).run(item -> this.survivalEnvironmentTracker.tick(System.nanoTime() / 1000000L, this.compatActivateService, this.policy()));
      this.on(EventAttackInputService.WORLD).run(item -> {
         this.survivalEnvironmentTracker.stop(this.compatActivateService, (Boolean)this.moduleBool3.get());
         this.compatActivateService.activate();
      });
   }
   ...
   public String status() {
      ...
      if (survivalItemData != null) {
         SurvivalAssessService.Advice advice = SurvivalAssessService.assess(
