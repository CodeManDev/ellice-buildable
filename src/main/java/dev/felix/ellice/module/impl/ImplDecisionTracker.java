...
public final class ImplDecisionTracker extends ModuleSettingsService {
  ...
  private final ModuleNameService moduleNameService3 =
      this.settingCategory("Shields")
          .description(
              "Recognizes directional blocking and uses an equipped offhand shield during weapon"
                  + " recovery.");
  private final ModuleNameService moduleNameService4 =
      this.settingCategory("Motion")
  ...
  private final ModuleNameService moduleNameService5 =
      this.settingCategory("Combat AI")
          .description(
              "Autonomous positioning that adapts to the opponent, cooldown and health. Manual"
                  + " Takeover keys give movement back.");
  private final ModuleNameService moduleNameService6 =
      this.settingCategory("Humanization")
          .description(
              "Adds reaction delay, overshoot, hesitation, and motor noise to test traces.");
  private final ModuleNameService moduleNameService7 =
      this.settingCategory("Quantization")
  ...
  private final ModuleNameService moduleNameService9 =
      this.settingCategory("Negative Tests")
  ...
  private final ModuleSetting.Bool moduleBool =
      this.setting(
          ...
          new ModuleSetting.Bool("Players", true)
              .description("Allows visible player entities to be selected as targets."));
  private final ModuleSetting.Bool moduleBool2 =
      this.setting(
          this.moduleNameService,
          new ModuleSetting.Bool("Mobs", true)
              .description("Allows visible mobs to be selected; allies are excluded."));
  private final ModuleSetting.Bool moduleBool3 =
      this.setting(
          this.moduleNameService,
          new ModuleSetting.Bool("Invisible", false)
              .description(
                  "Also selects invisible players and mobs. Blatant: humans cannot see them."));
  private final ModuleSetting.Number moduleNumber =
      this.setting(
          this.moduleNameService,
          new ModuleSetting.Number("Range", 4.5F, 1.0F, 12.0F, 0.5F)
  ...
  private final ModuleSetting.Mode moduleMode =
      this.setting(
          ...
          new ModuleSetting.Mode("FOV", new String[] {"180°", "360°", "Custom"}, "180°")
              .description(
                  "Searches the front 180°, the full 360°, or a custom angle around the current"
                      + " server rotation."));
  ...
  private final ModuleSetting.Number moduleNumber3 =
      this.setting(
          ...
          new ModuleSetting.Number("Attack Range", 3.0F, 1.0F, 6.0F, 0.1F)
              .description(
                  "Sets attack reach in blocks, capped by Minecraft's entity interaction range."));
  private final ModuleSetting.Mode moduleMode2 =
      this.setting(
          this.moduleNameService,
          new ModuleSetting.Mode(
                  "Target Priority",
                  new String[] {"Smart", "Nearest", "Lowest Health", "Highest Health", "Crosshair"},
                  "Smart")
              .description(
                  "Smart weighs reach, turn angle and health. Other modes prioritize one factor."));
  private final ModuleSetting.Bool moduleBool4 =
      this.setting(
          this.moduleNameService,
          new ModuleSetting.Bool("Keep Combo Target", true)
              .description(
                  "Stays on a visible target within attack reach; switches when it becomes"
                      + " unreachable."));
  private final ModuleSetting.Number moduleNumber4 =
      this.setting(
          ...
          new ModuleSetting.Number("Switch Delay (ticks)", 0.0F, 0.0F, 20.0F, 1.0F)
              .description(
                  "Waits this many movement ticks after a target change before the first attack."
                      + " Rotation continues."));
  private final ModuleSetting.Mode moduleMode3 =
      this.setting(
          ...
          new ModuleSetting.Mode(
                  "Aim Point",
                  new String[] {"Auto", "Head", "Chest", "Legs", "Center", "Custom"},
                  "Auto")
              .description(
                  "Auto keeps the visible lattice point. Other modes prefer a height on the"
                      + " hitbox."));
  private final ModuleSetting.Number moduleNumber5 =
      this.setting(
          this.moduleNameService10,
          new ModuleSetting.Number("Aim Height", 0.55F, 0.05F, 0.95F, 0.01F)
              .description(
                  "Vertical hitbox fraction used when Aim Point is Custom. 0 is feet, 1 is the top"
                      + " of the box."));
  private final ModuleSetting.Bool moduleBool5 =
      this.setting(
          ...
          new ModuleSetting.Bool("Smart Clicks", true)
              .description(
                  "Waits for the target's hurt animation to end and a valid sent aim; requires two"
                      + " aim ticks when Combo Timing is off."));
  private final ModuleSetting.Bool moduleBool6 =
      this.setting(
          this.moduleNameService2,
          new ModuleSetting.Bool("Combo Timing", true)
              .description(
                  "Uses the live weapon cooldown and first valid aim tick without extra pauses;"
                      + " adapts immediately to weapon swaps."));
  private final ModuleSetting.Bool moduleBool7 =
      this.setting(
          this.moduleNameService2,
          new ModuleSetting.Bool("Sprint Reset", true)
              .description(
                  "Releases sprint for one grounded movement tick after a sprint attack, then"
                      + " permits normal sprint again."));
  private final ModuleSetting.Number moduleNumber6 =
      this.setting(
          this.moduleNameService2,
          new ModuleSetting.Number("Correction Recovery (ticks)", 3.0F, 0.0F, 20.0F, 1.0F)
              .description(
                  "Waits this many complete movement ticks after a server correction before"
                      + " reacquiring a target."));
  private final ModuleSetting.Number moduleNumber7 =
      this.setting(
          this.moduleNameService2,
          new ModuleSetting.Number("Sprint Reset (ticks)", 1.0F, 1.0F, 5.0F, 1.0F)
              .description(
                  "How many grounded movement ticks sprint stays released after a sprint attack."));
  private final ModuleSetting.Range moduleRange =
      this.setting(
          this.moduleNameService2,
          new ModuleSetting.Range("Click Speed (CPS)", 4.0F, 7.0F, 1.0F, 20.0F, 0.5F)
              .description(
                  "Limits click speed; weapon cooldown and Smart Clicks may reduce the actual"
                      + " attack rate."));
  private final ModuleSetting.Bool moduleBool8 =
      this.setting(
          ...
          new ModuleSetting.Bool("Wait for Cooldown", true)
              .description(
                  "Requires a full weapon cooldown before attacking. Off still uses CPS and native"
                      + " reduced-damage swings."));
  private final ModuleSetting.Bool moduleBool9 =
      this.setting(
          this.moduleNameService11,
          new ModuleSetting.Bool("Wait for Hurt Time", true)
              .description("Waits until the target's hurt animation ends before the next click."));
  private final ModuleSetting.Number moduleNumber8 =
      this.setting(
          this.moduleNameService11,
          new ModuleSetting.Number("Extra Aim Ticks", 0.0F, 0.0F, 4.0F, 1.0F)
              .description(
                  "Requires additional consecutive valid aim ticks beyond Smart Clicks / Combo"
                      + " Timing."));
  private final ModuleSetting.Number moduleNumber9 =
      this.setting(
          ...
          new ModuleSetting.Number("Hit Chance (%)", 100.0F, 1.0F, 100.0F, 1.0F)
              .description(
                  "Chance that a ready click is actually issued. Missed clicks still consume the"
                      + " CPS deadline."));
  private final ModuleSetting.Number moduleNumber10 =
      this.setting(
          ...
          new ModuleSetting.Number("Click Jitter (%)", 12.0F, 0.0F, 40.0F, 1.0F)
              .description(
                  "Extra interval variation used by Smart Clicks when Combo Timing is off."));
  private final ModuleSetting.Number moduleNumber11 =
      this.setting(
          this.moduleNameService11,
          new ModuleSetting.Number("Click Pause Chance (%)", 15.0F, 0.0F, 50.0F, 1.0F)
              .description(
                  "Chance of a one-tick pause after a Smart Click when Combo Timing is off."));
  private final ModuleSetting.Bool moduleBool10 =
      this.setting(
          this.moduleNameService12,
          new ModuleSetting.Bool("Weapons Only", false)
              .description(
                  "Tracks as usual but only attacks while holding a sword, axe, trident or mace."));
  private final ModuleSetting.Mode moduleMode4 =
      this.setting(
          this.moduleNameService3,
          new ModuleSetting.Mode(
                  "Shield Strategy",
                  new String[] {"Use Held Axe", "Wait for Opening", "Ignore"},
                  "Use Held Axe")
              .description(
                  "Waits when the opponent blocks this hit; Use Held Axe permits a held weapon that"
                      + " can disable their shield."));
  private final ModuleSetting.Bool moduleBool11 =
      this.setting(
          this.moduleNameService3,
          new ModuleSetting.Bool("AutoBlock", true)
              .description(
                  "Uses an equipped offhand shield against a nearby facing threat while your weapon"
                      + " recovers. Manual input takes priority."));
  private final ModuleSetting.Number moduleNumber12 =
      this.setting(
          this.moduleNameService3,
          new ModuleSetting.Number("Block Reaction (ticks)", 2.0F, 0.0F, 8.0F, 1.0F)
              .description(
                  "Observes the same threat for this many movement ticks before raising the"
                      + " shield."));
  private final ModuleSetting.Number moduleNumber13 =
      this.setting(
          ...
          new ModuleSetting.Number("Minimum Block (ticks)", 6.0F, 1.0F, 20.0F, 1.0F)
              .description(
                  "Keeps a useful block window through the shield's activation delay. Threat loss"
                      + " and manual input release it immediately."));
  private final ModuleSetting.Number moduleNumber14 =
      this.setting(
          this.moduleNameService3,
          new ModuleSetting.Number("Block Range", 4.0F, 1.0F, 6.0F, 0.1F)
              .description(
                  "Maximum distance for AutoBlock threat detection, still limited by Attack Range"
                      + " plus a short margin."));
  private final ModuleSetting.Bool moduleBool12 =
      this.setting(
          ...
          new ModuleSetting.Bool("Movement Correction", true)
              .description(
                  "Matches walking, strafing and sprint-jumps to the server rotation while"
                      + " preserving camera-relative input."));
  private final ModuleSetting.Mode moduleMode5 =
      this.setting(
          this.moduleNameService4,
          new ModuleSetting.Mode("Rotation Mode", new String[] {"Silent", "First Person"}, "Silent")
              .description(
                  "Silent keeps your mouse look free. First Person turns the camera toward the aim"
                      + " every frame; packets still use the GCD motor."));
  private final ModuleSetting.Number moduleNumber15 =
      this.setting(
          this.moduleNameService4,
          new ModuleSetting.Number("Camera Smoothness", 0.58F, 0.0F, 1.0F, 0.01F)
              .description(
                  "How floaty the first-person view is. 0 follows the aim immediately, 1 lags"
                      + " behind. Independent from packet Smoothness."));
  private final ModuleSetting.Number moduleNumber16 =
      this.setting(
          this.moduleNameService4,
          new ModuleSetting.Number("Camera Speed", 32.0F, 5.0F, 180.0F, 1.0F)
              .description(
                  "Maximum first-person turn rate in degrees per tick. Packets still use Turn"
                      + " Speed."));
  ...
  private final ModuleSetting.Number moduleNumber19 =
      this.setting(
          ...
          new ModuleSetting.Number("Pitch Speed (%)", 100.0F, 10.0F, 150.0F, 1.0F)
              .description(
                  "Pitch cap as a percentage of Turn Speed. 100 treats yaw and pitch equally."));
  private final ModuleSetting.Number moduleNumber20 =
      this.setting(
          this.moduleNameService13,
          new ModuleSetting.Number("Return Speed", 0.0F, 0.0F, 90.0F, 1.0F)
              .description(
                  "Turn-speed cap while returning to the camera after target loss. 0 uses Turn"
                      + " Speed."));
  private final ModuleSetting.Bool moduleBool13 =
      this.setting(
          ...
          new ModuleSetting.Bool("Combat AI", false)
              .description(
                  "Autonomously pursues, flanks, spaces and evades using a local opponent model."
                      + " Requires Movement Correction."));
  private final ModuleSetting.Mode moduleMode6 =
      this.setting(
          this.moduleNameService5,
          new ModuleSetting.Mode(
                  "Fighting Style", new String[] {"Balanced", "Pressure", "Defensive"}, "Balanced")
              .description(
                  "Balanced uses cooldown windows; Pressure closes gaps; Defensive creates space"
                      + " earlier at low health."));
  private final ModuleSetting.Number moduleNumber21 =
      this.setting(
          this.moduleNameService5,
          new ModuleSetting.Number("Preferred Distance", 2.45F, 0.5F, 4.0F, 0.05F)
              .description(
                  "Preferred distance to the opponent's hitbox, adapted to reach, health and the"
                      + " current exchange."));
  private final ModuleSetting.Number moduleNumber22 =
      this.setting(
          this.moduleNameService5,
          new ModuleSetting.Number("Pursuit Range", 5.0F, 1.0F, 8.0F, 0.25F)
              .description(
                  "Maximum distance for autonomous pursuit; target acquisition Range still"
                      + " applies."));
  private final ModuleSetting.Bool moduleBool14 =
      this.setting(
          ...
          new ModuleSetting.Bool("Adaptive Strafing", true)
              .description(
                  "Flanks opponents and adapts sides to their strafe. A/D biases the side; sprint"
                      + " follows your sprint key or AutoSprint."));
  private final ModuleSetting.Bool moduleBool15 =
      this.setting(
          this.moduleNameService5,
          new ModuleSetting.Bool("Pattern Learning", true)
              .description(
                  "Learns each opponent's recurring strafes and approach/retreat sequences. Only"
                      + " validated predictions influence movement; unreliable patterns fade."));
  private final ModuleSetting.MultiSelect moduleMultiSelect =
      this.setting(
          this.moduleNameService15,
          new ModuleSetting.MultiSelect(
                  "Manual Takeover",
                  new String[] {"Backward", "Sneak", "Jump"},
                  "Backward",
                  "Sneak",
                  "Jump")
  ...
  private final ModuleSetting.Number moduleNumber23 =
      this.setting(
          ...
          new ModuleSetting.Number("Jitter Intensity", 50.0F, 0.0F, 100.0F, 1.0F)
              .description(
                  "Scales slow aim-point drift, motor noise and tracking tempo variation; zero"
                      + " disables them."));
  private final ModuleSetting.Bool moduleBool16 =
      this.setting(
          this.moduleNameService6,
          new ModuleSetting.Bool("Overshoot 2-5°", true)
              .description("Occasionally passes the target by 2–5° before correcting."));
  private final ModuleSetting.Bool moduleBool17 =
      this.setting(
          this.moduleNameService6,
          new ModuleSetting.Bool("Reaction Delay 1-4 Ticks", true)
              .description(
                  "Waits 1–4 movement ticks before turning to a new target; an existing visible aim"
                      + " intersection needs no turn delay."));
  private final ModuleSetting.Bool moduleBool18 =
      this.setting(
          this.moduleNameService6,
          new ModuleSetting.Bool("GCD Snap Variation", true)
              .description("Varies the timing of the final sensitivity-grid correction."));
  private final ModuleSetting.Bool moduleBool19 =
      this.setting(
          this.moduleNameService6,
          new ModuleSetting.Bool("Micro Hesitation", true)
              .description("Occasionally pauses one tick between movement phases."));
  private final ModuleSetting.Range moduleRange2 =
      this.setting(
          this.moduleNameService14,
          new ModuleSetting.Range("Overshoot Degrees", 2.0F, 5.0F, 0.0F, 15.0F, 0.5F)
              .description(
                  "How far an overshoot passes the target before the corrective submovement."));
  private final ModuleSetting.Range moduleRange3 =
      this.setting(
          this.moduleNameService14,
  ...
  private final ModuleSetting.Range moduleRange4 =
      this.setting(
          this.moduleNameService14,
          new ModuleSetting.Range("Target Loss (ticks)", 2.0F, 4.0F, 0.0F, 10.0F, 1.0F)
              .description(
                  "How long a fully lost target is held before camera return, while Reaction Delay"
                      + " is enabled."));
  private final ModuleSetting.Range moduleRange5 =
      this.setting(
          this.moduleNameService14,
          new ModuleSetting.Range("Gap Reacquire (ticks)", 1.0F, 3.0F, 0.0F, 8.0F, 1.0F)
              .description(
                  "Delay before aiming through a newly visible opening, while Reaction Delay is"
                      + " enabled."));
  private final ModuleSetting.Mode moduleMode7 =
      this.setting(
          this.moduleNameService7,
          new ModuleSetting.Mode(
                  "Sensitivity Source", new String[] {"Minecraft", "Override"}, "Minecraft")
  ...
  private final ModuleSetting.Number moduleNumber25 =
      this.setting(
          this.moduleNameService7,
          new ModuleSetting.Number("GCD Offset Degrees", 0.0F, -0.05F, 0.05F, 5.0E-4F)
  ...
  private final ModuleSetting.Mode moduleMode8 =
      this.setting(
          ...
          new ModuleSetting.Mode(
                  "Test Profile", new String[] {"Human-like", "Impossible"}, "Human-like")
              .description(
                  "Chooses a human-like trace or an intentionally impossible control. Impossible is"
                      + " bannable; local anti-cheat tests only."));
  private final ModuleSetting.Mode moduleMode9 =
      this.setting(
          this.moduleNameService9,
          new ModuleSetting.Mode(
                  "Impossible Pattern",
                  new String[] {"Alternating Jerk", "Zero-Latency Snap", "Half-GCD"},
                  "Alternating Jerk")
              .description(
                  "Selects the jerk, instant-snap, or half-GCD fault pattern. Bannable; local"
                      + " anti-cheat tests only."));
  private final RotationSelectForCombatSelector rotationSelectForCombatSelector =
      new RotationSelectForCombatSelector();
  ...
  public ImplDecisionTracker() {
    super(
        ModuleBuilderData.builder("KillAura")
            .description("Automatically selects and attacks opponents.")
    ...
    this.moduleBool13
        .activeWhen(this.moduleBool12)
        .activeWhen(this.moduleMode8, "Human-like"::equals);

    for (ModuleSetting moduleSetting :
        new ModuleSetting[] {
          this.moduleMode6,
          this.moduleNumber21,
          this.moduleNumber22,
          this.moduleBool14,
          this.moduleBool15,
          this.moduleMultiSelect
        }) {
      moduleSetting
          .visibleWhen(this.moduleBool13)
          .activeWhen(this.moduleBool12)
          .activeWhen(this.moduleMode8, "Human-like"::equals);
    }
    ...
    this.on(EventAttackInputService.PLAYER_POSITION_CORRECTED)
        .run(
            item -> {
              ...
              this.combatCorrectedService.corrected(
                  this.timestamp, Math.round((Float) this.moduleNumber6.get()));
            });
  ...
  protected void onDisable() {
    ...
    CombatRotationOverride.start(
        ...
        CombatOwnsService.Owner.KILL_AURA,
  ...
  private void updateState() {
    enabled2 = false;
    this.timestamp++;
    if (!(Boolean) this.moduleBool13.get()
        || !(Boolean) this.moduleBool12.get()
        || !((String) this.moduleMode8.get()).equals("Human-like")) {
      this.combatDecisionTracker.clear();
    }
    ...
    if (this.combatCorrectedService.blocks(this.timestamp)) {
      ...
      if (CoreIsInitializedHandler.get()
          .modules()
        ...
        if (minecraft.player != null
          ...
          enabled2 =
              minecraft.player != null
                  ...
                  && this.combatAttackedService.suppress(
                      this.timestamp, this.uUID, minecraft.player.onGround());
          if (enabled2) {
          ...
          if (minecraft.player != null
            ...
            RotationObservationCapture.Observation observation =
                this.moduleBool13.get() && ((String) this.moduleMode8.get()).equals("Human-like")
                    ? RotationObservationCapture.capture(
                            ...
                            this.rotationData,
                            Math.min(
                                (Float) this.moduleNumber.get(), (Float) this.moduleNumber3.get()))
                        .orElse(null)
                    : null;
            CombatDecisionTracker.Decision decision = null;
            if (observation != null) {
              CombatDecisionTracker.Style style =
                  switch ((String) this.moduleMode6.get()) {
            ...
            boolean enabled =
                decision == null
                    ? this.combatCommitService.commit(
                        this.timestamp,
                        minecraft.player.getYRot(),
                        this.rotationData,
                        currentInput,
                        enabled2)
                    : this.combatCommitService.commitCorrected(
                        this.timestamp,
                        this.rotationData,
                        decision.input(),
                        enabled2 || decision.suppressSprint());
            if (enabled) {
              minecraft.player.setSprinting(false);
            }
          } else {
  ...
  private void updateState2() {
    ...
    if (currentRotationData != null) {
      ...
      this.rotationNextService2.synchronizeMotion(
          RotationObserveService.previousTickRotation().orElse(currentRotationData),
          currentRotationData);
    }
  }
  ...
  private void updateState3() {
    if (CombatOwnsService.owns(CombatOwnsService.Owner.KILL_AURA)) {
      if (LocalPhysicsFrameBus.wasAborted(this.timestamp)) {
        this.updateState11();
      } else if ((Boolean) this.moduleBool12.get()
          && this.rotationData != null
          && CoreIsInitializedHandler.mc().player != null) {
        ScaffoldRemapService.Input currentInput =
  ...
  private void updateState4() {
    if (CombatOwnsService.owns(CombatOwnsService.Owner.KILL_AURA)) {
      ...
      if (minecraft.player == null
          || minecraft.level == null
          || CompatAdapterService.rotationEnvironment().isEmpty()) {
        this.updateState11();
      }
    }
  }
  ...
  private void updateState5() {
    ...
    if (!result.isEmpty() && minecraft.player != null && minecraft.level != null) {
      ...
      RotationTargetFilter rotationTargetFilter =
          new RotationTargetFilter(
      ...
      if (currentResult.isEmpty()) {
        ...
        if (minecraft.screen != null) {
          ...
          RotationData currentRotationData =
              this.rotationData != null && CombatOwnsService.owns(CombatOwnsService.Owner.KILL_AURA)
                  ? this.rotationData
                  : rotationFrameContext.playerRotation();
          RotationFrameContext currentRotationFrameContext =
              new RotationFrameContext(
                  rotationFrameContext.eyePosition(),
                  currentRotationData,
                  rotationFrameContext.mouseSensitivity(),
                  rotationFrameContext.entities());
          double currentDoubleValue = enabled ? 360.0 : this.calculateValue();
          double nextDoubleValue =
              Math.min(
                  ((Float) this.moduleNumber3.get()).floatValue(),
                  minecraft.player.entityInteractionRange());
          RotationSelectForCombatSelector currentRotationSelectForCombatSelector =
              this.rotationSelectForCombatSelector;
          UUID currentUUID = this.uUID;

          Optional nextResult =
              currentRotationSelectForCombatSelector.selectForCombat(
                  currentRotationFrameContext,
                  currentUUID,
                  doubleValue,
                  nextDoubleValue,
                  currentDoubleValue,
                  switch ((String) this.moduleMode2.get()) {
                    ...
                    case "Highest Health" ->
                        RotationSelectForCombatSelector.Priority.HIGHEST_HEALTH;
          ...
          if (!nextResult.isEmpty()) {
            ...
            RotationSelectForCombatSelector.Selection selection =
                (RotationSelectForCombatSelector.Selection) nextResult.get();
            ...
            if (!currentEnabled) {
              ...
              this.count6 =
                  this.uUID != null ? Math.max(0, Math.round((Float) this.moduleNumber4.get())) : 0;
            }

            this.integer = selection.entity().entityId();
            this.uUID = selection.entity().uuid();
            RotationVector rotationVector =
                this.createRotationData7(selection.entity(), selection.aimPoint());
            ...
            double previousDoubleValue =
                ((String) this.moduleMode7.get()).equals("Override")
                    ? ((Float) this.moduleNumber24.get()).floatValue()
                    : rotationFrameContext.mouseSensitivity();
            RotationVector currentRotationVector =
                selection.entity().hitbox().sample(this.value2, this.value3, this.value4);
            if (!enabled
                && !this.checkCondition4(
                    selection.entity().hitbox(),
                    selection.entity().visibleAimPoints(),
                    rotationFrameContext.eyePosition(),
              ...
              if (enabled) {
                RotationVector nextRotationVector =
                    createRotationData72(
                        currentRotationVector, selection.entity().visibleAimPoints());
              ...
              if (!enabled) {
                RotationVector sourceRotationVector =
                    this.rotationNextService4.next(
                        selection.entity().hitbox(),
                        currentRotationVector,
                        this.integer,
                        ((Float) this.moduleNumber23.get()).floatValue() / 100.0);
                if (combatCompatibility.hasBlockLineOfSight(
                    minecraft, rotationFrameContext.eyePosition(), sourceRotationVector)) {
              ...
              RotationData previousRotationData =
                  !enabled && this.moduleNumber23.get() > 0.0F
                      ? RotationData.lookAt(
                          rotationFrameContext.eyePosition(), previousRotationVector)
                      : null;
              double sourceDoubleValue =
                  enabled ? ((Float) this.moduleNumber25.get()).floatValue() : 0.0;
              ...
              if (enabled) {
                switch ((String) this.moduleMode9.get()) {
                  ...
                  case "Half-GCD":
                    sourceDoubleValue =
                        -this.rotationVanillaGcdService.vanillaGcd(previousDoubleValue) * 0.5;
                }

                rotationHumanizationProfile =
                    new RotationHumanizationProfile(
                        90.0, 0.0, 0.0, false, false, false, false, sourceDoubleValue);
              } else {
                rotationHumanizationProfile =
                    new RotationHumanizationProfile(
                        ((Float) this.moduleNumber17.get()).floatValue(),
                          ...
                          && combatCompatibility.hasBlockLineOfSight(
                              ...
                              rotationFrameContext
                                  ...
                                  .add(
                                      sourceRotationData
                                          .direction()
                                          .multiply(optionalDouble.getAsDouble() + 1.0E-4)))
                      ? 1
                      : 0;
  ...
  private void updateState6() {
    ...
    if (CompatCanStartService.maintain(minecraft)) {
      ...
      this.updateState7();
      this.rotationData6 = null;
    } else if (!this.combatCorrectedService.blocks(this.timestamp)
        && CompatAvailableService.availableForPlanning(minecraft)) {
      ...
      if (rotationTarget == null) {
        ...
        LivingEntity livingEntity =
            minecraft.level.getEntity(rotationTarget.entityId())
                        instanceof LivingEntity currentLivingEntity
        ...
        LocalPlayer localPlayer = minecraft.player;
        CombatDecideService.Action action =
            this.combatDecideService.decide(
                this.timestamp,
          ...
          default:
            if (CompatAvailableService.available(minecraft)) {
              if (livingEntity == null || this.checkCondition2(minecraft, livingEntity)) {
                this.values.loseTarget();
              } else if (!(Boolean) this.moduleBool3.get()
                  && minecraft.player != null
                ...
                CombatData combatData =
                    combatCompatibility == null
                        ...
                        : combatCompatibility
                            .captureAttack(
                                CoreIsInitializedHandler.mc(), rotationTarget, doubleValue)
                            .orElse(null);
                if (combatData == null) {
                  this.values.loseTarget();
                } else {
                  ...
                  if ((Boolean) this.moduleBool10.get() && !checkCondition7(minecraft)) {
                    ...
                    int currentValue =
                        localPlayer != null && localPlayer.onGround() && localPlayer.isSprinting()
                            ? 1
                            : 0;
                    if (this.values.ready(
                        this.timestamp, offset, rotationTarget.uuid(), combatData, profile)) {
                      if (profile.hitChance() < 1.0
                          && this.random.nextDouble() >= profile.hitChance()) {
                        this.values.attacked(this.timestamp, offset, combatData, profile);
                        if (localPlayer != null) {
                          localPlayer.swing(InteractionHand.MAIN_HAND);
                        }
                      } else {
                        if (combatCompatibility.attack(
                            CoreIsInitializedHandler.mc(),
                            rotationTarget,
                            doubleValue,
                            this.moduleBool8.get() ? 1.0F : 0.0F)) {
                          this.values.attacked(this.timestamp, offset, combatData, profile);
                          this.timestamp4++;
                          ...
                          this.combatDecisionTracker.attacked(
                              this.timestamp, rotationTarget.uuid());
                          if ((Boolean) this.moduleBool7.get()) {
                            this.combatAttackedService.attacked(
                                this.timestamp,
                                rotationTarget.uuid(),
                                (currentValue != 0),
  ...
  private boolean checkCondition2(Minecraft minecraft, LivingEntity livingEntity) {
    ...
    boolean enabled =
        CompatBlocksAttackFromService.blocksAttackFrom(livingEntity, minecraft.player.position());
    ...
    if (currentSnapshot != null) {
      ...
      enabled |=
          CompatBlocksAttackFromService.blocksAttackFrom(
              livingEntity, new Vec3(rotationVector.x(), rotationVector.y(), rotationVector.z()));
    }

    return !enabled
        ? false
        : !((String) this.moduleMode4.get()).equals("Use Held Axe")
            || !CompatBlocksAttackFromService.canDisableWithHeldWeapon(minecraft, livingEntity);
  }
  ...
  private boolean checkCondition3(Minecraft minecraft, LivingEntity livingEntity) {
    ...
    if (currentSnapshot != null
      ...
      return (rotationVector.x() * currentX + rotationVector.z() * doubleValue) / currentDoubleValue
              > 0.4
  ...
  private void updateState7() {
    Minecraft minecraft = CoreIsInitializedHandler.mc();
    if (CompatCanStartService.holding(minecraft)
  ...
  private void updateState8() {
    ...
    if (this.rotationNextService2 == null
        || offset != this.timestamp2
        || !currentText.equals(this.text)) {
      this.updateState10();
    }
  }
    ...
    if (!this.checkCondition6() && this.rotationData != null) {
      if (this.rotationNextService3.completed(this.rotationData)) {
        ...
        RotationPublishService.publishReturning(
            rotationFrameContext.playerRotation(), this.rotationData, minecraft.player.yBodyRot);
      }
    } else {
      if (this.rotationData != null) {
        CombatOwnsService.release(CombatOwnsService.Owner.KILL_AURA);
  ...
  private void updateState12(
      RotationBoundingBox rotationBoundingBox, RotationVector rotationVector, int value) {
    this.count2 = value;
    this.value2 =
        calculateValue4(rotationVector.x(), rotationBoundingBox.minX(), rotationBoundingBox.maxX());
    this.value3 =
        calculateValue4(rotationVector.y(), rotationBoundingBox.minY(), rotationBoundingBox.maxY());
    this.value4 =
        calculateValue4(rotationVector.z(), rotationBoundingBox.minZ(), rotationBoundingBox.maxZ());
    this.updateState14();
  }
  ...
  private boolean checkCondition4(
    ...
    int currentValue =
        nextRotationVector.subtract(currentRotationVector).lengthSquared() <= 1.0E-10 ? 1 : 0;
    int nextValue =
        currentValue != 0
                && !RotationSelectForCombatSelector.hasHorizontalMargin(
                    rotationBoundingBox, currentRotationVector)
                && items.stream()
                    .anyMatch(
                        item ->
                            RotationSelectForCombatSelector.hasHorizontalMargin(
                                rotationBoundingBox, item))
            ? 1
            : 0;
    ...
    if (this.count3 == value) {
      RotationVector previousRotationVector =
          rotationBoundingBox.sample(this.value5, this.value6, this.value7);
      byteValue =
          (byte)
              (createRotationData72(previousRotationVector, items)
    ...
    if (byteValue == 0) {
      RotationVector sourceRotationVector =
          this.rotationSelectForCombatSelector.chooseStableAimPoint(
              rotationVector, rotationBoundingBox, currentRotationVector, items);
      this.count3 = value;
      this.value5 =
          calculateValue4(
              sourceRotationVector.x(), rotationBoundingBox.minX(), rotationBoundingBox.maxX());
      this.value6 =
          calculateValue4(
              sourceRotationVector.y(), rotationBoundingBox.minY(), rotationBoundingBox.maxY());
      this.value7 =
          calculateValue4(
              sourceRotationVector.z(), rotationBoundingBox.minZ(), rotationBoundingBox.maxZ());
      this.count4 =
          currentValue == 0 && this.moduleBool17.get()
              ? this.calculateValue3(this.moduleRange5)
              : 0;
    }
  ...
  private RotationData createRotationData(Minecraft minecraft) {
    ...
    return this.rotationNextService2.continueRotation(
        this.rotationData,
        this.rotationData,
        doubleValue,
        ((Float) this.moduleNumber17.get()).floatValue());
  ...
  private void updateState16(
      CombatCompatibility combatCompatibility, Minecraft minecraft, RotationData rotationData) {
    combatCompatibility.applyServerRotation(
        minecraft, rotationData, CombatOwnsService.Owner.KILL_AURA);
    if (this.checkCondition6()) {
      CompatActiveService.follow(
          minecraft,
          rotationData,
  ...
  private double calculateValue() {
    return switch ((String) this.moduleMode.get()) {
      case "180°" -> 180.0;
      case "Custom" ->
          Math.max(1.0, Math.min(360.0, ((Float) this.moduleNumber2.get()).floatValue()));
      default -> 360.0;
    };
  }
  ...
  private RotationVector createRotationData7(
      RotationTarget rotationTarget, RotationVector rotationVector) {
    double doubleValue =
        switch ((String) this.moduleMode3.get()) {
          case "Head" -> 0.88;
          case "Chest" -> 0.62;
          case "Legs" -> 0.2;
          case "Center" -> 0.5;
  ...
  private int calculateValue3(ModuleSetting.Range range) {
    ...
    return value != currentValue && this.random != null
        ? value + this.random.nextInt(currentValue - value + 1)
        : value;
  }
  ...
  public record TickBaseWindow(
      UUID target, double reach, int stableTicks, double turnSpeed, long attacks) {}
}
