...
public final class ImplProfileService extends ModuleSettingsService {
   private final ModuleNameService moduleNameService = this.settingCategory("Timing")
      .description("Controls how the client game clock changes over time.");
   private final ModuleNameService moduleNameService2 = this.settingCategory("Packet pulse")
      .description("Uses movement packets that really reached the connection as its phase clock.");
   private final ModuleSetting.Mode moduleMode = this.setting(
      this.moduleNameService, new ModuleSetting.Mode("Mode", new String[]{"Constant", "Pulse", "Random", "Packet Pulse"}, "Constant")
   );
   ...
   private final ModuleSetting.Number moduleNumber4 = this.setting(
      this.moduleNameService,
      new ModuleSetting.Number("Boost Ticks", 8.0F, 1.0F, 40.0F, 1.0F)
   );
   private final ModuleSetting.Number moduleNumber5 = this.setting(
      this.moduleNameService,
      new ModuleSetting.Number("Brake Ticks", 4.0F, 1.0F, 40.0F, 1.0F)
   );
   ...
   private final ModuleSetting.Number moduleNumber7 = this.setting(
      ...
      new ModuleSetting.Number("Packets per Phase", 4.0F, 1.0F, 20.0F, 1.0F)
         .description("Switches between boost and brake after this many accepted movement packets.")
   );
   ...
   public ImplProfileService() {
      super(
         ModuleBuilderData.builder("Timer")
            ...
            .description("Changes the client game clock with steady, pulsing, random and packet-driven timing.")
            .build()
      );
      this.moduleNumber.visibleWhen(this.moduleMode, "Constant"::equals);
      this.moduleNumber2.visibleWhen(this.moduleMode, item -> item.equals("Pulse") || item.equals("Packet Pulse"));
      this.moduleNumber3.visibleWhen(this.moduleMode, item -> item.equals("Pulse") || item.equals("Packet Pulse"));
   ...
   public TimerModeTracker.Profile profile() {
      return new TimerModeTracker.Profile(
         TimerModeTracker.Mode.valueOf(((String)this.moduleMode.get()).toUpperCase(Locale.ROOT).replace(' ', '_')),
   ...
   protected void onEnable() {
      ...
      this.on(EventAttackInputService.TICK).priority(EventIsAfterHandler.Priority.FIRST).run(item -> TimerStateController.tick());
      this.on(EventAttackInputService.OUTGOING_PACKET_ACCEPTED).run(item -> TimerStateController.accepted(item.packet()));
