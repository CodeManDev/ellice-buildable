...
public final class AccentComponent extends ModuleSettingsService {
  ...
  private final ModuleSetting.Color accentColor =
      this.setting(
          new ModuleSetting.Color("Accent", DEFAULT_ACCENT_COLOR)
              .description(
                  "Sets the client-wide interactive accent; hover and pressed shades are derived"
                      + " automatically."));
  private final ModuleSetting.Color surfaceColor =
      this.setting(
  ...
  private final ModuleSetting.Number blurRadius =
      this.setting(
          new ModuleSetting.Number("Blur", 16.0F, 0.0F, 32.0F, 1.0F)
              .description(
                  "Sets the shared backdrop-blur radius for themed UI surfaces; 0 keeps them"
                      + " sharp."));
  ...
  private void notifyThemeChanged() {
    if (CoreIsInitializedHandler.isReady()
        && CoreIsInitializedHandler.get().pluginLoader() != null) {
