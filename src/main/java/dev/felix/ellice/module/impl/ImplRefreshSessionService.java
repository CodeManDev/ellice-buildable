...
public final class ImplRefreshSessionService extends ModuleSettingsService {
  ...
  private final ModuleSetting.Text moduleText =
      this.setting(
          (new ModuleSetting.Text("Custom name", "Player", 64) {
                @Override
              ...
              .description(
                  "Your local display name throughout ellice and Minecraft. Blank uses Player; your"
                      + " server identity stays unchanged."));
  ...
  public ImplRefreshSessionService(Supplier<String> supplier, Runnable currentRunnable) {
    super(
        ModuleBuilderData.builder("NameChanger")
            .description(
                "Replaces your visible username across the client with a custom display name.")
            .category(ModuleFeatureType.CLIENT)
            .build());
    this.text = supplier;
    this.runnable = currentRunnable;
    this.moduleText.onChange(
        item -> {
          if (this.isEnabled()) {
            this.updateState();
          }
