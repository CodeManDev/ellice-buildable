...
public final class RenderMatrixService {
  ...
  public static void apply(
      PoseStack poseStack, HumanoidArm humanoidArm, float value, float currentValue) {
    ImplActiveService implActive = ImplActiveService.active();
    poseStack.mulPose(
        matrix(
            humanoidArm == HumanoidArm.RIGHT,
            value,
            implActive != null && implActive.blockSwing() ? currentValue : 0.0F));
  }
  ...
  public static Matrix4f matrix(boolean enabled, float value, float currentValue) {
    ...
    return new Matrix4f()
        .translation(nextValue * 0.56F, -0.52F - calculateValue(value) * 0.6F, -0.72F)
  ...
  private static Matrix4f createMatrix4f(float value) {
    Matrix4f matrix4f =
        new Matrix4f()
            .translation(value * 1.13F / 16.0F, 0.2F, 0.070625F)
            .rotateY(-1.5707964F)
            .rotateZ(0.43633232F)
            .scale(0.68F);
    return new Matrix4f()
