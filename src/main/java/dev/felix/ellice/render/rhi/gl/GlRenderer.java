...
public class GlRenderer implements RhiCommandBuffer {
  private final GlShaderFactory glShaderFactory;
  private RhiBlendStateService.PipelineHandle fgwppd7bvwdo =
      RhiBlendStateService.PipelineHandle.NONE;
  private int boundProgramId;
  private int boundVertexArrayId;
  private RhiBlendStateService.IndexType f25dux3jku1 = RhiBlendStateService.IndexType.UINT32;
  private final Map<Integer, Map<String, Uniform>> uniformCachesByProgram =
      new HashMap<Integer, Map<String, Uniform>>();
  private Map<String, Uniform> activeUniformCache;
  private final Map<Integer, Integer> vertexArrayBuffers = new HashMap<Integer, Integer>();
  private static final int MAX_TEXTURE_UNITS = 16;
  private static final int ALL_FRAMEBUFFER_ASPECTS =
      RhiBlendStateService.FramebufferAspect.COLOR.bit
          | RhiBlendStateService.FramebufferAspect.DEPTH.bit
          | RhiBlendStateService.FramebufferAspect.STENCIL.bit;
  private static final int DEPTH_STENCIL_ASPECTS =
      RhiBlendStateService.FramebufferAspect.DEPTH.bit
          | RhiBlendStateService.FramebufferAspect.STENCIL.bit;
  private final int[] boundTextureIds = new int[16];
  private final int[] boundSamplerIds = new int[16];
  private final Map<Integer, Integer> boundUniformBuffers = new HashMap<Integer, Integer>();
  private final Map<Integer, Integer> fdvxvihqybac = new HashMap<Integer, Integer>();
  private final ArrayList<GLState> savedStates = new ArrayList();

  public void invalidateUniformCache(int n) {
    this.uniformCachesByProgram.remove(n);
    if (this.boundProgramId == n) {
      this.boundProgramId = 0;
      this.activeUniformCache = null;
    }
  }

  public void invalidateVaoCache(int n) {
    this.vertexArrayBuffers.remove(n);
    if (this.boundVertexArrayId == n) {
      this.boundVertexArrayId = 0;
    }
  }

  public void invalidateBufferCache(int n) {
    this.vertexArrayBuffers.entrySet().removeIf(entry -> (Integer) entry.getValue() == n);
    this.boundUniformBuffers.entrySet().removeIf(entry -> (Integer) entry.getValue() == n);
    this.fdvxvihqybac.entrySet().removeIf(entry -> (Integer) entry.getValue() == n);
  }

  public void invalidateTextureCache(int n) {
    for (int i = 0; i < this.boundTextureIds.length; ++i) {
      if (this.boundTextureIds[i] != n) continue;
      this.boundTextureIds[i] = -1;
    }
  }

  public void invalidateSamplerCache(int n) {
    for (int i = 0; i < this.boundSamplerIds.length; ++i) {
      if (this.boundSamplerIds[i] != n) continue;
      this.boundSamplerIds[i] = -1;
    }
  }

  private void updateState() {
    Arrays.fill(this.boundTextureIds, -1);
    Arrays.fill(this.boundSamplerIds, -1);
    this.boundUniformBuffers.clear();
  ...
  GlRenderer(GlShaderFactory glShaderFactory) {
    this.glShaderFactory = glShaderFactory;
    this.updateState();
  }
  ...
  public void saveGLState() {
    ...
    this.savedStates.add(
        new GLState(
            GL11.glGetInteger((int) 35725),
            GL11.glGetInteger((int) 34229),
            GL11.glGetInteger((int) 34964),
            GL11.glGetInteger((int) 35055),
            GL11.glGetInteger((int) 36010),
            GL11.glGetInteger((int) 36006),
            GL11.glGetInteger((int) 3074),
            nArray6,
            n2,
            nArray,
            nArray2,
            GL11.glGetInteger((int) 3317),
            GL11.glGetInteger((int) 3314),
            GL11.glGetInteger((int) 3315),
            GL11.glGetInteger((int) 3316),
            GL11.glGetInteger((int) 3041),
            GL11.glGetInteger((int) 3040),
            GL11.glGetInteger((int) 32971),
            GL11.glGetInteger((int) 32970),
            GL11.glGetInteger((int) 32777),
            GL11.glGetInteger((int) 34877),
            GL11.glIsEnabled((int) 3042),
            GL11.glIsEnabled((int) 2929),
            GL11.glIsEnabled((int) 2884),
            GL11.glIsEnabled((int) 2960),
            GL11.glIsEnabled((int) 3089),
            GL11.glGetBoolean((int) 2930),
            bl4,
            bl3,
            bl2,
            bl,
            GL11.glGetInteger((int) 2932),
            GL11.glGetInteger((int) 2885),
            GL11.glGetInteger((int) 2886),
            nArray3,
            nArray4,
            nArray5));
  ...
  public void restoreGLState() {
    ...
    GLState gLState = this.savedStates.remove(this.savedStates.size() - 1);
    ...
    GL11.glViewport(
        (int) gLState.viewport[0],
        (int) gLState.viewport[1],
        (int) gLState.viewport[2],
        (int) gLState.viewport[3]);
    GL11.glScissor(
        (int) gLState.scissorBox[0],
        (int) gLState.scissorBox[1],
        (int) gLState.scissorBox[2],
        (int) gLState.scissorBox[3]);
    ...
    GlRenderer.setCapabilityEnabled(3042, gLState.blend);
    GL14.glBlendFuncSeparate(
        (int) gLState.blendSrc,
        (int) gLState.blendDst,
        (int) gLState.blendSrcA,
        (int) gLState.blendDstA);
    GL20.glBlendEquationSeparate((int) gLState.blendEqRGB, (int) gLState.blendEqAlpha);
    GlRenderer.setCapabilityEnabled(2929, gLState.depth);
    GL11.glDepthFunc((int) gLState.depthFunc);
    GL11.glDepthMask((boolean) gLState.depthMask);
    GL11.glColorMask(
        (boolean) gLState.colorMaskR,
        (boolean) gLState.colorMaskG,
        (boolean) gLState.colorMaskB,
        (boolean) gLState.colorMaskA);
    GlRenderer.setCapabilityEnabled(2884, gLState.cull);
    ...
    GlRenderer.setCapabilityEnabled(2960, gLState.stencil);
    GlRenderer.setCapabilityEnabled(3089, gLState.scissor);
    this.fgwppd7bvwdo = RhiBlendStateService.PipelineHandle.NONE;
    this.boundProgramId = 0;
    this.activeUniformCache = null;
    this.boundVertexArrayId = 0;
  ...
  public void beginRenderPass(
    ...
    this.configureDrawBuffers(framebufferHandle);
    try (MemoryStack memoryStack = MemoryStack.stackPush(); ) {
  ...
  public void bindPipeline(RhiBlendStateService.PipelineHandle pipelineHandle) {
    ...
    GlShaderFactory.PipelineState pipelineState =
        this.glShaderFactory.getPipeline(pipelineHandle.id());
    if (pipelineState.program() != this.boundProgramId) {
      GL20.glUseProgram((int) pipelineState.program());
      this.boundProgramId = pipelineState.program();
      this.activeUniformCache = null;
    }
    if (pipelineState.vao() != 0 && pipelineState.vao() != this.boundVertexArrayId) {
      GL30.glBindVertexArray((int) pipelineState.vao());
      this.boundVertexArrayId = pipelineState.vao();
    }
    this.mnbe52lygeq(pipelineState.blend());
    this.applyDepthStencilState(pipelineState.depthStencil());
    this.applyRasterizerState(pipelineState.rasterizer());
  }
  ...
  public void bindVertexBuffer(RhiBlendStateService.BufferHandle bufferHandle, int n) {
    Integer n2;
    GlShaderFactory.PipelineState pipelineState =
        this.glShaderFactory.getPipeline(this.fgwppd7bvwdo.id());
    int n3 = pipelineState.vao();
    if (n3 != 0 && n3 != this.boundVertexArrayId) {
      GL30.glBindVertexArray((int) n3);
      this.boundVertexArrayId = n3;
    }
    Integer n4 = n2 = n3 != 0 ? this.vertexArrayBuffers.get(n3) : null;
    if (n2 != null && n2.intValue() == bufferHandle.id()) {
      return;
    }
    GL15.glBindBuffer((int) 34962, (int) bufferHandle.id());
    if (pipelineState.layout() != null) {
      for (RhiBlendStateService.VertexAttribute vertexAttribute :
          pipelineState.layout().attributes()) {
        if (vertexAttribute.format() == RhiBlendStateService.VertexFormat.INT) {
          GL30.glVertexAttribIPointer(
              (int) vertexAttribute.location(),
              (int) vertexAttribute.components(),
              (int) GlShaderFactory.glVertexAttribType(vertexAttribute.format()),
              (int) pipelineState.layout().stride(),
              (long) vertexAttribute.offset());
          continue;
        }
      ...
      if (n3 != 0) {
        this.vertexArrayBuffers.put(n3, bufferHandle.id());
      }
    }
  }
  ...
  public void bindUniformBuffer(RhiBlendStateService.BufferHandle bufferHandle, int n) {
    ...
    Integer n3 = this.boundUniformBuffers.get(n);
    if (n3 != null && n3 == n2) {
      return;
    }
    GL30.glBindBufferBase((int) 35345, (int) n, (int) n2);
    this.boundUniformBuffers.put(n, n2);
  }
  ...
  public void bindTexture(RhiBlendStateService.TextureHandle textureHandle, int n) {
    this.bindTextureUnit(textureHandle != null ? textureHandle.id() : 0, 0, n);
  }
  ...
  public void bindTexture(
    ...
    this.bindTextureUnit(
        textureHandle != null ? textureHandle.id() : 0,
        samplerHandle != null ? samplerHandle.id() : 0,
        n);
  }

  private void bindTextureUnit(int n, int n2, int n3) {
    int n4;
    int n5 = n4 = n3 >= 0 && n3 < this.boundTextureIds.length ? 1 : 0;
    if (n4 != 0 && this.boundTextureIds[n3] == n && this.boundSamplerIds[n3] == n2) {
      return;
    }
    ...
    if (n4 != 0) {
      this.boundTextureIds[n3] = n;
      this.boundSamplerIds[n3] = n2;
    }
  }
  ...
  private Uniform getUniformCacheEntry(String string) {
    Uniform uniform;
    Map<String, Uniform> map = this.activeUniformCache;
    if (map == null) {
      this.activeUniformCache =
          map =
              this.uniformCachesByProgram.computeIfAbsent(this.boundProgramId, n -> new HashMap());
    }
    if ((uniform = map.get(string)) == null) {
      uniform =
          new Uniform(GL20.glGetUniformLocation((int) this.boundProgramId, (CharSequence) string));
  ...
  public void pushInt(String string, int n) {
    Uniform uniform = this.getUniformCacheEntry(string);
  ...
  public void draw(int n, int n2, int n3) {
    int n4 =
        GlShaderFactory.glTopology(
            this.glShaderFactory.getPipeline(this.fgwppd7bvwdo.id()).topology());
    if (n2 <= 1) {
  ...
  public void blitFramebuffer(
      ...
      int n9) {
    if (n9 == 0 || (n9 & ~ALL_FRAMEBUFFER_ASPECTS) != 0) {
    ...
    if ((n9 & RhiBlendStateService.FramebufferAspect.COLOR.bit) != 0) {
      this.configureReadBuffer(framebufferHandle);
      this.configureDrawBuffer(framebufferHandle2);
    }
    int n11 = (n9 & DEPTH_STENCIL_ASPECTS) != 0 ? 1 : 0;
    GL30.glBlitFramebuffer(
        ...
        (int) n8,
        (int) n10,
        (int) (n11 == 0 && filterMode == RhiBlendStateService.FilterMode.LINEAR ? 9729 : 9728));
  }

  private void configureDrawBuffers(RhiBlendStateService.FramebufferHandle framebufferHandle) {
    if (framebufferHandle.id() == 0) {
      GL11.glDrawBuffer((int) 1029);
      return;
    }
    int n = this.glShaderFactory.getFramebufferColorAttachmentCount(framebufferHandle.id());
    if (n < 0) {
  ...
  private void configureReadBuffer(RhiBlendStateService.FramebufferHandle framebufferHandle) {
    int n = this.glShaderFactory.getFramebufferColorAttachmentCount(framebufferHandle.id());
    GL11.glReadBuffer((int) (framebufferHandle.id() == 0 ? 1029 : (n == 0 ? 0 : 36064)));
  }

  private void configureDrawBuffer(RhiBlendStateService.FramebufferHandle framebufferHandle) {
    int n = this.glShaderFactory.getFramebufferColorAttachmentCount(framebufferHandle.id());
  ...
  private void mnbe52lygeq(RhiBlendStateService.BlendState blendState) {
    ...
    GL14.glBlendFuncSeparate(
        (int) GlShaderFactory.glBlendFactor(blendState.srcColor()),
        (int) GlShaderFactory.glBlendFactor(blendState.dstColor()),
        (int) GlShaderFactory.glBlendFactor(blendState.srcAlpha()),
        (int) GlShaderFactory.glBlendFactor(blendState.dstAlpha()));
    GL20.glBlendEquationSeparate(
        (int) GlShaderFactory.glBlendOp(blendState.colorOp()),
        (int) GlShaderFactory.glBlendOp(blendState.alphaOp()));
  }

  private void applyDepthStencilState(RhiBlendStateService.DepthStencilState depthStencilState) {
    ...
    GlRenderer.setCapabilityEnabled(2960, depthStencilState.stencilTest());
  }

  private void applyRasterizerState(RhiBlendStateService.RasterizerState rasterizerState) {
    if (rasterizerState.cullMode() == RhiBlendStateService.CullMode.NONE) {
      GL11.glDisable((int) 2884);
    } else {
      GL11.glEnable((int) 2884);
      GL11.glCullFace(
          (int) (rasterizerState.cullMode() == RhiBlendStateService.CullMode.FRONT ? 1028 : 1029));
    }
    GL11.glFrontFace(
        (int) (rasterizerState.frontFace() == RhiBlendStateService.FrontFace.CCW ? 2305 : 2304));
    GL11.glPolygonMode((int) 1032, (int) (rasterizerState.wireframe() ? 6913 : 6914));
  }

  private static void setCapabilityEnabled(int n, boolean bl) {
    if (bl) {
      GL11.glEnable((int) n);
    } else {
