...
public final class Render3dSceneService implements AutoCloseable {
   ...
   public static final ShaderDefinition WORLD_ATMOSPHERE_SHADER = new ShaderDefinition(
      "ellice:world-atmosphere", "fullscreen.vert", "render3d/world_atmosphere.frag"
   );
   public static final ShaderDefinition PBR_SHADER = new ShaderDefinition("ellice:pbr", "render3d/pbr.vert", "render3d/pbr.frag");
   public static final ShaderDefinition WORLD_GRADE_SHADER = new ShaderDefinition(
      "ellice:world-grade", "fullscreen.vert", "render3d/world_grade.frag"
   );
   ...
   public Render3dSceneService(Path path) {
      ...
      this.renderer2 = new SceneObjectRenderer(this.rhiOperationHandler, this.shaderOverrideRootService);
      this.renderer3 = new Render3dWantsCaptureService(this.rhiOperationHandler, this.shaderOverrideRootService);
      this.renderer4 = new PostRenderer(this.rhiOperationHandler, this.shaderOverrideRootService);
      this.renderer4
         .register(new PostIdService("ellice:world-atmosphere", WORLD_ATMOSPHERE_SHADER, true).stage(PostIdService.Stage.BEFORE_HAND));
      this.renderer4
         .register(
      ...
      CoreIsInitializedHandler.LOGGER.info("ellice 3D engine ready (GLSL 330, live shaders: {})", this.shaderOverrideRootService.overrideRoot());
   }
   ...
   public Render3dRenderer createView() {
      this.updateState2();
      Render3dRenderer render3dRenderer = new Render3dRenderer(this.rhiOperationHandler, this.shaderOverrideRootService);
   ...
   public void preparePlayerFireAuras(List<PlayerMotionSample> items, FireEffectSettings fireEffectSettings) {
      if (!this.enabled) {
   ...
   public void renderGeometry(EventAttackInputService.WorldRender worldRender, FramebufferInfo framebufferInfo) {
      if (!this.enabled && worldRender != null && framebufferInfo != null) {
         long longValue = System.nanoTime();
         float currentHeight = this.timestamp2 == 0L
            ? 0.016666668F
         ...
         Render3dViewService currentWidth = new Render3dViewService(
         ...
         this.updateState(() -> {
            RhiBlendStateService.FramebufferHandle framebufferHandle = new RhiBlendStateService.FramebufferHandle(framebufferInfo.framebufferId());
            this.renderer2.render(this.renderer, this.rhiOperationHandler.encoder(), framebufferHandle, currentWidth);
            DiagnosticsAddFrameListenerService diagnosticsAddFrameListener = DiagnosticsAddFrameListenerService.get();
            diagnosticsAddFrameListener.begin("shaderesp-depth");

            try {
               this.renderer3.captureSceneDepth(this.rhiOperationHandler.encoder(), framebufferHandle, currentWidth);
            } finally {
               diagnosticsAddFrameListener.end();
            }

            this.renderer4.captureDepth(this.rhiOperationHandler.encoder(), framebufferHandle, currentWidth);
            this.renderer4.renderBeforeHand(this.rhiOperationHandler.encoder(), framebufferHandle, currentWidth);
         });
      }
   }

   public void renderPost(EventAttackInputService.WorldPostRender worldPostRender, FramebufferInfo framebufferInfo) {
      if (!this.enabled && worldPostRender != null && framebufferInfo != null && this.renderer5 != null) {
         Render3dViewService render3dView = this.renderer5;
         this.renderer5 = null;
         if (render3dView.width() == framebufferInfo.width() && render3dView.height() == framebufferInfo.height()) {
            this.updateState(() -> {
               RhiBlendStateService.FramebufferHandle framebufferHandle = new RhiBlendStateService.FramebufferHandle(framebufferInfo.framebufferId());
               this.renderer4.render(this.rhiOperationHandler.encoder(), framebufferHandle, render3dView);
               DiagnosticsAddFrameListenerService diagnosticsAddFrameListener = DiagnosticsAddFrameListenerService.get();
               diagnosticsAddFrameListener.begin("shaderesp");

               try {
                  this.renderer3.render(this.rhiOperationHandler.encoder(), framebufferHandle, render3dView);
               } finally {
                  diagnosticsAddFrameListener.end();
               }
            });
