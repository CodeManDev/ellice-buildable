...
final class KawasePostProcessor implements AutoCloseable {
   ...
   KawasePostProcessor(RhiOperationHandler rhiOperation, ShaderOverrideRootService shaderOverrideRoot) {
      this.rhiOperationHandler = rhiOperation;
      this.fmvr01y3d7 = this.createShaderRecipeService(shaderOverrideRoot, "extract", RhiBlendStateService.BlendState.DISABLED);
      this.shaderRecipeService = this.createShaderRecipeService(shaderOverrideRoot, "down", RhiBlendStateService.BlendState.DISABLED);
      this.shaderRecipeService2 = this.createShaderRecipeService(shaderOverrideRoot, "up", RhiBlendStateService.BlendState.DISABLED);
      this.shaderRecipeService3 = this.createShaderRecipeService(shaderOverrideRoot, "composite", Render3dWantsCaptureService.OUTLINE_BLEND);
   }

   private ShaderRecipeService createShaderRecipeService(ShaderOverrideRootService shaderOverrideRoot, String shaderSource, RhiBlendStateService.BlendState blendState) {
      return new ShaderRecipeService(
         this.rhiOperationHandler,
         shaderOverrideRoot,
         new ShaderData(
            new ShaderDefinition("ellice:player-kawase-" + shaderSource, "fullscreen.vert", "render3d/player_kawase_" + shaderSource + ".frag"),
            vertexLayout,
            blendState,
   ...
   void render(
      ...
      if (kawaseBloomSettings.renderable()) {
         ...
         RhiBlendStateService.PipelineHandle currentPipelineHandle = this.shaderRecipeService3.resolve();
         RhiBlendStateService.PipelineHandle nextPipelineHandle = kawaseBloomSettings.bloomRenderable() ? this.shaderRecipeService.resolve() : RhiBlendStateService.PipelineHandle.NONE;
         RhiBlendStateService.PipelineHandle previousPipelineHandle = kawaseBloomSettings.bloomRenderable() ? this.shaderRecipeService2.resolve() : RhiBlendStateService.PipelineHandle.NONE;
         if (pipelineHandle.valid() && currentPipelineHandle.valid() && (!kawaseBloomSettings.bloomRenderable() || nextPipelineHandle.valid() && previousPipelineHandle.valid())) {
            Render3dSourceWidthService currentWidth = Render3dSourceWidthService.create(render3dView.width(), render3dView.height(), kawaseBloomSettings.radiusPixels(), kawaseBloomSettings.levels());
            this.updateState4(currentWidth, kawaseBloomSettings.bloomRenderable() ? currentWidth.levels() : 1);
            ...
            if (kawaseBloomSettings.bloomRenderable()) {
               for (int index = 1; index < this.renderer.length; index++) {
                  ...
                  rhiCommandBuffer.pushVec2("uSourceTexel", 1.0F / currentSurface.width(), 1.0F / currentSurface.height());
               ...
               for (int currentIndex = this.renderer.length - 2; currentIndex >= 0; currentIndex += -1) {
                  updateState3(rhiCommandBuffer, this.renderer2[currentIndex], previousPipelineHandle, bufferHandle);
            ...
            rhiCommandBuffer.bindTexture(sourceTextureHandle.valid() ? sourceTextureHandle : textureHandle, 4);
            ...
            rhiCommandBuffer.pushFloat("uStrength", kawaseBloomSettings.bloomRenderable() ? kawaseBloomSettings.strength() : 0.0F);
   ...
   private static void updateState(RhiCommandBuffer rhiCommandBuffer, Render3dViewService render3dView, KawaseBloomSettings kawaseBloomSettings) {
      ...
      try {
         rhiCommandBuffer.pushMatrix4("uInvProjection", render3dView.inverseProjection().get(stack.mallocFloat(16)));
   ...
   private static void updateState3(
      RhiCommandBuffer rhiCommandBuffer, KawasePostProcessor.Surface surface, RhiBlendStateService.PipelineHandle pipelineHandle, RhiBlendStateService.BufferHandle bufferHandle
   ) {
      rhiCommandBuffer.clearScissor();
   ...
   private void updateState4(Render3dSourceWidthService render3dSourceWidth, int index) {
      if (this.count != render3dSourceWidth.sourceWidth() || this.count2 != render3dSourceWidth.sourceHeight() || this.renderer.length != index) {
         this.clearTargets();
   ...
   private KawasePostProcessor.Surface createSurface(int value, int currentValue) {
      RhiBlendStateService.TextureHandle textureHandle = this.rhiOperationHandler
         .createTexture(RhiBlendStateService.TextureDescriptor.renderTarget(value, currentValue, RhiBlendStateService.TextureFormat.RGBA16F));

      try {
         RhiBlendStateService.FramebufferHandle framebufferHandle = this.rhiOperationHandler.createFramebuffer(textureHandle);
         if (textureHandle.valid() && framebufferHandle.valid()) {
            return new KawasePostProcessor.Surface(textureHandle, framebufferHandle, value, currentValue);
         } else {
   ...
   private record Surface(RhiBlendStateService.TextureHandle texture, RhiBlendStateService.FramebufferHandle framebuffer, int width, int height) {
   }
}
