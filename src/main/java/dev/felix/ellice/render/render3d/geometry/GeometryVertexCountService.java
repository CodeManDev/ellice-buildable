...
public final class GeometryVertexCountService {
      ...
      if (floats.length != 0 && floats.length % 12 == 0) {
         ...
         if (ints.length >= 3 && ints.length % 3 == 0) {
            ...
            for (byte previousIndex = 0; previousIndex < ints.length; previousIndex += 3) {
               ...
               if (sourceIndex == targetIndex || targetIndex == inputIndex || inputIndex == sourceIndex) {
                  throw new IllegalArgumentException("triangle " + previousIndex / 3 + " repeats a vertex index");
               }
            ...
            throw new IllegalArgumentException("index data must contain a non-zero whole number of triangles");
         }
      } else {
         throw new IllegalArgumentException("vertex data must contain a non-zero whole number of 12-float vertices");
      }
   }
   ...
   private static void updateState(float[] floats, int value) {
      ...
      for (int index = 0; index < 12; index++) {
         if (!Float.isFinite(floats[currentValue + index])) {
            throw new IllegalArgumentException("vertex " + value + " component " + index + " must be finite");
         }
      }
      ...
      double nextDoubleValue = (double)nextValue * targetValue + (double)previousValue * inputValue + (double)sourceValue * outputValue;
      if (Math.abs(nextDoubleValue) > 0.002) {
         throw new IllegalArgumentException("vertex " + value + " tangent must be orthogonal to its normal");
      }

      if (Math.abs(Math.abs(resultValue) - 1.0) > 1.0E-4) {
         throw new IllegalArgumentException("vertex " + value + " tangent handedness must be -1 or +1");
      }
   }

   private static void updateState2(int value, int currentValue, int nextValue) {
      if (value < 0 || value >= currentValue) {
         throw new IllegalArgumentException("index " + nextValue + " references vertex " + value + " outside [0, " + currentValue + ")");
      }
   }

   private static void updateState3(float[] floats, int value, int currentValue, int nextValue, int previousValue) {
      int index = value * 12;
      ...
      double inputDoubleValue = currentDoubleValue * targetDoubleValue - nextDoubleValue * sourceDoubleValue;
      double outputDoubleValue = nextDoubleValue * previousDoubleValue - doubleValue * targetDoubleValue;
      double resultDoubleValue = doubleValue * sourceDoubleValue - currentDoubleValue * previousDoubleValue;
      double candidateDoubleValue = inputDoubleValue * inputDoubleValue + outputDoubleValue * outputDoubleValue + resultDoubleValue * resultDoubleValue;
      if (Double.isFinite(candidateDoubleValue) && !(candidateDoubleValue <= 0.0)) {
         double selectedDoubleValue = floats[index + 3] + floats[currentIndex + 3] + floats[nextIndex + 3];
         double defaultDoubleValue = floats[index + 3 + 1] + floats[currentIndex + 3 + 1] + floats[nextIndex + 3 + 1];
         double initialDoubleValue = floats[index + 3 + 2] + floats[currentIndex + 3 + 2] + floats[nextIndex + 3 + 2];
         double resolvedDoubleValue = inputDoubleValue * selectedDoubleValue + outputDoubleValue * defaultDoubleValue + resultDoubleValue * initialDoubleValue;
         if (!(resolvedDoubleValue > 0.0)) {
            throw new IllegalArgumentException("triangle " + previousValue + " winding disagrees with its vertex normals");
         }
      } else {
   ...
   private static double calculateValue2(float value, float currentValue, float nextValue) {
      return (double)value * value + (double)currentValue * currentValue + (double)nextValue * nextValue;
   }

   private static GeometryVertexCountService.BoundingSphere createBoundingSphere(float[] floats, int value) {
      ...
      for (int nextIndex = 0; nextIndex < value; nextIndex++) {
         ...
         doubleValue = Math.max(doubleValue, currentDoubleValue * currentDoubleValue + nextDoubleValue * nextDoubleValue + previousDoubleValue * previousDoubleValue);
      }
      ...
      return new GeometryVertexCountService.BoundingSphere(selectedValue, defaultValue, initialValue, resolvedValue);
   }
   ...
   public boolean equals(Object value) {
      if (this == value) {
         ...
         return !(value instanceof GeometryVertexCountService geometryVertexCount)
            ? false
            : Arrays.equals(this.float2, geometryVertexCount.float2) && Arrays.equals(this.int2, geometryVertexCount.int2);
      }
   }
   ...
   public String toString() {
      return "MeshData[vertices=" + this.count + ", triangles=" + this.triangleCount() + ", bounds=" + this.boundingSphere2 + "]";
   }
   ...
   public static final class BoundingSphere {
      ...
      public Vector3f center(Vector3f vector3f) {
         return Objects.requireNonNull(vector3f, "destination").set(this.value4, this.value5, this.value6);
      }
      ...
      public boolean contains(float value, float currentValue, float nextValue) {
         ...
         return doubleValue * doubleValue + currentDoubleValue * currentDoubleValue + nextDoubleValue * nextDoubleValue <= (double)this.value7 * this.value7;
      }
