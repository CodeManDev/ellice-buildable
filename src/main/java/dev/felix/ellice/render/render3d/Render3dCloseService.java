...
final class Render3dCloseService implements AutoCloseable {
   ...
   private static final RhiBlendStateService.VertexLayout vertexLayout = RhiBlendStateService.VertexLayout.of(
      8, new RhiBlendStateService.VertexAttribute(0, 2, RhiBlendStateService.VertexFormat.FLOAT, 0L)
   );
   private static final RhiBlendStateService.DepthStencilState depthStencilState = new RhiBlendStateService.DepthStencilState(
      true, false, RhiBlendStateService.CompareOp.LESS_OR_EQUAL, false
   );
   private static final RhiBlendStateService.BlendState blendState = new RhiBlendStateService.BlendState(
      true,
   ...
   private static final ShaderDefinition shaderData2 = new ShaderDefinition(
      "ellice:selective-glow-mask", "render3d/glow_mask.vert", "render3d/glow_mask.frag"
   );
   private static final ShaderDefinition shaderData22 = new ShaderDefinition(
      "ellice:selective-glow-blur", "fullscreen.vert", "render3d/glow_blur.frag"
   );
   private static final ShaderDefinition shaderData23 = new ShaderDefinition(
   ...
   private RhiBlendStateService.TextureHandle textureHandle3 = RhiBlendStateService.TextureHandle.NONE;
   ...
   private RhiBlendStateService.DepthCopyTarget depthCopyTarget = RhiBlendStateService.DepthCopyTarget.NONE;
   private int count;
   private int count2;
   private int count3;
   private int count4;
   ...
   Render3dCloseService(RhiOperationHandler rhiOperation, ShaderOverrideRootService shaderOverrideRoot) {
      this.rhiOperationHandler = rhiOperation;
      this.shaderRecipeService = new ShaderRecipeService(
         rhiOperation,
         shaderOverrideRoot,
         new ShaderData(
            shaderData2,
      ...
      this.shaderRecipeService2 = new ShaderRecipeService(
         rhiOperation,
         shaderOverrideRoot,
         new ShaderData(
            shaderData22,
      ...
      this.shaderRecipeService3 = new ShaderRecipeService(
         rhiOperation,
         shaderOverrideRoot,
         new ShaderData(
            shaderData23,
   ...
   void render(List<Render3dCloseService.Draw> items, RhiCommandBuffer rhiCommandBuffer, RhiBlendStateService.FramebufferHandle framebufferHandle, Render3dViewService render3dView) {
      if (!this.enabled) {
         if (items.isEmpty()) {
            this.idle();
         } else {
            ...
            RhiBlendStateService.PipelineHandle currentPipelineHandle = this.shaderRecipeService2.resolve();
            RhiBlendStateService.PipelineHandle nextPipelineHandle = this.shaderRecipeService3.resolve();
            if (pipelineHandle.valid() && currentPipelineHandle.valid() && nextPipelineHandle.valid()) {
               if (this.checkCondition(framebufferHandle, render3dView.width(), render3dView.height())) {
                  ...
                  boolean currentWidth = this.rhiOperationHandler
                     .blitFramebufferDepth(
                        framebufferHandle, this.depthCopyTarget.framebuffer(), 0, 0, render3dView.width(), render3dView.height(), 0, 0, render3dView.width(), render3dView.height()
                     );
                  if (!currentWidth) {
                     ...
                     this.updateState3(rhiCommandBuffer, nextPipelineHandle, this.textureHandle2, framebufferHandle, render3dView);
                  }
               }
            }
         }
   ...
   private void updateState(List<Render3dCloseService.Draw> items, RhiCommandBuffer rhiCommandBuffer, RhiBlendStateService.PipelineHandle pipelineHandle, Render3dViewService render3dView) {
      ...
      for (Render3dCloseService.Draw currentDraw : items) {
         ...
         rhiCommandBuffer.pushVec4("uGlowColor", currentGlow.red(), currentGlow.green(), currentGlow.blue(), currentGlow.alpha());
   ...
   private void updateState3(
      ...
      try {
         updateState9(rhiCommandBuffer, "uInvProjection", render3dView.inverseProjection(), stack.mallocFloat(16));
      } catch (Throwable exception) {
         if (stack != null) {
            try {
               stack.close();
   ...
   private boolean checkCondition(RhiBlendStateService.FramebufferHandle currentFramebufferHandle, int value, int currentValue) {
      ...
      RhiBlendStateService.TextureHandle previousTextureHandle = RhiBlendStateService.TextureHandle.NONE;
      RhiBlendStateService.FramebufferHandle nextFramebufferHandle = RhiBlendStateService.FramebufferHandle.NONE;
      RhiBlendStateService.FramebufferHandle previousFramebufferHandle = RhiBlendStateService.FramebufferHandle.NONE;
      RhiBlendStateService.FramebufferHandle sourceFramebufferHandle = RhiBlendStateService.FramebufferHandle.NONE;
      RhiBlendStateService.DepthCopyTarget currentDepthCopyTarget = RhiBlendStateService.DepthCopyTarget.NONE;

      try {
         currentDepthCopyTarget = this.rhiOperationHandler.createCompatibleDepthCopyTarget(currentFramebufferHandle, nextValue, previousValue);
         ...
         currentTextureHandle = this.rhiOperationHandler
            .createTexture(RhiBlendStateService.TextureDescriptor.renderTarget(nextValue, previousValue, RhiBlendStateService.TextureFormat.RGBA16F));
         nextTextureHandle = this.rhiOperationHandler
            .createTexture(RhiBlendStateService.TextureDescriptor.renderTarget(sourceValue, targetValue, RhiBlendStateService.TextureFormat.RGBA16F));
         previousTextureHandle = this.rhiOperationHandler
            .createTexture(RhiBlendStateService.TextureDescriptor.renderTarget(sourceValue, targetValue, RhiBlendStateService.TextureFormat.RGBA16F));
         nextFramebufferHandle = this.rhiOperationHandler.createFramebuffer(currentTextureHandle, currentDepthCopyTarget.texture());
         ...
         this.updateState8(nextFramebufferHandle, previousFramebufferHandle, sourceFramebufferHandle, currentDepthCopyTarget, currentTextureHandle, nextTextureHandle, previousTextureHandle);
   ...
   private void updateState6(String text) {
      ...
      if (this.timestamp5 == 0L || offset - this.timestamp5 >= 5000000000L) {
         ...
         CoreIsInitializedHandler.LOGGER.warn("{}; base 3D material remains visible without glow", text);
      }
   }
   ...
   private static void updateState9(RhiCommandBuffer rhiCommandBuffer, String text, Matrix4f matrix4f, FloatBuffer floatBuffer) {
