...
public final class ShaderSetColorService {
   private final ConcurrentHashMap<String, ShaderSetColorService.UniformValue> text = new ConcurrentHashMap<>();
   private final ConcurrentHashMap<Integer, ShaderSetColorService.TextureBinding> entries = new ConcurrentHashMap<>();
   ...
   public ShaderSetColorService set(String currentText, float value, float currentValue) {
      this.text.put(createText(currentText), new ShaderSetColorService.Vec2Value(value, currentValue));
      return this;
   }

   public ShaderSetColorService set(String currentText, float value, float currentValue, float nextValue) {
      this.text.put(createText(currentText), new ShaderSetColorService.Vec3Value(value, currentValue, nextValue));
      return this;
   }

   public ShaderSetColorService set(String currentText, float value, float currentValue, float nextValue, float previousValue) {
      this.text.put(createText(currentText), new ShaderSetColorService.Vec4Value(value, currentValue, nextValue, previousValue));
      return this;
   }
   ...
   public ShaderSetColorService set(String text, Vector3f vector3f) {
      return vector3f == null ? this.remove(text) : this.set(text, vector3f.x, vector3f.y, vector3f.z);
   }

   public ShaderSetColorService set(String text, Vector4f vector4f) {
      return vector4f == null ? this.remove(text) : this.set(text, vector4f.x, vector4f.y, vector4f.z, vector4f.w);
   }
   ...
   public synchronized ShaderSetColorService texture(String text, RhiBlendStateService.TextureHandle textureHandle, int textureId) {
      return this.texture(text, textureHandle, RhiBlendStateService.SamplerHandle.NONE, textureId);
   }

   public synchronized ShaderSetColorService texture(String text, RhiBlendStateService.TextureHandle textureHandle, RhiBlendStateService.SamplerHandle samplerHandle, int textureId) {
      if (textureId >= 0 && textureId < 16) {
         String currentText = createText(text);
         RhiBlendStateService.TextureHandle currentTextureHandle = textureHandle == null ? RhiBlendStateService.TextureHandle.NONE : textureHandle;
         RhiBlendStateService.SamplerHandle currentSamplerHandle = samplerHandle == null ? RhiBlendStateService.SamplerHandle.NONE : samplerHandle;
         this.entries.entrySet().removeIf(entry -> entry.getKey() != textureId && entry.getValue().uniform().equals(currentText));
         this.entries.put(textureId, new ShaderSetColorService.TextureBinding(currentText, currentTextureHandle, currentSamplerHandle, textureId));
         return this;
      } else {
         throw new IllegalArgumentException("Texture unit out of portable range [0, 16): " + textureId);
      }
   }
   ...
   public void apply(RhiCommandBuffer rhiCommandBuffer) {
      for (Entry entry : this.text.entrySet()) {
         ((ShaderSetColorService.UniformValue)entry.getValue()).apply(rhiCommandBuffer, (String)entry.getKey());
      }
      ...
      for (ShaderSetColorService.TextureBinding textureBinding : (Iterable<ShaderSetColorService.TextureBinding>) (Iterable<?>) (arrayList)) {
         if (textureBinding.sampler().valid()) {
            rhiCommandBuffer.bindTexture(textureBinding.texture(), textureBinding.sampler(), textureBinding.unit());
         } else {
   ...
   private record TextureBinding(String uniform, RhiBlendStateService.TextureHandle texture, RhiBlendStateService.SamplerHandle sampler, int unit) {
   }
   ...
   private record Vec3Value(float x, float y, float z) implements ShaderSetColorService.UniformValue {
      @Override
      public void apply(RhiCommandBuffer rhiCommandBuffer, String text) {
   ...
   private record Vec4Value(float x, float y, float z, float w) implements ShaderSetColorService.UniformValue {
      @Override
      public void apply(RhiCommandBuffer rhiCommandBuffer, String text) {
