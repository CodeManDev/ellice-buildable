      ...
      this.updateState2(
          () -> {
            ...
            try {
              currentTextureHandle =
                  this.rhiOperationHandler.createTexture(
                      RhiBlendStateService.TextureDescriptor.renderTarget(
                          previousValue, sourceValue, RhiBlendStateService.TextureFormat.RGBA8));
              nextTextureHandle =
                  this.rhiOperationHandler.createTexture(
                      RhiBlendStateService.TextureDescriptor.renderTarget(
                          previousValue, sourceValue, RhiBlendStateService.TextureFormat.DEPTH24));
              ...
              rhiCommandBuffer.beginRenderPass(currentFramebufferHandle, 0.08F, 0.1F, 0.13F, 1.0F);
      ...
      if (render3dView.width() == this.count && render3dView.height() == this.count2) {
        this.updateState2(
            () -> {
              ...
              rhiCommandBuffer.clearScissor();
              rhiCommandBuffer.setColorWriteMask(true, true, true, true);
              rhiCommandBuffer.beginRenderPass(this.framebufferHandle, 0.08F, 0.1F, 0.13F, 1.0F);
              ...
              this.renderer2.render(
                  this.renderer, rhiCommandBuffer, this.framebufferHandle, render3dView);
            });
        this.enabled2 = true;
      } else {
  ...
  public void close() {
    if (!this.enabled) {
      ...
      try {
        ...
        LoggerFactory.getLogger(Render3dRenderer.class)
            .debug("Offscreen view GL release skipped without context", exception);
