...
public final class Render3dAddService {
  ...
  private volatile Vector3f vector3f = new Vector3f(0.42F, 0.48F, 0.62F);
  private volatile float value = 0.16F;

  public Render3dIdService add(
      GpuMesh gpuMesh, MaterialErFactory materialEr, GeometryXService geometryX) {
    Render3dIdService render3dId =
        new Render3dIdService(this.atomicLong.getAndIncrement(), gpuMesh, materialEr, geometryX);
  ...
  public Render3dAddService ambient(Vector3f currentVector3f, float currentValue) {
    if (currentVector3f != null
        && Float.isFinite(currentVector3f.x)
        && Float.isFinite(currentVector3f.y)
        && Float.isFinite(currentVector3f.z)
