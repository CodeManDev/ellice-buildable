...
public record KawaseBloomSettings(
   ...
   public static final KawaseBloomSettings NONE = new KawaseBloomSettings(false, 0, 0, 0.0F, 0.0F, 2, 0.0F, Render3dFeatureType.BOTH);
   ...
   public KawaseBloomSettings(int value, int currentValue, float nextValue, float previousValue, int sourceValue, float targetValue, Render3dFeatureType render3dFeatureType) {
      this(true, value, currentValue, nextValue, previousValue, sourceValue, targetValue, render3dFeatureType);
   }

   public boolean hasVisible() {
      return this.enabled && this.visibility.includesVisible() && calculateValue(this.visibleColor) != 0;
   }

   public boolean hasOccluded() {
      return this.enabled && this.visibility.includesOccluded() && calculateValue(this.occludedColor) != 0;
   }
   ...
   private static void updateState(String text, float value, float currentValue, float nextValue) {
      if (!Float.isFinite(value) || value < currentValue || value > nextValue) {
         throw new IllegalArgumentException(text + " must be finite and in [" + currentValue + ", " + nextValue + "]");
      }
   }
}
