...
public final class GeometryXService {
   ...
   public GeometryXService(double doubleValue, double currentDoubleValue, double nextDoubleValue, Quaternionf quaternionf, Vector3f vector3f) {
      this(doubleValue, currentDoubleValue, nextDoubleValue, (Quaternionfc)quaternionf, (Vector3fc)vector3f);
   }

   public GeometryXService(double doubleValue, double currentDoubleValue, double nextDoubleValue, Quaternionfc quaternionfc, Vector3fc vector3fc) {
      ...
      double currentX = (double)quaternionfc.x() * quaternionfc.x() + (double)quaternionfc.y() * quaternionfc.y() + (double)quaternionfc.z() * quaternionfc.z() + (double)quaternionfc.w() * quaternionfc.w();
      if (currentX <= 1.0E-20) {
   ...
   public Vector3d position(Vector3d vector3d) {
      return Objects.requireNonNull(vector3d, "destination").set(this.value2, this.value3, this.value4);
   }
   ...
   public GeometryXService translated(double doubleValue, double currentDoubleValue, double nextDoubleValue) {
      updateState(doubleValue, "deltaX");
      updateState(currentDoubleValue, "deltaY");
      updateState(nextDoubleValue, "deltaZ");
      return this.withPosition(this.value2 + doubleValue, this.value3 + currentDoubleValue, this.value4 + nextDoubleValue);
   }
   ...
   public Matrix4f modelMatrixRelativeTo(double doubleValue, double currentDoubleValue, double nextDoubleValue) {
      return this.modelMatrixRelativeTo(doubleValue, currentDoubleValue, nextDoubleValue, new Matrix4f());
   }

   public Matrix4f modelMatrixRelativeTo(double doubleValue, double currentDoubleValue, double nextDoubleValue, Matrix4f matrix4f) {
      ...
      return matrix4f.identity().translate(value, currentValue, nextValue).rotate(this.quaternionf).scale(this.vector3f);
   }

   public Matrix4f relativeModelMatrix(double doubleValue, double currentDoubleValue, double nextDoubleValue) {
   ...
   private static float calculateValue(double doubleValue, double currentDoubleValue, String text) {
      ...
      if (Double.isFinite(nextDoubleValue)
         ...
         throw new IllegalArgumentException("relative " + text + " translation is outside the finite float range");
      }
   }
   ...
   private static void updateState3(float value, String text) {
      ...
      if (value <= 0.0F) {
         throw new IllegalArgumentException(text + " must be positive; mirror geometry in the mesh instead");
      }
   }
