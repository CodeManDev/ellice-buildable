...
public final class ShaderRecipeService implements AutoCloseable {
   ...
   public ShaderRecipeService(RhiOperationHandler rhiOperation, ShaderOverrideRootService shaderOverrideRoot, ShaderData currentShaderData) {
   ...
   public RhiBlendStateService.PipelineHandle resolve() {
      ...
      RhiBlendStateService.PipelineHandle currentPipelineHandle = RhiBlendStateService.PipelineHandle.NONE;

      try {
         ShaderDefinition shaderDefinition = this.shaderData.shader();
         String currentText = this.shaderOverrideRootService.resolve(shaderDefinition.vertexAsset()).source();
         String nextText = this.shaderOverrideRootService.resolve(shaderDefinition.fragmentAsset()).source();
         shaderHandle = this.rhiOperationHandler.createShader(RhiBlendStateService.ShaderStage.VERTEX, currentText);
         currentShaderHandle = this.rhiOperationHandler.createShader(RhiBlendStateService.ShaderStage.FRAGMENT, nextText);
         currentPipelineHandle = this.rhiOperationHandler
            .createPipeline(
         ...
         CoreIsInitializedHandler.LOGGER.info("Shader pipeline '{}' active at revision {}", shaderDefinition.id(), longValue);
      } catch (RuntimeException exception) {
         this.text = exception.getMessage() == null ? exception.getClass().getSimpleName() : exception.getMessage();
         CoreIsInitializedHandler.LOGGER
            .error("Shader reload failed for '{}'; keeping last good pipeline: {}", this.shaderData.shader().id(), this.text);
      } finally {
