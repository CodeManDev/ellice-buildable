...
public final class GeometryCubeService {
   ...
   public static GeometryVertexCountService plane(float value, float currentValue) {
      ...
      if (Float.isFinite(nextValue) && nextValue != 0.0F && Float.isFinite(previousValue) && previousValue != 0.0F) {
         float[] floats = new float[48];
         int sourceValue = 0;
         sourceValue = calculateValue3(
            floats, sourceValue, -nextValue, 0.0F, -previousValue, 0.0F, 1.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F, 0.0F, -1.0F
         );
         sourceValue = calculateValue3(
            floats, sourceValue, -nextValue, 0.0F, previousValue, 0.0F, 1.0F, 0.0F, 0.0F, 1.0F, 1.0F, 0.0F, 0.0F, -1.0F
         );
         sourceValue = calculateValue3(
            floats, sourceValue, nextValue, 0.0F, previousValue, 0.0F, 1.0F, 0.0F, 1.0F, 1.0F, 1.0F, 0.0F, 0.0F, -1.0F
         );
         calculateValue3(floats, sourceValue, nextValue, 0.0F, -previousValue, 0.0F, 1.0F, 0.0F, 1.0F, 0.0F, 1.0F, 0.0F, 0.0F, -1.0F);
   ...
   public static GeometryVertexCountService heart(float value, float currentValue, float nextValue, int previousValue, int sourceValue) {
      updateState(value, "width");
      ...
      if (Float.isFinite(targetValue) && targetValue != 0.0F && Float.isFinite(inputValue) && inputValue != 0.0F && Float.isFinite(outputValue) && outputValue != 0.0F) {
         ...
         if (currentLongValue <= 178956970L && nextLongValue <= 2147483647L) {
            ...
            for (int index = 0; index < previousValue; index++) {
               ...
               double outputDoubleValue = 16.0 * targetDoubleValue * targetDoubleValue * targetDoubleValue
                  + 0.8 * targetDoubleValue;
               double resultDoubleValue = 13.0 * inputDoubleValue
            ...
            for (int currentIndex = 0; currentIndex <= previousValue; currentIndex++) {
               ...
               double pendingDoubleValue = 16.0 * computedDoubleValue * computedDoubleValue * computedDoubleValue
                  + 0.8 * computedDoubleValue;
               double activeDoubleValue = 13.0 * cachedDoubleValue
               ...
               currentDoubles[currentIndex] = (activeDoubleValue - initialDoubleValue) * selectedDoubleValue;
            ...
            for (int nextIndex = 0; nextIndex < previousValue; nextIndex++) {
               ...
               double[] sourceDoubles = createDouble(fallbackDoubleValue, candidateDoubleValue, selectedDoubleValue);
               candidateValue = calculateValue3(
                  floats,
                  candidateValue,
                  0.0F,
                  0.0F,
            ...
            double primaryDoubleValue = Math.sin(
               3.141592653589793 * Math.floor(sourceValue * 0.5) / sourceValue
            );
            ...
            for (int previousIndex = 1; previousIndex < sourceValue; previousIndex++) {
               ...
               for (int sourceIndex = 0; sourceIndex <= previousValue; sourceIndex++) {
                  ...
                  double localDoubleValue = outputValue * secondaryDoubleValue * requestedDoubleValue * requestedDoubleValue * startDoubleValue;
                  double storedDoubleValue = -outputValue * secondaryDoubleValue * requestedDoubleValue * requestedDoubleValue * maximumDoubleValue;
                  double createdDoubleValue = secondaryDoubleValue * secondaryDoubleValue * requestedDoubleValue * actualDoubleValue * (expectedDoubleValue * startDoubleValue - minimumDoubleValue * maximumDoubleValue);
                  double updatedDoubleValue = calculateValue(localDoubleValue, storedDoubleValue, createdDoubleValue);
            ...
            for (int targetIndex = 0; targetIndex < previousValue; targetIndex++) {
               ...
               double[] targetDoubles = createDouble(firstDoubleValue, candidateDoubleValue, selectedDoubleValue);
               candidateValue = calculateValue3(
                  floats,
                  candidateValue,
                  0.0F,
   ...
   private static double[] createDouble(double doubleValue, double currentDoubleValue, double nextDoubleValue) {
      double previousDoubleValue = -6.283185307179586 * doubleValue;
      double sourceDoubleValue = Math.sin(previousDoubleValue);
      double targetDoubleValue = Math.cos(previousDoubleValue);
      double inputDoubleValue = -(48.0 * sourceDoubleValue * sourceDoubleValue + 0.8)
         * targetDoubleValue
         * currentDoubleValue;
      ...
      return new double[]{inputDoubleValue * resultDoubleValue, outputDoubleValue * resultDoubleValue};
   }

   private static double calculateValue(double doubleValue, double currentDoubleValue, double nextDoubleValue) {
      double previousDoubleValue = doubleValue * doubleValue + currentDoubleValue * currentDoubleValue + nextDoubleValue * nextDoubleValue;
   ...
   private static int calculateValue2(
      ...
      value = calculateValue3(floats, value, currentValue, nextValue, previousValue, resolvedValue, computedValue, cachedValue, 0.0F, 0.0F, pendingValue, activeValue, fallbackValue, primaryValue);
      value = calculateValue3(floats, value, sourceValue, targetValue, inputValue, resolvedValue, computedValue, cachedValue, 1.0F, 0.0F, pendingValue, activeValue, fallbackValue, primaryValue);
      value = calculateValue3(floats, value, outputValue, resultValue, candidateValue, resolvedValue, computedValue, cachedValue, 1.0F, 1.0F, pendingValue, activeValue, fallbackValue, primaryValue);
      return calculateValue3(floats, value, selectedValue, defaultValue, initialValue, resolvedValue, computedValue, cachedValue, 0.0F, 1.0F, pendingValue, activeValue, fallbackValue, primaryValue);
   }
