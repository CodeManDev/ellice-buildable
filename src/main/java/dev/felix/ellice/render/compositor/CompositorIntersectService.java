...
public final class CompositorIntersectService {
   ...
   public static float[] intersect(
      float[] floats, float value, float currentValue, float nextValue, float previousValue, float sourceValue, float targetValue, float inputValue, float outputValue
   ) {
      ...
      if (nextValue > 0.0F && previousValue > 0.0F && Math.max(Math.max(resolvedValue, computedValue), Math.max(cachedValue, pendingValue)) > 0.0F) {
         updateState(currentFloats, value, currentValue, nextValue, previousValue, resolvedValue, computedValue, cachedValue, pendingValue);
      }

      return currentFloats;
   }

   public static void apply(RhiCommandBuffer rhiCommandBuffer, float[] floats, float value, int currentValue) {
      ...
      if (floats == null) {
         ...
         rhiCommandBuffer.setScissor(previousValue, currentValue - inputValue, Math.max(0, sourceValue - previousValue), Math.max(0, inputValue - targetValue));
         ...
         for (int index = 0; index < outputValue; index++) {
            ...
            rhiCommandBuffer.pushVec4(
               "uClipRects[" + index + "]", floats[currentIndex] * nextValue, floats[currentIndex + 1] * nextValue, floats[currentIndex + 2] * nextValue, floats[currentIndex + 3] * nextValue
            );
            rhiCommandBuffer.pushVec4(
               "uClipRadii[" + index + "]", floats[currentIndex + 4] * nextValue, floats[currentIndex + 5] * nextValue, floats[currentIndex + 6] * nextValue, floats[currentIndex + 7] * nextValue
            );
         }
      }
   }
   ...
   public static boolean same(float[] floats, float[] currentFloats) {
      return floats == currentFloats || floats != null && currentFloats != null && Arrays.equals(floats, currentFloats);
   }

   public static int roundedMaskCount(float[] floats) {
      return floats != null && floats.length > 4 ? Math.max(0, Math.min(8, Math.round(floats[4]))) : 0;
   }
