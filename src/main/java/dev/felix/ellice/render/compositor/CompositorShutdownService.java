...
public class CompositorShutdownService {
  ...
  private final RhiBlendStateService.TextureHandle[] textureHandle =
      new RhiBlendStateService.TextureHandle[5];
  private final RhiBlendStateService.FramebufferHandle[] framebufferHandle =
      new RhiBlendStateService.FramebufferHandle[5];
  ...
  public CompositorShutdownService(
      RhiOperationHandler rhiOperation, RhiBlendStateService.BufferHandle currentBufferHandle) {
  ...
  public void execute(
    ...
    if (this.enabled) {
      ...
      if (!themeBlurQualityData.blurEnabled()) {
        ...
        if (value != this.count2
            || currentValue != this.count3
        ...
        for (int currentIndex = this.blurQuality2.levels() - 1;
            currentIndex > 0;
            currentIndex += -1) {
          rhiCommandBuffer.beginRenderPass(this.framebufferHandle[currentIndex - 1]);
          rhiCommandBuffer.setViewport(
              0, 0, this.int2[currentIndex - 1], this.int3[currentIndex - 1]);
  ...
  private void updateState(int value, int currentValue) {
    ...
    for (int index = 0; index < this.blurQuality2.levels(); index++) {
      ...
      this.framebufferHandle[index] =
          this.rhiOperationHandler.createFramebuffer(this.textureHandle[index]);
