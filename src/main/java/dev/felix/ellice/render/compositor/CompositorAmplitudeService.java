...
public final class CompositorAmplitudeService {
   ...
   public static final CompositorAmplitudeService EMPTY = new CompositorAmplitudeService(new float[0]);
   ...
   public CompositorAmplitudeService(float[] floats) {
      if (floats != null && floats.length != 0) {
         for (int index = 0; index < 512; index++) {
            ...
            for (int currentIndex = currentLength; currentIndex < Math.min(nextLength, floats.length); currentIndex++) {
               float value = floats[currentIndex];
               if (Float.isFinite(value)) {
                  this.float2[index] = Math.max(this.float2[index], Math.max(0.0F, Math.min(1.0F, value)));
               }
            }
         }
      }
