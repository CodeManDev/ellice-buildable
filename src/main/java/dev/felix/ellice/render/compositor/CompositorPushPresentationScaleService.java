...
public class CompositorPushPresentationScaleService {
  ...
  private static final String[] text2 =
      IntStream.range(0, 40).mapToObj(item -> "uPoints[" + item + "]").toArray(String[]::new);
  private CompositorAmplitudeService compositorAmplitudeService;
  private static final String[] text3 =
      IntStream.range(0, 128).mapToObj(item -> "uSamples[" + item + "]").toArray(String[]::new);
  ...
  private final ConcurrentLinkedQueue<CompositorPushPresentationScaleService.PendingFont>
  ...
  private final List<CompositorPushPresentationScaleService.ColorPickerCmd> items6 =
      new ArrayList<>();
  private final List<CompositorPushPresentationScaleService.CurveGraphCmd> items7 =
      new ArrayList<>();
  private final List<CompositorPushPresentationScaleService.WaveformCmd> items8 = new ArrayList<>();
  private final List<CompositorPushPresentationScaleService.TimeSeriesCmd> items9 =
      new ArrayList<>();
  ...
  private final List<CompositorPushPresentationScaleService.LiquidSwitchCmd> items12 =
      new ArrayList<>();
  private final List<CompositorPushPresentationScaleService.TooltipBubbleCmd> items13 =
      new ArrayList<>();
  ...
  private final List<CompositorPushPresentationScaleService.LayerEffect> items31 =
      new ArrayList<>();
  private final Map<Integer, CompositorPushPresentationScaleService.LayerCache> entries2 =
      new HashMap<>();
  ...
  public static final CompositorPushPresentationScaleService.CurveGraphStyle
      DEFAULT_CURVE_GRAPH_STYLE =
  ...
  public void roundedRect(
      float x, float y, float width, float height, float value, int currentValue) {
    this.updateState3(
        x,
        y,
        width,
  ...
  public void roundedRect(
      float x, float y, float width, float height, float value, int currentValue, float nextValue) {
    this.updateState3(
        x,
        y,
        width,
  ...
  public void gradientRect(
      float x, float y, float width, float height, int value, int currentValue) {
    this.items5.add(
        new CompositorPushPresentationScaleService.GradCmd(
            x, y, width, height, value, currentValue, this.float3));
  }
  ...
  public void curveGraph(
    ...
    if (curveValue != null
      ...
      if (!(pendingValue <= 0.0F) && !(activeValue <= 0.0F)) {
        CompositorPushPresentationScaleService.CurveGraphStyle currentCurveGraphStyle =
            curveGraphStyle != null ? curveGraphStyle : DEFAULT_CURVE_GRAPH_STYLE;
        this.items7.add(
  ...
  float[] queuedCurveGraphPlot(int value) {
    ...
    return new float[] {
      curveGraphCmd.plotX, curveGraphCmd.plotY, curveGraphCmd.plotW, curveGraphCmd.plotH
    };
  }
  ...
  public void studioPrimitive(
      ...
      int inputValue,
      float outputValue,
      CompositorPushPresentationScaleService.StudioPaint studioPaint) {
    if (studioPaint != null
        && studioPaint.kind >= 0
        && studioPaint.kind <= 6
        && Float.isFinite(value)
      ...
      if (!ThemeBlurQualityData.current().animatedBackground()) {
        ...
        this.items11.add(
            new CompositorPushPresentationScaleService.MeshCmd(
                ...
                this.float3,
                null,
                new CompositorPushPresentationScaleService.MenuPaint(
                    targetValue,
                    outputValue,
                    resultValue,
    ...
    if (!(resultOpacity <= 0.001F)) {
      this.items6.add(
          new CompositorPushPresentationScaleService.ColorPickerCmd(
              ...
              targetOpacity,
              inputOpacity,
              this.calculateValue(outputOpacity),
              4,
              resultOpacity,
              this.float3));
    }
  ...
  public boolean isLayerCacheValid(
      int value,
      int currentValue,
      float nextValue,
      float previousValue,
  ...
  public void rect(float x, float y, float width, float height, int value) {
    this.updateState3(
        x, y, width, height, 0.0F, 0.0F, 0.0F, 0.0F, value, 0.0F, 0.0F, 1073741824, 0.0F, 0);
  }
  ...
  public float textWidthLiteral(
      String id, float value, TextMode textMode, String currentId, float currentValue) {
    this.count12++;
  ...
  public void text(
      float value, float currentValue, String currentText, float nextValue, int previousValue) {
    if (this.textTextService != null) {
      this.textTextService.text(
          value,
          currentValue,
          this.createText(currentText),
          this.calculateValue(nextValue),
          previousValue,
          TextData.NONE,
          this.float3);
  ...
  public float textWidth(String id, float value) {
    return this.textTextService != null
        ? this.textTextService.measureWidth(this.createText(id), this.calculateValue(value))
        : 0.0F;
  }

  public float textWidth(String id, float value, TextMode textMode) {
    return this.textTextService != null
        ? this.textTextService.measureWidth(
            this.createText(id), this.calculateValue(value), textMode)
        : 0.0F;
  }
  ...
  public float textWidth(String id, float value, TextMode textMode, String currentId) {
    return this.textTextService != null
        ? this.textTextService.measureWidth(
            this.createText(id), this.calculateValue(value), textMode, currentId)
        : 0.0F;
  }

  private TextData createTextData(TextData textData) {
    TextData currentTextData = textData != null ? textData : TextData.NONE;
    return this.value4 != 1.0F
            || currentTextData.hasShadow()
                && ThemeBlurQualityData.current().shadowStrength() != 1.0F
        ? new TextData(
    ...
    if (this.textTextService == null) {
      ...
      for (String nextId : id.split("\n", -1)) {
        nextValue =
            Math.max(
                nextValue,
                this.textWidth(nextId, value, textMode, currentId)
                    + TextTextService.trackingAdvance(
                        nextId, nextId.length(), this.calculateValue(currentValue)));
      }
  ...
  public int fontMetricsRevision() {
    return this.count13
        + (this.textTextService != null ? 1 : 0)
        + PrivacyRevisionService.revision();
  }
  ...
  public void requestFontFamily(
      String text, TextTextureService.FontSource fontSource, String currentText) {
              ...
              () -> {
                try {
                  TextTextureService textTexture =
                      fontSource == TextTextureService.FontSource.CLASSPATH
                  ...
                  this.concurrentLinkedQueue.add(
                      new CompositorPushPresentationScaleService.PendingFont(text, textTexture));
                } catch (Throwable exception) {
                  CoreIsInitializedHandler.LOGGER.error(
                      "Font rasterize failed for {} ({})",
                      new Object[] {text, currentText, exception});
    ...
    if (!this.concurrentLinkedQueue.isEmpty() && this.textTextService != null) {
      ...
      while ((pendingFont = this.concurrentLinkedQueue.poll()) != null) {
        try {
          ...
          CoreIsInitializedHandler.LOGGER.error(
              "Font upload failed for {}", pendingFont.key, exception);
        } finally {
  ...
  public void addLoading(
      ...
      float outputValue) {
    this.items14.add(
        new CompositorPushPresentationScaleService.LoadingCmd(
            value,
            currentValue,
            nextValue,
            previousValue,
  ...
  public float textLineHeight(float value, TextMode textMode, String text) {
    if (text == null) {
      ...
      return this.textTextService != null
          ? this.textTextService.lineHeight(this.calculateValue(value), textMode, text)
          : this.calculateValue(value);
    }
  }
  ...
  public float textLineHeight(float value) {
    return this.textTextService != null
        ? this.textTextService.lineHeight(this.calculateValue(value))
        : this.calculateValue(value);
  }

  public float textLineHeight(float value, TextMode textMode) {
    return this.textTextService != null
        ? this.textTextService.lineHeight(this.calculateValue(value), textMode)
        : this.calculateValue(value);
  }

  private PrivacyRevisionService.Mapping createMap(String text) {
    return this.count12 == 0
        ? PrivacyRevisionService.map(text)
        : new PrivacyRevisionService.Mapping(text, text, List.of());
  }

  public float textCaretX(
      String currentText, int value, float currentValue, TextMode textMode, String nextText) {
    PrivacyRevisionService.Mapping mapping = this.createMap(currentText);
    return this.textTextService == null
        ? 0.0F
        : this.textTextService.caretX(
            mapping.text(),
            mapping.displayOffset(value),
            this.calculateValue(currentValue),
            textMode,
            nextText);
  }
  ...
  public float textCaretX(
    ...
    try {
      return this.textCaretX(mapping.text(), previousValue, currentValue, textMode, nextText)
          + TextTextService.trackingAdvance(
              mapping.text(), previousValue, this.calculateValue(nextValue));
    } finally {
      this.count12--;
    }
  }

  public float[] textSelectionSpans(
      String text,
      int value,
      int currentValue,
      float nextValue,
    ...
    if (previousValue != 0.0F
        && text != null
        && TextTextService.trackingAdvance(
                this.createText(text), this.createText(text).length(), previousValue)
            != 0.0F) {
      int currentLength = Math.max(0, Math.min(text.length(), Math.min(value, currentValue)));
      int nextLength =
          Math.max(currentLength, Math.min(text.length(), Math.max(value, currentValue)));
      if (currentLength == nextLength) {
        return new float[0];
      }

      float sourceValue =
          this.textCaretX(text, currentLength, nextValue, textMode, currentText, previousValue);
      float targetValue =
          this.textCaretX(text, nextLength, nextValue, textMode, currentText, previousValue);
  ...
  public float[] textSelectionSpans(
    ...
    return this.textTextService == null
        ? new float[0]
        : this.textTextService.selectionSpans(
            mapping.text(),
            mapping.displayOffset(value),
            mapping.displayOffset(currentValue),
            this.calculateValue(nextValue),
            textMode,
            nextText);
  }
  ...
  public float[] textSelectionSpans(String text, int value, int currentValue, float nextValue) {
    return this.textSelectionSpans(
        text, value, currentValue, nextValue, TextMode.REGULAR, null, 0.0F);
  }

  public float[] textSelectionSpans(
      String text, int value, int currentValue, float nextValue, TextMode textMode) {
  ...
  public void drawTexture(
      ...
      int textureId) {
    this.drawTexture(
        textureHandle, y, width, height, value, currentValue, nextValue, textureId, 0.0F);
  }
  ...
  public void drawMorphSdfTexture(
    ...
    if (textureHandle != null
        && textureHandle.valid()
        && currentTextureHandle != null
        && currentTextureHandle.valid()
        && textureId >>> 24 != 0) {
      if (Float.isFinite(width)
        ...
        this.items4.add(
            new CompositorPushPresentationScaleService.SdfIconCmd(
                ...
                Math.max(0.0F, Math.min(1.0F, width)),
                height,
                value,
                currentValue,
                nextValue,
  ...
  public void registerImageTexture(String text, BufferedImage bufferedImage) {
    if (this.rhiOperationHandler != null && bufferedImage != null) {
      ...
      if (width > 0 && height > 0) {
        ...
        RhiBlendStateService.TextureHandle currentTextureHandle =
            this.uploadImageTexture(RhiBlendStateService.TextureHandle.NONE, bufferedImage);
  ...
  public RhiBlendStateService.TextureHandle uploadImageTexture(
  ...
  public RhiBlendStateService.TextureHandle uploadRgbaTexture(
      RhiBlendStateService.TextureHandle textureHandle,
      int textureId,
      ...
      if (longValue <= 2147483647L && byteBuffer.remaining() >= longValue) {
        ...
        if (textureHandle != null && textureHandle.valid()) {
          this.rhiOperationHandler.updateTexture(
              textureHandle, 0, 0, textureId, currentTextureId, currentByteBuffer);
        ...
        CoreIsInitializedHandler.LOGGER.warn(
            "Rejected RGBA upload {}x{} (buffer too small)", textureId, currentTextureId);
        return textureHandle != null ? textureHandle : RhiBlendStateService.TextureHandle.NONE;
      }
    } else {
      return textureHandle != null ? textureHandle : RhiBlendStateService.TextureHandle.NONE;
      ...
      if (value != 0) {
        ...
        try {
          ...
          RhiBlendStateService.FramebufferHandle framebufferHandle =
              new RhiBlendStateService.FramebufferHandle(height);
          ...
          if (this.value3 > 0.001F && this.renderer2 != null) {
            ...
            this.renderer2.renderBackgroundDesaturate(
                rhiCommandBuffer, framebufferHandle, x, y, this.value3);
          ...
          for (int index = 0; index < currentSize; index++) {
            ...
            int nextSize =
                index < this.items15.size() ? this.items15.get(index) : this.items.size();
            int previousValue = index > 0 ? this.items16.get(index - 1) : 0;
            int previousSize =
                index < this.items16.size() ? this.items16.get(index) : this.items2.size();
            int sourceValue = index > 0 ? this.items17.get(index - 1) : 0;
            int sourceSize =
                index < this.items17.size() ? this.items17.get(index) : this.items3.size();
            int targetValue = index > 0 ? this.items18.get(index - 1) : 0;
            int targetSize =
                index < this.items18.size() ? this.items18.get(index) : this.items4.size();
            ...
            int outputSize =
                index < this.items20.size() ? this.items20.get(index) : this.items5.size();
            int resultValue = index > 0 ? this.items21.get(index - 1) : 0;
            int resultSize =
                index < this.items21.size() ? this.items21.get(index) : this.items6.size();
            int candidateValue = index > 0 ? this.items22.get(index - 1) : 0;
            int candidateSize =
                index < this.items22.size() ? this.items22.get(index) : this.items7.size();
            int selectedValue = index > 0 ? this.items23.get(index - 1) : 0;
            int selectedSize =
                index < this.items23.size() ? this.items23.get(index) : this.items8.size();
            int defaultValue = index > 0 ? this.items24.get(index - 1) : 0;
            int defaultSize =
                index < this.items24.size() ? this.items24.get(index) : this.items9.size();
            int initialValue = index > 0 ? this.items25.get(index - 1) : 0;
            int initialSize =
                index < this.items25.size() ? this.items25.get(index) : this.items10.size();
            int resolvedValue = index > 0 ? this.items26.get(index - 1) : 0;
            int resolvedSize =
                index < this.items26.size() ? this.items26.get(index) : this.items11.size();
            int computedValue = index > 0 ? this.items27.get(index - 1) : 0;
            int computedSize =
                index < this.items27.size() ? this.items27.get(index) : this.items12.size();
            int cachedValue = index > 0 ? this.items28.get(index - 1) : 0;
            int cachedSize =
                index < this.items28.size() ? this.items28.get(index) : this.items13.size();
            int pendingValue = index > 0 ? this.items29.get(index - 1) : 0;
            int pendingSize =
                index < this.items29.size()
            ...
            CompositorPushPresentationScaleService.LayerRanges layerRanges =
                new CompositorPushPresentationScaleService.LayerRanges(
                    nextValue,
                    nextSize,
                    previousValue,
                    previousSize,
                    ...
                    pendingValue,
                    pendingSize);
            CompositorPushPresentationScaleService.LayerEffect activeSize =
                index < this.items31.size() ? this.items31.get(index) : null;
            if (activeSize != null && this.pipelineHandle6 != null) {
              rhiCommandBuffer.endRenderPass();
              this.updateState7(
                  rhiCommandBuffer, framebufferHandle, x, y, width, layerRanges, activeSize);
            } else {
          ...
          if (this.svgQueueService != null && this.svgQueueService.cmdCount() > activeValue) {
            this.svgQueueService.flushRange(
                rhiCommandBuffer, x, y, width, activeValue, this.svgQueueService.cmdCount());
          }
          ...
          try {
            ...
            CoreIsInitializedHandler.LOGGER.error(
                "Compositor restoreGLState failed", nextException);
          }
  ...
  public RhiBlendStateService.TextureHandle captureActiveFramebuffer() {
    if (this.enabled3
      ...
      this.rhiOperationHandler2.blitFramebuffer(
        ...
        this.textureHandle =
            this.rhiOperationHandler.createTexture(
                RhiBlendStateService.TextureDescriptor.renderTarget(
                    value, currentValue, RhiBlendStateService.TextureFormat.RGBA8));
  ...
  private CompositorPushPresentationScaleService.LayerCache createLayerCache(
      int value, int currentValue, int nextValue) {
    CompositorPushPresentationScaleService.LayerCache layerCache =
        this.entries2.computeIfAbsent(
            value, item -> new CompositorPushPresentationScaleService.LayerCache());
    ...
    layerCache.texture =
        this.rhiOperationHandler.createTexture(
            RhiBlendStateService.TextureDescriptor.renderTarget(
                currentValue, nextValue, RhiBlendStateService.TextureFormat.RGBA8));
  ...
  private void updateState7(
    ...
    if (layerEffect.cacheKey() == 0) {
      ...
      if (this.textureHandle2.valid() && this.framebufferHandle2.valid()) {
        ...
        rhiCommandBuffer.clearScissor();
        this.updateState9(
            rhiCommandBuffer, value, currentValue, doubleValue, layerEffect, this.textureHandle2);
      } else {
        rhiCommandBuffer.beginRenderPass(framebufferHandle);
        rhiCommandBuffer.setViewport(0, 0, value, currentValue);
        this.updateState8(rhiCommandBuffer, value, currentValue, doubleValue, layerRanges);
      ...
      CompositorPushPresentationScaleService.LayerCache layerCache =
          this.createLayerCache(layerEffect.cacheKey(), value, currentValue);
      ...
      rhiCommandBuffer.clearScissor();
      this.updateState9(
          rhiCommandBuffer, value, currentValue, doubleValue, layerEffect, layerCache.texture);
    }
  }
  ...
  private void updateState8(
    ...
    if (layerRanges.glassStart() < layerRanges.glassEnd()
      ...
      while (nextValue < layerRanges.rectEnd()
        ...
        if (index < layerRanges.glassEnd()) {
          previousValue =
              Math.min(previousValue, Math.max(nextValue, this.items2.get(index).rectIdx));
        }

        if (currentIndex < layerRanges.meshEnd()) {
          previousValue =
              Math.min(previousValue, Math.max(nextValue, this.items11.get(currentIndex).rectIdx));
        }

        if (nextIndex < layerRanges.liquidSwitchEnd()) {
          previousValue =
              Math.min(previousValue, Math.max(nextValue, this.items12.get(nextIndex).rectIdx));
        }

        if (previousIndex < layerRanges.tooltipBubbleEnd()) {
          previousValue =
              Math.min(previousValue, Math.max(nextValue, this.items13.get(previousIndex).rectIdx));
        }

        if (sourceIndex < layerRanges.curveGraphEnd()) {
          previousValue =
              Math.min(previousValue, Math.max(nextValue, this.items7.get(sourceIndex).rectIdx));
        }

        if (targetIndex < layerRanges.waveformEnd()) {
          previousValue =
              Math.min(previousValue, Math.max(nextValue, this.items8.get(targetIndex).rectIdx));
        }

        if (inputIndex < layerRanges.timeSeriesEnd()) {
          previousValue =
              Math.min(previousValue, Math.max(nextValue, this.items9.get(inputIndex).rectIdx));
        }
        ...
        if (previousValue > nextValue) {
          ...
          if (this.enabled2) {
            this.updateState13(
                rhiCommandBuffer, value, currentValue, doubleValue, nextValue, sourceValue);
          } else {
            this.updateState15(
                rhiCommandBuffer, value, currentValue, doubleValue, nextValue, sourceValue);
          }
          ...
          for (byteValue = 0;
              index < layerRanges.glassEnd()
                  && (nextValue >= layerRanges.rectEnd()
                      || this.items2.get(index).rectIdx <= nextValue);
              byteValue = 1) {
            if (this.pipelineHandle5 != null) {
              this.updateState17(
                  rhiCommandBuffer, value, currentValue, doubleValue, index, index + 1);
            }

            index++;
          }

          while (currentIndex < layerRanges.meshEnd()
              && (nextValue >= layerRanges.rectEnd()
                  || this.items11.get(currentIndex).rectIdx <= nextValue)) {
          ...
          while (nextIndex < layerRanges.liquidSwitchEnd()
              && (nextValue >= layerRanges.rectEnd()
                  || this.items12.get(nextIndex).rectIdx <= nextValue)) {
            if (this.pipelineHandle12 != null) {
              this.updateState24(
                  rhiCommandBuffer, value, currentValue, doubleValue, nextIndex, nextIndex + 1);
            }
          ...
          while (previousIndex < layerRanges.tooltipBubbleEnd()
              && (nextValue >= layerRanges.rectEnd()
                  || this.items13.get(previousIndex).rectIdx <= nextValue)) {
          ...
          while (sourceIndex < layerRanges.curveGraphEnd()
              && (nextValue >= layerRanges.rectEnd()
                  || this.items7.get(sourceIndex).rectIdx <= nextValue)) {
            if (this.pipelineHandle15 != null) {
              this.updateState28(
                  rhiCommandBuffer, value, currentValue, doubleValue, sourceIndex, sourceIndex + 1);
            }
          ...
          while (targetIndex < layerRanges.waveformEnd()
              && (nextValue >= layerRanges.rectEnd()
                  || this.items8.get(targetIndex).rectIdx <= nextValue)) {
            if (this.pipelineHandle16 != null) {
              this.updateState30(
                  rhiCommandBuffer, value, currentValue, doubleValue, targetIndex, targetIndex + 1);
            }
          ...
          while (inputIndex < layerRanges.timeSeriesEnd()
              && (nextValue >= layerRanges.rectEnd()
                  || this.items9.get(inputIndex).rectIdx <= nextValue)) {
            if (this.pipelineHandle17 != null) {
              this.updateState29(
                  rhiCommandBuffer, value, currentValue, doubleValue, inputIndex, inputIndex + 1);
            }
          ...
          if (byteValue == 0) {
            ...
            if (this.enabled2) {
              this.updateState13(
                  rhiCommandBuffer, value, currentValue, doubleValue, nextValue, targetValue);
            } else {
              this.updateState15(
                  rhiCommandBuffer, value, currentValue, doubleValue, nextValue, targetValue);
            }
    ...
    if (this.textTextService != null && layerRanges.txtStart() < layerRanges.txtEnd()) {
      DiagnosticsAddFrameListenerService.get().begin("text");
      this.textTextService.flushRange(
          rhiCommandBuffer,
          value,
          currentValue,
          doubleValue,
          layerRanges.txtStart(),
          layerRanges.txtEnd());
      DiagnosticsAddFrameListenerService.get().end();
    }
  ...
  private void updateState9(
    ...
    int selectedValue =
        layerEffect.material() != 0 && this.pipelineHandle7 != null && this.pipelineHandle7.valid()
            ? 1
            : 0;
    ...
    rhiCommandBuffer.pushVec4(
        "uRadius",
        layerEffect.tl() * previousValue,
        layerEffect.tr() * previousValue,
        layerEffect.br() * previousValue,
        layerEffect.bl() * previousValue);
    ...
    rhiCommandBuffer.pushFloat(
        "uBlurRadius", ThemeBlurQualityData.current().blur(layerEffect.blur()) * previousValue);
    ...
    if (selectedValue != 0) {
      ...
      rhiCommandBuffer.pushVec4(
          "uMouse", defaultValue, initialValue, this.enabled5 ? 1.0F : 0.0F, resolvedValue);
      rhiCommandBuffer.pushVec4(
          "uMaterialParams",
          layerEffect.materialIntensity(),
          layerEffect.materialScale(),
          layerEffect.materialSpeed(),
          layerEffect.materialDistortion());
      rhiCommandBuffer.pushVec4(
          "uMaterialState",
          layerEffect.materialGlow(),
          layerEffect.hoverProgress(),
          layerEffect.pressProgress(),
          (float) (this.timestamp % 100000L));
  ...
  private static boolean checkCondition2(
      ...
      float nextValue,
      float previousValue) {
    return checkCondition3(layerCache.x, value)
        && checkCondition3(layerCache.y, currentValue)
        && checkCondition3(layerCache.w, nextValue)
        && checkCondition3(layerCache.h, previousValue);
  }
  ...
  private void updateState10() {
    if (this.timestamp % 300L == 0L) {
      ...
      while (currentIterator.hasNext()) {
        CompositorPushPresentationScaleService.LayerCache layerCache =
            (CompositorPushPresentationScaleService.LayerCache)
                ((Entry) currentIterator.next()).getValue();
  ...
  private boolean checkCondition4() {
    return this.enabled6
        && this.compositorShutdownService != null
        && this.compositorShutdownService.texture() != null
        && this.compositorShutdownService.texture().valid();
  }
  ...
  private void updateState11(
    ...
    while (previousValue < currentSize) {
      ...
      while (currentIndex < currentSize
          && checkCondition5(this.items.get(currentIndex).clip(), currentFloats)) {
        currentIndex++;
      }

      CompositorIntersectService.apply(
          rhiCommandBuffer, currentFloats, (float) doubleValue, currentValue);
  ...
  private void updateState12(
    ...
    for (CompositorPushPresentationScaleService.RectCmd rectCmd : this.items) {
      ...
      rhiCommandBuffer.pushVec4(
          "uRadius",
          rectCmd.tl * nextValue,
          rectCmd.tr * nextValue,
          rectCmd.br * nextValue,
          rectCmd.bl * nextValue);
      float previousValue = rectCmd.shadow * 1.5F * nextValue;
      rhiCommandBuffer.pushVec4(
          "uShadow", 0.0F, rectCmd.shadow * 0.5F * nextValue, previousValue, 0.0F);
      ...
      CompositorIntersectService.apply(
          rhiCommandBuffer, rectCmd.clip, (float) doubleValue, currentValue);
      rhiCommandBuffer.draw(4, 1, 0);
  ...
  private void updateState13(
    ...
    if (sourceValue > 0) {
      ...
      try {
        this.updateState14(
            rhiCommandBuffer, value, currentValue, doubleValue, nextValue, previousValue);
      } finally {
        DiagnosticsAddFrameListenerService.get().end();
      }
    }
  ...
  private void updateState14(
    ...
    while (inputValue < sourceValue) {
      ...
      while (previousIndex < sourceValue
          && checkCondition5(this.items.get(nextValue + previousIndex).clip(), currentFloats)) {
        previousIndex++;
      }
  ...
  private void updateState16(
    ...
    for (int index = nextValue; index < previousValue; index++) {
      ...
      rhiCommandBuffer.pushVec4(
          "uRect",
          rectCmd.x * targetValue,
          rectCmd.y * targetValue,
          rectCmd.w * targetValue,
          rectCmd.h * targetValue);
      rhiCommandBuffer.pushVec4("uColor", rectCmd.r, rectCmd.g, rectCmd.b, rectCmd.a);
      rhiCommandBuffer.pushVec4(
          "uRadius",
          rectCmd.tl * targetValue,
          rectCmd.tr * targetValue,
          rectCmd.br * targetValue,
          rectCmd.bl * targetValue);
      float inputValue = rectCmd.shadow * 1.5F * targetValue;
      rhiCommandBuffer.pushVec4(
          "uShadow", 0.0F, rectCmd.shadow * 0.5F * targetValue, inputValue, 0.0F);
      updateState32(rhiCommandBuffer, "uShadowColor", rectCmd.shadowColor);
  ...
  private void updateState18(
    ...
    if (this.pipelineHandle5 != null && this.checkCondition4()) {
      ...
      for (int index = previousValue; index < sourceValue; index++) {
        ...
        rhiCommandBuffer.pushVec4(
            "uRadius",
            glassCmd.tl * targetValue,
            glassCmd.tr * targetValue,
            glassCmd.br * targetValue,
            glassCmd.bl * targetValue);
        ...
        rhiCommandBuffer.pushFloat(
            ...
            defaultValue > 0.001F
                ...
                : 0.0F);
        rhiCommandBuffer.pushVec4(
            "uMouse", resolvedValue, computedValue, this.enabled5 ? 1.0F : 0.0F, cachedValue);
        rhiCommandBuffer.pushVec4("uEdgeWobble", defaultValue, initialValue, 0.0F, 0.0F);
        rhiCommandBuffer.pushVec4(
            "uRubber",
            glassCmd.rubberSide,
            glassCmd.rubberTangent * targetValue,
            glassCmd.rubberAmount * targetValue,
            glassCmd.rubberWidth * targetValue);
        ...
        RhiBlendStateService.TextureHandle textureHandle =
            this.compositorShutdownService.sourceTexture() != null
                    && this.compositorShutdownService.sourceTexture().valid()
  ...
  private void updateState19(
    ...
    for (int index = nextValue; index < previousValue; index++) {
      ...
      if (texCmd.texture != null && texCmd.texture.valid()) {
        ...
        rhiCommandBuffer.pushVec4(
            "uRect",
            texCmd.x * sourceValue,
            texCmd.y * sourceValue,
            texCmd.w * sourceValue,
            texCmd.h * sourceValue);
        ...
        rhiCommandBuffer.pushVec4(
            "uRadius",
            texCmd.tl * sourceValue,
            texCmd.tr * sourceValue,
            texCmd.br * sourceValue,
            texCmd.bl * sourceValue);
  ...
  private void updateState20(
    ...
    for (int index = nextValue; index < previousValue; index++) {
      ...
      if (sdfIconCmd.texture != null && sdfIconCmd.texture.valid()) {
        ...
        rhiCommandBuffer.pushVec4(
            "uRect",
            sdfIconCmd.x * sourceValue,
            sdfIconCmd.y * sourceValue,
            sdfIconCmd.w * sourceValue,
            sdfIconCmd.h * sourceValue);
        ...
        rhiCommandBuffer.pushInt("uTexture", 0);
        CompositorIntersectService.apply(
            rhiCommandBuffer, sdfIconCmd.clip, sourceValue, currentValue);
        rhiCommandBuffer.draw(4, 1, 0);
  ...
  private void updateState21(
    ...
    for (int index = nextValue; index < previousValue; index++) {
      ...
      rhiCommandBuffer.pushVec4(
          "uRect",
          gradCmd.x * sourceValue,
          gradCmd.y * sourceValue,
          gradCmd.w * sourceValue,
          gradCmd.h * sourceValue);
  ...
  private void updateState22(
    ...
    for (int index = nextValue; index < previousValue; index++) {
      ...
      rhiCommandBuffer.bindPipeline(
          outputValue != 0
              ? this.pipelineHandle11
              : (inputValue != 0 ? this.pipelineHandle10 : this.pipelineHandle9));
      rhiCommandBuffer.bindVertexBuffer(this.bufferHandle, 0);
      CompositorIntersectService.apply(rhiCommandBuffer, meshCmd.clip, sourceValue, currentValue);
      rhiCommandBuffer.pushVec2("uResolution", value, currentValue);
      rhiCommandBuffer.pushVec4(
          "uRect",
          meshCmd.x * sourceValue,
          meshCmd.y * sourceValue,
          meshCmd.w * sourceValue,
          meshCmd.h * sourceValue);
      ...
      rhiCommandBuffer.pushFloat(
          "uTime",
          outputValue != 0
              ? meshCmd.menuTime.time
              : (inputValue != 0 ? meshCmd.studio.time : targetValue));
      if (outputValue != 0) {
      ...
      if (inputValue != 0) {
        ...
        rhiCommandBuffer.pushVec4(
            "uParams", meshCmd.studio.a, meshCmd.studio.b, meshCmd.studio.c, meshCmd.studio.d);
        rhiCommandBuffer.pushVec2(
            "uAlpha", (meshCmd.c0 >>> 24 & 0xFF) / 255.0F, (meshCmd.c1 >>> 24 & 0xFF) / 255.0F);
  ...
  private void updateState23(
    ...
    for (int index = nextValue; index < previousValue; index++) {
      ...
      rhiCommandBuffer.pushVec4(
          "uRect",
          loadingCmd.x * sourceValue,
          loadingCmd.y * sourceValue,
          loadingCmd.w * sourceValue,
          loadingCmd.h * sourceValue);
  ...
  private void updateState24(
    ...
    for (int index = nextValue; index < previousValue; index++) {
      CompositorPushPresentationScaleService.LiquidSwitchCmd liquidSwitchCmd =
          this.items12.get(index);
      CompositorIntersectService.apply(
          rhiCommandBuffer, liquidSwitchCmd.clip, sourceValue, currentValue);
      rhiCommandBuffer.pushVec2("uResolution", value, currentValue);
      rhiCommandBuffer.pushVec4(
          "uRect",
          liquidSwitchCmd.x * sourceValue,
          liquidSwitchCmd.y * sourceValue,
          liquidSwitchCmd.w * sourceValue,
          liquidSwitchCmd.h * sourceValue);
      rhiCommandBuffer.pushVec2(
          "uSize", liquidSwitchCmd.w * sourceValue, liquidSwitchCmd.h * sourceValue);
      ...
      rhiCommandBuffer.pushVec4(
          "uActive",
          liquidSwitchCmd.activeX * sourceValue,
          liquidSwitchCmd.activeW * sourceValue,
          liquidSwitchCmd.transition,
          liquidSwitchCmd.opacity);
      rhiCommandBuffer.pushVec4(
          "uHover",
          liquidSwitchCmd.hoverX * sourceValue,
          liquidSwitchCmd.hoverW * sourceValue,
          liquidSwitchCmd.hoverMix,
          0.0F);
  ...
  private void updateState25(
    ...
    for (int index = nextValue; index < previousValue; index++) {
      CompositorPushPresentationScaleService.TooltipBubbleCmd tooltipBubbleCmd =
          this.items13.get(index);
      CompositorIntersectService.apply(
          rhiCommandBuffer, tooltipBubbleCmd.clip, sourceValue, currentValue);
      ...
      rhiCommandBuffer.pushVec4(
          "uRect",
          tooltipBubbleCmd.x * sourceValue,
          tooltipBubbleCmd.y * sourceValue,
          tooltipBubbleCmd.w * sourceValue,
          tooltipBubbleCmd.h * sourceValue);
      rhiCommandBuffer.pushFloat("uRadius", tooltipBubbleCmd.radius * sourceValue);
      rhiCommandBuffer.pushVec3(
          "uTail",
          tooltipBubbleCmd.tailX * sourceValue,
          tooltipBubbleCmd.tailW * sourceValue,
          tooltipBubbleCmd.tailH * sourceValue);
      ...
      rhiCommandBuffer.pushFloat(
          "uInnerHighlightSize", tooltipBubbleCmd.innerHighlightSize * sourceValue);
  ...
  private static void updateState26(RhiCommandBuffer rhiCommandBuffer, String text, int value) {
    rhiCommandBuffer.pushVec3(
        text, (value >> 16 & 0xFF) / 255.0F, (value >> 8 & 0xFF) / 255.0F, (value & 0xFF) / 255.0F);
  }
  ...
  private void updateState27(
    ...
    for (int index = nextValue; index < previousValue; index++) {
      ...
      CompositorIntersectService.apply(
          rhiCommandBuffer, colorPickerCmd.clip, sourceValue, currentValue);
      rhiCommandBuffer.pushVec2("uResolution", value, currentValue);
      rhiCommandBuffer.pushVec4(
          "uRect",
          colorPickerCmd.x * sourceValue,
          colorPickerCmd.y * sourceValue,
          colorPickerCmd.w * sourceValue,
          colorPickerCmd.h * sourceValue);
  ...
  private void updateState28(
    ...
    for (int index = nextValue; index < previousValue; index++) {
      ...
      CompositorIntersectService.apply(
          rhiCommandBuffer, curveGraphCmd.clip, sourceValue, currentValue);
      rhiCommandBuffer.pushVec2("uResolution", value, currentValue);
      rhiCommandBuffer.pushVec4(
          "uRect",
          curveGraphCmd.x * sourceValue,
          curveGraphCmd.y * sourceValue,
          curveGraphCmd.w * sourceValue,
          curveGraphCmd.h * sourceValue);
      rhiCommandBuffer.pushVec4(
          "uPlot",
          curveGraphCmd.plotX * sourceValue,
          curveGraphCmd.plotY * sourceValue,
          curveGraphCmd.plotW * sourceValue,
          curveGraphCmd.plotH * sourceValue);
      rhiCommandBuffer.pushFloat("uRadius", curveGraphCmd.radius * sourceValue);

      rhiCommandBuffer.pushInt(
          "uCurveType",
          switch (curveValue.type()) {
            case LINEAR -> 0;
            case CUBIC_BEZIER -> 1;
            case STEPS -> 2;
          });
      rhiCommandBuffer.pushVec4(
          "uBezier", curveValue.x1(), curveValue.y1(), curveValue.x2(), curveValue.y2());
      rhiCommandBuffer.pushInt("uSteps", curveValue.steps());

      rhiCommandBuffer.pushInt(
          "uStepMode",
          switch (curveValue.stepMode()) {
            case JUMP_START -> 0;
            case JUMP_END -> 1;
            case JUMP_NONE -> 2;
            case JUMP_BOTH -> 3;
  ...
  private void updateState29(
    ...
    for (int index = nextValue; index < previousValue; index++) {
      ...
      CompositorIntersectService.apply(
          rhiCommandBuffer, timeSeriesCmd.clip, sourceValue, currentValue);
      rhiCommandBuffer.pushVec2("uResolution", value, currentValue);
      rhiCommandBuffer.pushVec4(
          "uRect",
          timeSeriesCmd.x * sourceValue,
          timeSeriesCmd.y * sourceValue,
          timeSeriesCmd.w * sourceValue,
          timeSeriesCmd.h * sourceValue);
  ...
  private void updateState30(
    ...
    for (int index = nextValue; index < previousValue; index++) {
      ...
      CompositorIntersectService.apply(
          rhiCommandBuffer, waveformCmd.clip, sourceValue, currentValue);
      ...
      rhiCommandBuffer.pushVec4(
          "uRect",
          waveformCmd.x * sourceValue,
          waveformCmd.y * sourceValue,
          waveformCmd.w * sourceValue,
          waveformCmd.h * sourceValue);
  ...
  private void updateState31(
    ...
    for (int index = nextValue; index < previousValue; index++) {
      ...
      rhiCommandBuffer.pushVec4(
          "uRect",
          rectCmd.x * sourceValue,
          rectCmd.y * sourceValue,
          rectCmd.w * sourceValue,
          rectCmd.h * sourceValue);
      rhiCommandBuffer.pushVec4("uColor", rectCmd.r, rectCmd.g, rectCmd.b, rectCmd.a);
      rhiCommandBuffer.pushVec4(
          "uRadius",
          rectCmd.tl * sourceValue,
          rectCmd.tr * sourceValue,
          rectCmd.br * sourceValue,
          rectCmd.bl * sourceValue);
      float targetValue = rectCmd.shadow * 1.5F * sourceValue;
      rhiCommandBuffer.pushVec4(
          "uShadow", 0.0F, rectCmd.shadow * 0.5F * sourceValue, targetValue, 0.0F);
      updateState32(rhiCommandBuffer, "uShadowColor", rectCmd.shadowColor);
  ...
  public int readPixel(int value, int currentValue, int nextValue) {
    ...
    GL11.glReadPixels(
        value, nextValue - currentValue - 1, 1, 1, GL11.GL_RGBA, GL11.GL_UNSIGNED_BYTE, byteBuffer);
  ...
  public void pushClip(
    ...
    this.float3 =
        CompositorIntersectService.intersect(
            this.float3,
            x,
            y,
            width,
  ...
  public void circle(
      float value, float currentValue, float nextValue, int previousValue, float sourceValue) {
    ...
    try {
      ...
      RhiBlendStateService.ShaderHandle shaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.VERTEX, RhiRepository.vertex("sdf_rect"));
      RhiBlendStateService.ShaderHandle currentShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.FRAGMENT, RhiRepository.fragment("sdf_rect"));
      this.pipelineHandle =
      ...
      try {
        RhiBlendStateService.ShaderHandle nextShaderHandle =
            this.rhiOperationHandler.createShader(
                RhiBlendStateService.ShaderStage.VERTEX, RhiRepository.vertex("sdf_rect_batch"));
        RhiBlendStateService.ShaderHandle previousShaderHandle =
            this.rhiOperationHandler.createShader(
                RhiBlendStateService.ShaderStage.FRAGMENT,
                RhiRepository.fragment("sdf_rect_batch"));
        ...
        CoreIsInitializedHandler.LOGGER.info(
            "ellice: SSBO not available, using per-rect uniforms (GLSL 330)");
        this.enabled2 = false;
      }

      RhiBlendStateService.ShaderHandle sourceShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.VERTEX, RhiRepository.vertex("texture_quad"));
      RhiBlendStateService.ShaderHandle targetShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.FRAGMENT, RhiRepository.fragment("texture_quad"));
      ...
      RhiBlendStateService.ShaderHandle inputShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.VERTEX, RhiRepository.vertex("glass_panel"));
      RhiBlendStateService.ShaderHandle outputShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.FRAGMENT, RhiRepository.fragment("glass_panel"));
      ...
      RhiBlendStateService.ShaderHandle resultShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.VERTEX, RhiRepository.vertex("texture_quad"));
      RhiBlendStateService.ShaderHandle candidateShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.FRAGMENT, RhiRepository.fragment("sdf_icon"));
      ...
      RhiBlendStateService.ShaderHandle selectedShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.VERTEX, RhiRepository.vertex("texture_quad"));
      RhiBlendStateService.ShaderHandle defaultShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.FRAGMENT,
              RhiRepository.fragment("filtered_texture"));
      ...
      RhiBlendStateService.ShaderHandle initialShaderHandle =
          RhiBlendStateService.ShaderHandle.NONE;
      RhiBlendStateService.ShaderHandle resolvedShaderHandle =
          RhiBlendStateService.ShaderHandle.NONE;
      ...
      try {
        initialShaderHandle =
            this.rhiOperationHandler.createShader(
                RhiBlendStateService.ShaderStage.VERTEX, RhiRepository.vertex("texture_quad"));
        resolvedShaderHandle =
            this.rhiOperationHandler.createShader(
                RhiBlendStateService.ShaderStage.FRAGMENT, RhiRepository.fragment("ui_material"));
      ...
      RhiBlendStateService.ShaderHandle computedShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.VERTEX, RhiRepository.vertex("gradient"));
      RhiBlendStateService.ShaderHandle cachedShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.FRAGMENT, RhiRepository.fragment("gradient"));
      ...
      RhiBlendStateService.ShaderHandle pendingShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.VERTEX, RhiRepository.vertex("gradient"));
      RhiBlendStateService.ShaderHandle activeShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.FRAGMENT, RhiRepository.fragment("mesh_card"));
      this.pipelineHandle9 =
          this.rhiOperationHandler.createPipeline(
              RhiBuilderData.graphics()
      ...
      RhiBlendStateService.ShaderHandle fallbackShaderHandle =
          RhiBlendStateService.ShaderHandle.NONE;
      RhiBlendStateService.ShaderHandle primaryShaderHandle =
          RhiBlendStateService.ShaderHandle.NONE;

      try {
        fallbackShaderHandle =
            this.rhiOperationHandler.createShader(
                RhiBlendStateService.ShaderStage.VERTEX, RhiRepository.vertex("spotify_bg"));
        primaryShaderHandle =
            this.rhiOperationHandler.createShader(
                RhiBlendStateService.ShaderStage.FRAGMENT, RhiRepository.fragment("module_studio"));
        this.pipelineHandle10 =
            this.rhiOperationHandler.createPipeline(
                RhiBuilderData.graphics()
        ...
        CoreIsInitializedHandler.LOGGER.warn(
            "Module Studio shader unavailable; using the mesh fallback", nextException);
      } finally {
      ...
      RhiBlendStateService.ShaderHandle secondaryShaderHandle =
          RhiBlendStateService.ShaderHandle.NONE;
      RhiBlendStateService.ShaderHandle tertiaryShaderHandle =
          RhiBlendStateService.ShaderHandle.NONE;

      try {
        secondaryShaderHandle =
            this.rhiOperationHandler.createShader(
                RhiBlendStateService.ShaderStage.VERTEX, RhiRepository.vertex("spotify_bg"));
        tertiaryShaderHandle =
            this.rhiOperationHandler.createShader(
                RhiBlendStateService.ShaderStage.FRAGMENT, RhiRepository.fragment("menu_horizon"));
        this.pipelineHandle11 =
            this.rhiOperationHandler.createPipeline(
                RhiBuilderData.graphics()
        ...
        CoreIsInitializedHandler.LOGGER.warn(
            "Menu shader unavailable; using the mesh fallback", previousException);
      } finally {
      ...
      RhiBlendStateService.ShaderHandle temporaryShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.VERTEX, RhiRepository.vertex("gradient"));
      RhiBlendStateService.ShaderHandle requestedShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.FRAGMENT, RhiRepository.fragment("liquid_switcher"));
      this.pipelineHandle12 =
          this.rhiOperationHandler.createPipeline(
              RhiBuilderData.graphics()
      ...
      try {
        RhiBlendStateService.ShaderHandle actualShaderHandle =
            this.rhiOperationHandler.createShader(
                RhiBlendStateService.ShaderStage.VERTEX, RhiRepository.vertex("gradient"));
        RhiBlendStateService.ShaderHandle expectedShaderHandle =
            this.rhiOperationHandler.createShader(
                RhiBlendStateService.ShaderStage.FRAGMENT, RhiRepository.fragment("font_loading"));
        this.pipelineHandle18 =
            this.rhiOperationHandler.createPipeline(
                RhiBuilderData.graphics()
        ...
        CoreIsInitializedHandler.LOGGER.warn(
            "ellice loading-spinner pipeline disabled", sourceException);
        this.pipelineHandle18 = null;
      }

      RhiBlendStateService.ShaderHandle minimumShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.VERTEX, RhiRepository.vertex("sdf_rect"));
      RhiBlendStateService.ShaderHandle maximumShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.FRAGMENT, RhiRepository.fragment("sdf_tooltip"));
      this.pipelineHandle13 =
          this.rhiOperationHandler.createPipeline(
              RhiBuilderData.graphics()
      ...
      RhiBlendStateService.ShaderHandle startShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.VERTEX, RhiRepository.vertex("gradient"));
      RhiBlendStateService.ShaderHandle endShaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.FRAGMENT, RhiRepository.fragment("colorpicker"));
      this.pipelineHandle14 =
          this.rhiOperationHandler.createPipeline(
              RhiBuilderData.graphics()
      ...
      try {
        ...
        storedShaderHandle =
            this.rhiOperationHandler.createShader(
                RhiBlendStateService.ShaderStage.FRAGMENT, RhiRepository.fragment("curve_graph"));
        this.pipelineHandle15 =
            this.rhiOperationHandler.createPipeline(
                RhiBuilderData.graphics()
      ...
      RhiBlendStateService.ShaderHandle createdShaderHandle =
          RhiBlendStateService.ShaderHandle.NONE;
      RhiBlendStateService.ShaderHandle updatedShaderHandle =
          RhiBlendStateService.ShaderHandle.NONE;
      ...
      try {
        ...
        updatedShaderHandle =
            this.rhiOperationHandler.createShader(
                RhiBlendStateService.ShaderStage.FRAGMENT,
                RhiRepository.fragment("audio_waveform"));
        this.pipelineHandle16 =
            this.rhiOperationHandler.createPipeline(
                RhiBuilderData.graphics()
        ...
        CoreIsInitializedHandler.LOGGER.warn(
            "ellice audio waveform shader disabled", inputException);
      ...
      RhiBlendStateService.ShaderHandle removedShaderHandle =
          RhiBlendStateService.ShaderHandle.NONE;
      ...
      try {
        ...
        firstShaderHandle =
            this.rhiOperationHandler.createShader(
                RhiBlendStateService.ShaderStage.FRAGMENT, RhiRepository.fragment("time_series"));
        this.pipelineHandle17 =
            this.rhiOperationHandler.createPipeline(
                RhiBuilderData.graphics()
      ...
      this.compositorShutdownService =
          new CompositorShutdownService(this.rhiOperationHandler, this.bufferHandle);
      ...
      this.textTextureService =
          new TextTextureService(
              TextMode.REGULAR,
              "/assets/ellice/fonts/Minecraft",
              "/assets/ellice/fonts/Minecraft.ttf");
      this.textTextureService.generate(this.rhiOperationHandler);
      this.textTextService =
          new TextTextService(
              this.rhiOperationHandler,
              this.entries,
              this.textTextureService.isReady() ? this.textTextureService : null);
  ...
  private void updateState35(int value) {
    if (this.bufferHandle2 == null || value > this.count) {
      ...
      this.bufferHandle2 =
          this.rhiOperationHandler.createBuffer(
              this.count * 40L * 4L,
              RhiBlendStateService.BufferUsage.STORAGE.bit,
    ...
    public CurveGraphStyle(
      ...
      if (Float.isFinite(curveFringeWidth) && !(curveFringeWidth < curveWidth)) {
        ...
        throw new IllegalArgumentException(
            "Curve fringe width must be finite and at least the curve width");
      }
    }
  }
  ...
  private record GradCmd(
      float x, float y, float w, float h, int colorTop, int colorBottom, float[] clip) {}
