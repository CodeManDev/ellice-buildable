...
public final class CompositorEmptyService {
  ...
  private static final CompositorEmptyService compositorEmptyService =
      new CompositorEmptyService(List.of());
  ...
  public CompositorEmptyService(
      List<CompositorEmptyService.Sample> currentItems, double doubleValue) {
    if (Double.isFinite(doubleValue) && !(doubleValue <= 0.0)) {
      this.items =
          List.copyOf(
              currentItems.subList(Math.max(0, currentItems.size() - 40), currentItems.size()));
      ...
      this.value2 =
          Math.max(
              ...
              Math.ceil(
                  this.items.stream()
                          .mapToDouble(CompositorEmptyService.Sample::value)
                          .max()
                          .orElse(1.0)
                      * 1.15));
    } else {
