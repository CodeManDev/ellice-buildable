...
public final class WorldRenderer implements AutoCloseable {
   ...
   private RhiBlendStateService.PipelineHandle pipelineHandle3 = RhiBlendStateService.PipelineHandle.NONE;
   private RhiBlendStateService.PipelineHandle pipelineHandle4 = RhiBlendStateService.PipelineHandle.NONE;
   ...
   public void render(
      List<WorldRenderer.Selection> items, EventAttackInputService.WorldRender worldRender, int width, int height, WorldRenderer.Style style, float value
   ) {
      if (width > 0 && height > 0 && !items.isEmpty() && (style.fill() || style.outline())) {
         ...
         try (RhiOperationHandler.StateScope stateScope = this.rhiOperationHandler.preserveState()) {
            ...
            for (WorldRenderer.Selection selection : items) {
               ...
               WorldRenderer.GpuShape gpuShape = this.entries.computeIfAbsent(selection.mesh(), this::createGpuShape);
               if (!(selection.opacity() <= 0.001F)) {
                  if (style.fill() && gpuShape.faceCount() > 0) {
                     rhiCommandBuffer.bindPipeline(style.seeThrough() ? this.pipelineHandle2 : this.pipelineHandle);
                     this.updateState(rhiCommandBuffer, worldRender, selection, style, width, height, value);
                  ...
                  if (style.outline() && gpuShape.edgeCount() > 0) {
                     rhiCommandBuffer.bindPipeline(style.seeThrough() ? this.pipelineHandle4 : this.pipelineHandle3);
                     this.updateState(rhiCommandBuffer, worldRender, selection, style, width, height, value);
   ...
   private void updateState(
      ...
      try {
         ...
         rhiCommandBuffer.pushMatrix4("uProjection", worldRender.projectionMatrix().get(stack.mallocFloat(16)));
   ...
   private void updateState2() {
      if (!this.pipelineHandle.valid()) {
         RhiBlendStateService.ShaderHandle shaderHandle = this.createShaderHandle("block_selection", RhiBlendStateService.ShaderStage.VERTEX);
         RhiBlendStateService.ShaderHandle currentShaderHandle = this.createShaderHandle("block_selection", RhiBlendStateService.ShaderStage.FRAGMENT);
         RhiBlendStateService.ShaderHandle nextShaderHandle = this.createShaderHandle("block_selection_edge", RhiBlendStateService.ShaderStage.VERTEX);
         RhiBlendStateService.ShaderHandle previousShaderHandle = this.createShaderHandle("block_selection_edge", RhiBlendStateService.ShaderStage.FRAGMENT);
         RhiBlendStateService.VertexLayout vertexLayout = RhiBlendStateService.VertexLayout.of(
            24,
         ...
         this.pipelineHandle = this.createPipelineHandle(shaderHandle, currentShaderHandle, vertexLayout, true);
         this.pipelineHandle2 = this.createPipelineHandle(shaderHandle, currentShaderHandle, vertexLayout, false);
         this.pipelineHandle3 = this.createPipelineHandle(nextShaderHandle, previousShaderHandle, currentVertexLayout, true);
         this.pipelineHandle4 = this.createPipelineHandle(nextShaderHandle, previousShaderHandle, currentVertexLayout, false);
      }
   }

   private RhiBlendStateService.ShaderHandle createShaderHandle(String shaderName, RhiBlendStateService.ShaderStage shaderStage) {
      RhiBlendStateService.ShaderHandle shaderHandle = this.rhiOperationHandler
         .createShader(shaderStage, shaderStage == RhiBlendStateService.ShaderStage.VERTEX ? RhiRepository.vertex(shaderName) : RhiRepository.fragment(shaderName));
      this.items.add(shaderHandle);
      return shaderHandle;
   }

   private RhiBlendStateService.PipelineHandle createPipelineHandle(
      RhiBlendStateService.ShaderHandle shaderHandle, RhiBlendStateService.ShaderHandle currentShaderHandle, RhiBlendStateService.VertexLayout currentVertexLayout, boolean enabled
   ) {
      return this.rhiOperationHandler
         .createPipeline(
            RhiBuilderData.graphics()
               ...
               .depthStencil(new RhiBlendStateService.DepthStencilState(enabled, false, RhiBlendStateService.CompareOp.LESS_OR_EQUAL, false))
   ...
   private WorldRenderer.GpuShape createGpuShape(WorldData worldData) {
      ...
      try {
         if (worldData.faces().length > 0) {
            bufferHandle = this.rhiOperationHandler
               .createBuffer(
                  RhiBlendStateService.BufferUsage.flags(RhiBlendStateService.BufferUsage.VERTEX), RhiBlendStateService.BufferAccess.STATIC, worldData.faces()
               );
         }
         ...
         if (worldData.edges().length > 0) {
            currentBufferHandle = this.rhiOperationHandler
               .createBuffer(
                  RhiBlendStateService.BufferUsage.flags(RhiBlendStateService.BufferUsage.VERTEX), RhiBlendStateService.BufferAccess.STATIC, worldData.edges()
               );
         }

         return new WorldRenderer.GpuShape(bufferHandle, currentBufferHandle, worldData.faces().length / 6, worldData.edges().length / 8);
   ...
   public void close() {
      if (this.rhiOperationHandler != null) {
         ...
         for (RhiBlendStateService.PipelineHandle currentPipelineHandle : List.of(this.pipelineHandle, this.pipelineHandle2, this.pipelineHandle3, this.pipelineHandle4)) {
            this.rhiOperationHandler.destroyPipeline(currentPipelineHandle);
         }

         this.items.forEach(this.rhiOperationHandler::destroyShader);
         this.items.clear();
         this.pipelineHandle = this.pipelineHandle2 = this.pipelineHandle3 = this.pipelineHandle4 = RhiBlendStateService.PipelineHandle.NONE;
      }
   }

   private record GpuShape(RhiBlendStateService.BufferHandle faces, RhiBlendStateService.BufferHandle edges, int faceCount, int edgeCount) {
   }
