...
public record WorldData(float[] faces, float[] edges) {
   public static WorldData of(VoxelShape voxelShape) {
      ...
      if (currentIndex > 0 && nextIndex > 0 && previousIndex > 0 && (long)currentIndex * nextIndex * previousIndex <= 32768L) {
   ...
   private static void updateState(List<Float> items, double[][] doubles, int[] ints, int index, int value) {
      ...
      for (int currentValue : new int[]{0, 1, 2, 0, 2, 3}) {
         ...
         floats[currentIndex] = (float)doubles[currentIndex][ints[currentIndex] + (currentValue != 1 && currentValue != 2 ? 0 : 1)];
         floats[nextIndex] = (float)doubles[nextIndex][ints[nextIndex] + (currentValue >= 2 ? 1 : 0)];
         Collections.addAll(items, floats[0], floats[1], floats[2], index == 0 ? value : 0.0F, index == 1 ? value : 0.0F, index == 2 ? value : 0.0F);
      }
   }
