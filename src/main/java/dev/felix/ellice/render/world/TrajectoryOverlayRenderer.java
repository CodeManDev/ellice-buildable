...
public final class TrajectoryOverlayRenderer {
  ...
  public void render(
      ...
      int height,
      double doubleValue,
      TrajectoryOverlayRenderer.Style style) {
    ...
    if (this.rhiOperationHandler == null) {
      ...
      this.shaderHandle =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.VERTEX, RhiRepository.vertex("trajectory_ribbon"));
      this.shaderHandle2 =
          this.rhiOperationHandler.createShader(
              RhiBlendStateService.ShaderStage.FRAGMENT,
              RhiRepository.fragment("trajectory_ribbon"));
      RhiBlendStateService.VertexLayout currentVertexLayout =
          RhiBlendStateService.VertexLayout.of(
              ...
              new RhiBlendStateService.VertexAttribute(
                  2, 1, RhiBlendStateService.VertexFormat.FLOAT, 24L),
              new RhiBlendStateService.VertexAttribute(
                  3, 1, RhiBlendStateService.VertexFormat.FLOAT, 28L),
              new RhiBlendStateService.VertexAttribute(
                  4, 1, RhiBlendStateService.VertexFormat.FLOAT, 32L));
      this.pipelineHandle =
    ...
    for (int currentIndex = 0; currentIndex < items.size() - 1; currentIndex++) {
      ...
      index =
          this.calculateValue4(
              floats,
              index,
              (Vec3) items.get(currentIndex),
              (Vec3) items.get(currentIndex + 1),
              (float) doubles[currentIndex],
              (float) doubles[currentIndex + 1],
              currentValue);
    }
  ...
  private int calculateValue4(
      ...
      float previousValue) {
    value =
        this.calculateValue5(floats, value, vec3, currentVec3, -1.0F, currentValue, previousValue);
    value =
        this.calculateValue5(floats, value, vec3, currentVec3, 1.0F, currentValue, previousValue);
  ...
  public record Style(
      int startColor, int endColor, float widthPx, float glow, boolean impactMarker) {}

  public record Trajectory(
      List<Vec3> points,
      Vec3 impact,
      Direction impactSide,
      TrajectoryOverlayRenderer.ImpactKind impactKind,
      int ticks) {
