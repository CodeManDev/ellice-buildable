...
public final class MediaReleaseTracker implements AutoCloseable {
  ...
  public MediaReleaseTracker(
      Path path, int value, int currentValue, int nextValue, int previousValue) {
  ...
  public boolean ready() {
    return !this.enabled
        && !this.enabled2
        && !this.path.isDone()
        && this.arrayBlockingQueue.remainingCapacity() > 0;
  }
    ...
    try {
      ...
      label333:
      {
        ...
        while (true) {
          ...
          currentFFmpegFrameRecorder =
              new FFmpegFrameRecorder(currentPath.toFile(), this.count, this.count2, 0);
      ...
      if (fFmpegFrameRecorder == null) {
        throw new IllegalStateException(
            "No H.264 encoder available (tried VideoToolbox + OpenH264)", exception);
      }
      ...
      try {
        ...
        while (!this.enabled2) {
          ...
          if (byteBuffer == null) {
            ...
            try {
              fFmpegFrameRecorder.recordImage(
                  this.count, this.count2, 8, 4, this.count * 4, 26, new Buffer[] {byteBuffer});
              currentIndex++;
            } finally {
