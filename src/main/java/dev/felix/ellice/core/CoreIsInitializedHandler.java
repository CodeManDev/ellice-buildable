...
public final class CoreIsInitializedHandler {
  ...
  private final ModuleBindService moduleBindService =
      new ModuleBindService(this.moduleRegisterService);
  private final CommandExecuteService commandExecuteService =
      new CommandExecuteService(
          ...
          item ->
              this.sceneDtService
                  .toasts()
                  .show("Client commands", item, 4.0F, SceneRenderer.Type.INFO));
  ...
  private final CompositorPushPresentationScaleService compositorPushPresentationScaleService =
      new CompositorPushPresentationScaleService();
  ...
  private void updateState() {
    ...
    this.configRepository =
        new ConfigRepository(path.resolve("ellice-ui/client-settings.json"), this.values2);
    this.renderer3 = new Render3dSceneService(path);
    this.friendsStateController = new FriendsStateController(path);
    this.moduleRegisterService.discoverAll(
    ...
    this.configRepository.onPreferencesChanged(
        item -> this.discordStateController.enabled(item.discordRichPresence()));
    this.renderer2 = new StudioRenderer(path, this.moduleRegisterService);
    this.renderer2.load();
    this.configRepository2 =
        new LocalConfigRepository(
            path, this.moduleRegisterService, this.values, this.hudItemsService);
    ...
    BuiltinRepository builtinRepository =
        new BuiltinRepository(path.resolve("ellice-ui/clickgui.json"));
    this.screenDelayQueue.register(
        "clickgui", () -> new ClickGuiScreen(this.moduleRegisterService, builtinRepository));
    this.screenDelayQueue.register(
        "client-settings", () -> new ClientSettingsScreen(this.configRepository));
    this.screenDelayQueue.register("configs", () -> new ConfigsScreen(this.configRepository2));
    this.renderer2.opener(
        item -> {
    ...
    this.screenDelayQueue.register(
        ...
        () ->
            new HudEditorScreen(
                this.layoutFindSelector, this.renderer, this.layoutOperationHandler));
    this.changelogRepository = ChangelogRepository.load();
    this.changelogLastSeenService =
        new ChangelogLastSeenService(FabricLoader.getInstance().getGameDir());
    this.screenDelayQueue.register(
        "changelog", () -> new ChangelogScreen(this.changelogRepository));
    this.overlayOnActivateHandler =
        new OverlayOnActivateHandler(
            this.sceneDtService, this.compositorPushPresentationScaleService);
    ...
    this.overlayOnActivateHandler2.setHasNews(
        this.changelogRepository.latest() != null
            && this.changelogLastSeenService.isUnseen(this.createText2()));
    this.diagnosticsRecorderService =
        new DiagnosticsRecorderService(
            this.sceneDtService, this.items, () -> this.renderer3.shaderEspDiagnostics());
    this.eventSubscribeService.subscribe(
        EventAttackInputService.WORLD_RENDER,
        EventIsAfterHandler.Priority.LAST,
        item -> {
          FramebufferInfo framebufferInfo =
              CompatAdapterService.mainFramebuffer(Minecraft.getInstance());
    ...
    this.eventSubscribeService.subscribe(
        EventAttackInputService.WORLD, item -> this.renderer3.clearWorld());
    this.eventSubscribeService.subscribe(
        EventAttackInputService.WORLD, item -> this.renderer2.releaseActions());
    ...
    if (this.layoutFindSelector == null) {
      ...
      LOGGER.info(
          "Loaded HUD layout '{}' ({} elements)",
          this.layoutFindSelector.name,
          this.layoutFindSelector.elements.size());
    }

    this.configRepository2.bindHudLayout(this.layoutFindSelector, currentPath);
    this.diagnosticsIsRecordingService =
        new DiagnosticsIsRecordingService(this.items, FabricLoader.getInstance().getGameDir());
    ...
    this.screenDelayQueue.register(
        "friends",
        () -> new FriendsScreen(this.friendsStateController, FriendSyncController::players));
    this.screenDelayQueue.register(
        "anticheats", () -> new AnticheatScreen(this.anticheatRepository));
    ...
    this.screenDelayQueue.register(
        "account-manager",
        () ->
            new BuiltinSessionTracker(
                this.accountAllService, this.compositorPushPresentationScaleService));
    this.builtinActionsTracker =
        new BuiltinActionsTracker(
            this.serverViewTracker,
            this.menuLoaderTracker,
            () ->
                new BuiltinActionsTracker.Session(
            ...
            new BuiltinActionsTracker.Actions(
                ...
                (item, currentItem) ->
                    this.screenDelayQueue.open(
                        new ServerPreviewScreen(
                            this.serverViewTracker,
    ...
    this.overlayOnActivateHandler.onActivate(
        item ->
            this.screenDelayQueue.open(
                new BuiltinSessionTracker(
                    this.accountAllService, this.compositorPushPresentationScaleService)));
    CompatAdapterService.registerScreenClickGuard(
        (item, currentItem) -> {
          if (!isReady()) {
            return true;
          }
          ...
          return !sceneDt.hasInteractiveAt(
              displayMetrics.vanillaX(item), displayMetrics.vanillaY(currentItem));
        });
    this.eventSubscribeService.subscribe(
        ...
        item -> {
          ...
          int nextValue =
              !(minecraft.screen instanceof CoreSetBehindScreenService)
                      && (this.screenDelayQueue == null || !this.screenDelayQueue.isActive())
                      && (this.overlayOnActivateHandler == null
                          || !this.overlayOnActivateHandler.isVisible())
                      && (this.overlayOnActivateHandler2 == null
                          || !this.overlayOnActivateHandler2.isVisible())
                  ? 0
                  : 1;
          ...
          if (nextValue != 0) {
            ...
            this.sceneDtService.mouseInput(
                previousValue, sourceValue, (targetValue != 0), (inputValue != 0));
          }

          this.items.end();
          this.sceneDtService.setFramebufferInfo(
              (float) this.compatData6.renderScale(), this.compatData6.framebufferHeight());
    ...
    ClientTickEvents.END_CLIENT_TICK.register(
        (EndTick)
            item -> {
              if (isReady()) {
                this.discordStateController.tick(
                    item, (int) this.moduleRegisterService.enabled().count());
              ...
              if (item.isPress() && item.keyCode() == 298) {
                ...
                if (this.diagnosticsIsRecordingService != null) {
                  if (this.diagnosticsIsRecordingService.isRecording()) {
                    ...
                    if (this.diagnosticsRecorderService != null
                        && !this.diagnosticsRecorderService.isVisible()) {
    ...
    this.eventSubscribeService.subscribe(
        EventAttackInputService.RENDER,
        new EventIsAfterHandler<EventAttackInputService.Render>() {
          boolean done;

          public void handle(EventAttackInputService.Render render) {
            if (!this.done) {
              this.done = true;
              CoreIsInitializedHandler.this.updateState2(
                  CoreIsInitializedHandler.this.sceneDtService.root());
            }
          }
        });
    ClientLifecycleEvents.CLIENT_STARTED.register(
        (ClientStarted) item -> this.configRepository2.load());
    LOGGER.info("{} initialized with {} module(s)", "ellice", this.moduleRegisterService.size());
  }

  public boolean showMainMenu() {
    return !this.enabled2
            && this.builtinActionsTracker != null
            && this.screenDelayQueue != null
            && mc().level == null
  ...
  private String createText2() {
    return this.changelogRepository != null && this.changelogRepository.latest() != null
        ? this.changelogRepository.latest().version()
        : VERSION;
  }

  public boolean handleKeyInput(long longValue, int value, int currentValue, int nextValue) {
    if (!this.enabled2
        && longValue == CompatAdapterService.windowHandle(mc())
        && mc().isWindowActive()) {
      Screen currentScreen = mc().screen;
      EventAttackInputService.Key key =
          this.eventSubscribeService.post(
              EventAttackInputService.KEY,
              new EventAttackInputService.Key(value, currentValue, nextValue));
  ...
  private void updateState2(ScenePctService<?> scenePct) {
    if (scenePct.isDraggable() && scenePct.getId() != null) {
      ...
      DisplayMetrics displayMetrics =
          CompatAdapterService.uiViewport(
              minecraft, minecraft.screen instanceof CoreSetBehindScreenService);
      ...
      float[] floats =
          this.configRepository2.getPosition(
              scenePct.getId(), currentWidth, currentHeight, value, currentValue);
      if (floats != null) {
        scenePct.x = floats[0];
        scenePct.y = floats[1];
      }
  ...
  public void beginRenderFrame() {
    ...
    if (minecraft.screen instanceof TitleScreen
        && this.screenDelayQueue != null
        && !this.screenDelayQueue.isActive()) {
      this.showMainMenu();
    }
