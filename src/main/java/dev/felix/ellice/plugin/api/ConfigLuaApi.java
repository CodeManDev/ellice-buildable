...
public final class ConfigLuaApi implements ApiOperationHandler {
   ...
   public void register(Globals globals) {
      ...
      luaTable.set("delete", new OneArgFunction() {
         public LuaValue call(LuaValue luaValue) {
            return LuaValue.valueOf(CoreIsInitializedHandler.get().config().deleteConfig(luaValue.tojstring()));
         }
      });
      luaTable.set("active", new ZeroArgFunction() {
         public LuaValue call() {
