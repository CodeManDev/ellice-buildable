...
public final class ClientLuaApi implements ApiOperationHandler {
  private final Map<String, List<LuaFunction>> text = new HashMap<>();
        ...
        new TwoArgFunction() {
          public LuaValue call(LuaValue var1, LuaValue var2x) {
            if (var2x.isfunction()) {
              ClientLuaApi.this
                  .text
                  .computeIfAbsent(var1.tojstring(), var0 -> new ArrayList<>())
                  .add(var2x.checkfunction());
            }
      ...
      for (LuaFunction var5 : (Iterable<LuaFunction>) (Iterable<?>) (var3)) {
        try {
          ...
          CoreIsInitializedHandler.LOGGER.error(
              "Lua event error [{}]: {}", var1, var7.getMessage());
        }
      }
    }
  }
