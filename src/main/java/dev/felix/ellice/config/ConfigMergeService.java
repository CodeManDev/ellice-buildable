...
public final class ConfigMergeService {
   ...
   public static void merge(JsonObject jsonObject) {
      if (!jsonObject.has("ESP") && (jsonObject.has("2D ESP") || jsonObject.has("Player Fire ESP"))) {
         ...
         targetJsonObject.entrySet().forEach(entry -> sourceJsonObject.add((String)entry.getKey(), ((JsonElement)entry.getValue()).deepCopy()));
         Set values = Set.of("Range", "Max Targets", "Include Self");
         createJsonObject(currentJsonObject, "settings").entrySet().forEach(entry -> {
            String text = switch ((String)entry.getKey()) {
