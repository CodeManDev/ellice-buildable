...
public record InventoryRoleData(List<InventoryRoleData.Role> slots) {
   public static final InventoryRoleData BALANCED = of(
   ...
   public static final InventoryRoleData COMBAT = of(
      InventoryRoleData.Role.SWORD,
   ...
   public static final InventoryRoleData BUILDING = of(
      InventoryRoleData.Role.PICKAXE,
      InventoryRoleData.Role.AXE,
   ...
   public static InventoryRoleData parse(String text) {
      if (text == null) {
         ...
         return new InventoryRoleData(Arrays.stream(text.split(",", -1)).map(String::trim).map(InventoryRoleData.Role::valueOf).toList());
      }
   }
   ...
   public boolean locked(int value) {
      return value >= 36 && value <= 44 && this.slots.get(value - 36) == InventoryRoleData.Role.LOCKED;
   }

   public enum Role {
      LOCKED("Locked", "lock", "Keep this slot exactly as it is; its item is never used as a source."),
      SWORD("Sword", "swords", "Best sword by damage, speed, enchantments and durability."),
      PICKAXE("Pickaxe", "construction", "Best available pickaxe; the current item wins equal scores."),
      ...
      BLOCKS("Blocks", "deployed_code", "A large stack of solid building blocks; multiple slots use distinct stacks."),
      ...
      public boolean accepts(InventoryKindData inventoryKindData) {
         ...
         return (boolean)(switch (this) {
            ...
            case RANGED -> inventoryKindData.kind() != InventoryKindData.Kind.BOW && inventoryKindData.kind() != InventoryKindData.Kind.CROSSBOW ? 0 : 1;
            ...
            case SILK_TOUCH -> inventoryKindData.kind() == InventoryKindData.Kind.PICKAXE && inventoryKindData.stats().enchantment("silk_touch") > 0 ? 1 : 0;
            case FORTUNE -> inventoryKindData.kind() == InventoryKindData.Kind.PICKAXE && inventoryKindData.stats().enchantment("fortune") > 0 ? 1 : 0;
         });
      }
      ...
      private static InventoryRoleData.Role[] $values() {
         return new InventoryRoleData.Role[]{
            LOCKED, SWORD, PICKAXE, AXE, SHOVEL, HOE, RANGED, BLOCKS, FOOD, HEALING, PEARLS, SHIELD, UTILITY, TOTEM, SILK_TOUCH, FORTUNE
         };
      }
   }
}
