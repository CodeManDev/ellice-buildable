...
public final class InventoryEnvironmentTracker {
  ...
  public void tick(
      long longValue,
      InventoryEnvironmentTracker.Environment environment,
      InventoryData inventoryData,
      InventoryEnvironmentTracker.Timing timing) {
    ...
    if (result.isEmpty()) {
      ...
      if (!this.items2.equals(inventorySlotData.slots())
          || !this.items3.equals(inventorySlotData.owned())) {
        this.timestamp3 = -1L;
        this.items2 = inventorySlotData.slots();
        this.items3 = inventorySlotData.owned();
      }

      if (!inventorySlotData.paused()
          && !inventorySlotData.carriedItem()
          && inventorySlotData.activationHeld()) {
        if (this.pending != null) {
          InventorySlotData.Slot currentSlot =
              inventorySlotData.slots().stream()
                  .filter(item -> item.index() == this.pending.slot().index())
                  .findFirst()
                  .orElse(null);
          int currentValue =
              currentSlot != null
                      && InventoryEnvironmentTracker.Fingerprint.of(currentSlot)
                          .equals(InventoryEnvironmentTracker.Fingerprint.of(this.pending.slot()))
                  ? 0
                  : 1;
          if (currentValue != 0) {
            if (inventorySlotData.ownedCount(this.pending.slot().item())
                > this.pending.ownedCount()) {
              this.count4++;
            }
        ...
        for (InventorySlotData.Slot nextSlot : inventorySlotData.slots()) {
          if (this.entries.getOrDefault(InventoryEnvironmentTracker.Fingerprint.of(nextSlot), 0)
              >= 2) {
        ...
        this.plan2 =
            this.inventoryPlanService2.plan(
                inventorySlotData, inventoryData, this.count2, this.count3, hashSet);
        if (this.plan2.candidates().isEmpty()) {
          this.selection = null;
          if (!this.plan2.complete()) {
            this.timestamp3 = -1L;
            this.text =
                this.plan2.retryBlocked()
            ...
            if (longValue - this.timestamp >= 900L) {
              ...
              if (timing.autoClose()
                  ...
                  && longValue - this.timestamp3 >= timing.closeDelayMs()) {
          ...
          InventoryEnvironmentTracker.Fingerprint fingerprint =
              InventoryEnvironmentTracker.Fingerprint.of(candidate.slot());
          InventoryEnvironmentTracker.Selection currentSelection =
              InventoryEnvironmentTracker.Selection.of(candidate.slot());
          if (!currentSelection.equals(this.selection)) {
            this.selection = currentSelection;
            this.timestamp2 =
                Math.max(this.timestamp2, longValue + this.calculateValue(candidate, timing));
          }
          ...
          if (longValue >= this.timestamp2) {
            ...
            if (enabled) {
              ...
              this.pending =
                  new InventoryEnvironmentTracker.Pending(
                      previousSlot, inventorySlotData.ownedCount(previousSlot.item()), longValue);
  ...
  private int calculateValue(
      InventoryLootPlanner.Candidate candidate, InventoryEnvironmentTracker.Timing timing) {
        ...
        this.count2 < 0
            ...
            : (int)
                Math.min(
                    90.0,
                    Math.hypot(
                            candidate.slot().x() - this.count2, candidate.slot().y() - this.count3)
                        * 0.5);
    return currentX
        + (candidate.slot().item().kind().equipment()
            ? this.calculateValue3(65, 145)
            : this.calculateValue3(15, 55));
  }
  ...
  private int calculateValue2(InventoryEnvironmentTracker.Timing timing) {
    ...
    double doubleValue =
        0.55 * this.value
            ...
            + 0.225 * this.randomGenerator.nextDouble();
    int currentValue =
        timing.clickMinMs() + (int) ((timing.clickMaxMs() - timing.clickMinMs()) * doubleValue);
    return currentValue
        + (this.randomGenerator.nextDouble() < 0.08 ? this.calculateValue3(90, 220) : 0);
  }
  ...
  private record Fingerprint(int slot, String id, Object components, int count) {
    static InventoryEnvironmentTracker.Fingerprint of(InventorySlotData.Slot slot) {
      ...
      return new InventoryEnvironmentTracker.Fingerprint(
          slot.index(),
          inventoryKindData.id(),
          inventoryKindData.components(),
          inventoryKindData.count());
    }
  }
