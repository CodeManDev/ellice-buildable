...
public final class InventoryScoreService {
  ...
  public static double score(InventoryKindData inventoryKindData, List<InventoryKindData> items) {
    ...
    if (inventoryKindData.kind().armor()) {
      ...
      for (InventoryKindData.Kind currentKind :
          List.of(
        ...
        if (currentKind != inventoryKindData.kind()) {
          InventoryKindData currentInventoryKindData =
              items.stream()
      ...
      doubleValue =
          switch (inventoryKindData.kind()) {
            case SWORD, AXE, TRIDENT, MACE -> {
              double targetDoubleValue =
                  currentStats.attackDamage()
                      + (currentStats.enchantment("sharpness") == 0
                          ? 0.0
                          : 0.5 * currentStats.enchantment("sharpness") + 0.5);
              double inputDoubleValue = Math.clamp(currentStats.attackSpeed(), 0.1, 4.0);
            ...
            case BOW ->
                8
                    + currentStats.enchantment("power") * 2
                    + currentStats.enchantment("infinity") * 3
                    + currentStats.enchantment("flame")
                    + currentStats.enchantment("punch") * 0.5;
            case CROSSBOW ->
                8
                    + currentStats.enchantment("quick_charge") * 2
                    + currentStats.enchantment("multishot") * 2
    ...
    if (inventoryKindData.kind().equipment()) {
      doubleValue +=
          currentStats.enchantment("unbreaking") * 0.35 + currentStats.enchantment("mending") * 1.1;
      doubleValue *= 0.78 + 0.22 * Math.sqrt(currentStats.durability());
  ...
  private static double calculateValue3(
    ...
    double previousDoubleValue =
        Math.clamp(
            doubleValue - nextDoubleValue / (2.0 + Math.max(0.0, currentDoubleValue) / 4.0),
