...
public final class SoupClickTracker {
  ...
  public void stop(SoupClickTracker.Environment environment) {
    ...
    if (soupConfirmedData != null
        && soupConfirmedData.ownedInventory()
        && !soupConfirmedData.manualInput()
  ...
  public void tick(
      long longValue, SoupClickTracker.Environment environment, SoupClickTracker.Policy policy) {
    ...
    if (soupConfirmedData == null) {
      ...
      if (soupConfirmedData.manualInput()) {
        ...
        if (this.phase == SoupClickTracker.Phase.USE) {
          SoupConfirmedData.Confirmed currentConfirmed =
              soupConfirmedData.confirmed().get(this.count2);
          int currentCount =
              currentConfirmed.revision() <= this.timestamp4
                      || soupConfirmedData.soups() >= this.count6
                      || currentConfirmed.stack().kind() == SoupKindData.Kind.SOUP
                          && currentConfirmed.stack().count() >= this.soupKindData.count()
                  ? 0
                  : 1;
          if (currentCount != 0) {
            environment.finishUse(this.enabled);
            ...
            this.updateState2(
                longValue, environment, soupConfirmedData, "No soup response from server", policy);
          }
        } else if (this.phase != SoupClickTracker.Phase.IDLE
            && !soupConfirmedData.inventoryOpen()) {
          environment.yieldInventory();
          this.updateState3();
          ...
          if (this.phase == SoupClickTracker.Phase.INGREDIENTS) {
            if (this.step != null
              ...
              this.updateState2(
                  ...
                  environment,
                  soupConfirmedData,
                  "Inventory changed; crafting stopped",
                  policy);
              return;
            }
            ...
            if (!this.items.isEmpty()) {
              ...
              this.updateState2(
                  longValue, environment, soupConfirmedData, "Ingredients changed", policy);
              return;
            }
          ...
          if (this.phase == SoupClickTracker.Phase.RECIPE) {
            if (!soupConfirmedData.cursor().empty()) {
              this.updateState2(
                  longValue, environment, soupConfirmedData, "Cursor changed", policy);
            } else {
              if (soupConfirmedData.confirmed().get(0).revision() > this.timestamp3
                ...
                if (environment.click(soupConfirmedData, 0, 0, SoupClickTracker.Click.QUICK_MOVE)) {
                  ...
                  this.updateState2(
                      longValue, environment, soupConfirmedData, "Crafting result changed", policy);
                }
              } else if (longValue >= this.timestamp2) {
                this.updateState2(
                    longValue,
                    environment,
                    soupConfirmedData,
                    "Server did not supply a soup recipe",
                    policy);
              }
            }
            ...
            if (soupConfirmedData.confirmed().get(0).revision() > this.timestamp3
              ...
              if (policy.needsSoup(soupConfirmedData)
                  || soupConfirmedData.soups() >= policy.stock()) {
                this.updateState(longValue, environment, soupConfirmedData, policy);
              }
            } else if (longValue >= this.timestamp2) {
              this.updateState2(
                  longValue, environment, soupConfirmedData, "Craft was not confirmed", policy);
            }
            ...
            if (currentEnabled && currentValue >= 0) {
            ...
            } else if (!policy.refill()
                || currentValue >= 0
                || soupConfirmedData.inventorySoup() < 0) {
              int nextValue =
                  policy.safeCraft()
                          && (longValue - this.timestamp6 < 1500L
                              || !(soupConfirmedData.health()
                                  >= soupConfirmedData.maxHealth() * 0.8F))
                      ? 0
                      : 1;
              if (policy.craft()
                  && soupConfirmedData.soups() < policy.stock()
                  && (nextValue != 0 || currentEnabled && soupConfirmedData.soups() == 0)) {
                List<SoupClickTracker.Step> currentItems = collectValues(soupConfirmedData);
                if (currentItems.isEmpty()) {
                  this.text = "Need a bowl and both mushrooms";
                  this.updateState(longValue, environment, soupConfirmedData, policy);
                } else if (!soupConfirmedData.hasOutputRoom()
                    && currentItems.stream()
                        .noneMatch(
                            currentItem ->
                                currentItem.slot() >= 9 && currentItem.item().count() == 1)) {
                  this.text = "Need room for a crafted soup";
                  this.updateState(longValue, environment, soupConfirmedData, policy);
                } else if (this.checkCondition(longValue, environment, soupConfirmedData, policy)) {
                  this.timestamp3 = soupConfirmedData.confirmed().get(0).revision();
                  if (soupConfirmedData.hasOutputRoom()
                      && environment.placeRecipe(soupConfirmedData)) {
                    this.phase = SoupClickTracker.Phase.RECIPE;
                    this.timestamp2 = longValue + policy.responseMillis();
                  } else {
                ...
                this.text =
                    currentValue >= 0
                        ? "Ready · " + soupConfirmedData.soups() + " soups"
                        : "No soup in hotbar";
                this.updateState(longValue, environment, soupConfirmedData, policy);
              }
            } else if (this.checkCondition(longValue, environment, soupConfirmedData, policy)) {
              ...
              if (environment.click(
                  soupConfirmedData, this.count3, this.count4 - 36, SoupClickTracker.Click.SWAP)) {
  ...
  private boolean checkCondition(
      long longValue,
      SoupClickTracker.Environment environment,
      SoupConfirmedData soupConfirmedData,
      SoupClickTracker.Policy policy) {
    if (soupConfirmedData.inventoryOpen()) {
      return true;
    }
  ...
  private static List<SoupClickTracker.Step> collectValues(SoupConfirmedData soupConfirmedData) {
    if (soupConfirmedData.cursor().empty() && soupConfirmedData.emptyGrid()) {
      ...
      for (SoupKindData.Kind currentKind :
          List.of(
              SoupKindData.Kind.BOWL,
              SoupKindData.Kind.RED_MUSHROOM,
              SoupKindData.Kind.BROWN_MUSHROOM)) {
        ...
        for (int currentIndex = 9; currentIndex <= 44; currentIndex++) {
          if (((SoupKindData) arrayList.get(currentIndex)).kind() == currentKind
              ...
              && (value < 0
                  || ((SoupKindData) arrayList.get(currentIndex)).count()
                      < ((SoupKindData) arrayList.get(value)).count())) {
            value = currentIndex;
          }
        }
        ...
        currentArrayList.add(
            new SoupClickTracker.Step(
                value, 0, soupKindData, SoupKindData.EMPTY, SoupKindData.EMPTY, soupKindData));
        currentArrayList.add(
            new SoupClickTracker.Step(
                index,
                soupKindData.count() == 1 ? 0 : 1,
                SoupKindData.EMPTY,
                soupKindData,
                soupKindData.count(1),
                currentSoupKindData));
  ...
  public record Policy(
    ...
    public boolean needsSoup(SoupConfirmedData soupConfirmedData) {
      return this.mode == SoupClickTracker.Mode.SOUP_PVP
          ? soupConfirmedData.health() < soupConfirmedData.maxHealth()
              && soupConfirmedData.health() <= this.healthHearts * 2.0F
