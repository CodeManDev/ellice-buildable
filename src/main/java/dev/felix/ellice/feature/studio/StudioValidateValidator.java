...
{
    public static List<String> validate(final StudioShapeService studioShapeService, final boolean b) {
        ...
        if (studioShapeService.name == null || studioShapeService.name.isBlank() || studioShapeService.name.length() > 48) {
            coll.add("Use a project name with 1–48 characters.");
        }
        if (!m7hdfsbvvz93(studioShapeService.width, Float.intBitsToFloat(1107296256), Float.intBitsToFloat(1153957888)) || !m7hdfsbvvz93(studioShapeService.height, Float.intBitsToFloat(1107296256), Float.intBitsToFloat(1148846080))) {
        ...
        for (StudioShapeService.Shape shape : studioShapeService.shapes) {
            if (shape == null || shape.id == null || shape.kind == null || shape.finish == null || shape.alignment == null || shape.name == null || shape.parent == null || shape.text == null) {
                ...
                if (shape.id.length() > 64 || shape.name.length() > 48 || shape.text.length() > 256 || shape.parent.length() > 64) {
                    coll.add("Shape text is too long.");
                }
                if (m7hdfsbvvz93(shape.x, Float.intBitsToFloat(-981860352), Float.intBitsToFloat(1165623296)) && m7hdfsbvvz93(shape.y, Float.intBitsToFloat(-981860352), Float.intBitsToFloat(1165623296)) && m7hdfsbvvz93(shape.width, Float.intBitsToFloat(1082130432), Float.intBitsToFloat(1157234688)) && m7hdfsbvvz93(shape.height, Float.intBitsToFloat(1082130432), Float.intBitsToFloat(1157234688)) && m7hdfsbvvz93(shape.opacity, 0.0f, 1.0f) && m7hdfsbvvz93(shape.radius, 0.0f, Float.intBitsToFloat(1140457472)) && m7hdfsbvvz93(shape.fontSize, Float.intBitsToFloat(1090519040), Float.intBitsToFloat(1126170624)) && m7hdfsbvvz93(shape.stroke, 1.0f, Float.intBitsToFloat(1120403456))) {
                    continue;
                }
        ...
        for (final StudioShapeService.Block block : studioShapeService.blocks) {
            if (block == null || block.id == null || block.kind == null || block.inputs == null || block.defaults == null || block.text == null || block.target == null) {
                ...
                if (!m7hdfsbvvz93(block.x, Float.intBitsToFloat(-969179136), Float.intBitsToFloat(1178304512)) || !m7hdfsbvvz93(block.y, Float.intBitsToFloat(-969179136), Float.intBitsToFloat(1178304512)) || !m7hdfsbvvz93(block.number, Float.intBitsToFloat(-943501312), Float.intBitsToFloat(1203982336)) || block.id.length() > 64 || block.text.length() > 256 || block.target.length() > 80 || block.inputs.size() > 8 || block.defaults.size() > 8) {
                    coll.add("Invalid block properties.");
                }
                for (final Map.Entry entry : block.defaults.entrySet()) {
                    if (block.kind.port((String)entry.getKey()) == null || entry.getValue() == null || !m7hdfsbvvz93((float)entry.getValue(), Float.intBitsToFloat(-943501312), Float.intBitsToFloat(1203982336))) {
        ...
        for (StudioShapeService.Block block2 : studioShapeService.blocks) {
            for (Map.Entry entry2 : block2.inputs.entrySet()) {
                ...
                final StudioShapeService.Block block3 = studioShapeService.block((String)entry2.getValue());
        ...
        if (b && studioShapeService.shapes.isEmpty() && studioShapeService.blocks.stream().noneMatch(block4 -> block4.kind == StudioMode.MODULE_GATE)) {
            coll.add("Add a shape or module action first.");
        }
        return (List<String>)List.copyOf((Collection<?>)coll);
    }
    
    private static boolean mhkyqdmqzra0(final StudioShapeService studioShapeService, final StudioShapeService.Block block, final Map<String, Integer> map) {
    ...
    public static String connectionError(final StudioShapeService studioShapeService, final String s, final String s2, final String s3) {
        ...
        final boolean mhkyqdmqzra0 = mhkyqdmqzra0(studioShapeService, block2, new HashMap<String, Integer>());
        if (s4 == null) {
    ...
    public static Frame evaluate(final StudioShapeService studioShapeService, final LayoutOperationHandler layoutOperationHandler, final float n) {
        ...
        for (final StudioShapeService.Block block : studioShapeService.blocks) {
            if (block.kind.output != null) {
                ...
                switch (block.kind) {
                    ...
                    case SCALE: {
                        appearance2.scale = clamp(input.number, Float.intBitsToFloat(1028443341), Float.intBitsToFloat(1082130432));
                        continue;
                    }
                    case OPACITY: {
                    ...
                    case POSITION_X: {
                        appearance2.x = clamp(input.number, Float.intBitsToFloat(-981860352), Float.intBitsToFloat(1165623296));
                        continue;
                    }
                    case POSITION_Y: {
                        appearance2.y = clamp(input.number, Float.intBitsToFloat(-981860352), Float.intBitsToFloat(1165623296));
                        continue;
                    }
                }
            }
    ...
    private static boolean m7hdfsbvvz93(final float f, final float n, final float n2) {
    ...
    private static final class Evaluation
    {
        ...
        Value value(final StudioShapeService.Block block) {
            ...
            final Value value3 = switch (block.kind) {
                ...
                case MULTIPLY -> Value.number(this.input(block, "A").number * this.input(block, "B").number);
                case DIVIDE -> Value.number((Math.abs(this.input(block, "B").number) < Float.intBitsToFloat(925353388)) ? 0.0f : (this.input(block, "A").number / this.input(block, "B").number));
                case CLAMP -> Value.number(StudioValidateValidator.clamp(this.input(block, "Value").number, 0.0f, 1.0f));
                case WAVE -> Value.number((1.0f + (float)Math.sin(this.input(block, "Time").number * this.input(block, "Speed").number * Double.longBitsToDouble(4614256656552045848L) * Double.longBitsToDouble(4611686018427387904L))) * Float.intBitsToFloat(1056964608));
                case LESS -> Value.bool(this.input(block, "A").number < this.input(block, "B").number);
                case GREATER -> Value.bool(this.input(block, "A").number > this.input(block, "B").number);
                case AND -> Value.bool(this.input(block, "A").truth && this.input(block, "B").truth);
                case NOT -> Value.bool(!this.input(block, "Value").truth);
                case FORMAT -> Value.text(this.vars.interpolate(block.text.replace("{value}", this.input(block, "Value").caption())));
                case MIX_COLOR -> Value.color(StudioValidateValidator.mix(this.input(block, "From").color, this.input(block, "To").color, this.input(block, "Amount").number));
    ...
    public record Value(StudioMode.Type type, float number, String text, int color, boolean truth) {
        static Value number(final float n) {
            return new Value(StudioMode.Type.NUMBER, StudioValidateValidator.mevwdwqvjs3m(n), "", 0, false);
        }
        ...
        public String caption() {
            return switch (this.type) {
                default -> throw new MatchException(null, null);
                case NUMBER -> String.format(Locale.ROOT, (Math.abs(this.number - Math.round(this.number)) < Double.longBitsToDouble(4562254508917369340L)) ? "%.0f" : "%.2f", this.number);
                case TEXT -> this.text;
    ...
    public record Frame(Map<String, Appearance> shapes, Map<String, Value> values, Map<String, Boolean> moduleGates) {}
}
