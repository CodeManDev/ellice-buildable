...
public final class StudioOpenPanelService extends ModuleSettingsService implements ModuleOperationHandler {
   final StudioShapeService project;
   private final ModuleRegisterService moduleRegisterService;
   private final Runnable runnable;
   final ModuleSetting.Number x = this.setting(
      new ModuleSetting.Number("Offset X", 16.0F, 0.0F, 2000.0F, 1.0F).exactInput()
   );
   final ModuleSetting.Number y = this.setting(
      new ModuleSetting.Number("Offset Y", 16.0F, 0.0F, 2000.0F, 1.0F).exactInput()
   );
   final ModuleSetting.Number scale = this.setting(
      new ModuleSetting.Number(
         "Scale", 0.6F, 0.25F, 2.0F, 0.05F
      )
   );
   final ModuleSetting.Number opacity = this.setting(new ModuleSetting.Number("Opacity", 1.0F, 0.0F, 1.0F, 0.05F));
   final ModuleSetting.Mode anchor = this.setting(
      new ModuleSetting.Mode("Anchor", new String[]{"Top left", "Top right", "Bottom left", "Bottom right", "Center"}, "Top left")
   );
   private final Map<Module, StudioOpenPanelService.Gate> entries = new LinkedHashMap<>();
   private String text = "";

   StudioOpenPanelService(StudioShapeService studioShape, ModuleRegisterService moduleRegister, Runnable currentRunnable) {
      super(
         ModuleBuilderData.builder(studioShape.name)
            .description("Made in Module Studio · " + studioShape.shapes.size() + " shapes · " + studioShape.blocks.size() + " blocks")
   ...
   void gates(Map<String, Boolean> currentEntries) {
      for (Entry entry : currentEntries.entrySet()) {
         ...
         if (module != null && !(module instanceof StudioOpenPanelService) && !(module instanceof ImplOpenPanelService)) {
