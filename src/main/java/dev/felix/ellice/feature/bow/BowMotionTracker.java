...
public final class BowMotionTracker {
  ...
  public BowMotionTracker.Motion observe(
      long longValue, UUID currentUUID, RotationVector rotationVector, boolean enabled) {
    if (!currentUUID.equals(this.uUID)
        || this.rotationData72 == null
        || longValue - this.timestamp > 3L
        || longValue <= this.timestamp
        || rotationVector.subtract(this.rotationData72).length()
            > 1.5 * (longValue - this.timestamp)) {
      this.clear();
      ...
      double currentDoubleValue =
          Math.max(1L, longValue - this.bowObserveService.estimate().tick());
      RotationVector currentRotationVector =
          rotationVector.subtract(this.rotationData72).multiply(1.0 / currentDoubleValue);
      BowObserveService.Estimate currentEstimate =
          this.bowObserveService.observe(longValue, rotationVector);
      ...
      this.rotationData74 =
          createRotationData7(
              new RotationVector(
                  currentX.x() - this.rotationData73.x(),
                  0.0,
                  currentX.z() - this.rotationData73.z()),
              0.045);
      ...
      int currentValue =
          Math.max(3, Math.min(8, (int) Math.ceil(this.value + this.value2 * 2.0 + 1.0)));
      ...
      if (enabled) {
        this.rotationData73 =
            new RotationVector(this.rotationData73.x(), 0.0, this.rotationData73.z());
      }
    ...
    return new BowMotionTracker.Motion(
        this.rotationData73,
        this.rotationData74,
        nextDoubleValue,
        enabled,
  ...
  private static RotationVector createRotationData7(
    ...
    return rotationVector.length() > doubleValue
        ? rotationVector.multiply(doubleValue / rotationVector.length())
        : rotationVector;
  }

  public record Motion(
      RotationVector velocity,
      RotationVector acceleration,
      double confidence,
      boolean grounded,
      int ageTicks,
