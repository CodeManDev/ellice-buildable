...
public final class BowTrajectorySolver {
  ...
  public static Optional<BowTrajectorySolver.Solution> solve(
      RotationVector rotationVector,
      RotationVector currentRotationVector,
      double doubleValue,
      DoubleFunction<RotationVector> doubleFunction) {
    if (Double.isFinite(doubleValue) && !(doubleValue < 0.3) && !(doubleValue > 3.0)) {
      double currentDoubleValue = 0.05;
      double currentLength =
          createRotationData7(
                      rotationVector,
                      currentRotationVector,
                      (RotationVector) doubleFunction.apply(currentDoubleValue),
                      currentDoubleValue)
                  .length()
              - doubleValue;

      for (double nextDoubleValue = 0.25; nextDoubleValue <= 80.0; nextDoubleValue += 0.25) {
        double nextLength =
            createRotationData7(
                        rotationVector,
                        currentRotationVector,
        ...
        if (currentLength >= 0.0 && nextLength <= 0.0) {
          ...
          for (int index = 0; index < 28; index++) {
            ...
            if (createRotationData7(
                        rotationVector,
                        currentRotationVector,
                        (RotationVector) doubleFunction.apply(targetDoubleValue),
                        targetDoubleValue)
                    .length()
                > doubleValue) {
          ...
          RotationVector previousRotationVector =
              createRotationData7(
                  rotationVector, currentRotationVector, nextRotationVector, inputDoubleValue);
          return Optional.of(
              new BowTrajectorySolver.Solution(
                  RotationData.lookAt(new RotationVector(0.0, 0.0, 0.0), previousRotationVector),
                  inputDoubleValue,
                  nextRotationVector));
        }
  ...
  public static RotationVector position(
      RotationVector rotationVector, RotationVector currentRotationVector, double width) {
    if (Double.isFinite(width) && !(width < 0.0) && !(width > 85.0)) {
      double height = calculateValue(width);
      return rotationVector
          .add(currentRotationVector.multiply(height))
          .add(new RotationVector(0.0, -calculateValue2(width), 0.0));
    } else {
      throw new IllegalArgumentException("Invalid flight time");
    }
  }

  public static OptionalDouble hitFraction(
      RotationVector rotationVector,
      RotationVector currentRotationVector,
    ...
    for (int index = 0; index < 3; index++) {
      ...
      if (Math.abs(nextX) < 1.0E-12) {
        ...
        currentDoubleValue =
            Math.min(currentDoubleValue, Math.max(sourceDoubleValue, targetDoubleValue));
  ...
  private static RotationVector createRotationData7(
    ...
    return nextRotationVector
        ...
        .add(new RotationVector(0.0, calculateValue2(doubleValue), 0.0))
        .multiply(1.0 / calculateValue(doubleValue))
  ...
  private static double calculateValue(double doubleValue) {
    ...
    return (1.0 - currentDoubleValue) / 0.009999990463256836
        + (doubleValue - value) * currentDoubleValue;
  }
