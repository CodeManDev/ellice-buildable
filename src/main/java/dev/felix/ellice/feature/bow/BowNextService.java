...
public final class BowNextService {
  ...
  public BowNextService.Step next(
    ...
    double sourceDoubleValue =
        RotationData.clampPitch(currentRotationData.pitch()) - rotationData.pitch();
    double targetDoubleValue = currentDoubleValue / (1.0 + nextDoubleValue * 5.0);
    this.value =
        calculateValue2(
            this.value,
            calculateValue(previousDoubleValue, currentDoubleValue, targetDoubleValue),
            targetDoubleValue);
    this.value2 =
        calculateValue2(
            this.value2,
            calculateValue(sourceDoubleValue, currentDoubleValue, targetDoubleValue),
            targetDoubleValue);
    ...
    RotationVanillaGcdService.QuantizedRotation quantizedRotation =
        this.rotationVanillaGcdService.quantize(
            rotationData,
            new RotationData(
                rotationData.yaw() + this.value,
                RotationData.clampPitch(rotationData.pitch() + this.value2)),
            doubleValue,
            0.0);
  ...
  private static double calculateValue(
    ...
    return Math.copySign(
        Math.min(currentDoubleValue, Math.sqrt(2.0 * nextDoubleValue * Math.abs(doubleValue))),
        doubleValue);
  }
  ...
  private static double calculateValue2(
      double doubleValue, double currentDoubleValue, double nextDoubleValue) {
    return doubleValue
        + Math.max(-nextDoubleValue, Math.min(nextDoubleValue, currentDoubleValue - doubleValue));
  }
