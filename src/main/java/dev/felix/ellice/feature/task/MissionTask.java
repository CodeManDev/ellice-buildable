...
public final class MissionTask implements TaskStep {
  ...
  public static MissionTask stoneAge() {
    return new MissionTask(
        ...
        taskData -> {
          boolean enabled =
              Stream.of("stone", "iron", "diamond", "netherite")
                  .anyMatch(material -> taskData.count("minecraft:" + material + "_pickaxe") > 0);
          return enabled
              ? List.of()
              : List.of(new CraftingTask(new CraftingTask.Config("minecraft:stone_pickaxe", 1)));
        });
  }
  ...
  public TaskAction tick(long longValue, TaskOperationHandler taskOperation) {
    ...
    if (taskData.threatened()) {
      return this.createTaskOperationHandler2(
          taskOperation, taskData, "Players nearby — standing down");
    }
    ...
    if (this.taskOperationHandler3 == null) {
      ...
      if (this.taskOperationHandler3 == null) {
        return this.createTaskOperationHandler23(
            new TaskAction.Done("Mission complete: " + this.text));
      }
  ...
  private TaskAction createTaskOperationHandler2(
      TaskOperationHandler taskOperation, TaskData taskData, String currentText) {
    ...
    this.travelStatusTracker.start(
        nearGoal, TravelPolicy.legit(taskData.canSprint()), taskData.player());
  ...
  private TaskAction createTaskOperationHandler22(
    ...
    if (this.travelStatusTracker.status() == TravelStatusTracker.Status.ARRIVED) {
      return this.createTaskOperationHandler23(
          new TaskAction.Abort("Mission aborted (" + this.text2 + ") — back at anchor"));
      ...
      return this.createTaskOperationHandler23(
          new TaskAction.Abort("Mission aborted (" + this.text2 + ") — anchor unreachable"));
    }
  }
