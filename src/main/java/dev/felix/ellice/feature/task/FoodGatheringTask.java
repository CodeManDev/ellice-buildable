...
public final class FoodGatheringTask implements TaskStep {
  public static final List<FoodGatheringTask.CropKind> CROPS =
      List.of(
          ...
          new FoodGatheringTask.CropKind(
              Set.of("minecraft:beetroots"), 3, "minecraft:beetroot_seeds"),
  ...
  public static final Set<String> PREY =
      Set.of(
          "minecraft:cow",
          "minecraft:pig",
          "minecraft:sheep",
  ...
  public TaskAction tick(long longValue, TaskOperationHandler taskOperation) {
    ...
    if (edible(taskData) >= this.config2.wantItems()) {
      return this.createTaskOperationHandler28(
          new TaskAction.Done("Gathered " + edible(taskData) + " food"));
    }
  ...
  private TaskAction createTaskOperationHandler22(
    ...
    if (this.count3 > 0) {
      return this.createTaskOperationHandler28(
          new TaskAction.Done(
              "Gathered "
                  + this.count3
                  + "/"
                  + this.config2.wantItems()
                  + " food — wilds are empty"));
      ...
      return this.createTaskOperationHandler28(
          new TaskAction.Abort("No food found — wilds are empty"));
    }
  }
  ...
  private static boolean checkCondition(
      TaskWorldView taskWorldView,
      int value,
      int currentValue,
      int nextValue,
      FoodGatheringTask.CropKind cropKind) {
  ...
  private TaskAction createTaskOperationHandler23(
    ...
    if (this.taskData2 != null) {
      if (this.taskData2.distSq(taskData.player().x, taskData.player().y, taskData.player().z)
          > 27.040000000000003) {
        this.taskData2 = null;
        this.text = "";
        this.taskData23 = null;
        ...
        String currentX =
            taskOperation
                .blocks()
                .blockId(this.taskData2.x(), this.taskData2.y(), this.taskData2.z());
          ...
          if (!nextText.isEmpty()
              && SOIL.contains(
                  taskOperation
                      .blocks()
                      .blockId(
                          taskBlockPosition.x(), taskBlockPosition.y() - 1, taskBlockPosition.z()))
              && taskOperation
                  .blocks()
    ...
    for (TaskNearestService.Find find :
      ...
      if (nextCropKind != null
          && checkCondition(taskOperation.blocks(), find.x(), find.y(), find.z(), nextCropKind)) {
        TaskBlockPosition currentTaskBlockPosition = find.pos();
        if (!this.values2.contains(currentTaskBlockPosition)) {
          if (currentTaskBlockPosition.distSq(
      ...
      this.taskConfigTracker3 =
          new ExplorationTask(
              ExplorationTask.Config.standard(
                  hashSet, this.config2.scanRadius(), this.config2.yRadius()),
              "food");
    }
  ...
  private static TaskOperationHandler.SeenEntity createSeenEntity(
  ...
  private TaskAction createTaskOperationHandler25(
    ...
    TaskOperationHandler.SeenEntity seenEntity =
        taskOperation.entities().stream()
            .filter(item -> item.ref() == this.count9)
            .findFirst()
            .orElse(null);
      ...
      this.taskStateController =
          new TaskStateController(
              ...
              new TaskBlockPosition(
                  (int) Math.floor(this.value5),
                  (int) Math.floor(this.value6),
                  (int) Math.floor(this.value7)));
    ...
    if (calculateValue(seenEntity, taskData) > 9.0) {
      TaskBlockPosition currentX =
          new TaskBlockPosition(
              (int) Math.floor(seenEntity.x()),
              (int) Math.floor(seenEntity.y()),
              (int) Math.floor(seenEntity.z()));
  ...
  private static double calculateValue(
      TaskOperationHandler.SeenEntity seenEntity, TaskData taskData) {
  ...
  private TaskAction createTaskOperationHandler26(
    ...
    if (taskBlockPosition.distSq(taskData.player().x, taskData.player().y, taskData.player().z)
        > 17.64) {
      return this.createTaskOperationHandler2(
          taskOperation, taskData, taskBlockPosition, 2.5, "Getting to water");
    }
  ...
  private static TaskOperationHandler.SeenEntity createSeenEntity2(
      TaskOperationHandler taskOperation, TaskData taskData) {
    TaskOperationHandler.SeenEntity seenEntity = null;
  ...
  private TaskAction createTaskOperationHandler27(
    ...
    this.count4 = value;
    if (this.count7 < this.config2.maxChests()
        && (this.taskData2 == null || size - this.timestamp3 <= 6000L)) {
      if (this.taskData2 == null) {
        for (TaskNearestService.Find find :
            TaskNearestService.nearest(
                taskOperation.blocks(),
  ...
  private static boolean checkCondition2(
    ...
    for (int[] currentInts : ints) {
      if (taskOperation
          .blocks()
          .blockId(find.x() + currentInts[0], find.y(), find.z() + currentInts[1])
          .equals(find.id())) {
        return true;
      }
    }
