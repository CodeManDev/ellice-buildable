...
public final class TaskNearestService {
  ...
  public static List<TaskNearestService.Find> nearest(
    ...
    if (!values.isEmpty()
      ...
      int targetValue =
          Math.max(taskWorldView.minY(), (int) Math.floor(currentDoubleValue - currentValue));
      int inputValue =
          Math.min(taskWorldView.maxY(), (int) Math.ceil(currentDoubleValue + currentValue));
      ArrayList arrayList = new ArrayList();

      for (int index = previousValue - value; index <= previousValue + value; index++) {
        for (int currentIndex = sourceValue - value;
            currentIndex <= sourceValue + value;
            currentIndex++) {
      ...
      return arrayList.size() > nextValue
          ? List.copyOf(arrayList.subList(0, nextValue))
