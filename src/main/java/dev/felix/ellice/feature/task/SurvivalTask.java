...
public final class SurvivalTask implements TaskStep {
  ...
  public TaskAction tick(long longValue, TaskOperationHandler taskOperation) {
    ...
    SurvivalAssessService.Advice advice =
        SurvivalAssessService.assess(taskData.vitals(), this.config2.vitals());
    switch (advice.threat()) {
      ...
      default:
        this.enabled = false;
        if (FoodGatheringTask.edible(taskData) < this.config2.foodWant()
          ...
          return TaskDayNumberService.isNight(taskOperation.timeOfDay())
                  && taskOperation.skyAbove()
                  && currentLongValue != this.timestamp
              ? this.createTaskOperationHandler24(
                  longValue, taskOperation, taskData, currentLongValue)
  ...
  private TaskAction createTaskOperationHandler22(
    ...
    if (!this.enabled || this.travelStatusTracker.status() == TravelStatusTracker.Status.ARRIVED) {
      this.enabled = true;
      TravelIsInGoalHandler.NearGoal nearGoal =
          new TravelIsInGoalHandler.NearGoal(value, taskData.player().y + 5.0, currentValue, 1.2);
    ...
    TravelIsInGoalHandler.NearGoal currentNearGoal =
        new TravelIsInGoalHandler.NearGoal(value, taskData.player().y + 5.0, currentValue, 1.2);
  ...
  private TaskAction createTaskOperationHandler23(
    ...
    if (this.mode != SurvivalTask.Mode.FOOD || this.taskConfigTracker7 == null) {
      ...
      this.taskConfigTracker7 =
          new FoodGatheringTask(FoodGatheringTask.Config.standard(this.config2.foodWant()));
    }
  ...
  private TaskAction createTaskOperationHandler24(
      long size, TaskOperationHandler taskOperation, TaskData taskData, long currentSize) {
