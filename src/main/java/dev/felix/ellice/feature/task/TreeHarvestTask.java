...
public final class TreeHarvestTask implements TaskStep {
  ...
  public TreeHarvestTask(
      TreeHarvestTask.Config currentConfig, ToIntFunction<TaskData> currentToIntFunction) {
  ...
  private TaskAction createTaskOperationHandler2(
    ...
    if (this.calculateValue(taskData) == 0) {
      return this.createTaskOperationHandler210(
          new TaskAction.Done("Collected " + this.count6 + " logs"));
    }

    List currentItems =
        TaskNearestService.nearest(
    ...
    if (currentItems.isEmpty()) {
      if (!this.enabled3) {
        ...
        return this.createTaskOperationHandler210(
            new TaskAction.Abort("No trees anywhere — ground is empty"));
      }
    } else {
      this.find = this.createFind(taskOperation, (TaskNearestService.Find) currentItems.getFirst());
      TaskBlockPosition currentX =
          new TaskBlockPosition(this.find.x(), this.find.y(), this.find.z());
      if (currentX.equals(this.taskData22) && this.count6 == this.count10) {
        if (++this.count11 >= 2) {
          return this.createTaskOperationHandler210(
              new TaskAction.Abort("Trees will not break — moving on"));
        }
      } else {
        this.count11 = 0;
      ...
      while (this.count5 - this.find.y() < 12
          && this.config2
              .logIds()
              .contains(
                  taskOperation.blocks().blockId(this.find.x(), this.count5 + 1, this.find.z()))) {
        this.count5++;
      }
      ...
      TravelIsInGoalHandler.NearGoal nextX =
          new TravelIsInGoalHandler.NearGoal(this.find.x(), this.find.y(), this.find.z(), 2.5);
  ...
  private TaskNearestService.Find createFind(
      TaskOperationHandler taskOperation, TaskNearestService.Find find) {
    int currentY = find.y();

    while (currentY - 1 >= taskOperation.blocks().minY()
        && this.config2
            .logIds()
            .contains(taskOperation.blocks().blockId(find.x(), currentY - 1, find.z()))) {
      currentY += -1;
    }

    return new TaskNearestService.Find(
        find.x(),
        currentY,
        find.z(),
        taskOperation.blocks().blockId(find.x(), currentY, find.z()),
        find.distSq());
  }
  ...
  private TaskAction createTaskOperationHandler23(
    ...
    if (nextX.distSq(taskData.player().x, taskData.player().y, taskData.player().z)
        > 27.040000000000003) {
      this.stage = TreeHarvestTask.Stage.GOTO;
      TravelIsInGoalHandler.NearGoal previousX =
          new TravelIsInGoalHandler.NearGoal(this.find.x(), this.count4, this.find.z(), 1.0);
      this.travelStatusTracker.start(
          previousX, this.createTravelData2(taskData), taskData.player());
  ...
  private void updateState(TaskOperationHandler taskOperation, TaskData taskData) {
    List<TaskNearestService.Find> currentX =
        TaskNearestService.nearest(
            taskOperation.blocks(),
            this.find.x() + 0.5,
            this.find.y() + 2,
    ...
    arrayList.sort(
        (item, currentItem) -> {
          double doubleValue =
              item.pos().distSq(taskData.player().x, taskData.player().y, taskData.player().z);
        ...
        && !this.config2
            ...
            .contains(
                taskOperation
                    .blocks()
                    .blockId(
                        this.items.getFirst().x(),
                        this.items.getFirst().y(),
                        this.items.getFirst().z()))) {
  ...
  private TaskAction createTaskOperationHandler25(
      long size, TaskOperationHandler taskOperation, TaskData taskData, boolean currentEnabled) {
    ...
    this.taskStateController =
        new TaskStateController(
            hashSet, new TaskBlockPosition(this.find.x(), this.find.y(), this.find.z()));
    this.stage = TreeHarvestTask.Stage.COLLECT;
    return this.createTaskOperationHandler26(size, taskOperation, taskData);
  }

  private TaskAction createTaskOperationHandler26(
      long size, TaskOperationHandler taskOperation, TaskData taskData) {
    TaskAction taskAction = this.taskStateController.tick(size, taskOperation);
    this.text3 =
        "Collecting wood ("
            + this.createText(taskData)
            + ") · "
            + this.taskStateController.status();
    if (taskAction instanceof TaskAction.Abort abort) {
      ...
      if (this.calculateValue(taskData) != 0 || !text.isEmpty() && taskData.count(text) <= 0) {
        if (this.count6 == this.count8 && this.calculateValue(taskData) > 0) {
          if (++this.count9 >= 2) {
            return this.createTaskOperationHandler210(
                new TaskAction.Abort("No wood reached the inventory"));
          }
        } else {
          this.count9 = 0;
        }
  ...
  private TaskAction createTaskOperationHandler27(
      long size, TaskOperationHandler taskOperation, TaskData taskData) {
    TaskBlockPosition currentX = new TaskBlockPosition(this.find.x(), this.find.y(), this.find.z());
    String text = this.config2.saplingFor().getOrDefault(this.find.id(), "");
    if (this.enabled2) {
      this.text3 =
          taskOperation.blocks().blockId(currentX.x(), currentX.y(), currentX.z()).equals(text)
              ? "Replanted"
              : "Replant failed";
      return this.createTaskOperationHandler29(size, taskOperation, taskData);
    } else {
      String nextX =
          taskOperation.blocks().blockId(this.find.x(), this.find.y() - 1, this.find.z());
      boolean previousX =
          taskOperation
              .blocks()
              .blockId(currentX.x(), currentX.y(), currentX.z())
              .equals("minecraft:air");
      if (!text.isEmpty()
          && this.config2.soilIds().contains(nextX)
          && previousX
          && taskData.count(text) > 0) {
        this.enabled2 = true;
  ...
  private TaskAction createTaskOperationHandler28(
      long size, TaskOperationHandler taskOperation, TaskData taskData) {
    if (this.taskConfigTracker3 == null) {
      this.taskConfigTracker3 =
          new ExplorationTask(
              ExplorationTask.Config.standard(
                  this.config2.logIds(), this.config2.scanRadius(), this.config2.yRadius()),
              "trees");
    }
    ...
    if (taskAction instanceof TaskAction.Done) {
      ...
      return this.createTaskOperationHandler210(
          new TaskAction.Abort("No trees anywhere: " + abort.reason()));
    } else {
  ...
  private TaskAction createTaskOperationHandler29(
      long size, TaskOperationHandler taskOperation, TaskData taskData) {
    if (this.calculateValue(taskData) == 0) {
      return this.createTaskOperationHandler210(
          new TaskAction.Done("Collected " + this.count6 + " logs"));
  ...
  public record Config(
    ...
    public static TreeHarvestTask.Config overworld(int value) {
      ...
      return new TreeHarvestTask.Config(
          values, entries, currentValues, nextValues, value, 24, 10, 12);
    }
  }
