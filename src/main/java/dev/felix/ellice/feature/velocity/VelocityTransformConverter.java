...
public final class VelocityTransformConverter {
  ...
  public static RotationVector transform(
      VelocityTransformConverter.Mode mode,
    ...
    if (mode != VelocityTransformConverter.Mode.PUSH
        && mode != VelocityTransformConverter.Mode.JUMP) {
      if (mode == VelocityTransformConverter.Mode.CANCEL
          || doubleValue == 0.0 && currentDoubleValue == 0.0) {
      ...
      RotationVector nextRotationVector =
          enabled ? currentRotationVector : currentRotationVector.subtract(rotationVector);
      double nextDoubleValue =
          doubleValue / 100.0 * (mode == VelocityTransformConverter.Mode.REVERSE ? -1 : 1);
      RotationVector currentX =
          new RotationVector(
              nextRotationVector.x() * nextDoubleValue,
              nextRotationVector.y() * currentDoubleValue / 100.0,
              nextRotationVector.z() * nextDoubleValue);
