...
public final class TravelStatusTracker {
   ...
   private final PriorityQueue<TravelStatusTracker.Entry> priorityQueue = new PriorityQueue<>(
      Comparator.comparingDouble(TravelStatusTracker.Entry::f).thenComparing((item, currentItem) -> Double.compare(currentItem.step.g, item.step.g))
   );
   ...
   public void favorPath(List<TravelData> items) {
      ...
      if (items != null) {
         for (TravelData travelData : items) {
            ...
            this.values2.add(key((int)Math.floor(vector3d.x), (int)Math.floor(vector3d.z), vector3d.y));
         }
      }
   }

   public void start(TravelIsInGoalHandler travelIsInGoal, TravelPolicy travelPolicy, Vector3d currentVector3d) {
      ...
      if (!this.travelLoadedHandler.loaded(value, currentValue)) {
         ...
         if (doubleValue == null) {
            ...
            TravelStatusTracker.Step step = new TravelStatusTracker.Step(key(value, currentValue, doubleValue), doubleValue, 0.0, null, null);
            this.entries.put(step.key, 0.0);
            this.priorityQueue.add(new TravelStatusTracker.Entry(step, this.value5 * travelIsInGoal.heuristic(value, doubleValue, currentValue)));
   ...
   public void tick(Vector3d vector3d, long longValue) {
      if (this.travelIsInGoalHandler != null) {
         ...
         if (this.status2 == TravelStatusTracker.Status.PLANNING || this.status2 == TravelStatusTracker.Status.READY && !this.enabled) {
         ...
         if (this.status2 == TravelStatusTracker.Status.READY) {
            ...
            if (this.checkCondition4(vector3d)) {
               ...
               for (int index = this.count5; index < Math.min(this.items.size(), this.count5 + 12); index++) {
                  Vector3d currentVector3d = this.items.get(index).pos();
                  Double doubleValue = this.travelLoadedHandler.floor((int)Math.floor(currentVector3d.x), (int)Math.floor(currentVector3d.z), currentVector3d.y);
         ...
         } else if ((this.status2 == TravelStatusTracker.Status.UNEXPLORED || this.status2 == TravelStatusTracker.Status.UNAVAILABLE)
   ...
   private void updateState(long offset, long currentOffset) {
      ...
      while (!this.priorityQueue.isEmpty() && System.nanoTime() < nextOffset) {
         ...
         if (!(
            currentStep.g
               > this.entries.getOrDefault(currentStep.key, Double.POSITIVE_INFINITY)
                  + 1.0E-9
         )) {
            ...
            if (this.travelIsInGoalHandler.isInGoal(currentStep.key.x, currentStep.y, currentStep.key.z)) {
      ...
      } else if (this.status2 == TravelStatusTracker.Status.READY && !this.enabled && this.priorityQueue.isEmpty()) {
         this.enabled = true;
         this.timestamp2++;
      }
   }
   ...
   private void updateState2(TravelStatusTracker.Step step) {
      for (TravelTypeData travelTypeData : TravelTypeData.generate(this.travelData2, this.travelLoadedHandler, step.key.x, step.y, step.key.z)) {
         double doubleValue = travelTypeData.costTicks();
         if (this.values2.contains(key(travelTypeData.destX(), travelTypeData.destZ(), travelTypeData.destY()))) {
         ...
         if (step.parent != null
            ...
            && TravelTypeData.lineOfSight(
               this.travelLoadedHandler, step.parent.key.x, step.parent.y, step.parent.key.z, travelTypeData.destX(), travelTypeData.destY(), travelTypeData.destZ()
            )) {
            double nextDoubleValue = step.parent.g
               + TravelFallCostService.walkCost(
                  Math.hypot(travelTypeData.destX() - step.parent.key.x, travelTypeData.destZ() - step.parent.key.z), this.travelData2.canSprint()
               )
         ...
         TravelStatusTracker.Key currentKey = key(travelTypeData.destX(), travelTypeData.destZ(), travelTypeData.destY());
         if (!(
            currentDoubleValue + 0.01
               >= this.entries.getOrDefault(currentKey, Double.POSITIVE_INFINITY)
         )) {
            this.entries.put(currentKey, currentDoubleValue);
            TravelStatusTracker.Step nextStep = new TravelStatusTracker.Step(currentKey, travelTypeData.destY(), currentDoubleValue, currentStep, travelTypeData);
            this.priorityQueue
               .add(new TravelStatusTracker.Entry(nextStep, currentDoubleValue + this.value5 * this.travelIsInGoalHandler.heuristic(currentKey.x, travelTypeData.destY(), currentKey.z)));
         }
      }
   }
   ...
   private void updateState3() {
      ...
      for (TravelStatusTracker.Entry entry : (Iterable<TravelStatusTracker.Entry>) (Iterable<?>) (arrayList)) {
         TravelStatusTracker.Step currentStep = entry.step;
         this.priorityQueue
            .add(new TravelStatusTracker.Entry(currentStep, currentStep.g + this.value5 * this.travelIsInGoalHandler.heuristic(currentStep.key.x, currentStep.y, currentStep.key.z)));
      }
   }

   private void updateState4() {
      for (double doubleValue : new double[]{
         1.0,
         1.5,
         2.5,
         5.0
         ...
         for (TravelStatusTracker.Entry entry : this.priorityQueue) {
            ...
            double nextDoubleValue = Math.hypot(
               nextStep.key.x + 0.5 - this.vector3d.x,
               nextStep.key.z + 0.5 - this.vector3d.z
            );
            if (!(nextDoubleValue < 5.0)) {
               double previousDoubleValue = nextStep.g + doubleValue * this.travelIsInGoalHandler.heuristic(nextStep.key.x, nextStep.y, nextStep.key.z);
   ...
   private void updateState5(TravelStatusTracker.Step step, boolean enabled) {
      ...
      for (TravelStatusTracker.Step currentStep = step; currentStep != null; currentStep = currentStep.parent) {
      ...
      if (currentArrayList.size() == 1) {
         currentArrayList.add(
            TravelData.walk(
               new Vector3d(((TravelData)currentArrayList.getFirst()).pos()).add(0.01, 0.0, 0.0), 0.0
            )
         );
      }
      ...
      for (int index = 1; index < this.items.size(); index++) {
         this.value6 = this.value6 + this.items.get(index - 1).pos().distance(this.items.get(index).pos());
      }
   ...
   private ArrayList<TravelData> collectValues(ArrayList<TravelStatusTracker.Step> arrayList) {
      ...
      if (arrayList.size() <= 2) {
         ...
         for (int currentIndex = 2; currentIndex < arrayList.size(); currentIndex++) {
            ...
            Vector3d currentVector3d = createVector3d((TravelStatusTracker.Step)arrayList.get(currentIndex));
            ...
            if (value == 0
               ...
               Vector3d nextVector3d = createVector3d((TravelStatusTracker.Step)arrayList.get(currentIndex - 1));
               double doubleValue = Math.hypot(nextVector3d.x - vector3d.x, nextVector3d.z - vector3d.z);
               currentArrayList.add(TravelData.walk(nextVector3d, TravelFallCostService.walkCost(doubleValue, this.travelData2.canSprint())));
            }
         }
   ...
   private static Vector3d createVector3d(TravelStatusTracker.Step step) {
      return new Vector3d(
         step.key.x + 0.5, step.y, step.key.z + 0.5
      );
   }
   ...
   private void updateState6(Vector3d vector3d) {
      ...
      for (int index = 1; index < this.items.size(); index++) {
         ...
         double inputDoubleValue = previousDoubleValue * previousDoubleValue + sourceDoubleValue * sourceDoubleValue + targetDoubleValue * targetDoubleValue;
         double outputDoubleValue = inputDoubleValue < 1.0E-9
            ? 0.0
            : ((vector3d.x - currentVector3d.x) * previousDoubleValue + (vector3d.y - currentVector3d.y) * sourceDoubleValue + (vector3d.z - currentVector3d.z) * targetDoubleValue) / inputDoubleValue;
         ...
         double defaultDoubleValue = (resultDoubleValue - vector3d.x) * (resultDoubleValue - vector3d.x) + (candidateDoubleValue - vector3d.y) * (candidateDoubleValue - vector3d.y) + (selectedDoubleValue - vector3d.z) * (selectedDoubleValue - vector3d.z);
   ...
   private boolean checkCondition4(Vector3d vector3d) {
      ...
      return doubleValue < 2.0
         && Math.abs(vector3d.y - currentVector3d.y) < 2.0
         && !this.enabled2;
   }

   private Double calculateValue(int value, int currentValue, double doubleValue) {
      long longValue = (long)value << 42 ^ (long)currentValue << 20 ^ Math.round(doubleValue * 8.0) & 1048575L;
   ...
   public enum Status {
      ...
      private static TravelStatusTracker.Status[] $values() {
         return new TravelStatusTracker.Status[]{IDLE, PLANNING, READY, ARRIVED, UNAVAILABLE, UNEXPLORED};
      }
   }

   private record Step(TravelStatusTracker.Key key, double y, double g, TravelStatusTracker.Step parent, TravelTypeData move) {
   }
}
