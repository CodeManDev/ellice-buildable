...
public record TravelTypeData(
  ...
  static TravelTypeData walk(
      int value,
      double doubleValue,
      int currentValue,
      double currentDoubleValue,
      TravelTypeData.Type type) {
    return new TravelTypeData(
        value, doubleValue, currentValue, currentDoubleValue, type, false, 0, 0, 0);
  }

  static TravelTypeData place(
      int value,
      double doubleValue,
      int currentValue,
      double currentDoubleValue,
  ...
  public static List<TravelTypeData> generate(
    ...
    double nextDoubleValue =
        currentCanSprint
            ? TravelFallCostService.SPRINT_DIAGONAL
            : TravelFallCostService.WALK_DIAGONAL;
      ...
      for (int[] currentInts : DIAGONAL) {
        ...
        if (travelLoaded.loaded(sourceValue, targetValue)) {
          Double sourceDoubleValue =
              travelLoaded.floor(value + currentInts[0], currentValue, doubleValue);
          Double targetDoubleValue =
              travelLoaded.floor(value, currentValue + currentInts[1], doubleValue);
  ...
  private static void updateState(
    ...
    if (travelLoaded.headroom(nextValue, currentDoubleValue, previousValue)) {
      TravelLoadedHandler.Hazard currentHazard =
          travelLoaded.hazard(nextValue, currentDoubleValue, previousValue);
      if (currentHazard != TravelLoadedHandler.Hazard.VOID
          && currentHazard != TravelLoadedHandler.Hazard.DANGER) {
        double previousDoubleValue = currentDoubleValue - doubleValue;
        int sourceValue =
            !travelLoaded.water(nextValue, currentDoubleValue, previousValue)
                    && !travelLoaded.water(value, doubleValue, currentValue)
                ? 0
                : 1;
        if (sourceValue != 0) {
          items.add(
        ...
        } else if (travelLoaded.climbable(nextValue, currentDoubleValue, previousValue)
            || travelLoaded.climbable(value, doubleValue, currentValue)) {
          ...
          items.add(
              walk(
                  ...
                  currentDoubleValue,
                  previousValue,
                  sourceDoubleValue + nextDoubleValue * 0.2,
                  TravelTypeData.Type.CLIMB));
            ...
            if (!(targetDoubleValue > travelPolicy.maxFallHeight() + 0.01)) {
              ...
              items.add(
                  walk(
                      ...
                      previousValue,
                      inputDoubleValue,
                      targetDoubleValue > 0.5
                          ? TravelTypeData.Type.FALL
                          : TravelTypeData.Type.TRAVERSE));
            }
          } else if (!enabled || travelPolicy.allowDiagonalAscend()) {
            if (travelLoaded.jumpHeadroom(nextValue, currentDoubleValue, previousValue)
                || !(previousDoubleValue > 0.5)) {
              items.add(
                  walk(
                      nextValue,
                      currentDoubleValue,
      ...
      if (travelLoaded.jumpHeadroom(value, doubleValue, currentValue)) {
        ...
        for (int index = 2; index <= sourceValue; index++) {
          ...
          for (int currentIndex = 1; currentIndex < index; currentIndex++) {
            ...
            TravelLoadedHandler.Hazard currentHazard =
                travelLoaded.hazard(outputValue, doubleValue, resultValue);
            if (currentHazard != TravelLoadedHandler.Hazard.DANGER
                && currentHazard == TravelLoadedHandler.Hazard.VOID) {}
          }
          ...
          if (byteValue == 0) {
            Double previousDoubleValue =
                travelLoaded.floor(
                    targetValue,
                    inputValue,
                    doubleValue + (travelPolicy.allowParkourAscend() ? 1.0 : 0.0));
            if (previousDoubleValue != null) {
              ...
              if (!(sourceDoubleValue > 1.01)
                ...
                TravelLoadedHandler.Hazard nextHazard =
                    travelLoaded.hazard(targetValue, previousDoubleValue, inputValue);
                if (nextHazard != TravelLoadedHandler.Hazard.DANGER
                    && nextHazard != TravelLoadedHandler.Hazard.VOID) {
                  int candidateValue = sourceDoubleValue > 0.5 ? 1 : 0;
                  if (candidateValue == 0 || travelPolicy.allowParkourAscend()) {
                    double targetDoubleValue =
                        currentDoubleValue * index
                            + 2.2 * (index - 1)
                    ...
                    items.add(
                        walk(
                            ...
                            targetDoubleValue,
                            candidateValue != 0
                                ? TravelTypeData.Type.PARKOUR_ASCEND
                                : TravelTypeData.Type.PARKOUR));
                    break;
                  }
                }
              }
            ...
            if (travelLoaded.headroom(nextValue, nextDoubleValue, previousValue)) {
              items.add(
                  place(
                      ...
                      nextDoubleValue,
                      previousValue,
                      currentDoubleValue + travelPolicy.placePenalty(),
                      TravelTypeData.Type.BRIDGE,
                      nextValue,
                      sourceValue,
                      previousValue));
            }
        ...
        if (travelLoaded.headroom(value, currentDoubleValue, currentValue)) {
          items.add(
              place(
                  ...
                  currentDoubleValue,
                  currentValue,
                  travelPolicy.jumpPenalty() + travelPolicy.placePenalty(),
                  TravelTypeData.Type.PILLAR,
                  value,
                  nextValue,
                  currentValue));
        }
  ...
  private static void updateState5(
    ...
    if (travelLoaded.water(value, doubleValue, currentValue)) {
      ...
      if (currentDoubleValue != null
          ...
          && travelLoaded.headroom(value, currentDoubleValue, currentValue)) {
        items.add(
            walk(
                value,
                currentDoubleValue,
  ...
  public static boolean lineOfSight(
    ...
    int sourceValue =
        Math.max(Math.abs(nextValue - value), Math.abs(previousValue - currentValue)) * 2 + 1;
    int index = 1;
    ...
    while (index < sourceValue) {
      ...
      int inputValue =
          (int) Math.floor(currentValue + 0.5 + previousDoubleValue * targetDoubleValue);
      ...
      if (outputDoubleValue != null && !(Math.abs(outputDoubleValue - inputDoubleValue) > 1.01)) {
        ...
        TravelLoadedHandler.Hazard currentHazard =
            travelLoaded.hazard(targetValue, outputDoubleValue, inputValue);
        if (currentHazard != TravelLoadedHandler.Hazard.DANGER
            && currentHazard != TravelLoadedHandler.Hazard.VOID) {
          index++;
          continue;
