...
public final class PrivacyRevisionService {
  private static volatile PrivacyRevisionService.Policy policy =
      new PrivacyRevisionService.Policy("", "", 0);
  ...
  public static PrivacyRevisionService.Mapping map(String text) {
    PrivacyRevisionService.Policy currentPolicy = policy;
    if (text != null
        && !currentPolicy.original.isEmpty()
        && text.length() >= currentPolicy.original.length()) {
      ...
      while (index <= text.length() - currentPolicy.original.length()) {
        if (!currentPolicy.alias.equalsIgnoreCase(currentPolicy.original)
            && text.startsWith(currentPolicy.alias, index)) {
          index += currentPolicy.alias.length();
        } else {
          int currentLength = index + currentPolicy.original.length();
          if (!text.regionMatches(
                  true, index, currentPolicy.original, 0, currentPolicy.original.length())
            ...
            arrayList.add(
                new PrivacyRevisionService.Span(
                    index, currentLength, nextLength, stringBuilder.length()));
            value = currentLength;
            index = currentLength;
          }
        }
      ...
      PrivacyRevisionService.Mapping previousLength =
          arrayList.isEmpty()
              ? new PrivacyRevisionService.Mapping(text, text, List.of())
              : new PrivacyRevisionService.Mapping(
                  text,
                  stringBuilder.append(text, value, text.length()).toString(),
  ...
  public record Mapping(String source, String text, List<PrivacyRevisionService.Span> spans) {
    ...
    public int displayOffset(int value) {
      int currentLength =
          Math.max(0, Math.min(this.source == null ? 0 : this.source.length(), value));
      int currentValue = 0;
      ...
      for (PrivacyRevisionService.Span span : this.spans) {
        ...
        if (currentLength <= span.end()) {
          ...
          int previousValue =
              Math.round(
                  (float) (currentLength - span.start()) * nextValue / (span.end() - span.start()));
