...
final class SpeedMotionService extends SpeedEnableService {
   ...
   public void motion(SpeedMovingService speedMoving) {
      ...
      if (this.kind == SpeedMotionService.Kind.OnGround) {
         ...
         if (speedMoving.moving() && !speedMoving.jumpKey && !speedMoving.obstructed() && !speedMoving.horizontalCollision && speedMoving.tick % 2 != 0) {
            this.value3 = 0.4;
         }
      ...
      if (this.kind != SpeedMotionService.Kind.OnGround) {
         ...
         if (this.kind == SpeedMotionService.Kind.YPort) {
            if (this.count < 1) {
               ...
               if ((speedMoving.horizontalCollision || speedMoving.forward == 0.0F) && speedMoving.sideways == 0.0F) {
               ...
               } else if (speedMoving.collision(0.0, speedMoving.my, 0.0) || speedMoving.verticalCollision) {
                  this.count = 1;
               }
            ...
            int currentCount = this.count == 1 && speedMoving.verticalCollision && speedMoving.moving() ? 1 : 0;
            if (currentCount != 0) {
               this.value = doubleValue
                  + (currentValue != 0 ? 1.35 : 0.25)
                  - 0.01;
            }

            int nextCount = this.count != 2 || !speedMoving.verticalCollision || !speedMoving.moving() || currentValue != 0 && !speedMoving.ground ? 0 : 1;
            if (nextCount != 0) {
               ...
               this.value = this.value
                  * (
                     currentValue != 0
                        ? 1.533
                        : (
                           this.kind == SpeedMotionService.Kind.OldSlow
                              ? 1.749
                              : 2.149
                        )
                  );
