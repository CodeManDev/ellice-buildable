...
public final class SpeedActivateService {
   ...
   private static final ConcurrentLinkedQueue<SpeedActivateService.Incoming> concurrentLinkedQueue = new ConcurrentLinkedQueue<>();
   ...
   private static void updateState(boolean enabled) {
      ...
      if (enabled && speedStrafeHandler != null && localPlayer != null && minecraft.player == localPlayer && minecraft.level == object) {
   ...
   private static boolean checkCondition2() {
      return CoreIsInitializedHandler.isReady() && CoreIsInitializedHandler.get().modules().get("ScaffoldWalk").map(item -> item.isEnabled()).orElse(false);
   }
      ...
      if (!checkCondition()) {
         ...
         while ((incoming = concurrentLinkedQueue.poll()) != null) {
            if (incoming.generation() == timestamp) {
               ...
               } else if (incoming.packet() instanceof ClientboundSetEntityMotionPacket clientboundSetEntityMotionPacket
                  && SpeedStateController.entityId(clientboundSetEntityMotionPacket) == localPlayer.getId()) {
                  speedStrafeHandler.velocity(currentSpeedMoving, SpeedStateController.fallDamage(clientboundSetEntityMotionPacket));
               }
         ...
         if ((Boolean)implBoolService.autoSprint.get() && nextSpeedMoving.moving() && !text.equals("legacy/Legit")) {
         ...
         if (speedStrafeHandler.autoJump(previousSpeedMoving) && !speedStrafeHandler.cancelJump(previousSpeedMoving)) {
   ...
   public static float strafe(Object value, float currentValue) {
      if (value == localPlayer && checkCondition3()) {
         ...
         updateState4(speedMoving);
         if (Double.isFinite(speedMoving.strafeAcceleration) && speedMoving.strafeAcceleration >= 0.0) {
   ...
   public static Vec3 move(Object value, MoverType moverType, Vec3 vec3) {
      if (value == localPlayer && moverType == MoverType.SELF && checkCondition3()) {
         ...
         updateState4(speedMoving);
         return checkCondition4(currentMove.x, currentMove.y, currentMove.z) ? new Vec3(currentMove.x, currentMove.y, currentMove.z) : vec3;
      } else {
         return vec3;
      }
   }
   ...
   public static float jumpPower(Object value, float currentValue) {
      return value == localPlayer && checkCondition3() ? speedStrafeHandler.jumpPower(createSpeedMovingService(), currentValue) : currentValue;
   }
   ...
   public static float timerMultiplier() {
      return checkCondition3() && !Minecraft.getInstance().level.tickRateManager().isFrozen() ? (float)value : 1.0F;
   }
   ...
   public static void packet(EventAttackInputService.Packet currentPacket) {
      if (!currentPacket.isCancelled() && implBoolService != null) {
         ...
         if (currentPacket.isIncoming()) {
            if (nextPacket instanceof ClientboundPlayerPositionPacket || nextPacket instanceof ClientboundSetEntityMotionPacket) {
            ...
            if (minecraft.isSameThread() && checkCondition3()) {
               if (enabled
                  ...
                  || !(nextPacket instanceof ServerboundKeepAlivePacket) && !(nextPacket instanceof ServerboundPongPacket)) {
                  if (nextPacket instanceof ServerboundMovePlayerPacket serverboundMovePlayerPacket) {
                     SpeedStrafeHandler.Packet currentY = new SpeedStrafeHandler.Packet(serverboundMovePlayerPacket.getY(localPlayer.getY()), serverboundMovePlayerPacket.isOnGround(), serverboundMovePlayerPacket.hasPosition());
                     speedStrafeHandler.packet(createSpeedMovingService(), currentY);
                     SpeedMovePacketAccess speedMovePacketAccess = (SpeedMovePacketAccess)serverboundMovePlayerPacket;
                  ...
                  if (value > 0) {
                     items.add(new SpeedActivateService.DelayedPacket(nextPacket, System.currentTimeMillis() + value, minecraft.getConnection()));
   ...
   private static SpeedMovingService createSpeedMovingService() {
      ...
      speedMoving.web = minecraft.level.getBlockState(currentLocalPlayer.blockPosition()).is(Blocks.COBWEB);
      speedMoving.carpet = minecraft.level.getBlockState(currentLocalPlayer.blockPosition()).getBlock() instanceof CarpetBlock;
      speedMoving.usingItem = currentLocalPlayer.isUsingItem();
      speedMoving.consuming = currentLocalPlayer.isUsingItem() && SpeedStateController.consuming(currentLocalPlayer.getUseItem());
      speedMoving.leatherBoots = currentLocalPlayer.getItemBySlot(EquipmentSlot.FEET).is(Items.LEATHER_BOOTS);
   ...
   private static void updateState4(SpeedMovingService speedMoving) {
      if (localPlayer != null) {
         ...
         if (checkCondition4(speedMoving.x, speedMoving.y, speedMoving.z)
            && (localPlayer.getX() != speedMoving.x || localPlayer.getY() != speedMoving.y || localPlayer.getZ() != speedMoving.z)) {
   ...
   private static boolean checkCondition4(double doubleValue, double currentDoubleValue, double nextDoubleValue) {
      return Double.isFinite(doubleValue) && Double.isFinite(currentDoubleValue) && Double.isFinite(nextDoubleValue);
   }
   ...
   private static final class Actions implements SpeedMovingService.Actions {
      ...
      public void timer(double doubleValue) {
         if (Double.isFinite(doubleValue) && !(doubleValue <= 0.0)) {
            ...
            SpeedActivateService.count3 = SpeedActivateService.text != null && SpeedActivateService.text.startsWith("nextgen/") ? 2 : 0;
         }
      }
      ...
      public boolean collision(double doubleValue, double currentDoubleValue, double nextDoubleValue) {
         return Minecraft.getInstance()
            .level
            .getBlockCollisions(SpeedActivateService.localPlayer, SpeedActivateService.localPlayer.getBoundingBox().move(doubleValue, currentDoubleValue, nextDoubleValue))
            .iterator()
            .hasNext();
      }

      @Override
      public boolean blockAtFeet(double doubleValue, double currentDoubleValue, double nextDoubleValue) {
         return !Minecraft.getInstance()
            .level
            .getBlockState(
               BlockPos.containing(
                  SpeedActivateService.localPlayer.getX() + doubleValue, SpeedActivateService.localPlayer.getY() + currentDoubleValue, SpeedActivateService.localPlayer.getZ() + nextDoubleValue
               )
            )
            .isAir();
      }
      ...
      public boolean hasLanding() {
         ...
         for (int index = 0; index < 500 && vec3.y >= clientLevel.getMinY() - 2; index++) {
            ...
            if (currentVec3.y < 0.0 && clientLevel.getBlockCollisions(SpeedActivateService.localPlayer, aABB.expandTowards(currentVec3)).iterator().hasNext()) {
               return true;
            }

            vec3 = vec3.add(currentVec3);
            aABB = currentAABB;
            currentVec3 = new Vec3(
               currentVec3.x * 0.91,
               (currentVec3.y - 0.08) * 0.98,
               currentVec3.z * 0.91
            );
         }
      ...
      public void positionPacket(SpeedMovingService speedMoving, double currentY, boolean enabled) {
         SpeedActivateService.localPlayer.connection.send(new Pos(speedMoving.x, speedMoving.y + currentY, speedMoving.z, enabled, speedMoving.horizontalCollision));
      }
      ...
      public void fullPacket(SpeedMovingService speedMoving, boolean enabled) {
         SpeedActivateService.localPlayer
            .connection
            .send(new PosRot(speedMoving.x, speedMoving.y, speedMoving.z, speedMoving.yaw, speedMoving.pitch, enabled, speedMoving.horizontalCollision));
      }

      @Override
      public void piercingAttack(int value, int currentValue, boolean enabled, boolean currentEnabled, boolean nextEnabled, int nextValue, String text) {
         SpeedStateController.piercingAttack(value, currentValue, enabled, currentEnabled, nextEnabled, nextValue, text);
      }
   }
