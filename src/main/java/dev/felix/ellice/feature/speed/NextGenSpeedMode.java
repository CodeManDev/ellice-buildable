...
final class NextGenSpeedMode implements SpeedStrafeHandler {
  ...
  public void update(SpeedMovingService speedMoving) {
    ...
    switch (this.text) {
      ...
      case "Custom":
        if (speedMoving.moving()) {
          ...
          if (this.checkCondition("Vertical modification")) {
            speedMoving.my =
                speedMoving.my
                    - this.calculateValue(speedMoving.my <= 0.0 ? "Fall pull down" : "Pull down");
          }
          ...
          if (this.checkCondition("Strafe")) {
            if (this.count6 > 0) {
              ...
              speedMoving.strafe(
                  this.checkCondition("Custom speed")
                      ? this.calculateValue("Speed")
                      : speedMoving.speed(),
                  this.calculateValue("Strafe strength"));
            }
          }
        ...
        if (this.count7 == this.count) {
          speedMoving.strafe(
              this.enabled4
                  ? Math.max(speedMoving.speed(), 0.2857671997172534)
                  : speedMoving.speed());
          this.count7 = -1;
        }
        break;
      case "YPort":
        ...
        break;
      case "PiercingAttack":
        speedMoving.actions.piercingAttack(
          ...
          speedMoving.multiply(
              1.0
                  + (this.checkCondition("Horizontal acceleration")
                      ? 4.0E-4 + 7.0E-4 * speedMoving.speedLevel()
                      : 0.0));
          if (this.checkCondition("Vertical acceleration")
              && speedMoving.my < 0.0
              && speedMoving.fallDistance < 1.0) {
      ...
      case "HypixelLowHop":
        if (speedMoving.ground) {
          ...
          if (speedMoving.speedLevel() == 2
              && (speedMoving.airTicks == 1
                  || speedMoving.airTicks == 2
                  || speedMoving.airTicks == 5
                  || speedMoving.airTicks == 6
                  || speedMoving.airTicks == 8)) {
      ...
      case "Vulcan286":
        if (currentCount == 1) {
          speedMoving.strafe(
              speedMoving.sideways != 0.0F ? 0.3345 : 0.3355 * (1.0 + this.count3 * 0.3819));
        }

        if (currentCount == 2 && speedMoving.sprinting) {
          speedMoving.strafe(
              speedMoving.sideways != 0.0F ? 0.3235 : 0.3284 * (1.0 + this.count3 * 0.355));
        }
      ...
      case "Vulcan288":
        ...
        if (!speedMoving.ground
            && Math.abs(speedMoving.fallDistance) > 0.0
      ...
      case "VulcanGround286":
        if (speedMoving.moving()
            ...
            && !speedMoving.jumpKey) {
          speedMoving.strafe(
              speedMoving.speedLevel() > 0 ? 0.59 : (speedMoving.sideways != 0.0F ? 0.41 : 0.42));
          speedMoving.my = 0.005;
        }
        break;
      case "GrimCollide":
        if (speedMoving.moving()) {
          speedMoving.forwardBoost(
              this.calculateValue("Boost speed")
                  * speedMoving.actions.nearbyEntities(this.calculateValue("Shrink box")));
        }
        break;
      case "NCP":
        if (this.checkCondition("Pull down") && !speedMoving.ground) {
          if (speedMoving.airTicks == this.speedOperationHandler.integer("On tick")) {
            speedMoving.strafe();
            speedMoving.my =
                speedMoving.my - 0.1523351824467155 * this.calculateValue("Motion multiplier");
          }

          if (this.checkCondition("On hurt")
              && speedMoving.hurtTime >= 5
              && speedMoving.my >= 0.0) {
        ...
        if (speedMoving.moving()) {
          if (speedMoving.ground) {
            speedMoving.strafe(
                Math.max(speedMoving.speed(), 0.281 + 0.199999999 * speedMoving.speedLevel()));
      ...
      case "Intave14":
        if (this.checkCondition("Strafe")
            && speedMoving.sprinting
            && (speedMoving.ground || speedMoving.airTicks == 11)) {
      ...
      case "HylexGround":
        if (speedMoving.ground
            && speedMoving.groundTicks > 5
            && speedMoving.hurtTime < 1
            && speedMoving.speedLevel() < 1) {
          speedMoving.multiply(speedMoving.sideways == 0.0F ? 1.2174 : 1.214);
        }
        break;
      case "BlocksMC":
        if (speedMoving.ground) {
        ...
        double doubleValue = 0.06 + (speedMoving.ground ? 0.12 : 0.21) + speedMoving.my / 20.0;
        ...
        if (this.checkCondition("Round strafe yaw")) {
          currentDoubleValue =
              Math.toRadians(Math.rint(Math.toDegrees(currentDoubleValue) / 45.0) * 45.0);
        }
  ...
  public float jumpPower(SpeedMovingService speedMoving, float value) {
    return switch (this.text) {
      case "Custom" ->
          this.checkCondition("Vertical modification")
                  && Math.abs(this.calculateValue("Jump height") - 0.42) > 1.0E-6
  ...
  public void beforeJump(SpeedMovingService speedMoving) {
    if (this.text.equals("HypixelBHop")) {
      speedMoving.strafe(
          Math.max(
              speedMoving.speed(),
              this.enabled ? 0.0 : 0.281 + 0.008003278196411223 * speedMoving.speedLevel()));
    }

    if (this.text.equals("HypixelLowHop")) {
      speedMoving.strafe(Math.max(speedMoving.speed(), 0.247 + 0.15 * speedMoving.speedLevel()));
    }
  }
  ...
  public void afterJump(SpeedMovingService speedMoving) {
    ...
    switch (this.text) {
      ...
      case "VerusB3882":
        ...
        break;
      case "Vulcan288":
        speedMoving.strafe(this.count3 != 0 ? 0.771 : 0.5);
        break;
      case "Custom":
        if (this.checkCondition("Horizontal modification")
            && this.calculateValue("Jump off multiplier") != 0.0) {
    ...
    switch (this.text) {
      ...
      case "SentinelDamage":
        if (speedMoving.hurtTime > 0 && !this.enabled2) {
          ...
          this.count10 = (int) Math.ceil((speedMoving.actions.millis() - this.timestamp) / 50.0);
        }
  ...
  public void velocity(SpeedMovingService speedMoving, boolean enabled) {
    if (this.text.equals("HypixelBHop")
        || this.text.equals("Custom") && this.checkCondition("Strafe knockback")) {
