...
final class AtaniSpeedMode extends SpeedEnableService {
  ...
  private static void updateState(SpeedMovingService speedMoving, double doubleValue) {
    ...
    speedMoving.mx =
        doubleValue
            * (-currentDoubleValue * Math.sin(previousDoubleValue)
                + nextDoubleValue * Math.cos(previousDoubleValue));
    speedMoving.mz =
        doubleValue
            * (currentDoubleValue * Math.cos(previousDoubleValue)
                + nextDoubleValue * Math.sin(previousDoubleValue));
  }
  ...
  public boolean autoJump(SpeedMovingService speedMoving) {
    return (this.kind == AtaniSpeedMode.Kind.IntaveGroundStrafe
            || this.kind == AtaniSpeedMode.Kind.IncognitoNormal)
        && speedMoving.ground
        && speedMoving.moving();
  }
  ...
  private void updateState2(SpeedMovingService speedMoving) {
    if (this.kind == AtaniSpeedMode.Kind.IntaveGroundStrafe && !speedMoving.obstructed()) {
      if (speedMoving.ground && speedMoving.moving()) {
        ...
        speedMoving.airSpeed(0.02 + speedMoving.actions.random() / 2000.0);
      ...
      double doubleValue =
          Math.max(0.0, 0.272 * (1.0 + 0.2 * speedMoving.speedLevel()) - 0.2874999940395355);
      if (!speedMoving.moving()) {
        speedMoving.stop();
      } else {
        updateState(
            speedMoving,
            speedMoving.ground
                ? 0.36 + speedMoving.actions.random() / 70.0 + doubleValue
                : speedMoving.speed() - (speedMoving.actions.random() - 0.5) / 70.0);
      }
    ...
    switch (this.kind) {
      case Karhu:
        ...
        if (speedMoving.ground) {
          ...
          speedMoving.timer(1.0 + (speedMoving.actions.random() - 0.5) / 100.0);
        }
  ...
  enum Kind {
    ...
    private static AtaniSpeedMode.Kind[] $values() {
      return new AtaniSpeedMode.Kind[] {
        BHop, Karhu, IntaveGroundStrafe, IncognitoNormal, NCPHop, MineMenClub
      };
    }
  }
}
