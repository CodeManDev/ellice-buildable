...
final class EtbSpeedMode extends SpeedEnableService {
  ...
  private boolean checkCondition() {
    return this.kind == EtbSpeedMode.Kind.OldGuardian
        || this.kind == EtbSpeedMode.Kind.GuardianYport;
  }
  ...
  public void motion(SpeedMovingService speedMoving) {
    if (!this.checkCondition()) {
      ...
      if (speedMoving.ground && speedMoving.moving()) {
        speedMoving.my = this.kind == EtbSpeedMode.Kind.OldGuardian ? 0.4 : 0.07999999999999999;
        speedMoving.positionPacket(1.0E-9, speedMoving.ground);
        speedMoving.strafe(this.kind == EtbSpeedMode.Kind.OldGuardian ? 1.75 : 0.7);
      } else {
        speedMoving.strafe();
      }
    }
  ...
  public void move(SpeedMovingService speedMoving, SpeedStrafeHandler.Move currentMove) {
    if (!this.checkCondition()) {
      ...
      if (currentValue != 0 && this.count == 1) {
        ...
        currentMove.y = speedMoving.my = this.kind == EtbSpeedMode.Kind.Mineplex ? 0.3 : 0.3999;
        this.value =
            this.kind == EtbSpeedMode.Kind.Mineplex
                ? 0.64
                : this.value * (this.kind == EtbSpeedMode.Kind.Bhop ? 2.1 : 1.58);
        if (this.kind == EtbSpeedMode.Kind.HypixelHop) {
        ...
        if (speedMoving.collision(0.0, speedMoving.my, 0.0)
            || speedMoving.verticalCollision && this.count > 0) {
