...
final class SpeedStrafeService extends SpeedEnableService {
   ...
   private double calculateValue(SpeedMovingService speedMoving) {
      ...
      int value = !(Math.abs(speedMoving.forward) >= 0.8)
            && !(Math.abs(speedMoving.sideways) >= 0.8)
         ? 0
         : 1;
      return 0.221
         * (value != 0 ? 1.3F : 1.0F)
         * (1.0 + 0.2 * potion(speedMoving));
   }
      ...
      if (this.kind == SpeedStrafeService.Kind.NCP) {
         ...
         if (!speedMoving.moving()) {
            ...
            if (speedMoving.airTicks == 0) {
               double currentDoubleValue = !speedMoving.horizontalCollision && (float)this.values.number("Jump motion") == 0.4F
                  ? 0.4000000059604645
                  : 0.41999998688697815;
         ...
         speedMoving.strafeAcceleration = Math.max(doubleValue, this.value)
         ...
         switch (speedMoving.airTicks) {
            case 0:
               speedMoving.jump();
               speedMoving.strafe(
                  speedMoving.speedAmplifier >= 0 ? 0.6 : 0.485
               );
               break;
            case 1:
            case 2:
