...
final class NcpSpeedMode implements SpeedStrafeHandler {
   ...
   public void enable(SpeedMovingService speedMoving) {
      switch (this.text) {
         case "NCPBHop":
            this.count = !speedMoving.collision(0.0, speedMoving.my, 0.0) && !speedMoving.verticalCollision ? 4 : 1;
            break;
         case "SNCPBHop":
            this.value = 0.0;
            this.count = 4;
   ...
   public void update(SpeedMovingService speedMoving) {
      switch (this.text) {
         ...
         case "UNCPHop":
            ...
            if (speedMoving.ground) {
               this.value3 = speedMoving.speedLevel() >= 1
                  ? 0.4562999904155731
                  : 0.3384999930858612;
               speedMoving.jump();
            } else {
         ...
         case "UNCPHopNew":
            ...
            if (speedMoving.hurtTime <= 1 && ++this.count4 == this.speedOperationHandler.integer("On tick") && this.checkCondition("Pull down")) {
               speedMoving.strafe();
               speedMoving.my = -0.1523351824467155;
            }

            if (this.checkCondition("On hurt") && speedMoving.hurtTime >= 2 && speedMoving.hurtTime <= 4 && speedMoving.my >= 0.0) {
   ...
   public void motion(SpeedMovingService speedMoving) {
      switch (this.text) {
         ...
         case "Boost":
            ...
            int value = !speedMoving.collision(speedMoving.mx / currentDoubleValue, 0.0, speedMoving.mz / currentDoubleValue) ? 1 : 0;
