...
public final class SpeedMovingService {
   ...
   public double direction() {
      ...
      double y = this.forward < 0.0F
         ? -0.5
         : (this.forward > 0.0F ? 0.5 : 1.0);
   ...
   public void strafeYaw(double doubleValue, double currentDoubleValue, double nextDoubleValue) {
      if (this.moving()) {
         this.mx = this.mx * (1.0 - currentDoubleValue) - Math.sin(nextDoubleValue) * doubleValue * currentDoubleValue;
         this.mz = this.mz * (1.0 - currentDoubleValue) + Math.cos(nextDoubleValue) * doubleValue * currentDoubleValue;
      }
   }
   ...
   public interface Actions {
      ...
      void piercingAttack(int value, int currentValue, boolean enabled, boolean currentEnabled, boolean nextEnabled, int nextValue, String text);
   }
}
