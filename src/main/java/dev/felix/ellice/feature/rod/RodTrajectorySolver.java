...
public final class RodTrajectorySolver {
   ...
   public static RodTrajectorySolver.Launch launch(RodTrajectorySolver.Kind kind, RotationVector rotationVector, RotationVector currentRotationVector, RotationData rotationData) {
      ...
      if (kind == RodTrajectorySolver.Kind.ROD) {
         ...
         return new RodTrajectorySolver.Launch(
            rotationVector.add(
               new RotationVector(
                  -Math.sin(doubleValue) * 0.3,
                  0.0,
                  Math.cos(doubleValue) * 0.3
               )
            ),
   ...
   public static RotationVector position(RodTrajectorySolver.Kind kind, RodTrajectorySolver.Launch launch, double width) {
      if (Double.isFinite(width) && !(width < 0.0) && !(width > 26.0)) {
         ...
         for (int index = 0; index < Math.ceil(width); index++) {
            ...
            rotationVector = rotationVector.add(currentRotationVector.multiply(Math.min(1.0, width - index)));
            if (kind == RodTrajectorySolver.Kind.ROD) {
   ...
   public static Optional<RodTrajectorySolver.Solution> solve(
      RodTrajectorySolver.Kind kind, RotationVector rotationVector, RotationVector currentRotationVector, DoubleFunction<RotationVector> doubleFunction
   ) {
      double doubleValue = 0.05;
      double currentDoubleValue = calculateValue(
         kind,
         rotationVector,
      ...
      for (double nextDoubleValue = 0.25;
         nextDoubleValue <= 24.0;
         nextDoubleValue += 0.25
      ) {
         double previousDoubleValue = calculateValue(kind, rotationVector, currentRotationVector, (RotationVector)doubleFunction.apply(nextDoubleValue), nextDoubleValue);
         if (currentDoubleValue > 0.0 && previousDoubleValue <= 0.0) {
            ...
            for (int index = 0; index < 24; index++) {
               ...
               if (calculateValue(kind, rotationVector, currentRotationVector, (RotationVector)doubleFunction.apply(inputDoubleValue), inputDoubleValue) > 0.0) {
            ...
            RotationVector nextRotationVector = createRotationData7(kind, rotationVector, currentRotationVector, (RotationVector)doubleFunction.apply(outputDoubleValue), outputDoubleValue);
   ...
   private static double calculateValue(RodTrajectorySolver.Kind kind, RotationVector rotationVector, RotationVector currentRotationVector, RotationVector nextRotationVector, double doubleValue) {
      RotationVector previousRotationVector = createRotationData7(kind, rotationVector, currentRotationVector, nextRotationVector, doubleValue);
      double currentX = Math.hypot(previousRotationVector.x(), previousRotationVector.z());
      double currentLength = kind == RodTrajectorySolver.Kind.ROD
         ? 0.6
            + 0.5 * previousRotationVector.length() / Math.max(currentX, 1.0E-9)
         : 1.5;
      return previousRotationVector.length() - currentLength;
   }

   private static RotationVector createRotationData7(RodTrajectorySolver.Kind kind, RotationVector rotationVector, RotationVector currentRotationVector, RotationVector nextRotationVector, double doubleValue) {
      ...
      if (kind == RodTrajectorySolver.Kind.ROD) {
         ...
         previousRotationVector = rotationVector.add(
            new RotationVector(sourceRotationVector.x(), 0.0, sourceRotationVector.z())
               .multiply(0.3 / Math.max(currentX, 1.0E-9))
         );
      } else {
         previousRotationVector = rotationVector.add(new RotationVector(0.0, -0.10000000149011612, 0.0));
      }

      double currentDoubleValue = kind == RodTrajectorySolver.Kind.ROD
         ? 0.92
         : 0.9900000095367432;
      int value = (int)doubleValue;
      double nextDoubleValue = (1.0 - Math.pow(currentDoubleValue, value)) / (1.0 - currentDoubleValue) + (doubleValue - value) * Math.pow(currentDoubleValue, value);
      if (kind != RodTrajectorySolver.Kind.ROD) {
         nextDoubleValue *= currentDoubleValue;
      }

      double currentY = position(kind, new RodTrajectorySolver.Launch(rotationData7, rotationData7), doubleValue).y();
      RotationVector targetRotationVector = nextRotationVector.subtract(previousRotationVector).subtract(new RotationVector(0.0, currentY, 0.0)).multiply(1.0 / nextDoubleValue);
      return kind == RodTrajectorySolver.Kind.ROD ? targetRotationVector : targetRotationVector.subtract(currentRotationVector);
   }
