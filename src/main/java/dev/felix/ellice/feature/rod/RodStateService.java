...
public final class RodStateService {
  ...
  public void before(
      long longValue, RodStateService.Environment environment, RodStateService.Policy policy) {
    try {
  ...
  private void updateState(
      long offset, RodStateService.Environment environment, RodStateService.Policy policy) {
    if (!environment.context()) {
      ...
      if (this.state2 == RodStateService.State.AIM) {
        ...
        if (this.state2 == RodStateService.State.WAIT_RESET) {
          ...
          if (offset - this.timestamp < policy.serverWait()
              && currentHook != RodStateService.Hook.OTHER) {
  ...
  private void updateState2(
    ...
    if (this.active() && this.timestamp6 != offset) {
      if (!environment.context() || !environment.valid()) {
        this.stop(environment);
      } else if (this.state2 == RodStateService.State.AIM
          || this.state2 == RodStateService.State.REEL) {
        ...
        if (value != 0 && policy.saveRod()) {
          ...
          if (value == 0
              || environment.hook() != RodStateService.Hook.NONE
                  && environment.hook() != RodStateService.Hook.OTHER) {
