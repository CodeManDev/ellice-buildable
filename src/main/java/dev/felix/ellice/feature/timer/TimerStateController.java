...
public final class TimerStateController {
   ...
   public static String status() {
      ...
      String text = profile2 != null
            && (profile2.mode() == TimerModeTracker.Mode.PULSE || profile2.mode() == TimerModeTracker.Mode.PACKET_PULSE)
         ? (timerModeTracker.boostPhase() ? "boost" : "brake")
         : "active";
      return implProfile.modeName() + " · " + createText(timerModeTracker.multiplier()) + "x · " + text;
   }
   ...
   private static boolean checkCondition() {
      if (implProfileService != null && profile2 != null && CoreIsInitializedHandler.isReady()) {
         ...
         return minecraft.player != null && minecraft.level != null && !minecraft.isPaused() && !minecraft.level.tickRateManager().isFrozen();
      } else {
         return false;
      }
   }
