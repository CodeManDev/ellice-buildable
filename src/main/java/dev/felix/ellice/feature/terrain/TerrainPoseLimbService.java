...
public final class TerrainPoseLimbService {
   ...
   public static GeometryXService poseLimb(double doubleValue, double currentDoubleValue, double nextDoubleValue, float value, float currentValue, float nextValue, TerrainPoseLimbService.Limb limb) {
      Quaternionf quaternionf = new Quaternionf().rotateY((float)(3.141592653589793 - Math.toRadians(value)));
      Quaternionf currentQuaternionf = new Quaternionf(quaternionf).mul(new Quaternionf().rotateX(currentValue));
      Vector3f vector3f = new Vector3f(limb.pivotX, limb.pivotY, limb.pivotZ).mul(0.05625F).rotate(quaternionf);
      return new GeometryXService(doubleValue + vector3f.x, currentDoubleValue - nextValue + vector3f.y, nextDoubleValue + vector3f.z, currentQuaternionf, new Vector3f(0.05625F));
   }

   public static GeometryXService poseTorsoChild(
      double doubleValue, double currentDoubleValue, double nextDoubleValue, float value, float currentValue, float nextValue, float previousValue, TerrainPoseLimbService.Limb limb
   ) {
      if (limb != TerrainPoseLimbService.Limb.HEAD && limb != TerrainPoseLimbService.Limb.ARM_LEFT && limb != TerrainPoseLimbService.Limb.ARM_RIGHT) {
      ...
      Quaternionf nextQuaternionf = new Quaternionf(quaternionf).mul(currentQuaternionf).mul(new Quaternionf().rotateX(nextValue));
      Vector3f vector3f = new Vector3f(TerrainPoseLimbService.Limb.TORSO.pivotX, TerrainPoseLimbService.Limb.TORSO.pivotY, TerrainPoseLimbService.Limb.TORSO.pivotZ)
         .mul(0.05625F);
      Vector3f currentVector3f = new Vector3f(
         ...
         .mul(0.05625F);
      Vector3f nextVector3f = new Vector3f(vector3f).rotate(quaternionf).add(new Vector3f(currentVector3f).rotate(new Quaternionf(quaternionf).mul(currentQuaternionf)));
      return new GeometryXService(doubleValue + nextVector3f.x, currentDoubleValue - previousValue + nextVector3f.y, nextDoubleValue + nextVector3f.z, nextQuaternionf, new Vector3f(0.05625F));
   }
   ...
   public static TerrainPoseLimbService.LimbMesh limbMesh(boolean enabled, boolean currentEnabled, TerrainPoseLimbService.Limb limb) {
      float[] floats = CapePlayerService.player(enabled, currentEnabled);
      ...
      float[] previousFloats = nextFloats == null ? currentFloats : createFloat(currentFloats, nextFloats);
      ...
      return new TerrainPoseLimbService.LimbMesh(new GeometryVertexCountService(previousFloats, ints), limb.pivotX, limb.pivotY, limb.pivotZ);
   }

   private static GeometryVertexCountService createGeometryVertexCountService(float[] floats, int value, int currentValue, float nextValue, float previousValue, float sourceValue) {
      float[] currentFloats = createFloat2(floats, value, currentValue, nextValue, previousValue, sourceValue);
   ...
   private static float[] createFloat2(float[] floats, int value, int currentValue, float nextValue, float previousValue, float sourceValue) {
      byte byteValue = 8;
      float[] currentFloats = new float[currentValue * 12];

      for (int index = 0; index < currentValue; index++) {
         int targetValue = index % 3;
         int currentIndex = (value + index - targetValue + (targetValue == 0 ? 0 : (targetValue == 1 ? 2 : 1))) * 8;
         ...
         float initialValue = (float)Math.sqrt(candidateValue * candidateValue + defaultValue * defaultValue);
   ...
   public enum Limb {
      ...
      private static TerrainPoseLimbService.Limb[] $values() {
         return new TerrainPoseLimbService.Limb[]{HEAD, TORSO, ARM_RIGHT, ARM_LEFT, LEG_RIGHT, LEG_LEFT};
      }
   }

   public record LimbMesh(GeometryVertexCountService mesh, float pivotX, float pivotY, float pivotZ) {
   }
}
