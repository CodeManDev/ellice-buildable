...
public final class TerrainGroundTracker {
  ...
  private final PriorityQueue<TerrainGroundTracker.Entry> priorityQueue =
      new PriorityQueue<>(
          Comparator.comparingDouble(TerrainGroundTracker.Entry::priority)
              .thenComparingDouble(item -> -item.step.cost));
  ...
  public void moved(TerrainKindData currentTerrainKindData, Vector3d vector3d) {
    if (this.terrainKindData != null
        && this.terrainKindData.id().equals(currentTerrainKindData.id())) {
  ...
  private void updateState(Vector3d currentVector3d) {
    ...
    this.value3 =
        currentVector3d.distance(
            this.terrainKindData.x(), this.terrainKindData.y(), this.terrainKindData.z());
    if (this.ground.loaded(currentValue, nextValue)
        && this.ground.loaded(currentX, previousValue)) {
      Double doubleValue = this.calculateValue(currentValue, nextValue, currentVector3d.y);
      Double currentY =
          this.calculateValue(currentX, previousValue, this.terrainKindData.y() + 0.1);
      if (doubleValue != null && currentY != null) {
        this.value = currentY;
        TerrainGroundTracker.Step step =
            new TerrainGroundTracker.Step(
                createKey(currentValue, nextValue, doubleValue), doubleValue, 0.0, null);
        this.entries.put(step.key, 0.0);
        this.priorityQueue.add(
            new TerrainGroundTracker.Entry(
                step, this.calculateValue2(currentValue, nextValue, doubleValue)));
  ...
  private static TerrainGroundTracker.Key createKey(
      int value, int currentValue, double doubleValue) {
    ...
    if (this.terrainKindData != null) {
      ...
      if (this.status2 == TerrainGroundTracker.Status.PLANNING) {
        ...
        while (!this.priorityQueue.isEmpty()
            && index++ < 96
            && System.nanoTime() - longValue < 800000L) {
          ...
          if (!(currentStep.cost
            ...
            for (int currentIndex = 0; currentIndex < 4; currentIndex++) {
              int currentValue =
                  currentStep.key.x + (currentIndex == 0 ? 1 : (currentIndex == 1 ? -1 : 0));
              int nextValue =
                  currentStep.key.z + (currentIndex == 2 ? 1 : (currentIndex == 3 ? -1 : 0));
              if (!(Math.abs(currentValue - this.vector3d.x) > 512.0)
                ...
                if (doubleValue != null
                  ...
                  TerrainGroundTracker.Key currentKey =
                      createKey(currentValue, nextValue, doubleValue);
                  double currentDoubleValue =
                      currentStep.cost + 1.0 + Math.abs(doubleValue - currentStep.y) * 0.4;
                  if (!(currentDoubleValue + 1.0E-6
                      >= this.entries.getOrDefault(currentKey, Double.POSITIVE_INFINITY))) {
                    this.entries.put(currentKey, currentDoubleValue);
                    TerrainGroundTracker.Step nextStep =
                        new TerrainGroundTracker.Step(
                            currentKey, doubleValue, currentDoubleValue, currentStep);
                    this.priorityQueue.add(
                        new TerrainGroundTracker.Entry(
                            nextStep,
                            currentDoubleValue
                                + this.calculateValue2(currentValue, nextValue, doubleValue)));
                  }
                }
              }
            }
      ...
      if (this.status2 == TerrainGroundTracker.Status.READY
          || this.status2 == TerrainGroundTracker.Status.ARRIVED) {
        this.updateState3(currentVector3d);
        if (this.value3 < 2.0 && Math.abs(currentVector3d.y - this.value) < 2.0) {
        ...
        } else if (longValue - this.timestamp3 > 1000000000L
            && this.status2 == TerrainGroundTracker.Status.READY) {
          this.timestamp3 = longValue;

          for (int nextIndex = this.count2;
              nextIndex < Math.min(this.items.size(), this.count2 + 10);
              nextIndex++) {
            Vector3d nextVector3d = this.items.get(nextIndex);
            Double nextDoubleValue =
                this.ground.floor(
                    (int) Math.floor(nextVector3d.x),
                    (int) Math.floor(nextVector3d.z),
                    nextVector3d.y);
      ...
      } else if ((this.status2 == TerrainGroundTracker.Status.UNEXPLORED
              || this.status2 == TerrainGroundTracker.Status.UNAVAILABLE)
  ...
  private void updateState2(TerrainGroundTracker.Step step) {
    ...
    for (TerrainGroundTracker.Step currentStep = step;
        currentStep != null;
        currentStep = currentStep.parent) {
      arrayList.add(new Vector3d(currentStep.key.x + 0.5, currentStep.y, currentStep.key.z + 0.5));
    }
    ...
    for (int index = 1; index < this.items.size(); index++) {
      this.value4 =
          this.value4 + this.items.get(index - 1).distance((Vector3dc) this.items.get(index));
    }
  ...
  private void updateState3(Vector3d vector3d) {
    ...
    for (int index = 1; index < this.items.size(); index++) {
      ...
      double previousDoubleValue =
          Math.clamp(
              new Vector3d(vector3d).sub(currentVector3d).dot(previousVector3d)
                  / previousVector3d.lengthSquared(),
              0.0,
              1.0);
      double sourceDoubleValue =
          new Vector3d(previousVector3d)
              .mul(previousDoubleValue)
              .add(currentVector3d)
  ...
  public TerrainGroundTracker.Instruction instruction() {
    ...
    if (this.status2 == TerrainGroundTracker.Status.UNEXPLORED) {
      return new TerrainGroundTracker.Instruction(
          "navigation", "Explore closer to find a route", this.value3);
    }

    if (this.status2 == TerrainGroundTracker.Status.UNAVAILABLE) {
      return new TerrainGroundTracker.Instruction(
          "navigation", "No walkable route found", this.value3);
    }
    ...
    for (int index = this.count2 + 1; index + 1 < this.items.size(); index++) {
      Vector3d vector3d =
          new Vector3d((Vector3dc) this.items.get(index))
              .sub((Vector3dc) this.items.get(index - 1));
      Vector3d currentVector3d =
          new Vector3d((Vector3dc) this.items.get(index + 1))
              .sub((Vector3dc) this.items.get(index));
      ...
      if (Math.abs(currentDoubleValue) > 0.2) {
        return new TerrainGroundTracker.Instruction(
            currentDoubleValue > 0.0 ? "turn_right" : "turn_left",
            currentDoubleValue > 0.0 ? "Turn right" : "Turn left",
            doubleValue);
      }
    }

    return new TerrainGroundTracker.Instruction(
        "navigation", "Continue to your destination", this.value3);
  }
  ...
  public enum Status {
    ...
    private static TerrainGroundTracker.Status[] $values() {
      return new TerrainGroundTracker.Status[] {
  ...
  private record Step(
      TerrainGroundTracker.Key key, double y, double cost, TerrainGroundTracker.Step parent) {}
}
