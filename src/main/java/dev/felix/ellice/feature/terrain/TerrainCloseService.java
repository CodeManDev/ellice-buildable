...
final class TerrainCloseService implements AutoCloseable {
   ...
   private RhiBlendStateService.SamplerHandle samplerHandle = RhiBlendStateService.SamplerHandle.NONE;
      ...
      for (boolean enabled : new boolean[]{false, true}) {
         for (boolean currentEnabled : new boolean[]{false, true}) {
            ...
            for (TerrainPoseLimbService.Limb currentLimb : limb) {
               TerrainPoseLimbService.LimbMesh currentLimbMesh = TerrainPoseLimbService.limbMesh(enabled, currentEnabled, currentLimb);
               this.renderer3[index][currentLimb.ordinal()] = render3dScene.createMesh(currentLimbMesh.mesh());
            }
         }
      }
   }

   void update(List<CompatLoadedHandler.MapPlayer> items, TerrainViewportService.Frame frame, long offset) {
      this.updateState(items, frame, offset, false);
   }

   void updateRecorded(List<CompatLoadedHandler.MapPlayer> items, TerrainViewportService.Frame frame, long offset) {
      this.updateState(items, frame, offset, true);
   }

   private void updateState(List<CompatLoadedHandler.MapPlayer> items, TerrainViewportService.Frame frame, long offset, boolean enabled) {
      ...
      for (CompatLoadedHandler.MapPlayer mapPlayer : items) {
         ...
         if (entry != null && (entry.skinGlId != mapPlayer.skinGlId() || entry.slim != mapPlayer.slim() || entry.legacy != mapPlayer.legacySkin())) {
      ...
      for (UUID uUID : this.entries.keySet().stream().filter(item -> !hashSet.contains(item)).toList()) {
   ...
   private MaterialErFactory createMaterialErFactory(String text) {
      return MaterialErFactory.builder(text, new ShaderDefinition("ellice:map-player-surface", "render3d/pbr.vert", "terrain/player.frag"))
         .build()
         .color("uBaseColor", -1)
   ...
   private TerrainCloseService.Entry createEntry(CompatLoadedHandler.MapPlayer mapPlayer, long size) {
      RhiBlendStateService.TextureHandle textureHandle = this.createTextureHandle(mapPlayer.skinGlId());
      MaterialErFactory materialEr;
      if (textureHandle.valid()) {
         materialEr = this.createMaterialErFactory("ellice:map-player")
            .texture("uBaseColorMap", textureHandle, 0)
            .uniform("uHasBaseColorMap", 1)
            .uniform("uRoughness", 0.85F);
      } else if (mapPlayer.skinGlId() > 0) {
         materialEr = this.createMaterialErFactory("ellice:map-player")
            .texture("uBaseColorMap", new RhiBlendStateService.TextureHandle(mapPlayer.skinGlId()), this.createSamplerHandle(), 0)
            .uniform("uHasBaseColorMap", 1)
            .uniform("uRoughness", 0.85F);
      } else {
         materialEr = this.createMaterialErFactory("ellice:map-player-fallback")
      ...
      for (TerrainPoseLimbService.Limb currentLimb : limb) {
         entry.objects[currentLimb.ordinal()] = this.renderer2
            .add(
               this.renderer3[index][currentLimb.ordinal()],
               materialEr,
               TerrainPoseLimbService.poseLimb(mapPlayer.x(), mapPlayer.y(), mapPlayer.z(), mapPlayer.yaw(), 0.0F, 0.0F, currentLimb)
            );
      }
   ...
   private void updateState2(TerrainCloseService.Entry entry, CompatLoadedHandler.MapPlayer mapPlayer, TerrainViewportService.Frame frame, long offset) {
      double doubleValue = entry.lastNanos == 0L
         ? 0.0
         : Math.clamp(
            (offset - entry.lastNanos) / 1.0E9, 0.0, 0.25
         );
      ...
      entry.crouchBlend = (float)(
         entry.crouchBlend + (nextValue - entry.crouchBlend) * Math.clamp(doubleValue * 8.0, 0.0, 1.0)
      );
      ...
      int resultValue = frame.project(nextDoubleValue, previousDoubleValue + 0.9, sourceDoubleValue) != null ? 1 : 0;
      this.updateState3(entry, mapPlayer, new GeometryXService(nextDoubleValue, previousDoubleValue, sourceDoubleValue), (resultValue != 0));
      ...
      for (TerrainPoseLimbService.Limb currentLimb : limb) {
         GeometryXService geometryX = switch (currentLimb) {
            case LEG_RIGHT, ARM_LEFT, LEG_LEFT, ARM_RIGHT -> {
               float candidateValue = switch (currentLimb) {
                  case LEG_RIGHT, ARM_LEFT -> targetValue * currentValue;
                  default -> -targetValue * currentValue;
               };
               if (currentLimb != TerrainPoseLimbService.Limb.ARM_RIGHT && currentLimb != TerrainPoseLimbService.Limb.ARM_LEFT) {
                  yield TerrainPoseLimbService.poseLimb(nextDoubleValue, previousDoubleValue, sourceDoubleValue, outputValue, candidateValue, sourceValue, currentLimb);
               } else {
                  float selectedValue = candidateValue * 0.9F;
                  yield TerrainPoseLimbService.poseTorsoChild(nextDoubleValue, previousDoubleValue, sourceDoubleValue, outputValue, previousValue, selectedValue, sourceValue, currentLimb);
               }
            }
            case TORSO -> TerrainPoseLimbService.poseLimb(nextDoubleValue, previousDoubleValue, sourceDoubleValue, outputValue, previousValue, sourceValue, currentLimb);
            case HEAD -> TerrainPoseLimbService.poseTorsoChild(
               nextDoubleValue, previousDoubleValue, sourceDoubleValue, outputValue, previousValue, inputValue - previousValue * 0.6F, sourceValue, currentLimb
            );
         };
   ...
   private void updateState3(TerrainCloseService.Entry entry, CompatLoadedHandler.MapPlayer mapPlayer, GeometryXService geometryX, boolean enabled) {
      List items = mapPlayer.model();
      if (entry.nativeSample != items) {
         for (int index = 0; index < items.size(); index++) {
            CompatLoadedHandler.PlayerPart playerPart = (CompatLoadedHandler.PlayerPart)items.get(index);
            ...
            if (index < entry.nativeObjects.size()) {
               ...
               render3dId = this.renderer2
                  .add(
                     this.renderer.createMesh(playerPart.mesh()),
                     this.createMaterialErFactory("ellice:map-player-native").uniform("uRoughness", 0.85F),
                     geometryX
                  );
            ...
            render3dId.material()
               .texture("uBaseColorMap", new RhiBlendStateService.TextureHandle(playerPart.texture()), this.createSamplerHandle(), 0)
   ...
   static double dampPosition(double x, double y, float width, double height) {
      if (!Double.isFinite(height) || height <= 0.0) {
         ...
         return Math.abs(y - x) > 16.0
            ? y
            : x + (y - x) * -Math.expm1(-width * height);
      } else {
         return x;
      }
   }
   ...
   static float dampAngle(float value, float currentValue, float nextValue, double doubleValue) {
      if (!Double.isFinite(doubleValue) || doubleValue <= 0.0) {
         ...
         return !Float.isFinite(currentValue) ? value : value + wrapDegrees(currentValue - value) * (1.0F - (float)Math.exp(-Math.max(0.0F, nextValue) * doubleValue));
      }
   }
   ...
   private RhiBlendStateService.TextureHandle createTextureHandle(int textureId) {
      ...
      try {
         textureDownload = this.rhiOperationHandler.downloadTexture(new RhiBlendStateService.TextureHandle(textureId));
      ...
      if (textureDownload != null && textureDownload.complete()) {
         try {
            RhiBlendStateService.TextureDescriptor currentWidth = new RhiBlendStateService.TextureDescriptor(
