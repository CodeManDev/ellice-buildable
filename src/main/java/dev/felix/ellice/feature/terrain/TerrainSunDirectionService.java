...
public final class TerrainSunDirectionService {
   public static final Vector3f DEFAULT_SUN = new Vector3f(
         -0.55F, 0.55F, -0.35F
      )
      .normalize();
   ...
   public static DirectionalLight mapSun(Vector3f vector3f, float value) {
      ...
      if (!(currentValue <= 0.02F) && !(currentVector3f.y < -0.02F)) {
         ...
         float previousValue = Math.clamp(1.0F - nextValue * 1.6F, 0.0F, 1.0F) * Math.clamp(currentValue * 2.0F, 0.0F, 1.0F);
         Vector3f nextVector3f = new Vector3f(
            1.0F,
            0.96F - 0.41F * previousValue,
            0.9F - 0.6F * previousValue
         );
         return new DirectionalLight(currentVector3f, nextVector3f, 0.5F + 1.9F * currentValue);
      } else {
         Vector3f previousVector3f = new Vector3f(-currentVector3f.x, Math.clamp(-currentVector3f.y, 0.25F, 1.0F), -currentVector3f.z).normalize();
         return new DirectionalLight(
            previousVector3f,
            new Vector3f(0.55F, 0.68F, 0.95F),
            0.5F
         );
      }
   }
   ...
   public static Vector3f mapAmbientColor(float value) {
      float currentValue = calculateValue(value, 1.0F);
      return new Vector3f(
         0.35F + 0.2F * currentValue,
         0.42F + 0.2F * currentValue,
         0.6F + 0.15F * currentValue
      );
   }
   ...
   public static Vector3f gradeTint(float value, float currentValue) {
      ...
      float previousValue = Float.isFinite(currentValue) ? Math.clamp(currentValue, 0.0F, 1.0F) : 0.65F;
      float sourceValue = Math.clamp(1.0F - previousValue * 1.6F, 0.0F, 1.0F) * Math.clamp(nextValue * 2.0F, 0.0F, 1.0F);
      return new Vector3f(
   ...
   public static Vector3f cinematicSunDirection(double x, double y) {
      ...
      double doubleValue = -0.45
         + 0.1
            * (height * height * (3.0 - 2.0 * height));
      double currentDoubleValue = 0.36
         - 0.06 * Math.sin(3.141592653589793 * height);
      double nextDoubleValue = Math.cos(currentDoubleValue);
      return new Vector3f((float)(Math.sin(doubleValue) * nextDoubleValue), (float)Math.sin(currentDoubleValue), (float)(Math.cos(doubleValue) * nextDoubleValue)).normalize();
   }
   ...
   public static CompatLoadedHandler.SkyState cinematicSky(double doubleValue, double currentDoubleValue) {
      return new CompatLoadedHandler.SkyState(cinematicSunDirection(doubleValue, currentDoubleValue), cinematicDaylight());
   }
}
