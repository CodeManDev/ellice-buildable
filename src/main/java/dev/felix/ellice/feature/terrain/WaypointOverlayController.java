...
public final class WaypointOverlayController extends ScenePctService<WaypointOverlayController>
  ...
  public WaypointOverlayController(
      TerrainMapController terrainMapController, Render3dSceneService render3dScene) {
  ...
  public void update(
    ...
    if (!currentEnabled) {
      ...
      Vector3d vector3d =
          new Vector3d(
              worldRender.cameraPos().x, worldRender.cameraPos().y, worldRender.cameraPos().z);
      Matrix4f matrix4f =
          new Matrix4f(worldRender.projectionMatrix()).mul(worldRender.viewMatrix());
      this.frame =
          new TerrainViewportService.Frame(
              vector3d,
              vector3d,
              matrix4f,
          ...
          this.renderer.visibleWaypoints().stream()
              .sorted(
                  Comparator.<TerrainKindData>comparingInt(
                      ...
                      .thenComparingDouble(
                          item -> vector3d.distanceSquared(item.x(), item.y(), item.z())))
              .limit(12L)
              .toList();
    ...
    if (this.frame != null) {
      ...
      for (TerrainKindData currentTerrainKindData : this.items) {
        ...
        if (currentX.isFinite()) {
          ...
          int nextValue =
              !(currentX.w <= 0.0F) && !(Math.abs(value) > 0.91) && !(Math.abs(currentValue) > 0.84)
                  ? 0
                  : 1;
          if (nextValue == 0 || enabled) {
            ...
            if (nextValue != 0) {
              float targetValue = Math.max(Math.abs(value) / 0.88F, Math.abs(currentValue) / 0.74F);
              previousValue = (value / targetValue + 1.0F) * this.cw / 2.0F;
              sourceValue = (currentValue / targetValue + 1.0F) * this.ch / 2.0F;
            }

            previousValue = Math.clamp(previousValue, 24.0F, Math.max(24.0F, this.cw - 24.0F));
            sourceValue = Math.clamp(sourceValue, 68.0F, Math.max(68.0F, this.ch - 34.0F));
            arrayList.add(
                new TerrainReservedService.Anchor(
                    ...
                    this.frame
                        .eye()
                        .distance(
                            currentTerrainKindData.x(),
                            currentTerrainKindData.y(),
                            currentTerrainKindData.z()),
                    enabled,
                    terrainKindData != null
                        && terrainKindData.id().equals(currentTerrainKindData.id()),
      ...
      this.terrainReservedService.present(
          arrayList, TerrainReservedService.View.WORLD, null, false);
    }
  }

  private static String createText(float value, float currentValue) {
    int index =
        Math.floorMod((int) Math.round(Math.atan2(currentValue, value) / 0.7853981633974483), 8);
