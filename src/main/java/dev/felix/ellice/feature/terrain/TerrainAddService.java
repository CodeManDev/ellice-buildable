...
public final class TerrainAddService {
      ...
      if (Double.isFinite(sample.seconds())
         ...
         for (CompatLoadedHandler.MapPlayer mapPlayer : sample.players()) {
            for (CompatLoadedHandler.PlayerPart playerPart : mapPlayer.model()) {
               ...
               if (!this.values2.contains(geometryVertexCount) && currentValues.add(geometryVertexCount)) {
                  currentSize += (long)geometryVertexCount.vertexCount() * geometryVertexCount.vertexStrideBytes() + geometryVertexCount.indexCount() * 4L;
               }
            }
         }
   ...
   public void finish(double doubleValue) {
      ...
      for (TerrainAddService.Sample sample : this.items) {
         sample.terrain().forEach((item, currentItem) -> {
      ...
      for (int index = 0; index < this.items.size(); index++) {
         ...
         hashMap.forEach((item, currentItem) -> {
            if (currentSample.seconds() < (Double)currentHashMap.get(item)) {
               nextHashMap.put(item, currentItem);
            }
         });
         this.items
            .set(index, new TerrainAddService.Sample(currentSample.seconds(), currentSample.player(), currentSample.players(), nextHashMap, currentSample.sky(), currentSample.ground()));
      }

      this.value = Math.clamp(
         doubleValue, Math.max(0.041666666666666664, this.items.getLast().seconds()), this.value2
      );
      this.enabled = true;
   }
   ...
   public TerrainAddService.Sample at(double doubleValue) {
      ...
      if (sample != nextSize && !(doubleValue <= sample.seconds())) {
         double currentDoubleValue = Math.clamp((doubleValue - sample.seconds()) / (nextSize.seconds() - sample.seconds()), 0.0, 1.0);
         HashMap hashMap = new HashMap();
         nextSize.players().forEach(item -> hashMap.put(item.id(), item));
         ArrayList arrayList = new ArrayList();

         for (CompatLoadedHandler.MapPlayer mapPlayer : sample.players()) {
            CompatLoadedHandler.MapPlayer currentMapPlayer = (CompatLoadedHandler.MapPlayer)hashMap.get(mapPlayer.id());
            if (currentMapPlayer != null
               && !(
                  new Vector3d(mapPlayer.x(), mapPlayer.y(), mapPlayer.z()).distanceSquared(currentMapPlayer.x(), currentMapPlayer.y(), currentMapPlayer.z())
                     > 64.0
               )) {
               arrayList.add(
                  new CompatLoadedHandler.MapPlayer(
                     ...
                     (float)(
                        mapPlayer.yaw() + Math.IEEEremainder(currentMapPlayer.yaw() - mapPlayer.yaw(), 360.0) * currentDoubleValue
                     ),
                     (float)calculateValue(mapPlayer.pitch(), currentMapPlayer.pitch(), currentDoubleValue),
                     mapPlayer.slim(),
                     mapPlayer.skinGlId(),
         ...
         return new TerrainAddService.Sample(
            doubleValue, new Vector3d(sample.player()).lerp(nextSize.player(), currentDoubleValue), arrayList, sample.terrain(), sample.sky(), sample.ground()
         );
      } else {
         return sample;
      }
