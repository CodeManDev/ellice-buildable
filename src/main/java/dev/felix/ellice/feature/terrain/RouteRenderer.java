...
final class RouteRenderer implements AutoCloseable {
   ...
   private final MaterialErFactory materialErFactory = MaterialErFactory.builder(
         "ellice:map-route", new ShaderDefinition("ellice:map-route", "terrain/route.vert", "terrain/route.frag")
      )
   ...
   void update(TerrainGroundTracker terrainGroundTracker, TerrainViewportService.Frame frame) {
      ...
      if (currentItems != this.items) {
         ...
         if (currentItems.size() >= 2) {
            ...
            this.renderer3 = this.renderer2.add(this.renderer4, this.materialErFactory, new GeometryXService(vector3d.x, vector3d.y, vector3d.z));
      ...
      if (this.renderer3 != null) {
         float currentHeight = (float)Math.clamp(
            frame.distance() * 0.828427 / frame.height() * 4.0,
            0.18,
            0.45
         );
         this.materialErFactory
            ...
            .uniform(
               "uReveal", (float)Math.min(1.0, (System.nanoTime() - this.timestamp) / 4.5E8)
            );
      }
   }
