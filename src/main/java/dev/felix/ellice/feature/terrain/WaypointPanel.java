    ...
    String nextText =
        this.terrainGroundTracker.status() == TerrainGroundTracker.Status.READY
            ? (this.terrainGroundTracker.remaining() / 4.3 < 60.0
                ? "<1 min walk"
                : (int) Math.ceil(this.terrainGroundTracker.remaining() / 4.3 / 60.0) + " min walk")
            : "";
    return ComponentBoxService.panel(
            ...
            ComponentBoxService.row(
                        ...
                        .onMount(
                            item -> {
                              ...
                              MotionAnimateService.animate(
                                  ...
                                  1.0F,
                                  SceneEaseHandler.Tween.ease(0.16F));
                              MotionAnimateService.animate(
                                  item,
                ...
                ComponentBoxService.column(
                    ...
                    MaterialTextService.text(
                            currentInstruction.text(), 14.0F, MaterialIsLightService.ON_SURFACE)
                        .props(item -> item.fontFamily("material-roboto-medium").wordWrap(true)),
                    MaterialTextService.text(
                        height != 0
                            ? terrainKindData.name()
                            : (this.terrainGroundTracker.status()
                                    == TerrainGroundTracker.Status.READY
                                ? "In "
                                    + TerrainReservedService.distance(currentInstruction.distance())
                                : "Walking directions"),
                        11.0F,
            ...
            ComponentBoxService.row(
                ...
                ComponentBoxService.column(
                    ...
                    MaterialTextService.text(
                            terrainKindData.name(), 12.0F, MaterialIsLightService.ON_SURFACE)
                        .props(item -> item.fontFamily("material-roboto-medium")),
                    MaterialTextService.text(
                        height != 0
                            ? "Destination reached"
                            : currentText + (nextText.isBlank() ? "" : " · " + nextText),
                        11.0F,
                        MaterialIsLightService.ON_SURFACE_VARIANT)),
                MaterialTextService.iconButton(
                    "map.navigation.recenter", "person", "Recenter on player", this.runnable)))
        .key("navigation-card")
        .onMount(
            item -> {
              ...
              MotionAnimateService.animate(
                  ...
                  MotionColorsContainer.Floats.TRANSLATE_Y,
                  0.0F,
                  SceneEaseHandler.Spring.GENTLE);
            });
  }
