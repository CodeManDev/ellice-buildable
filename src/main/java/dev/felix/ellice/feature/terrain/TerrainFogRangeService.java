...
public final class TerrainFogRangeService {
  ...
  public static int[] targetSize(
      float value, float currentValue, float nextValue, boolean enabled) {
    double doubleValue = Math.max(0.01, nextValue) * (enabled ? 1.25 : 1.0);
    doubleValue *=
        Math.min(1.0, 2048.0 / (Math.max(1.0F, Math.max(value, currentValue)) * doubleValue));
    return new int[] {
      Math.max(32, (int) Math.round(value * doubleValue)),
      Math.max(32, (int) Math.round(currentValue * doubleValue))
    };
  }
}
