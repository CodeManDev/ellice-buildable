...
public final class SkyTextureRenderer implements AutoCloseable {
   ...
   private RhiBlendStateService.SamplerHandle samplerHandle2 = RhiBlendStateService.SamplerHandle.NONE;
   ...
   public SkyTextureRenderer(Render3dSceneService render3dScene) {
      ...
      this.shaderRecipeService = new ShaderRecipeService(
         ...
         new ShaderData(
            new ShaderDefinition("ellice:terrain-cinematic-grade", "fullscreen.vert", "terrain/cinematic_grade.frag"),
            vertexLayout,
   ...
   public RhiBlendStateService.TextureHandle render(
      RhiBlendStateService.TextureHandle colorTexture, RhiBlendStateService.TextureHandle depthTexture, int width, int height, float nearPlane, float farPlane, float focusDistance, double time
   ) {
               ...
               if (this.textureHandle.valid()
                  ...
                  if (pipelineHandle.valid()) {
                     ...
                     try {
                        ...
                        int depthAvailable = depthTexture != null
                              && depthTexture.valid()
                        ...
                        rhiCommandBuffer.pushFloat("uFocusDist", Float.isFinite(focusDistance) && focusDistance > 0.0F ? focusDistance : 20.0F);
   ...
   private void updateState(int value, int currentValue) {
      ...
      if (!this.samplerHandle.valid()) {
         this.samplerHandle = this.rhiOperationHandler
            .createSampler(
      ...
      if (!this.samplerHandle2.valid()) {
         this.samplerHandle2 = this.rhiOperationHandler.createSampler(RhiBlendStateService.SamplerDescriptor.NEAREST_CLAMP);
      }
      ...
      if (!this.textureHandle.valid() || nextValue != this.count || previousValue != this.count2) {
         ...
         RhiBlendStateService.TextureHandle currentTextureHandle = this.rhiOperationHandler
            .createTexture(
         ...
         RhiBlendStateService.FramebufferHandle currentFramebufferHandle = this.rhiOperationHandler.createFramebuffer(currentTextureHandle);
   ...
   public void close() {
      ...
      if (!this.framebufferHandle.valid() && !this.bufferHandle.valid() && !this.samplerHandle.valid() && !this.samplerHandle2.valid()) {
         runnable.run();
      } else {
   ...
   public record Look(
      ...
      public Look(
         ...
         exposure = calculateValue(
            exposure, 0.06F, -3.0F, 3.0F
         );
         ...
         ssaoRadius = calculateValue(
            ssaoRadius, 1.5F, 0.2F, 4.0F
         );
