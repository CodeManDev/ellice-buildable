...
public final class TerrainFarForService {
  ...
  private static final double[] double2 = new double[] {0.3, 0.48, 0.7, 1.0, 1.35};
  ...
  public static float farFor(double doubleValue) {
    double currentDoubleValue = Double.isFinite(doubleValue) ? Math.max(0.5, doubleValue) : 20.0;
  ...
  public TerrainFarForService(TerrainAddService terrainAdd, UUID currentUUID) {
    ...
    double doubleValue =
        calculateValue13(sample, currentVector3d.x, currentVector3d.z, currentVector3d.y);
    ...
    this.value14 =
        vector3d == null
            ? -0.6
            : Math.atan2(vector3d.x - sample.player().x, vector3d.z - sample.player().z)
                + 1.5707963267948966;
    ...
    this.spring4 =
        new TerrainFarForService.Spring(
            this.calculateValue3(sample, currentVector3d, currentDoubleValue, 0.0));
    this.spring5 =
        new TerrainFarForService.Spring(
            calculateValue12(
                    sample, currentVector3d, this.value14, this.spring4.value, currentDoubleValue)
                + calculateValue9(0.0));
    this.value15 = 0.0;
    ...
    if (vector3d != null && vector3d.distanceSquared(sample.player()) < 576.0) {
      ...
      for (CompatLoadedHandler.MapPlayer mapPlayer : sample.players()) {
        if (!mapPlayer.id().equals(this.uUID)) {
          double currentX =
              sample.player().distanceSquared(mapPlayer.x(), mapPlayer.y(), mapPlayer.z());
  ...
  private double calculateValue(TerrainAddService.Sample sample, Vector3d vector3d) {
    ...
    if (currentVector3d != null && currentVector3d.distanceSquared(sample.player()) < 576.0) {
      doubleValue =
          Math.max(doubleValue, vector3d.distance(currentVector3d.add(0.0, 1.0, 0.0)) + 2.0);
    }
  ...
  private double calculateValue3(
      TerrainAddService.Sample sample,
      Vector3d vector3d,
      double doubleValue,
  ...
  private static double calculateValue4(
    ...
    double previousDoubleValue =
        Math.clamp((nextDoubleValue - doubleValue) / (currentDoubleValue - doubleValue), 0.0, 1.0);
  ...
  private static double calculateValue6(double doubleValue) {
    ...
    sourceDoubleValue =
        sourceDoubleValue * (1.0 - previousDoubleValue) + 44.0 * previousDoubleValue;
  ...
  private static Vector3d createVector3d3(double doubleValue, double currentDoubleValue) {
    return new Vector3d(
        Math.sin(doubleValue) * Math.cos(currentDoubleValue),
        Math.sin(currentDoubleValue),
        Math.cos(doubleValue) * Math.cos(currentDoubleValue));
  }

  private static double calculateValue10(
      TerrainAddService.Sample sample,
      Vector3d vector3d,
      Vector3d currentVector3d,
      double doubleValue) {
    Vector3d nextVector3d =
        TerrainPickService.pick(
            sample.terrain().values(),
            new TerrainViewportService.Ray(vector3d, currentVector3d),
            doubleValue + 1.0);
    return nextVector3d == null
        ? doubleValue
        : Math.max(0.5, vector3d.distance(nextVector3d) - 1.0);
  }

  private static double calculateValue11(
      TerrainAddService.Sample sample,
      Vector3d vector3d,
      double doubleValue,
      double currentDoubleValue,
    ...
    double resultDoubleValue =
        calculateValue10(
            sample, vector3d, createVector3d3(doubleValue, currentDoubleValue), nextDoubleValue);
    resultDoubleValue =
        Math.min(
            resultDoubleValue,
            calculateValue10(
                sample,
                vector3d,
                createVector3d3(doubleValue + inputDoubleValue, currentDoubleValue),
                nextDoubleValue));
    resultDoubleValue =
        Math.min(
            resultDoubleValue,
    ...
    resultDoubleValue =
        Math.min(
            ...
            calculateValue10(
                sample,
                vector3d,
                createVector3d3(doubleValue, currentDoubleValue + outputDoubleValue),
                nextDoubleValue));
    return Math.min(
        resultDoubleValue,
        calculateValue10(
            sample,
            vector3d,
            createVector3d3(doubleValue, Math.max(0.05, currentDoubleValue - outputDoubleValue)),
            nextDoubleValue));
  }

  private static double calculateValue12(
      TerrainAddService.Sample sample,
      Vector3d vector3d,
      double doubleValue,
      double currentDoubleValue,
  ...
  private static double calculateValue13(
      TerrainAddService.Sample sample,
      double doubleValue,
      double currentDoubleValue,
      double nextDoubleValue) {
  ...
  private static boolean checkCondition(
      TerrainAddService.Sample sample, Vector3d vector3d, Vector3d currentVector3d) {
    ...
    Vector3d sourceVector3d =
        TerrainPickService.pick(
            sample.terrain().values(),
            new TerrainViewportService.Ray(vector3d, previousVector3d),
            currentLength);
    return sourceVector3d != null
        && vector3d.distance(sourceVector3d) > 1.0
        && vector3d.distance(sourceVector3d) < currentLength - 0.3;
  }
  ...
  private TerrainFarForService.Pose createPose(
      TerrainAddService.Sample sample, Vector3d vector3d, double doubleValue) {
    ...
    Vector3d nextVector3d =
        new Vector3d(currentVector3d)
            .mul(Math.min(this.spring4.value, nextDoubleValue))
            .add(vector3d);
    double previousDoubleValue =
        calculateValue13(sample, nextVector3d.x, nextVector3d.z, nextVector3d.y);
    if (nextVector3d.y < previousDoubleValue + 0.3) {
      nextVector3d.y = previousDoubleValue + 0.3;
    }

    return new TerrainFarForService.Pose(
        vector3d, nextVector3d, this.spring6.value, this.spring7.value);
  }
  ...
  private void updateState2(int currentValue) {
    ...
    Vector3d currentVector3d =
        new Vector3d(this.spring.value, this.spring2.value, this.spring3.value);
    double currentDoubleValue =
        calculateValue13(sample, currentVector3d.x, currentVector3d.z, currentVector3d.y);
    ...
    double outputDoubleValue =
        calculateValue2(targetDoubleValue, sourceDoubleValue) * inputDoubleValue;
    if (outputDoubleValue > 26.0 && targetDoubleValue > 1.0E-9) {
      double resultDoubleValue =
          Math.clamp(targetDoubleValue * 1.05 * inputDoubleValue / 26.0, 0.2, 1.0);
      sourceDoubleValue =
          Math.clamp(Math.toDegrees(2.0 * Math.asin(resultDoubleValue) / 0.9), 36.0, 50.0);
    }
    ...
    double selectedDoubleValue =
        Math.clamp(
            calculateValue2(targetDoubleValue, this.spring6.value) * inputDoubleValue, 5.5, 26.0);
    ...
    double pendingDoubleValue =
        calculateValue12(
                ...
                this.spring6.value)
            + cachedDoubleValue;
    TerrainAddService.Sample currentSample =
        this.terrainAddService.at(Math.min(this.terrainAddService.duration(), doubleValue + 1.2));
    double activeDoubleValue = previousDoubleValue + calculateValue5(doubleValue + 0.6) * 1.2;
    pendingDoubleValue =
        Math.max(
            pendingDoubleValue,
            calculateValue12(
                    currentSample,
                    this.createVector3d2(currentSample),
                    activeDoubleValue,
                    this.spring4.value,
                    this.spring6.value)
                + calculateValue8(
                    calculateValue4(3.5, 4.5, doubleValue + 1.2),
                    calculateValue4(13.5, 14.5, doubleValue + 1.2),
                    calculateValue4(17.5, 18.5, doubleValue + 1.2)));
    ...
    double fallbackDoubleValue =
        calculateValue13(
            sample,
            currentVector3d.x
                + Math.sin(previousDoubleValue) * Math.cos(pendingDoubleValue) * this.spring4.value,
            currentVector3d.z
                + Math.cos(previousDoubleValue) * Math.cos(pendingDoubleValue) * this.spring4.value,
            currentVector3d.y + Math.sin(pendingDoubleValue) * this.spring4.value);
    if (fallbackDoubleValue + 0.3
        > currentVector3d.y + Math.sin(pendingDoubleValue) * this.spring4.value) {
      pendingDoubleValue = Math.min(1.35, pendingDoubleValue + 0.08);
    }

    pendingDoubleValue = Math.clamp(pendingDoubleValue, 0.05, 1.35);
    this.spring5.advance(pendingDoubleValue, pendingDoubleValue > this.spring5.value ? 8.0 : 3.0);
  ...
  public TerrainViewportService.Frame frame(double doubleValue, int value, int currentValue) {
    if (Double.isFinite(doubleValue) && value > 0 && currentValue > 0) {
      double currentDoubleValue =
          Math.clamp(doubleValue, 0.0, this.terrainAddService.duration()) / 0.016666666666666666;
      ...
      if (Double.isFinite(doubleValue)) {
        double previousDoubleValue = Math.max(0.5, currentVector3d.distance(vector3d)) * 0.0035;
        ...
        if (nextVector3d.lengthSquared() > 1.0E-12) {
          ...
          if (previousVector3d.lengthSquared() > 1.0E-8) {
            ...
            Vector3d sourceVector3d =
                new Vector3d(previousVector3d).cross(nextVector3d).normalize();
            currentVector3d
                .fma(Math.sin(doubleValue * 0.9) * previousDoubleValue, previousVector3d)
                .fma(
                    Math.sin(doubleValue * 1.27 + 1.3) * previousDoubleValue * 0.7, sourceVector3d);
          }
        }
      }
      ...
      Matrix4f matrix4f =
          new Matrix4f()
              .lookAt(
                  (float) targetVector3d.x,
                  (float) targetVector3d.y,
                  (float) targetVector3d.z,
                  0.0F,
                  0.0F,
                  0.0F,
                  0.0F,
      ...
      Matrix4f currentMatrix4f =
          new Matrix4f()
              .perspective(
                  (float) Math.toRadians(sourceDoubleValue),
      ...
      return new TerrainViewportService.Frame(
          ...
          currentVector3d,
          nextMatrix4f,
          new Matrix4f(nextMatrix4f).invert(),
          value,
          currentValue,
          inputDoubleValue,
          matrix4f,
  ...
  private static final class Spring {
    ...
    void advance(double doubleValue, double currentDoubleValue) {
      ...
      this.value =
          doubleValue
              + (nextDoubleValue + previousDoubleValue * 0.016666666666666666) * sourceDoubleValue;
      this.velocity =
          (this.velocity - currentDoubleValue * previousDoubleValue * 0.016666666666666666)
              * sourceDoubleValue;
    }
  }
}
