...
final class GroundMarkerRenderer implements AutoCloseable {
  ...
  private final MaterialErFactory materialErFactory =
      MaterialErFactory.builder(
              "ellice:map-ground-locator",
              new ShaderDefinition(
                  "ellice:map-ground-locator", "render3d/pbr.vert", "terrain/marker.frag"))
  ...
  void update(List<TerrainKindData> items, UUID uUID, TerrainViewportService.Frame frame) {
    TerrainKindData terrainKindData =
        items.stream().filter(item -> item.id().equals(uUID)).findFirst().orElse(null);
    if (terrainKindData != null
        && frame.project(terrainKindData.x(), terrainKindData.y(), terrainKindData.z()) != null) {
      if (this.renderer4 == null) {
        this.renderer4 =
            this.renderer2.add(this.renderer3, this.materialErFactory, GeometryXService.IDENTITY);
      }
      ...
      Vector4f currentX =
          frame
              ...
              .transform(
                  new Vector4f(
                      (float) (terrainKindData.x() - frame.focus().x),
                      (float) (terrainKindData.y() - frame.focus().y),
                      (float) (terrainKindData.z() - frame.focus().z),
                      1.0F));
      float currentHeight =
          (float)
              Math.clamp(
                  currentX.w * 2.0 / frame.projection().m11() / frame.height() * 24.0, 0.6, 12.0);
      this.materialErFactory.color(
          "uColor",
          MaterialIsLightService.layer(
              terrainKindData.color(), MaterialIsLightService.PRIMARY, 0.25F));
      this.renderer4.transform(
