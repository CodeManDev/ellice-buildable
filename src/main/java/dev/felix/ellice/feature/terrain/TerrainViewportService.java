...
public final class TerrainViewportService {
   ...
   private final TerrainViewportService.Spring spring = new TerrainViewportService.Spring(Math.log(220.0));
   private final TerrainViewportService.Spring spring2 = new TerrainViewportService.Spring(Math.toRadians(-35.0));
   private final TerrainViewportService.Spring spring3 = new TerrainViewportService.Spring(Math.toRadians(58.0));
   ...
   public void flyTo(double doubleValue, double currentDoubleValue, double nextDoubleValue) {
      if (Double.isFinite(doubleValue) && Double.isFinite(currentDoubleValue) && Double.isFinite(nextDoubleValue)) {
         this.spring6 = new TerrainViewportService.Spring[]{
         ...
         this.spring6[0].target = Math.clamp(
            doubleValue, -2.9999984E7, 2.9999984E7
         );
         this.spring6[1].target = Math.clamp(
            currentDoubleValue, -4096.0, 4096.0
         );
         this.spring6[2].target = Math.clamp(
            nextDoubleValue, -2.9999984E7, 2.9999984E7
         );
         this.vector3d = null;
         this.value5 = this.value6 = 0.0;
      }
   ...
   public TerrainViewportService.Frame frame() {
      ...
      if (this.spring5.value > 0.0) {
         ...
         currentMatrix4f.m00(currentMatrix4f.m00() / sourceValue).m11(currentMatrix4f.m11() / sourceValue).m22(currentMatrix4f.m22() / sourceValue).m23(currentMatrix4f.m23() / sourceValue).m32(currentMatrix4f.m32() / sourceValue);
         float targetValue = sourceValue * 0.41421357F;
         Matrix4f nextMatrix4f = new Matrix4f()
            .ortho(
      ...
      return new TerrainViewportService.Frame(
         ...
         new Vector3d(this.value2 + currentValue, this.value3 + nextValue, this.value4 + previousValue),
         previousMatrix4f,
   ...
   public void zoom(double doubleValue, float value, float currentValue, Vector3d currentVector3d) {
      if (Double.isFinite(doubleValue)) {
         this.spring6 = null;
         this.spring.target = Math.clamp(
            this.spring.target - doubleValue * 0.16,
            Math.log(12.0),
            Math.log(8192.0)
         );
         if (currentVector3d != null && currentVector3d.isFinite()) {
            this.spring4.target = Math.clamp(
               currentVector3d.y, -4096.0, 4096.0
            );
         }

         this.vector3d = currentVector3d != null ? new Vector3d(currentVector3d) : this.frame().ray(value, currentValue).onPlane(this.value3);
   ...
   public void pan(float value, float currentValue, float nextValue, float previousValue, double doubleValue) {
      ...
      if (currentVector3d != null && nextVector3d != null) {
         ...
         if (doubleValue > 0.001 && doubleValue < 0.15) {
            this.value5 = Math.clamp(
               currentDoubleValue / doubleValue, -3000.0, 3000.0
            );
            this.value6 = Math.clamp(
               nextDoubleValue / doubleValue, -3000.0, 3000.0
            );
         }
   ...
   public void northUp() {
      this.spring2.target = this.spring2.value
         - Math.IEEEremainder(this.spring2.value, 6.283185307179586);
      this.vector3d = null;
   }
   ...
   public void topDown(boolean enabled) {
      this.spring3.target = Math.toRadians(
         enabled ? 90.0 : 58.0
      );
   ...
   public void distance(double doubleValue) {
      if (Double.isFinite(doubleValue)) {
         this.spring
            .snap(Math.log(Math.clamp(doubleValue, 12.0, 8192.0)));
      }
   ...
   public void zoomTo(double doubleValue) {
      if (Double.isFinite(doubleValue)) {
         this.spring.target = Math.log(
            Math.clamp(doubleValue, 12.0, 8192.0)
         );
      }
   ...
   public void advance(double doubleValue) {
      if (Double.isFinite(doubleValue) && !(doubleValue <= 0.0)) {
         ...
         if (this.vector3d != null) {
            Vector3d currentVector3d = this.frame()
   ...
   private void updateState() {
      this.value2 = Math.clamp(
         this.value2, -2.9999984E7, 2.9999984E7
      );
      this.value4 = Math.clamp(
         this.value4, -2.9999984E7, 2.9999984E7
      );
   }
   ...
   public record Frame(
      ...
      public TerrainViewportService.Ray ray(float value, float currentValue) {
         ...
         Vector4f vector4f = this.inverse.transform(new Vector4f(nextValue, previousValue, -1.0F, 1.0F));
         Vector4f currentVector4f = this.inverse.transform(new Vector4f(nextValue, previousValue, 1.0F, 1.0F));
         ...
         Vector3d currentVector3d = new Vector3d(currentVector4f.x - vector4f.x, currentVector4f.y - vector4f.y, currentVector4f.z - vector4f.z).normalize();
      ...
      public Vector3d project(double doubleValue, double currentDoubleValue, double nextDoubleValue) {
         Vector4f vector4f = this.viewProjection
            .transform(new Vector4f((float)(doubleValue - this.focus.x), (float)(currentDoubleValue - this.focus.y), (float)(nextDoubleValue - this.focus.z), 1.0F));
         if (vector4f.isFinite() && !(vector4f.w <= 0.001F)) {
            ...
            return !(vector4f.x < -1.0F)
                  ...
                  && !(vector4f.z < -1.0F)
                  && !(vector4f.z > 1.0F)
               ? new Vector3d((vector4f.x + 1.0F) * this.width / 2.0F, (1.0F - vector4f.y) * this.height / 2.0F, vector4f.z)
               : null;
         } else {
            return null;
         }
   ...
   private static final class Spring {
      ...
      boolean settled() {
         return Math.abs(this.value - this.target) < 1.0E-6
            && Math.abs(this.velocity) < 1.0E-5;
      }
      ...
      void advance(double doubleValue, double currentDoubleValue) {
         ...
         this.value = this.target + (nextDoubleValue + previousDoubleValue * doubleValue) * sourceDoubleValue;
         this.velocity = (this.velocity - currentDoubleValue * previousDoubleValue * doubleValue) * sourceDoubleValue;
         if (this.settled()) {
