...
public final class TerrainRenderer {
   ...
   public static boolean ready(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      compositorPushPresentationScale.requestFontFamily("ellice-navigation", TextTextureService.FontSource.CLASSPATH, "/assets/ellice/fonts/material/ElliceNavigation.ttf");
      return compositorPushPresentationScale.isFontFamilyReady("ellice-navigation");
   }

   public static void draw(CompositorPushPresentationScaleService compositorPushPresentationScale, String currentText, float width, float height, float value, int currentValue) {
      if (ready(compositorPushPresentationScale)) {
         float nextValue = compositorPushPresentationScale.textWidth(currentText, value, TextMode.REGULAR, "ellice-navigation");
         float previousValue = compositorPushPresentationScale.textLineHeight(value, TextMode.REGULAR, "ellice-navigation");
         compositorPushPresentationScale.text(width + (value - nextValue) / 2.0F, height + (value - previousValue) / 2.0F, currentText, value, currentValue, textData);
      }
   }
