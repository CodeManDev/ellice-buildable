...
public record TerrainLayerData(TerrainData key, List<TerrainLayerData.Layer> layers) {
   ...
   public static TerrainLayerData.Layer copy(TerrainLayerData.Pass pass, ByteBuffer byteBuffer, int value, int currentValue, float nextValue) {
   ...
   public static void appendColumn(TerrainLayerData.Layer layer, int value, ByteBuffer byteBuffer) {
      ...
      for (int index = 0; index < layer.vertexCount; index++) {
         ...
         byteBuffer.putFloat(currentValue + nextValue, currentByteBuffer.getFloat(nextValue) + value * 16.0F);
      }
   }

   public record Layer(TerrainLayerData.Pass pass, byte[] vertices, int vertexCount, float alphaCutoff) {
      public Layer(TerrainLayerData.Pass pass, byte[] vertices, int vertexCount, float alphaCutoff) {
         if (pass != null && vertices != null && vertexCount >= 0 && vertexCount % 4 == 0 && vertices.length == vertexCount * 28L) {
            this.pass = pass;
