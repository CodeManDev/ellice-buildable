...
final class FrameCaptureBuffer implements AutoCloseable {
   ...
   private int calculateValue(int value, boolean enabled) {
      ...
      RhiBlendStateService.TextureDownload textureDownload = rhiOperation.downloadTexture(new RhiBlendStateService.TextureHandle(value));
      if (textureDownload != null && textureDownload.complete()) {
         try {
            long currentWidth = (long)textureDownload.width() * textureDownload.height() * (enabled ? 6 : 4);
            if (this.timestamp + currentWidth > 268435456L) {
               throw new IllegalStateException("Recording texture limit reached (256 MiB)");
            }

            RhiBlendStateService.TextureHandle nextWidth = rhiOperation.createTexture(
               new RhiBlendStateService.TextureDescriptor(
                  ...
                  enabled ? RhiBlendStateService.FilterMode.NEAREST_MIPMAP_LINEAR : RhiBlendStateService.FilterMode.NEAREST,
   ...
   List<CompatLoadedHandler.MapPlayer> capture(List<CompatLoadedHandler.MapPlayer> items) {
      ...
      for (CompatLoadedHandler.MapPlayer mapPlayer : items) {
         ...
         for (CompatLoadedHandler.PlayerPart playerPart : mapPlayer.model()) {
            currentArrayList.add(new CompatLoadedHandler.PlayerPart(playerPart.mesh(), this.calculateValue(playerPart.texture(), false), playerPart.color()));
         }
