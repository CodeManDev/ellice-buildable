...
public final class TerrainRecoveredFactory {
   ...
   public static GeometryVertexCountService create(List<Vector3d> items) {
      ...
      for (int currentIndex = 0; currentIndex < arrayList.size(); currentIndex++) {
         ...
         Vector3d sourceVector3d = new Vector3d(previousVector3d).sub((Vector3dc)arrayList.get(Math.max(0, currentIndex - 1)));
         Vector3d previousSize = new Vector3d((Vector3dc)arrayList.get(Math.min(arrayList.size() - 1, currentIndex + 1))).sub(previousVector3d);
         if (currentIndex > 0) {
         ...
         double currentDoubleValue = Math.min(
            2.0, 1.0 / Math.max(0.5, targetVector3d.dot(previousSize))
         );
         ...
         for (int nextIndex = 0; nextIndex < 2; nextIndex++) {
            ...
            currentSize[previousIndex] = (float)(previousVector3d.x - nextVector3d.x + inputVector3d.x * nextDoubleValue * 0.125);
            currentSize[previousIndex + 1] = (float)(previousVector3d.y - nextVector3d.y + 0.07);
            currentSize[previousIndex + 2] = (float)(previousVector3d.z - nextVector3d.z + inputVector3d.z * nextDoubleValue * 0.125);
