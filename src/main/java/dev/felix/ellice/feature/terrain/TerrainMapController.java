...
public final class TerrainMapController implements AutoCloseable {
   ...
   public TerrainMapController(final CompatLoadedHandler compatLoaded, Path currentPath, Render3dSceneService render3dScene) {
      ...
      this.terrainGroundTracker = new TerrainGroundTracker(new TerrainGroundTracker.Ground() {
         @Override
   ...
   public void navigate(TerrainKindData terrainKindData) {
      if (this.inCurrentDimension(terrainKindData)) {
         ...
         this.timestamp3++;
         Vector3d currentX = new Vector3d(this.world2.player())
         ...
         this.renderer3
            ...
            .zoomTo(
               Math.max(
                  60.0,
                  this.world2.player().distance(terrainKindData.x(), terrainKindData.y(), terrainKindData.z()) * 1.7
               )
            );
      }
   }
   ...
   public TerrainKindData finishWaypointDrag(boolean enabled) {
      ...
      this.terrainKindData = null;
      return currentTerrainKindData != null && enabled && !this.save(currentTerrainKindData) ? null : currentTerrainKindData;
   }
   ...
   public void beginFrame() {
      ...
      double doubleValue = this.timestamp2 == 0L
         ? 0.016666666666666666
         : Math.min(
            0.1, (longValue - this.timestamp2) / 1.0E9
         );
      ...
      if (sourceValue != 0 || targetValue != 0) {
         ...
         if (sourceValue != 0 && currentWorld != null) {
            this.renderer3.camera.center(currentWorld.player().x, currentWorld.player().y, currentWorld.player().z);
      ...
      this.object2 = value;
      this.vector3d = this.world2 != null && this.world2.player() != null ? new Vector3d(this.world2.player()) : null;
      TerrainKindData currentTerrainKindData = this.terrainObserveService
         .observe(this.world2, this.object2, this.compatLoadedHandler.playerDead(), this.enabled4, System.currentTimeMillis());
      ...
      if (this.world2 != null && (this.enabled || this.enabled2)) {
         ...
         int inputValue = this.world2 != null && "minecraft:overworld".equals(this.world2.dimension()) ? 1 : 0;
         ...
         if (this.entries.isEmpty()) {
            ...
            for (CompatLoadedHandler.MapPlayer mapPlayer : this.entries) {
               Double currentX = this.compatLoadedHandler.shadowGround(mapPlayer.x(), mapPlayer.z(), mapPlayer.y());
               if (currentX != null) {
         ...
         this.renderer4.camera.center(this.world2.player().x, this.world2.player().y, this.world2.player().z);
         ...
         if (longValue - this.timestamp > 250000000L) {
            int currentCount = Math.clamp(
               (int)Math.ceil(view.camera.frame().distance() / 24.0), 2, this.count
            );
            this.items = this.compatLoadedHandler.sections(currentVector3d.x, currentVector3d.z, currentCount);
   ...
   public void render() {
      if (this.world2 != null && (this.enabled || this.enabled2)) {
         ...
         currentView.shadows
            .update(this.entries, currentView.target.view().scene().sun().direction(), currentView.camera.frame(), currentView.players, this.entries2);
         long currentLongValue = renderInterval(this.enabled, currentView.camera.moving(), !this.entries.isEmpty());
         if (currentView.target.needsRender() || longValue - currentView.lastRender >= currentLongValue) {
            ...
            this.renderer2
               .render(
                  ...
                  this.enabled && "minecraft:overworld".equals(this.world2.dimension()) && !currentView.camera.topDown()
               );
   ...
   private static void updateState(TerrainMapController.View currentView, CompatLoadedHandler.SkyState skyState) {
      if (skyState != null) {
         currentView.target.view().scene().sun(TerrainSunDirectionService.mapSun(skyState.sunDirection(), skyState.daylight()));
         currentView.target
            .view()
            .scene()
            .ambient(TerrainSunDirectionService.mapAmbientColor(skyState.daylight()), TerrainSunDirectionService.mapAmbientIntensity(skyState.daylight()));
      }
   }
   ...
   public void returnToPlayer() {
      if (this.world2 != null) {
         this.renderer3.camera.flyTo(this.world2.player().x, this.world2.player().y, this.world2.player().z);
      }
   }

   public List<TerrainKindData> waypoints() {
      if (this.world2 != null && this.terrainPointsService != null) {
         List<TerrainKindData> items = this.terrainPointsService
            .points(this.world2.dimension())
            .stream()
            .filter(item -> item.kind() != TerrainKindData.Kind.DEATH || this.timestamp4 <= 0L || item.createdAt() >= this.timestamp4)
            .toList();
         return this.terrainKindData == null
            ? items
            : items.stream().map(item -> item.id().equals(this.terrainKindData.id()) ? this.terrainKindData : item).toList();
      } else {
         return List.of();
      }
   }
   ...
   public TerrainKindData add(String text, Vector3d vector3d, int value) {
      if (this.world2 != null && this.terrainPointsService != null && vector3d != null) {
         TerrainKindData terrainKindData = new TerrainKindData(UUID.randomUUID(), text, this.world2.dimension(), vector3d.x, vector3d.y, vector3d.z, value);
   ...
   public TerrainKindData add(String text, Vector3d vector3d, TerrainKindData.Kind kind) {
      if (this.world2 != null && vector3d != null) {
         TerrainKindData terrainKindData = new TerrainKindData(
            UUID.randomUUID(), text, this.world2.dimension(), vector3d.x, vector3d.y, vector3d.z, kind.color(), kind, System.currentTimeMillis()
         );
         return this.save(terrainKindData) ? terrainKindData : null;
      } else {
         return null;
   ...
   public TerrainKindData planPortal(TerrainKindData terrainKindData) {
      ...
      if (portalPlan != null && portalPlan.insideBorder()) {
         ...
         TerrainKindData currentX = new TerrainKindData(
            UUID.randomUUID(),
   ...
   public void remove(UUID uUID) {
      if (this.terrainPointsService != null) {
         try {
            ...
            this.timestamp3++;
            this.text = "";
            if (this.terrainGroundTracker.target() != null && this.terrainGroundTracker.target().id().equals(uUID)) {
               this.terrainGroundTracker.stop();
            }
         } catch (IOException iOException) {
            this.text = iOException.getMessage();
   ...
   private static final class View {
      ...
      View(TerrainMapRenderer terrainMapRenderer, Render3dSceneService render3dScene) {
         ...
         this.target.view().scene().sun(TerrainSunDirectionService.mapSun(TerrainSunDirectionService.DEFAULT_SUN, 1.0F));
         this.target.view().scene().ambient(TerrainSunDirectionService.mapAmbientColor(1.0F), TerrainSunDirectionService.mapAmbientIntensity(1.0F));
