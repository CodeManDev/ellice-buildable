...
public final class MovementBurstScheduleService {
  ...
  public static List<MovementBurstScheduleService.SchedClaim> burstSchedule(
      Vec3 vec3, Vec3 currentVec3, double[] doubles, int value) {
    ...
    double previousDoubleValue =
        Math.sqrt(doubleValue * doubleValue + currentDoubleValue * currentDoubleValue);
    ...
    if (previousDoubleValue < 0.5) {
      double outputDoubleValue =
          Math.signum(nextDoubleValue) * Math.min(Math.abs(nextDoubleValue), 1.4);
    ...
    for (int index = 0; index < currentValue; index++) {
      ...
      arrayList.add(
          new MovementBurstScheduleService.SchedClaim(
              index / value, selectedDoubleValue, defaultDoubleValue, initialDoubleValue));
    }

    return arrayList;
  }

  public static List<MovementBurstScheduleService.SchedClaim> triggerSchedule(
      Vec3 vec3, Vec3 currentVec3, int value) {
    ...
    double inputDoubleValue =
        Math.signum(nextDoubleValue) * Math.min(Math.abs(nextDoubleValue), 2.24);
    ...
    for (int index = 0; index < value && selectedDoubleValue > 1.0; index++) {
      ...
      int currentValue = Math.min(8, (int) Math.ceil(Math.min(selectedDoubleValue, 48.0) / 6.0));
      currentValue = Math.max(1, currentValue);

      for (int currentIndex = 0;
          currentIndex < currentValue && selectedDoubleValue > 1.0E-9;
          currentIndex++) {
        double defaultDoubleValue = Math.min(6.0, selectedDoubleValue);
        double initialDoubleValue =
            Math.signum(inputDoubleValue) * Math.min(0.28, Math.abs(inputDoubleValue));
        ...
        arrayList.add(
            new MovementBurstScheduleService.SchedClaim(
                byteValue, outputDoubleValue, resultDoubleValue, candidateDoubleValue));
        byteValue += 20;
      }
