...
public final class MovementAvailableService {
   ...
   public static boolean forward(Minecraft minecraft) {
      return AutoWalkModule.isActive() && available(minecraft) && !minecraft.options.keyDown.isDown() && !minecraft.options.keyShift.isDown();
   }
   ...
   public static boolean jump(Minecraft minecraft) {
      if (AutoJumpModule.isActive() && available(minecraft)) {
         ...
         return localPlayer.onGround()
            ...
            && (forward(minecraft) || currentOptions.keyUp.isDown() || currentOptions.keyDown.isDown() || currentOptions.keyLeft.isDown() || currentOptions.keyRight.isDown());
      } else {
         return false;
      }
   }
   ...
   public static boolean requested(KeyMapping keyMapping) {
      ...
      if (keyMapping.isDown()) {
         ...
         return minecraft == null
            ? false
            : keyMapping == minecraft.options.keyUp && forward(minecraft) || keyMapping == minecraft.options.keyJump && (jump(minecraft) || VelocityActivateService.jumpRequested());
      }
   }
}
