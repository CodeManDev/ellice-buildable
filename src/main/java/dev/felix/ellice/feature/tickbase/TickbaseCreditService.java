...
public final class TickbaseCreditService {
  ...
  public boolean beforeTick(
      long longValue, int value, TickbaseCreditService.Environment environment) {
  ...
  public boolean beforeTick(
      long longValue,
      int value,
      TickbaseCreditService.Mode mode,
      TickbaseCreditService.Environment environment) {
    if (this.enabled2) {
      return false;
    }
    ...
    if (this.count2 == 0) {
      ...
      this.count2 =
          Math.clamp(environment.plan(Math.clamp(value, 1, 8)), 0, Math.clamp(value, 1, 8));
      if (this.count2 == 0) {
        return false;
      }
    }
  ...
  public void afterTick(
      long longValue, long currentLongValue, TickbaseCreditService.Environment environment) {
    if (!this.enabled2 && this.enabled) {
      ...
      try {
        ...
        while (this.count > 0
            && this.timestamp2 == nextLongValue
            && environment.valid()
            && environment.continueShift()) {
          this.count--;
          this.timestamp4++;
  ...
  private void updateState(
      TickbaseCreditService.Mode currentMode, TickbaseCreditService.Environment environment) {
