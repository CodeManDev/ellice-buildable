...
public final class RotationPublishService {
  ...
  public static void publish(RotationData rotationData, RotationData currentRotationData) {
    ...
    snapshot2 =
        new RotationPublishService.Snapshot(
            nextRotationData, currentRotationData, doubleValue, nextDoubleValue);
  }
  ...
  public static void publishReturning(
    ...
    double nextDoubleValue =
        currentDoubleValue + RotationData.yawDelta(currentDoubleValue, doubleValue) * 0.45;
    double previousDoubleValue = RotationData.yawDelta(nextDoubleValue, currentRotationData.yaw());
    ...
    if (Math.abs(previousDoubleValue) > targetDoubleValue) {
      nextDoubleValue +=
          previousDoubleValue - Math.copySign(targetDoubleValue, previousDoubleValue);
    }

    snapshot2 =
        new RotationPublishService.Snapshot(
            nextRotationData, currentRotationData, currentDoubleValue, nextDoubleValue);
  }
  ...
  public record Snapshot(
      RotationData previous, RotationData current, double previousBodyYaw, double bodyYaw) {}
}
