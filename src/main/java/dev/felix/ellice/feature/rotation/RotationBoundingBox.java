...
public record RotationBoundingBox(double minX, double minY, double minZ, double maxX, double maxY, double maxZ) {
   private static final double EPSILON = 1.0E-12;
   private static final double[] HORIZONTAL_AIM_FRACTIONS = new double[]{
      0.08,
      0.29,
      0.5,
      0.71,
   ...
   private static final double[] VERTICAL_AIM_FRACTIONS = new double[]{
      0.06,
      0.2,
      0.35,
      0.5,
   ...
   public RotationBoundingBox(double minX, double minY, double minZ, double maxX, double maxY, double maxZ) {
   ...
   public RotationVector sample(double doubleValue, double currentDoubleValue, double nextDoubleValue) {
      return new RotationVector(
         calculateValue(this.minX, this.maxX, doubleValue), calculateValue(this.minY, this.maxY, currentDoubleValue), calculateValue(this.minZ, this.maxZ, nextDoubleValue)
      );
   }

   public List<RotationVector> aimSamples() {
      ArrayList currentLength = new ArrayList(HORIZONTAL_AIM_FRACTIONS.length * VERTICAL_AIM_FRACTIONS.length * HORIZONTAL_AIM_FRACTIONS.length);
   ...
   public double visibleAngularWidth(RotationVector rotationVector, RotationVector currentRotationVector, List<RotationVector> items) {
      if (this.contains(currentRotationVector) && !items.isEmpty() && !items.stream().anyMatch(item -> !this.contains(item))) {
         ...
         for (RotationVector nextRotationVector : (Iterable<RotationVector>) (Iterable<?>) (currentItems)) {
            RotationData currentRotationData = RotationData.lookAt(rotationVector, nextRotationVector);
            double sourceDoubleValue = RotationData.yawDelta(rotationData.yaw(), currentRotationData.yaw());
         ...
         RotationVector previousRotationVector = this.sample(
            0.5,
            0.5,
            0.5
         );
         double currentLength = Math.max(0.01, previousRotationVector.subtract(rotationVector).length());
         ...
         double nextLength = currentDoubleValue - doubleValue + resultDoubleValue / HORIZONTAL_AIM_FRACTIONS.length;
         double previousLength = previousDoubleValue - nextDoubleValue + candidateDoubleValue / VERTICAL_AIM_FRACTIONS.length;
         double selectedDoubleValue = Math.min(Math.min(resultDoubleValue, nextLength), Math.min(candidateDoubleValue, previousLength));
   ...
   private List<RotationVector> collectValues(RotationVector rotationVector, List<RotationVector> items) {
      int index = 0;
      ...
      for (int currentIndex = 1; currentIndex < items.size(); currentIndex++) {
         double currentDoubleValue = ((RotationVector)items.get(currentIndex)).subtract(rotationVector).lengthSquared();
      ...
      while (nextIndex < previousIndex) {
         ...
         for (int sourceIndex = 0; sourceIndex < items.size(); sourceIndex++) {
            if (!currentSize[sourceIndex] && this.checkCondition(currentRotationVector, (RotationVector)items.get(sourceIndex))) {
   ...
   private boolean checkCondition(RotationVector rotationVector, RotationVector currentRotationVector) {
      ...
      double previousDoubleValue = Math.abs(rotationVector.z() - currentRotationVector.z()) / nextDoubleValue;
   ...
   public OptionalDouble rayIntersection(RotationVector rotationVector, RotationVector currentRotationVector, double doubleValue) {
      if (Double.isFinite(doubleValue) && !(doubleValue < 0.0)) {
         ...
         double[] nextX = new double[]{currentRotationVector.x(), currentRotationVector.y(), currentRotationVector.z()};
         ...
         for (int index = 0; index < 3; index++) {
            ...
            if (Math.abs(previousDoubleValue) < 1.0E-12) {
               ...
               double targetDoubleValue = (currentDoubles[index] - currentX[index]) / previousDoubleValue;
         ...
         return currentDoubleValue <= doubleValue ? OptionalDouble.of(currentDoubleValue) : OptionalDouble.empty();
      } else {
   ...
   private static double calculateValue2(double doubleValue, double currentDoubleValue) {
      return Math.toDegrees(
         2.0 * Math.atan2(doubleValue * 0.5, currentDoubleValue)
      );
   }
}
