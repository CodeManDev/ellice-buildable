...
public final class RotationSynchronizeService {
  ...
  public RotationVanillaGcdService.QuantizedRotation next(
    ...
    return this.next(
        rotationData, currentRotationData, doubleValue, currentDoubleValue, nextDoubleValue, true);
  }
  ...
  public RotationVanillaGcdService.QuantizedRotation next(
    ...
    double targetDoubleValue =
        calculateValue(
            ...
            nextDoubleValue,
            enabled);
    RotationVanillaGcdService.QuantizedRotation quantizedRotation =
        this.rotationVanillaGcdService.quantize(
            rotationData,
            new RotationData(
                    rotationData.yaw() + sourceDoubleValue,
                    rotationData.pitch() + targetDoubleValue)
                .withPitchClamped(),
            doubleValue,
            currentDoubleValue);
  ...
  private static double calculateValue(
    ...
    double targetDoubleValue =
        Math.clamp(currentDoubleValue, -previousDoubleValue, previousDoubleValue);
    if (enabled) {
      double inputDoubleValue =
          (Math.sqrt(
                      sourceDoubleValue * sourceDoubleValue
                          + 8.0 * sourceDoubleValue * Math.abs(currentDoubleValue))
                  - sourceDoubleValue)
              * 0.5;
      targetDoubleValue =
          Math.copySign(Math.min(Math.abs(targetDoubleValue), inputDoubleValue), targetDoubleValue);
    }

    double outputDoubleValue =
        doubleValue
            + Math.clamp(targetDoubleValue - doubleValue, -sourceDoubleValue, sourceDoubleValue);
    double resultDoubleValue = Math.min(Math.abs(currentDoubleValue), previousDoubleValue);
    return currentDoubleValue >= 0.0
        ? Math.clamp(outputDoubleValue, 0.0, resultDoubleValue)
        : Math.clamp(outputDoubleValue, -resultDoubleValue, 0.0);
  }
}
