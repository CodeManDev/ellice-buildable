...
public final class RotationVanillaGcdService {
   ...
   public RotationVanillaGcdService.QuantizedRotation quantize(RotationData rotationData, RotationData currentRotationData, double doubleValue, double currentDoubleValue) {
      ...
      RotationData nextRotationData = value != 0 ? new RotationData((float)rotationData.yaw(), (float)rotationData.pitch()) : rotationData;
      long longValue = Math.round(RotationData.yawDelta(nextRotationData.yaw(), currentRotationData.yaw()) / previousDoubleValue);
      long currentLongValue = Math.round((currentRotationData.pitch() - nextRotationData.pitch()) / previousDoubleValue);
      ...
      if (value != 0) {
         ...
         float sourceValue = Math.max(-90.0F, Math.min(90.0F, (float)nextRotationData.pitch() + nextValue));
         previousRotationData = new RotationData(previousValue, sourceValue);
      } else {
         currentLongValue = calculateValue2(nextRotationData.pitch(), currentLongValue, previousDoubleValue);
         previousRotationData = new RotationData(nextRotationData.yaw() + longValue * previousDoubleValue, nextRotationData.pitch() + currentLongValue * previousDoubleValue);
      }

      return new RotationVanillaGcdService.QuantizedRotation(previousRotationData, nextDoubleValue, previousDoubleValue, longValue, currentLongValue);
   }

   private static long calculateValue2(double doubleValue, long longValue, double currentDoubleValue) {
   ...
   public record QuantizedRotation(RotationData rotation, double vanillaGcd, double effectiveGcd, long yawMouseCounts, long pitchMouseCounts) {
   }
}
