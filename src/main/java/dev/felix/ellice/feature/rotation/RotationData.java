...
public record RotationData(double yaw, double pitch) {
   ...
   public static RotationData lookAt(RotationVector rotationVector, RotationVector currentRotationVector) {
      ...
      double nextX = Math.toDegrees(Math.atan2(nextRotationVector.z(), nextRotationVector.x())) - 90.0;
   ...
   public static RotationData interpolate(RotationData rotationData, RotationData currentRotationData, double doubleValue) {
      double currentDoubleValue = Math.max(0.0, Math.min(1.0, doubleValue));
      return new RotationData(rotationData.yaw + yawDelta(rotationData.yaw, currentRotationData.yaw) * currentDoubleValue, rotationData.pitch + (currentRotationData.pitch - rotationData.pitch) * currentDoubleValue);
   }

   public static double distance(RotationData rotationData, RotationData currentRotationData) {
      return Math.hypot(yawDelta(rotationData.yaw, currentRotationData.yaw), currentRotationData.pitch - rotationData.pitch);
   }
