...
public final class NametagsValueService extends ScenePctService<NametagsValueService> {
   ...
   public NametagsValueService value(float currentValue, float nextValue, boolean enabled) {
      ...
      if (previousValue == 0 && enabled) {
         if (currentValue != this.value4) {
            MotionAnimateService.animate(this, HEALTH, currentValue, SceneEaseHandler.Tween.ease(0.18F));
         }

         if (nextValue != this.value5) {
            MotionAnimateService.animate(this, ABSORPTION, nextValue, SceneEaseHandler.Tween.ease(0.18F));
         }
      } else {
   ...
   protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      ...
      int value = MaterialIsLightService.layer(MaterialIsLightService.SURFACE_HIGHEST, MaterialIsLightService.ON_SURFACE, 0.18F);
      compositorPushPresentationScale.roundedRect(this.cx, this.cy, this.cw, this.ch, height, mulAlpha(value, this.effectiveOpacity));
      ...
      if (width > 0.1F) {
         int targetValue = this.value2 < 0.5F
         ...
         compositorPushPresentationScale.roundedRect(this.cx, nextValue, width, previousValue, sourceValue, mulAlpha(targetValue, this.effectiveOpacity));
         ...
         compositorPushPresentationScale.roundedRect(
            ...
            inputValue / 2.0F,
            mulAlpha(
               ThemeMixService.lighten(targetValue, 0.22F), this.effectiveOpacity * 0.8F
            )
         );
      }
      ...
      if (outputValue > 0.1F) {
         ...
         compositorPushPresentationScale.roundedRect(this.cx + width, nextValue, outputValue, previousValue, sourceValue, mulAlpha(resultValue, this.effectiveOpacity));
         ...
         compositorPushPresentationScale.roundedRect(
            ...
            candidateValue / 2.0F,
            mulAlpha(
               ThemeMixService.lighten(resultValue, 0.18F), this.effectiveOpacity * 0.8F
            )
         );
         if (width > 0.1F) {
            compositorPushPresentationScale.roundedRect(
