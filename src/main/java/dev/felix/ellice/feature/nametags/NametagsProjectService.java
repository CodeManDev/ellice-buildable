...
public final class NametagsProjectService {
  ...
  public static NametagsProjectService.Point project(
      double doubleValue,
      double currentDoubleValue,
      double nextDoubleValue,
      Matrix4fc matrix4fc,
      Matrix4fc currentMatrix4fc,
      float value,
      float currentValue) {
    if (value > 0.0F && currentValue > 0.0F) {
      Vector4f vector4f =
          new Vector4f(
              (float) doubleValue, (float) currentDoubleValue, (float) nextDoubleValue, 1.0F);
      ...
      if (vector4f.isFinite() && !(vector4f.w <= 0.01F)) {
        ...
        return !(nextValue < -1.0F)
            ...
            ? new NametagsProjectService.Point(
                (nextValue * 0.5F + 0.5F) * value, (0.5F - previousValue * 0.5F) * currentValue)
            : null;
      } else {
        return null;
      }
  ...
  public static NametagsProjectService.Bounds bounds(
    ...
    return point != null
        ...
        ? new NametagsProjectService.Bounds(
            ...
            Math.clamp(point.y - currentValue - 5.0F, 4.0F, previousValue - currentValue - 4.0F),
            value,
            currentValue)
        : null;
  }
