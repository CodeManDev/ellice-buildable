...
public final class NametagsRenderer implements ComponentOperationHandler {
   ...
   public boolean update(NametagsData currentNametagsData, float value, boolean currentEnabled) {
      int currentValue = Objects.equals(this.nametagsData, currentNametagsData) && this.value2 == value && this.enabled == currentEnabled ? 0 : 1;
   ...
   public ComponentKeyService<?> render(ComponentThemeService componentTheme) {
      ...
      String currentText = light ? TextCodec.normalizeWhite(currentNametagsData.name()) : currentNametagsData.name();
      String nextText = light ? TextCodec.normalizeWhite(currentNametagsData.detail()) : currentNametagsData.detail();
      ArrayList arrayList = new ArrayList();
      if (currentNametagsData.item()) {
         arrayList.add(
            ComponentBoxService.<NametagsItemService>node(
                  "nametag-icon", NametagsItemService::new, currentItem -> currentItem.id("nametag-icon").item(currentNametagsData.icon(), currentNametagsData.accent())
               )
               .style(
                  ComponentStyleService.style()
                     .size(
                        LayoutOperationHandler.px(18.0F * currentWidth), LayoutOperationHandler.px(18.0F * currentWidth)
                     )
                     .shrink(0.0F)
                     .build()
               )
      ...
      arrayList.add(
         ComponentBoxService.text(
               currentText,
               currentItem -> currentItem.id("nametag-name")
                  ...
                  .textAlign(!currentNametagsData.item() && currentNametagsData.detail().isEmpty() ? SceneTextService.TextAlign.CENTER : SceneTextService.TextAlign.LEFT)
            )
            .style(
               ComponentStyleService.style()
      ...
      if (!nextText.isEmpty()) {
         arrayList.add(
            ComponentBoxService.text(
                  nextText,
                  currentItem -> currentItem.id("nametag-detail")
                     .fontSize(8.5F * currentWidth)
                     .lineHeight(20.0F * currentWidth)
                     .fontFamily("material-roboto")
                     .color(currentNametagsData.item() ? currentNametagsData.accent() : MaterialIsLightService.ON_SURFACE_VARIANT)
               )
               .style(ComponentStyleService.style().height(LayoutOperationHandler.px(20.0F * currentWidth)).shrink(0.0F).build())
               .key("detail")
         );
      }
      ...
      if (currentNametagsData.health() >= 0.0F) {
         ...
         currentArrayList.add(
            ComponentBoxService.<NametagsValueService>node(
                  "nametag-health", NametagsValueService::new, item -> item.id("nametag-health").value(currentNametagsData.health(), currentNametagsData.absorption(), currentEnabled)
               )
               .style(
                  ComponentStyleService.style()
                     .width(LayoutOperationHandler.percent(100.0F))
      ...
      return ComponentBoxService.panel(
         ...
         .onMount(item -> {
            if (nextEnabled) {
               ...
               MotionAnimateService.animate(item, MotionColorsContainer.Floats.OPACITY, 1.0F, SceneEaseHandler.Tween.ease(0.14F));
               MotionAnimateService.animate(item, MotionColorsContainer.Floats.TRANSLATE_Y, 0.0F, SceneEaseHandler.Tween.ease(0.14F));
            }
         });
   }
   ...
   public static float width(CompositorPushPresentationScaleService compositorPushPresentationScale, NametagsData nametagsData, float value, float currentValue) {
      float nextValue = compositorPushPresentationScale.textWidth(nametagsData.name(), 10.5F * value, TextMode.REGULAR, "material-roboto-medium")
         + (
            nametagsData.detail().isEmpty()
               ? 0.0F
               : compositorPushPresentationScale.textWidth(nametagsData.detail(), 8.5F * value, TextMode.REGULAR, "material-roboto")
                  + 6.0F * value
         )
