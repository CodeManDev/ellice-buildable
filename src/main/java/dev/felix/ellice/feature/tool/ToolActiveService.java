...
public final class ToolActiveService {
   ...
   public void begin(ToolActiveService.Hand currentHand, int value, boolean currentEnabled, boolean nextEnabled) {
      if (value >= 0 && value < 9) {
         this.release(true);
   ...
   public boolean mine(Supplier<Boolean> supplier) {
      ...
      try {
         boolean enabled = this.createValue(() -> {
            this.hand.sync();
            return (Boolean)supplier.get();
         });
         if (!enabled) {
   ...
   public void release(boolean currentEnabled) {
      ...
      if (currentHand != null && !this.enabled3) {
         ...
         try {
            ...
            this.enabled3 = false;
            if (currentHand.valid()) {
               if (!this.enabled && this.enabled2 && nextEnabled && currentHand.selected() == this.count) {
