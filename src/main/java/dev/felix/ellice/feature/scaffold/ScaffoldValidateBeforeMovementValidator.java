...
public final class ScaffoldValidateBeforeMovementValidator {
  ...
  public static ScaffoldValidateBeforeMovementValidator.PrepareCheck validateBeforeMovement(
      ScaffoldValidateBeforeMovementValidator.Arm currentArm,
      long longValue,
    ...
    if (longValue != currentArm.tick()) {
      ...
      return evaluation != null && evaluation.ready()
          ? new ScaffoldValidateBeforeMovementValidator.PrepareCheck(
              true, ScaffoldValidateBeforeMovementValidator.Failure.NONE)
          : createPrepareCheck(ScaffoldValidateBeforeMovementValidator.Failure.UNSAFE_RAY_ENVELOPE);
    }
  }

  public static ScaffoldValidateBeforeMovementValidator.UseDisposition classifyUse(
    ...
    if (value >= 0) {
      ...
      return enabled
          ...
          : ScaffoldValidateBeforeMovementValidator.UseDisposition.NOT_ISSUED;
    }
  }

  public static boolean followingMovementMatches(
      ScaffoldValidateBeforeMovementValidator.Arm currentArm,
      long longValue,
      RotationData rotationData) {
    Objects.requireNonNull(currentArm, "arm");
    return longValue == currentArm.precedingMovementOrdinal() + 1L
        && sameSentRotation(currentArm.wireRotation(), rotationData);
  }

  public static boolean sameSentRotation(
      RotationData rotationData, RotationData currentRotationData) {
    return rotationData != null && currentRotationData != null
        ? Float.floatToIntBits((float) rotationData.yaw())
                == Float.floatToIntBits((float) currentRotationData.yaw())
            && Float.floatToIntBits((float) rotationData.pitch())
                == Float.floatToIntBits((float) currentRotationData.pitch())
        : false;
  }

  private static ScaffoldValidateBeforeMovementValidator.PrepareCheck createPrepareCheck(
  ...
  public enum Failure {
    ...
    private static ScaffoldValidateBeforeMovementValidator.Failure[] $values() {
      return new ScaffoldValidateBeforeMovementValidator.Failure[] {
        NONE, STALE_TICK, MOVEMENT_RACED_USE, TARGET_CHANGED, ROTATION_CHANGED, UNSAFE_RAY_ENVELOPE
      };
    }
  }

  public record PrepareCheck(
      boolean ready, ScaffoldValidateBeforeMovementValidator.Failure failure) {
  ...
  public enum UseDisposition {
    ...
    private static ScaffoldValidateBeforeMovementValidator.UseDisposition[] $values() {
      return new ScaffoldValidateBeforeMovementValidator.UseDisposition[] {
        ISSUED, NOT_ISSUED, AMBIGUOUS_NATIVE_RESULT
      };
    }
  }
}
