...
public record ScaffoldData(
  ...
  public boolean acceptsPitch(double doubleValue) {
    return !this.autoPitch
        || doubleValue >= this.minimumPitch - 1.0E-5 && doubleValue <= this.maximumPitch + 1.0E-5;
  }
  ...
  public double yawOffset(double doubleValue, RotationVector rotationVector) {
    return switch (this.angleMode) {
      ...
      case "Auto Diagonal" -> {
        if (Math.abs(Math.sin(Math.toRadians(doubleValue * 2.0))) > 0.1) {
          yield 0.0;
        } else {
          RotationVector currentRotationVector =
              new RotationData(doubleValue + 135.0, 0.0).direction();
          RotationVector nextRotationVector =
              new RotationData(doubleValue + 225.0, 0.0).direction();
          ...
          yield currentRotationVector.x() * currentX
                      + currentRotationVector.z() * currentDoubleValue
                  >= nextRotationVector.x() * currentX + nextRotationVector.z() * currentDoubleValue
              ? -45.0
              : 45.0;
        }
      }
