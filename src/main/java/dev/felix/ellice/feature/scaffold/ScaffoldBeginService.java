...
public final class ScaffoldBeginService {
   ...
   public static ScaffoldBeginService.State begin(long longValue, int value, ScaffoldDecisionTracker.TargetKey targetKey, long currentLongValue, int currentValue, int nextValue) {
      if (currentLongValue >= 0L && currentValue >= 1 && nextValue >= 1) {
         ...
         return new ScaffoldBeginService.State(new ScaffoldBeginService.Key(longValue, value, targetKey), ScaffoldBeginService.Phase.GRACE, nextLongValue, Long.MIN_VALUE);
      } else {
   ...
   public static ScaffoldBeginService.Transition advance(
      ScaffoldBeginService.State currentState, long longValue, ScaffoldBeginService.Evidence evidence, ScaffoldBeginService.TargetState currentTargetState
   ) {
      ...
      if (currentState.phase() == ScaffoldBeginService.Phase.GRACE) {
         ...
         ScaffoldBeginService.State nextState = new ScaffoldBeginService.State(
            currentState.key(), ScaffoldBeginService.Phase.PREDICTION_RESOLUTION_REQUESTED, currentState.reconcileNotBeforeTick(), longValue
         );
         return new ScaffoldBeginService.Transition(nextState, ScaffoldBeginService.Action.REQUEST_PREDICTION_RESOLUTION);
      } else {
         ...
         return switch (currentTargetState) {
            ...
            case REPLACEABLE, OTHER -> createTransition(currentState, ScaffoldBeginService.Action.TERMINAL_ABORT);
         };
      }
   }
   ...
   public static boolean matchesTarget(ScaffoldBeginService.State state, ScaffoldDecisionTracker.TargetKey targetKey) {
   ...
   public static ScaffoldBeginService.State retire(ScaffoldBeginService.State currentState) {
      Objects.requireNonNull(currentState, "state");
      return new ScaffoldBeginService.State(currentState.key(), ScaffoldBeginService.Phase.RETIRED, currentState.reconcileNotBeforeTick(), currentState.resolutionRequestedTick());
   }

   public static ScaffoldBeginService.Transition terminalAbort(ScaffoldBeginService.State currentState) {
      Objects.requireNonNull(currentState, "state");
   ...
   private static ScaffoldBeginService.Transition createTransition(ScaffoldBeginService.State state, ScaffoldBeginService.Action action) {
   ...
   public enum Action {
      ...
      private static ScaffoldBeginService.Action[] $values() {
         return new ScaffoldBeginService.Action[]{NONE, REQUEST_PREDICTION_RESOLUTION, ACCEPT, REJECT, TERMINAL_ABORT};
      }
   }
   ...
   public record State(ScaffoldBeginService.Key key, ScaffoldBeginService.Phase phase, long reconcileNotBeforeTick, long resolutionRequestedTick) {
      public State(ScaffoldBeginService.Key key, ScaffoldBeginService.Phase phase, long reconcileNotBeforeTick, long resolutionRequestedTick) {
         ...
         if (phase == ScaffoldBeginService.Phase.PREDICTION_RESOLUTION_REQUESTED && resolutionRequestedTick < 0L) {
