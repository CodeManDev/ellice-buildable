...
public final class ScaffoldLandingService {
    ...
    if (!scaffoldPlayerSnapshot.onGround()
      ...
      for (int index = 1; index <= 40; index++) {
        RotationVector currentX =
            rotationVector.add(
                new RotationVector(
                    currentRotationVector.x() + worldVector.x() * doubleValue,
                    currentRotationVector.y(),
                    currentRotationVector.z() + worldVector.z() * doubleValue));
        if (currentRotationVector.y() < 0.0 && currentX.y() <= value + 1) {
          return Optional.of(
              new ScaffoldLandingService.Landing(
                  rotationVector,
                  new RotationVector(currentX.x(), value + 1, currentX.z()),
                  index));
        }
  ...
  public static List<BlockCoordinates> cells(
      RotationVector rotationVector, RotationVector currentRotationVector, int value) {
    if (Math.hypot(
            currentRotationVector.x() - rotationVector.x(),
            currentRotationVector.z() - rotationVector.z())
        > 10.0) {
      return List.of();
    }
    ...
    double currentDoubleValue =
        previousValue == 0 ? Double.POSITIVE_INFINITY : 1.0 / Math.abs(previousX);
    double nextDoubleValue =
        sourceValue == 0 ? Double.POSITIVE_INFINITY : 1.0 / Math.abs(doubleValue);
    double sourceX =
        previousValue == 0
            ? Double.POSITIVE_INFINITY
            : ((previousValue > 0 ? currentX + 1 : currentX) - rotationVector.x()) / previousX;
    double previousDoubleValue =
        sourceValue == 0
            ? Double.POSITIVE_INFINITY
            : ((sourceValue > 0 ? currentValue + 1 : currentValue) - rotationVector.z())
                / doubleValue;
      ...
      if (query.isReplaceable(blockCoordinates)) {
        ...
        for (ScaffoldMode currentScaffoldMode : scaffoldMode) {
          ...
          if (!query.isReplaceable(currentBlockCoordinates)
            ...
            if (!(nextRotationVector.subtract(scaffoldPlayerSnapshot.eyePosition()).length()
                > doubleValue)) {
              double currentX =
                  scaffoldPlayerSnapshot
                      ...
                      .dot(
                          new RotationVector(
                              nextScaffoldMode.x(), nextScaffoldMode.y(), nextScaffoldMode.z()));
              if (!(currentX <= 1.0E-4)) {
                double nextDoubleValue =
                    RotationData.distance(
                        rotationData,
                        RotationData.lookAt(
                            scaffoldPlayerSnapshot.eyePosition(), nextRotationVector));
