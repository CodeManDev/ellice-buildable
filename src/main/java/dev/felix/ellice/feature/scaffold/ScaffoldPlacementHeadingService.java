...
public final class ScaffoldPlacementHeadingService {
   public static double placementHeading(ScaffoldPlayerSnapshot scaffoldPlayerSnapshot, ScaffoldRemapService.WorldVector worldVector, double doubleValue) {
      ...
      double currentDoubleValue = scaffoldPlayerSnapshot.velocity().z() + worldVector.z() * doubleValue;
   ...
   public static ScaffoldPlayerSnapshot nextFrame(ScaffoldPlayerSnapshot scaffoldPlayerSnapshot, ScaffoldRemapService.WorldVector worldVector, double doubleValue, boolean enabled) {
      return nextFrame(scaffoldPlayerSnapshot, worldVector, doubleValue, enabled, 0.41999998688697815);
   }

   public static ScaffoldPlayerSnapshot nextFrame(ScaffoldPlayerSnapshot scaffoldPlayerSnapshot, ScaffoldRemapService.WorldVector worldVector, double doubleValue, boolean enabled, double currentDoubleValue) {
      if (!Double.isFinite(doubleValue) || doubleValue < 0.0) {
         ...
         double currentY = scaffoldPlayerSnapshot.onGround() ? (enabled ? currentDoubleValue : 0.0) : scaffoldPlayerSnapshot.velocity().y();
         RotationVector currentX = scaffoldPlayerSnapshot.feetPosition()
            .add(new RotationVector(scaffoldPlayerSnapshot.velocity().x() + worldVector.x() * doubleValue, currentY, scaffoldPlayerSnapshot.velocity().z() + worldVector.z() * doubleValue));
         return new ScaffoldPlayerSnapshot(
            currentX.add(scaffoldPlayerSnapshot.eyePosition().subtract(scaffoldPlayerSnapshot.feetPosition())),
            currentX,
   ...
   public static ScaffoldPlayerSnapshot clipLanding(ScaffoldPlayerSnapshot scaffoldPlayerSnapshot, ScaffoldPlayerSnapshot currentScaffoldPlayerSnapshot, int width, Predicate<BlockCoordinates> predicate) {
      ...
      if (!scaffoldPlayerSnapshot.onGround()
         ...
         && ScaffoldHasContinuousSupportService.hasContinuousSupport(
            scaffoldPlayerSnapshot.feetPosition(), scaffoldPlayerSnapshot.feetPosition(), width, 1.0E-6, predicate::test
         )) {
         RotationVector currentY = new RotationVector(0.0, doubleValue - currentScaffoldPlayerSnapshot.feetPosition().y(), 0.0);
         return new ScaffoldPlayerSnapshot(
            currentScaffoldPlayerSnapshot.eyePosition().add(currentY),
            currentScaffoldPlayerSnapshot.feetPosition().add(currentY),
            new RotationVector(currentScaffoldPlayerSnapshot.velocity().x(), 0.0, currentScaffoldPlayerSnapshot.velocity().z()),
   ...
   public static RotationData turnToward(RotationData rotationData, RotationData currentRotationData, double doubleValue, double currentDoubleValue) {
      if (Double.isFinite(currentDoubleValue) && !(currentDoubleValue <= 0.0)) {
         ...
         double inputDoubleValue = targetDoubleValue > currentDoubleValue ? currentDoubleValue / targetDoubleValue : 1.0;
