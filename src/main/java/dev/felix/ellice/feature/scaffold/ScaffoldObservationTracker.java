...
public final class ScaffoldObservationTracker {
  private static final ScaffoldRemapService.WorldVector entries =
  ...
  private ScaffoldObservationTracker.Snapshot snapshot2 =
      new ScaffoldObservationTracker.Snapshot(
  ...
  public ScaffoldObservationTracker.Snapshot update(
      ScaffoldObservationTracker.Observation currentObservation) {
    ...
    int value =
        this.timestamp != Long.MIN_VALUE && currentObservation.tick() == this.timestamp + 1L
            ? 1
            : 0;
    this.timestamp = currentObservation.tick();
    ScaffoldRemapService.Input input = currentObservation.requestedInput();
    ScaffoldObservationTracker.InputSignature currentInputSignature =
        new ScaffoldObservationTracker.InputSignature(input.forwardAxis(), input.leftAxis());
    ScaffoldRemapService.WorldVector worldVector =
        createMap(currentObservation.cameraYaw(), currentInputSignature);
    double currentX =
        Math.hypot(currentObservation.velocity().x(), currentObservation.velocity().z());
    ScaffoldRemapService.WorldVector nextX =
        currentX < 1.0E-9
            ? entries
            : new ScaffoldRemapService.WorldVector(
                currentObservation.velocity().x() / currentX,
                currentObservation.velocity().z() / currentX);
    byte byteValue = 0;
    if (currentInputSignature.forward() == 0 && currentInputSignature.left() == 0) {
      this.count2++;
      this.updateState();
      this.phase2 =
          this.count2 >= this.policy2.idleDwellTicks() && currentX < this.policy2.movingSpeed()
    ...
    if (!(this.entries2.length() < 1.0E-9)
        ...
        && (this.phase2 != ScaffoldObservationTracker.Phase.COASTING
            || !(currentX < this.policy2.movingSpeed()))) {
      ...
      if (doubleValue <= this.policy2.turnDeadbandDegrees()) {
        ...
        if (value != 0
            ...
            && calculateValue(this.entries3, worldVector)
                <= this.policy2.candidateToleranceDegrees()) {
          this.count++;
        ...
        this.phase2 =
            doubleValue >= this.policy2.reversalDegrees()
                ? ScaffoldObservationTracker.Phase.REVERSING
                : ScaffoldObservationTracker.Phase.TURN_PENDING;
      ...
      this.phase2 =
          currentX < this.policy2.movingSpeed()
              ...
              : (this.checkCondition(nextX, worldVector, currentX)
                  ? ScaffoldObservationTracker.Phase.REVERSING
                  : ScaffoldObservationTracker.Phase.TRACKING);
      return this.createSnapshot(worldVector, nextX, currentX, (byteValue != 0));
    }
  }
  ...
  private ScaffoldObservationTracker.Snapshot createSnapshot(
      ScaffoldRemapService.WorldVector worldVector,
  ...
  private static ScaffoldRemapService.WorldVector createMap(
      double doubleValue, ScaffoldObservationTracker.InputSignature inputSignature) {
    return ScaffoldRemapService.worldVector(
        doubleValue, inputSignature.forward(), inputSignature.left());
  }
  ...
  private static double calculateValue(
    ...
    double doubleValue =
        Math.max(-1.0, Math.min(1.0, worldVector.dot(currentWorldVector) / currentLength));
  ...
  public record Observation(
      long tick,
      ScaffoldRemapService.Input requestedInput,
      double cameraYaw,
  ...
  public enum Phase {
    ...
    private static ScaffoldObservationTracker.Phase[] $values() {
      return new ScaffoldObservationTracker.Phase[] {
        IDLE, STARTING, TRACKING, TURN_PENDING, REVERSING, COASTING
      };
    }
  }
  ...
  public record Policy(
    ...
    public static ScaffoldObservationTracker.Policy defaults() {
      return new ScaffoldObservationTracker.Policy(12.0, 12.0, 120.0, 2, 0.01, 2);
    }
  }
  ...
  public record Snapshot(
    ...
    public boolean allowsStructuralPlanning() {
      return this.phase != ScaffoldObservationTracker.Phase.IDLE
          && this.phase != ScaffoldObservationTracker.Phase.COASTING;
    }

    public ScaffoldSweepsService pathEnvelope(double doubleValue, double currentDoubleValue) {
      return ScaffoldSweepsService.of(
          this.requestedDirection, doubleValue, this.momentumDirection, currentDoubleValue);
    }
  }
}
