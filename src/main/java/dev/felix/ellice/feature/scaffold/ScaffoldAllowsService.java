...
public final class ScaffoldAllowsService {
   ...
   public static boolean allows(
      ScaffoldPlayerSnapshot scaffoldPlayerSnapshot, PlacementCandidate placementCandidate, ScaffoldRemapService.WorldVector worldVector, double doubleValue, double currentDoubleValue, double nextDoubleValue, ContinuousSupportChecker.Query query
   ) {
      ...
      if (Double.isFinite(nextDoubleValue) && !(nextDoubleValue < 0.0)) {
         ...
         if (scaffoldMode != ScaffoldMode.UP && scaffoldMode != ScaffoldMode.DOWN) {
            ...
            if (!(Math.abs(scaffoldPlayerSnapshot.feetPosition().y() - (blockCoordinates.y() + 1.0)) > 1.0E-4)
               ...
               double previousX = (scaffoldPlayerSnapshot.eyePosition().x() - placementCandidate.hitPoint().x()) * scaffoldMode.x()
                  + (scaffoldPlayerSnapshot.eyePosition().z() - placementCandidate.hitPoint().z()) * scaffoldMode.z();
               ...
               RotationVector currentRotationVector = rotationVector.add(new RotationVector(currentX * sourceDoubleValue, 0.0, previousDoubleValue * sourceDoubleValue));
               return !ScaffoldHasContinuousSupportService.hasContinuousSupport(rotationVector, currentRotationVector, blockCoordinates.y(), doubleValue, query)
                  ? false
                  : PlacementRuleEvaluator.allows(scaffoldPlayerSnapshot, worldVector, blockCoordinates.y(), doubleValue, currentDoubleValue, nextDoubleValue, query);
            } else {
               return false;
            }
         } else {
