...
public final class ContinuousSupportChecker {
  ...
  public static boolean hasContinuousSupport(
      ...
      int value,
      ContinuousSupportChecker.Query currentQuery) {
    ...
    if (Double.isFinite(previousDoubleValue)
      ...
      for (int index = 0; index <= currentValue; index++) {
        ...
        for (double candidateDoubleValue : new double[] {-0.24, 0.0, 0.24}) {
          int currentX =
              (int)
                  Math.floor(
                      rotationVector.x()
                          + sourceDoubleValue * resultDoubleValue
                          + inputDoubleValue * candidateDoubleValue);
          int nextValue =
              (int)
                  Math.floor(
                      rotationVector.z()
                          + targetDoubleValue * resultDoubleValue
                          + outputDoubleValue * candidateDoubleValue);
