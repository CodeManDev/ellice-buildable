...
public final class ScaffoldDecisionTracker {
  ...
  private ScaffoldDecisionTracker.CognitivePhase cognitivePhase2 =
      ScaffoldDecisionTracker.CognitivePhase.IDLE;
  private ScaffoldDecisionTracker.AimPhase aimPhase2 = ScaffoldDecisionTracker.AimPhase.HOLD;
  private ScaffoldDecisionTracker.HandPhase handPhase2 =
      ScaffoldDecisionTracker.HandPhase.UNCHANGED;
  ...
  public ScaffoldDecisionTracker.Decision prepareTick(
      long longValue, ScaffoldDecisionTracker.Context context) {
    return this.createDecision(longValue, context);
  }

  private ScaffoldDecisionTracker.Decision createDecision(
      long size, ScaffoldDecisionTracker.Context currentContext) {
  ...
  public Optional<ScaffoldDecisionTracker.Decision> evaluatePlaceWindow(
      ScaffoldDecisionTracker.PlaceWindow placeWindow) {
    ...
    if (placeWindow.tick() == this.timestamp6
      ...
      boolean currentEnabled =
          this.checkCondition3(
              placeWindow.tick(), placeWindow.ray(), placeWindow.minimumFaceMargin());
  ...
  public void quarantinePendingTarget(
      ScaffoldDecisionTracker.TargetKey currentTargetKey, long longValue) {
  ...
  public boolean retireAttempt(
      long longValue,
      ScaffoldDecisionTracker.TargetKey currentTargetKey,
      ScaffoldDecisionTracker.AttemptOutcome attemptOutcome,
      long currentLongValue) {
    Objects.requireNonNull(attemptOutcome, "outcome");
    if (longValue != 0L
        && longValue == this.timestamp9
      ...
      this.handPhase2 =
          this.count >= 0
              ? ScaffoldDecisionTracker.HandPhase.READY
              : ScaffoldDecisionTracker.HandPhase.UNCHANGED;
      this.cognitivePhase2 = ScaffoldDecisionTracker.CognitivePhase.SEARCH;
      this.aimPhase2 =
          attemptOutcome == ScaffoldDecisionTracker.AttemptOutcome.ACCEPTED
              ? ScaffoldDecisionTracker.AimPhase.TRANSITION
              : ScaffoldDecisionTracker.AimPhase.HOLD;
      this.enabled = false;
      this.timestamp3 = 0L;
      this.timestamp4 = 0L;
      this.timestamp2 = Long.MIN_VALUE;
  ...
  private boolean checkCondition(ScaffoldDecisionTracker.Context context) {
    ...
    if (this.count != context.requiredSlot()
  ...
  private void updateState(
    ...
    if (currentTargetKey == null) {
      ...
      this.enabled =
          pathRelation.isPredictableChain() && checkCondition4(this.targetKey2, currentTargetKey);
      ...
      this.aimPhase2 =
          this.enabled
              ? ScaffoldDecisionTracker.AimPhase.TRANSITION
              : ScaffoldDecisionTracker.AimPhase.PRIMARY;
      this.timestamp2 = Long.MIN_VALUE;
      this.count3++;
    }
  }
  ...
  private boolean checkCondition3(
      long longValue, ScaffoldDecisionTracker.RaySample raySample, double doubleValue) {
    return raySample != null
        && raySample.sampledTick() == longValue
        && raySample.target().equals(this.targetKey)
        && raySample.edgeMargin() >= doubleValue;
  }
  ...
  private static boolean checkCondition4(
    ...
    if (targetKey != null
      ...
      int currentX =
          Math.abs(targetKey.targetPosition().x() - currentTargetKey.targetPosition().x());
  ...
  private ScaffoldDecisionTracker.Decision createDecision3(
      boolean enabled, boolean currentEnabled, boolean nextEnabled, long size) {
    return new ScaffoldDecisionTracker.Decision(
        this.cognitivePhase2,
        this.aimPhase2,
        this.handPhase2,
        this.targetKey,
        enabled,
  ...
  public enum CognitivePhase {
    ...
    private static ScaffoldDecisionTracker.CognitivePhase[] $values() {
      return new ScaffoldDecisionTracker.CognitivePhase[] {
        IDLE, SEARCH, COMMITTED, VERIFYING, WAIT_FEEDBACK, RECONCILING, RECOVER
      };
    }
  }
  ...
  public enum HandPhase {
    ...
    private static ScaffoldDecisionTracker.HandPhase[] $values() {
      return new ScaffoldDecisionTracker.HandPhase[] {
        UNCHANGED, SELECTING, SETTLING, READY, CLICK_ARMED, USE_COOLDOWN
      };
    }
  }
  ...
  public record RaySample(
      ScaffoldDecisionTracker.TargetKey target, double edgeMargin, long sampledTick) {
    public RaySample(
        ScaffoldDecisionTracker.TargetKey target, double edgeMargin, long sampledTick) {
  ...
  public record TargetKey(
      BlockCoordinates targetPosition, BlockCoordinates supportPosition, ScaffoldMode clickedFace) {
    public TargetKey(
        BlockCoordinates targetPosition,
        BlockCoordinates supportPosition,
        ScaffoldMode clickedFace) {
    ...
    public static ScaffoldDecisionTracker.TargetKey from(PlacementCandidate placementCandidate) {
      ...
      return new ScaffoldDecisionTracker.TargetKey(
