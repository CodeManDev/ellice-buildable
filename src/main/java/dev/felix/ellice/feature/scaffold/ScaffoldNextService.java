...
public final class ScaffoldNextService {
   ...
   public ScaffoldNextService.Step next(ScaffoldNextService.Request request, boolean currentEnabled) {
      ...
      if (request.tick() < this.timestamp) {
         this.updateState2();
         return this.step = this.createStep(request.current(), request.sensitivity(), ScaffoldNextService.Phase.REACTION);
      }
      ...
      int currentValue = request.rayInsideTarget() && request.rayMargin() >= (nextEnabled ? currentDoubleValue : doubleValue) ? 1 : 0;
      if (currentValue != 0) {
         this.enabled = true;
         this.count2 = 0;
         this.value8 = request.rayMargin();
         this.updateState2();
         return this.step = this.createStep(request.current(), request.sensitivity(), ScaffoldNextService.Phase.HOLD);
      }
      ...
      if (currentEnabled
         ...
         this.updateState2();
         return this.step = this.createStep(request.current(), request.sensitivity(), ScaffoldNextService.Phase.COAST);
      }
      ...
      RotationData rotationData = new RotationData(request.desired().yaw() + this.value6, request.desired().pitch() + this.value7)
         .withPitchClamped();
      ...
      ScaffoldNextService.AxisStep axisStep = this.createAxisStep(
         ...
         nextDoubleValue,
         nextDoubleValue
            * (0.3 + (1.0 - request.smoothness()) * 0.18)
      );
      ScaffoldNextService.AxisStep currentAxisStep = this.createAxisStep(
         ...
         nextDoubleValue
            * (0.24 + (1.0 - request.smoothness()) * 0.15)
      );
      double targetDoubleValue = Math.hypot(axisStep.velocity(), currentAxisStep.velocity());
      double inputDoubleValue = targetDoubleValue > nextDoubleValue ? nextDoubleValue / targetDoubleValue : 1.0;
      RotationData nextRotationData = new RotationData(
            request.current().yaw() + axisStep.velocity() * inputDoubleValue, request.current().pitch() + currentAxisStep.velocity() * inputDoubleValue
         )
         .withPitchClamped();
      ScaffoldNextService.CountPair countPair = this.createCountPair(request.current(), nextRotationData, currentRotationData, request.sensitivity(), nextDoubleValue);
      RotationData previousRotationData = applyMouseCounts(request.current(), countPair.yaw(), countPair.pitch(), request.sensitivity());
      ...
      double candidateDoubleValue = RotationData.yawDelta(request.current().yaw(), previousRotationData.yaw());
      ...
      return this.step = new ScaffoldNextService.Step(
   ...
   public static RotationData applyMouseCounts(RotationData rotationData, long longValue, long currentLongValue, double height) {
   ...
   private void updateState(ScaffoldNextService.Request request) {
      ...
      int currentValue = predictableChain
         ? (int)calculateValue7(request.situation().pathRelation().transitionDelayTicks(), 0L, request.maximumReactionTicks())
         : this.calculateValue4(request.situation().urgency(), request.minimumReactionTicks(), request.maximumReactionTicks());
      this.timestamp = request.tick() + currentValue;
      int nextValue = predictableChain && currentCount == 0 && request.situation().motionPhase() == ScaffoldMotionPhaseData.MotionPhase.GROUNDED ? 1 : 0;
      if (nextValue == 0) {
         this.value = 0.0;
         this.value2 = 0.0;
         this.value3 = 0.0;
      ...
      if (request.situation().suppressEndpointError()) {
         ...
         double doubleValue = Math.min(
            request.angularTargetWidth() * 0.06,
            0.045 + request.maximumTurnSpeed() * 0.0011
         );
   ...
   private ScaffoldNextService.AxisStep createAxisStep(double doubleValue, double currentDoubleValue, double nextDoubleValue, double previousDoubleValue, double sourceDoubleValue, double targetDoubleValue) {
      double inputDoubleValue = Math.signum(doubleValue);
      double outputDoubleValue = Math.sqrt(Math.max(0.0, 2.0 * targetDoubleValue * Math.abs(doubleValue)));
      ...
      double candidateDoubleValue = calculateValue6(resultDoubleValue - nextDoubleValue, -targetDoubleValue, targetDoubleValue);
      double selectedDoubleValue = Math.max(0.08, targetDoubleValue * 0.48);
      double defaultDoubleValue = calculateValue5(previousDoubleValue, candidateDoubleValue, selectedDoubleValue);
      double initialDoubleValue = calculateValue6(nextDoubleValue + defaultDoubleValue, -sourceDoubleValue, sourceDoubleValue);
   ...
   private ScaffoldNextService.CountPair createCountPair(RotationData rotationData, RotationData currentRotationData, RotationData nextRotationData, double doubleValue, double currentDoubleValue) {
      double nextDoubleValue = this.rotationVanillaGcdService.vanillaGcd(doubleValue);
      long size = Math.round(RotationData.yawDelta(rotationData.yaw(), currentRotationData.yaw()) / nextDoubleValue);
      long currentSize = Math.round((currentRotationData.pitch() - rotationData.pitch()) / nextDoubleValue);
      ...
      for (int index = 0; index < 6; index++) {
         ...
         for (int currentIndex = 0; currentIndex < 6; currentIndex++) {
            ...
            RotationData sourceRotationData = applyMouseCounts(rotationData, nextSize, previousSize, doubleValue);
            double inputDoubleValue = RotationData.yawDelta(rotationData.yaw(), sourceRotationData.yaw());
            double outputDoubleValue = sourceRotationData.pitch() - rotationData.pitch();
            if (checkCondition(sourceDoubleValue, inputDoubleValue) && checkCondition(targetDoubleValue, outputDoubleValue) && !(Math.hypot(inputDoubleValue, outputDoubleValue) > currentDoubleValue)) {
   ...
   private static double calculateValue(double doubleValue, double currentDoubleValue) {
      if (!checkCondition(currentDoubleValue, doubleValue)) {
         return doubleValue != 0.0 && currentDoubleValue != 0.0 && Math.signum(doubleValue) == Math.signum(currentDoubleValue) ? currentDoubleValue : 0.0;
      } else {
         return doubleValue;
      }
   }
   ...
   private static boolean checkCondition(double doubleValue, double currentDoubleValue) {
      return currentDoubleValue == 0.0 ? true : doubleValue != 0.0 && Math.signum(currentDoubleValue) == Math.signum(doubleValue) && Math.abs(currentDoubleValue) <= Math.abs(doubleValue);
   }
   ...
   private double calculateValue2(ScaffoldNextService.Request request) {
      ...
      return Math.min(
         ...
         Math.max(0.5, request.maximumTurnSpeed() * request.situation().turnSpeedScale() * doubleValue)
      );
   }

   private double calculateValue3(ScaffoldNextService.Request request) {
      double doubleValue = Math.min(
         0.08, request.situation().urgency() * 0.025
      );
      double currentDoubleValue = Math.min(
         0.08, this.value5 * 0.006
      );
      return request.minimumHoldMargin() + doubleValue + currentDoubleValue;
   }

   private ScaffoldNextService.Phase createPhase(ScaffoldNextService.Request request, RotationData rotationData) {
      if (request.situation().pathRelation() == ScaffoldMotionPhaseData.PathRelation.RECOVERY) {
         ...
         return RotationData.distance(request.current(), rotationData)
               <= Math.max(
                  0.6, request.angularTargetWidth() * 0.18
               )
   ...
   private ScaffoldNextService.Step createStep(RotationData rotationData, double doubleValue, ScaffoldNextService.Phase phase) {
   ...
   private int calculateValue4(double doubleValue, int value, int currentValue) {
      ...
      double currentDoubleValue = Math.exp(
         Math.log(1.75)
            + this.random.nextGaussian() * 0.24
      );
   ...
   private static double calculateValue5(double doubleValue, double currentDoubleValue, double nextDoubleValue) {
      return doubleValue + calculateValue6(currentDoubleValue - doubleValue, -nextDoubleValue, nextDoubleValue);
   }

   private static double calculateValue6(double doubleValue, double currentDoubleValue, double nextDoubleValue) {
   ...
   public enum Phase {
      ...
      private static ScaffoldNextService.Phase[] $values() {
         return new ScaffoldNextService.Phase[]{REACTION, PRIMARY, HOMING, COAST, HOLD, CHAIN_TRANSITION, RECOVERY};
      }
   }
   ...
   public record Request(
      ...
      public Request(
         ...
         this(longValue, rotationData, currentRotationData, value, doubleValue, currentDoubleValue, nextDoubleValue, previousDoubleValue, enabled, sourceDoubleValue, 1, 3, 0.075, scaffoldMotionPhaseData);
      }
