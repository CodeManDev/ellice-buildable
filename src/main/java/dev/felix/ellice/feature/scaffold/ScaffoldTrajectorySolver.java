...
public final class ScaffoldTrajectorySolver {
  ...
  public Optional<ScaffoldTrajectorySolver.Solution> solveKeepingYaw(
    ...
    if (scaffoldMode != ScaffoldMode.UP && scaffoldMode != ScaffoldMode.DOWN && value >= 1) {
      ...
      double currentX =
          currentRotationVector.x() * scaffoldMode.x()
              + currentRotationVector.z() * scaffoldMode.z();
      double nextX =
          (rotationVector.x() - placementCandidate.hitPoint().x()) * scaffoldMode.x()
              + (rotationVector.z() - placementCandidate.hitPoint().z()) * scaffoldMode.z();
      if (!(nextX <= 1.0E-6) && !(currentX >= -1.0E-6)) {
        double nextDoubleValue = -nextX / currentX;
        RotationVector nextRotationVector =
            rotationVector.add(currentRotationVector.multiply(nextDoubleValue));
        double previousX =
            scaffoldMode != ScaffoldMode.EAST && scaffoldMode != ScaffoldMode.WEST
        ...
        if (!(previousX < currentDoubleValue) && !(previousX > 1.0 - currentDoubleValue)) {
          ...
          double currentY =
              Math.toDegrees(
                  Math.atan2(
                      rotationVector.y()
                          - (placementCandidate.supportPosition().y() + 1 - currentDoubleValue),
                      nextDoubleValue));
          double nextY =
              Math.toDegrees(
                  Math.atan2(
                      rotationVector.y()
                          - (placementCandidate.supportPosition().y() + currentDoubleValue),
                      nextDoubleValue));
          long longValue =
              (long) Math.floor((currentY - rotationData.pitch()) / previousDoubleValue) - 1L;
          long currentLongValue =
              (long) Math.ceil((nextY - rotationData.pitch()) / previousDoubleValue) + 1L;
          long nextLongValue = Math.max(longValue, Math.min(currentLongValue, 0L));
          int index = 0;

          for (long currentIndex = 0L;
              currentIndex <= currentLongValue - longValue && index < value;
              currentIndex++) {
              ...
              if (previousLongValue >= longValue && previousLongValue <= currentLongValue) {
                RotationData currentRotationData =
                    ScaffoldNextService.applyMouseCounts(
                        rotationData, 0L, previousLongValue, doubleValue);
                double previousY =
                    rotationVector.y()
                        - Math.tan(Math.toRadians(currentRotationData.pitch())) * nextDoubleValue
                        - placementCandidate.supportPosition().y();
                if (!(previousY < currentDoubleValue) && !(previousY > 1.0 - currentDoubleValue)) {
                  PlacementCandidate currentPlacementCandidate =
                      placementCandidate.withSurfacePoint(previousX, previousY);
                  index++;
                  Optional result =
                      probe
                          .raycast(currentPlacementCandidate, currentRotationData)
                          .filter(
                              item ->
                                  checkCondition(placementCandidate, item)
                                      && !item.inside()
                                      && item.faceEdgeMargin() >= currentDoubleValue);
                  if (result.isPresent()) {
                    return Optional.of(
                        new ScaffoldTrajectorySolver.Solution(
                            currentRotationData,
                            currentPlacementCandidate,
                            (PlacementCandidate) result.get(),
                            0L,
                            previousLongValue,
                            -Math.abs(currentRotationData.pitch() - rotationData.pitch())));
                  }
  ...
  public Optional<ScaffoldTrajectorySolver.Solution> solveNearbyYaw(
    ...
    for (int index = 1; index <= nextValue && ints[0] < currentValue; index++) {
      for (int previousValue : new int[] {-1, 1}) {
        ...
        RotationData currentRotationData =
            ScaffoldNextService.applyMouseCounts(rotationData, sourceValue, 0L, doubleValue);
        Optional result =
            this.solveKeepingYaw(
                currentRotationData,
                rotationVector,
                placementCandidate,
                doubleValue,
                currentDoubleValue,
                Math.min(32, currentValue - ints[0]),
                currentProbe);
        if (result.isPresent()) {
          ScaffoldTrajectorySolver.Solution solution =
  ...
  public Optional<ScaffoldTrajectorySolver.Solution> solve(
    ...
    if (value >= 1
      ...
      double nextDoubleValue = Math.max(0.08, Math.min(0.32, currentDoubleValue + 0.035));
      ScaffoldTrajectorySolver.SurfacePoint surfacePoint = createSurfacePoint(placementCandidate);
      LinkedHashSet linkedHashSet =
          collectValues(rotationVector, placementCandidate, surfacePoint, currentDoubleValue);
      ...
      currentLinkedHashSet.add(new ScaffoldTrajectorySolver.SurfacePoint(0.5, 0.5));
      if (value > 1) {
          ...
          for (int currentIndex = 0; currentIndex < value; currentIndex++) {
            currentLinkedHashSet.add(
                new ScaffoldTrajectorySolver.SurfacePoint(
                    calculateValue5(nextDoubleValue, 1.0 - nextDoubleValue, index, value),
                    calculateValue5(nextDoubleValue, 1.0 - nextDoubleValue, currentIndex, value)));
          }
        }
      }
      ...
      arrayList.sort(
          Comparator.<ScaffoldTrajectorySolver.SurfacePoint>comparingInt(
                  item -> item.equals(surfacePoint) ? 0 : (linkedHashSet.contains(item) ? 1 : 2))
      ...
      for (ScaffoldTrajectorySolver.SurfacePoint currentSurfacePoint :
          (Iterable<ScaffoldTrajectorySolver.SurfacePoint>) (Iterable<?>) (arrayList)) {
        PlacementCandidate currentPlacementCandidate =
            placementCandidate.withSurfacePoint(
                currentSurfacePoint.first(), currentSurfacePoint.second());

        RotationData currentRotationData;
        try {
          currentRotationData =
              RotationData.lookAt(rotationVector, currentPlacementCandidate.hitPoint());
            ...
            new ScaffoldTrajectorySolver.AimPoint(
                ...
                Math.round(
                    RotationData.yawDelta(rotationData.yaw(), currentRotationData.yaw())
                        / previousDoubleValue),
                Math.round(
                    (currentRotationData.pitch() - rotationData.pitch()) / previousDoubleValue)));
      }

      ScaffoldTrajectorySolver.CachedProbe cachedProbe =
          new ScaffoldTrajectorySolver.CachedProbe(currentProbe, nextValue);
      ...
      for (int nextIndex = 0; nextIndex <= currentValue; nextIndex++) {
        ...
        for (ScaffoldTrajectorySolver.CountOffset countOffset : collectValues2(nextIndex)) {
          for (ScaffoldTrajectorySolver.AimPoint aimPoint :
              (Iterable<ScaffoldTrajectorySolver.AimPoint>) (Iterable<?>) (currentSize)) {
            ...
            Optional result =
                cachedProbe
                    .findResult(aimPoint.anchor(), nextRotationData)
            ...
            if (result.isPresent()) {
              ...
              double targetDoubleValue =
                  Math.min(nextPlacementCandidate.faceEdgeMargin(), currentDoubleValue + 0.16);
              double inputDoubleValue = aimPoint.point().equals(surfacePoint) ? 0.85 : 0.0;
              double outputDoubleValue =
                  targetDoubleValue * 10.0
              ...
              if (this.selectionPolicy
                  == ScaffoldTrajectorySolver.SelectionPolicy.MINIMUM_ROTATION) {
                outputDoubleValue =
                    -RotationData.distance(rotationData, nextRotationData)
  ...
  private static boolean checkCondition(
      PlacementCandidate placementCandidate, PlacementCandidate currentPlacementCandidate) {
  ...
  private static double calculateValue2(
      ScaffoldTrajectorySolver.SurfacePoint surfacePoint,
      ScaffoldTrajectorySolver.SurfacePoint currentSurfacePoint) {
  ...
  private static LinkedHashSet<ScaffoldTrajectorySolver.SurfacePoint> collectValues(
    ...
    if (scaffoldMode != ScaffoldMode.UP && scaffoldMode != ScaffoldMode.DOWN) {
      ...
      double currentX =
          (rotationVector.x() - currentRotationVector.x()) * scaffoldMode.x()
              + (rotationVector.z() - currentRotationVector.z()) * scaffoldMode.z();
      if (!(currentX <= 1.0E-6) && !(currentX >= 0.1)) {
        ...
        for (double previousDoubleValue : new double[] {nextDoubleValue, nextX}) {
          for (double sourceDoubleValue : new double[] {0.25, 0.5, 1.0}) {
            double targetDoubleValue = currentX * sourceDoubleValue;
            linkedHashSet.add(
                new ScaffoldTrajectorySolver.SurfacePoint(
                    calculateValue3(previousDoubleValue - targetDoubleValue, currentDoubleValue),
                    surfacePoint.second()));
            linkedHashSet.add(
                new ScaffoldTrajectorySolver.SurfacePoint(
                    calculateValue3(previousDoubleValue + targetDoubleValue, currentDoubleValue),
  ...
  private static ScaffoldTrajectorySolver.SurfacePoint createSurfacePoint(
      PlacementCandidate placementCandidate) {
    ...
    return switch (placementCandidate.clickedFace()) {
      case UP, DOWN ->
          new ScaffoldTrajectorySolver.SurfacePoint(
              calculateValue6(currentX), calculateValue6(height));
      case NORTH, SOUTH ->
          new ScaffoldTrajectorySolver.SurfacePoint(
              calculateValue6(currentX), calculateValue6(currentY));
      case WEST, EAST ->
          new ScaffoldTrajectorySolver.SurfacePoint(
              calculateValue6(height), calculateValue6(currentY));
    };
  }

  private static double calculateValue5(
      double doubleValue, double currentDoubleValue, int value, int currentValue) {
    return currentValue <= 1
        ? (doubleValue + currentDoubleValue) * 0.5
        : doubleValue + (currentDoubleValue - doubleValue) * value / (currentValue - 1.0);
  }
  ...
  private record AimPoint(
      ScaffoldTrajectorySolver.SurfacePoint point,
      PlacementCandidate anchor,
      long yawCenter,
      long pitchCenter) {}
  ...
  private static final class CachedProbe {
    ...
    private final Map<ScaffoldTrajectorySolver.RotationKey, Optional<PlacementCandidate>> entries =
        new HashMap<>();
    private int count2;
    ...
    private Optional<PlacementCandidate> findResult(
        PlacementCandidate placementCandidate, RotationData rotationData) {
      RotationData currentRotationData =
          new RotationData((float) rotationData.yaw(), (float) rotationData.pitch());
      ScaffoldTrajectorySolver.RotationKey rotationKey =
          ScaffoldTrajectorySolver.RotationKey.createRotationKey(currentRotationData);
      ...
      Optional currentResult =
          Objects.requireNonNull(
              this.probe2.raycast(placementCandidate, currentRotationData),
              "Native ray probe returned null");
  ...
  public interface Probe {
    Optional<PlacementCandidate> raycast(
        PlacementCandidate placementCandidate, RotationData rotationData);
  }

  private record RotationKey(int yawBits, int pitchBits) {
    private static ScaffoldTrajectorySolver.RotationKey createRotationKey(
        RotationData rotationData) {
      return new ScaffoldTrajectorySolver.RotationKey(
          Float.floatToIntBits((float) rotationData.yaw()),
          Float.floatToIntBits((float) rotationData.pitch()));
    }
  }
