...
public final class ScaffoldPlanService {
  ...
  private static final ScaffoldMode[] scaffoldMode =
      new ScaffoldMode[] {
        ScaffoldMode.DOWN,
        ScaffoldMode.NORTH,
        ScaffoldMode.SOUTH,
  ...
  public Optional<PlacementCandidate> plan(
    ...
    if (Double.isFinite(currentDoubleValue) && !(currentDoubleValue <= 0.0)) {
      ...
      for (Entry entry :
          createMap(scaffoldPlayerSnapshot, currentX, nextDoubleValue, doubleValue, value)
              .entrySet()) {
        ...
        if (query.isReplaceable(blockCoordinates)) {
          for (ScaffoldMode currentScaffoldMode : scaffoldMode) {
            ...
            if (!query.isReplaceable(currentBlockCoordinates)
                && query.isFaceSturdy(currentBlockCoordinates, nextScaffoldMode)) {
              RotationVector rotationVector =
                  currentBlockCoordinates.facePoint(nextScaffoldMode, 0.5, 0.5);
              double nextLength =
                  rotationVector.subtract(scaffoldPlayerSnapshot.eyePosition()).length();
              if (!(nextLength > currentDoubleValue)) {
                PlacementCandidate currentPlacementCandidate =
                    new PlacementCandidate(
                        blockCoordinates,
                        currentBlockCoordinates,
                        nextScaffoldMode,
                        rotationVector);
                if (currentPredicate.test(currentPlacementCandidate)
                    && canReachFace(
                        ...
                        new ScaffoldRemapService.WorldVector(currentX, nextDoubleValue),
                        query.groundFriction())) {
                  RotationData currentRotationData =
                      RotationData.lookAt(
                          ScaffoldMotionPhaseData.predictedEyeAtClick(
                              scaffoldPlayerSnapshot, currentPlacementCandidate, worldVector),
                          rotationVector);
                  double sourceDoubleValue =
                      RotationData.distance(rotationData, currentRotationData) + nextLength * 0.05;
                  if ((Integer) entry.getValue() < currentValue
                      || sourceDoubleValue < previousDoubleValue) {
    ...
    if (scaffoldMode != ScaffoldMode.UP && scaffoldMode != ScaffoldMode.DOWN) {
      ...
      if (!(currentDoubleValue > 1.0E-4) && !(nextX > 1.0E-4)) {
        double nextDoubleValue =
            Double.isFinite(doubleValue) && doubleValue >= 0.0 && doubleValue <= 1.0
                ? doubleValue
                : 1.0;
        double previousDoubleValue =
            Math.max(0.0, scaffoldPlayerSnapshot.velocity().dot(currentX))
                * ScaffoldCoastTicksService.coastTicks(
                    scaffoldPlayerSnapshot.onGround() ? nextDoubleValue : 1.0);
  ...
  private static LinkedHashMap<BlockCoordinates, Integer> createMap(
      ScaffoldPlayerSnapshot scaffoldPlayerSnapshot,
      double doubleValue,
      double currentDoubleValue,
      double nextDoubleValue,
    ...
    double inputDoubleValue =
        currentDoubleValue * targetDoubleValue + scaffoldPlayerSnapshot.velocity().z() * 1.5;
    int currentValue = 0;

    for (double outputDoubleValue : new double[] {0.0, -0.24, 0.24}) {
      double nextX =
          scaffoldPlayerSnapshot.feetPosition().x() + previousDoubleValue * outputDoubleValue;
      double resultDoubleValue =
          scaffoldPlayerSnapshot.feetPosition().z() + sourceDoubleValue * outputDoubleValue;
      currentValue =
          calculateValue3(
              nextX,
              resultDoubleValue,
              nextX + currentX,
              resultDoubleValue + inputDoubleValue,
              value,
              linkedHashMap,
              currentValue);
    }
    ...
    if (scaffoldPlayerSnapshot.velocity().y() < -0.02) {
      ...
      for (int index = previousX; index <= sourceX; index++) {
        for (int currentIndex = nextValue; currentIndex <= previousValue; currentIndex++) {
          currentValue =
              calculateValue4(
                  linkedHashMap, new BlockCoordinates(index, value, currentIndex), currentValue);
        }
      }
    }
    ...
    arrayList.sort(
        Comparator.<BlockCoordinates>comparingDouble(
                item ->
                    calculateValue(scaffoldPlayerSnapshot, item, doubleValue, currentDoubleValue))
            .thenComparingDouble(
                item ->
                    calculateValue2(
                        scaffoldPlayerSnapshot, item, doubleValue, currentDoubleValue)));
  ...
  private static int calculateValue3(
      ...
      double previousDoubleValue,
      int value,
      LinkedHashMap<BlockCoordinates, Integer> linkedHashMap,
      int currentValue) {
    ...
    currentValue =
        calculateValue4(
            linkedHashMap, new BlockCoordinates(nextValue, value, previousValue), currentValue);
    ...
    double inputDoubleValue =
        inputValue == 0 ? Double.POSITIVE_INFINITY : 1.0 / Math.abs(sourceDoubleValue);
    double outputDoubleValue =
        outputValue == 0 ? Double.POSITIVE_INFINITY : 1.0 / Math.abs(targetDoubleValue);
    ...
    double selectedDoubleValue =
        inputValue == 0
            ...
            : Math.abs((resultDoubleValue - doubleValue) / sourceDoubleValue);
    double defaultDoubleValue =
        outputValue == 0
            ? Double.POSITIVE_INFINITY
            : Math.abs((candidateDoubleValue - currentDoubleValue) / targetDoubleValue);
    ...
    for (int index = 0;
        ...
        currentValue =
            calculateValue4(
                linkedHashMap,
                new BlockCoordinates(nextValue, value, previousValue),
                currentValue)) {
      int resultValue =
          !(selectedDoubleValue < defaultDoubleValue)
                  && (!(Math.abs(selectedDoubleValue - defaultDoubleValue) < 1.0E-9)
                      || !(Math.abs(sourceDoubleValue) >= Math.abs(targetDoubleValue)))
              ? 0
              : 1;
      if (resultValue != 0) {
