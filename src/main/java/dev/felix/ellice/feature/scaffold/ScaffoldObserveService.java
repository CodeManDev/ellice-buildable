...
public final class ScaffoldObserveService {
  ...
  public ScaffoldObserveService.Phase observe(
      long longValue,
      boolean enabled,
    ...
    if (longValue <= this.timestamp
        || this.timestamp != Long.MIN_VALUE && longValue != this.timestamp + 1L) {
      this.reset();
    }
    ...
    if (enabled) {
      ...
      this.phase2 =
          this.count <= value
              ...
              : (previousEnabled && currentEnabled && nextEnabled
                  ? ScaffoldObserveService.Phase.RETURN
                  : ScaffoldObserveService.Phase.BUILD);
    }
  ...
  public boolean mayJump(
      ...
      double doubleValue,
      ScaffoldMovementProfile scaffoldMovementProfile) {
