...
public record ScaffoldMotionPhaseData(
  ...
  public static ScaffoldMotionPhaseData analyze(
    ...
    if ((!nextFinite || !(nextDoubleValue < 0.0))
        && (nextFinite || Double.isNaN(nextDoubleValue))) {
      double previousDoubleValue = scaffoldPlayerSnapshot.horizontalSpeed();
      ScaffoldMotionPhaseData.MotionPhase motionPhase =
          createMotionPhase(scaffoldPlayerSnapshot, previousDoubleValue);
      double currentX =
          previousDoubleValue >= 1.0E-6
              ? Math.toDegrees(
                      Math.atan2(
      ...
      if (currentFinite) {
        ...
        double targetDoubleValue =
            calculateValue4((previousDoubleValue - 0.0125) / 0.0625, 0.18, 1.0);
        sourceDoubleValue =
            doubleValue + RotationData.yawDelta(doubleValue, currentX) * targetDoubleValue;
      } else {
      ...
      double candidateDoubleValue =
          Double.isFinite(doubleValue)
              ? Math.abs(RotationData.yawDelta(doubleValue, sourceDoubleValue))
              : 0.0;
      double selectedDoubleValue =
          Math.max(Math.abs(outputDoubleValue), Math.abs(resultDoubleValue));
      double defaultDoubleValue =
          Math.min(Math.abs(outputDoubleValue), Math.abs(resultDoubleValue));
      ScaffoldMotionPhaseData.PathPattern pathPattern =
          motionPhase == ScaffoldMotionPhaseData.MotionPhase.STATIONARY
      ...
      RotationVector rotationVector =
          predictedEyeAtClick(
              ...
              placementCandidate,
              currentFinite
                  ? new ScaffoldRemapService.WorldVector(outputDoubleValue, resultDoubleValue)
                  : null);
      ...
      if (resolvedDoubleValue <= 32.0) {
        ...
        viewRelation =
            initialDoubleValue < 0.0
                ? ScaffoldMotionPhaseData.ViewRelation.STRAFE_LEFT
                : ScaffoldMotionPhaseData.ViewRelation.STRAFE_RIGHT;
      } else {
      ...
      ScaffoldMotionPhaseData.PathRelation pathRelation =
          enabled
              ...
              : createPathRelation(
                  currentPlacementCandidate, placementCandidate, pathPattern, viewRelation);
      double nextX = placementCandidate.targetPosition().x() + 0.5;
      double computedDoubleValue = placementCandidate.targetPosition().z() + 0.5;
      double previousX =
          Math.hypot(
              nextX - scaffoldPlayerSnapshot.feetPosition().x(),
              computedDoubleValue - scaffoldPlayerSnapshot.feetPosition().z());
      double cachedDoubleValue = nextFinite ? nextDoubleValue : previousDoubleValue;
      double pendingDoubleValue =
          cachedDoubleValue < 0.0125 ? 64.0 : Math.max(0.0, (previousX - 0.3) / cachedDoubleValue);
      double activeDoubleValue = calculateValue4((5.0 - pendingDoubleValue) / 4.5, 0.0, 1.0);
      double fallbackDoubleValue =
          previousDoubleValue < 0.0125 ? 0.0 : 0.055 + activeDoubleValue * 0.035;
      ...
      if (viewRelation == ScaffoldMotionPhaseData.ViewRelation.STRAFE_LEFT
          || viewRelation == ScaffoldMotionPhaseData.ViewRelation.STRAFE_RIGHT) {
      ...
      double temporaryDoubleValue =
          pathPattern == ScaffoldMotionPhaseData.PathPattern.TURNING ? 0.14 : 0.0;
      if (motionPhase == ScaffoldMotionPhaseData.MotionPhase.STATIONARY) {
      ...
      return new ScaffoldMotionPhaseData(
          pathPattern,
          viewRelation,
          pathRelation,
          sourceDoubleValue,
  ...
  private static ScaffoldMotionPhaseData.MotionPhase createMotionPhase(
      ScaffoldPlayerSnapshot scaffoldPlayerSnapshot, double doubleValue) {
  ...
  private static ScaffoldMotionPhaseData.PathRelation createPathRelation(
    ...
    if (placementCandidate == null) {
      ...
      int currentValue =
          currentY == 0
                  && currentX + value == 1
                  && currentPlacementCandidate.supportPosition().equals(blockCoordinates)
              ? 1
              : 0;
      if (currentValue == 0) {
  ...
  private static double calculateValue(
      ScaffoldMotionPhaseData.ViewRelation viewRelation, double doubleValue) {
    double currentDoubleValue =
        switch (viewRelation) {
          case FORWARD -> 0.11;
          case BACKWARD -> 0.18;
  ...
  static RotationVector predictedEyeAtClick(
  ...
  static RotationVector predictedEyeAtClick(
    ...
    RotationVector currentX =
        new RotationVector(
            placementCandidate.clickedFace().x(),
            placementCandidate.clickedFace().y(),
            placementCandidate.clickedFace().z());
    double doubleValue =
        scaffoldPlayerSnapshot.eyePosition().subtract(placementCandidate.hitPoint()).dot(currentX);
    if (placementCandidate.clickedFace() != ScaffoldMode.UP
        && placementCandidate.clickedFace() != ScaffoldMode.DOWN) {
      double currentDoubleValue = scaffoldPlayerSnapshot.velocity().dot(currentX);
      double nextX =
          worldVector == null
              ? 0.0
              : worldVector.x() * currentX.x() + worldVector.z() * currentX.z();
      if (worldVector != null) {
        if (nextX <= 1.0E-4) {
      ...
      double nextDoubleValue =
          calculateValue4(0.14 + Math.max(0.0, currentDoubleValue) * 0.65, 0.14, 0.27);
      ...
      if (scaffoldPlayerSnapshot.onGround()) {
        double sourceDoubleValue = calculateValue2((doubleValue - 0.025) / 0.019999999999999997);
        double targetDoubleValue =
            calculateValue2((scaffoldPlayerSnapshot.horizontalSpeed() - 0.0125) / 0.0625);
      ...
      RotationVector previousX =
          createRotationData7(
              ...
              currentDoubleValue);
      if (worldVector != null) {
        RotationVector sourceX =
            createRotationData7(
                new RotationVector(worldVector.x(), 0.0, worldVector.z()),
                currentX,
                previousDoubleValue,
                nextX);
        double inputDoubleValue = calculateValue4(currentDoubleValue / 0.075, 0.0, 1.0);
        previousX =
            sourceX.multiply(1.0 - inputDoubleValue).add(previousX.multiply(inputDoubleValue));
      }

      double outputDoubleValue =
          currentDoubleValue > 1.0E-4
              ? Math.min(4.0, previousDoubleValue / currentDoubleValue)
              : 0.0;
      double currentY =
          scaffoldPlayerSnapshot.onGround()
              ? 0.0
              : calculateValue3(scaffoldPlayerSnapshot.velocity().y(), outputDoubleValue);
      return scaffoldPlayerSnapshot
          .eyePosition()
          .add(currentX.multiply(previousDoubleValue))
          .add(previousX)
          .add(new RotationVector(0.0, currentY, 0.0));
    } else {
      return scaffoldPlayerSnapshot.eyePosition();
    }
  }
  ...
  private static RotationVector createRotationData7(
    ...
    RotationVector nextRotationVector =
        rotationVector
            .subtract(currentRotationVector.multiply(currentDoubleValue))
            .multiply(doubleValue / currentDoubleValue);
    double currentX = Math.hypot(nextRotationVector.x(), nextRotationVector.z());
    return currentX > 0.55 ? nextRotationVector.multiply(0.55 / currentX) : nextRotationVector;
  }
  ...
  public enum MotionPhase {
    ...
    private static ScaffoldMotionPhaseData.MotionPhase[] $values() {
      return new ScaffoldMotionPhaseData.MotionPhase[] {
        STATIONARY, GROUNDED, ASCENDING, APEX, DESCENDING
      };
    }
  }
  ...
  public enum PathPattern {
    ...
    private static ScaffoldMotionPhaseData.PathPattern[] $values() {
      return new ScaffoldMotionPhaseData.PathPattern[] {
        STATIONARY, AXIS_ALIGNED, DIAGONAL, TURNING
      };
    }
  }
  ...
  public enum ViewRelation {
    ...
    private static ScaffoldMotionPhaseData.ViewRelation[] $values() {
      return new ScaffoldMotionPhaseData.ViewRelation[] {
        FORWARD, BACKWARD, STRAFE_LEFT, STRAFE_RIGHT, OBLIQUE
      };
    }
  }
}
