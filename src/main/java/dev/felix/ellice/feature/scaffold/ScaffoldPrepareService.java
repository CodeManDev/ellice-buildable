...
public final class ScaffoldPrepareService implements ScaffoldOperationHandler {
  ...
  public void prepare(
                  ...
                  CoreIsInitializedHandler.LOGGER.info(
                      ...
                      new Object[] {
                        ...
                        scaffoldCompatibility.isPlayerSupporting(
                            minecraft, entry.getValue().targetPosition())
                      });
                }
    ...
    ScaffoldPlayerSnapshot scaffoldPlayerSnapshot =
        scaffoldCompatibility.capture(minecraft).orElse(null);
    RotationData currentRotationData = RotationObserveService.current().orElse(null);
    this.value5 =
        currentRotationData != null && this.rotationData3 != null
    ...
    if (scaffoldPlayerSnapshot != null
        ...
        && (scaffoldPlayerSnapshot.onGround()
            || (!scaffoldSettings.onlyOnGround() || this.enabled)
                && scaffoldSettings.jumpScaffold())) {
      if (scaffoldData.activated(
        ...
        RotationData nextRotationData =
            scaffoldSettings.clearObstacles()
        ...
        if (nextRotationData != null) {
          ...
          ScaffoldRemapService.Input nextInput =
              new ScaffoldRemapService.Input(
                  false, false, false, false, currentInput.jump(), currentInput.sneak(), false);
          this.entries2 =
              this.entries.remap(
          ...
          this.enabled8 =
              ScaffoldShouldActivateService.shouldActivate(
                  scaffoldSettings.towerMode(),
                  true,
                  currentInput,
                  scaffoldPlayerSnapshot.horizontalSpeed(),
                  scaffoldSettings.towerMaximumDrift());
          this.scaffoldData64 =
              this.enabled8
                      ...
                      .plan(
                          ...
                          new ScaffoldShouldActivateService.Query() {
                            ...
                            public boolean isReplaceable(BlockCoordinates blockCoordinates) {
                              return scaffoldCompatibility.isReplaceable(
                                  minecraft, blockCoordinates);
                            }

                            @Override
                            public boolean isFaceSturdy(
                                BlockCoordinates blockCoordinates, ScaffoldMode scaffoldMode) {
                              return scaffoldCompatibility.isFaceSturdy(
                                  minecraft, blockCoordinates, scaffoldMode);
                            }
                          },
                          nextDoubleValue,
                          scaffoldSettings.towerFootMargin(),
                          4)
                      .orElse(null)
                  : null;
          int currentValue =
          ...
          if (currentValue != 0) {
            ...
            if (!scaffoldSettings.keepTargetWhenStopped() || this.rotationData == null) {
              this.suspend(
                  minecraft, scaffoldSettings.returnSpeed(), scaffoldSettings.returnSmoothness());
          ...
          if (scaffoldPlayerSnapshot.onGround()
              || !this.enabled
              || !scaffoldSettings.lockBridgeHeight()) {
            ...
            if (scaffoldPlayerSnapshot.onGround()) {
              ...
              for (int currentCount = this.count;
                  currentCount >= this.count - 1;
                  currentCount += -1) {
                if (scaffoldCompatibility.isPlayerSupporting(
                    minecraft, new BlockCoordinates(currentX, currentCount, nextValue))) {
            ...
            if (!scaffoldPlayerSnapshot.onGround()) {
              ...
              label701:
              for (int nextCount = this.count;
                  nextCount >= this.count - scaffoldData.rescueDepth();
                  nextCount += -1) {
                for (int index = nextX - 1; index <= nextX + 1; index++) {
                  for (int currentIndex = previousValue - 1;
                      currentIndex <= previousValue + 1;
                      currentIndex++) {
                    if (scaffoldCompatibility.isPlayerSupporting(
                        minecraft, new BlockCoordinates(index, nextCount, currentIndex))) {
          ...
          this.value4 = 0.0;
          int sourceValue =
              this.timestamp >= this.timestamp3
                      && this.values.attempt(
                          this.timestamp,
                          scaffoldData.clickRhythm(),
                          scaffoldSettings.useInterval())
                  ? 1
                  : 0;
          int targetValue =
              this.rotationData != null
                      ...
                      && (!scaffoldData.cameraAim()
                          || ScaffoldValidateBeforeMovementValidator.sameSentRotation(
                              this.rotationData, scaffoldPlayerSnapshot.playerRotation()))
                      && scaffoldCompatibility.canStartUseItem(minecraft)
                      && RotationObserveService.current()
                          .filter(
                              item ->
                                  ScaffoldValidateBeforeMovementValidator.sameSentRotation(
                                      item, this.rotationData))
                          .isPresent()
          ...
          double previousDoubleValue =
              scaffoldPlayerSnapshot.onGround()
                  ? scaffoldCompatibility.groundInputAccelerationEstimate(
                      minecraft, currentInput.forwardAxis(), currentInput.leftAxis(), false)
                  : scaffoldCompatibility.airInputAccelerationEstimate(
                      minecraft, currentInput.forwardAxis(), currentInput.leftAxis());
          double sourceDoubleValue =
              currentHasHorizontalIntent
                  ? ScaffoldPlacementHeadingService.placementHeading(
                      scaffoldPlayerSnapshot, currentWorldVector, previousDoubleValue)
                  : this.value;
          double targetDoubleValue =
              this.rotationVanillaGcdService.vanillaGcd(scaffoldPlayerSnapshot.mouseSensitivity());
          if (this.enabled8 && !scaffoldData.cameraAim()) {
            ...
            this.text = scaffoldData.acquisitionProfile();
              ...
              || Math.abs(RotationData.yawDelta(this.value, sourceDoubleValue))
                  > scaffoldData.headingTolerance()
            ...
            int inputValue =
                previousRotationData != null && !currentEnabled && currentDoubleValue == this.value2
                    ? 0
                    : 1;
            this.text = scaffoldData.acquisitionProfile();
            ...
            double inputDoubleValue =
                Math.abs(Math.sin(Math.toRadians(this.value * 2.0))) > 0.99
                    ? targetDoubleValue
                    : 0.0;
            long longValue =
                inputValue != 0
                    ? Math.round(
                        (currentDoubleValue - this.rotationData.pitch()) / targetDoubleValue)
                    : 0L;
            if (scaffoldData.autoPitch()) {
              long currentLongValue =
                  (long)
                      Math.ceil(
                          (scaffoldData.minimumPitch() - this.rotationData.pitch())
                              / targetDoubleValue);
              long nextLongValue =
                  (long)
                      Math.floor(
                          (scaffoldData.maximumPitch() - this.rotationData.pitch())
                              / targetDoubleValue);
                        ...
                        Math.round(
                            RotationData.yawDelta(
                                    ...
                                    this.value
                                        + 180.0
                                        + inputDoubleValue
                                        + scaffoldData.yawOffset(
                                            this.value, scaffoldPlayerSnapshot.feetPosition()))
                                / targetDoubleValue),
                        longValue,
                        scaffoldPlayerSnapshot.mouseSensitivity())
                    : this.rotationData;
          ...
          } else if (currentHasHorizontalIntent
              && Math.abs(RotationData.yawDelta(this.value, sourceDoubleValue)) > 1.0E-6) {
            double outputDoubleValue = RotationData.yawDelta(this.value, sourceDoubleValue);
            this.rotationData2 =
                new RotationData(
                    this.rotationData2.yaw() + outputDoubleValue, this.rotationData2.pitch());
            this.value = sourceDoubleValue;
          }

          RotationData sourceRotationData =
              scaffoldData.cameraAim() && previousRotationData != null
          ...
          double resultDoubleValue =
              this.scaffoldBudgetService.budget(
                  ...
                  this.enabled6
                      ? Math.min(doubleValue, scaffoldSettings.maximumClickSpeed())
                      : doubleValue,
          ...
          int outputValue =
              this.timestamp >= this.timestamp3
                  ...
                  ? 1
                  : 0;
          if (scaffoldSettings.stopWhenEmpty()
              && scaffoldCompatibility.findPlaceableHotbarSlot(minecraft) < 0) {
            ...
            ScaffoldRemapService.Result result =
                this.entries.remap(
                    ...
                    false,
                    true);
            RotationVector previousX =
                scaffoldPlayerSnapshot
                    .feetPosition()
                    .add(
                        new RotationVector(
                            result.serverWorldVector().x() * scaffoldSettings.edgeSneakDistance()
                                + scaffoldPlayerSnapshot.velocity().x()
                                    * scaffoldSettings.sneakLeadTicks(),
                            0.0,
                            result.serverWorldVector().z() * scaffoldSettings.edgeSneakDistance()
            ...
            int resultValue =
                previousCount == 0
                        || "Eagle".equals(scaffoldSettings.sneakMode())
                            && !this.scaffoldSneakService.eagleDue(scaffoldData.eagleInterval())
                    ? 0
                    : 1;
            this.enabled4 =
                this.scaffoldSneakService.sneak(
                    this.timestamp,
                    scaffoldSettings.sneakMode(),
                    (resultValue != 0),
                    "Start".equals(scaffoldSettings.sneakMode())
                        ? !this.enabled6
                        : outputValue == 0,
                    scaffoldSettings.sneakWhileFalling()
                        && !scaffoldPlayerSnapshot.onGround()
                        && scaffoldPlayerSnapshot.velocity().y() < -0.03,
                    scaffoldSettings.sneakOnInputError()
                        && result.angularErrorDegrees() > scaffoldSettings.maximumInputError(),
            ...
            if (!this.enabled8
                ...
                && !this.enabled2
                && candidateValue == 0
                && scaffoldData.diagonalAlignment()
                && !scaffoldData.cameraAim()) {
              this.enabled2 = true;
              double sourceX =
                  scaffoldPlayerSnapshot.feetPosition().x() * Math.signum(currentWorldVector.z())
                      - scaffoldPlayerSnapshot.feetPosition().z()
                          * Math.signum(currentWorldVector.x());
              double candidateDoubleValue = sourceX - Math.floor(sourceX);
              if (scaffoldPlayerSnapshot.onGround()
                  && Math.abs(Math.sin(Math.toRadians(this.value * 2.0))) > 0.99
                  && (candidateDoubleValue < 0.2 || candidateDoubleValue > 0.8)) {
                ScaffoldRemapService.Input targetX =
                    this.entries.steerToward(
                        scaffoldPlayerSnapshot.playerRotation().yaw(),
            ...
            if (Double.isFinite(selectedDoubleValue) && !(selectedDoubleValue < 0.0)) {
              double inputX =
                  scaffoldPlayerSnapshot.feetPosition().x()
                      + scaffoldPlayerSnapshot.velocity().x()
                      + result.serverWorldVector().x() * selectedDoubleValue;
              double defaultDoubleValue =
                  scaffoldPlayerSnapshot.feetPosition().z()
                      + scaffoldPlayerSnapshot.velocity().z()
                      + result.serverWorldVector().z() * selectedDoubleValue;
              RotationVector currentY =
                  new RotationVector(
                      inputX, scaffoldPlayerSnapshot.feetPosition().y(), defaultDoubleValue);
              boolean currentHasContinuousSupport =
                  ScaffoldHasContinuousSupportService.hasContinuousSupport(
                      currentY,
                      currentY,
                      this.count,
                      ...
                      item -> scaffoldCompatibility.isPlayerSupporting(minecraft, item));
              RotationVector outputX =
                  scaffoldPlayerSnapshot
                      .eyePosition()
                      .add(
                          new RotationVector(
                              inputX - scaffoldPlayerSnapshot.feetPosition().x(),
                              0.0,
                              defaultDoubleValue - scaffoldPlayerSnapshot.feetPosition().z()));
              boolean nextEnabled =
                  this.checkCondition(minecraft, scaffoldCompatibility, scaffoldSettings, outputX);
              int selectedValue =
                  "Edge Window".equals(scaffoldData.jumpTiming())
                          && (currentHasContinuousSupport || nextEnabled)
                      ? 0
                      : 1;
              boolean previousEnabled =
                  this.scaffoldSneakService.jump(
                      currentInput.jump(),
              ...
              int defaultValue = Math.abs(Math.sin(Math.toRadians(this.value * 2.0))) > 0.1 ? 1 : 0;
              ...
              int sourceCount =
                  scaffoldSettings.safetyBrake()
                          ...
                          && !PlacementRuleEvaluator.allows(
                              ...
                              this.count,
                              scaffoldSettings.minimumFootOverlap(),
                              ScaffoldCoastTicksService.coastTicks(
              ...
              if (!this.enabled8
                ...
                if (placementCandidate == null
                    ...
                    && scaffoldCompatibility.isReplaceable(
                        minecraft, this.scaffoldData62.targetPosition())
                    && scaffoldCompatibility.isFaceSturdy(
                        minecraft,
                        this.scaffoldData62.supportPosition(),
                        this.scaffoldData62.clickedFace())
                ...
                if (placementCandidate != null
                      ...
                      (item, currentItem) -> {
                        ...
                        return !scaffoldData.acceptsPitch(currentItem.pitch())
                            ...
                            : scaffoldCompatibility.raycastPlacementFromEye(
                                minecraft, item, currentItem, nextDoubleValue, resultX);
                      };
                  ...
                  if (currentResult.isEmpty() && computedValue > 0) {
                    currentResult =
                        this.scaffoldTrajectorySolver.solveNearbyYaw(
                            this.rotationData,
                            resultX,
                            placementCandidate,
                            scaffoldPlayerSnapshot.mouseSensitivity(),
                  ...
                  if (currentResult.isEmpty()
                      && computedValue > 0
                      && (!scaffoldSettings.lockRotation()
                          || scaffoldData.rescueTurns()
                              && (!currentHasHorizontalIntent || nextY != 0))) {
                    currentResult =
                  ...
                  if (currentResult.isPresent()) {
                    RotationData targetRotationData =
                        ((ScaffoldTrajectorySolver.Solution) currentResult.get()).rotation();
                    if (!scaffoldPlayerSnapshot.onGround() || targetRotationData.pitch() < 89.9) {
                      this.rotationData2 =
                          outputValue != 0
                              ? targetRotationData
                              : new RotationData(
                                  this.rotationData2.yaw(), targetRotationData.pitch());
              ...
              if (!this.enabled8
                  ...
                  && scaffoldCompatibility.isPlayerSupporting(
                      ...
                      new BlockCoordinates(
                          (int) Math.floor(scaffoldPlayerSnapshot.feetPosition().x()),
                          this.count,
                          (int) Math.floor(scaffoldPlayerSnapshot.feetPosition().z())))
                  && Math.abs(this.rotationData.pitch() - currentDoubleValue) > targetDoubleValue) {
                RotationData inputRotationData =
                    ScaffoldNextService.applyMouseCounts(
                        this.rotationData,
                        0L,
                        Math.round(
              ...
              if (scaffoldSettings.safetyBrake()
                  && scaffoldPlayerSnapshot.onGround()
                  && !previousEnabled
                  && candidateValue == 0) {
                ScaffoldRemapService.Result nextResult =
                    this.entries.remap(
                        scaffoldPlayerSnapshot.playerRotation().yaw(),
                        this.rotationData.yaw(),
                        currentInput,
                ...
                double resolvedDoubleValue =
                    scaffoldCompatibility.groundInputAccelerationEstimate(
                        minecraft,
                        nextResult.serverInput().forwardAxis(),
                        nextResult.serverInput().leftAxis(),
                        false);
                if (!Double.isFinite(resolvedDoubleValue) || resolvedDoubleValue < 0.0) {
                  resolvedDoubleValue = 0.0;
                }

                RotationVector candidateX =
                    scaffoldPlayerSnapshot
                        .eyePosition()
                        .add(
                            new RotationVector(
                                scaffoldPlayerSnapshot.velocity().x()
                                    + nextResult.serverWorldVector().x() * resolvedDoubleValue,
                                0.0,
                                scaffoldPlayerSnapshot.velocity().z()
                                    + nextResult.serverWorldVector().z() * resolvedDoubleValue));
                sourceCount |=
                    (!this.enabled6
                                || !this.checkCondition(
                                    minecraft, scaffoldCompatibility, scaffoldSettings, candidateX))
                            && !PlacementRuleEvaluator.allows(
                                scaffoldPlayerSnapshot,
              ...
              ScaffoldRemapService.Input sourceInput =
                  new ScaffoldRemapService.Input(
        ...
        this.text2 =
            !scaffoldData.activated(
                    minecraft.options.keyUse.isDown(), minecraft.options.keyShift.isDown())
                ? "Hold "
                    + ("Hold Use".equals(scaffoldData.activation()) ? "use" : "sneak")
                    + " to build"
                : "Manual input";
      }
    } else {
  ...
  private void updateState(Minecraft minecraft, CombatCompatibility combatCompatibility) {
    ...
    PlacementOverrideBus.Override override =
        new PlacementOverrideBus.Override(
                true, this.entries2.serverInput(), this.enabled4, true, true)
    ...
    LocalPhysicsFrameBus.publish(
        new LocalPhysicsFrameBus.Frame(
            this.timestamp, (float) this.rotationData.yaw(), override.localInput(), true));
    minecraft.player.setSprinting(false);
  }
  ...
  private boolean checkCondition(
    ...
    if (this.timestamp + 1L >= this.timestamp2
        ...
        && (scaffoldSettings.tuning().autoSelectBlocks()
            || value == scaffoldCompatibility.selectedHotbarSlot(minecraft))
        && (scaffoldSettings.tuning().cameraAim()
            || scaffoldSettings.tuning().acceptsPitch(this.rotationData.pitch()))
        && !RotationObserveService.current()
            .map(
                item ->
                    RotationData.distance(item, this.rotationData)
                        > scaffoldSettings.maximumClickSpeed())
            .orElse(true)) {
      ScaffoldBlockHit blockHitResult =
          scaffoldCompatibility
              .raycastBlockFromEye(
                  minecraft, this.rotationData, scaffoldSettings.reach(), rotationVector)
              .orElse(null);
      if (blockHitResult != null
        ...
        PlacementCandidate placementCandidate =
            scaffoldCompatibility.placementFromRay(minecraft, blockHitResult).orElse(null);
        return placementCandidate != null
            ...
            && (placementCandidate.targetPosition().equals(blockHitResult.blockPosition())
                || scaffoldCompatibility.isFaceSturdy(
                    minecraft, blockHitResult.blockPosition(), blockHitResult.face()));
      } else {
        return false;
      }
    } else {
  ...
  private boolean checkCondition2(
    ...
    ScaffoldBlockHit blockHitResult =
        scaffoldCompatibility
            .raycastBlockFromEye(
                minecraft, this.rotationData, doubleValue, scaffoldPlayerSnapshot.eyePosition())
            .orElse(null);
    if (blockHitResult != null
        && !blockHitResult.inside()
        && blockHitResult.face() != ScaffoldMode.DOWN
      ...
      int currentY =
          this.enabled8 && this.scaffoldData64 != null
              ? this.scaffoldData64.targetPosition().y()
              : Math.min(
                  this.count,
                  (int) Math.floor(scaffoldPlayerSnapshot.feetPosition().y() - 1.0 + 1.0E-6));
      if (blockCoordinates.y() == currentY
          ...
          && (blockCoordinates.equals(blockHitResult.blockPosition())
              || scaffoldCompatibility.isFaceSturdy(
                  minecraft, blockHitResult.blockPosition(), blockHitResult.face()))) {
        double currentX =
            Math.max(
                blockCoordinates.x() - scaffoldPlayerSnapshot.feetPosition().x(),
                scaffoldPlayerSnapshot.feetPosition().x() - (blockCoordinates.x() + 1));
        double currentDoubleValue =
            Math.max(
                blockCoordinates.z() - scaffoldPlayerSnapshot.feetPosition().z(),
                scaffoldPlayerSnapshot.feetPosition().z() - (blockCoordinates.z() + 1));
        if (!(Math.max(currentX, currentDoubleValue) > scaffoldSettings.tuning().footDistance())
            && !(scaffoldPlayerSnapshot.feetPosition().y() - (blockCoordinates.y() + 1)
                > scaffoldSettings.tuning().rescueDrop())
            && (this.enabled8
                || scaffoldSettings.tuning().cameraAim()
                || scaffoldSettings.tuning().acceptsPitch(this.rotationData.pitch()))) {
          if (!this.enabled8
              || this.scaffoldData64 != null
                  && blockCoordinates.equals(this.scaffoldData64.targetPosition())
                  && blockHitResult.face() == ScaffoldMode.UP) {
            int value = scaffoldCompatibility.findPlaceableHotbarSlot(minecraft);
            if (value >= 0
                && (scaffoldSettings.tuning().autoSelectBlocks()
              ...
              if (++this.count3 > scaffoldSettings.rayDwellTicks()
                  && enabled
                ...
                if (currentSnapshot != null
                    && ScaffoldValidateBeforeMovementValidator.sameSentRotation(
                        currentSnapshot.rotation(), this.rotationData)
                    && !scaffoldCompatibility
                        .raycastPlacementFromEye(
                            minecraft,
                            placementCandidate,
                  ...
                  try {
                    currentEnabled =
                        CompatProfileTracker.place(
                            minecraft, scaffoldCompatibility, value, placementCandidate);
                  } finally {
                  ...
                  if (this.enabled3) {
                    CoreIsInitializedHandler.LOGGER.info(
                        "[Scaffold] use tick={} sequence={} consumed={} target={} face={} eye={}"
                            + " rotation={} previous={}",
                        new Object[] {
                          this.timestamp,
                          this.count2,
                ...
                this.text2 =
                    this.count3 <= scaffoldSettings.rayDwellTicks()
                        ? "Holding aim"
      ...
      this.count3 = 0;
      this.text2 =
          this.value5 > scaffoldSettings.maximumClickSpeed() ? "Settling aim" : "Finding a face";
      return false;
    }
  }
  ...
  public void suspend(Minecraft minecraft, double doubleValue, double currentDoubleValue) {
    if (this.rotationData != null) {
      CombatRotationOverride.start(
          minecraft, CombatOwnsService.Owner.SCAFFOLD, doubleValue, currentDoubleValue);
    }
  ...
  @Override
  public void reset(
      Minecraft minecraft, ScaffoldCompatibility scaffoldCompatibility, boolean currentEnabled) {
    this.release();
    this.enabled = false;
