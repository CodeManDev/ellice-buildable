...
public final class ScaffoldChooseService {
    ...
    public static Choice choose(List<Stack> list, int n, int n2, Policy policy) {
        List<Stack> list2 = list.stream().filter(stack -> (stack.safe() && stack.count() > 0 ? 1 : 0) != 0).filter(stack -> !policy.excluded().contains(stack.item())).filter(stack -> (policy.autoSelect() || stack.slot() == n ? 1 : 0) != 0).filter(stack -> (!policy.autoSelect() || !"Fixed Slot".equals(policy.order()) || stack.slot() == policy.fixedSlot() ? 1 : 0) != 0).filter(stack -> (!policy.preferredOnly() || policy.preferred().contains(stack.item()) ? 1 : 0) != 0).toList();
        ...
        Comparator<Stack> comparator = Comparator.comparingInt(stack -> ScaffoldChooseService.m5lbg7eg741(stack, n, policy));
        comparator = comparator.thenComparingInt(stack -> stack.slot() == n2 ? 0 : 1).thenComparingInt(stack -> "Smallest Stack".equals(policy.order()) ? stack.count() : -stack.count()).thenComparingInt(Stack::slot);
    ...
    public record Policy(boolean autoSelect, String order, int fixedSlot, List<String> preferred, boolean preferredOnly, Set<String> excluded, int reserve) {
        public Policy {
