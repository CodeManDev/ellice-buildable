...
public record ScaffoldBlockHit(BlockCoordinates blockPosition, ScaffoldMode face, RotationVector hitPoint, boolean inside) {
   public ScaffoldBlockHit(BlockCoordinates blockPosition, ScaffoldMode face, RotationVector hitPoint, boolean inside) {
   ...
   public double faceEdgeMargin() {
      ...
      double previousDoubleValue = Math.min(Math.min(currentDoubleValue, 1.0 - currentDoubleValue), Math.min(nextDoubleValue, 1.0 - nextDoubleValue));
   ...
   public static ScaffoldBlockHit from(PlacementCandidate placementCandidate) {
      ...
      return new ScaffoldBlockHit(placementCandidate.supportPosition(), placementCandidate.clickedFace(), placementCandidate.hitPoint(), placementCandidate.inside());
   }
}
