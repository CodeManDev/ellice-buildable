...
public final class CombatAllowsService {
  ...
  public static boolean allows(
    ...
    if (Double.isFinite(nextDoubleValue)
      ...
      for (int index = 0; index <= 4; index++) {
        ...
        double currentX =
            (rotationVector.x() + worldVector.x() * previousDoubleValue) / (1.0 - nextDoubleValue);
        double sourceDoubleValue =
            (rotationVector.z() + worldVector.z() * previousDoubleValue) / (1.0 - nextDoubleValue);
  ...
  private static boolean checkCondition(
      double doubleValue, double currentDoubleValue, CombatAllowsService.Terrain terrain) {
