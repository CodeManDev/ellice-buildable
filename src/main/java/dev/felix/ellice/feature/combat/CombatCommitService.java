...
public final class CombatCommitService {
  ...
  public boolean commit(
      long l, double d, RotationData rotationData, ScaffoldRemapService.Input input) {
    return this.commit(l, d, rotationData, input, false);
  }

  public boolean commit(
      long l, double d, RotationData rotationData, ScaffoldRemapService.Input input, boolean bl) {
    ScaffoldRemapService.Result result = this.entries.remap(d, (float) rotationData.yaw(), input);
    return this.commitCorrected(l, rotationData, result.serverInput(), bl);
  }

  public boolean commitCorrected(
      long l, RotationData rotationData, ScaffoldRemapService.Input input, boolean bl) {
    boolean bl2;
    ...
    LocalPhysicsFrameBus.publish(
        new LocalPhysicsFrameBus.Frame(l, (float) rotationData.yaw(), input, bl2));
    return bl2;
  }
