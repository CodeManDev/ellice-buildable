...
public final class CombatReleaseTracker {
   ...
   public boolean ready(long longValue, long currentLongValue, UUID currentUUID, CombatData combatData, CombatReleaseTracker.Profile currentProfile) {
      ...
      if (!currentUUID.equals(this.uUID) || longValue > this.timestamp + 1L || longValue < this.timestamp) {
   ...
   public void attacked(long longValue, long currentLongValue, CombatData combatData, CombatReleaseTracker.Profile profile) {
      double doubleValue = profile.minCps() + this.random.nextDouble() * (profile.maxCps() - profile.minCps());
   ...
   public record Profile(
      ...
      public Profile(double doubleValue, double currentDoubleValue, boolean enabled, boolean currentEnabled) {
         this(
            doubleValue,
            currentDoubleValue,
            enabled,
      ...
      public int requiredAimTicks() {
         return Math.max(1, (this.smart && !this.comboTiming ? 2 : 1) + Math.max(0, this.extraAimTicks));
      }
   }
}
