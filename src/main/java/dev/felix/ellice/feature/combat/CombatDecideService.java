...
public final class CombatDecideService {
   ...
   public CombatDecideService.Action decide(long longValue, CombatDecideService.Frame frame) {
      ...
      if (frame.holding()) {
         if (value != 0 && frame.target().equals(this.uUID2) && this.timestamp2 != Long.MIN_VALUE) {
            long currentLongValue = Math.max(Math.max(1, frame.minimumHoldTicks()), frame.activationTicks() + 1L);
            return longValue - this.timestamp2 >= currentLongValue && calculateValue(frame) <= 1.0 ? CombatDecideService.Action.RELEASE : CombatDecideService.Action.HOLD;
         } else {
         ...
         long nextLongValue = Math.max(Math.max(1, frame.minimumHoldTicks()), frame.activationTicks() + 1L) + 1L;
         return value != 0
               && frame.available()
   ...
   private static double calculateValue(CombatDecideService.Frame frame) {
      return Float.isFinite(frame.attackStrength()) && Float.isFinite(frame.weaponCooldownTicks()) && !(frame.weaponCooldownTicks() <= 0.0F)
         ? (1.0 - Math.max(0.0F, Math.min(1.0F, frame.attackStrength()))) * frame.weaponCooldownTicks()
         : 0.0;
   }
