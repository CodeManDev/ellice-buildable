...
public final class PearlTrajectorySolver {
  ...
  public static PearlTrajectorySolver.Launch launch(
  ...
  public static List<PearlTrajectorySolver.Solution> solve(
      RotationVector rotationVector,
      RotationVector currentRotationVector,
    ...
    RotationVector previousRotationVector =
        launch(rotationVector, currentRotationVector, new RotationData(0.0, 0.0)).origin();
    double currentDoubleValue = 0.05;
    double nextDoubleValue =
        calculateValue(
            previousRotationVector, currentRotationVector, nextRotationVector, currentDoubleValue);
    ...
    for (double previousDoubleValue = 0.25;
      ...
      if (Math.signum(nextDoubleValue) != Math.signum(sourceDoubleValue)) {
        ...
        arrayList.add(
            new PearlTrajectorySolver.Solution(
                RotationData.lookAt(
                    rotationData7,
                    createRotationData7(
                        previousRotationVector,
  ...
  private static double calculateValue(
      ...
      RotationVector nextRotationVector,
      double doubleValue) {
    return createRotationData7(
                rotationVector, currentRotationVector, nextRotationVector, doubleValue)
            .length()
        - 1.5;
  }
  ...
  private static double calculateValue2(double doubleValue) {
    ...
    return 0.9900000095367432
        * ((1.0 - currentDoubleValue) / 0.009999990463256836
