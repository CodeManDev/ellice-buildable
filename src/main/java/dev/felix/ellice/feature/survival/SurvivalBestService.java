...
public final class SurvivalBestService {
  private static final Comparator<SurvivalBestService.Pick> comparator =
      Comparator.comparingDouble(SurvivalBestService.Pick::saturation)
          .reversed()
    ...
    for (int index = 0; index < items.size(); index++) {
      ...
      if (item != null && item.count() > 0) {
        ...
        if (currentNutrition != null) {
          SurvivalBestService.Pick currentPick =
              new SurvivalBestService.Pick(
                  index,
                  item.id(),
                  item.count(),
                  currentNutrition.food(),
                  currentNutrition.saturation());
