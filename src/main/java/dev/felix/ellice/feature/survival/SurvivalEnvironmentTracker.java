...
public final class SurvivalEnvironmentTracker {
   ...
   public void tick(long longValue, SurvivalEnvironmentTracker.Environment environment, SurvivalEnvironmentTracker.Policy currentPolicy) {
         ...
         if (survivalItemData.manualInput()) {
            ...
            if (this.phase == SurvivalEnvironmentTracker.Phase.USE) {
               if (survivalItemData.food() > this.count3) {
                  ...
                  this.updateState(longValue, environment, survivalItemData, "No eating response from server", currentPolicy);
               }
            } else {
               if (this.phase == SurvivalEnvironmentTracker.Phase.REFILL) {
                  ...
                  if (!this.text2.equals(currentItem.id()) || currentItem.count() <= 0) {
                     this.updateState(longValue, environment, survivalItemData, "Inventory changed; refill stopped", currentPolicy);
                     return;
                  }
               ...
               if (survivalItemData.usingItem()) {
                  ...
                  if (currentResult.isPresent()
                     && (result.isEmpty() || calculateValue((SurvivalBestService.Pick)currentResult.get(), (SurvivalBestService.Pick)result.get()) < 0)) {
                     SurvivalBestService.Pick pick = (SurvivalBestService.Pick)currentResult.get();
                     result = Optional.of(new SurvivalBestService.Pick(9, pick.id(), pick.count(), pick.food(), pick.saturation()));
                  }

                  if (enabled && result.isPresent()) {
                     SurvivalBestService.Pick currentPick = (SurvivalBestService.Pick)result.get();
                     int value = currentPick.index() == 9 ? SurvivalItemData.offhandMenuSlot() : SurvivalItemData.hotbarMenuSlot(currentPick.index());
                     this.count2 = value;
                     ...
                     if (nextResult.isPresent()) {
                        ...
                        if (currentValue >= 0 && environment.swap(survivalItemData, SurvivalItemData.storageMenuSlot(((SurvivalBestService.Pick)nextResult.get()).index()), currentValue)) {
   ...
   private static int calculateValue(SurvivalBestService.Pick pick, SurvivalBestService.Pick currentPick) {
   ...
   private void updateState(long offset, SurvivalEnvironmentTracker.Environment environment, SurvivalItemData survivalItemData, String text, SurvivalEnvironmentTracker.Policy policy) {
   ...
   public record Policy(int foodThreshold, float criticalHearts, boolean refill, int actionMillis, int responseMillis, boolean restoreSlot) {
      public Policy(int foodThreshold, float criticalHearts, boolean refill, int actionMillis, int responseMillis, boolean restoreSlot) {
         if (foodThreshold >= 0
            && foodThreshold <= 19
      ...
      public boolean needsFood(SurvivalItemData survivalItemData) {
         return survivalItemData.food() <= this.foodThreshold
            ? true
            : this.criticalHearts > 0.0F && survivalItemData.food() < 20 && survivalItemData.health() + survivalItemData.absorption() <= this.criticalHearts * 2.0F;
      }
   }
}
