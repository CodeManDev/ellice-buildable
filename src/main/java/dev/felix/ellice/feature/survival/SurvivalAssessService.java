...
public final class SurvivalAssessService {
  ...
  public static SurvivalAssessService.Advice assess(
      SurvivalData survivalData, SurvivalAssessService.Policy currentPolicy) {
    Objects.requireNonNull(survivalData, "snapshot");
    Objects.requireNonNull(currentPolicy, "policy");
    if (!survivalData.alive()) {
      return new SurvivalAssessService.Advice(
          SurvivalAssessService.Threat.NONE, "Waiting for respawn");
    } else if (survivalData.eyeInFluid() && survivalData.air() <= currentPolicy.minAir()) {
      return new SurvivalAssessService.Advice(
          SurvivalAssessService.Threat.DROWNING, "Air low — surface now");
    } else if (survivalData.fireTicks() > 0) {
      return new SurvivalAssessService.Advice(
          SurvivalAssessService.Threat.BURNING, "Burning — extinguish or get to water");
    } else if (!survivalData.onGround()
        && !survivalData.inWater()
        && survivalData.fallDistance() > currentPolicy.maxSafeFall()) {
      return new SurvivalAssessService.Advice(
          SurvivalAssessService.Threat.FALLING, "Falling — place water or land safe");
    } else if (survivalData.effectiveHealth() <= currentPolicy.criticalHearts() * 2.0F) {
      return new SurvivalAssessService.Advice(
          SurvivalAssessService.Threat.CRITICAL_HEALTH, "Health critical — eat to regen or flee");
    } else {
      return survivalData.food() <= currentPolicy.foodThreshold()
          ? new SurvivalAssessService.Advice(
              SurvivalAssessService.Threat.HUNGRY, "Hungry — eat the best stored food")
  ...
  public enum Threat {
    ...
    private static SurvivalAssessService.Threat[] $values() {
      return new SurvivalAssessService.Threat[] {
        NONE, HUNGRY, CRITICAL_HEALTH, BURNING, DROWNING, FALLING
      };
    }
  }
}
