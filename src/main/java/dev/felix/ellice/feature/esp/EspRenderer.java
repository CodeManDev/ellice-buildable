...
public final class EspRenderer implements ComponentOperationHandler {
  private EspLabelData espData2;
  private EspData fkec6a94xp6;
  private int count;
  private Captions renderer;
  ...
  public boolean update(EspLabelData espLabelData, EspData espData, int n, Captions captions) {
    boolean bl =
        !Objects.equals(this.espData2, espLabelData)
            || !Objects.equals(this.fkec6a94xp6, espData)
            || this.count != n
            || !Objects.equals(this.renderer, captions);
    this.espData2 = Objects.requireNonNull(espLabelData);
    this.fkec6a94xp6 = Objects.requireNonNull(espData);
    this.count = n;
    this.renderer = Objects.requireNonNull(captions);
    return bl;
  }

  @Override
  public ComponentKeyService<?> render(ComponentThemeService componentThemeService) {
    EspLabelData espLabelData = this.espData2;
    EspData espData = this.fkec6a94xp6;
    Captions captions = this.renderer;
    int n = this.count;
    ...
    arrayList.add(
        ComponentBoxService.node(
                ...
                EspValueService::new,
                espValueService ->
                    espValueService.value(espLabelData, espData, n).pointerEvents(false),
    ...
    if (captions.name()) {
      arrayList.add(
          ComponentBoxService.panel(
                  ComponentStyleService.style()
                      .width(LayoutOperationHandler.px(captions.nameWidth()))
                      .height(LayoutOperationHandler.px(EspRenderer.nameHeight(f)))
                      .absolute()
                      .padding(f, Float.intBitsToFloat(0x40800000) * f)
                  ...
                  sceneCornerRadiusService ->
                      ((SceneCornerRadiusService)
                              ((SceneCornerRadiusService)
                                  ...
                                  .position(
                                      captions.nameX(),
                                      -EspRenderer.nameHeight(f)
                                          - Float.intBitsToFloat(0x40800000) * f))
                          .backgroundColor(-921363676)
                          .pointerEvents(false),
                  ComponentBoxService.text(
                          espLabelData.name(),
                          sceneTextService ->
                              sceneTextService
                                  .fontSize(Float.intBitsToFloat(1092091904) * f)
                                  .color(-985604)
    ...
    if (captions.distance()) {
      ...
      arrayList.add(
          ComponentBoxService.column(
                  ComponentStyleService.style()
                      .width(LayoutOperationHandler.px(captions.distanceWidth()))
                      .height(LayoutOperationHandler.px(Float.intBitsToFloat(1093664768) * f))
                      .absolute()
                      .inset(
                          LayoutOperationHandler.percent(Float.intBitsToFloat(1120403456)),
                      ...
                      .build(),
                  ComponentBoxService.text(
                          espLabelData.metres() + " m",
                          sceneTextService ->
                              sceneTextService
                                  .fontSize(Float.intBitsToFloat(1091043328) * f)
                                  .color(-3221017)
                                  .style(
                                      TextData.outline(
                                              Float.intBitsToFloat(1059481190) * f, -435153635)
                                          .withShadow(
                                              0.0f,
                                              Float.intBitsToFloat(0x3F000000) * f,
                                              -1342177280))
                                  .maxLines(1)
                                  .textAlign(SceneTextService.TextAlign.CENTER)
                                  .pointerEvents(false))
              ...
              .props(
                  layoutContainerNode ->
                      ((LayoutContainerNode) layoutContainerNode.position(captions.distanceX(), f2))
                          .pointerEvents(false))
              .key("distance"));
    }
        ...
        .props(
            layoutContainerNode -> {
              ...
              if (!espData.animations()) {
                MotionAnimateService.cancel(
                    layoutContainerNode, MotionColorsContainer.Floats.OPACITY);
  ...
  public static float distanceOffset(EspData espData, EspLabelData espLabelData) {
    return espData.armorBar() && espLabelData.armor() > 0.0f
