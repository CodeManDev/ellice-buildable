...
public final class EspValueService extends ScenePctService<EspValueService> {
   ...
   public EspValueService value(EspLabelData espLabelData, EspData currentEspData, int currentValue) {
      ...
      if (nextValue != 0) {
         ...
         if (espLabelData.health() != this.value6) {
            MotionAnimateService.animate(this, HEALTH, espLabelData.health(), SceneEaseHandler.Tween.ease(0.16F));
         ...
         if (espLabelData.absorption() != this.value7) {
            MotionAnimateService.animate(this, ABSORPTION, espLabelData.absorption(), SceneEaseHandler.Tween.ease(0.18F));
         }

         if (espLabelData.armor() != this.value8) {
            MotionAnimateService.animate(this, ARMOR, espLabelData.armor(), SceneEaseHandler.Tween.ease(0.18F));
         }
      }
   ...
   protected void draw(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      if (this.espData2 != null && !(this.cw <= 0.0F) && !(this.ch <= 0.0F)) {
         int y = switch (this.espData.colorMode()) {
            case "Health" -> healthColor(this.value2);
            case "Team" -> this.espData2.teamColor() == 0 ? this.espData.color() : this.espData2.teamColor();
   ...
   private void updateState(CompositorPushPresentationScaleService compositorPushPresentationScale, int value, int currentValue, float nextValue) {
      float previousValue = 0.85F;
      compositorPushPresentationScale.roundedRect(this.cx, this.cy, this.cw, this.ch, previousValue, previousValue, previousValue, previousValue, value, 0.0F, 0.0F, 0, nextValue, currentValue);
   }

   private void updateState2(CompositorPushPresentationScaleService compositorPushPresentationScale, float value, int currentValue) {
      ...
      if (!this.espData.corners()) {
         if (nextEnabled) {
            updateState4(compositorPushPresentationScale, this.cx, this.cy, this.cx + this.cw, this.cy, nextValue, currentValue);
         }

         if (previousEnabled) {
            updateState4(compositorPushPresentationScale, this.cx, this.cy + this.ch, this.cx + this.cw, this.cy + this.ch, nextValue, currentValue);
         }

         if (enabled) {
            updateState4(compositorPushPresentationScale, this.cx, this.cy, this.cx, this.cy + this.ch, nextValue, currentValue);
         }

         if (currentEnabled) {
            updateState4(compositorPushPresentationScale, this.cx + this.cw, this.cy, this.cx + this.cw, this.cy + this.ch, nextValue, currentValue);
         }
      } else {
         if (enabled && nextEnabled) {
            updateState3(compositorPushPresentationScale, this.cx, this.cy, previousValue, sourceValue, nextValue, currentValue);
         }

         if (currentEnabled && nextEnabled) {
            updateState3(compositorPushPresentationScale, this.cx + this.cw, this.cy, -previousValue, sourceValue, nextValue, currentValue);
         }

         if (enabled && previousEnabled) {
            updateState3(compositorPushPresentationScale, this.cx, this.cy + this.ch, previousValue, -sourceValue, nextValue, currentValue);
         }

         if (currentEnabled && previousEnabled) {
            updateState3(compositorPushPresentationScale, this.cx + this.cw, this.cy + this.ch, -previousValue, -sourceValue, nextValue, currentValue);
         }
      }
   }

   private static void updateState3(CompositorPushPresentationScaleService compositorPushPresentationScale, float value, float currentValue, float nextValue, float previousValue, float sourceValue, int targetValue) {
      updateState4(compositorPushPresentationScale, value, currentValue, value + nextValue, currentValue, sourceValue, targetValue);
      updateState4(compositorPushPresentationScale, value, currentValue, value, currentValue + previousValue, sourceValue, targetValue);
   }

   private static void updateState4(CompositorPushPresentationScaleService compositorPushPresentationScale, float value, float currentValue, float nextValue, float previousValue, float sourceValue, int targetValue) {
   ...
   private void updateState5(CompositorPushPresentationScaleService compositorPushPresentationScale) {
      ...
      if (this.espData.healthBar()
         ...
         compositorPushPresentationScale.rect(nextValue, this.cy, 2.0F * value, this.ch, mulAlpha(-1289079744, currentValue));
         if (this.value3 > this.value2 + 0.001F) {
            compositorPushPresentationScale.rect(
               nextValue, this.cy + this.ch * (1.0F - this.value3), 2.0F * value, this.ch * this.value3, mulAlpha(-588788053, currentValue)
            );
         }
      ...
      if (this.espData.armorBar()
         ...
         compositorPushPresentationScale.rect(this.cx, previousValue, this.cw, 1.4F * value, mulAlpha(-1289079744, currentValue));
         compositorPushPresentationScale.rect(this.cx, previousValue, this.cw * this.value5, 1.4F * value, mulAlpha(-5584914, currentValue));
      }
   }
