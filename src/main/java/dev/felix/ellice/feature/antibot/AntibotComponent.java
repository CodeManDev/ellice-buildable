...
final class AntibotComponent {
  ...
  String observe(
    ...
    if (enabled
        && finite(rotationVector)
        && finite(currentRotationVector)
      ...
      this.items.addLast(
          new AntibotComponent.Sample(
              longValue, rotationVector, currentRotationVector, doubleValue));
      ...
      if (this.items.size() >= 24 && longValue - this.items.getFirst().time >= 1200L) {
        ...
        if (!(nextRotationVector.length() < 0.5) && !(nextRotationVector.length() > 8.0)) {
          ...
          for (AntibotComponent.Sample nextSample : this.items) {
            previousRotationVector =
                previousRotationVector.add(nextSample.other.subtract(nextSample.self));
            ...
            if (currentSample != null) {
              ...
              sourceDoubleValue +=
                  Math.abs(RotationData.yawDelta(currentSample.yaw, nextSample.yaw));
              if (currentX > 0.04) {
                RotationVector nextX =
                    new RotationVector(
                        inputRotationVector.x() / currentX,
                        0.0,
                        inputRotationVector.z() / currentX);
                if (targetRotationVector != null) {
                  previousDoubleValue +=
                      Math.toDegrees(
                          Math.acos(
                              Math.max(-1.0, Math.min(1.0, nextX.dot(targetRotationVector)))));
                }
          ...
          for (AntibotComponent.Sample previousSample : this.items) {
            targetDoubleValue =
                Math.max(
                    ...
                    previousSample
                        .other
                        .subtract(previousSample.self)
                        .subtract(previousRotationVector)
                        .length());
            inputDoubleValue =
                Math.max(
                    inputDoubleValue,
                    createRotationData7(previousSample).subtract(sourceRotationVector).length());
          }
  ...
  static boolean finite(RotationVector rotationVector) {
    return rotationVector != null
        && Double.isFinite(rotationVector.x())
        && Double.isFinite(rotationVector.y())
        && Double.isFinite(rotationVector.z());
  }
