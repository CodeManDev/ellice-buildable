...
public final class CapeSearchSelector {
  ...
  public static CapeSearchSelector.Page search(
      CapeSearchSelector.Provider value, String text, int currentValue, String currentText)
      throws IOException {
    String nextText;
    if (value == CapeSearchSelector.Provider.GIPHY) {
      ...
      nextText =
          "https://commons.wikimedia.org/w/api.php?action=query&format=json&formatversion=2&generator=search&gsrnamespace=6&gsrlimit=12&gsroffset="
              + currentValue
              + "&gsrsearch="
  ...
  static CapeSearchSelector.Page parse(
      CapeSearchSelector.Provider value, JsonObject jsonObject, int currentValue)
      throws IOException {
    try {
      ...
      if (value == CapeSearchSelector.Provider.GIPHY) {
        ...
        if (currentJsonObject != null
            && currentValue + arrayList.size() < currentJsonObject.get("total_count").getAsInt()) {
          nextValue = currentValue + arrayList.size();
        }
      } else if (jsonObject.has("query")) {
        ArrayList<JsonObject> currentArrayList = new ArrayList<>();

        for (JsonElement currentJsonElement :
            jsonObject.getAsJsonObject("query").getAsJsonArray("pages")) {
          currentArrayList.add(currentJsonElement.getAsJsonObject());
        }

        currentArrayList.sort(
            Comparator.comparingInt(item -> item.has("index") ? item.get("index").getAsInt() : 0));

        for (JsonObject nextJsonObject : (Iterable<JsonObject>) (Iterable<?>) (currentArrayList)) {
          if (nextJsonObject.has("imageinfo")) {
            JsonObject previousJsonObject =
                nextJsonObject.getAsJsonArray("imageinfo").get(0).getAsJsonObject();
            JsonObject sourceJsonObject = previousJsonObject.getAsJsonObject("extmetadata");
            String text =
                str(nextJsonObject, "title")
                    .replaceFirst("^File:", "")
                    .replaceFirst("(?i)\\.gif$", "");
            String currentText =
                createText(sourceJsonObject, "Artist")
                    + " · "
                    + createText(sourceJsonObject, "LicenseShortName");
            arrayList.add(
                new CapeSearchSelector.Entry(
                    str(nextJsonObject, "pageid"),
                    text,
                    str(previousJsonObject, "url"),
                    previousJsonObject.has("thumburl")
                        ? str(previousJsonObject, "thumburl")
                        : str(previousJsonObject, "url"),
  ...
  private static CapeSearchSelector.Entry createEntry(JsonObject jsonObject) {
    ...
    JsonObject nextJsonObject =
        currentJsonObject.has("downsized")
            ? currentJsonObject.getAsJsonObject("downsized")
            : currentJsonObject.getAsJsonObject("original");
    JsonObject previousJsonObject =
        currentJsonObject.has("fixed_width_still")
            ? currentJsonObject.getAsJsonObject("fixed_width_still")
            : nextJsonObject;
    String text =
        jsonObject.has("username") && !str(jsonObject, "username").isBlank()
            ? "@" + str(jsonObject, "username")
            : "GIPHY";
    return new CapeSearchSelector.Entry(
        str(jsonObject, "id"),
        str(jsonObject, "title"),
        str(nextJsonObject, "url"),
        str(previousJsonObject, "url"),
        str(jsonObject, "url"),
        text,
        CapeSearchSelector.Provider.GIPHY);
  }

  private static String createText(JsonObject jsonObject, String text) {
    return jsonObject != null && jsonObject.has(text)
        ? str(jsonObject.getAsJsonObject(text), "value")
            .replaceAll("<[^>]*>", "")
            .replace("&amp;", "&")
        : "";
  }
  ...
  static String str(JsonObject jsonObject, String text) {
    return jsonObject != null && jsonObject.has(text) && !jsonObject.get(text).isJsonNull()
  ...
  public record Entry(
      ...
      String page,
      String credit,
      CapeSearchSelector.Provider provider) {}
