...
public final class CapePreviewRenderer {
   ...
   public static BufferedImage render(BufferedImage bufferedImage, CapePreviewRenderer.Style style) {
      if (style.minecraftTexture()) {
         ...
         try {
            ...
            currentGraphics2D.fillRect(0, 0, nextBufferedImage.getWidth(), nextBufferedImage.getHeight());
            currentGraphics2D.setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BILINEAR);
            ...
            double doubleValue = style.fit() == CapePreviewRenderer.Fit.FIT ? Math.min(width, height) : Math.max(width, height);
            int currentWidth = style.fit() == CapePreviewRenderer.Fit.STRETCH ? nextBufferedImage.getWidth() : (int)Math.round(bufferedImage.getWidth() * doubleValue);
            int currentHeight = style.fit() == CapePreviewRenderer.Fit.STRETCH ? nextBufferedImage.getHeight() : (int)Math.round(bufferedImage.getHeight() * doubleValue);
            int nextWidth = (nextBufferedImage.getWidth() - currentWidth) / 2;
            int nextHeight = (nextBufferedImage.getHeight() - currentHeight) / 2;
            currentGraphics2D.drawImage(bufferedImage, style.mirror() ? nextWidth + currentWidth : nextWidth, nextHeight, style.mirror() ? -currentWidth : currentWidth, currentHeight, null);
         } finally {
   ...
   public record Style(CapePreviewRenderer.Fit fit, boolean minecraftTexture, boolean mirror, int background, double speed) {
      public Style(CapePreviewRenderer.Fit fit, boolean minecraftTexture, boolean mirror, int background, double speed) {
         if (fit == null) {
            fit = CapePreviewRenderer.Fit.FILL;
         }

         speed = Double.isFinite(speed)
            ? Math.clamp(speed, 0.25, 3.0)
            : 1.0;
         this.fit = fit;
      ...
      public static CapePreviewRenderer.Style defaults() {
         return new CapePreviewRenderer.Style(CapePreviewRenderer.Fit.FILL, false, false, -15264476, 1.0);
      }
   }
}
