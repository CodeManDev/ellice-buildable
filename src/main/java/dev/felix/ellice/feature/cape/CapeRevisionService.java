...
public final class CapeRevisionService implements AutoCloseable {
  ...
  public CapeRevisionService(
      CapeRepository currentCapeRepository, Consumer<Runnable> currentConsumer) {
    ...
    this.loaded =
        new CapeRevisionService.Loaded(
            new CapeRepository.Selection(
                "preset", "Aurora", "Aurora", "", "Made for ellice", this.renderer),
            this.preset("Aurora"),
            null);
  }
  ...
  public void choosePreset(String text) {
    ...
    this.updateState(
        new CapeRevisionService.Loaded(
            new CapeRepository.Selection(
                "preset", text, text, "", "Made for ellice", this.renderer),
            this.preset(text),
            null));
  }
        ...
        this.createValue(
            () -> {
              ...
              if (selection == null) {
                ...
                return new CapeRevisionService.Loaded(
                    selection, CapeRecoveredFactory.create(selection.reference()), null);
              } else if ("giphy".equals(selection.kind())) {
                CapeSearchSelector.Entry entry =
                    CapeSearchSelector.giphy(selection.reference(), this.text5);
                byte[] bytes =
                    CapeHttpsService.get(CapeHttpsService.https(entry.media()), 16777216);
                ...
                return new CapeRevisionService.Loaded(
                    selection, CapeCodec.decode(currentBytes), currentBytes);
              }
            },
            item -> {
  ...
  public void importFile(Path path) {
    this.updateState2(
        () -> {
          ...
          int width =
              !path.getFileName().toString().toLowerCase(Locale.ROOT).endsWith(".gif")
                      && capeSize.frame(0).getWidth() == capeSize.frame(0).getHeight() * 2
                  ? 1
                  : 0;
          CapePreviewRenderer.Style style =
              new CapePreviewRenderer.Style(
                  CapePreviewRenderer.Fit.FILL, (width != 0), false, -15264476, 1.0);
          return new CapeRevisionService.Loaded(
              new CapeRepository.Selection(
                  "file", "", path.getFileName().toString(), "", "Local file", style),
              capeSize,
              bytes);
        });
  }
          ...
          if (!host.equals("giphy.com") && !host.equals("www.giphy.com")) {
            ...
            if (value != 0) {
              throw new IOException(
                  "For GIPHY, use its search or paste the GIF page link with your API key"
                      + " configured.");
            }
  ...
  private CapeRevisionService.Loaded createLoaded(CapeSearchSelector.Entry entry)
      throws IOException {
  ...
  public void apply(Runnable runnable) {
    if (this.loaded != null && !this.enabled3 && !this.enabled6) {
      ...
      CapeRepository.Selection currentSelection =
          new CapeRepository.Selection(
              selection.kind(),
              selection.reference(),
              selection.title(),
      ...
      this.createValue(
          () -> {
            ...
            return new CapeRevisionService.Loaded(
                currentSelection, currentLoaded.animation(), currentLoaded.bytes());
          },
          item -> {
            this.enabled6 = false;
            this.loaded2 = item;
  ...
  public void original() {
    if (!this.enabled6) {
      ...
      this.enabled6 = true;
      this.timestamp++;
      CapeRepository.Selection selection =
          new CapeRepository.Selection(
  ...
  public void saveKey(String text) {
    this.createValue(
        ...
        item -> {
          this.text5 = item;
          this.status(
              item.isBlank() ? "GIPHY disconnected." : "GIPHY key saved on this device.", false);
        },
              ...
              if (nextValue == this.count2) {
                ...
                this.status(
                    this.items.isEmpty()
                        ...
                        : "Select a GIF to preview it on your cape.",
                    false);
    ...
    if (this.text3.add(entry.id())) {
      this.createValue2(
          ...
          () -> {
            ...
            try (MemoryCacheImageInputStream memoryCacheImageInputStream =
              ...
              try {
                ...
                return CapeCodec.copy(
                    bufferedImage,
                    Math.max(1, (int) (width * doubleValue)),
                    Math.max(1, (int) (height * doubleValue)));
              } finally {
                imageReader.dispose();
              }
            }
            ...
            for (JsonElement jsonElement : jsonObject.getAsJsonArray("properties")) {
              if (jsonElement.getAsJsonObject().get("name").getAsString().equals("textures")) {
                ...
                JsonObject nextJsonObject =
                    currentJsonObject.getAsJsonObject("textures").getAsJsonObject("SKIN");
                String text =
                    nextJsonObject.get("url").getAsString().replaceFirst("^http:", "https:");
                BufferedImage bufferedImage =
                    ImageIO.read(
                        new ByteArrayInputStream(
                            CapeHttpsService.get(CapeHttpsService.https(text), 1048576)));
                if (bufferedImage != null
                    && bufferedImage.getWidth() == 64
                    && (bufferedImage.getHeight() == 64 || bufferedImage.getHeight() == 32)) {
                  return new CapeRevisionService.Skin(
                      bufferedImage,
                      nextJsonObject.has("metadata")
                          && "slim"
                              .equals(
                                  CapeSearchSelector.str(
                                      nextJsonObject.getAsJsonObject("metadata"), "model")));
                }
              }
            }
  ...
  private <T> Future<?> createValue(
      Callable<T> callable, Consumer<T> consumer, Consumer<String> currentConsumer) {
    return this.createValue2(this.executor, callable, consumer, currentConsumer);
  }

  private <T> Future<?> createValue2(
      ExecutorService executor,
      Callable<T> callable,
      Consumer<T> currentConsumer,
      Consumer<String> nextConsumer) {
    return this.enabled
