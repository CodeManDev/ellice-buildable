...
public final class CapePlayerService {
  ...
  public static float[] player(boolean enabled, boolean currentEnabled) {
    ...
    capePlayer.updateState(-4.0F, 24.0F, -4.0F, 8, 8, 8, 0, 0, 64, currentEnabled ? 32 : 64, 0.0F);
    capePlayer.updateState(
        -4.0F, 12.0F, -2.0F, 8, 12, 4, 16, 16, 64, currentEnabled ? 32 : 64, 0.0F);
    capePlayer.updateState(
    ...
    capePlayer.updateState(
        ...
        0.0F);
    capePlayer.updateState(-4.0F, 0.0F, -2.0F, 4, 12, 4, 0, 16, 64, currentEnabled ? 32 : 64, 0.0F);
    capePlayer.updateState(
        0.0F,
        0.0F,
        -2.0F,
        ...
        0.0F);
    capePlayer.updateState(
        -4.0F, 24.0F, -4.0F, 8, 8, 8, 32, 0, 64, currentEnabled ? 32 : 64, 0.45F);
    if (!currentEnabled) {
      capePlayer.updateState(-4.0F, 12.0F, -2.0F, 8, 12, 4, 16, 32, 64, 64, 0.23F);
      capePlayer.updateState(-4 - value, 12.0F, -2.0F, value, 12, 4, 40, 32, 64, 64, 0.23F);
      capePlayer.updateState(4.0F, 12.0F, -2.0F, value, 12, 4, 48, 48, 64, 64, 0.23F);
      capePlayer.updateState(-4.0F, 0.0F, -2.0F, 4, 12, 4, 0, 32, 64, 64, 0.23F);
  ...
  public static float[] cape(double doubleValue) {
    CapePlayerService capePlayer = new CapePlayerService();
    capePlayer.updateState(-5.0F, 8.0F, -1.0F, 10, 16, 1, 0, 0, 64, 32, 0.0F);
    float[] floats = capePlayer.createFloat();
    double currentDoubleValue = Math.toRadians(10.0 + Math.sin(doubleValue * 1.7) * 2.0);
    ...
    for (byte index = 0; index < floats.length; index += 8) {
      ...
      floats[index + 1] =
          (float)
              (24.0
                  - sourceDoubleValue * nextDoubleValue
                  + targetDoubleValue * previousDoubleValue);
      floats[index + 2] =
          (float)
              (3.0 + sourceDoubleValue * previousDoubleValue + targetDoubleValue * nextDoubleValue);
      double inputDoubleValue = floats[index + 4];
      double outputDoubleValue = -floats[index + 5];
      floats[index + 3] = -floats[index + 3];
      floats[index + 4] =
          (float) (inputDoubleValue * nextDoubleValue + outputDoubleValue * previousDoubleValue);
      floats[index + 5] =
          (float) (-inputDoubleValue * previousDoubleValue + outputDoubleValue * nextDoubleValue);
    }
  ...
  private void updateState(
    ...
    this.updateState2(
        new float[][] {
          {initialValue, computedValue, cachedValue},
          {defaultValue, computedValue, cachedValue},
          {defaultValue, resolvedValue, cachedValue},
          {initialValue, resolvedValue, cachedValue}
        },
        0.0F,
        0.0F,
        -1.0F,
    ...
    this.updateState2(
        new float[][] {
          {defaultValue, computedValue, pendingValue},
          {initialValue, computedValue, pendingValue},
          {initialValue, resolvedValue, pendingValue},
          {defaultValue, resolvedValue, pendingValue}
        },
        0.0F,
        0.0F,
        1.0F,
  ...
  private void updateState2(
    ...
    float[][] currentFloats =
        new float[][] {
          {previousValue / outputValue, sourceValue / resultValue},
          {targetValue / outputValue, sourceValue / resultValue},
          {targetValue / outputValue, inputValue / resultValue},
          {previousValue / outputValue, inputValue / resultValue}
        };

    for (int index : new int[] {0, 1, 2, 0, 2, 3}) {
      for (float candidateValue :
          new float[] {
            floats[index][0],
            floats[index][1],
            floats[index][2],
            ...
            currentValue,
            nextValue,
            currentFloats[index][0],
            currentFloats[index][1]
          }) {
