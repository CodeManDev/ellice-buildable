...
public final class CapeCodec {
    ...
    if (bytes.length != 0 && bytes.length <= 16777216) {
      try (MemoryCacheImageInputStream memoryCacheImageInputStream =
        ...
        try {
          ...
          int value = 0;
          Node node =
              enabled && imageReader.getStreamMetadata() != null
          ...
          if (width >= 1
              ...
              && width <= 4096
              && height <= 4096
              && (long) width * height <= 4194304L) {
            Node nextNode = createNode(node, "GlobalColorTable");
            int currentValue = calculateValue(nextNode, "backgroundColorIndex", -1);
            if (nextNode != null) {
              for (Node previousNode = nextNode.getFirstChild();
                  previousNode != null;
                  previousNode = previousNode.getNextSibling()) {
            ...
            while (!Thread.currentThread().isInterrupted()) {
              label414:
              {
                ...
                if (sourceValue < 0
                    || targetValue < 0
                    || sourceValue + nextValue > width
                    || targetValue + previousValue > height) {
                  throw new IOException("GIF frame lies outside its canvas.");
                }

                boolean currentEnabled =
                    "TRUE".equalsIgnoreCase(createText(inputNode, "transparentColorFlag", "FALSE"));
                int inputValue =
                    currentEnabled
                            && calculateValue(inputNode, "transparentColorIndex", -2)
                                == currentValue
                        ? 0
                        : value;
                ...
                BufferedImage currentBufferedImage =
                    "restoreToPrevious".equals(text) ? copy(bufferedImage, width, height) : null;
                ...
                arrayList.add(
                    copy(
                        bufferedImage,
                        Math.max(1, (int) Math.round(width * doubleValue)),
                        Math.max(1, (int) Math.round(height * doubleValue))));
  ...
  private static Node createNode(Node node, String text) {
    if (node != null) {
      for (Node currentNode = node.getFirstChild();
          currentNode != null;
  ...
  private static String createText(Node node, String text, String currentText) {
    Node currentNode =
        node != null && node.getAttributes() != null
            ? node.getAttributes().getNamedItem(text)
            : null;
