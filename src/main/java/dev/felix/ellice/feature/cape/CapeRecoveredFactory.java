...
public final class CapeRecoveredFactory {
   public static final List<String> NAMES = List.of("Aurora", "Ember", "Ender", "Ocean", "ellice", "Starlight");
   ...
   public static CapeSizeService create(String text) {
      ...
      for (int currentIndex = 0; currentIndex < 40; currentIndex++) {
         ...
         double doubleValue = currentIndex
            / 40.0
            * 3.141592653589793
            * 2.0;
         ...
         for (int nextIndex = 0; nextIndex < 128; nextIndex++) {
            for (int previousIndex = 0; previousIndex < 80; previousIndex++) {
               ...
               double previousDoubleValue = 0.5
                  + 0.5
                     * Math.sin(
                        currentDoubleValue * 5.0
                           + nextDoubleValue * 8.0
                           - doubleValue
                     );
               float value = (float)(
               ...
               int currentValue = Color.HSBtoRGB(
                  value,
                  0.65F,
                  (float)(0.24 + 0.46 * previousDoubleValue)
               );
               ...
               int sourceValue = Math.abs(nextValue - 4.5) + Math.abs(previousValue - 5)
                        < 3.0
                     && Math.abs(nextValue - 4.5) + Math.abs(previousValue - 5)
                        > 1.4
                  ? 1
                  : 0;
               if (sourceValue != 0) {
