...
public final class NotebotCodec {
   ...
   public static boolean supports(Path path) {
      if (path != null && path.getFileName() != null) {
         ...
         return text.endsWith(".mp3") || text.endsWith(".wav") || text.endsWith(".flac") || text.endsWith(".ogg") || text.endsWith(".m4a");
      } else {
         return false;
      }
   }
   ...
   public NotebotTitleService decode(Path path, DoubleConsumer doubleConsumer) throws Exception {
      ...
      if (Files.isRegularFile(currentPath) && supports(currentPath)) {
         ...
         try {
            ...
            while ((frame = fFmpegFrameGrabber.grabSamples()) != null) {
               ...
               if (frame.samples != null && frame.samples.length != 0) {
                  ...
                  int currentValue = doubleValue > 0.0
                     ? (int)Math.min(
                        99.0,
                        longValue * 100.0 / 22050.0 / doubleValue
                     )
                     : 0;
         ...
         if (notebotTitle.notes().isEmpty()) {
            throw new IOException("No clear melody found. Try a piano, instrumental or isolated melody track.");
         }
