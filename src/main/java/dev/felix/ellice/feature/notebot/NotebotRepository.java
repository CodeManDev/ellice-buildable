...
public final class NotebotRepository {
  ...
  public NotebotRepository(
      NotebotAvailableHandler notebotAvailable, LongSupplier currentLongSupplier) {
  ...
  public double displayPosition() {
    if (this.notebotTitleService != null
        ...
        && !this.enabled) {
      double x =
          Math.max(0.0, Math.min(1.0, (this.longSupplier.getAsLong() - this.timestamp3) / 5.0E7));
  ...
  public void seek(double doubleValue) {
    if (this.notebotTitleService != null && Double.isFinite(doubleValue)) {
      this.value =
          Math.max(0L, Math.min(this.notebotTitleService.durationTicks(), Math.round(doubleValue)));
      this.updateState();
      ...
      } else if (this.phase2 == NotebotRepository.Phase.FINISHED
  ...
  public void start(NotebotAvailableHandler.Scan scan, boolean enabled, int currentValue) {
    if (this.notebotTitleService != null) {
      ...
      if (!scan.context().available()) {
        ...
        try {
          this.notebotChannelData =
              NotebotChannelData.create(
                  this.notebotTitleService, scan.blocks(), enabled, currentValue);
  ...
  public void pause() {
    if (this.phase2 == NotebotRepository.Phase.PLAYING
        || this.phase2 == NotebotRepository.Phase.TUNING) {
  ...
  public void stop() {
    this.phase2 =
        this.notebotTitleService == null
            ? NotebotRepository.Phase.EMPTY
            : NotebotRepository.Phase.READY;
    this.value = 0.0;
    this.count2 = 0;
    this.count = 0;
    this.timestamp3 = this.longSupplier.getAsLong();
    this.text =
        this.notebotTitleService == null
            ? "Choose a local audio file to begin."
            : "Stopped. Your track is ready.";
  }
      ...
      if (!this.checkCondition()) {
        ...
        if (this.enabled) {
          ...
          if (this.pendingTune != null) {
            this.pendingTune =
                new NotebotRepository.PendingTune(
                    this.pendingTune.world(),
                    this.pendingTune.block(),
                    this.pendingTune.sentAt() + currentLongValue);
          }
        ...
        if (this.phase2 == NotebotRepository.Phase.TUNING) {
          ...
          if (doubleValue > 8.0) {
            this.pause();
            this.text =
                "Paused after a long frame stall. Resume to continue without a burst of old notes.";
          } else if (doubleValue != 0.0) {
            double currentDoubleValue =
                Math.min(
                    this.notebotTitleService.durationTicks(),
                    this.value + doubleValue * this.value2);
            long nextLongValue = 0L;
            int index = 0;
            int currentIndex = this.count2;

            while (currentIndex < this.notebotChannelData.hits().size()
                && this.notebotChannelData.hits().get(currentIndex).tick() < currentDoubleValue) {
              NotebotChannelData.Hit hit = this.notebotChannelData.hits().get(currentIndex++);
              long previousLongValue = 1L << hit.channel();
              if (index != this.notebotPositionData.length
                  && (nextLongValue & previousLongValue) == 0L) {
                NotebotChannelData.Channel currentChannel =
                    this.notebotChannelData.channels().get(hit.channel());
                Optional result =
                    this.notebotAvailableHandler.inspect(currentChannel.block().position());
                if (result.isEmpty()
                    || !((NotebotPositionData) result.get())
                        .instrument()
                        .equals(currentChannel.block().instrument())
                    || ((NotebotPositionData) result.get()).pitch()
                        != currentChannel.targetPitch()) {
                  this.pause();
    ...
    if (this.checkCondition2(offset)) {
      while (this.count3 < this.notebotChannelData.channels().size()) {
        ...
        if (result.isEmpty()
            || !((NotebotPositionData) result.get())
                .instrument()
                .equals(channel.block().instrument())) {
          this.updateState4(
              "A note block moved, changed instrument or became unreachable. Scan again.");
          return;
        }
        ...
        if (notebotPositionData.pitch() != channel.targetPitch()) {
          ...
          this.pendingTune =
              new NotebotRepository.PendingTune(this.object, notebotPositionData, offset);
          return;
        }
  ...
  private void updateState4(String currentText) {
    this.phase2 = NotebotRepository.Phase.ERROR;
    this.text =
        currentText != null && !currentText.isBlank() ? currentText : "Playback is unavailable.";
  }
