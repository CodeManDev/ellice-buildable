...
public record NotebotChannelData(
    List<Channel> channels, List<Hit> hits, int tuningClicks, int matchedPercent) {
  public NotebotChannelData {
    channels = List.copyOf(channels);
    hits = List.copyOf(hits);
  }

  public static NotebotChannelData create(
      NotebotTitleService notebotTitleService, List<NotebotPositionData> list, boolean bl, int n) {
    int matchedNotes = 0;
    if (n < 1 || n > 3) {
      throw new IllegalArgumentException("Voices must be 1\u20133");
    }
    List<NotebotPositionData> list2 =
        list.stream().filter(NotebotPositionData::melodic).distinct().limit(64L).toList();
    if (list2.isEmpty()) {
      ...
      while (hashSet.size() < list2.size()) {
              ...
              d2 +=
                  noteWeights[j]
                      * Math.max(
                          0.0,
                          dArray[j]
                              - NotebotChannelData.calculateValue(
                                  j, notebotPositionData.baseMidi() + i));
            }
            if (!((d2 -=
                    (double) notebotPositionData.clicksTo(i)
                        * Double.longBitsToDouble(4502148214488346440L))
                > d)) continue;
            d = d2;
        ...
        if (channel2 != null) {
          ...
          for (int i = 0; i < dArray.length; ++i) {
            dArray[i] = Math.min(dArray[i], NotebotChannelData.calculateValue(i, channel2.midi()));
          }
          continue;
        }
        break;
    ...
    for (NotebotTitleService.Note note : arrayList) {
      ...
      for (int i = 1; i < arrayList2.size(); ++i) {
        if (!(NotebotChannelData.calculateValue(note.midi(), ((Channel) arrayList2.get(i)).midi())
            < NotebotChannelData.calculateValue(
                note.midi(), ((Channel) arrayList2.get(n6)).midi()))) continue;
        n6 = i;
      }
    ...
    int n7 =
        arrayList2.stream()
            .mapToInt(channel -> channel.block().clicksTo(channel.targetPitch()))
            .sum();
    return new NotebotChannelData(
        arrayList2,
        linkedHashMap.values().stream()
            .sorted(
                Comparator.comparingInt(Hit::tick)
                    .thenComparing(Comparator.comparingDouble(Hit::strength).reversed()))
            .toList(),
        n7,
        Math.round(
            (float) matchedNotes * Float.intBitsToFloat(1120403456) / (float) arrayList.size()));
  }

  private static double calculateValue(int n, int n2) {
    int n3 = Math.abs(n - n2);
    int n4 = Math.min(n3 % 12, 12 - n3 % 12);
    return (double) n4 * Double.longBitsToDouble(0x4010000000000000L)
        + (double) n3 * Double.longBitsToDouble(4590429028186199163L);
  }
