...
public final class NotebotDecoderTracker implements AutoCloseable {
   ...
   public NotebotDecoderTracker(NotebotAvailableHandler notebotAvailable, Executor executor, NotebotDecoderTracker.Decoder decoder) {
      this(notebotAvailable, executor, decoder, System::nanoTime);
   }

   public NotebotDecoderTracker(NotebotAvailableHandler notebotAvailable, Executor currentExecutor, NotebotDecoderTracker.Decoder currentDecoder, LongSupplier longSupplier) {
      ...
      this.scan2 = new NotebotAvailableHandler.Scan(new NotebotAvailableHandler.Context(null, "Scan the blocks around you.", false), List.of(), 0, 0);
   }
   ...
   public boolean performing() {
      return this.notebotRepository.phase() == NotebotRepository.Phase.PLAYING || this.notebotRepository.phase() == NotebotRepository.Phase.TUNING;
   }
   ...
   public void seek(double doubleValue) {
      if (!this.enabled && this.notebotRepository.song() != null && Double.isFinite(doubleValue)) {
         this.notebotRepository.seek(Math.max(0.0, Math.min(1.0, doubleValue)) * this.notebotRepository.song().durationTicks());
         this.updateState();
      }
   }
   ...
   public void importSong(Path path) {
      ...
      if (this.executor2 == null || this.executor2.isShutdown()) {
         this.executor2 = Executors.newSingleThreadExecutor(item -> {
      ...
      this.future = this.executor2.submit(() -> {
         try {
            NotebotTitleService notebotTitle = this.decoder.decode(path, item -> this.dispatch(() -> {
            ...
            this.dispatch(() -> {
               if (currentCount == this.count && this.enabled) {
                  ...
                  this.text = currentText != null && !currentText.isBlank() ? currentText : "The audio could not be decoded. Try another file.";
                  this.updateState();
               }
            });
         }
   ...
   public void tick() {
      ...
      String text = this.notebotRepository.phase()
         + ":"
