...
public final class AnticheatCodec {
   ...
   public static final String DATA = "https://maninmyvan.github.io/Minecraft-Anticheat-List/anticheats.json";
   ...
   public static List<AnticheatCodec.Entry> parse(String text) {
      ...
      if (!jsonArray.isEmpty() && jsonArray.size() <= 2000) {
         ...
         for (JsonElement jsonElement : jsonArray) {
            if (jsonElement.isJsonObject()) {
               ...
               if (!currentText.isBlank() && hashSet.add(currentText.toLowerCase(Locale.ROOT))) {
                     ...
                     for (JsonElement previousJsonElement : jsonObject.getAsJsonArray("links")) {
                        if (previousJsonElement.isJsonObject()) {
                           ...
                           try {
                              ...
                              if (isWebLink(uRI)) {
                                 currentArrayList.add(new AnticheatCodec.Link(string(currentJsonObject, "name", uRI.getHost()), uRI));
                              }
                  ...
                  try {
                     currentStatus = AnticheatCodec.Status.valueOf(string(jsonObject, "status", "CHECKING").toUpperCase(Locale.ROOT));
         ...
         arrayList.sort(Comparator.comparing(AnticheatCodec.Entry::name, String.CASE_INSENSITIVE_ORDER));
   ...
   static String string(JsonObject jsonObject, String text, String currentText) {
      ...
      return jsonElement != null && jsonElement.isJsonPrimitive() ? createText(jsonElement.getAsString()) : currentText;
   }
   ...
   public record Entry(
      ...
      public AnticheatCodec.Entry withStatus(AnticheatCodec.Status status) {
         return new AnticheatCodec.Entry(this.name, this.platforms, status, this.versions, this.price, this.links, this.github, this.spigot);
      }

      public boolean matches(String text, String currentText, boolean currentMatches) {
         if ((currentText.isEmpty() || this.platforms.contains(currentText)) && (!currentMatches || this.price.equalsIgnoreCase("Free"))) {
            String nextText = (this.name
                  + " "
            ...
            return Arrays.stream(text.strip().toLowerCase(Locale.ROOT).split("\\s+")).allMatch(nextText::contains);
         } else {
            return false;
         }
      }

      public boolean matches(String text, String currentText, boolean currentMatches, AnticheatCodec.Status currentStatus) {
         return (currentStatus == null || this.status == currentStatus) && this.matches(text, currentText, currentMatches);
      }
   }
   ...
   public enum Sort {
      ...
      public Comparator<AnticheatCodec.Entry> comparator(boolean enabled) {
         Comparator<AnticheatCodec.Entry> currentComparator = switch (this) {
            case NAME -> Comparator.comparing(AnticheatCodec.Entry::name, String.CASE_INSENSITIVE_ORDER);
            case STATUS -> Comparator.comparingInt(item -> item.status().ordinal());
            case PLATFORM -> Comparator.comparing(item -> String.join(", ", item.platforms()), String.CASE_INSENSITIVE_ORDER);
            case PRICE -> Comparator.<AnticheatCodec.Entry, String>comparing(item -> createText2(item.price()))
               .thenComparingDouble(item -> calculateValue(item.price()));
         };
         return (enabled ? currentComparator.reversed() : currentComparator).thenComparing(AnticheatCodec.Entry::name, String.CASE_INSENSITIVE_ORDER);
      }
      ...
      private static String createText2(String text) {
         if (text.equalsIgnoreCase("Free")) {
            ...
            return !text.matches(".*\\d.*") ? "2" + text : "1" + text.replaceAll("[0-9.,\\s]", "").toUpperCase(Locale.ROOT);
         }
      }

      private static double calculateValue(String text) {
         Matcher currentMatcher = Pattern.compile("[0-9]+(?:[.,][0-9]+)?").matcher(text);
         return currentMatcher.find() ? Double.parseDouble(currentMatcher.group().replace(',', '.')) : 0.0;
      }
   ...
   public enum Status {
      ...
      private static AnticheatCodec.Status[] $values() {
         return new AnticheatCodec.Status[]{ACTIVE, INACTIVE, DISCONTINUED, OLD, UNAVAILABLE, UNKNOWN, CHECKING};
      }
   }
}
