...
public final class ElliceStringVault {
  ...
  public static Object resolve(
      Lookup caller, String name, Class<?> type, String payload, long keyA, long keyB)
  ...
  public static CallSite link(
      Lookup caller, String name, MethodType type, String payload, long keyA, long keyB, int kind)
      throws Throwable {
    ...
    if (kind == 0) {
      ...
      if (kind != 1
          ...
          || type.parameterType(0) != caller.lookupClass()) {
  ...
  private static String decrypt(Lookup caller, String name, String payload, long keyA, long keyB)
      throws GeneralSecurityException {
    ...
    cipher.init(
        2, new SecretKeySpec(key, "AES"), new GCMParameterSpec(128, Arrays.copyOf(encoded, 12)));
    cipher.updateAAD(
        (caller.lookupClass().getName() + "|" + name).getBytes(StandardCharsets.UTF_8));
  ...
  public static String open(String payload) {
    return payload != null && !payload.isEmpty()
        ? CACHE.computeIfAbsent(payload, ElliceStringVault::decode)
        : "";
  }
