...
public record FriendProfile(UUID id, UUID uuid, String name, String note, boolean favorite, Set<FriendsMode> excluded) {
   public FriendProfile(UUID id, UUID uuid, String name, String note, boolean favorite, Set<FriendsMode> excluded) {
   ...
   public boolean matches(UUID uUID, String text) {
      return this.uuid != null ? this.uuid.equals(uUID) : !this.name.isEmpty() && this.name.equalsIgnoreCase(text);
   }
